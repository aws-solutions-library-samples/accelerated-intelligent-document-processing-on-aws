...
- No document above 100 rows, and no reference corpus **in the A/B itself**: `corefast` only.
  Both were measured on the published build for the guidance-paper refresh the same weekend —
  `scaling` to 3,200 rows at two models, RealKIE and the OCR benchmark at two models, and the
  extraction/classification/confidence model sweeps — see
  [config-guidance.md](../config-guidance.md), whose Appendix A lists every result directory.
...
> changes discussed above. **Two of the directories it cites now hold published-build data:**
> `benchmarks/results/v0.6.8/scalingsimple__extraction-model-sonnet5/` and
> `benchmarks/results/v0.6.8/sizerab*/` were re-run on the published 0.6.8 build for the
> [Configuration Guidance](../config-guidance.md) refresh (one result set per release, per
> `RETENTION.md`); the dev2 numbers quoted below survive in git history at commit `996bfe152`.
> On the published build the 800-row simple-mode document completed in all three repeats
> (guidance paper §3 has the full 8-of-11 picture across suites), and the shipped confidence
> batch ceiling now matches the tuned setting (§7 there).
