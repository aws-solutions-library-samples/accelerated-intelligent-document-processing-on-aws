...
> are recalled from memory. Supporting data: `benchmarks/results/v0.6.4/` and
> `benchmarks/results/v0.6.5/` (`summary.{json,csv}`, `cell_stats.csv`, and a `meta` block
> recording each side's deployed `stack_version`).
...
- The wider v0.6.5 cross-config grid (7 synthetic docs, product-default Sonnet 5) finds the
  same hazard much **worse** at scale: mean recall **0.294**, with an 800-row document
  returning **0 rows**. See [config-guidance §2](../config-guidance.md).
...
Data: `benchmarks/results/v0.6.5-intconf-sonnet46/`. Reproduce:
...
AWS_PROFILE=default python3 benchmarks/harness/aggregate.py --run benchmarks/results/run-<stamp> --out benchmarks/results/v0.6.4
...
AWS_PROFILE=default python3 benchmarks/harness/run_matrix.py --stack <STACK> --suite corefast --native-upload --max-inflight 6
AWS_PROFILE=default python3 benchmarks/harness/aggregate.py --run benchmarks/results/run-<stamp2> --out benchmarks/results/v0.6.5

# 3. compare + charts
python3 benchmarks/harness/aggregate.py --compare benchmarks/results/v0.6.5/summary.json --baseline benchmarks/results/v0.6.4/summary.json
python3 benchmarks/harness/aggregate.py --figures-compare benchmarks/results/v0.6.5/summary.json benchmarks/results/v0.6.4/summary.json --labels v0.6.5 v0.6.4
```
