...
* **Data-Mart Rollup Lambda Role** (`DataMartRollupFunction`, scheduled hourly + daily + reconciler + migration state-machine task-mode invocations):
  ...
  * `glue:GetDatabases`, `glue:GetTables` — required by the rollup's `information_schema.columns` query that discovers `document_sections_*` tables at run time (without these Athena returns 0 rows from information_schema, silently, and the `doc_class` CTE ends up empty)
