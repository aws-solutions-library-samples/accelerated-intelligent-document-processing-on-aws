...
> ⚠️ **Simple + `integrated` is automatically scored in a separate pass for list-bearing classes.** On a class that declares any list field (including every multi-instance class, whose `instances` array is a list), a Simple-mode section ignores `integrated` and runs the standalone Assessment step instead, recording an `info`-level `confidence_integrated_downgraded` issue, `metadata.confidence_mode_effective: separate` and the effective mode in the Processing Flow; a config-time warning names the affected classes. It costs more than the single inference you configured — on the benchmark grid about 2.5× per 100-row document (batched list scoring) — for a result that is complete. Scalar-only classes keep the single-inference saving, and a class with its own `x-aws-idp-extraction-task-prompt` is left in integrated mode (the downgrade works by swapping the prompt). In practice this reserves 1S-TopK for scalar-only classes. The reason it truncates: Benchmarked on a 100-row transaction list, Simple + `integrated` returned **10 of 100 rows** while Simple + `separate` returned 100 of 100 in every repeat, at ~1.5–2× the cost. The mechanism is output volume: the TopK envelope costs several guesses *per cell*, so a list that fits comfortably in a plain extraction can exceed what the model will emit in one response, and it stops emitting rows rather than erroring. Two changes reduce it — list cells are now asked for a **single** guess (`G1/P1` only) instead of four, and the prompt no longer told the model to make each guess *"as short as possible"* (which was also causing it to return shortened *values*) — but the fundamental single-response limit remains. **On list-bearing schemas use `separate`.** See [config-guidance](./benchmarking/config-guidance.md) for the measured comparison.
