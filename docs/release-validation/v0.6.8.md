...
- **Documents above 100 rows, and real (non-synthetic) corpora, in the release A/B itself** —
  the A/B is `corefast` by design. Both are covered by the guidance-paper refresh below
  (`scaling` to 3,200 rows at two models; RealKIE and the OCR benchmark at two models).
...
- ~~**The premium and lightweight model studies.**~~ **Done on 2026-09-12/13, after this record
  was first written.** The `astravalue` and `astracap` suites, the extraction-model sweep
  (Nova Lite, Nova Pro, Sonnet 5, Sonnet 5 `:1m`, Opus 5, GPT-6 Astra), the classification and
  confidence model sweeps, and the full guidance-paper refresh (`core` with both reference
  corpora, `scaling`, `cost`, `intconf`, `advverify`, and every knob suite) were run on the
  reference stack — 3,455 runs, $2,114 — and `docs/benchmarking/config-guidance.md` is
  re-headed to v0.6.8 with a new §5 model-selection section (PR #888). That work found two
  further product defects: [#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894)
  (a multi-instance-wrapped class with a long list cannot finish confidence assessment) and
  [#895](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/895)
  (Nova Lite cannot run the agentic path; every failure retried as transient), plus
  [#892](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/892)
  (the TestRunner rejects any compressed configuration carrying a float — fix in PR #893),
  which had to be hot-patched onto the reference stack before the OCR-benchmark corpus would
  launch. None is a regression; all three are in the guidance paper's backlog.
