...
| `make check-lint-debt` | that debt list staying honest: an excluded file that *gained* a finding, one that is now clean and should be delisted, a dead path, or a bare directory name in any exclusion array. `ruff.toml` excluded five bare directory names until [#975](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/975), which match at any depth, so 442 of 1230 tracked `.py` files were read by neither gate |
