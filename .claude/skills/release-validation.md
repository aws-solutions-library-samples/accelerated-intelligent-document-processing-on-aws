...
|---|---|---|
| **extraction-model sweep** — `coresynth --set extraction_model=<m>` for every value in the matrix's `sweeps.extraction_model` axis (today `nova_lite`, `nova_pro`, `sonnet5`, `sonnet5_1m`, `opus5`, `opus55`, `astra`; Sonnet 4.6 is the control). Read the axis, do not trust this list: it is a convenience copy and the count beside it is what makes a stale copy invisible — the sweep would simply run one model short. A model that cannot run the agentic path (the Nova models — #895) never finishes `coresynth`; run `simplegrid --set extraction_model=<m>` for it instead and say so in the table | 7 × 133 (or 63) | a model table: recall, cell accuracy, typed accuracy, cost/doc, wall, mean confidence and % below 0.9 — same grid, same docs, so rows are comparable |
