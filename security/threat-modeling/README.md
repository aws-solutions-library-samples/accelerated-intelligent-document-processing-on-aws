...
| 3.6 | 2026-09-20 | **AUTH.T07: the config-version scope join moved off the mutable email onto the immutable Cognito `sub`, and PII.T04's existence oracle closed.** A control change, not a re-review — `Last reviewed against version` is unchanged. A scope row is now reached by two **disjoint key spaces** on `UsersTable`: a `SUB#<sub>` pointer item read with `GetItem`, tried first, and the `EmailIndex` GSI, tried second. Neither identifier is ever substituted for the other. This closes AUTH.T07's residual (1): an address that diverges from the row it should match — a case difference, an external-IdP `AttributeMapping` re-applied on every federated sign-in, a self-service change on a mutable pool — used to read as "no row", and "no row" means *unrestricted*, so a divergence silently lifted the restriction. The pointer reuses the table's existing `PK`/`SK` key schema rather than adding a GSI, so no deployment takes a table update and there is no index-backfill window in which a fail-closed lookup would deny every scoped caller; `dynamodb:GetItem` was already granted everywhere alongside `Query`, so no IAM changed either. `user_management` is the writer — `createUser` records the `sub`, `deleteUser` removes the pointer, and the Cognito sync that runs on every Admin `listUsers` back-fills rows written before pointers existed, which is the workflow an administrator must use to apply a scope at all. Rows the back-fill has not reached resolve on email exactly as before, which is the new residual (1) and is **unmeasured against a live deployment**. `scripts/tests/test_scope_lookup_fail_closed.py` gains rule **SCOPE4** (an identifier put to the wrong key space — the `SubIndex` shape this threat is named after) and now counts a `get_item` as a scope read, so the new key space is policed for fail-closed handling rather than invisible to every rule. AUTH.T07's residual (4) is removed: the gate's `PENDING_FIX` map is empty and no consumer is suppressed. **PII.T04** recorded, as a residual, that both `/report/{docId}` routes resolved the redaction record *before* the caller's scope, answering 404 for an absent id and 403 for a present one — an existence oracle over the audit table, one bit per request, to a caller entitled to none of it. Both routes now resolve scope first and answer identically whatever the id names. Status fields and counts are unchanged: 99 threats, and AUTH.T07 stays *Partially Mitigated* on its remaining residuals. |
