...
the status here stays *Open* and the issue number is recorded instead.

| ID | Threat | Fix in flight | Where |
|----|--------|---------------|-------|
| CHAT.T03 | Chat streaming Function URL enforces neither RBAC group nor session ownership | **issue #920** (pending) | [companion-chat.md](feature-threats/companion-chat.md) |
| CHAT.T06 | `/chat/agent` trusts a client-supplied `callerSub` over the SigV4 identity | **issue #920** (pending) | [companion-chat.md](feature-threats/companion-chat.md) |
...
to over-read: **AUTH.T14** (authorization is opt-in per resolver — there is no
...
| 10 | AUTH.T14 | No default deny at the dispatcher; authorization is opt-in per resolver | 6 | Partially Mitigated (#928) |
