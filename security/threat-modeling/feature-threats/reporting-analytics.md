...
| **Residual risk** | `listDocumentVersions` now requires an **assigned group** (`ANY_GROUP`), so the enumerable index of object keys *and* version ids is no longer reachable by a caller in no group — but every group reaches all of it, which is the axis that still matters here. Reading the bytes needs no API operation at all: `CognitoAuthorizedRole` grants `s3:GetObjectVersion` on the Output bucket to every authenticated user irrespective of group, and `getFilePresignedUrl` enforces the caller's scope on the key only for the two per-user-partitioned buckets, of which the Output bucket is not one, so a document key there is **not key-scoped** anyway (see UI.T06). So version history widens the practical impact of UI.T06, and the group floor does not narrow that. Recommend closing UI.T06 and scoping `listDocumentVersions` per document rather than per group — the group floor is the wrong axis for "may this caller see this document's history", and no setting of it supplies one. For redaction workflows, prefer the PII feature's *redact copy and stop* mode (which deletes the original) over manual replace-and-reupload. |
...
| `listDocumentVersions` has no per-document scope | RPT.T08 | **Open** — an assigned group is now required, so a caller in no group is refused; every group still enumerates every document's keys and version ids, which combines with UI.T06 |
