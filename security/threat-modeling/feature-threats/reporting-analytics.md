...
| **Residual risk** | `listDocumentVersions` is available to **any authenticated user**, including one in no Cognito group (it is one of the operations deliberately left `ANY`), so the enumerable index of object keys *and* version ids is the widest-reachable part of this. Reading the bytes needs no API operation: `CognitoAuthorizedRole` grants `s3:GetObjectVersion` on the Output bucket to every authenticated user irrespective of group, and `getFilePresignedUrl` — now `ANY_GROUP`, so at least an assigned group — enforces the caller's scope on the key only for the two per-user-partitioned buckets, of which the Output bucket is not one, so a document key there is **not key-scoped** anyway (see UI.T06). So version history widens the practical impact of UI.T06, and the group floor does not narrow that. Recommend closing UI.T06 and considering whether `listDocumentVersions` should be scoped to users who can see the underlying document. For redaction workflows, prefer the PII feature's *redact copy and stop* mode (which deletes the original) over manual replace-and-reupload. |
