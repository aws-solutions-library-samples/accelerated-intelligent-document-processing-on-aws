...
| **Likelihood** | Medium (requires knowing/deriving a key; several `ANY` list operations return keys, and `s3:ListBucket` on the browser's own role enumerates the document buckets directly) |
| **Severity** | High |
