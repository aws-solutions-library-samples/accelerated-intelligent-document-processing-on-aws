...
| **Likelihood** | Medium (requires knowing/deriving a key; `s3:ListBucket` on the browser's own role enumerates them directly, for any authenticated caller, which is what keeps this at Medium now that no `ANY` API operation returns a key) |
| **Severity** | High |
