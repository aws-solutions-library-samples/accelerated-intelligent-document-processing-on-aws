...
| Chat-with-Document is not restricted by `allowedConfigVersions` on the streaming route | AUTH.T07, CHAT.T03 | **Open** — the processor resolves the caller's scope from verified claims and denies a turn whose scope it cannot evaluate, so the check is live on the REST path. This transport forwards no claims, so the route reports an explicitly null caller identity and the check stands down, logging that it did so once per turn. Keying it to the body-supplied identity instead would not be a control: the caller it would restrict chooses the value. Closes with the same ID-token verification as the rows above (`GAP-07`) |
