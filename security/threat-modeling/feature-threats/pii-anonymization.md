...
| **Mitigations** | Storage is **opt-in**, not default. The table is **feature-owned** (in the feature's own stack, not a host table), with `SSESpecification` encryption and PITR. Access is **RBAC-gated to users whose `allowedConfigVersions` include the *original* document's config version** — the hook deliberately records the original's config version so the report can make that decision. Critically, the scope lookup **fails closed on both halves of the decision**: the lookup key is read from the `email` claim and nothing else, so a claims set carrying no email denies instead of resolving to an identifier that matches no `UsersTable` row (an empty page means "unrestricted", so a substituted identifier would have made an unresolvable caller an unrestricted one); and `_caller_allowed_versions` raises `ScopeLookupError` on a missing `UsersTable`, an absent caller email, or any DynamoDB error, with both the mapping and report routes denying rather than treating a transient failure as "unrestricted". All three `/report*` routes answer the same 403 for that condition, so a failed lookup cannot be mistaken for a truthful empty report on what is an audit view. This is the same contract the host now enforces through `idp_common.config_scope` (AUTH.T07); the rule is restated here rather than imported because this extension ships as its own stack with no `idp_common` layer, and `scripts/tests/test_scope_lookup_fail_closed.py` covers this file for key provenance and failure handling — it has no rule about the *matcher*, so the glob-matching restatement beside it is behaviourally identical today with nothing mechanical holding it there. **Residual:** the two single-record routes resolve the record before the caller's scope, so they answer 404 for an absent `documentId` and 403 for a present one — one bit per probe about whether a redaction record exists, disclosed to a caller not entitled to read it. The mapping is reachable **only** through that gated route: it is stored in DynamoDB rather than S3 specifically so that the host's `getFileContents` resolver — which proxies any Output-bucket key to any authenticated user without config-version scoping (UI.T06) — **cannot** be used to bypass the gate. The Redaction Report default view is metadata-only with no PII. |
