...
| **Mitigations** | Storage is **opt-in**, not default. The table is **feature-owned** (in the feature's own stack, not a host table), with `SSESpecification` encryption and PITR. Access is **RBAC-gated to users whose `allowedConfigVersions` include the *original* document's config version** — the hook deliberately records the original's config version so the report can make that decision. Critically, the scope lookup **fails closed on both halves of the decision**: each lookup key is read from its own claim and nothing else — the immutable Cognito `sub` for the `SUB#<sub>` pointer item, the `email` claim for the `EmailIndex` GSI — so neither is ever substituted for the other and a claims set carrying neither denies instead of resolving to an identifier that matches no `UsersTable` row (an empty page means "unrestricted", so a substituted identifier would have made an unresolvable caller an unrestricted one); and `_caller_allowed_versions` raises `ScopeLookupError` on a missing `UsersTable`, a caller with neither key, a `sub` that no pointer records when there is no email to fall back to, or any DynamoDB error on either key space, with both the mapping and report routes denying rather than treating a transient failure as "unrestricted". Resolving the `sub` first is what keeps the gate applied to a caller whose address has diverged from their row, which on this route would otherwise hand out a re-identification key. All three `/report*` routes answer the same 403 for that condition, so a failed lookup cannot be mistaken for a truthful empty report on what is an audit view. This is the same contract the host now enforces through `idp_common.config_scope` (AUTH.T07); the rule is restated here rather than imported because this extension ships as its own stack with no `idp_common` layer, and `scripts/tests/test_scope_lookup_fail_closed.py` covers this file for key provenance and failure handling — it has no rule about the *matcher*, so the glob-matching restatement beside it is behaviourally identical today with nothing mechanical holding it there. Both single-record routes resolve the caller's **scope before the record**, so a caller whose scope cannot be evaluated receives the same 403 body whatever `documentId` they name; the ordering is the control, and reversing it would restore a one-bit-per-probe oracle over the audit table's contents. **Residual:** the `sub` join applies only to a host `UsersTable` row that records a `sub` — a row the host's back-fill has not reached still resolves on email alone (see AUTH.T07 residual 1) — and none of this has been exercised against a live deployment. The mapping is reachable **only** through that gated route: it is stored in DynamoDB rather than S3 specifically so that the host's `getFileContents` resolver — which proxies any Output-bucket key to any caller holding one of the five groups, with no key-level or config-version scoping (UI.T06) — **cannot** be used to bypass the gate. That resolver's `ANY_GROUP` floor is not what makes this safe: every group satisfies it, and the browser's own Identity Pool role reads the Output bucket directly regardless of group. The Redaction Report default view is metadata-only with no PII. |
