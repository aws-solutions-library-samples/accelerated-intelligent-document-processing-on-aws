...
>
> **Shape of that surface at v0.6.10:** **119** operations reach the single route —
> 40 through `FIELD_FUNCTION_MAP` (an SSM-published field→Lambda map), **69**
> through `FIELD_ALIASES` onto shared resolvers, and **11** served in-process by
> `ddb_direct` (the former AppSync VTL resolvers). Those add to 120 rather than
> 119 because `getCircuitBreakerStatus` is in both the function map and
> `ddb_direct`; the distinct union is 119, matching
...
> Across all 119: 21 require `Admin`; 41 `Admin`+`Author`; 15
...
> 2 are IAM-only (rejected for every Cognito caller); **18 require any assigned
> group** (`ANY_GROUP` — every document read, the chat transcript read, the
...
> `cognito:groups` claim, so neither chat route's group check can be applied to
> callers arriving that way.
>
...
| **Mitigations** | **The dispatcher denies by default.** `http_api_dispatcher/authz.py` checks the caller's groups — read from the verified JWT claim, never the request body — against `api_rbac_manifest.json` before routing, and a field with **no entry** is refused with 403: an operation whose required groups were never declared is closed rather than open, so a forgotten resolver check on a **group-scoped** operation is no longer an open endpoint. ⚠️ **This does not cover every operation.** 8 of the 119 declared operations are declared `ANY` — the dispatcher enforces authentication but not group membership for those — so a forgotten resolver check on one of the 8 is still reachable by any authenticated caller, including a caller in no group at all where self-signup is enabled (`AllowedSignUpEmailDomain`, see AUTH.T04). They are `getMyProfile`, `listChatSessions`, `getLatestPublishedVersion`, the two fine-tuning job reads and the three feature-platform reads, and each entry in `scripts/api_rbac_expectations.yaml` records why `ANY` is the intended policy for **that operation** rather than for the section it sits in. That is the distinction that failed before: three of the operations since tightened carried a reason written for a set rather than for themselves — `listDocumentsDateShard` recorded only as "see `listDocumentsDateHour` — same index-partition enumeration, same decision", `getChatMessages` as "see `listChatSessions` — own-sessions-only ownership scope, same decision", and `getFinetuningJob` (which did **not** need tightening) as "see `listFinetuningJobs` — same platform state, same decision". A reason of that shape is read for whether it holds broadly, not for whether it holds of the one operation carrying it, and the mechanism it points at can differ: `listChatSessions` bounds its read with a DynamoDB key condition on the caller's own partition, while `getChatMessages` reads by session id and layers a separate ownership check that **returns true** when its table is unset. 109 require a group: 91 name a subset of roles and 18 are declared **`ANY_GROUP`** — any group `template.yaml` creates, so what those eighteen **operations** refuse is precisely the self-registered caller whose `cognito:groups` claim is empty. The remaining 2 are `IAM_ONLY`, which is not a group requirement at all: they reject every Cognito caller irrespective of group. The `ANY_GROUP` set is every document read (`getDocument`, `listDocuments`, `listDocumentsByDateRange`, `getDocumentCount`, `listDocumentsDateHour`, `listDocumentsDateShard`, `listDocumentVersions`, `getDocumentVersion`, `compareDocumentVersions`, `getStepFunctionExecution`, `getFileContents`, `getFilePresignedUrl`, `queryKnowledgeBase`), the chat transcript read `getChatMessages`, the processing-breaker badge `getCircuitBreakerStatus`, and `deleteAgentJob`, `deleteChatSession` and `sendChatDocumentMessage`; the sentinel is resolved against the `AWS::Cognito::UserPoolGroup` resources at build time, so a group added to the template joins the set without an edit per operation. Five of the seven added beyond the original eleven return no extracted value themselves; they were measured on a live stack composing into one chain for a caller in no group, from an object key through a run's section and page URIs, the list of extracted attribute names and a model-written description of the page contents, to the workflow execution input and step history. The sixth, `getChatMessages`, sits on top of a control that fails **open**. The seventh, `getCircuitBreakerStatus`, returns one field — `lastError` — that carries `Manual pause by <administrator email>: <reason>` after any manual pause. ⚠️ Three residuals remain, and none is closed by the group floor. **(1)** A group check is not a **per-document** check: for the Input and Output buckets `getFileContents` bounds itself with a bucket allow-list rather than per-document scope (UI.T06), so a caller holding any group may read the bytes of any document, whether or not that document is anything to do with them — there is no per-user axis recording which documents are whose for a key check to consult. ⚠️ **State the residual per axis, not per caller.** "Any document that group can see" is the wrong frame: it is true of an *unscoped* caller, for whom a document's visibility really is a group question, and it was false of a **scoped** one, for whom the entire point of `allowedConfigVersions` or `allowedTestSets` is content their group must NOT see. The object-read path now enforces both of those axes on the key — a scoped Author cannot read another configuration profile's revision bodies from the Configuration bucket, and a scoped Annotator cannot read another test set's source documents or ground truth from the Test Set bucket — so what is open is per-**document** scope on the document buckets, which the product does not define, and not the two per-user scopes it does. **(2)** The floor gates the **REST route only** — `POST /chat/document` on the chat Function URL reaches the same processor as `sendChatDocumentMessage` and forwards no group claim, so a groupless caller is refused on one path and not the other (`GAP-07`, AUTH.T14; declared as `transport_enforces: ANY` so check S9 reports the divergence rather than it being declared away). **(3) The floor gates the API, not the buckets, and the asset is in the buckets.** `CognitoIdentityPoolSetRole` attaches a **single** `authenticated` role with **no `RoleMappings`**, so group membership plays no part in which role a signed-in user assumes, and `CognitoAuthorizedRole` grants `s3:GetObject`, `s3:GetObjectVersion` and `s3:ListBucket` on the Input and Output buckets plus `kms:Decrypt` on the customer-managed key to **every** authenticated user, groupless included. This is the shipped read path, not a bypass: `FileViewer` defaults to signing in the browser, as do the page thumbnails, the page-image viewer and the document export. It was measured, not inferred: a self-registered caller in no group ran `cognito-identity get-id` and `get-credentials-for-identity` with their own ID token, assumed that role, and read 15,953 bytes of extracted output — names, addresses, net pay — with `s3api get-object`. No API operation supplies the key any more, and that is not what bounds the vector: `s3:ListBucket` on the same role enumerates the buckets, so key discovery and the fetch are both self-contained on the S3 side with **no resolver in the path at all**. UI.T06 already names "enumerate keys, then fetch the bytes directly from S3" as its vector; what is recorded here is that neither half needs an API operation, and that "any authenticated user" on that role includes a user in **none** of the five groups. Narrowing it means group-scoped Identity Pool `RoleMappings`, or narrowing the role and routing every read through a resolver — a change to the document-viewing data path, and not made here; the two shapes and the obstacles each runs into (the Annotator's per-object `allowedTestSets` scope has no IAM expression, four UI call sites sign in the browser, and Lambda's 6 MB response cap is why `getFilePresignedUrl` exists alongside `getFileContents`) are set out in [Identity Pool group scoping](../../../docs/planning/identity-pool-group-scoping-plan.md). ⚠️ **The two per-user-partitioned buckets are deliberately not on this role.** The **Configuration bucket** was, with `ListBucket`, which made the whole configuration-revision store — `config_revisions/<profile>/<nnnnnn>.json.gz`, every profile's recorded prompts and few-shot examples — enumerable and readable from the browser by any authenticated user, defeating `allowedConfigVersions` outright and with no API call to detect. It is now off the role, and the Test Set bucket was never on it, so both axes are reachable only through `getFileContents` / `getFilePresignedUrl`, where the key check applies. `scripts/tests/test_browser_s3_grants.py` pins which buckets this role may name, in both directions, so the set cannot grow back without the gate saying so. **An `identity` carried on the event is no longer authoritative** ([#978](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/978), closed): `idp_common.api_adapter.normalize_event` used to pass an event carrying its own `arguments` + `identity` through unchanged, so a principal holding `lambda:InvokeFunction` on the dispatcher directly could state its own `cognito:groups` and have this check made against its own claim — a privilege-escalation amplifier rather than an open door, since API Gateway builds the event itself and a request body cannot add top-level keys to it. The adapter now builds the identity from the authorizer's verified claims, refuses an asserted identity that contradicts them, and refuses one presented with no verified claims at all; the one shape still passed through is an explicitly null `identity`, which is this repository's marker for a service-to-service invocation gated by IAM on the function ARN and asserts no groups. A guard drives the dispatcher end to end with an operation and required groups read from the manifest at test time, and a second one discovers every consumer of the adapter by parsing the tree and fails if one lets the refusal escape as a 5xx instead of a 403. The manifest is generated from `scripts/api_rbac_expectations.yaml` (`scripts/sdlc/generate_api_rbac_manifest.py`), the same file the static scan and the live harness already assert against, so there is no second list to keep in step; it fails closed if unreadable. Resolver-level authorization for every operation remains in place on top of it — the dispatcher enforces the field-level floor, the resolver enforces per-object scope. Plus **automated per-operation authorization testing** — a static scan (including a manifest drift guard) and a live multi-role harness (`make api-test`, see §5) that fail on any missing/incorrect check and track known gaps; security review of new operations. **Not a control:** the `@aws_cognito_user_pools` directives in `schema.graphql` are **not** defense-in-depth here — nothing enforces them at runtime now that AppSync is gone, so they carry no enforcement weight at all. They are retained as the input for the dispatcher's shape validation and as documentation of intent, and they are read by the static scan as one of the things it checks *for drift against the code*. Reading them as a second layer is exactly the mistake AUTH.T08 describes. |
...
| **Description** | Non-admin users can be restricted to specific named configuration versions via an `allowedConfigVersions` list in the UsersTable. The scope is resolved by reading the caller's UsersTable row, and the lookup used to **fail open** — treat the caller as *unrestricted* — on two distinct paths. (a) The **lookup key** was derived from a fallback chain (`email` claim, else `identity.username`, else `cognito:username`, else `sub`) put to a single email-keyed index. A substituted identifier does not find the row by another route — it matches **no** row, and an empty page is indistinguishable from "this user has no restriction". Any verified claims set lacking an `email` claim therefore resolved to *unrestricted*, with **no AWS fault required**. The same arithmetic made **email itself** an unsafe sole join: an address that diverges from the one on the row (a case difference, an external-IdP attribute remap, a self-service change on a mutable pool) also reads as "no row", and so silently lifts the restriction with no attacker action at all. (b) A **failed query** (a missing `dynamodb:Query` grant, a wrong index name, a throttle) was caught and read the same way, so scope enforcement switched itself off precisely on the wiring drift it exists to survive. |
...
| **Mitigations** | **One fail-closed lookup, shared.** `resolve_allowed_config_versions` in `idp_common/config_scope.py` is now the single implementation, and it reads **two disjoint key spaces** on the UsersTable: the immutable Cognito `sub`, via a `SUB#<sub>` pointer item holding the row's `userId`, tried first; and the `email` claim, via the `EmailIndex` GSI, tried second. Each identifier goes only to the key space that indexes it, and neither is a fallback for the other — a `sub` is never put to an email-keyed condition. Preferring the `sub` is what stops a diverged address lifting the restriction. No UsersTable wired / neither key present / any failed read on **either** key space raises `ScopeLookupError`, which every consumer turns into a refusal; so does a caller presenting only a `sub` that no pointer records, because their row may predate the pointer writer and there is no second key to try. The pointer reuses the table's existing `PK`/`SK` key schema rather than adding a GSI, so **no deployment takes a table update** and there is no index-backfill window in which a fail-closed lookup would deny every scoped caller; `dynamodb:GetItem` was already granted to every consumer alongside `Query`. A pointer carries no `email` attribute, so it is absent from `EmailIndex` and cannot be returned by an email query in place of the row. `src/lambda/user_management` is the writer: `createUser` records the `sub` Cognito assigns, `updateUser` writes the pointer when a scope is set and deletes it when one is removed, `deleteUser` deletes it, and the Cognito sync that runs on every Admin `listUsers` records the `sub` and maintains the pointer on each row it can match — which is the workflow an administrator must use to apply a scope at all. That sync matches a Cognito account to its row on the recorded `sub`, then the exact address, then the **case-folded** address: matching on the exact address alone is what writes a *duplicate* row for a user whose address has changed, and a duplicate carries no scope. **A pointer exists only for a row that carries `allowedConfigVersions`**, which is what makes the preferred leg incapable of *widening* a scope: an unscoped row answers "unrestricted" through either key, so a pointer for one would pin that answer ahead of whatever the email join would have found — including ahead of the scoped row a duplicate was created beside. The reader enforces the same invariant from its side, treating a pointer that resolves an unscoped row as stale and falling back to the email join, which can only tighten. An **empty page still means unrestricted**, deliberately — scoping is opt-in per user, and denying there would lock every ordinary user out. The two document-list resolvers and the two artifacts that ship without an `idp_common` layer (`user_management`, the PII-anonymizer feature API) carry the rule verbatim rather than a variant of it. **Both directions are checked, not just the argument.** `reprocessDocument` now also resolves the profile each named document was last processed under and refuses any outside the caller's scope, so the control does not stand down when the caller omits `version`; a document whose profile cannot be established (no tracking row, no `ConfigVersion`, a failed read) is refused, and a batch is refused whole rather than part-processed. **The class is gated, not just the instances:** `scripts/tests/test_scope_lookup_fail_closed.py` walks every module under `nested/api-resolvers/src/lambda`, `src/lambda` and `feature-platform` that reads the UsersTable — recognised by the **table**, not by an index name, since naming the index wrongly is itself one of the ways this failed, and counting a `get_item` as well as a `query`, because the `sub` key space names no index at all — and fails if any derives a key from another claim, puts one key space's identifier to the other's (rule **SCOPE4**, which is the `SubIndex` shape this threat is named after), or handles a lookup failure with anything but a refusal (including a suppressing context manager, a conditional raise that falls through, and a success-shaped payload). It reported 37 rule hits over 15 distinct sites in 8 files before this change. Per-site unit suites assert the three outcomes (no `email` claim denies without querying; a DynamoDB error denies; an empty page stays unrestricted). Static check **S4** of `make api-test-static` now reads only the code an operation can *reach* — its dispatch branch bodies and their call closure — which is what exposed `getDocumentCount` declaring `scope_filtered: true` while resolving no caller at all; that operation now applies the same filters as `listDocuments`. Every scope-enforcing resolver holds `dynamodb:Query`/`GetItem` on the UsersTable and its `/index/*` (a missing grant on `ConfigurationResolverFunction` was found and fixed by the live harness on 2026-07-13); a failed lookup is logged at ERROR, and now denies rather than needing an alarm to be noticed. The **live scope suite in `make api-test`** seeds a scoped user and asserts an out-of-scope version is denied and that Admins are unaffected. **Residual:** (1) the `sub` join closes the email-divergence route only for a row that **records** a `sub`, and the back-fill cannot reach every row. It matches on the recorded `sub`, the exact address, or the case-folded address, so a user whose address was re-mapped or changed *beyond case* before this code ever ran matches none of them: the sync writes them a fresh unscoped row, as it always has, and their original scoped row keeps the email-only exposure indefinitely. (The scoped-rows-only pointer invariant is what stops that becoming *worse* than the pre-existing behaviour — the duplicate gets no pointer, so correcting the address still restores the scope.) The back-fill also runs only when an Admin opens User Management. `docs/rbac.md` carries a one-line `attribute_not_exists(cognitoSub)` scan that lists the rows still in that state, which is the only way to find them. Neither the writer, the back-fill nor the two-key read has been exercised against a live deployment — every statement about them is inferred from the code and from moto-backed suites; (2) nothing asserts at the adapter which *kind* of Cognito token produced the claims, so which token types can reach these operations is a property of the API Gateway authorizer configuration rather than of this code. **Measured on a live deployment 2026-09-20: an access token is refused.** The `COGNITO_USER_POOLS` authorizer on `POST /op/{field}` declares no `authorizationScopes`, and a Cognito **access** token — which decodes cleanly and even carries `cognito:groups` — is rejected **401 at the gateway, before any Lambda runs**, while the ID token from the same session succeeds. So the substituted-identifier path this residual describes is not reachable by presenting an access token to this API. It remains a property of the authorizer rather than of the code, so a future authorizer change could reopen it, and nothing in the tree asserts the token type; (3) the class gate checks **key provenance and failure handling only** — it has no rule about the *matcher*, so a divergent `scope_allows` restatement in an artifact that ships without the shared layer would not be caught (only the two byte-identical vendored copies are held to the letter, by a file-comparison test), and `lib/idp_common_pkg` is outside its scan roots, which leaves `idp_common/testset_scope.py` — a further `EmailIndex` consumer, for the independent `allowedTestSets` axis — unpoliced; its polarity is inverted, so the shapes are not live there; (4) one consumer the gate discovers is **suppressed** rather than enforced: `idp_common/testset_scope.py`, which resolves the independent `allowedTestSets` axis from the same `EmailIndex` with an `or`-chain key and a caught query failure — both shapes this gate exists to forbid. Its polarity is inverted (`assert_can_access_test_set` requires an *explicit* scope, so an unresolvable Annotator is denied), which makes those shapes an availability defect rather than an escalation one, and it has no `sub` join at all, so an Annotator whose address diverges is locked out of their queue. It is carried in `PENDING_FIX` with its two rules named, so any *other* rule firing on it still fails the gate and `test_the_pending_exemption_is_still_needed` deletes the entry the moment those two stop firing. |
...
| **An assigned group required for document content** | The 13 document reads, the chat transcript read, the processing-breaker badge and 3 mutations are declared `ANY_GROUP`, resolved at build time to every `AWS::Cognito::UserPoolGroup` the stack creates, so a self-registered user whose `cognito:groups` claim is empty is refused them. The document reads include the ones returning keys, URIs, attribute names and page descriptions rather than values, because those compose into one chain. Bounds what domain-scoped self-signup (AUTH.T04) reaches. Not a per-document control (UI.T06), not a control on the object bytes at all, and not applied on the chat Function URL (`GAP-07`) | AUTH.T03, AUTH.T04 |
...
| **Config-version scope** | `allowedConfigVersions` enforced in scope-aware resolvers, resolved from the caller's immutable Cognito `sub` via a UsersTable `SUB#<sub>` pointer and, failing that, from the `email` claim via `EmailIndex`; every consumer holds `dynamodb:Query`/`GetItem` on the table and its indexes, and any failure to evaluate the scope denies. See AUTH.T07 for what the `sub` join does **not** yet cover | AUTH.T07 |
...
| **Caller-supplied S3 key bounding** | `getFileContents` / `getFilePresignedUrl` apply both per-user scope axes to the requested **key**, in the one function both fields call before any S3 call: `allowedConfigVersions` against the profile in a `config_revisions/<profile>/` key, `allowedTestSets` against the test set in a `<test_set_id>/` key. Uses the canonical fail-closed lookups, so an unevaluable scope denies; a refusal names neither the object nor the scope subject. The two buckets it governs are also kept off the browser's Identity Pool role, so neither axis has a no-API path around it (`scripts/tests/test_browser_s3_grants.py`). Per-**document** scope on the Input/Output buckets remains open — UI.T06, [#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033) | AUTH.T07, UI.T06 |
