...
> 2 are IAM-only (rejected for every Cognito caller); **11 require any assigned
> group** (`ANY_GROUP` — the document-content reads and three mutations, resolved
> at build time to every group `template.yaml` creates); and **15 require only
> authentication**, which includes a caller in no group at all.
> 13 additionally enforce config-version scope, 4 filter
...
| **An assigned group required for document content** | The 8 document-content reads and 3 mutations are declared `ANY_GROUP`, resolved at build time to every `AWS::Cognito::UserPoolGroup` the stack creates, so a self-registered user whose `cognito:groups` claim is empty is refused them. Bounds what domain-scoped self-signup (AUTH.T04) reaches. Not a per-document control (UI.T06), and not applied on the chat Function URL (`GAP-07`) | AUTH.T03, AUTH.T04 |
