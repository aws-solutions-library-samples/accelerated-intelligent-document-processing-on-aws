...
| **Mitigations** | **In place today:** every resolver does redact — the sanitizer runs on the logged payload, so the exposure is limited to the key spellings the copies omit rather than to all sensitive keys; log groups are retention-bounded, and **106 of the 109** log groups declared across `template.yaml` (56), `nested/api-resolvers/template.yaml` (33) and `patterns/unified/template.yaml` (20) set `KmsKeyId` to the stack's customer-managed key — 54 via `!GetAtt CustomerManagedEncryptionKey.Arn` in the parent template and 52 via `!Ref CustomerManagedEncryptionKeyArn` in the two nested ones. **Not in place:** the remaining **three** do not, and one of them is `HttpApiDispatcherLogGroup` — the log group for the component every UI API request passes through, and therefore the one most likely to contain request payloads. It is encrypted with the CloudWatch Logs service key rather than the stack CMK, so it is outside the key policy and the key's grant-level audit trail. The other two are the `StacknameCheckFunction` and `ReadPreviousIDPPatternFunction` custom-resource groups, which handle no request data; both already carry a `cfn_nag` `W84` suppression and a `checkov:skip=CKV_AWS_158` comment in the template (`template.yaml:1718` and `:2760`), so for those two the decision was recorded even though nothing enforced it. Declaring `KmsKeyId: !Ref CustomerManagedEncryptionKeyArn` on the dispatcher's group is a one-line template change and closes the gap; **pending — do not read as present:** PR #973 makes exactly that change for the dispatcher group. **Do not merge this figure with the retention figure**, which is a different property of the same 109 resources: **108 of the 109** take `RetentionInDays` from `!Ref LogRetentionDays` and one hardcodes `30` — and that one is `HttpApiDispatcherLogGroup` again, the same resource as the encryption exception that matters. An earlier revision of this entry reported the retention count as though it were the encryption count, which is why both are now stated separately and labelled. **Scope:** these counts are for the three core templates only. Across every template in the repository that declares `AWSTemplateFormatVersion` outside `workshop/` there are **158** log groups, of which **27** set no `KmsKeyId` — the three here plus 24 in the `feature-platform/` stacks, the customer-deployable `samples/lambda-hook-inference/` hook stacks and the copy-me scaffolding, tracked as a follow-up in **issue #972**. The core-stack scope is the right one for this entry, because those 24 belong to optional or sample stacks a deployer opts into; the wider numbers are recorded here so the smaller ones are not read as solution-wide. **Pending — do not read as present:** replacing the ten hand-copied denylists with an import of the canonical list, so there is one list to extend, is tracked in **issue #921**. Until that merges, adding a key to the canonical list does **not** protect these ten resolvers. |
