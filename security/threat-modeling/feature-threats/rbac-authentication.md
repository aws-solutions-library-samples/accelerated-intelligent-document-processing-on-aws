...
| **Identity provenance** | `api_adapter.normalize_event` builds `identity` from the authorizer's verified claims only; an invocation that asserts its own `identity` is refused (403) rather than believed, so no caller can choose the groups the checks above read (issue #978). An explicitly null `identity` — the IAM-gated service-to-service marker, which asserts no groups — still passes through | AUTH.T03, AUTH.T16 |
