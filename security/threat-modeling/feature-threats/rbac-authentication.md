...
| **Description** | Non-admin users can be restricted to specific named configuration versions via an `allowedConfigVersions` list in the UsersTable. Consumers resolve this scope by querying the UsersTable `EmailIndex` GSI — email is the only identifier that joins a Cognito principal to a user row, because the row's own `PK`/`SK` are `USER#<userId>` where `userId` is a `uuid4` unrelated to the Cognito `sub`. Most consumers **fail open** (treat the caller as *unrestricted*) whenever the lookup returns nothing or raises. A missing `dynamodb:Query` IAM grant, a wrong table name, a wrong index name, or an identity/claim mismatch therefore silently disables scope enforcement entirely — a scoped user gains read access to every configuration version. The failure mode is silent by construction: a lookup that cannot be *evaluated* is indistinguishable, to the caller, from a user who has no restriction. |
...
| **Severity** | High |
| **Affected Components** | `configuration_resolver`, `get_stepfunction_execution_resolver`, `reprocess_document_resolver`, `sync_bda_idp_resolver`, `list_documents_*_resolver`, `chat_with_document_processor`, `idp_common.api_adapter` (which decides what identifier the scope is resolved *by*), UsersTable GSIs, resolver IAM roles |
| **Mitigations** | Every scope-enforcing consumer must be granted `dynamodb:Query`/`GetItem` on the UsersTable and its `/index/*` (verified — a missing grant on `ConfigurationResolverFunction` was found and fixed by the live harness on 2026-07-13). **Two consumers fail closed rather than open,** and they are the pattern the rest should follow: the pii-anonymizer feature API (`_caller_allowed_versions`) and `chat_with_document_processor` (`_allowed_config_versions_for_event`), where anything that stops the scope being evaluated — no table name wired, no caller email, any DynamoDB error, an event carrying no `identity` key — denies the request. The **live scope suite in `make api-test`** seeds a scoped user and asserts an out-of-scope version is denied and that Admins are unaffected, catching fail-open regressions. Its reach is narrow and worth knowing exactly: `run_scope_suite` makes **two** calls with the scoped token, against `getConfigVersion` and `getConfigVersions`. No other scope-enforcing operation is exercised live, including `sendChatDocumentMessage`, which is `skip_allowed` because an allowed-role call would start a chat turn. The chat path is covered by the processor's unit suite instead — including a check that the index the lookup names is one `template.yaml` declares, and that a writer stores the attribute it is keyed on. |
| **Residual risk** | Four gaps remain. **(1) Five named resolver call sites still fail open** — six source files, because `list_documents_*_resolver` is two. `configuration_resolver`, `get_stepfunction_execution_resolver`, `reprocess_document_resolver`, `sync_bda_idp_resolver` and `list_documents_{gsi,range}_resolver` each catch a lookup failure and continue as unrestricted, so the Description's silent-disable scenario is live for those ops; converting them to the fail-closed contract is outstanding work. `get_stepfunction_execution_resolver`'s own docstring names this threat, which is the pattern to copy — an accepted deviation should say so where the code is. **(2) The scope is resolved by email, and the email can diverge from the row.** `EmailIndex` matching is exact, and nothing normalises case on either side while Cognito re-applies the external-IdP `AttributeMapping` on every federated sign-in; a pool with `ExternalIdPEmailMutable=true` lets a user change their own address, where the verification gate prevents *stealing* another address but not orphaning their own row; and `user_management` deletes the UsersTable row before the Cognito account, warning rather than failing if the second step does not complete. Each route leaves a live account whose row cannot be found, and **an empty result reads as unrestricted in every consumer including the fail-closed ones** — by design, because scoping is opt-in per user, so "no row" cannot be distinguished from "no restriction". This residual is therefore *not* covered by converting a consumer to fail closed: the lookup succeeds and correctly reports nothing. It lifts the scope on the newly-enforced chat-over-REST path as much as anywhere else. [`docs/external-idp.md`](../../../docs/external-idp.md) reaches the same conclusion: the durable fix is to key user scope on the immutable Cognito `sub`, which requires a `sub` attribute on the table and a writer for it. **(3) Nothing constrains which identifier the claims yield.** `idp_common.api_adapter.normalize_event` substitutes another principal name when a claims set carries no `email`. `chat_with_document_processor` and `send_chat_document_message_resolver` refuse to resolve a scope from anything but the `email` claim, and deny when it is absent; applying the same rule at the adapter would cover every operation at once and is outstanding work — it touches all 118 of them, so it belongs in its own change. **(4) Chat-with-Document is not scope-restricted on the streaming transport.** The processor resolves the caller from verified claims, and a Lambda Function URL forwards none — its SigV4 principal is a pool-wide session name that proves *a* signed-in user is calling, not which one. The route therefore reports an explicitly null caller identity, the check stands down and logs that it did so on every turn, and the invocation is gated by IAM alone. Enforcing against the request body's self-asserted identity instead would not be a control, since the caller it would restrict chooses the value. Tracked as GAP-07 in `scripts/api_rbac_expectations.yaml`; it closes when that endpoint verifies a Cognito ID token, which is the same change CHAT.T03/CHAT.T06 need. |
...
| **Config-version scope** | `allowedConfigVersions` enforced in scope-aware resolvers, keyed on the caller's email via the UsersTable `EmailIndex`; resolver IAM roles granted UsersTable GSI Query. Whether a *failed* lookup denies or is read as "unrestricted" differs by consumer — see AUTH.T07's Residual risk | AUTH.T07 |
