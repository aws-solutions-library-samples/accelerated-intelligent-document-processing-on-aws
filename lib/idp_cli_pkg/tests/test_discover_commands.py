...
@pytest.mark.unit
def test_an_id_holding_a_path_separator_writes_outside_the_output_directory(
    writer_console, tmp_path
):
    """DEFECT: a model-supplied `$id` is used as a filename without sanitising.

    `class_name` comes straight from `schema.get("$id")` and is interpolated into
    `output_path / f"{class_name}.json"` (`cli.py:5751-5756`). A `$id` containing
    `/` or `..` therefore escapes the directory the user named: here `"../escaped"`
    lands in the *parent* of `-o`. `$id` is generated by the LLM from document
    content, so the value is attacker-influenced whenever the documents are —
    which for a document-processing accelerator is the normal case. A `$id` of
    `../../.aws/config` would be written to with the CLI user's own permissions.
    """
    out = tmp_path / "schemas"
    escaping = {"$id": "../escaped", "properties": {}}

    _writer()(str(out), [escaping, SCHEMA_B], writer_console, is_batch=True)

    assert (tmp_path / "escaped.json").is_file(), (
        "the schema landed outside the directory named by -o"
    )
    # The directory the user named holds only the well-behaved schema.
    assert [p.name for p in out.iterdir()] == ["W2.json"]
