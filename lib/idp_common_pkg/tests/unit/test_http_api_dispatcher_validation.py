...
def test_required_arg_count_is_stable(spec):
    ...
    visible in the diff (107 fields require a non-null arg as of this spec — the
    ...
    resetTestSetLabels the 101st, generateRuleJson the 102nd, and the five
    Configuration Profile revision ops — listConfigProfileRevisions,
    getConfigProfileRevision, restoreConfigProfileRevision,
    labelConfigProfileRevision, deleteConfigProfileRevision — 103–107, each
    requiring profileName and all but the first also revision)."""
    required = [
        f for f, v in spec["fields"].items() if any(a["non_null"] for a in v["args"])
    ]
    assert len(required) == 107
