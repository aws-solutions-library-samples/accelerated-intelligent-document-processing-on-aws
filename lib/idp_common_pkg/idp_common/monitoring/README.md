...
| `stepfunctions_service` | Step Functions execution analysis: `get_execution_arn_from_document`, `get_execution_data`, `analyze_execution_timeline`, `extract_failure_details`. Both `failed_state` answers come from the one shared rule in [Naming the state that failed](../README.md#-naming-the-state-that-failed), so they agree with each other and name the **terminal** failure — after a retry the execution survived, the first failure is a state that succeeded. `max_events` bounds the returned `states` timeline from the **newest** end; attribution reads the whole history however long it is. `analyze_execution_timeline` names the failure the history holds and does not decide whether the execution failed: a `SUCCEEDED` execution that recovered from a retry legitimately carries a `failed_state`, so check `overall_status` if that distinction matters. |
