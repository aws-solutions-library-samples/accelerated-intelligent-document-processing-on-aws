...
a numeral is to drift from the one the atom parser applies. The one place the two
shapes overlap is `nan`, `inf` and `infinity`, which the construction check reports
as undeclared names: Z3 has no sort for a non-finite value, so a constraint naming
one could not be evaluated either way.
...
Two properties of that helper matter if you write another service like these:
...
threads wide. That is larger than 5 on any container with more than one CPU, which
is why the defect showed up as throttling and retry cost on rule-heavy documents
