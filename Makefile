...
test-packages-cicd: ## CI-safe: run the package/Lambda suites NOT covered by idp_common_pkg test-cicd (all green headless, no AWS)
	...
	cd nested/api-resolvers/src/lambda/configuration_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
	cd nested/api-resolvers/src/lambda/get_stepfunction_execution_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
	cd nested/api-resolvers/src/lambda/list_documents_gsi_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
	cd nested/api-resolvers/src/lambda/list_documents_range_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
	cd nested/api-resolvers/src/lambda/reprocess_document_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
	cd nested/api-resolvers/src/lambda/sync_bda_idp_resolver && $(PYTHON) -m pytest -q -p no:cacheprovider
