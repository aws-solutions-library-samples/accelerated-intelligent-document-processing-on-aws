...
def is_throttling_exception(exception):
    ...
    return exception_name in THROTTLING_EXCEPTIONS or any(
        keyword in exception_message for keyword in THROTTLING_KEYWORDS
    )
...
def handler(event, context):
    ...
    #787: the outer wrapper covers the S3/DynamoDB loads that run before the
    assessment try/except (a botocore read timeout there used to fail the
    document with no retry); ``_handle`` keeps the finer-grained handling of the
    assessment call itself. Transient causes are re-raised under the one name
    AssessmentStep lists; hard errors keep their own name and are not retried.
    """
    try:
        return _handle(event, context)
    except Exception as e:
        raise_if_transient(
            e, where=f"assessment section {(event or {}).get('section_id')}"
        )
        raise
...
def _handle(event, context):
    ...
    logger.info(
        f"Starting assessment processing for event: {json.dumps(event, default=str)}"
    )
    ...
    if config_version:
        logger.info(
            f"Using configuration version {config_version} for document {document.id}"
        )
    else:
    ...
    logger.info(
        f"Processing assessment for document {document.id}, section {section_id}"
    )
    ...
    section_index = next(
        i for i, s in enumerate(document.sections) if s.section_id == section_id
    )
    logger.info(
        f"Section {section_id} is at index {section_index} in the Sections array"
    )
    ...
    if section.extraction_result_uri and section.extraction_result_uri.strip():
        try:
            logger.info(
                f"Checking extraction results for existing assessment: {section.extraction_result_uri}"
            )
            ...
            if extraction_data.get("explainability_info"):
                logger.info(
                    f"Skipping assessment for section {section_id} - extraction results already contain explainability_info"
                )
                ...
                section_document = Document(
                    ...
                    metering=copy.deepcopy(document.metering)
                ...
                try:
                    lambda_metering = calculate_lambda_metering(
                        assessment_context, context, start_time
                    )
                    ...
                    logger.warning(
                        f"Failed to add Lambda metering for assessment skip: {str(e)}"
                    )
                ...
                response = {
                    ...
                    "document": section_document.serialize_document(
                        working_bucket, f"assessment_skip_{section_id}", logger
                    ),
                }

                logger.info(
                    f"Assessment skipped - Response: {json.dumps(response, default=str)}"
                )
                return response
            else:
                logger.info(
                    f"Assessment needed for section {section_id} - no explainability_info found in extraction results"
                )
        except Exception as e:
            logger.warning(
                f"Error checking extraction results for assessment skip: {e}"
            )
    ...
    logger.info(
        f"Updating document status to ASSESSING (lightweight update) for document {document.input_key}"
    )
    try:
        ...
        logger.info(
            f"Status update result: {json.dumps(status_result, default=str)[:500]}"
        )
    except Exception as e:
    ...
    try:
        ...
        has_throttling, throttling_error = check_document_for_throttling_errors(
            updated_document
        )
        if has_throttling:
            logger.error(
                f"Throttling error detected in document errors: {throttling_error}"
            )
            logger.error("Raising ThrottlingException to trigger Step Functions retry")
            raise ThrottlingException(
                f"Throttling detected in document processing: {throttling_error}"
            )
        ...
        if is_throttling_exception(e):
            logger.error(
                f"Throttling exception detected: {type(e).__name__}. This will trigger state machine retry."
            )
    ...
    try:
        ...
        updated_document.metering = merge_metering_data(
            updated_document.metering, lambda_metering
        )
    except Exception as e:
    ...
    try:
        ...
        updated_section = next(
            s for s in updated_document.sections if s.section_id == section_id
        )
        original_input_key = document.input_key
        logger.info(
            f"Persisting assessment results for section {section_id} (index {section_index}) to DynamoDB for document {original_input_key}"
        )
    ...
    result = {
        "document": updated_document.serialize_document(
            working_bucket, f"assessment_{section_id}", logger
        ),
        "section_id": section_id,
    }
