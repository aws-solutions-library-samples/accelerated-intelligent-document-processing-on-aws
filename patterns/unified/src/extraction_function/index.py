...
def load_extraction_checkpoint(
    bucket: str, execution_arn: str, section_id: str
) -> dict | None:
    """
...
def delete_extraction_checkpoint(
    bucket: str, execution_arn: str, section_id: str
) -> None:
...
def handler(event, context):
    ...
    #787: Step Functions retries by the exception's CLASS NAME. A transient failure
    that reaches here as a plain Python exception — a botocore read timeout loading
    the document or the config, a Strands wrapper around one, a reset connection —
    would otherwise fail the document with no retry, while the same condition that
    ran into the Lambda timeout WAS retried. The whole handler is wrapped so the
    S3/DynamoDB loads are covered too; transient causes are re-raised under the one
    name ExtractionStep lists, hard errors keep their own name and are not retried
    (a bad schema cannot succeed on attempt 8).
    """
    try:
        return _handle(event, context)
    except Exception as e:
        raise_if_transient(
            e, where=f"extraction section {(event or {}).get('section_id')}"
        )
        raise
...
def _handle(event, context):
    ...
    if config_version:
        logger.info(
            f"Using configuration version {config_version} for document {full_document.id}"
        )
    else:
    ...
    logger.info(
        f"Updating document status to EXTRACTING (lightweight update) for document {full_document.input_key}"
    )
    try:
    ...
    execution_arn = getattr(full_document, "workflow_execution_arn", "") or event.get(
        "execution_arn", ""
    )
    agentic_enabled = (
    ...
    if agentic_enabled and working_bucket and execution_arn:
        checkpoint = load_extraction_checkpoint(
            working_bucket, execution_arn, section_id
        )
        if checkpoint and checkpoint.get("extraction_data"):
            ...
            if (
                ...
                and "source" in raw_checkpoint
                and "data" in raw_checkpoint
            ):
            ...
            if checkpoint_source == "intermediate_extraction":
                ...
                checkpoint_data = {
                    "_checkpoint_source": "intermediate_extraction",
                    **checkpoint_data,
                }
        ...
        def _checkpoint_cb(extraction_data: dict) -> None:
            save_extraction_checkpoint(
                working_bucket, execution_arn, section_id, extraction_data
            )
    ...
    try:
        ...
        logger.info(
            f"Persisting extraction results for section {section_id} (index {section_index}) to DynamoDB for document {original_input_key}"
        )
