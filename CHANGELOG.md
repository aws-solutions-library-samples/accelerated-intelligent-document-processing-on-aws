...
- **Two more consumers still read `type` straight off a `$ref` property, so a group authored in the UI schema editor read as untyped ([#678](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/678)).** [#638](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/638) fixed the confidence prompt's description formatter; these are the two places that same blind spot survived, found while assessing a follow-up report ([#664](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/664)). **(1)** Classification's `{CLASS_AND_ATTRIBUTE_NAMES_AND_DESCRIPTIONS}` placeholder silently dropped every child of a `$ref` group: with no `type` of its own the group fell to the scalar branch and was emitted as one bare leaf, so `Signatures` appeared where `Signatures.Signature-of-taxpayer1` and its sibling should have — while a byte-equivalent **inline** group walked correctly, making the listing depend on how the class happened to be authored rather than on what it contains. **(2)** The assessment escalation ladder's skip reason reported a `$ref` group as `declared as 'scalar'`, sending whoever read it hunting for a `type: string` that does not exist; and for the hand-authored case of an **array** in `$defs` it was not merely mislabelled but wrong — the field read as untyped, so the ladder abandoned rows it could in fact have recovered. The dereferencer #638 introduced moves to `config/schema_utils.py` (import-light, so classification can use it without pulling in the assessment service's Bedrock/S3 clients) and is now the single implementation behind all three consumers. Dereferencing also makes recursive `$defs` reachable for the first time, so the classification walk guards on the `$ref` targets already entered on the current branch and emits a re-entered property as a leaf rather than recursing forever. Deliberately **not** consolidated: `assessment/threshold_resolver.py`'s own `_deref`, whose dangling-ref and definition-wins-over-sibling semantics are load-bearing for threshold inheritance — switching it would change which threshold wins for a `$ref` property carrying a local `x-aws-idp-confidence-threshold`, which is not this fix's business. 20 new tests, including the recursive, dangling, remote and non-dict cases.
