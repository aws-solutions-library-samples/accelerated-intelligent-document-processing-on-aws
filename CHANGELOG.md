...
- **Test Studio: files dropped directly into an existing test-set folder in S3 no longer stayed invisible.** `getTestSets` short-circuited already-registered folders (`if prefix in existing_test_sets: continue`), so a manual `input/`/`baseline/` add updated neither `fileCount` nor the input↔baseline pairing check. Discovery now reconciles registered folders (allowlisted to `status in {COMPLETED, FAILED}` and `labelJobStatus != RUNNING`, with a 30 s per-row TTL so the UI's 3 s fast poll doesn't repeat two full paginated `list_objects_v2` calls per registered set on every tick). Signature is `input_count:baseline_count:max_last_modified|src=<synthetic\|uploaded>` — the marker check is folded in so a late `.source` write from a synthetic generator still invalidates the fast path. Genuinely-broken pairing on a new folder still flips to `FAILED`; a row already inside the labeling lifecycle (`labelState in {draft, labeled}`) is not hard-failed by transient partial pairing — the draft-labeling harvester writes baselines one document per poll, and `clear_draft_labels` deliberately keeps human labels while dropping drafts, both of which are legitimate partial-pairing states. Live-validated on `idp-dev-qs` — the first reconcile self-corrected two undocumented drifts (`fake-w2` 2000→2005, `realkie-fcc-verified` 75→85). Adds `updatedAt` (an ISO timestamp of the row's last reconcile — currently for API consumers, not yet surfaced in the UI) to the `TestSet` GraphQL type.
