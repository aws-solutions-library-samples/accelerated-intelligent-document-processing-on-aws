...
- ⚠️ **Uninstalling a feature destroyed the active configuration profile's revision history, and a stack that has had one uninstalled has already lost it** ([#1111](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1111)). The feature-platform hook registration Lambda wrote the profile head row with `put_item`, which replaces the entire item, and re-attached seven attributes — none of which was one of the five the configuration library requires be carried forward. So a single successful call deleted `LatestRevision`, `PublishedRevision`, the three `Bda*` fields linking the profile to its Bedrock Data Automation project, and the `_feature_id` stamp, and reported success. No concurrency was needed: it happened every time. Losing the revision counter is the unrecoverable part, because DynamoDB reads an absent attribute as zero for `ADD` — so the next save was numbered **revision 1 again** and overwrote the stored body of the original revision 1, whose S3 key is write-once by construction. The observable symptoms are a version history with a duplicated revision number and no "current" badge, new documents and test runs falling back to the mutable profile head instead of the published revision, the live revision becoming deletable because the guard that refuses it reads the pointer that is gone, and a profile silently unlinked from its BDA project. The write is now a targeted `update_item` that names only the configuration sections it changed, so an attribute it does not name survives and a head field added later needs no edit there, and it is conditional on the row still existing rather than re-creating a profile deleted mid-call. **Action:** the reachable path in a deployed stack is *uninstalling* a feature — both shipped feature stacks ship their hook inside a config preset and call the unregister side only, on CloudFormation Delete, and that write had the identical defect — so if a feature has ever been uninstalled from, or rolled back on, a stack, whichever profile was active at that moment lost its counters, and any revision cut after that overwrote an earlier body. This change does not reconstruct them; the revision bodies and the per-profile revision index that survived are what remain. Registering hooks at install is still the path the feature developer guide documents for a third-party feature, and it had the same defect. Two supporting changes: `DynamoDBClient.update_item` now accepts a `condition_expression`, which it had no way to express at all while `put_item` on the same class did, and a new gate finds every whole-item replacement of a profile head that the same function just read and requires it to derive its preserved set from the library's own list rather than restate it — the two pre-existing sites it also finds are registered, tracked and not changed here. The remaining sites of this shape catalogued in [#1111](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1111) are unchanged.
