...
- **The completeness settings now say what they actually do to a document** ([#1048](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1048), [#1047](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1047)). Two extraction messages and both documentation tiers told the reader that setting `minItems` on a list field makes a shortfall a hard constraint, and that `extraction.validation.fail_action: reject` marks a section failed. Neither is a document outcome: `extraction_list_truncated` is a warning, no processing issue changes a document's status at any severity, and `reject` records `parsing_succeeded: false`, which the Processing Report and the UI's report tab read and the status path does not. What `minItems` buys is an unambiguous, false-positive-free signal that a list is short, which is worth setting — and `extraction.row_shortfall_action: fail` remains the only extraction setting that turns an incompleteness detection into a failed document. Also now documented next to that setting: under `fail`, a list that returns a few percent of its rows fails the section while one that returns **none** of them still completes, because an all-empty list is reported by `extraction_incomplete` instead. That asymmetry is deliberate — a genuinely empty list is a common, legitimate outcome and is indistinguishable from total loss — and closing it needs its own measurement, tracked in [#1046](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1046) along with narrowing the OCR-evidence attribution that keeps `fail` opt-in. See [`extraction.row_shortfall_action`](docs/extraction-and-confidence.md#making-a-materially-incomplete-list-fail-the-section--extractionrow_shortfall_action).
