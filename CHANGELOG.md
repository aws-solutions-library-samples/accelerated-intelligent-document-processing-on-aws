...
- **Lending sample: the Payslip's `EmployeeNumber` and `PayrollNumber` are now extracted reliably.** On `samples/lending_package.pdf` the payslip is an ADP-style earnings statement whose only identifiers sit under a `CO. FILE DEPT. CLOCK NUMBER` header row — there is no "Employee Number" or "Payroll Number" label anywhere on the page — so the shipped descriptions ("unique identifier assigned by the employer", "payroll batch or sequence number") gave the model nothing to map them to: measured on the real page with Sonnet 5, both fields came back correct in 1 of 5 runs, `PayrollNumber` was usually null and `EmployeeNumber` sometimes the CLOCK value. The `lending-package-sample` and `lending-package-sample-govcloud` presets now name the alternative labels and the ADP column for each field; the same probe then returned both correct in 5 of 5 runs (a first variant that named the columns without ranking them fixed `PayrollNumber` but still confused CLOCK for NUMBER 2 of 5 times). Config-only; existing deployments pick it up by re-applying the preset or editing the two descriptions in the Web UI. ([#839](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/839))
