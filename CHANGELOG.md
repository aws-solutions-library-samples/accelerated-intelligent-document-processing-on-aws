...
- **Textract `SIGNATURES` is now on by default, and the shipped Textract cost figures were wrong by ~4×.** With signature detections finally usable end-to-end (see `### Fixed`), the default `ocr.features` becomes `TABLES` + `LAYOUT` + `SIGNATURES`. It adds **no Textract charge** in that combination: AWS bills features as separate usage types and emits none for a feature that is free in combination, which is what our own bill shows — months of `SyncTablesPagesProcessed` with **no** `SyncLayoutPagesProcessed` and **no** `SyncSignaturesPagesProcessed` despite both features being enabled throughout. Signature presence is a common extraction target (loan packages, tax forms, claims, consents), and it was previously opt-in behind a feature flag most users never found — including `lending-package-sample` and `lending-package-sample-govcloud`, whose READMEs both claim "LAYOUT, TABLES, and SIGNATURES features" while their configs set no `ocr.features` at all; they now get all three by inheriting the default. Two caveats are documented rather than assumed: AWS publishes the free-in-combination rule **only** for Layout-with-Tables, not for Signatures, so this rests on observed billing (and because `_feature_combo()` omits Signatures from the metering key, a future AWS change would under-report rather than surface itself); and a detection is **not** proof of a signature — Textract also flags stray pen marks at low confidence, which costs a few prompt tokens on affected pages and gives a model something to over-read, so corpora with no signature fields should drop the entry. **Separately, the shipped cost guidance was materially wrong:** `base-ocr.yaml` and `docs/configuration.md` both stated `TABLES`+`LAYOUT` ≈ **$0.065/page**, "~16×" the `LAYOUT`-only rate — that is the *Tables+Forms* rate. The real billed rate is **$0.015/page** (verified against the account's own invoice: 4,510 pages → $67.65 = exactly $0.015), i.e. ~3.75×, so the documented figure had been overstating the cost of the default configuration by more than 4×. The BDA-vs-Textract comparison built on that number is corrected too. Four new tests pin the default feature set, that it meters as `-Tables` alone, and that `SIGNATURES` is never left in the defaults without a paying feature (on its own it costs ~$0.0035/page).
