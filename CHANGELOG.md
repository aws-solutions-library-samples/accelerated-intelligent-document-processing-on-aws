...
- **`LogLevel` now defaults to `WARN` instead of `INFO`.** At `INFO` the accelerator can write S3 **presigned URLs**, document contents and PII into CloudWatch Logs — the residual risk left open by AppSec finding #9 (*TRACE/DEBUG Logging Enabled*), whose acceptance rested on `INFO` being a safe product default. It is not: the guidance to set `WARN` or `ERROR` for production has been in [well-architected](docs/well-architected.md) since v0.3, so the default disagreed with our own advice, and a deployment that never touched the parameter got the exposure. `patterns/unified/template.yaml` already defaulted to `WARN` and is unchanged — that is the evidence the safer default is operationally acceptable, not a guess. `template.yaml` (root) and `nested/bedrockkb/template.yaml` now default to `WARN` as well; every nested stack is passed `LogLevel` explicitly from the root, so the nested default only applies to a standalone nested deployment. ⚠️ **Two upgrade-visible consequences.** First, `WARN` suppresses the per-stage `INFO` progress lines many operators use to follow a document through the pipeline — set `LogLevel=INFO` (accepting the exposure above) or `DEBUG` while diagnosing, and note that an **existing** stack keeps whatever value it was deployed with: CloudFormation preserves the previous parameter value on update, so this only changes *new* stacks and stacks that explicitly re-specify the parameter. Second, API Gateway **access** logging on the Web UI REST stage is gated on `LogLevel` being `INFO`/`DEBUG` (finding API-GW-006) — request metadata only, never bodies — so the audit trail for requests that fail before the dispatcher (authorizer 401/403s, WAF blocks, CORS/gateway responses) is now **off** by default; a deployment that needs it must set `LogLevel=INFO` deliberately. The two are deliberately left coupled rather than split into a separate parameter, so the choice to emit request metadata stays one visible decision. `idp-cli deploy --log-level` also no longer treats `INFO` as "unset": its default is now unspecified rather than `INFO`, so `--log-level INFO` is honoured (previously it was silently dropped, which under the new template default would have quietly downgraded an explicit request to `WARN`) and omitting the flag preserves an existing stack's value. See [configuration](docs/configuration.md).
