...
- **A browser UX-test skill (`make ux-test`) — the one test layer in this repo that opens a browser.** There is a lot of testing here (`make test`, `api-test`, `stacktest-*`, SRT, ZAP, the benchmark suite) and none of it drives the UI, so everything below the UI can be green while a button does nothing or a critical flow is broken. That is not hypothetical: correcting a document's classification in a test set shipped broken for several versions and was found by a customer rather than by us. Each run produces **two** things, deliberately different in kind: functional pass/fail per flow, and a **usability critique** — the half no deterministic test can produce, and the reason a flow that technically works but confuses everyone who tries it now gets reported. Scope is intentionally narrow for the first pass: a **local browser against an already-deployed stack**, invoked by a developer. Explicitly *not* CI integration, remote/headless browser infrastructure, or a new extension — all three were considered and deferred, on the grounds that two or three flows working beats twenty half-specified. `scripts/ux_test_session.py` does setup and teardown *only*, creating a throwaway Cognito user so a UX run never uses a real operator's account, and reusing `rbac_common.py` rather than growing a second implementation of "make a user in this pool"; it deliberately holds no opinions, because a script pretending to assess usability is the thing this layer exists to avoid. `scripts/ux_flows.yaml` carries six starter flows whose ids **match the ground-truth correction QA plan**, so a finding is referenced the same way in both and customer-derived stories extend the file rather than being reported separately in different words; six known-missing flows are listed rather than silently absent. Safety properties are tested since the script touches Cognito: the throwaway address is `@example.invalid` (RFC 2606, so a stray invite cannot be delivered), passwords satisfy the pool policy and never repeat, and **teardown restores the app client's auth flows** — setup widens them to set a known password, and leaving that is a durable, invisible weakening of the stack's auth configuration. Setup prints its own teardown command, because an unrun teardown is the failure mode that leaves a known password behind. Three tests guard the *skill* rather than the code: that it still says to tear down and never to report a flow as passed without exercising it, that it is registered in `CLAUDE.md`, and that `.cline/skills/ux-test.md` is a symlink rather than a copy that would silently diverge. 15 new tests.

- **Classification errors are now visible where you can act on them — in the run results, in the document comparison, and linkable field-by-field.** From the Exxon POC review: *"the MAIN issue I was seeing was when we get classification wrong, how does that surface. Right now it's a mess."* It was surfaced as two aggregate percentages and nothing else, which understates it badly. Extraction runs against the schema of whatever class a document was assigned, so a wrong class makes every field for that document unreliable — and it can be *confidently* wrong, meaning the document raises few confidence alerts and therefore sorts **low priority** in the annotation queue's worst-first order. The failure most likely to matter was the one least likely to be seen. Three changes, and notably **no new computation** in any of them: `DocSplitMetrics.section_details_with_order` — `ground_truth_class`, `predicted_class`, and both page lists per section — was already computed for every evaluated document and serialized into its `results.json`. It was rendered only into the markdown evaluation report and then discarded: `test_execution_aggregation_function` loaded each document's `doc_split_metrics` and forwarded only the five numeric graded-packet fields. (1) **A `Classification errors` panel on the test run**, above the breakdown tables because a wrong class invalidates the field numbers below it, listing each affected document with expected vs predicted class and pages, each row deep-linking into the annotation queue where the class is corrected. (2) **The document viewer's `Show evaluation` now compares the class**, not just the fields — it previously said nothing about classification even though the class is model output too. (3) **Field-level deep links**: clicking a field offers `Copy link to field`, producing a URL that reopens the document with that field selected and scrolled to, for asking a colleague about one value in Slack. Three deliberate limits, each because the alternative is worse than the gap: the run-level list is **capped at 200** entries (the whole run result is one DynamoDB attribute, and a 500-document run with a wrong class list produces thousands) with wrong-class errors sorted first so page-order noise cannot crowd them out, and the true total reported when it truncates — "Showing the first 200 of 340", never a silent cut. The three kinds are kept **apart** rather than lumped as "errors": `class` (wrong schema ran), `unmatched` (a splitting difference, not a labelling one) and `order` (extraction unaffected — it is what split-accuracy-*with-order* penalises), because they imply different fixes. And the per-document banner **shows nothing rather than guessing** when a multi-section document cannot be matched to a ground-truth section unambiguously: `section_id` appears in the codebase as both `"1"` and `"section_1"` so ids are normalized to digits, and there is deliberately no page-overlap fallback because `ground_truth_pages` are 0-based indices while the UI's page ids are 1-based — overlap matching would be correct for some documents and off-by-one for others, and a *wrong* expected class would lead a reviewer to "correct" a document that was already right. Runs aggregated before this shipped re-aggregate automatically on first view (the cache-completeness guard gains the new key, which converges in one pass); the Athena fallback path has the percentages but not the per-section detail, and renders the panel empty rather than broken. Field deep links are navigation only — the recipient's access is still checked on arrival, so sharing one with someone outside the test set's scope grants nothing. Chosen over routing questions to a designated subject-matter expert's own queue, which assumes a customer organised that way and adds a workflow to maintain. 25 new tests (7 Python, 18 UI), including that a run with no misclassifications renders nothing rather than an empty table, that truncation is reported, and that an unmatchable section yields no banner.

- **New doc: [After Subscribing on AWS Marketplace](docs/marketplace-subscription-next-steps.md).** The page to register as a paid extension listing's post-subscribe / Quick Launch "Account login details" URL. A listing needs *some* URL there, and this product has no seller-hosted app to log in to — the buyer's AWS account is the identity, and the next step happens in their own IDP console. Per the AWS Marketplace docs the fulfillment URL may simply "provide instructions on their next steps", which is what this does: how to find your IDP console URL (the stack's `ApplicationWebURL` output), install as an admin, and what each subscription banner means, plus what to do if you don't have the accelerator deployed yet. Deliberately no `x-amzn-marketplace-token` / `ResolveCustomer` handshake — the seller-entitlement-service resolves entitlement on demand from the caller's verified AWS account id, so the registration callback is not load-bearing for this architecture.


- **Textract `SIGNATURES` is now on by default, and the shipped Textract cost figures were wrong by ~4×.** With signature detections finally usable end-to-end (see `### Fixed`), the default `ocr.features` becomes `TABLES` + `LAYOUT` + `SIGNATURES`. It adds **no Textract charge** in that combination, which the [Textract pricing page](https://aws.amazon.com/textract/pricing/) states outright: *"Signatures feature is included free of cost with any combination of Forms, Tables, Queries, and Layout"* (the footnote lives in the pricing widget's markup, alongside the same rule for Layout). Corroborated by our own bills, which show the mechanism — AWS emits no usage type at all for a feature that is free in combination: months of `SyncTablesPagesProcessed` with **no** `SyncLayoutPagesProcessed` and **no** `SyncSignaturesPagesProcessed` despite both features being enabled throughout. Used *alone*, `SIGNATURES` is billed at ~$0.0035/page, so the defaults must keep a paying feature alongside it. Signature presence is a common extraction target (loan packages, tax forms, claims, consents), and it was previously opt-in behind a feature flag most users never found — including `lending-package-sample` and `lending-package-sample-govcloud`, whose READMEs both claim "LAYOUT, TABLES, and SIGNATURES features" while their configs set no `ocr.features` at all; they now get all three by inheriting the default. One caveat is documented rather than assumed: a detection is **not** proof of a signature — Textract also flags stray pen marks at low confidence, which costs a few prompt tokens on affected pages and gives a model something to over-read, so corpora with no signature fields should drop the entry. **Separately, the shipped cost guidance was materially wrong:** `base-ocr.yaml` and `docs/configuration.md` both stated `TABLES`+`LAYOUT` ≈ **$0.065/page**, "~16×" the `LAYOUT`-only rate — that is the *Tables+Forms* rate. The real billed rate is **$0.015/page** (verified against the account's own invoice: 4,510 pages → $67.65 = exactly $0.015), i.e. ~3.75×, so the documented figure had been overstating the cost of the default configuration by more than 4×. The BDA-vs-Textract comparison built on that number is corrected too. Four new tests pin the default feature set, that it meters as `-Tables` alone, and that `SIGNATURES` is never left in the defaults without a paying feature (on its own it costs ~$0.0035/page).

- **Few-shot examples defined on a class are now actually used by extraction — the shipped prompts carry `{FEW_SHOT_EXAMPLES}`, and both key spellings are read ([#634](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/634)).** Defining `x-aws-idp-examples` used to be necessary but not sufficient, in two independent ways, each silent. (1) **No default extraction prompt contained the placeholder**, so examples were only ever sent if you *also* hand-edited the prompt — a class could carry carefully authored examples (including the exact counter-example for a misextraction, e.g. "small marks/artifacts are NOT signatures") and none of it reached the model. All three variants (`task_prompt`, `task_prompt_extraction_with_confidence`, `task_prompt_extraction_with_confidence_topk`) now include it, positioned before `<<CACHEPOINT>>` since examples are static per class and therefore cacheable; it renders empty and adds nothing for classes with no examples. (2) **The example reader only understood the legacy camelCase keys** (`classPrompt` / `attributesPrompt` / `imagePath`) — the `x-aws-idp-class-prompt` / `x-aws-idp-attributes-prompt` / `x-aws-idp-image-path` spelling that `docs/few-shot-examples.md` documents, and that the `X_AWS_IDP_*` constants in both the Python and TypeScript schema-constant modules declare, matched nothing, so a config authored from the docs was skipped as "example with empty attributesPrompt". Both forms are now accepted. Related hardening: an unreadable `imagePath` (bucket in another account/partition, deleted object, missing `s3:GetObject`) now **degrades to a text-only example with a warning instead of raising** — previously a `ValueError` that failed the whole section, which shipping the placeholder by default would have turned from latent into common. Note this changes prompt content (and therefore cost/behavior) for any class that has examples and previously relied on a prompt without the placeholder; configs that override a shipped `task_prompt` with their own copy still need to add the placeholder themselves. **The default classification prompt now carries the placeholder too**, for the same reason and with the same no-op behavior (classification reads each example's class-prompt, so an example with only an extraction prompt is still skipped there) — placed before `<<CACHEPOINT>>`, which matters more for classification since it runs per page. **And the `x-aws-idp-*` spelling is now canonical end-to-end, not merely tolerated.** The container was always `x-aws-idp-examples` while the entry fields lagged behind as camelCase, leaving the codebase split against itself: constants for the canonical names existed in both `config/schema_constants.py` and `src/ui/src/constants/schemaConstants.ts` — and were *imported but never used* in `config/migration.py` — the docs described the canonical form, and the UI wrote the legacy one. Now: config migration converts entry fields (canonical wins if both are present; malformed entries pass through untouched), the UI's schema editor writes canonical and strips the legacy alias from any example you edit while still displaying either, and the shipped `rvl-cdip-with-few-shot-examples` preset, the two `notebooks/misc/test_few_shot_*` notebooks, and every doc that showed the legacy keys (`docs/few-shot-examples.md`, `classification.md`, `pattern-2.md`, `idp-configuration-best-practices.md`, `config_library/README.md`, and the extraction/classification module READMEs) were converted. Readers keep accepting camelCase indefinitely, so **no existing config needs to change**. The pre-existing `test_migration_examples.py` suite asserted the old pass-through behavior and was updated to assert the rename while still pinning that prompt *content* is preserved verbatim. 19 new/updated library tests plus 4 UI tests, mutation-verified.

- **Self-review of the seller service against this repo's own standards found three convention regressions; all fixed.** Generic scanners had passed, but the project's conventions are stricter and were being missed. (1) **`ActivateFunctionRole` had no `PermissionsBoundary`** — every sibling feature-platform template has the `PermissionsBoundaryArn` parameter + condition + `!If`, and its absence is a *known-recurring* bug here: `test_permissions_boundary_coverage.py` exists because this exact omission previously broke `FeaturePlatformStack`, rolling the stack back on creation in SCP-enforced accounts. The template is now also **registered in that test's `DEPLOYED_TEMPLATES`** — it had been omitted, so the suite passed while giving false assurance (mutation-verified: removing the boundary now fails a named test). (2) **Log groups were unencrypted.** The earlier triage "accepted" checkov `CKV_AWS_158` against a generic rule without checking the codebase, which uses a CMK on 115 of 135 log groups — and these logs carry buyer AWS account ids and caller role ARNs, i.e. customer-identifying data for a seller. A dedicated `LogEncryptionKey` (symmetric, rotation enabled, *not* retained — unlike the signing key, retaining it would orphan a key on every test teardown) now encrypts both, with the CloudWatch Logs grant scoped by `kms:EncryptionContext:aws:logs:arn` to this account/region. (3) **`docs/aws-services-and-roles.md`** now documents the new role and the newly-used AWS Marketplace APIs, including why the marketplace actions use `Resource: "*"` (no resource-level support) and that the KMS/DynamoDB grants are sign-only and write-only. Five new static assertions cover the boundary, log encryption, grant scoping, key separation, and the no-retain choice.

- **~400 previously-ungated test suites now run in CI, and a payload-fuzz corpus for the activation endpoint.** No CI workflow referenced `feature-platform`, `idp_feature_sdk`, `idp_sdk`, or the seller service at all — the `developer-tests` job ran only `lib/idp_common_pkg` plus the UI. A `make test-packages-cicd` target already existed for exactly these suites and simply was never called; the workflow now calls it, and the target gains the seller-entitlement-service suite. Switching it on exposed why it had never run: CI installed only `idp_common_pkg`, so the suites could not even `import idp_sdk`. New `make install-first-party` installs all five first-party packages in a **single** pip pass — deliberately single, because they depend on each other by bare name and those names are squatted on public PyPI, so a split install resolves a sibling from PyPI — and then runs `scripts/check_first_party_deps.py`, which fails if any did. Verified in a throwaway worktree with a fresh `uv venv` replicating CI exactly: all five resolve from the checkout and the full target passes 1,055 tests across 11 suites. Enabling the target also exposed a latent environment dependency in a pre-existing suite: `src/lambda/chat_stream_processor/tests/` had **no `conftest.py`**, so `test_doc_processor_set_sink_redirects_emission` — which imports a module that builds a boto3 client at import scope — passed only where a region happened to be configured in the ambient environment and raised `NoRegionError` anywhere else. It had never been noticed because the test is `skipif(idp_common is None)` and the suite had never run in CI. Fixed with a conftest following the pattern the six sibling suites already use (`os.environ.setdefault("AWS_DEFAULT_REGION", …)`), so the suite is hermetic rather than the workflow papering over it. **New `test_payload_robustness.py`** asserts four invariants over ~60 hostile payloads (wrong JSON types, truncated/trailing JSON, 400-deep nesting, CRLF and null-byte log-forging, SQL/NoSQL/template/XSS/XXE strings, homoglyphs, zero-width and RTL-override characters, format specifiers, 10^500 integers): the handler never raises, always returns a well-formed response, **never** 5xx, and never reflects attacker input into the response body. It immediately found a **real bug in one of the security fixes above** — `product_id = (body.get("productId") or "").strip()` ran `.strip()` *before* the `isinstance` check, so `{"productId": 12345}` raised `AttributeError` → 502 and the type guard was unreachable dead code. Fixed by type-checking first. The same corpus runs over signed HTTP against a deployed stage in the dynamic test. **`tests/teardown_test_stack.sh`** makes a test deployment actually disposable: `TokenSigningKey` and `ActivationsTable` are `DeletionPolicy: Retain` (correct — destroying the key invalidates every issued token) and `DeletionPolicy` cannot be `Fn::If`'d per-environment, so `delete-stack` alone orphans a KMS key and a table on every run, with KMS having no immediate delete (7–30 day billed pending window). The script captures the retained physical ids *before* deleting the stack, then cleans them up, and refuses any stack name that doesn't look like a test stack. **ZAP/DAST was assessed and rejected with reasons:** it cannot SigV4-sign, so every request it sends is refused by the `AWS_IAM` authorizer before the Lambda is invoked — an active scan would report a clean 403 wall and prove nothing about the parser. The stated goal (hostile payloads must not break the service) is met directly by the corpus instead.

- **Security review of the Seller Entitlement Service: threat model (`SELL.T01–T10`) + static and dynamic security tests.** The service is the first component in this repo that runs in the **seller's** account rather than a customer's, is reachable by any AWS principal on the internet, and whose protected assets (signing key, customer roster, revenue) belong to the seller — so it did not fit the existing `FEAT.*` model. New `SELL` prefix and threat doc registered in the glossary and builder (83 → 93 threats; all reconciled counts regenerated). **Findings fixed during the review:** a non-object JSON body (`0`, `null`, `[]`) crashed the handler with an unhandled `AttributeError` → 502 plus a stack trace, reachable by *any* AWS account (now 400); the unknown-product `404` was a product-existence oracle that would confirm a **Limited**-state listing's private `productId` (now byte-identical to "not entitled", asserted by test); an unused `kms:GetPublicKey` grant removed (sign-only); tokens now carry a **`kid`**, without which the signing key could never be rotated — and which could not have been added later without breaking every deployed verifier; the request body is capped at 4 KB before parsing; and an allow-listed test account now gets **full** paid capability rather than being silently marked free-tier. Reserved Lambda concurrency added after an IaC scan flagged its absence — a real control here, since without it a flood from an arbitrary account would consume the seller account's entire Lambda pool. **Static tests** (`test_template_security.py`, 16 assertions) guard the invariants that live in CloudFormation where handler tests cannot see them and `cfn-lint` only checks syntax — SigV4 required, resource policy pinned to `POST /activate`, marketplace grants read-only, `kms:Sign` only, `dynamodb:UpdateItem` only, asymmetric key, no wildcard key-policy principal, retain policies, SSE/PITR, no hardcoded account ids, and API caching explicitly **not** enabled (a cached entitlement decision would outlive a cancellation). Every one was **mutation-tested**: flipping `AWS_IAM`→`NONE`, `UpdateItem`→`dynamodb:*`, adding `kms:PutKeyPolicy`, widening the resource policy, or disabling SSE each makes a specific test fail. **Dynamic test** (`dynamic_activation_test.py`) asserts what the deployed *stage* does rather than what the template says — unsigned→403, unentitled→403 with no internal detail leaked, unknown product byte-identical, malformed→400 not 502, and a real signature verification against the published public key. The repo's ZAP DAST and RBAC harnesses were assessed and deliberately **not** extended: ZAP cannot SigV4-sign (it could only confirm "everything is 403"), and the RBAC harness is built on Cognito groups this service does not have. IaC-scanner findings are triaged once in the README with accept/reject reasons.

- **Activation visibility for paid extensions: `idp-feature-cli seller-service activations`.** The seller service previously recorded attempts only as printf-style Lambda log lines plus an API access log with no `productId` and no reason — so answering "who has activated what?" meant hand-writing Logs Insights queries and regexing free text, and everything aged out with `LogRetentionInDays`. Not good enough for a commercial record. Adds a DynamoDB **activation roster**: one item per (buyer account, product) with first/last seen, attempt and grant counts, last outcome + reason, free-tier flag, and the service version that answered. `Retain`-on-delete with point-in-time recovery, because it is a record of who your customers are. A `ProductIndex` GSI answers "who activated product X, newest first" without a scan. Roster writes are **fail-open** — bookkeeping must never be the reason an entitled customer is refused a token; the logs remain the backstop. Also emits an `ActivationAttempt` CloudWatch metric (namespace `IDPSellerEntitlement`, dimensions `ProductId` + `Outcome`) via Embedded Metric Format, so no extra IAM and no latency on the activation path — the pre-existing access-log filter could only see the HTTP status, not which product was refused or why. The CLI reads the roster with filters (`--product-id`, `--buyer-account-id`, `--outcome`, `--since`, `--json`), resolves the table from the stack outputs so no name is needed, highlights refusals with their reasons, and degrades to a friendly message (not a traceback) when the stack or the roster output is absent — the latter suggests a redeploy, since an older deployment predates the table. `make seller-entitlement-service-activations` wraps it. Deliberately **no UI**: this service runs in the *seller's* account while the IDP web UI belongs to the *buyer's* stack, so there is nowhere in the product for a seller console — the CLI is the surface and CloudWatch has the graphs. 11 new SDK tests + 7 Lambda tests, covering GSI-vs-scan, pagination, Decimal counters, fail-open writes, and EMF payload validity.

- **`idp-feature-cli seller-service preflight|deploy`** — deploys the Seller Entitlement Service into a seller account, refusing if the credentials aren't the seller. Put in the extension-author CLI rather than a bare Makefile target for two reasons: the audience is the same people who already run `idp-feature-cli publish`/`deploy`, and the preflight is a safety guard whose silent failure mode is "every customer locked out", which deserves unit tests that a shell snippet in a Makefile would not get. It verifies **product ownership** (`marketplace-catalog:ListEntities`), not merely "is this a seller account" — an account-id comparison would pass for any seller, including one that doesn't sell this product. Refuses with an actionable message when the account owns no SaaS products (the common "ran it with dev credentials" case), when it owns different products (listing what it *does* own), when `--seller-account-id` doesn't match, or when `ListEntities` is denied — that last one fails rather than warns, because AccessDenied nearly always means these aren't seller credentials, with `--skip-ownership-check` as a documented escape hatch. It also catches passing the product *code* where the `prod-…` entity id is required, before any AWS call. Thin `make seller-entitlement-service` / `-preflight` wrappers give repo users discoverability via `make help` without duplicating logic. 23 unit tests; all four paths (owner / non-seller / wrong-account assertion / bad product id) exercised against the live seller and buyer accounts.

- **The Seller Entitlement Service template now carries a version**, stamped by `make version` and surfaced three ways: a `ServiceVersion` stack output, a `SERVICE_VERSION` env var recorded on every activation log line (so a seller debugging a customer's token knows which build minted it), and a single `ServiceMeta.ServiceVersion.Value` mapping as the one sed target. It uses a **literal** rather than the `<VERSION>` placeholder the published host template relies on, because this template is deployed directly by a seller (`sam deploy` / `idp-feature-cli`) and never passes through `idp-cli publish` — nothing would substitute a placeholder. A unit test asserts the shape so the sed target can't silently drift.

- **A ready-made Seller Entitlement Service for paid extensions** (`feature-platform/seller-entitlement-service/`). A Feature Platform extension deploys into the **buyer's** AWS account, and the buyer owns the Lambda, its environment variables and its code — so software running there cannot enforce its own licence, and the host's entitlement check is advisory by construction. Enforcement requires the *seller* to hold something the buyer needs at runtime. This is a self-contained SAM stack a seller deploys **once in their own seller account**: an extension POSTs `{"productId": "prod-…"}` and receives a short-lived, account-bound token signed by a KMS **asymmetric** key (asymmetric so the extension can verify with the public key without being able to mint). It is generic — any AWS partner publishing a Feature Platform extension can deploy it as-is and register their own product ids. Entitlement is checked with `SearchAgreements` `PartyType=Proposer` + `AcceptorAccountId`, which only works seller-side; `GetEntitlements` / `ResolveCustomer` are wired in for SaaS *Contract* listings and the SaaS registration flow. Authentication is API Gateway **`AWS_IAM`** with a resource policy admitting any AWS principal: API Gateway verifies SigV4 before invoking the Lambda, which reads the buyer account **only** from `requestContext.identity` and never from the request body — a body field would let anyone claim to be a subscribed account. That combination means the seller needs no advance knowledge of buyer accounts and buyers need no seller-issued credentials. Deliberate asymmetry with the host: this service **fails closed** (an error means the *seller's* infrastructure is broken, and minting tokens on our own failure would make the gate meaningless), while the extension is required to hold a **grace period** on the last-known-good token so a seller-side outage never locks out a paying customer. `allowFreeTier` issues a reduced-capability token to unsubscribed accounts so a listing with a free dimension stays usable. The README carries an explicit threat model that does **not** overclaim: it stops parameter-flipping, simulator-pointing and direct-template deploys, but not a customer who patches the extension — so it is deterrence plus a reconciliation trail, and it only becomes real enforcement to the extent the token gates something genuinely needed from the seller. 17 unit tests, most of them pinning the identity-spoofing and fail-closed properties. The entitlement design is **verified against the live listing across every caller/subscription combination** — unsubscribed buyer, subscribed buyer, and seller account. `SearchAgreements` answers correctly in all three (buyer-side `Acceptor` and seller-side `Proposer`); `GetEntitlements` returns an empty list in all three, including for a genuinely subscribed buyer, confirming a usage-based SaaS listing has no entitlement records anywhere. The seller-side query has both controls: a genuinely subscribed buyer returns its `ACTIVE` open-ended agreement, an unsubscribed buyer returns an empty list rather than an error. Two things that verification pinned down: the filter name is `ResourceIdentifier` (the docs' Proposer-side prose says `ResourceId`, which the service rejects with `ValidationException: Provided filter name is invalid`), and **`GetEntitlements` returns an empty list even from the seller account with the correct product code** — confirming that a usage-based SaaS listing has no entitlement records at all, so that API can never answer the subscription question from anywhere.

- **Host support for paid AWS Marketplace extensions, starting with Auto Optimizer.** The feature platform could describe a Marketplace extension but could not actually serve one. Four things were missing and are now in place. **(1) Per-region artifact mapping (catalog schema 1.1).** `sam package` bakes an absolute, region-specific `s3://bucket/key` `CodeUri` into the published template, and a Lambda's code bucket must be in the function's own region — so a paid extension needs one published copy per region. `config_library/extensions-marketplace.yaml` gains a `regions` map (explicit `sellerBucket` + version-free `templateKey` per region) plus `productId`, and `getFeatureLaunchUrl` **looks the caller's region up** and fails closed with "not available in `<region>`" when it's absent. It deliberately does not derive a bucket name from a basename plus `AWS::Region`: bucket names are global and guessable, so a derived name in an unpublished region could resolve to a bucket someone else owns and hand the customer a CloudFormation template we didn't write. **(2) Region availability in the UI.** `listCatalogFeatures` reports `availableInRegion` / `availableRegions`, computed with the same helper the launch resolver uses so the badge and the launcher cannot disagree; the catalog browser and feature page show where an extension *is* available instead of a Subscribe button that dead-ends. **(3) Runtime `latest.json`.** See the separate entry below. **(4) A production entitlement path** (`FeaturePlatformSubscriptionMode=marketplace-live`, the new default when no simulator endpoint is set) built on the **buyer-side** AWS Marketplace Agreement API — `SearchAgreements` filtered by `PartyType=Acceptor` + `AgreementType=PurchaseAgreement` + `ResourceIdentifier=<productId>` + `Status=ACTIVE`. `GetEntitlements` cannot serve as this gate, and fails in the worst possible way: it is a seller-side API, entitlement records only exist for SaaS *Contract* products (a usage-based SaaS Subscription has none), and from a buyer account it returns HTTP 200 with an **empty list** rather than an error — so a fail-closed gate on it would deny every legitimate customer while logging nothing, and would look healthy in CI against the simulator. License Manager's `ListReceivedLicenses` was also ruled out: it fails with `AccessDeniedException: Service role not found` until a service role exists in the buyer account, which can't be required of a customer. The live path distinguishes ACTIVE / NONE (authoritative — `SearchAgreements` is scoped to the caller's own account) / **UNKNOWN**, where a *failed* call degrades to advisory-ACTIVE with a loud log rather than denying, because failing closed on a permissions gap would lock a paying customer out of an extension they bought — each paid extension enforces its own runtime entitlement check, so a permissive host gate costs nothing. The simulator (`simulator` / `marketplace` / `auto`) is untouched: `marketplace-live` is a new tag on the existing `SIMULATOR_SOURCE_TAG` switch, and OSS extensions still short-circuit to ACTIVE in every mode. `scripts/marketplace/verify_entitlement.sh` reproduces the whole comparison against a live account with a positive control, so an empty result is distinguishable from a broken one. Docs across both tiers (`docs/feature-platform.md`, `docs/feature-platform-developer-guide.md`).

- **"Update available" badges now read `latest.json` at runtime, so an extension release no longer requires an accelerator release.** `latestVersion` was baked into `catalog.json` at *host*-publish time, which meant a new extension version stayed invisible to customers until the whole accelerator was re-released **and** the customer took that release — shipping an extension patch required shipping the platform. For a paid Marketplace extension on its own release cadence that is untenable; for OSS extensions it was merely bad. The extension publisher has always rewritten `<base>/latest.json` on every release (`{featureId, version, displayName, bundleSha256, publishedAt}`); the host simply stopped reading it. `listInstalledFeatures` now resolves the live value with the catalog as fallback. Because it runs on every page load it is **fail-soft** (unreachable object, bad JSON, blocked egress, unpublished region, or a feature absent from the catalog all fall back to the catalog value and then to no badge — never an error), **cached** (memoized per bucket/key for `LATEST_JSON_TTL_SECONDS`, default 300s, with a shorter negative TTL so a missing object doesn't cost a round trip every time, and a capped concurrent fan-out so wall time is one S3 round trip rather than N), and **anonymous first** (published artifacts are public-read, so the unsigned GET needs no IAM grant on the host role and no bucket-policy grant from the publisher — enabling this cannot regress an existing deployment; a refused public read retries *signed*, which lets an OSS extension self-published to a private bucket still get badges via `SellerBucketObjectArns`). `LATEST_JSON_LOOKUP=false` reverts to catalog-only.
...
- **The pipeline-hook mechanism now has end-to-end CI coverage (Step 14).** None of the seven hook points had any live test — unit tests exercise the dispatcher with fakes, but four things only exist on a real stack: the dispatcher reading a hook out of a real config version, `UpdateSchemaConfig` publishing the hook config sections (so the fields an admin edits exist on the deployed stack), a hook Lambda clearing the dispatcher's tag/name IAM condition, and a hook's `updatedDocument` surviving into the **persisted** document. That gap mattered because a hook which silently never fires is indistinguishable from one that ran and decided nothing — the workflow still succeeds and the document still reaches `COMPLETED` — which is exactly how [#599](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/599) shipped. The new step deploys a real `idp-citest-hook-*` Lambda built from the published `idp_common.hooks` helpers (so it also exercises the contract given to feature authors) and tagged `idp:feature-id` — the ABAC path installed features use, rather than the `GENAIIDP-*` admin name escape hatch, since the CI role's `iam:*`/`lambda:*` are scoped to `idp-*`, registers it at both `preprocessing` and `postprocessing` in a **dedicated, never-activated** config version, processes a document pinned to it, and asserts: the hook sections survived the config round-trip; `invoked >= 1` at both points along with the `configVersion` the dispatcher resolved; the update was **accepted** (`documentUpdatedBy` non-empty) rather than refused by the guardrails; and the marker reached the tracking row. Runs in the parallel pool and tears down its Lambda + role in a `finally` — a leftover hook ARN is the stale-ARN state that fails every subsequent document. 15 offline tests cover the parts that would otherwise fail silently in CI (the Lambda package's contents and the handler's logic). See [CI test coverage → Step 14](scripts/sdlc/docs/CI_TEST_COVERAGE.md).

- **Per-field accuracy now reports its sample size and margin of error.** Test Results showed each field's accuracy as a bare point estimate, so 100% measured on 3 observations and 100% measured on 300 rendered identically while justifying completely different decisions. This matters more per field than overall: a run's overall accuracy firms up within roughly a hundred documents because every document feeds it, whereas a field appearing once per document gains one observation per document — so a badly-broken field can sit inside a healthy-looking overall score. Two new columns, **Observations** and **95% margin**, with the interval in the cell tooltip and fields whose margin exceeds 10 points rendered subdued. Derived from the confusion-matrix counts already stored, so there is no new measurement, no schema change (`fieldMetrics` is already `AWSJSON`) and no migration — runs aggregated before this release still show both columns, because the UI computes from the same counts when the backend values are absent. Uses a Wilson score interval rather than the textbook normal approximation, which at 20 observations and 90% accuracy reports an upper bound of 103% — an impossible accuracy printed in exactly the low-evidence case the columns exist to surface. Documented limits: the interval covers sampling uncertainty only, not errors in the ground truth itself, not documents that are unrepresentative of production, and not observations that repeat within one document (table-row fields are not independent, so their margin reads tighter than it should — compare Observations against the run's document count). See [docs/test-studio.md](docs/test-studio.md#field-level-metrics).

### Changed

- **BREAKING (operators): entitlement authority is now declared PER EXTENSION (`licenseMode`), not chosen once for the whole stack.** One stack can finally host a listed, published extension whose subscription is confirmed against **real AWS Marketplace** alongside **in-development** extensions checked against the marketplace simulator and **OSS** extensions checked against nothing. Previously the authority was stack-wide, so pointing a stack at a simulator pointed it there for *everything* — including a live listed product — and the host showed a simulator-backed "Subscription active" for an extension that only honours real Marketplace, which the extension correctly contradicted. `licenseMode` (`none` | `simulated` | `marketplace-live` — the existing `entitlementSource` vocabulary, so both sides speak one language) is declared in two places on purpose: in `config_library/extensions-marketplace.yaml`, governing the **host's** check, and in the extension's own `feature.yaml`/template, governing the **extension's** check. The extension's value is propagated through `registerFeature` at install onto the `InstalledFeatures` row, which is what makes the catalog entry verifiable rather than aspirational — the host prefers it, so its check lands on the authority the extension actually honours, and it reports a mismatch (`licenseModeMismatch`) when the catalog disagrees instead of showing a subscription the extension will ignore. **The prerequisite was dropping env-var endpoint overrides:** `AWS_ENDPOINT_URL_MARKETPLACE_*` are per-*function*, so while boto3 read the endpoint from the environment a single `checkFeatureEntitlement` Lambda could only ever be aimed at one authority — that mechanism was not a stack-wide policy, it was what *forced* stack-wide behaviour. The resolver now pops those variables at import and passes `endpoint_url=` explicitly per request, caching one client per authority, and the reported source is anchored to the endpoint each individual call used. **`FeaturePlatformSubscriptionMode` is reduced to a kill switch** (`auto` = check nothing on this stack); its other values no longer select the authority or name the reported source and are read only as step 3 of the migration chain below — scheduled for removal one release after the next. **Migration:** resolution is install-row → catalog entry → legacy stack setting → `marketplace-live` for a marketplace entry / `none` for OSS, so an existing stack that never re-installs anything keeps working, and the one upgrade-sensitive combination (legacy `marketplace-live` + a simulator endpoint + a pre-`licenseMode` catalog) still calls the same API against the same endpoint as before. A missing `licenseMode` in `extensions-marketplace.yaml` is defaulted to `marketplace-live` at publish time — deliberately the *opposite* of the extension-side default (`none`), because the host must never over-claim verification while the extension must never lock a paying customer out of something they bought. Both defaults are load-bearing and documented as such. Also: the UI no longer offers the simulator-backed **Subscribe** button for an extension whose authority is `marketplace-live` (it could only mint a subscription that extension ignores), and `checkFeatureEntitlement` no longer returns `source: "none"` — a missing productCode on a simulator authority synthesises one rather than conflating a missing identity with a missing subscription. **For extension authors:** the `entitlementSource` union is unchanged and `FeatureContext` gains no fields, so no code change is required; set `marketplace.licenseMode` in your manifest to have the host agree with your own gate. See [feature-platform.md](docs/feature-platform.md) and the [developer guide](docs/feature-platform-developer-guide.md).

- **BREAKING (extension authors): `entitlementSource` loses two values and gains one — `marketplace` and `simulator` collapse into `simulated`.** Both named the *same code path*: the seller-side `GetEntitlements` API, which returns HTTP 200 with an empty list when called from a buyer account and therefore cannot verify anything against real AWS. Both were already treated as unverified and already shared one explanation string in the UI, so the distinction was one no extension could act on — while making `marketplace` (the weakest source) read as more authoritative than `marketplace-live` (the only verified one), which is exactly backwards. The union is now `marketplace-live | simulated | advisory | auto | oss | none`. **Upgrade:** if your extension matches `'simulator'` or `'marketplace'` literally, match `'simulated'` instead; extensions that only read `uiAccessAllowed` / `entitlementVerified` need no change. `SubscriptionMode` keeps all four values (`auto`, `simulator`, `marketplace`, `marketplace-live`) — the deployment mode is now deliberately decoupled from the source reported to extensions, so **no existing stack breaks on update**. `marketplace-live` was deliberately NOT renamed to `marketplace`: that would silently invert the meaning of a value extensions may already match on. `sample-feature` and `sample-health-insurance-review` → 0.2.1.

- **An OSS extension now reports `entitlementSource: oss` in every deployment mode.** The `auto` branch was evaluated before the OSS check, so on an auto-mode stack an open-source extension reported `auto`, and `oss` was not a dependable signal for "this is not a paid extension" — the one thing it exists to say. Being open-source is a property of the extension; a deployment mode cannot change it. The short-circuit remains an explicit `source == "oss"` test rather than "not a marketplace feature", because an absent or unreadable catalog entry is also not-marketplace, and treating *that* as OSS would grant a paid extension access with no entitlement check at all.

- The three entitlement Lambdas defaulted `SIMULATOR_SOURCE_TAG` inconsistently (`check_feature_entitlement` to `marketplace`, `subscribe`/`unsubscribe` to `simulator`) and the nested feature-platform stack defaulted `SubscriptionMode` to `marketplace` while the main stack defaulted to `marketplace-live`. All now default to `marketplace-live`, so a misconfigured or standalone deploy lands on a real check rather than one that cannot verify anything.

- **BREAKING (extension authors): `FeatureContext.subscriptionActive` is renamed `uiAccessAllowed`.** The old name invited exactly the wrong reading — it is a boolean computed by the host and handed to code running in the end user's browser, inside the customer's own AWS account, and it reads `true` whenever subscription checks are off (`auto`), simulated, or unreachable (`advisory`). Extensions were being taught to treat it as a licence check; the reference samples gated on it and none did any server-side verification. The new name says what it is: presentation, not authorization. `entitlementSource` and `entitlementVerified` were added alongside it for the honest signal. **Upgrade:** rename the field where your extension destructures it. All six bundled extensions are updated and version-bumped (`pii-anonymizer`, `sample-feature`, `sample-health-insurance-review` → 0.2.0; `confbench-testset` → 0.1.2; `idp-data-generator` → 0.1.3) so the "Update available" badge prompts admins to pick up the change. An extension installed *before* this release keeps working until updated, but renders inert (its buttons stay disabled) because the prop it reads is now absent — update the extension to fix. Done as a clean rename rather than a deprecated alias because no paid Marketplace extension has shipped to customers yet, so this is the last cheap moment to fix the name.

- **The subscription mode is no longer inferred from whether a simulator endpoint is set.** `FeaturePlatformSubscriptionMode` is now the single source of truth, and `FeaturePlatformSimulatorEndpoint` only decides *where* the call goes — two independent choices that were previously conflated. The coupling had a concrete cost: setting a simulator endpoint forced `marketplace`, which made the production path (`marketplace-live`, the buyer-side Agreement API) **unreachable whenever a simulator was configured**, so it could never be exercised in development. The nested stack now also sets `AWS_ENDPOINT_URL_MARKETPLACE_AGREEMENT` from the same simulator base URL — botocore derives that variable name from the service model, the same convention the existing entitlement override relies on, so a simulator can back the buyer-side path with no client code. A simulator that doesn't implement `SearchAgreements` simply errors, which maps to UNKNOWN → advisory ACTIVE with a loud log rather than a hard failure. `marketplace` is retained as an explicit mode for simulator work. **Upgrade note for developers using the simulator:** a stack with `FeaturePlatformSimulatorEndpoint` set now uses `marketplace-live` unless you set `FeaturePlatformSubscriptionMode=marketplace`, because the mode is no longer implied. The new `docs/feature-platform-developer-guide.md` → "Simulator fidelity contract" spells out what a faithful simulator must implement, starting with the point that a simulator making `GetEntitlements` *succeed* for a buyer-side caller reproduces the opposite of real AWS behavior and will certify a design that fails silently in production.

- **The Marketplace seller-template presign is gone; the template URL is a bare public S3 URL.** It could never have protected the artifacts it appeared to protect: the registered Quick Launch template URL is fetched by AWS Seller Ops during listing review and by CloudFormation in an arbitrary buyer account, and the Lambda code zips are fetched from the buyer's account at deploy time — all of which require public read, and none of which a presigned URL covers. Meanwhile its expiry was a real failure mode, 403-ing an admin who sat in the CloudFormation "Update stack" wizard longer than `PRESIGN_TTL_SECONDS`. Removing it also collapses the marketplace and OSS launch paths into one bare-URL code path, and drops the requirement that every customer configure `SellerBucketObjectArns` plus a seller bucket-policy grant just to click Launch. The entitlement check on the Launch button is now documented for what it always effectively was — an **advisory UX gate** that routes an unsubscribed admin to Subscribe; the commercial gate is the Marketplace subscription plus the extension's own runtime entitlement check, never artifact secrecy. `SellerBucketObjectArns` survives with a narrower purpose (signed `latest.json` reads from a *private* self-published artifacts bucket).

- **The Document List now shows only documents a user submitted; Test Studio runs have their own view.** Test Studio deliberately submits through the production pipeline so confidence and cost semantics match what real runs produce, but that made its documents indistinguishable from customer uploads in the tracking table — on a dev stack, 248 of 249 rows in the Document List were test artifacts. The copier now writes submission-source and test-set-id as S3 object metadata, `from_s3_event` reads them off the HEAD it already performs, and the tracking write turns them into an `ItemType` (`document` vs `test-document`) — the `TypeDateIndex` hash key, so the two views are separate index partitions rather than a filter applied after the page limit (which would have returned short, unpredictably-sized pages). **Upgrade note:** documents processed before this release carry no provenance metadata, so they stay in the Document List until the `ItemType` backfill state machine runs — it is triggered automatically by a CloudFormation custom resource on stack update, is idempotent, and resumes from a continuation token if it hits the Lambda timeout. It retypes a legacy row only when the key prefix has the copier's `<testSetId>-<YYYYMMDD>-<HHMMSS>/` shape **and** a matching test-run record still exists; the shape alone is not exclusive to test runs (nothing stops a customer from organising a bucket into timestamped folders) and retyping a real upload would remove it from the Document List, so an unconfirmed match is left visible. Consequence: legacy test documents whose run records have been aged out by `DataRetentionInDays` remain in the Document List, where they can be deleted individually.
...
- **Extensions UI: the subscription banner is one line, names AWS Marketplace, and links to where a subscription is actually managed.** "Subscription active" + a second row reading `Source: marketplace-live` becomes **Subscription active (AWS Marketplace)**, with a **Manage subscription** button pointing at the buyer's [AWS Marketplace subscriptions console](https://console.aws.amazon.com/marketplace/home#/subscriptions) — the only place a real subscription can be viewed, changed or cancelled, since the in-UI Cancel button drives the simulator's admin API and is offered for simulated grants only. The same button now appears on **every** state that has established the extension is paid and the subscription is active, not just the installed one: subscribed-but-not-yet-installed (admin) and awaiting-admin-install (non-admin) previously stated "your subscription is active" while offering no route to it, and a subscription is billable whether or not anyone has launched the stack yet. It is deliberately not gated on the IDP `Admin` group — the AWS console enforces its own IAM permissions, and an IDP viewer may be the account's billing owner. The friendly label is applied to the *verified* banner only: the unverified banner still names the raw source (`simulated`, `advisory`, `auto`) verbatim, because there the exact mode is the whole point, and an unrecognised source still passes through raw rather than being given an invented name.

- **The Extensions info panel no longer tells a paying customer to subscribe, and drops the "(future)" qualifiers.** For an uninstalled marketplace extension it read **Subscribe to install** — a guess, and wrong for exactly the customer it mattered to: someone who had already paid and only needed to launch the stack. The panel is built from `listCatalogFeatures` + `listInstalledFeatures`, neither of which carries an entitlement verdict; that verdict comes from `checkFeatureEntitlement`, which for a `marketplace-live` extension calls the AWS Marketplace Agreement API, and the feature page beside the panel has already made the call and shown the answer — mounting the hook again would duplicate a real Marketplace API call to word one status line. So the status now says only what the panel knows (**Not installed**, or **Not available in this Region**, which outranks it), and the subscription requirement is stated separately as the property of the *extension* that it is ("Requires an AWS Marketplace subscription.") — the same split, and the same wording, the nav's hover text already used. Also removed: the "Preview / the framework is still being built out / no Marketplace extensions are available yet" paragraph and all three `(future)` qualifiers, since Marketplace extensions ship now; the lifecycle list is reordered Subscribe → Install → Ready → Update to match what an admin actually does, and notes that Marketplace extensions are published per Region. New `FeaturesToolsPanel.test.tsx` covers the panel, which had no tests.

- **Auto Optimizer is labelled "Auto Optimizer (Beta)".** It is sold on AWS Marketplace under beta licence terms, so the name in the UI now matches what the customer agreed to. Set in three places that all have to agree, because two different ones win at different times: `config_library/extensions-marketplace.yaml` supplies the name before install, and the extension's own `FeatureDisplayName` / `feature.yaml` (closed-source repo) supplies it after — the host prefers the value `registerFeature` wrote. [docs/extensions/auto-optimizer.md](docs/extensions/auto-optimizer.md) is updated with it, and its availability section replaces the stale "private beta, ask your account team" text with the real path: subscribe on the listing with the account your stack runs in, then install as an admin.

- **The synthetic generator's render stage gets a longer, configurable timeout.** The per-stage cap was 1 hour and not reachable from a stack parameter, so it silently used its default. The cap applies per *stage*, and one stage renders a whole batch, so its cost scales with count x quality x augmentation rather than per document — and because the stage is all-or-nothing, exceeding it discards every document generated so far. Default raised to 3 hours and exposed as `SeedNodeTimeoutSeconds` (600–27000) on the Test Set Generator extension. The whole-pipeline timeout is now always at least twice the per-stage one however it is configured: the two are easy to misorder, and the failure is silent — a pipeline timeout at or below the stage timeout means raising the stage timeout appears to do nothing.

### Fixed

- **The Extensions nav showed a "Subscribe" badge for an extension the customer had already subscribed to.** The feature page said *Subscription active* while the nav said *Subscribe*, for a paid, confirmed subscription that simply wasn't installed yet. The badge was an inference — `marketplace + not installed ⇒ not subscribed` — and the nav has no way to check it: it is built from `listInstalledFeatures` + `listCatalogFeatures`, and neither carries an entitlement verdict. That is deliberate; the verdict comes from `checkFeatureEntitlement`, which is per-feature and for `marketplace-live` calls the AWS Marketplace Agreement API, so fanning it out across every catalog entry on every page render — to pick one word on a badge — would put a real Marketplace API call on the critical path of the whole application. The nav now states only what it knows: **Install** for anything not installed, OSS and marketplace alike, leaving the feature page (which has the verdict, and is where the click goes) to resolve whether the next step is subscribing or installing. "Install" rather than "Deploy" to match the vocabulary already used everywhere for this step. Marketplace extensions keep the distinction where it can be made honestly — the hover text now adds *"Requires an AWS Marketplace subscription"*, which is a property of the extension straight from the catalog rather than a claim about this customer. This module had no tests; it has six now, and the two covering the reported bug are mutation-verified against the shipped inference.


- **Textract signature detections were described only by raw coordinates, and both the extraction and the confidence model read them backwards ([#634](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/634)).** Surfacing the detection was necessary but not sufficient: given `signature 1: confidence=11.0 left=0.572 ...`, the confidence model concluded *"the OCR detected a signature region in the first (left) signature box"* and, of the cell that actually held the mark, *"the OCR did not detect any signature in this region"* — exactly inverted, on a form with two identically-labelled signature cells. Attributing a box to a field is spatial arithmetic that models get wrong, and the inline `[SIGNATURE]` token made it worse: reading order placed it immediately after taxpayer 1's date while the detection sat in taxpayer 2's cell, and the block's own wording ("matching the inline [SIGNATURE] token(s) above") reinforced that false association. Each detection is now reported with a confidence **band** (`11.0 (very low)`), its position in the left/right + upper/lower language field descriptions actually use (`right half, lower area (x=59%, y=89%)`), the OCR text it **sits on** or beside (`at: "Signature of taxpayer"; right: "Date"`, derived from existing LINE geometry, skipping single-character OCR noise), an explicit region total, and a caveat that the inline token's placement is not evidence of field membership. **Measured against the issue's document** (Sonnet 4.5, temperature 0, page images attached, repeated runs): the enrichment alone does *not* fix the extraction, nor do strict field descriptions alone, nor few-shot examples alone — all three together do, reproducibly. Notably, with descriptions + examples but *without* this enrichment the false positive does not disappear, it **migrates to the other taxpayer's field**: the model can tell a mark exists but not which cell owns it. `docs/configuration.md` now documents that recipe. Eleven new tests, mutation-verified. **Follow-up from live verification:** the version of this block that also named the OCR text around each detection (`at: "Signature of taxpayer"`) measured *worse* — naming a signature label beside the mark biases the model toward `true` (2/5 vs 9/9 on the same document) — so it was removed and a test pins the decision. What made the extraction deterministic in the end was a **field description that turns the detection confidence into an explicit decision rule** ("answer false unless a detected region with confidence >= 50 falls in this box", plus "ignore the inline [SIGNATURE] marker" and "a faint mark you can see does not override this"): 9/9 locally and correct on two consecutive live runs of the issue's document, with the confidence pass scoring it 1.0. `docs/configuration.md` carries the wording.

- **"Launch Stack" failed with "No active AWS Marketplace entitlement" for a customer who genuinely had one.** `getFeatureLaunchUrl` carried its own entitlement gate, separate from `checkFeatureEntitlement`, and it denied every subscribed customer on the production path for two independent reasons: it required a CustomerIdentifier from a request header or `DEFAULT_CUSTOMER_IDENTIFIER` — neither of which exists on a real-Marketplace stack, so it raised before making any API call — and it asked seller-side `GetEntitlements`, which returns HTTP 200 with an empty list from a buyer account for a usage-based SaaS listing. It only ever worked against the simulator. The result was the contradiction this platform has spent several changes unpicking: the feature page said "Subscription active" while the button said "no entitlement". The gate is **removed**, not repaired. `checkFeatureEntitlement` is the single host-side authority — it resolves the feature's own `licenseMode`, asks the buyer-side Agreement API against real AWS for `marketplace-live`, and the UI only offers Launch/Update when it answered ACTIVE — so a second gate implemented differently could agree only by coincidence. Nothing was protected by re-checking either: the template and code zips are public-read by necessity, so this was never a confidentiality boundary, and the commercial gate is the extension's own runtime check in the seller's account. Every marketplace test in that suite had monkeypatched the gate to `True`, which is exactly why none of them caught it; they now supply no entitlement state at all and assert the retired machinery is gone.

- **"Subscription required" for a paid extension rendered with no buttons at all — no way to reach the AWS Marketplace listing and subscribe.** `FeaturePage`'s `marketplaceUrls` prop is an override no route ever passes, so the URL it resolved was always `undefined` in production, and only two of the five call sites fell back to the catalog entry's `marketplaceListingUrl` — the NONE, EXPIRED and awaiting-install states did not. The in-UI **Subscribe** button had been masking that (it renders unconditionally for admins), so suppressing it for `marketplace-live` extensions — correct, since it drives the simulator and would mint a subscription the extension ignores — removed the mask and left an admin who genuinely needed to subscribe with nothing to click. The listing URL is now resolved once from the catalog for every state. A `marketplace-live` extension with no listing URL anywhere also no longer renders a silent dead end: it names the field and file to fix. Every UI test passed throughout, because the shared `renderPage` test helper supplies the very prop production never does — the new tests render without it, and are mutation-verified against the shipped code.

- **Textract's `SIGNATURES` output was almost entirely discarded: its confidence and position reached nothing, the Web UI could not show it, and the confidence prompt never saw it at all ([#634](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/634)).** A `SIGNATURE` block is a detection confidence plus a bounding box with **no text**, so every LINE-oriented consumer skipped it: `_generate_text_confidence_data` and `_build_page_data` both filter to `BlockType == "LINE"`. The only trace left was the linearizer's inline `[SIGNATURE]` token in the page text — which is *unpositioned* (reading order can drop it beside an unrelated field) and *unqualified*, making a real signature byte-identical to a 10%-confidence smudge. Observed on an unsigned IRS Form 4549: Textract flagged one region at **11.0%** confidence in the *second* taxpayer's cell and none in the first, yet nothing downstream could use either fact, and the extraction marked taxpayer 1 as signed (inferring it from the date in the adjacent cell). Detections are now carried in three places: `pageData.json` gains a `signatures` array (separate from `lines`, since the `ocr_only` geometry grounder matches values against line *text*); `textConfidence.json` and the parsed page text each get an appended `OCR signature detections` block giving every detection's normalized box and confidence — plain lines, deliberately not a markdown table, since page text is scanned by the agentic table parser. The Web UI page viewer lists them under **Signature detections** with a colour-coded confidence and a clickable bounding box. Note this adds a few tokens to the extraction and confidence prompts **only** for pages that actually have detections, and `docs/configuration.md` now documents that a detection is not proof of a signature — read the confidence, and say so in the field description if you extract a signed/unsigned boolean. Eleven new library tests plus two UI tests, mutation-verified.

- **The confidence (assessment) prompt showed no field descriptions at all for any class whose groups or lists live in `$defs` — which is every class the UI's schema editor produces ([#634](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/634)).** `_format_property_descriptions` read `type` and `description` directly off each property, but a `$ref` property carries neither — both live in the referenced `$defs` entry. So a class with `"Signatures": {"$ref": "#/$defs/Signatures"}` rendered as literally `Signatures  [  ]`: no group description, and — because the nested members are only walked for `type: object` — none of the member descriptions either. The confidence model was therefore asked to judge values whose meaning it had never been told, which matters most for fields whose *definition* is the whole question (a boolean "is a signature present?" scored 0.85 on a document with no signature). Extraction was never affected — it passes the raw schema, `$defs` included. Subschemas are now dereferenced (chains followed, sibling `description` overriding the definition's, cycles and dangling/remote refs degrading to previous behavior rather than raising) before type/description are read, at every level the formatter walks: property, group member, list `items`, and item column. Mutation-verified by three new tests.

- **The confidence pass ran blind on the default settings: page images were dropped whenever `geometry.mode` was `ocr_only` (the default) or `off`, so a `{DOCUMENT_IMAGE}` placeholder in the configured prompt rendered as an empty `<document-image>` block ([#634](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/634)).** The gate assumed that a confidence pass which is not asked for bounding boxes has no use for the page image — true for text-transcription fields, wrong for every field whose evidence is *visual*: signature / checkbox / stamp booleans, handwriting, struck-through values. Those are exactly the fields most likely to be extracted wrongly, and the image was the only thing that could have caught it; observed on a signature-presence boolean that the model marked `true` on an unsigned form and then rated 0.85, its own reason conceding it was "without being able to visually verify an actual signature mark". It also silently overrode the user's prompt and contradicted the documented behavior (`docs/extraction-and-confidence.md` already said images are attached iff `{DOCUMENT_IMAGE}` is present). Attachment is now decided solely by the prompt, which is also how the extraction path and the sharded agentic path already behaved — the latter deliberately passes images to the confidence call even when the *extraction* prompt has none, only for them to be discarded here. **Cost note:** on a config that keeps `{DOCUMENT_IMAGE}` in the confidence prompt (the shipped default), the confidence request now carries ~1.7K additional input tokens per page; to opt back into a text-only pass, remove the placeholder from `extraction.confidence.task_prompt`.

- **`marketplace.licenseMode` in a feature manifest was rejected by the manifest schema, making the whole manifest→template→install chain dead code as shipped.** The dataclass read it and `pack.py` baked it into the template, but `feature-manifest.schema.json` declares `marketplace.additionalProperties: false` and did not list the new field, so `idp-feature-cli validate` failed with `marketplace: Additional properties are not allowed ('licenseMode' was unexpected)` before the model ever saw the value — and the documented example in `feature-template/feature.yaml` led straight into that error. The schema now lists it with an `enum` of the three values, so a typo fails at publish time rather than being silently dropped at install. Found because a second gap sat behind it: **the token was baked on only one of the two publish paths.** `pack.py` and `publisher.py` each carried an independent `.replace()` chain over the same three `marketplace.*` tokens, and only `pack.py` learned about the third — so a feature published through `FeaturePublisher` shipped a literal `<FEATURE_LICENSE_MODE_TOKEN>` in its template. Both now call one `bake_marketplace_identity()` helper in `manifest.py`, next to the fields it reads. Regression tests cover every enum value through `load_manifest` (which validates), the rejected typo, and every token being baked on the publish path; all three mutation-verified.

- **`registerFeature` told operators to fix "the feature's manifest" for an invalid `licenseMode`, which is wrong advice for any extension that doesn't keep it there.** The host has no idea where the value came from: the bundled SDK bakes it from `marketplace.licenseMode`, but an extension is equally free to declare it as a CloudFormation `Mappings` constant, a stack parameter, or anything else its ui-deployer can read — the host only requires it in the payload. The warning now points at the ui-deployer's source rather than naming a file, and the developer guide documents the payload-only route explicitly, noting that an extension which already keeps the mode elsewhere should **omit** the manifest field: two declarations of one fact is exactly the drift `licenseModeMismatch` exists to catch.

- **Upgrading a commercial stack could wedge in `UPDATE_ROLLBACK_FAILED` on `iam:UpdateAssumeRolePolicy` AccessDenied ([#632](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/632)).** v0.6.2 added the two GovCloud Cognito federated principals to `CognitoAuthorizedRole`'s trust policy unconditionally. They are inert for STS at runtime — only the deploy region's principal is ever exercised — but *not* inert at deploy time: they are part of the trust-policy document, and changing that document on an already-existing role is a separate `iam:UpdateAssumeRolePolicy` call. Any pre-v0.6.2 → v0.6.2+ upgrade under a CloudFormation service role or permissions boundary lacking that action therefore failed mid-update, and the rollback needs the same action, so the stack could not self-recover without `continue-update-rollback --resources-to-skip`. Fresh deploys were never affected (the trust policy is set during `iam:CreateRole`). The GovCloud statements are now gated on a new `IsGovCloudPartition` condition, so commercial renders the byte-identical pre-v0.6.2 document (no diff → no API call) while GovCloud is unchanged. **Recovery for a wedged stack:** `continue-update-rollback --resources-to-skip <StackName>-CognitoAuthorizedRole`, then upgrade to this release. Two follow-ons: the **shipped `IDP-Cloudformation-Service-Role.yaml` now also grants `iam:UpdateAssumeRolePolicy`, `iam:PutRolePermissionsBoundary` and `iam:DeleteRolePermissionsBoundary`** (it granted none of them, so our own documented service role hit this too, boundary or not — and without them any *future* trust-policy or `PermissionsBoundaryArn` change would wedge again); and `scripts/sdlc/validate_service_role_permissions.py`, the CI check that should have caught the gap, **had been passing vacuously** — its IAM comparison crashed on the first `Fn::If`-wrapped inline policy and a broad `except` swallowed the error and returned an empty action set, so "no missing permissions" meant "nothing was compared". It now walks intrinsics tolerantly, fails loudly on an unparseable template, and additionally derives the IAM *control-plane* actions CloudFormation needs to manage the roles our templates declare — not just the actions those roles grant. Covered by `scripts/sdlc/tests/test_iam_trust_policy_partitions.py` (renders every role's trust policy per partition and rejects a GovCloud/China-only principal in a commercial rendering) and `test_validate_service_role_permissions.py`; all mutation-verified.

- **A simulator-backed entitlement check could report `entitlementSource: marketplace-live` — the one source extensions are documented to trust.** `SIMULATOR_SOURCE_TAG` was wired straight to the `FeaturePlatformSubscriptionMode` parameter and reported verbatim, so a stack deployed with `marketplace-live` while `AWS_ENDPOINT_URL_MARKETPLACE_*` pointed at a marketplace simulator labelled simulator answers as a verified live Marketplace check. `entitlementVerified` / `isVerifiedEntitlement()` are true for exactly that source, and the developer guide tells extension authors to use them to decide whether to warn — so an extension following the documented advice was silently fooled by a fake Marketplace. **Which API is called** (the mode) and **where the call goes** (the endpoint override) are independent axes, and only the second decides what the host may claim: the reported source is now *derived* from whether an endpoint override is in effect, so a simulator-backed check reports `simulated` however the mode is set, and no combination of parameters can forge `marketplace-live`. The combination is deliberately **not** refused at deploy time — pointing `marketplace-live` at the simulator is how the buyer-side path is developed; what was wrong was the claim, not the configuration — but the resolver now logs it loudly at cold start, and simulator-backed grants fire the `UnverifiedEntitlementGrant` metric like every other unverified source. Regression tests pin the invariant per override variable, including the global `AWS_ENDPOINT_URL`. **For extension authors:** the `entitlementSource` union is unchanged (`simulated` already existed for this case), so no code change is required — but `marketplace-live` now means strictly "real AWS answered".

- **Every entitlement check on a simulator-backed stack degraded to `advisory`, because the simulator rejects the buyer-side query.** Root cause from the deployed resolver's logs: `ValidationException: unknown filter name: PartyType` — the marketplace-simulator implements a subset of the Agreement API. It also records agreements under the product **code** rather than the product entity id, because its buyer console is keyed on `productCode`, so even a `PartyType`-less query found nothing under `productId`. The resolver now retries without `PartyType` and then under `productCode` — both **only** when an endpoint override is in effect, since real AWS accepts `PartyType` and rejects the reduced filter set outright (`Provided combination of filters is not supported`, verified against a live account, as is `Provided filter values is invalid` for a two-value `ResourceIdentifier`). The production query is unchanged and cannot be silently weakened, and because an override also forces the source to `simulated`, nothing found this way can be reported as verified. Separately, `advisory` diagnostics now distinguish **access denied** from **endpoint unreachable** instead of always blaming IAM, and the resolver warns at cold start when `MarketplaceAgreementRegion` isn't a Region the Agreement API is published in (`us-east-1` only, in the `aws` partition — read from the SDK's endpoint data rather than hardcoded).

- **The feature page rendered three host banners that contradicted each other.** A paid extension in an unverified-but-allowed state showed a yellow "Subscription not verified" directly above a green "Subscription active · Source: advisory", plus a "v0.5.1 — up to date (advisory)" version banner — so the page asserted both "not verified" and "active" at once, which is worse than either alone and made the extension's own honest "no subscription found" panel read as a fourth opinion rather than the answer. There is now exactly one subscription banner: when the grant is unverified it states what actually happened ("Access allowed without a verified subscription · source: `<source>`") and carries the admin Cancel action so the simulator dev loop keeps its off switch; the green banner renders only for a genuinely verified subscription. The version banner no longer mentions the subscription at all — it is about the version. The pre-install screens got the same treatment: `InstallPrompt` / `AwaitingAdminInstall` no longer claim "Your AWS Marketplace subscription is active" off the back of an `auto` / `advisory` / `simulated` grant.

- **OSS extensions no longer show a "Subscription active" banner with a Cancel Subscription button that cannot work.** An open-source extension has no subscription: `checkFeatureEntitlement` short-circuits it to ACTIVE, which made the banner render with `Source: oss` and offer admins an action `unsubscribeFeature` has no way to perform (it targets a Marketplace or simulator entitlement, and there is neither). The banner is now suppressed for OSS, matching the guard already applied to the "Subscription not verified" warning beside it. The feature itself renders exactly as before — this was presentation only, not a gate.

- **Subscribing to a Marketplace extension was impossible before installing it — which is the only time you'd do it.** `subscribeFeature` and `checkFeatureEntitlement` both read `productCode` / `marketplaceListingUrl` **only** from the `InstalledFeatures` DynamoDB row, and that row is written *at install time*. So on the not-yet-installed path the lookup necessarily came back empty: `checkFeatureEntitlement` returned `state: NONE, source: "none"` (the UI showed "no entitlement" with no way forward, even for a genuinely subscribed customer) and `subscribeFeature` raised `SubscribeError` in real-Marketplace mode, so the admin got an error instead of the listing page. `catalog.json` has carried both fields all along; both resolvers now fall back to it, with the install row still winning when present.

- **A Marketplace extension could only ever have worked in one region, and would have failed *after* the template validated.** `getFeatureLaunchUrl` read `sellerBucket` / `sellerBucketRegion` from the catalog literally, so every region resolved to the same bucket. The first-order effect was a wrong-region template URL; the damaging second-order effect was that the template itself is valid, so CloudFormation accepted it and then failed at Lambda creation because the baked `CodeUri` pointed at another region's objects — a confusing mid-deploy rollback for a paying customer rather than a clear up-front error. Now resolved by explicit per-region lookup that fails closed. An empty catalog `latestVersion` also no longer blocks a launch (the template URL is version-free and the version is baked into the published template), removing the last reason the launch path needed a host re-release.

- **The feature page re-derived the "Update available" badge as `latest !== installed`, resurrecting a downgrade prompt the resolver had already fixed.** `listInstalledFeatures` computes `updateAvailable` with proper SemVer comparison and only reports an update when the published version is strictly *newer* — precisely because a feature installed with `idp-feature-cli deploy --from-code` (the documented dev loop) can legitimately be **ahead** of what's published. `FeaturePage.tsx` ignored that field and recomputed with plain inequality, so it told an admin running v0.1.1 that v0.1.0 was "available". It now trusts the server's flag. Also removes the "AWS Marketplace–delivered extensions are a **future capability** — no paid extensions are available yet" copy, which goes stale the moment Auto Optimizer lists, and hides the **Cancel Subscription** button for the `marketplace-live` / `advisory` states — it drives the simulator's admin API, so against a real subscription it could only ever have failed (a real subscription is cancellable only in the buyer's AWS Marketplace console).
...
- **The `ItemType` backfill would not have run on any existing stack, so upgraded deployments would have kept showing test documents in the Document List with no signal why.** Two independent causes. `BackfillCustomResource`'s `BackfillVersion` property was left at `3.0`, and CloudFormation only sends an `Update` to a custom resource when one of its properties changes — so the trigger was never invoked. And had it been, the pre-check short-circuited to `ALREADY_DONE`: it samples one document and skips when `ItemType` and `ConfidenceAlertCount` are both present, which is true on every stack that ran the earlier backfill — precisely the stacks holding legacy test documents. A one-item sample can answer "is an attribute missing?" but not "is its value now wrong?", and retyping is the second kind. The version marker is now bumped to `4.0` (with a note that any change needing a re-run must bump it), and a changed marker starts the run unconditionally; the sample still short-circuits a no-op update, and an empty table still skips entirely.

- **The self-comparison guard never fired on the ordinary path, so a config could still be scored against its own draft labels.** `startTestRun`'s `configVersion` is optional and the draft-labeling modal defaults to "Active configuration", which sends nothing — and `_store_test_run_metadata` only wrote `ConfigVersion` when a version was explicitly passed. Both sides of the comparison were therefore `None` on every default run, the guard was inert, and the run folded into the confidence curve as near-perfect accuracy: the "99% accuracy, measured on this test set" outcome the guard exists to prevent. The runner now resolves the active version *before* capturing config and records that, which costs no extra scan, and pins the same value onto the copier message so the version the run reports is the version its documents were actually processed under. The guard also now checks every labeling job for the set rather than the set's `labelJobId` pointer, since a one-document re-extract moves that pointer and would otherwise un-gate the config that drafted everything else. `_record_confidence_curve` had no test coverage at all; it now has nine cases, including the re-extract case and a fail-open check (an unreadable job list must not silently stop the estimator from ever measuring).

- **A single failed document kept a draft-labeling job RUNNING forever.** The harvest counted any document not in `COMPLETED` as pending, including terminal `FAILED` / `ABORTED` / `REDACTED_SUPERSEDED`. Because every caller that displays a job drives the harvest on a timer, the annotation workspace then re-polled every 5 seconds indefinitely — each tick re-reading the set — and the "labeling in progress" banner never cleared. Terminal documents are now recorded as failed and the job completes without them, with a `failedDocuments` count explaining the gap between labeled and total (symmetric to the existing `skippedAlreadyLabeled`); a job where *every* document failed is reported FAILED rather than complete. A document with no tracking record at all cannot be distinguished from one that has not started, so it is given up on only after the job is six hours old.

- **One blank cell in a table excused the whole column from review.** Absent fields are excluded from `minConfidence` / `alertCount` because a blank W-2 box scores 0.0 legitimately — but the exclusion was keyed on the bare leaf field name, while confidences were matched on the same bare name. A single empty `Description` in a 200-row transaction table therefore excluded *every* `Description` score in the document. Measured on a three-row fixture, a document with two genuinely weak descriptions reported **0 alerts instead of 2**. Both walkers now build path-qualified keys (`Transactions[1].Description`) using the same convention as `curve_store`, so they align per occurrence. This understated review need on exactly the table-heavy documents the feature targets.

- **An Annotator who was also a Reviewer had object scope switched off entirely.** `_assert_annotator_scope` returned early for anyone holding Reviewer, so the `allowedTestSets` check on `claimReview` / `releaseReview` / `completeSectionReview` was skipped rather than intersected — and the two enforcement layers disagreed, since `testset_scope.assert_can_access_test_set` refuses that same caller. Only Admin and Author are exempt now, matching the library's unscoped set. Annotators are assigned by hand in Cognito (the role has no external-IdP mapping), so holding both groups is an easy mistake.
...
- **An unbounded scan loop could hang instead of failing.** `test_runner._resolve_active_config_key` paged with `while True`, which spins forever if DynamoDB ever returns a repeating `LastEvaluatedKey` — and hangs rather than fails under a mocked table, which is much harder to diagnose. Now bounded, and it also stops on a repeated token.

- **Uninstalling a feature that owns a `preprocessing` hook no longer fails every subsequent document.** Making the flat hook points registrable also made them *un*registrable, and that turns out to be load-bearing rather than tidiness. A flat point's hook is invoked **by ARN**, so an uninstalled feature's ARN left behind in the active config version names a Lambda that no longer exists — and the PII Anonymization extension's shipped preset sets `onError: fail`, which makes the dispatcher raise and the workflow land in its terminal `PreprocessingHookFailed` state. Every document processed after the uninstall would fail, with no signal beyond a `ResourceNotFoundException` in the dispatcher log, until an admin hand-edited the configuration. `unregisterFeatureHooks` now disables the section and nulls the ARN when the requesting feature is the recorded owner — the fail-safe end state — while leaving `args` in place so a re-install is a one-field change. The clear is scoped to the owner, so uninstalling one feature cannot disable another's hook. Note this is a **behavior change to feature teardown**: `unregisterFeatureHooks` was previously a no-op for the flat points (it only understood `postHook` lists), which is why the PII Anonymizer's own uninstall path documents its `_unregister_hooks()` call as backward-compatibility only.

- **Stale `preHook` references removed.** The dispatcher's hook-list field was chosen by testing whether the point name started with `pre`, with a documented `<section>.preHook` list that no code path ever read — dead since the `preprocessing` section became a single flat hook. That heuristic would also have mis-read the new `postprocessing` point (flat, but starting with `post`), so flat points are now an explicit set shared by the dispatcher and the registration resolver, and the comments in the PII Anonymizer that still described injecting into `preprocessing.preHook` now describe what the code does.

- **A settings-cache test failed only on CI runners with 50–300s of uptime.** `test_ssm_failure_on_stale_cache_defers_full_ttl` asserted on a *derived age in seconds* (`ttl - (monotonic() - _cache_time)`) after setting `_cache_time = 0.0` as an "expired" sentinel. That sentinel silently reads as "the boot instant", so the derived age was really the runner's uptime — the assertion `> 250` held only while uptime was under 50s (young runner) or above the 300s TTL (warm runner, where the cache genuinely expired and the SSM call was reached), and failed in the band between. Both the reported symptom (`got 8.7s`, on a runner ~291s into its life) and the "passes on re-run" behaviour fall directly out of that window. The test now drives an injected fake clock and asserts on retry **behaviour** — does a second read reach SSM before or after the window? — so it neither reads nor computes a wall-clock age, and passes identically at every simulated uptime from 0.5s upward. Three tests added alongside for the paths the rewrite made testable: the TTL cap, recovery back to normal TTL ageing after a success, and `invalidate()` overriding a pending deferral. (#609)

- **A new lint check (`make check-filtered-scans`) fails the build on any DynamoDB filtered `Scan` that cannot see all its matches.** The defect above has now appeared seven times across the platform with symptoms as unalike as "configuration appears empty", "a list view shows fewer rows than exist" and "pipeline hooks stop firing", so it is now enforced rather than re-reviewed. An AST pass over every Python file flags each `.scan(FilterExpression=...)` whose enclosing function never references `LastEvaluatedKey`, and reports whether the call is bounded by an explicit `Limit` or just the implicit 1MB page. Where a bounded sample is genuinely intended, an inline `# filtered-scan-ok: <reason>` marker suppresses it — the marker requires prose after the colon, so silencing the check leaves a reviewable justification instead of a bare pragma. Crucially it resolves **splatted keyword arguments** — `kwargs = {"FilterExpression": ...}; table.scan(**kwargs)` — which is the form every fix below uses; without that the check saw no keywords at all and skipped precisely the call sites it exists to protect. Two things are deliberately **not** accepted as evidence of paging: a boto3 paginator (it pages the call made *through* it and says nothing about a sibling `table.scan(...)` — that exact trap was live in `user_management`, which paged Cognito while its DynamoDB scan read one page), and a bare `"LastEvaluatedKey"` string constant that is never used (the key must appear as a subscript, membership test, `.get()` argument, or keyword). Wired into `make lint`, `make fastlint` and `make lint-cicd`, and covered by 21 tests in `scripts/sdlc/tests/test_check_filtered_scans.py` that pin both directions (flagged and not-flagged), including a regression test asserting the check catches a revert of the dispatcher fix, and a repo-wide clean assertion.

- **Three more unpaginated filtered scans, found by that check.** All silent, all growth-triggered. (1) **The admin user list was truncated to the first page** — an admin simply saw fewer users than exist, with no error and no indication the list was partial. (2) Worse, `sync_cognito_users_to_dynamodb` built its "already in DynamoDB" email set from one page, so **every user beyond it looked absent and was re-created under a fresh `uuid4`** — duplicating the record on each sync. (3) BDA project deletion's `BdaProject#` tracking cleanup could **miss the very row it was deleting**, leaving an orphaned tracking entry pointing at a project that no longer exists. The one intentional bounded sample in the codebase — `backfill_gsi_attributes` sampling a single `doc#` row to detect whether the backfill already ran — is annotated with its reason rather than changed, since any row answers that question and paginating would cost a full table scan to learn the same thing.

- **The `make test` gate silently skipped the ConfBench extension's test suite.** `feature-platform/confbench-testset/tests` (93 tests, added in #593) was in neither `RUN_ROOTS` nor `QUARANTINE`, so `scripts/run_all_tests.py` **hard-failed its own registration guard** on a clean checkout — meaning the guard that exists to stop new tests being silently skipped was itself blocking the whole gate for everyone. The root is now registered and runs. `test_planner.py` self-skips via `importorskip` unless `huggingface_hub` and `pyarrow` are installed (`ingest/planner.py` imports both at module scope, and they are heavy runtime pins rather than general test dependencies); the other three modules run unconditionally. The gate now covers **46 roots**, up from 45.

- **Transient Bedrock streaming errors were not retried, because the same condition is spelled differently by the streaming API.** `ConverseStream` reports `internalServerException` where `Converse` reports `InternalServerException`, and the retryable-error set was matched case-sensitively, so a transient server-side fault on a streaming call (agent chat, streaming summarization) failed the request on the first attempt while the identical fault on a non-streaming call retried with backoff. Two lowercase duplicates had been added to the set to patch individual instances of this; matching now folds case in both the sync and async decorators, so one spelling per condition covers both APIs. **Behavior change beyond the bug:** the *sync* decorator previously had its own hardcoded list of four retryable codes (`ThrottlingException`, `ModelErrorException`, `ValidationException`) and now shares `DEFAULT_RETRYABLE_ERRORS` with the async one — so every synchronous Bedrock call in the product now also retries quota, service-unavailable, internal-server and read-timeout conditions. Those are all genuinely transient, and the divergence was an oversight rather than a decision, but the effect is that a call which used to fail fast on e.g. `ServiceQuotaExceededException` now spends its retry ladder first: expect higher tail latency in exchange for fewer hard failures.

- **"Update available" no longer offers a downgrade.** `listInstalledFeatures` computed `updateAvailable` as `latestVersion != installedVersion`, so any difference lit the badge — including a catalog *behind* the installed version. An admin running an extension at v0.1.1 was told "Version v0.1.0 is available". That state is routine rather than exceptional: `idp-feature-cli deploy --from-code` (the documented per-extension dev loop) installs a newer extension immediately, while `catalog.json` only refreshes on a host stack create/update, so the catalog is legitimately older for the whole window between the two. Now compares parsed SemVer and requires strictly newer — numerically (`0.1.10` > `0.1.9`, which string comparison gets backwards) and with SemVer §11 prerelease precedence (`1.0.0` is an update over `1.0.0-rc1`, not the reverse). Non-SemVer version strings fall back to the previous inequality check and log a warning, so an unusual scheme over-reports rather than hiding a real update.

- **Test Studio: "N/K completed" where N > K when the S3 test-set folder had drifted past the test set's declared fileCount.** Reported as `85/10 completed` on `realkie-fcc-verified` and `2005/5 completed` on `fake-w2`. The runner (`test_runner/index.py`) wrote `FilesCount = test_set.fileCount` (10, 5) into the run's metadata, but the copier (`src/lambda/test_file_copier/index.py`) listed everything under `s3://<test-set-bucket>/<testset>/input/` — 85 files and 2005 files respectively, because extra objects had been dropped into those folders outside the test-set flow. Nothing capped the S3 listing when `numberOfFiles` wasn't in the SQS message (the copier only capped on the explicit-override path), so the copier wrote all 85 / 2005 files into `Files` and stamped a `doc#<run-id>/<file>` record for each. The status resolver then iterated the polluted `Files` list, counted every one as completed, and divided by the unchanged `FilesCount` denominator → `85/10`, `2005/5`. The runner now always sends `filesToProcess` (defaulting to `test_set.fileCount`, overridable by the user's `numberOfFiles`) in the SQS payload; the copier always caps the S3 listing at that count after a deterministic sort (so repeat runs pick the same subset), warns loudly when the S3 folder exceeds the declared count, and falls back to `numberOfFiles` for messages that were enqueued before the field was added — no regression during the deploy window. New unit tests in `src/lambda/test_file_copier/test_index.py` pin both the cap and the backward-compatible fallback.

- **Test sets are now versioned, provenance-tracked benchmark objects, and can be built from unlabeled documents.** Previously a test set was effectively a folder of files that had to arrive with complete ground truth: creating one required a `baseline/` folder for every input, results were scored against whatever the labels happened to be at that moment, and there was no record of where the documents came from or whether a human had ever checked the labels. Six related capabilities close that gap. (1) **Versioning** — a test set now has a mutable working draft plus immutable published versions (`v1`, `v2`, …) and an *active reference* that scoring follows, the same model as a VCS working tree/commit/tag. Version numbers are allocated with an atomic counter and published items are write-protected, so two concurrent publishes get distinct versions instead of one silently overwriting the other's "immutable" version; the active-reference pointer only ever moves forward, so publishes landing out of order cannot rewind it. (2) **Run pinning** — each test run records the test set version it scored against alongside the config version it already captured, so comparing two runs distinguishes a metric that moved because the *configuration* changed from one that moved because the *ground truth* did. (3) **Draft labeling** — upload documents with **no** `baseline/` folder and run the active configuration over them to generate ground-truth candidates with per-field confidence, then review the weakest first. This runs the documents through the ordinary OCR → classification → extraction → assessment pipeline and harvests the results into the test set's `baseline/` prefix, rather than adding a second extraction path whose confidence semantics could drift from the ones real evaluations report. (4) **Label provenance** — every label is tagged `draft-machine`, `reviewed-human`, `uploaded`, or `synthetic` and surfaced as a chip, with machine drafts never styled like human-verified labels; re-running draft labeling replaces *only* labels that are themselves machine drafts, so reviewed and hand-uploaded ground truth survive a re-run with a newer config. (5) **Confidence-guided review** — the document list shows each document's lowest per-field confidence (the weakest field, not an average that would hide it), colored against the assessment config's own alert threshold rather than fixed cutoffs, and defaults to worst-first ordering. (6) **Membership editing and source provenance** — documents can be removed from a set (input object plus baseline folder, with `fileCount` recounted from S3), and each set records whether its documents were uploaded, synthetically generated, or mixed. Three validation gates that required a `baseline/` folder were relaxed to accept a fully-unlabeled set while still rejecting a *partially* labeled one (which indicates an incomplete upload, not a label-later workflow) — one of them had been making documents-only sets invisible to test-set discovery entirely. **Backward compatible**: pre-existing test sets read as never-published drafts, their runs pin no version, and no backfill is needed. Known limitation: a published version snapshots metadata rather than the label *bytes* — all versions share one `baseline/` prefix, so editing the draft changes what a published version resolves to. True immutability needs an S3 object-version manifest or copy-on-publish, a choice coupled to `DataRetentionInDays`. Live-validated end-to-end on a dev stack: a documents-only test set registered as `unlabeled`, draft labeling took it to `draft` with per-field confidence and thresholds harvested into the baseline (a 14-section document split correctly across sections), and a re-run left a human-reviewed label untouched. `startTestRun` also gained `objectKeys` for running an explicit list of documents rather than the first N. See [docs/test-studio.md](docs/test-studio.md#versioning-test-sets).

- **Review-effort estimator: a measured answer to "how many documents must I review?"** Reviewing an entire test set is expensive and most of that effort confirms labels that were already correct. Test Studio can now estimate the review depth needed to hit a target accuracy from a measured per-set **confidence→accuracy curve** — `P(correct | confidence)` — and returns documents-to-review, the implied confidence cutoff, residual error, effort, an error-burndown curve, and the reliability table itself (`estimateReviewEffort`, Admin/Author). The curve improves from three sources of increasing fidelity: a cross-set prior for cold starts; **each reviewer's verdicts** (a field they changed is an incorrect observation, one they left alone is correct — recorded by `completeSectionReview`, and since review is worst-first these land exactly in the low-confidence range the estimate is most sensitive to); and a **scoring run**, the only source that measures the high-confidence range human review never opens. Curves are keyed by `(test set, config version)` because confidence semantics shift across models and prompts, and are stored as a per-bin count table so folding in observations is an atomic DynamoDB `ADD` — several reviewers finishing at once all contribute instead of overwriting each other. **The estimate always states how much to trust itself**, because a number computed from a generic prior otherwise looks identical to a measured one: every response carries an estimate-confidence state (`prior` / `partially-measured` / `measured` / `unreliable`), a calibration block, and a docs-to-review *range* that tightens as observations accumulate. Two failure modes that would let the estimator certify an inaccurate set are detected explicitly rather than assumed away — an **overconfident** model (wrong *and* confident, so errors sit in the auto-accepted zone worst-first review never visits) and **degenerate** confidence (every field scores alike, so the review order is arbitrary). Notably ECE does *not* detect the second case — a single populated bin is trivially well-calibrated, measured at ECE ≈ 0.0025 in test — so bin coverage is tracked as an independent signal. Either sets `unreliable` and recommends reviewing everything instead of returning an actionable-looking number. The standing mitigation for the first is an **audit sample**: a random spot-check of the high-confidence zone, sized for detection power (~30 samples ≈ 45% chance of surfacing a 2% error rate) rather than as a fraction of review depth, with its cost included in the reported effort rather than excluded to flatter the headline. Implementation reuses `stickler-eval`'s existing calibration machinery — its `ECEMetric` bins *are* the reliability table and fold in without rebinning — rather than fitting a curve independently. Also fixes the aggregation Lambda's DynamoDB policy from read-only to CRUD: the curve write would otherwise fail with AccessDenied and, because the update is deliberately best-effort, fail *silently*, leaving the estimator prior-driven forever with nothing in the logs. See [docs/test-studio.md](docs/test-studio.md#how-much-review-is-enough) and the [module README](lib/idp_common_pkg/idp_common/evaluation/README.md).

- 🔒 **Team annotation: a scoped, collaborative, worst-first queue for ground-truth review.** Several people can now review one test set in parallel from a shared queue ordered lowest-confidence first, so each review removes the most expected error. The queue is a *view over the existing HITL review machinery* rather than a parallel system — annotation reuses the same ground-truth editor, the same claim-to-lock exclusivity, and the same audit trail (`getAnnotationQueue`). A document claimed by another annotator drops out of everyone else's "next in queue" so parallel workers self-partition and self-balance without collisions, while a document you already claimed stays available *to you* so an interrupted session resumes. Documents with no labels at all sort first, since an unlabeled document is the least trustworthy thing in the set rather than the most. Queue responses carry set-level shared progress (reviewed / remaining / claimed-by-others) so every annotator and the set owner see the same picture. **New `Annotator` role, scoped by `allowedTestSets`** — a least-privilege persona for someone onboarded to a single labeling effort, often external. An annotator can read and annotate only their assigned test set(s); they cannot list other test sets, run configurations, publish versions, or review production documents, and `skipAllSectionsReview` is deliberately withheld because accepting labels unseen is a set-owner decision. This makes the "share a queue link with your annotator" flow safe: the link only navigates, and access is gated by the scoped Cognito session and **re-verified server-side on every operation**, so a leaked URL is useless without a scoped login and revocation is a user-management change rather than a URL rotation. `allowedTestSets` is a distinct axis from the existing `allowedConfigVersions`, not a replacement; a user may carry both. Scope enforcement lives in one shared module (`idp_common.testset_scope`) because three separate deploy artifacts apply the same rule and a check that drifted between them would be a privilege-escalation bug. It **fails closed** in every ambiguous state — an annotator with no recorded scope, no user record, or an unresolvable lookup is denied rather than treated as unrestricted, the opposite of the config-version scope path where an `AccessDenied` on the users table silently widens access (a hazard already flagged in that code). Unit tests are written adversarially: they assert the deny direction for cross-set claims, production-document review, unscoped annotators, and non-annotator roles, and were each confirmed to fail when the scope check is removed. See [docs/test-studio.md](docs/test-studio.md#team-annotation-the-scoped-queue).

- **Test Studio UI: an annotator workspace, and one wizard for creating test sets.** The scoped annotation queue, draft labeling, and the review-effort estimator were all built and live but had no interface — this makes them usable, and fixes the creation flow that was the hardest part of Test Studio to learn. **Annotator workspace** (`/test-studio/sets/<id>/annotate`): a worst-first queue rail beside the *existing* ground-truth editor, shared team progress, and "Save & next in queue". Deliberately thin — the annotation surface is the same editor used everywhere else, and the only new UI is the rail, the progress banner, and the queue navigation. Signing in as an annotator lands directly in the assigned set's queue with a single-link navigation and nothing else; several assigned sets get a chooser. **Saves now reach the review API**: `GroundTruthVisualEditor` gained an optional `onSave` prop, and the workspace supplies one that calls `completeSectionReview` instead of the editor's default direct-to-S3 write. That is what finally engages claim-to-lock, the `reviewed-human` tag (so a later draft-labeling run cannot overwrite a correction), the audit trail, and the confidence-curve learning the estimator depends on — all implemented server-side and previously unreachable. The default path is untouched, so every existing caller keeps its behaviour. Opening a document claims it; an already-claimed rejection is treated as a normal race (warn, refresh, advance) rather than a dead end. **Create test set wizard**: replaces four modals reached from two dropdowns whose options read near-identically while doing different things — "Add Test Set → Existing Files / New Upload" versus "Add Documents → From Existing Files / From Upload". The wizard's first step names sources by *outcome* (ready to publish / needs labeling / synthetic) rather than mechanism, and uploads use Cloudscape `FileUpload` with drag-and-drop instead of a raw `<input type="file">`. The Test Sets header drops from **seven** competing controls to three (Refresh, Actions, Create test set), with everything selection-dependent grouped under Actions — including a new **Annotate ground truth** entry so owners reach a queue without hand-building a URL. `TestSets.tsx` shrinks from 1941 to 1260 lines, and the per-modal duplicated state resets (each repeated in both dismiss and cancel handlers) collapse to one `reset()`. **Role plumbing**: `use-user-role` filtered Cognito groups against a hardcoded list that omitted `Annotator`, so a scoped annotator resolved to *no role* and the navigation fallback handed them a document list they cannot read; the group list is now a named constant with a warning comment. User Management gained the `Annotator` persona plus a test-set assignment control, a Test Set Scope column, and a client-side refusal to create an annotator with no assigned set (the server denies every set in that state, so the invite would be misleading). See [docs/test-studio.md](docs/test-studio.md#team-annotation-the-scoped-queue).

- **"How much of this set must a human actually review?" now has a screen.** `estimateReviewEffort` was implemented, live, and unreachable — the **Annotate** button jumped straight to the queue, so an owner committed a team without knowing whether that meant 4 documents or 2,000. A **Set up team annotation** modal now gates that button: three coarse strategies (review the lowest-confidence documents / review everything / accept machine labels as-is) with the documents-to-review count, effort, resulting accuracy and audit-sample size for each, and the measured curve — target-accuracy slider, error-burndown chart, and the reliability table itself — collapsed under *Show the math*. Every number comes from the API rather than being recomputed client-side, so the screen cannot drift from the model behind it. Two deliberate departures from the design mockup: the "rough estimate" caveat is driven by `estimateConfidence` (four states, four banners) rather than hardcoded, because a prior-driven guess and a measured curve otherwise look identical on screen — the one thing this screen must never do; and `recommendReviewAll` disables the worst-first option instead of leaving a recommendation the estimate itself contradicts.

- 🔒 **The review estimator now checks whether confidence can *rank* correctness, not just whether it is calibrated.** Reliability was gated on ECE and bin coverage, and both missed the failure that matters most. Measured on a 100-document W-2 set (~4,000 field observations per arm, identical documents, one config value different): one confidence model scored **ECE 0.032 across 7 bins — passing both gates — with AUROC 0.480**, i.e. *worse than chance* at ranking correctness. All 77 of its errors sat in the top confidence bin, so worst-first review reached none of them and reviewing the lowest-confidence 30% found **fewer** errors than reviewing at random (ECARB@30 of 0.9×). The other arm scored a *worse* ECE (0.054) with **AUROC 0.878** and 58% of errors reachable worst-first. Calibration answers "is 0.9 really 90%?"; discrimination answers "are the wrong answers the low-scoring ones?" — and only the second justifies reviewing a subset at all. `CalibrationHealth` gains `auroc` and `undiscriminating`, computed from the bin counts already stored (no new state), and an undiscriminating curve now sets `recommendReviewAll` and says why. Binning discards within-bin ordering so this AUROC reads *lower* than an unbinned one (0.742 vs Stickler's 0.878 on the same run); that bias is one-directional and therefore safe for a gate — it can call a good ranker mediocre, never the reverse — so Stickler's value remains the one to quote as a metric.

- **Test Studio: annotators can correct a misclassified document, and review is ranked by confidence alerts.** Feedback pass on the annotation workspace. **Class correction.** The document class was free text, so a bank check classified as a bank statement could be renamed but not repaired — the fields under it were extracted against the wrong schema and stayed that way. It is now a dropdown of the classes the document's *own* config version defines (not the deployment's active config, which may have moved on since the labels were written), with **Change class & re-extract** running the document again and blocking until the corrected labels land. Implemented as a new `reextractTestSetDocument` op rather than through `processChanges`, because the write-back is what matters: draft labels are harvested from a labeling job, so a document reprocessed outside a job of its own would extract correctly and never reach the baseline. Starting a one-document labeling job reuses the existing harvest path with its overwrite safety, pruning and curve bookkeeping intact. Which run produced a document's pipeline copy is resolved **per document** from the set's accumulated job records, not from a single per-set pointer: the review key is `{runId}/{filename}`, so resolving the whole set through the newest run would make every document *other* than the re-extracted one look unprocessed — no review key, "not ready to annotate" — the moment one annotator used the feature. For the same reason the harvest advances every in-flight job rather than only the newest, so a full run cannot be orphaned mid-flight by a re-extract. The class is pinned by writing it to the baseline first — classification is skipped for pages that already carry one — which also demotes reviewed sections to `draft-machine`: the one place that happens deliberately, because the harvest refuses to overwrite reviewed labels and would otherwise report success while leaving the wrong-class fields in place. Annotator-reachable, scoped per set via `allowedTestSets`; a wrong class would otherwise block an annotator entirely. **Confidence alerts replace the lowest-confidence score** as the headline and the queue's sort key, matching what human review has always shown in the Document List: a field below its configured threshold is an alert, and whether it missed by 2 points or 40 does not change that somebody must look at it. A count also describes the work as an annotator meets it — a document with eight weak fields is more work than one with a single weaker field — where the single lowest score does not. Absent fields are excluded on the same grounds as `minConfidence` (a blank W-2 box is a correct reading, not an alert); the percentage stays in the cell's popover, and the calibration curve, ECE and AUROC continue to consume per-field confidence unchanged. **Revision history** now works on the annotation path: `EditHistoryTab` renders `_editHistory` from inside the label JSON, but only the direct-to-S3 editor wrote one — a review saved through `completeSectionReview` recorded the reviewer in DynamoDB, where the viewer cannot reach it. The trail is now appended to the baseline with the identity from the Cognito token and a field-level diff against the replaced label, seeded from the *stored* history so a client that does not round-trip it cannot erase prior reviewers' entries. **Clear draft labels** (Admin/Author) discards machine drafts and leaves documents in place, for re-labeling with a corrected config; only labels explicitly tagged `draft-machine` go, so a config retry can never destroy reviewed or uploaded ground truth. Plus: label quality now leads with the estimated accuracy rather than a tier badge (a "Gold" badge reads as a certification the accelerator has not issued), corrected tier colors, per-document review actions moved to the top of the pane, a selectable full queue link, a per-document **Annotate** action, prerequisite guidance linking to Discovery/Quick Start for users who arrive with documents but no configuration, and precise naming for *extraction labels* vs *class labels*. Two guards worth naming: claiming a document is now a conditional write, so two annotators clicking **Claim** at the same moment can no longer both win (the loser is told who holds it and moves on); and a scoring run is refused as a calibration observation when it would be measuring a config against its own output — a set still carrying that config's draft labels would otherwise fold in as near-perfect accuracy and report "measured on this test set" for labels no human ever saw. See [docs/test-studio.md](docs/test-studio.md#team-annotation-the-scoped-queue) and [docs/rbac.md](docs/rbac.md#the-annotator-role).

- **One image pane for both document editors.** `VisualEditorModal` (human review) carried a 240-line inline reimplementation of the shared `PageImageViewer` — its own presigned-URL loader, its own `BoundingBox`, its own zoom/pan/drag handlers, and a line-for-line copy of the auto-centering calculation `PageImageViewer.zoomToField` already contained — so every fix to the image pane had to be made twice. It now uses the shared component: **679 lines of duplication removed** and the file drops from 1,986 to ~1,290 lines. `zoomToField` was already implemented in `PageImageViewer` but exposed only by assigning it onto a DOM ref that nothing read, making it unreachable; a new `zoomToFieldOnFocus` prop turns it into a real capability, defaulted **off** so the ground-truth editor keeps highlighting-without-zooming while human review keeps zooming to the field being edited (losing that in the move would have been a silent regression). Both viewer props are memoized on the `PageIds` *content* rather than object identity, because `SectionsPanel` rebuilds `sectionData` inline on every render and the props feed the viewer's image-load effect. Version pinning for historical runs is unchanged. Prompted by Spencer's review note that maintaining two annotation surfaces means fixing everything twice; the tabs and field rendering were already shared, this was the remaining half.
...
- **Mixed test sets treated already-labeled documents as urgent work.** A set combining generated (or uploaded) ground truth with bare documents was mishandled in three places, all from one confusion: "no confidence" meant both *nothing has been attempted* and *authored, so there is nothing to assess*. (1) **Draft labeling ran on everything** — documents that already had ground truth got inference spent on them and the result thrown away, since the overwrite guard correctly refuses to replace anything not tagged `draft-machine`. Labeling now defaults to just the documents that need it, reports how many it skipped (`skippedAlreadyLabeled`), and refuses a fully-labeled set outright with a message pointing at scoring instead; prior machine drafts stay eligible so re-running on a better config still works. (2) **The annotation queue sorted ground truth first** — both cases collapsed to the same sentinel, so an annotator opening a mixed set was pointed at generated ground truth *ahead of* the genuinely uncertain drafts, inverting the entire point of worst-first. Unlabeled now sorts first and authored ground truth last. (3) **The estimator counted ground truth as reviewable**, inflating `docsToReview` by the already-finished portion of the set. Deliberately *not* changed: generated ground truth is still not recorded as 100% accurate — it is assumed correct, not measured, and recording an unearned `(confidence 1.0, correct)` observation would stuff the top bin of the confidence curve with data nobody observed, which is exactly the overconfidence the estimator exists to detect.

- **A document with any empty field reported 0.0% confidence.** A blank W-2 box scores confidence 0.0 with a reason like *"No locality name found in OCR results"* — a correct reading of an empty box, not a low-quality extraction. Because `minConfidence` took the raw minimum across every field, one blank box dragged the whole document to **0.0%** in Test Studio even when every populated field scored above 0.99, which made every generated test set look worthless. Absent fields (null, empty string, empty container) are now excluded from the headline score; they keep their 0.0 in `explainability_info`, where the per-field detail is still true. The document listing recomputes rather than trusting a stored value, so labels written before this fix repair themselves without re-labeling.

- **The review estimate's confidence cutoff was always null on large sets.** `impliedCutoff` — and the cutoff on every point of the error-burndown curve — came back `null` for any set bigger than the estimator's 500-document sample, because the unsampled documents were padded with the "no confidence" sentinel, which sorts to the front and suppressed the value. The sampled distribution is now extrapolated across the set instead.

- **Draft labeling never advanced from the annotation workspace, so an annotator's queue stayed empty indefinitely.** Reported as "it looks like it's stuck" on a banner reading *0 of 5 document(s) labeled*. The labeling job itself was fine — it finished in ~3 minutes with uploaded ground truth correctly preserved — but the page had no way to show it. Labels are harvested **on read**, so whoever polls is what drives the harvest forward; `TestSetDetail` has that poll loop and `AnnotationWorkspace` had none. The job only progressed at all because the owner-facing detail page happened to be open in another tab, and an `Annotator` — for whom the workspace is the *only* page they can reach — cannot open that page, so their queue would never have filled and nothing would have said why. `getAnnotationQueue` now harvests the set's active labeling job itself and returns `labelJobStatus` / `labelJobLabeled` / `labelJobTotal`; the harvest is best-effort, so a failure is logged and the queue still serves whatever is already labeled. The workspace polls every 5 s while a job runs and shows an in-progress alert — keyed on an explicit tick rather than the labeled count, since a long run reports the same count for minutes and keying on it would stop re-arming the timer exactly when polling is still needed. The "Queue complete — every document has been reviewed" success alert is now gated on the job being finished; it previously claimed completion while labels were still being produced.

- **An Annotator's assigned test sets always showed as "None assigned" in User Management, and re-editing the scope silently revoked it.** The assignment was stored correctly — `list_users` copied `allowedConfigVersions` out of the user record but never `allowedTestSets`, so the Test Set Scope column was empty for every annotator regardless of what was saved. The consequence went past a wrong label: the **Edit scope** modal prefills from that same list, so it opened with nothing selected, and saving sent `allowedTestSets: null`, which genuinely revokes access to every set. An operator who noticed the empty column and tried to correct it therefore could not succeed. Root cause was duplication — four paths (`create_user`, `update_user`, `get_my_profile`, `list_users`) each assembled the `User` response independently and had drifted; they now share one `user_response_from_item()` helper, so adding a third scope axis cannot repeat it. `saveEditScope` also now *refuses* an empty annotator selection rather than only warning about it, matching the create path (the server denies every set in that state, so saving it is never intended). Adds a test suite for this Lambda, which had none, including one that asserts all three read paths return the same keys — the divergence that caused this is invisible to any single-path test.

- **"View Source Document" downloaded the file instead of showing it in the viewer — for generated test sets only.** A test set created by the [Test Set Generator](docs/extensions/idp-data-generator.md) rendered nothing in the document viewer; the browser downloaded the PDF instead. A zip-uploaded set on the same screen worked, which made it look like a viewer bug rather than a metadata one. Two independent causes, both fixed: the generator's `upload_file` call left the object at S3's `binary/octet-stream` default (a browser handed that type downloads rather than renders), and `getFilePresignedUrl` set no response-header overrides, so the stored type was passed through verbatim. The resolver now always sets `ResponseContentType` from the type it already corrects by extension, and adds `ResponseContentDisposition: inline` for the types a browser can display (PDF, `image/*`, `text/*`) — deliberately *not* for spreadsheets and Word documents, which should keep downloading rather than rendering as gibberish in a frame. Overriding at presign time also repairs documents **already** stored with the wrong type, which fixing the uploader alone would not.
...
- **A test set could show "Generating…" forever.** The generator extension writes `GENERATING` onto the host test-set record and its runtime is what clears it, so a runtime that dies mid-run — or an extension that is uninstalled — leaves the status with no owner. Test Studio renders it as an in-progress spinner, and because the state is a database record rather than client state it survives reloads and redeploys. Observed on a dev stack: a set stuck for over an hour, then permanently once the extension was removed. The host resolver now fails a non-terminal test set whose status has gone stale, with per-status windows because the plausible durations differ by orders of magnitude (a synthetic generation legitimately runs for hours; a file copy does not). Deliberately in the host rather than the extension, since the point is to survive the extension being absent. Records written before the status timestamp existed are left alone rather than presumed dead, and the write is conditional so a job that reports in mid-flight is not clobbered. Every writer of a non-terminal status stamps the time — the resolver's two `UPDATING` writes, the file copier, and the generator — without which the corresponding window is simply unreachable. The generator's job record is also seeded with a heartbeat when the processor accepts the job, so a container that never starts (an image pull that fails, an OOM during import) is reapable rather than invisible, and reaping a job now also releases the host test-set record: the job list is not what a user sees spinning. The generator also heartbeats its own job record now, carrying elapsed minutes so a long run says how long it has been going instead of sitting at a percentage — progress previously advanced only as documents *completed*, so a stage that finished none (augmentation, most obviously) was indistinguishable from a dead runtime.

- **`labelState` now corrects in both directions.** It could only ever ratchet upward, so adding documents without baselines to a labelled set left "labeled" asserting complete ground truth that no longer existed. The check is keyed on membership change rather than on the current state, so one probe both promotes an under-stated set and demotes an over-stated one, and a set whose membership has not changed costs nothing.

- **Generated documents were rejected in a loop because the schema forbade what its own field descriptions instructed.** IDP config classes are written for extraction, where the convention for a field the document does not contain is an explicit null — many descriptions say "Output null if not shown". Handing that schema to the generator made the instruction unsatisfiable: a JSON Schema `string` cannot hold null, so the generator obeyed the description, validation rejected the document, the critic bounced it, and it retried until the render stage's budget was gone and the whole batch was lost. Measured on one 11-document run: 119 critic rejections and 138 instances of that single contradiction. The generator's copy of the schema now widens optional scalar leaves to accept null — resolving it in the direction the config author intended rather than rewriting descriptions the accelerator does not own. Required fields stay strict (a null there is a real failure), and objects and arrays are not widened, which would let the generator drop whole structures instead of one value. An `enum` on an optional field gains `null` too, since an enum constrains the value independently of `type` and widening one without the other leaves the field in the same retry loop. Extraction is unaffected.

- **A test set holding complete ground truth could report "Unlabeled" with no estimated accuracy.** `labelState` is derived once, when a set is registered, and afterwards moved only by draft-label harvest and reset — nothing re-derived it when *already labelled* documents were added later, which is exactly what the synthetic generator does, writing documents and baselines straight to S3 for a set that already exists. Observed on a dev stack: 47 documents of real ground truth invisible to the feature whose subject is measured label quality. `listTestSets` now repairs it, deliberately strictly, because overstating is worse than understating: sets owned by a labeling job are never touched (the harvest writes machine drafts under the same prefix and only sets labelState when it completes, so promoting a running — or failed — harvest would put a "Labeled" badge on unreviewed machine output and suppress the "Labeling failed" warning), coverage must be complete by the same helper registration uses (one labelled document out of 47 is not a labelled set), and drafts are recorded as `draft` rather than `labeled`. Probes are capped per call and each result is recorded against the set's file count, so a set is probed once per membership change instead of on every read. Known limitation: baselines added for documents already in the set leave the file count unchanged and are not re-probed until it changes.

- **Dismissing a test-set completion notice did nothing.** The message was stored on the record and rendered by the test-set list, so dismissing it cleared only client state, the next poll re-read the attribute, and nothing ever deleted it — sets announced work from earlier sessions indefinitely. No operation persists one now. Synchronous mutations return the message in their response, which is where a confirmation belongs and is already how the detail page shows reset and clear-draft-labels results; the asynchronous add, whose mutation returns before the copy finishes, is announced by the list detecting the transition and showing one transient message, the same pattern synthetic-generation jobs already use. Values stored before this release stop being rendered on upgrade and are cleared the next time a set is touched.
