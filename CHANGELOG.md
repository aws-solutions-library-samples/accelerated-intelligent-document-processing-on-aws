...
- **Multi-instance detection now also runs on Advanced (agentic) extraction — for sections short enough not to be sharded** ([#772](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/772), option 1). `extraction.multi_instance_detection` used to be skipped whenever agentic extraction was on, so a section that silently dropped records of one class raised no warning in exactly the mode recommended for large multi-page tables. The agentic path is driven by a generated Pydantic model rather than the wire schema, so the probe is now generated onto the transport model for the single-agent call and popped from the result immediately afterwards — before validation and escalation, and before the integrated field assessment is lifted (the probe key is removed from that too) — so only declared fields flow on. Sharded sections are still not probed: per-shard counts have no sound reconciliation (sum double-counts a document spanning a boundary, max under-counts records spread across shards). Measured on a throwaway stack (2026-09-11, 40 paired documents per corpus, every section agentic and unsharded, zero false positives on either corpus, the three-record `paystub_multi_instance.pdf` flagged): accuracy −1.5 points on RealKIE-FCC-Verified (Sonnet 5; 7 better / 11 worse / 22 identical, sign test p = 0.48) and −0.9 on the OCR benchmark (Nova 2 Lite; 3 / 9 / 28, p = 0.15) — the same size as the Simple-mode cost and, unlike it, not statistically distinguishable from zero at n = 40. Tokens: on the OCR benchmark the agent's extraction input grew about 25% (the probe rides in the tool schema and its restatement on every agent turn); on RealKIE the arms differed by −7% with turn-count noise dominating. As in Simple mode, treat detection as a diagnostic to run once rather than leave on. Docs: [extraction-and-confidence.md](docs/extraction-and-confidence.md).
- **Benchmarks: a suite that can answer "is a premium model worth it?" for OpenAI GPT-6 Astra.** `astravalue` compares Claude Sonnet 5 against Astra (~4x the input price) with only `extraction.model` differing, in both simple and advanced mode, at `repeats: 5` so a cost claim is variance-aware. Sizing its documents exposed the fact that makes the suite work: what Simple mode sends is dominated by **page images**, not OCR text — a 300-DPI Letter page is ~11,220 tokens (2550x3300/750, Bedrock's own formula) against ~600 for that page's text, so the pipeline costs ~12,000 tokens per **page** and the Simple-mode ceiling is ~11 pages for Sonnet 5 and ~61 for Astra. Measured live: an 81-page fixture estimated 964,223 tokens and a 164-page one 1,971,931, and *Astra* rejected both. So the discriminating band is 12-61 pages, most of it already covered by existing fixtures; only a difficulty fixture was added, `dense_250` (26 pages, ~304K — 8 columns, 4 interleaved lists, long free text, 15% OCR noise, typed value truth). `astracap` now runs `scale_3200` (66 pages, ~790K) where **both** models fail in simple mode, because that ceiling is itself the finding: Astra moves the limit from ~11 pages to ~61, it does not remove it. `repeats: 5` also exposes Astra's implicit caching, the one mechanism that can close a 4x gap. The suite deliberately does **not** raise the agentic shard cap to unbounded: that 5-page default is a *timeout* guard, so an unbounded arm would measure a Lambda timeout rather than a model — a `wide25` arm answers "buy a bigger model or just widen the shard?" instead. Two coverage gaps in the standard grid are closed at the same time: **Claude Opus 5** had never appeared in *any* model axis in the matrix despite being selectable and the most capable Claude, so no published guidance covered it, and Astra was only in its own suite — both are now in the `extraction_model` sweep, which is what feeds the Configuration Guidance paper's model section. The model-pinned cells live in their own `model_premium_cells` registry rather than `core_cells`, because `core_cells` feeds `core`/`coresynth`/`corefast`/`full`: putting them there inflated the standard grid 37% (133 -> 182 runs) and, on documents over ~11 pages, added cells that are *expected* to fail. See [benchmarking](docs/benchmarking/index.md).

- **Test Studio confidence curves are keyed by revision family, not by profile alone** ([#698](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/698)). A profile's revisions shared one confidence→accuracy curve — right for a prompt tweak, wrong after a model swap or an assessment change, after which the review-effort estimate reported a relationship measured under settings that no longer existed, with no signal. Now the test runner computes the profile's **confidence fingerprint** (a hash of the confidence-relevant configuration: extraction model and sampling, assessment settings) from the configuration it captures and stamps it on the run item; the harvest copies it onto each drafted label; and all three curve sites — the scoring aggregation, the review Lambda and the estimate — key on `curve#<profile>@<fingerprint>` together (keying one site alone would have split scoring and review observations across two keys). Every observation is also folded into the profile's pooled curve and the set aggregate, and the estimate reads revision → pooled → aggregate → prior and **says which it used**: `curveSource` gains `revision`, the API returns `confidenceFingerprint` / `confidenceFingerprintSource` (`drafting-run` | `mixed-revisions`), and the modal note explains a pooled fallback after a model swap. Runs recorded before the stamp contribute only to the pooled curve. No new IAM: the fingerprint rides on records the sites already read. See [Test Studio](docs/test-studio.md#the-curve-is-measured-and-it-improves).
- **Discovery now suggests — and never silently sets — several documents per section when the sample holds more than one record of the discovered class** ([#765](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/765)). Discovery is the only stage that sees the pages while authoring the schema, so it is asked in the same model call, as diagnostic metadata stripped before validation, how many separate complete documents of the class the sample contains (the #753 probe's question: count documents, not pages or repeated headers). Two or more produce a suggestion on the job: the job details page shows *"This sample appears to contain N 'X' records"* with a one-click **Enable several documents per section** action that sets `x-aws-idp-multi-instance` on the class through the normal configuration update and reminds you to migrate baselines; the jobs table shows an *"N records in sample"* badge; the job record and GraphQL carry `multiInstanceHint` (JSON). The setting is never changed by Discovery itself — enabling it changes the result shape for the class, and one sample cannot tell a multi-record class from a packet that should have been split, so the alternative is named alongside. A re-run on a class you already flagged (kept by the #764 carry-forward) says the flag is already on. See [Discovery](docs/discovery.md#samples-that-hold-several-records-of-one-class).
- **Prompt caching: the product now says whether a cache point did anything — per phase and per class.** Cache read and write tokens were metered and priced but never read back, so a `<<CACHEPOINT>>` that was inert (prefix below the model's minimum) or write-only (paying 1.25× with no read ever landing) looked exactly like one that worked. Each section's `result.json` now carries `metadata.prompt_cache` — read / written / uncached input tokens, requests, read share, and one of six states: `caching`, `write-only`, `never-cached` (a cache point reached a supporting model and did nothing), `disabled` (`extraction.prompt_cache: off`, so zero/zero is intended), `no-cache-point` (no marker in the prompt, or a model the client sends no cache points to — Claude reports `cacheReadInputTokens: 0` either way, so the counts alone cannot tell this from inert; for the implicit-caching models, OpenAI GPT-6 Astra and GPT-5.x, this is the healthy state and the report says so rather than implying they cannot cache) or `no-cache-data` (the backend reported no cache units; nothing can be concluded). Measured reads or writes always win over the flag. Shown in the section's Processing Report tab and text report (for *never cached*, naming the model's minimum cacheable prefix and pointing at `idp-cli config validate`), on each phase's subtotal row of the document cost table (derived from the document's metering map, so that level cannot tell inert from never-sent and says so; the extraction knob is only named on the Extraction row), and, because section results are flattened into the `document_sections_<class>` Athena tables, as `"metadata.prompt_cache.*"` columns for per-class queries across documents. Extraction only, escalation calls included. Completes [#780](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/780). See [docs/configuration.md](docs/configuration.md#reading-cache-efficiency-back-per-phase-and-per-class).
- **The document processing workflow has an execution-level timeout: `WorkflowExecutionTimeoutSeconds` (default 6 hours) and a `WorkflowTimeoutsAlarm`.** The state machine had no top-level `TimeoutSeconds`, so any execution that stopped making progress for a reason the per-state guards did not cover (a stalled Map branch, a multi-hour retry backoff, a future unbounded state) could run for the Standard-workflow ceiling of one year, emitting neither `ExecutionsFailed` nor `ExecutionTime` while holding a workflow-concurrency slot — a stack could lose capacity with every alarm reading OK. The bound is a parameter in the General Configuration section (300 s to one year; 21600 default) substituted into the ASL; tripping it ends the execution `TIMED_OUT`, which the existing status-change rule routes to the workflow tracker (slot released, document marked) and which the new alarm on `ExecutionsTimedOut` raises. The default rests on a 30-day measurement of a production stack: median 0.7 min, p90 4.6 min, longest successful run 37 min, and 117 failed runs of ~5 hours that were a single state's Lambda retry storm — roughly ten times the longest success. Be clear about what the default bounds: a hang, a lost callback, or a storm that compounds across states; it does **not** shorten that single-state storm, which the state's own `Retry` policy ends inside 6 hours (lower the bound to about 3 hours if you want it to). The tracker records `WorkflowStatus` `TIMED_OUT` on the document so a timeout is distinguishable from an ordinary failure, and the dashboard's Workflow Executions widget plots timed-out runs. A timed-out execution discards completed work, so raise the bound for very large packets if successful runs approach it. See [docs/monitoring.md](docs/monitoring.md). ([#757](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/757))
- **Cohere Parse (serverless) LambdaHook — the cheapest OCR backend, with table and figure geometry but no confidence scores.** New sample hook `samples/lambda-hook-inference/GENAIIDP-cohere-parse-hook/` integrates the hosted [Cohere Parse](https://cohere.com/blog/parse) API (`parse-v5.0`, `POST /v2/parse`) as an OCR backend via the existing LambdaHook feature — fully serverless, no SageMaker/GPU, at **$1.50 / 1,000 pages** (2.7× cheaper than the Mistral OCR hook, the same list price as Textract `DetectDocumentText`); `pricing.yaml` gains the `GENAIIDP-cohere-parse-hook` entry. ⚠️ **Cohere Parse returns no confidence scores at all**, so unlike the Mistral hook this backend cannot ground extraction confidence in OCR quality — `{OCR_TEXT_CONFIDENCE}` reads `N/A` per line and confidence comes from the assessment model alone; do not tune HITL thresholds against OCR confidence on it. It does return **bounding boxes for tables and figures** (never for text), and the hook attaches each element's normalized box to every line derived from it, which the OCR service already treats as paragraph-level geometry — so tables and figures highlight in the Web UI Visual Editor with no core change. Parse emits tables as **HTML**, which the deterministic table parser (agentic extraction) cannot read, so the hook **converts them to Markdown pipe tables by default** (`CONVERT_HTML_TABLES=false` to opt out; `colspan` expanded, a nested table left as HTML) — without that, every table would silently fall back to pure-LLM extraction and lose the row-completeness guarantee. Also note: images only (no PDF — fine, the accelerator sends page images), 20 MB / 50 MP per image, nine stable languages against Mistral's 170, and a flat 500 requests/minute for trial *and* production keys, hence built-in 429/5xx retry with exponential backoff. Parse is offered on Amazon SageMaker and Cohere's Model Vault but **not** on Amazon Bedrock; a self-hosted variant is noted as a starting point, not shipped. The samples README gains an OCR-hook comparison table (confidence / geometry / tables / languages / price) to make the trade-off explicit. See [docs/lambda-hook-inference.md](docs/lambda-hook-inference.md#cohere-parse-integration).

- **Prompt caching: a per-class warning when a cache point can never engage, and an `extraction.prompt_cache: off` knob.** A `<<CACHEPOINT>>` only creates a cache entry if the prefix before it clears the model's minimum cacheable prefix — 512 tokens on Claude Opus 5 / Fable 5, 1,024 on Sonnet 5 / Sonnet 4.6 / Opus 4.8, 2,048 on Opus 4.7 and **4,096 on Opus 4.6 / 4.5 / Haiku 4.5**; below it Bedrock reports zero cache reads and writes, raises nothing, and bills the prefix at full price on every request. Measured across the shipped presets, 25% of classes never cache at the 1,024 tier and none do on Haiku 4.5, with nothing in the product saying so. `idp-cli config validate` (and the SDK validate operation) now warns per class, naming the class's estimated prefix and the model's minimum; the estimate is chars/4, about ±10% against Bedrock's own count on the 32 surveyed classes, so a class within that band is reported as "may not cache" rather than declared safe. Extraction prompts only (classification, assessment and rule-validation markers are not checked and are unaffected by the knob). A cache write also costs 1.25× and pays back only on a second same-prefix request inside the 5-minute TTL, so a low-volume deployment paid about +25% on the prefix for nothing with no way to decline; `extraction.prompt_cache: off` (also in the Web UI's Extraction settings; YAML's bare `off` parses as `false` and is accepted) now sends no cache points on either extraction path. Docs carry the per-model table with the non-monotonicity called out. Not in this change: a per-class cache-efficiency view in the Processing Report ([#780](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/780) stays open for it).
...
- **OpenAI GPT-6 Astra is now selectable — and it is the first OpenAI model that works for agentic extraction and outside the US.** `us.openai.gpt-6-astra` and `global.openai.gpt-6-astra` are available for OCR, classification, extraction (**including agentic/Advanced**), assessment, summarization, evaluation and Chat-with-Document. Despite the shared `openai.` prefix this is **not** the `bedrock-mantle` Responses API path the GPT-5.x models use: Astra is a plain Converse model, closer in behaviour to xAI Grok, which is exactly why tool use works for it and not for GPT-5.6. The capability gates key on the route rather than the vendor prefix, so no GPT-5.x behaviour changed. 1.05M context window, 128K max output, automatic (implicit) prompt caching that needs no `<<CACHEPOINT>>` marker, and `reasoning_effort` accepting `none`/`low`/`medium`/`high`/`xhigh`/`max` (**not** `minimal`, which is GPT-5.x-only) in place of temperature/top_p, which it rejects with a 400. `global.` reaches **every** Region including all EU and APAC ones — the first OpenAI option usable in an EU deployment — and costs about 10% less than `us.`. **Not supported:** Discovery and Policy Discovery (Astra rejects whole-PDF `document` blocks — rejected by config validation and guarded at runtime), in-Region inference (CRIS-only; the bare `openai.gpt-6-astra` is rejected), `flex`/`priority` service tiers, explicit `cachePoint` blocks, and GovCloud. ⚠️ **Cost:** Astra is the priciest model offered ($11/$55 per 1M in/out on `us.`, $10/$50 on `global.`), and it bills in two context bands — input above 272K tokens roughly doubles the rate, which `pricing.yaml` cannot express, so cost reports under-report such prompts. See [OpenAI Models](docs/openai-models.md#gpt-6-astra-converse).
...
- **`make lint` no longer prints the ~112 advisory cfn-lint warnings; it prints a per-rule count and `make cfn-lint-warnings` lists them.** The cfn-lint gate added to `lint`/`fastlint`/`lint-cicd` is errors-only by design, but it echoed every `W*` finding, and nearly all of them are one false-positive class: an optional parameter with `Default: ""` (such as `LambdaSecurityGroupId` or `BastionHostSubnetId`) "is not a valid `sg-…`/`subnet-…` id", reported without noticing the referencing resource sits behind a `DeployInVPC`-style condition (W1030/W1031), plus a few condition inferences of the same shape (W1028/W1001). Three hundred lines of that hid the one line that matters. The rules are **not** disabled — a genuinely malformed hardcoded id or ARN is still caught and every `E*` finding is always printed in full — but warnings are now summarised as `112 advisory warning(s) not listed (W1030 x88, …)`; run `make cfn-lint-warnings` or set `CFN_LINT_SHOW_WARNINGS=1` to see them. The warnings themselves predate the gate: `template.yaml` carried 41 at the commit that introduced it and 35 now.
- **`LogLevel` now defaults to `WARN` instead of `INFO`.** At `INFO` the accelerator can write S3 presigned URLs, document contents and PII into CloudWatch Logs — the residual risk behind AppSec finding #9 (*TRACE/DEBUG Logging Enabled*), and contrary to the production guidance [well-architected](docs/well-architected.md) has carried since v0.3. The root template, `nested/bedrockkb`, `nested/multi-doc-discovery`, `feature-platform/main-stack-extensions` and the feature scaffold now default to `WARN` (`patterns/unified` already did). The Feature Platform and installable-feature templates spelled the level `WARNING`, so a root `LogLevel=WARN` failed the nested stack's parameter validation; they now accept `WARN` as well, and a test asserts every nested enum is a superset of the root's. Existing stacks keep their deployed value, and the six shipped installable features still launch at the `INFO` pinned in their `feature.yaml` (the host value is not forwarded). `WARN` turns off the per-stage `INFO` progress lines, the Web UI REST API access log (finding API-GW-006) and the dashboard's "executions over latency threshold" widget, which parses an `INFO` log line — see [Monitoring](docs/monitoring.md#loglevel--what-warn-turns-off). `idp-cli deploy --log-level` no longer defaults to `INFO`: omit it to take the template default or preserve an existing stack's value, and `--log-level INFO` is now honoured instead of being dropped as "unset". See [configuration](docs/configuration.md#general-parameters).
- **The shard budget now subtracts the measured prompt overhead, so `context_buffer` is a safety margin on top of what the prompt really costs.** Shard planning budgeted against OCR page text only; the per-request overhead — system prompt, the task prompt with the class schema rendered in, few-shot text, the forced toolSpec, the agent system prompt, tool schema and schema restatement on the Advanced path — was subtracted nowhere and silently absorbed by the blanket 30% buffer. That mis-budgeted in both directions: a prompt-heavy configuration could still hit `Input too long` with the only remedy being trial-and-error on `context_buffer`, and prompt de-duplication (#710's knobs) could never change a shard count because the reclaimed tokens came off a reserve that was already unused. `compute_sizing_plan` now takes `prompt_overhead_tokens`, estimated by the extraction service from what it would actually send (chars/4, like the page text: prompt text, few-shot text, the real tool schema and the restatement; the other agent tool specs are not counted), and reports it in the plan and, for Advanced extraction, the Processing Report. Turning the schema restatement off now frees one schema copy's worth of shard budget. Magnitude on the shipped default model (Sonnet 5): the text-only budget was 18,400 tokens (140,000 usable minus an 89,600 output reserve and a 32,000 image reserve); the shipped presets' Advanced overhead is ~4k–8k, so budgets land near 10k–14k. Processed pages measure ~650–1,060 OCR tokens, so five-page shards (3k–5k) stay page-bound and typical documents do not change shard count; only pages denser than ~2k tokens each could split a page-bound shard. Not done: the shard-boundary benchmark the issue calls for. ([#775](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/775))
- **Two IAM action wildcards are gone: the AgentCore runtime manager and the S3 Vectors manager now grant only the APIs their handlers call.** An AppSec review flagged both roles for granting a service-wide action wildcard on `Resource: "*"`. The data generator's `AgentCoreRuntimeManagerRole` held `bedrock-agentcore:*` — 255 actions, including every `InvokeAgentRuntime*` variant and the whole gateway, memory, browser, code-interpreter and identity surface — where its handler calls five: `CreateAgentRuntime`, `ListAgentRuntimes`, `GetAgentRuntime`, `UpdateAgentRuntime`, `DeleteAgentRuntime`. It now grants exactly those, split into two statements because the IAM service authorization reference declares **no resource type** for `CreateAgentRuntime` and `ListAgentRuntimes` (they must stay on `Resource: "*"`); the other three are scoped to `runtime/*` in the deploying account and Region, the id segment staying a wildcard because the runtime id is service-generated and does not exist at deploy time. The prior suppression claimed an allow-list "breaks create/delete" because `CreateAgentRuntime` transitively provisions endpoints — the service reference declares no dependent actions for any of these APIs, so the caller does not need the endpoint permissions. The bedrockkb `S3VectorManagerRole` enumerated twelve `s3vectors` actions on `Resource: "*"`, including the four **data-plane** ones (`PutVectors`, `GetVectors`, `QueryVectors`, `DeleteVectors`) that only the knowledge base service role needs and this Lambda never calls; it now grants bucket create/get/delete on `bucket/*` in the deploying account and Region and index create/delete on `bucket/*/index/<index name>`, with `ListVectorBuckets`/`ListIndexes`/`GetIndex` dropped as uncalled. The bucket segment is a wildcard rather than the stack-derived name because the handler lowercases and sanitizes the name it is given (`sanitize_bucket_name()`), nested stack names always carry uppercase, CloudFormation has no lowercase intrinsic, and `!GetAtt` off `S3VectorBucketAndIndex` would be circular — this role backs the very Lambda that creates it. The index name reaches the handler unsanitized, so it is exact. A new unit suite (`nested/bedrockkb/src/s3_vectors_manager/tests/`, run by `make test-packages-cicd`) pushes realistic nested-stack names through the sanitizer and asserts the template's granted ARN patterns match the ARNs the handler actually builds — and that the exact stack-name ARN would not have. Both `cfn_nag` W11 suppression reasons and the SRT `LAMBDA-005` entry were rewritten to state what is still unscoped and why, rather than justifying wildcards that no longer exist.
...
- **CSP hardening (AppSec review of v0.6.x): `script-src` no longer trusts every HTTPS origin, and API Gateway hosting finally gets a CSP at all.** Two findings against the shipped Content-Security-Policy. (1) `script-src` carried a blanket `https:`, which admits script from *any* HTTPS origin and makes the directive close to decorative; it is now `'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net/npm/` (the `/npm/` prefix excludes jsDelivr's `/gh/` and `/wp/` proxies). The one surviving cross-origin source is deliberate and load-bearing: `@monaco-editor/react` does **not** bundle Monaco — its loader fetches `https://cdn.jsdelivr.net/npm/monaco-editor@<version>/min/vs` at runtime and nothing calls `loader.config()` to point it at a self-hosted copy, so dropping `https:` outright would blank every editor in the UI (Configuration, Pricing, Model limits, schema preview, page-text editor, document viewer). One pinned origin instead of all of them; self-hosting Monaco out of the Web UI bucket would remove even that, and is recorded as the follow-up. `'unsafe-inline'`/`'unsafe-eval'` are still there pending the same Monaco work. (2) The policy lived only in the CloudFront `ResponseHeadersPolicy`, which is conditional on `WebUIHosting=CloudFront` — so **GovCloud and private-network deployments, which must use `WebUIHosting=APIGateway`, ran with no CSP whatsoever** (threat UI.T07, whose own recommendation this implements). The two S3-proxy methods that serve the SPA document and its assets now send the same policy as a static response header on all three of their status codes (a 404 for a missing asset key is still a response from the SPA's own origin), held once in `Mappings -> WebUiSecurity` of the api-resolvers template. The only intended difference from the CloudFront copy is `frame-ancestors 'none'` rather than `'self'`, matching the stricter `X-Frame-Options: DENY` that mode already sends — a CSP `frame-ancestors` overrides `X-Frame-Options` wherever both are understood, so copying `'self'` verbatim would have quietly *relaxed* framing there. The REST API deployment logical ids are bumped V5 -> V6 so the stage re-snapshots; without that an upgraded stack keeps serving the CSP-less responses. A new guard, `scripts/tests/test_csp_policies_in_sync.py`, compares the two policies directive-by-directive (with `frame-ancestors` asserted as the one intended difference), requires every `script-src` source to name a specific origin, and checks that both S3-proxy methods actually map the header from the shared mapping — the previous sync mechanism was a comment, which is how APIGateway mode went several releases with no CSP in the first place. Threat model updated (UI.T01, UI.T07 now closed, FEAT.T01), plus [API Gateway hosting](docs/apigateway-hosting.md), which claimed that mode sets no security headers at all, and [private-network deployment](docs/deployment-private-network.md), which now documents the browser's jsDelivr egress requirement and the blank-editor symptom when it is missing.

### Fixed

- **The Code Intelligence Agent was enabled by default, so a fresh deployment sent chat queries to a third-party service before anyone asked it to.** The docs have always said *"Code Intelligence is disabled by default for security reasons"*, but every entry point disagreed with them: the Web UI's chat context initialized `enableCodeIntelligence: true` (the checkbox rendered pre-checked, and a session reset re-checked it), and all three backend paths — the `sendAgentChatMessage` resolver, the streaming `/chat/agent` route, and the `agent_chat_processor` handler that does the actual filtering — read the flag with a `True` fallback, so a caller that simply omitted the field got the agent too. The agent registers `https://mcp.deepwiki.com/mcp`, hardcoded, unauthenticated and with no allowlist, and the orchestrator routes any code-related question to it; that means on a stack nobody had configured for it, prompt text left the account with the operator's only signal being an advisory line of UI copy. All four defaults are now `false`, which makes the checkbox the actual consent gate rather than a way to turn something off after the fact, and the three backend paths require a **literal boolean `true`** rather than a truthy value: the streaming `/chat/agent` route reads raw client JSON with no schema validation, where `bool("false")` is `True`, so a coercion would have let any authenticated caller re-enable the flow with a string. Explicitly passing `true` is unchanged, so a deployment that wants the agent keeps it by checking the box — there is no new parameter and nothing to redeploy for. Note what this does **not** change: when it is enabled the endpoint is still hardcoded and still unauthenticated, so the warning about sensitive information continues to apply. See [Code Intelligence](docs/code-intelligence.md).
- **Two test runs of the same test set started within one second no longer collapse into one** ([#879](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/879)). A run id is `<test set name>-<UTC timestamp to the second>`, and the run's metadata was written unconditionally — so two arms of an A/B launched back to back (a warm TestRunner answers in ~300 ms), or two users clicking Run in the same second, got the same id: the second write replaced the first run's metadata and both copier messages staged into the same `<id>/` input prefix, attaching every document to one run. The metadata write is now conditional (`attribute_not_exists(PK)`); on a collision the id's timestamp moves to the next second and the write is retried (bounded), so the run the caller and the copier are told about is the one that exists. The id keeps its shape — the UI and the GSI backfill parse `-YYYYMMDD-HHMMSS` out of object keys — and `CreatedAt` stays exact. The detection A/B harness (`benchmarks/harness/detection_ab_teststudio.py`) additionally refuses to record a launch in which the stack issued one id twice.
- **A test run pinned to a revision that does not exist is now refused at submit, and a missing revision body no longer masquerades as an IAM error** ([#878](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/878)). `startTestRun` accepted a profile that did not exist, or a revision whose body could not be read, with two warnings: the run was stored with an empty captured configuration, the profile and revision were stamped onto every copied document, and each document then failed in OCR — minutes later, forty times over — with `AccessDenied … not authorized to perform: s3:ListBucket`. That error pointed at IAM because every role reading revision bodies had `s3:GetObject` on `config_revisions/*` but no `s3:ListBucket` on the bucket, and S3 reports a *missing* key as 403 rather than 404 without it, so the store's clear "revision is not available" path never fired. Now: the runner rejects a named profile that does not exist (`Configuration profile 'X' not found`) and a revision whose body cannot be read (`Revision rN of configuration profile 'X' is not available (deleted, pruned, or never existed)`) before anything is written or queued; every revision-body reader (eight functions in the unified pattern stack, the TestRunner) also gets `s3:ListBucket` scoped to `s3:prefix: config_revisions/*`, so a missing body is a 404; and a 403 on a body read raises a message naming both possible causes instead of being treated as an unknown error. A new template gate (`scripts/tests/test_config_revision_read_grants.py`) keeps every GetObject-on-revisions grant paired with the scoped ListBucket. The detection A/B harness no longer assumes revision 1 (default: the profile's published revision) and pre-flights each profile's existence. See [Configuration Profiles](docs/configuration-profiles.md#pinning-a-revision-for-processing).
- **The AgentCore Gateway manager Lambda can no longer grant itself IAM permissions.** Its execution role held `iam:CreateRole`, `iam:AttachRolePolicy` and `iam:PassRole` on `Resource: "*"`, so the role could attach any managed policy — `AdministratorAccess` included — to itself: a privilege-escalation path flagged as a High finding by the Security Invariant scan (`iam-role-without-privilege-escalation` scenario 1.1, finding b2b397a9) and confirmed in AppSec review. None of it was reachable. The function is a CloudFormation custom resource that creates one AgentCore Gateway and one Lambda target; the starter toolkit only calls `iam:CreateRole`/`iam:AttachRolePolicy` when `create_mcp_gateway` is given no `role_arn`, and this code always passes `AgentCoreGatewayExecutionRole`'s ARN. The one caller that did mutate a role, `GatewayClient.fix_iam_permissions`, is now not called at all: it rewrote the execution role's trust policy to the same document `AgentCoreGatewayExecutionRole` already declares and put an inline policy for the toolkit's own sample function (`AgentCoreLambdaTestFunction`), which does not exist in this stack. It was already failing silently — the policy never granted `iam:PutRolePolicy` or `iam:UpdateAssumeRolePolicy`, and the helper swallows its own errors as a best-effort warning — so removing the call changes no deployed behaviour and drops a misleading log line. What remains is `iam:GetRole` and `iam:PassRole` scoped to the single `AgentCoreGatewayExecutionRole` ARN, which is what `CreateGateway` needs to hand the role to `bedrock-agentcore`. The role's own permissions are unchanged, so existing gateways keep working and no stack update beyond the template is required. The other roles the same scan flagged are Isengard account baseline roles (`EpoxyAccessRole`, `IibsAdminAccess-DO-NOT-DELETE` and similar), not shipped by this solution. Because the SI scan runs against a *deployed* account rather than a PR, a reintroduction would have shipped and only surfaced on the next scan, so `scripts/tests/test_iam_privilege_escalation.py` now gates it at review time: no template may grant an IAM permission-mutation action (put/attach a policy, create a role or user, rewrite a trust policy, move a permissions boundary, `iam:*`, or a bare `*`) outside three hand-applied deployment-role templates that are allowlisted with written justification and re-checked for staleness, and `iam:PassRole` may not be granted on `Resource: "*"` anywhere. Templates are discovered by walking the tree rather than listed, so a new one is covered the day it is added, and sixteen meta-test cases — including the exact shape b2b397a9 shipped as, a grant hidden in one `Fn::If` branch, and the two shapes the gate must *not* fire on (an `Effect: Deny` on a mutation action, and read-only `iam:GetRole`/`iam:ListRoles`) — pin the rules so they cannot silently weaken. Confirmed against the pre-fix template: all three rules fail on it.
- **Chat-with-Document's config-version scope check no longer treats "cannot evaluate the scope" as "unrestricted".** `_get_user_allowed_config_versions` returned `None` on every failure, and `None` is the value `idp_common.config_scope` reads as *no restriction* — so an unset `USERS_TABLE_NAME` or an event carrying no caller identity silently disabled `allowedConfigVersions` RBAC for **every** caller on an authorization path, with nothing in the logs saying enforcement had stopped. That is the fail-open shape of AUTH.T07 (config-version scope bypass), and it is the defect the pii-anonymizer feature API was deliberately written *not* to have: its `_caller_allowed_versions` raises `ScopeLookupError` on a missing UsersTable, an absent caller identity, or a DynamoDB error, and its callers deny. The chat processor now follows that contract for the two precondition failures — it raises `ScopeLookupError`, publishes an `assistant_error`, returns `scope_unavailable`, and never reaches Bedrock. Both deployed copies are covered (the processor Lambda and the byte-identical vendored copy the streaming Function URL builds from), and `USERS_TABLE_NAME` plus `dynamodb:Query` on the UsersTable and its `/index/*` were already wired on both functions, so the new deny path cannot fire for a correctly deployed stack. Deliberately unchanged: scoping is **opt-in**, so a caller with no UsersTable row, or a row with an empty scope, is still unrestricted — failing closed on *that* would lock out every ordinary user, which is a different bug, not a stricter version of this one. ⚠️ **Not fixed here, and documented inline at the call site:** the DynamoDB-failure path still fails open, because the query names a `SubIndex` GSI that **does not exist** — the UsersTable defines only `EmailIndex`, no writer ever stores a `sub` attribute, and `user_management` keys rows on a random `uuid4`, so the query has raised `ValidationException` on every call since it was introduced and this scope check has never actually enforced anything. Raising there today would deny every chat turn. Closing it requires first giving the processor a resolvable caller identity, which the streaming transport cannot supply: the Function URL is `AuthType=AWS_IAM` reached with Identity Pool SigV4 credentials, carrying no email and no user-pool `sub` (tracked with AUTH.T07 and the companion-chat threat). The failure is now logged at ERROR rather than WARNING so the fail-open is at least never silent.
- **Two state machines carried hardcoded `arn:aws:states:::` service-integration ARNs that Step Functions rejects in GovCloud, and the gate meant to catch them never looked at their directory.** `make check-arn-partitions` iterated a hardcoded list of template paths, so it never scanned `nested/`, `samples/`, `notebooks/`, `scripts/`, `iam-roles/` or `src/lambda/` — 30 CloudFormation templates and 4 Step Functions definitions exist, the list covered 10 and 1. Templates and definitions are now discovered by **content** (anything declaring `AWSTemplateFormatVersion`; any JSON with a `"StartAt"` key) through `scripts/discover_templates.sh`, the same discovery `make cfn-lint` uses, so the two gates cannot drift and a new template anywhere is covered on creation. Running the wider gate surfaced and fixed: the Multi-Document Discovery and Fine-Tuning state machines' six `arn:aws:states:::dynamodb:updateItem` / `s3:putObject` integrations (now `arn:${Partition}:…` with `Partition` substituted, the form the main workflow already used — in GovCloud those states failed with "resource belongs to a different partition"); the Multi-Document Discovery CodeBuild's ECR login host, which spelled out `.amazonaws.com` instead of `${AWS::URLSuffix}` (wrong in `aws-cn`); and the notebook demo Lambda template's KMS principal, CloudWatch Logs service principal and console link. The only exemption is by path, not rule: `scripts/sdlc/cfn/` (the SDLC pipeline's own commercial-account infrastructure, which names a commercial cross-account principal) is skipped with its reason recorded in the Makefile, mirroring the exclusion the Python ARN check already had. See [infrastructure skill](.claude/skills/infrastructure.md).

- **Lending sample: the Payslip's `EmployeeNumber` and `PayrollNumber` are now extracted reliably.** On `samples/lending_package.pdf` the payslip is an ADP-style earnings statement whose only identifiers sit under a `CO. FILE DEPT. CLOCK NUMBER` header row (`ABC 126543 123456 12345 00000000`) - there is no "Employee Number" or "Payroll Number" label anywhere on the page - so the shipped descriptions ("unique identifier assigned by the employer", "payroll batch or sequence number") gave the model nothing to map them to: measured on the real page with Sonnet 5, `PayrollNumber` was usually null and `EmployeeNumber` moved between the CLOCK and NUMBER columns from run to run. The `lending-package-sample` and `lending-package-sample-govcloud` presets now name the alternative labels and, for ADP statements, the column each field is: `EmployeeNumber` is the FILE value (ADP's per-employee file number) and `PayrollNumber` is the NUMBER value (the check/advice number for the payment). With that text the same probe returned FILE and NUMBER in 5 of 5 runs. Note this is the reverse of the values the original report expected (`00000000` / `126543`), which do not appear as ground truth anywhere in the repository. Config-only. A stack upgraded to this build whose active profile is still `default` receives the new text through the stack-deployment revision; a stack on another profile re-applies the preset or edits the two descriptions in the Web UI. ([#839](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/839))

- **A model that caches implicitly is no longer told it cannot cache.** The prompt-cache report's `no-cache-point` state said *"the prompt has no `<<CACHEPOINT>>` marker or the model does not support prompt caching"* — in the section report, the Web UI, and the docs. The second clause is false for three selectable models: OpenAI GPT-6 Astra caches **implicitly** on Converse (measured: a repeated 2,707-token prefix billed `inputTokens=2` / `cacheReadInputTokens=2707`) and in fact *rejects* an explicit cache point, and GPT-5.4/5.5 cache automatically over ~1,024 tokens. (GPT-5.6 Sol/Terra/Luna are deliberately not in that set: their caching is explicit, driven by a `<<CACHEPOINT>>` marker the client translates into a Responses-API breakpoint, so for them "no marker" really is "no caching" and adding one is the right remedy.) A user selecting any of them saw, on a cold first document, a message implying caching was unavailable — and the obvious reaction to that reading (add a `<<CACHEPOINT>>` marker) is exactly wrong for Astra. The six states are unchanged, so nothing in Athena or the cost table moves; only the wording, which now distinguishes "we sent no cachePoint" from "this model cannot cache" and names implicit caching where it applies. Warm runs were already correct: measured reads win, so the state becomes `caching` once a prefix repeats. See [docs/configuration.md](docs/configuration.md#reading-cache-efficiency-back-per-phase-and-per-class).
- **The benchmark harness now runs the reference corpora instead of silently measuring 7 of 9 documents** ([#766](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/766)). `run_matrix.py` launches each reference corpus (RealKIE-FCC-Verified, the OCR benchmark, `bank_real`) as a test set through the TestRunner — `numberOfFiles` = the corpus's `n`, config version = the suite's cell built onto the corpus's **own** base config (`make_configs.py --class realkie` / `ocr_bench`, same `--set` overrides) — polls until every document of the run has finished rather than the first, and `aggregate.py` expands each run into one row per document (`sub_doc`) scored by the existing `analyze.score_reference`, so a cell's roll-up averages over the real corpus. A corpus whose cell index is not built is reported with the exact `make_configs.py` command and recorded as `docs_unlaunchable`; a runner rejection (test set not on the stack) is printed instead of swallowed. `doc_matrix.yaml` reference entries gain an explicit `class`. `core_docs` now means all nine of its documents.
- **A LambdaHook that took longer than 60 seconds was abandoned and re-invoked, re-running whatever it charges for.** The Bedrock client is given a deliberate 300s read timeout, but the Lambda client used for `LambdaHook` invocations was built with no `botocore.Config` at all — so it inherited boto3's defaults: a **60-second read timeout**, botocore's own retries, and a 10-connection pool. A hook is allowed to run for its whole Lambda timeout (up to 900s) and is explicitly documented as free to call a slow third-party service, so any hook slower than a minute hit the read timeout, and *two* retry layers then re-invoked it — starting the same paid work again while the first invocation was still running, and never making progress. Measured with the new Cohere Parse hook, whose hosted API takes ~75s per page at the median: one 5-page document produced **44 hook invocations and zero completed pages**. The client now waits 840s, sizes its connection pool for the OCR worker fan-out, and leaves retrying to `_invoke_lambda_hook_with_retry`, which backs off, logs, and knows which errors are worth another attempt — botocore retrying underneath it duplicated third-party spend invisibly. 840s rather than the full 900s Lambda maximum deliberately: the *calling* function is itself capped at 900s, so waiting the same would let the caller be killed before botocore ever raises — and Step Functions would then retry the whole OCR task, re-running every page's paid call. The 60s of headroom means the timeout surfaces as an error inside the caller, which can log it and fail the page cleanly; a hook is expected to keep its own timeout, and therefore its internal retry budget, below that. Fast hooks (Mistral OCR, Chandra) were unaffected, which is why this went unseen.

- **A LambdaHook OCR backend with geometry but no confidence scores reported every line at confidence 0.0.** `textractBlocks` treats `Confidence` and `Geometry` as independently optional, but the confidence-table builder defaulted a missing score to `0.0` — so a hook returning boxes without scores told the assessment model, through `{OCR_TEXT_CONFIDENCE}`, that every line on the page was maximally unreliable, suppressing extraction confidence document-wide. That is worse than returning no blocks at all. A LINE with no `Confidence` is now rendered as `N/A`. Latent until now: Textract and the Mistral hook always supply a score; the Cohere Parse hook added in this release does not.
- **The review-effort estimate now reads the curve for the set's own configuration instead of the set-wide blend.** Curves have always been recorded per Configuration Profile, but the estimate keyed its read on a `boundConfigVersion` field that nothing ever wrote, so every estimate fell back to the set's aggregate curve — a mixture of every configuration the set had been labeled or scored under — and could still report `measured` and a **gold** tier. The configuration is now resolved from the `configVersion` argument, then the set's declared `configVersion` (written by the confbench and synthetic-data set creators and already what the UI treats as the set's configuration), then the bound field, then the set's draft-labeling jobs (every job's run carries the `ConfigVersion` the runner resolved; if the jobs disagree, as after a one-document re-extract under another profile, the combined curve is used and labeled `mixed`); re-extraction and version snapshots use the same resolution, so the snapshot's `configVersion` is no longer always null. When the per-configuration curve is empty the aggregate is still served, but the estimate now says so: new `curveSource` (`config` | `aggregate` | `prior`) and `configVersionSource` (`argument` | `test-set` | `bound` | `drafting-run` | `mixed`) fields on the API, and a note under the confidence banner in the review-effort modal. Upgrade-visible: a set whose estimate used to blend configurations now reads a thinner, honest curve and can drop out of the gold tier. Keying by revision fingerprint remains #698. ([#759](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/759))
- **Metering no longer breaks on Bedrock's new structured `usage` members.** Bedrock's Converse `usage` block now includes `cacheDetails`, a list of per-TTL prompt-cache write breakdowns. The whole block was spread into each call's metering, so every subsequent merge summed an integer with a list, logged `unsupported operand type(s) for +: 'int' and 'list'` on each document, and stored the list where a count belongs; the cost report then recorded a placeholder quantity of 1.0 for it (at $0, since the unit has no price) and the Web UI showed a spurious `cacheDetails` row. Only numeric usage members enter metering now (the same for the LambdaHook route). Nothing is lost: the cache read/write token counts that pricing uses are separate numeric members and are kept.
- **Advanced extraction: the class's root description and nested group descriptions now reach the tool schema the agent sees.** The Advanced (agentic) path hands the model a tool whose input schema is generated from a Pydantic model, and that generation dropped every object-level `description`: the class's root description on every class, and `$defs` group descriptions with it (lending `Payslip` kept 34 of 37 descriptions; four of the six lending classes, whose only guidance is a root description, sent none at all). It was masked because the task prompt still carries a prose copy of the schema, and would have become a real loss the moment that copy is reduced (#710). The generator now emits each object's description as the generated class docstring, which Pydantic places on the wire schema, and both wrapper subclasses carry it; a test asserts description parity for every class in every shipped preset (one gap remains: a description on the *items* of a scalar array has no home in a plain string list and is still dropped — the array field's own description survives; see the schema README). The tool schema grows by the length of those descriptions (all inside the prompt-cache prefix). ([#836](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/836))
...
- **Confidence scoring of long lists on Nova Lite no longer pays a 60-second, 10,000-token loop per document.** Every 100-row bank statement scored with the default confidence model (Amazon Nova Lite, temperature 0) truncated its first 25-row batch at the model's 10,000-token cap; the adaptive splitter recovered the rows, but only after the wasted call. Replaying the exact inputs offline showed the cause: asked to score 25 near-identical rows, the model emitted the same `{"confidence": 1.0}` row object 189 times until the cap stopped it (4/4 replays; it still looped with no page images, with no OCR text, and in 2 of 3 replays with no text-confidence block), while 13 rows looped 1 run in 5 and 8 rows never — the token math allows 41, so no token constant expresses it. Two guards: Nova Lite/Micro now score at most **12 rows per call** (a measured per-family ceiling under both the derived size and `list_batch_size`; Nova Pro/Premier/Nova 2 are unmeasured and unchanged), and a confidence call on those two models requests an **output budget** (one leaf per scalar or list cell plus overhead; floor 2,000, cap the model maximum) instead of the model's full cap, so a degenerate response is cut off at ~2,000–4,600 tokens and recovered by the existing splitter; every other confidence model, including the escalation model, still requests its maximum output. No configuration change; `list_batch_size` keeps its meaning as a ceiling. (#861)
- **Federated users can sign in more than once on new deployments: `ExternalIdPEmailMutable` parameter.** Cognito re-applies the identity provider's attribute mapping on every federated sign-in, and the pool's `email` attribute was declared immutable, so a SAML/OIDC user could sign in exactly once — the second attempt was rejected with `user.email: Attribute cannot be updated` and IdP-driven role changes never reached an existing user. An unconditional `Mutable: true` had been shipped and reverted because a Cognito schema flag is fixed at pool creation and the change wedged every existing stack's update in `UPDATE_ROLLBACK_FAILED`. The flag is now gated on a new parameter in the *External Identity Provider (Federation)* section, defaulting to the historical `false` so existing stacks are untouched; **set it to `true` when creating any stack that uses an external IdP** — `idp-cli deploy` does so automatically on stack creation when `ExternalIdPType` is set. It must never be changed on an existing stack. Because a mutable `email` could otherwise be rewritten by a native user's own token (and the API keys config-version scope on the token's email claim), pools created with the flag also require verification of the *new* address before a self-service email change takes effect. The CLI and SDK refuse an update that tries to change the flag, since that is the wedge, and drop it on a create against a template that predates it. Known limitation on a mutable pool, documented: config-version scope is keyed on email and a missing user row means unrestricted, so a scoped *native* user could widen their own scope by verifying a fresh address through a direct Cognito API call — bounded to visibility, no role gain; keying scope on the immutable `sub` is the follow-up, and is why the flag stays opt-in for federated deployments. Existing federated deployments still need a new pool (new stack) to get the fix; the docs describe the bridge and the `sub` re-keying caveat. The live federation check (`make verify-idp-federation`) now asserts repeat sign-in on a mutable pool instead of recording a NOTE. See [docs/external-idp.md](docs/external-idp.md#federated-email-attribute-mutability). ([#835](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/835))
- **A confidence alert on a batched list row now names the row's position in the document, not its position inside the model call that scored it.** Batched assessment slices a large list across several inferences and each call enumerates only the rows it was handed, so its alerts said `transactions[6]` for what is row 16 of the merged list — mislabeled for a reviewer, and colliding with the genuine `transactions[6]` from the first slice (the dedupe fix in #815 deliberately left indexed alerts alone for exactly this reason). The stored list is a derived copy of `explainability_info` (#821), so it is now built as one: after the batches are merged — and, on the integrated path, after the missing-row retry has spliced its rows in — the surface is regenerated from the final merged assessment by the shared threshold enrichment, whose enumeration of the full list makes every row index global by construction. The retry path's slice-relative alerts stop mattering because they are no longer part of the surface. Alerts on rows regenerate at the per-cell grain (`transactions[16].date`), matching the standalone path's per-sub-field alerts. Because the regeneration runs on the single-call branch too, the whole standalone (`separate`) alert surface is now derived this way, with two edge differences from the per-call emission it replaces: a non-numeric or missing confidence no longer produces an alert (it was coerced to 0.0 and alerted), and nested groups are walked to any depth rather than one level. ([#813](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/813))
- **A page image over 3.75 MiB no longer fails the whole document.** Amazon Bedrock enforces its 5 MiB per-image limit on the **base64-encoded** payload, so the effective budget for a stored page image is 3.75 MiB (3,932,160 bytes), not 5 MiB — and nothing checked it: with the default `image.target_width`/`target_height` (empty = no resize) any page above that size failed extraction with a hard `ValidationException: image exceeds 5 MB maximum` and the document went to `FAILED`, with four of the eleven failures on a shipped 293-document corpus (3.8%) coming from source images comfortably under 5 MB. Every Bedrock image attachment now passes through a fit that downscales an oversize image proportionally, in as few passes as possible (same format first, JPEG only if a lossless format still will not fit), and the Converse API's 8,000 px per-side cap is enforced the same way. The reduction is logged and, for extraction, recorded per page in the section's `metadata.image_downscale` so a page the model saw at lower resolution than stored is auditable. The encoded-size arithmetic and the corpus boundary from the report are unit-tested; the docs now state the effective budget next to the image settings. See [docs/configuration.md](docs/configuration.md#effective-per-image-budget-375-mib-enforced-post-base64). ([#778](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/778))
- **A Simple-mode extraction that returned a fraction of a long list no longer reports success silently, and the too-large-section failure now says what to change.** With over-splitting fixed (#726), a Simple-mode section is one request: an 800-row / 17-page statement returned 43 rows with `COMPLETED` and no processing issue (the population check passes because the list is non-empty), and 25+ pages failed with Bedrock's bare *Input is too long for requested model*. A new warning, `extraction_rows_below_ocr_estimate`, compares the rows extracted for a list of objects with the rows in the section's OCR tables of the same shape (column count equal to the list item's property count; fewer than half, at least 30 rows), so a second table of another shape, a form's key/value blocks, a footer with pipes or a list of scalars never count against it; it needs OCR that emits Markdown tables, i.e. Textract `TABLES` or BDA; lists of the same shape are judged as one group, and an array of instances is compared through its inner lists. The failure is now raised as `ExtractionInputTooLarge` from Bedrock's error with the estimated request size, the model's window and the remedy (Advanced extraction, which shards, or splitting the document) in the Step Functions cause and the extraction log; it stays unretried. Simple mode deliberately does not shard — that is Advanced mode's capability. (#843)
...
- **Thirteen Lambdas had no log group of their own, so their logs were kept forever.** A function with no `LoggingConfig` logs to the group Lambda auto-creates, and Lambda's auto-create sets **no retention policy** — so nothing ever expired and the storage was billed indefinitely. Affected the six UI-facing feature-platform resolvers (`listInstalledFeatures`, `listCatalogFeatures`, `getFeatureLaunchUrl`, `checkFeatureEntitlement`, `subscribeFeature`, `unsubscribeFeature`), which are hit on every page load; `BatchPreProcessorFunction`, whose volume scales with documents through the Jobs API batch path; and **the five Lambda hook functions in `samples/lambda-hook-inference/`** (nine declarations — four are declared both in the aggregate template and in their own sub-template), which the pipeline invokes once per page or document, so their volume is the highest of the set; and the OIDC verification fixture's `IdpFunction`, so an interrupted `verify-idp-federation` run cannot leave a never-expiring group behind. Each now has a log group at `/${AWS::StackName}/lambda/<FunctionLogicalId>` honouring `LogRetentionDays` (default 30, newly added as a parameter on the sample stacks) and, where the stack has one, the customer-managed KMS key; `LogRetentionDays` is now also forwarded to the feature-platform nested stack, which never received it. Two smaller fixes came out of the same sweep: `CognitoUserPoolEmailDomainVerifyFunctionLogGroup` was missing its function's `Condition`, so it was created as an empty group on stacks where the function does not exist; and `idp_sdk`'s teardown now removes the auto-created groups these functions leave behind on stacks that predate this change. **Lambdas that run only during a CloudFormation stack operation are deliberately left as they are** — custom-resource handlers and the feature-platform install hooks (`RegisterFeature`, `RegisterFeatureHooks`, `ApplyFeatureConfigPreset`) are very low volume and log nothing but stack operations, so indefinite retention on an auto-created group is an accepted cost. ⚠️ **The old auto-created `/aws/lambda/<fn>` groups are not deleted by a stack update** and remain with no expiry until removed by hand or by an `idp_sdk` teardown. A new gate, `scripts/tests/test_lambda_log_groups.py`, holds every template in the repo that declares a Lambda to four rules — a `LoggingConfig` that resolves to a real log group in the same template, a non-null `RetentionInDays`, no `LogGroupName` referencing a function resource in any intrinsic form (the [#818](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/818) defect), and `Condition` parity between a group and its function — plus a meta-test that fails if a new template with Lambdas is not listed, and meta-tests pinning the bypasses that earlier revisions of the gate accepted — found across three review rounds, including `RetentionInDays: !Ref 'AWS::NoValue'` (which cfn-lint does not flag either) and an `Fn::If` branch yielding `AWS::NoValue`. ([#826](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/826))
