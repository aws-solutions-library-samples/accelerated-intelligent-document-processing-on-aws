...
- **Deleting a version that is not there no longer moves `VersionCount`.** `delete_document_run` decremented the document's `VersionCount` unconditionally, so a retried mutation, a double click, or an unknown `run_id` through the API drifted the counter below the number of runs actually recorded — and nothing recomputes it. The delete now asks for the deleted item back (`ReturnValues=ALL_OLD`) and decrements only when there was one, returning `False` otherwise; `DynamoDBClient.delete_item` gains an optional `return_values` for that.
