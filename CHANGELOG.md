...
- **The review-effort estimate now reads the curve for the set's own configuration instead of the set-wide blend.** Curves have always been recorded per Configuration Profile, but the estimate keyed its read on a `boundConfigVersion` field that nothing ever wrote, so every estimate fell back to the set's aggregate curve — a mixture of every configuration the set had been labeled or scored under — and could still report `measured` and a **gold** tier. The configuration is now resolved from the `configVersion` argument, then the bound field, then the set's draft-labeling run (`labelJobId` → the run's resolved `ConfigVersion`); re-extraction and version snapshots use the same resolution, so the snapshot's `configVersion` is no longer always null. When the per-configuration curve is empty the aggregate is still served, but the estimate now says so: new `curveSource` (`config` | `aggregate` | `prior`) and `configVersionSource` fields on the API, and a note under the confidence banner in the review-effort modal. Upgrade-visible: a set whose estimate used to blend configurations now reads a thinner, honest curve and can drop out of the gold tier. Keying by revision fingerprint remains #698. ([#759](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/759))
