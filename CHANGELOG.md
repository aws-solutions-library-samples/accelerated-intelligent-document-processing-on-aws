...
- **The type gate now checks calls into the shared `idp_common` library, and three statements that raise on every execution have been fixed** ([#1109](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1109)). `basedpyright` honours `PYTHONPATH`, and neither `make typecheck` nor either CI set one; with `reportMissingImports` configured `"none"`, `idp_common` did not resolve, so the checker knew no signature in the shared library and could produce no diagnostic about any call into it — the boundary nearly every Lambda and script uses to reach this solution's core. The gate read all 1,314 tracked `.py` files, reported a count matching `git ls-files` exactly, exited 0, and could not have caught a signature change at any call site. `pyrightconfig.json` now carries an `extraPaths` naming the five first-party package roots, so resolution is a property of the configuration rather than of how the gate was invoked; the entries are **relative**, which pyright resolves against the directory holding the config, so they cannot resolve against a different checkout — an absolute path can, and an environment-level fix that does so produces confident wrong answers rather than silent ones. Eleven errors were behind it and all eleven are fixed rather than baselined, so the gate is at zero with resolution live and no new suppression list was added. Three were live faults: the rule-validation orchestrator's failure handler named a `Status.ERROR` that is not in the enum and passed an `error_message` keyword that `update_document_status` does not accept, so a document whose rule-validation orchestration failed had its status left untouched and the error text discarded — it now records the failure through `update_document`, which persists it; the configuration updater called `set_bda_project_arn` without the ARN it requires, so `bdaSyncStatus` was never set to `needs-sync` and the resulting `TypeError` surfaced only as a warning log — it now calls `set_bda_sync_status`, which writes just that attribute; and `scripts/discover_model_limits.py` constructed `BedrockClient(region_name=...)` against a parameter named `region`. One was a declaration defect in the library: `ConfigurationReader.get_merged_configuration`'s overloads omitted the default on `as_model`, so both callers relying on the documented dict default matched no overload at all. `scripts/tests/test_pyright_config.py` fails on the specific combination that made this invisible — `reportMissingImports: "none"` together with unresolvable first-party packages — asserts the search paths stay relative and inside the repository, and confirms resolution by **running** basedpyright against a throwaway project rather than by reading the JSON, with a negative control so the probe cannot pass vacuously. `reportMissingImports` stays at `"none"`, measured rather than assumed: raised to `"warning"` it reports 44 findings over 25 modules and none is first-party.
