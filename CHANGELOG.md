...
- **CloudWatch alarms now reach somebody: the `AdminEmail` address is subscribed to the alerts topic** ([#922](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/922)). All 12 alarms in the stack publish to one topic, `AlertsTopic`, and no `AWS::SNS::Subscription` existed anywhere in the repository — the topic ARN was emitted as the `SNSAlertsTopicARN` output and an operator was tacitly expected to subscribe by hand, so a default deployment raised every alarm to an audience of nobody. This is the third instance of the same silence in this subsystem, after the non-existent metric name ([#746](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/746)) and the KMS grant that was gated on the circuit breaker; `scripts/tests/test_alerts_topic_subscription.py` now pins that the topic has a subscriber and that no alarm publishes to a topic with none. ⚠️ **An SNS email subscription delivers nothing until the recipient clicks the confirmation link** — a second email, *"AWS Notification - Subscription Confirmation"*, arrives at `AdminEmail` and the subscription stays in `PendingConfirmation` until it is followed, without failing the deployment. `--headless` deployments have no `AdminEmail` and so get no subscription. See [Who receives the alerts](docs/monitoring.md#who-receives-the-alerts) for the status check and for subscribing a distribution list, AWS Chatbot or a pager instead of one mailbox.
