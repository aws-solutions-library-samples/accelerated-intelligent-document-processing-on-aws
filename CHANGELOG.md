...
- **Extraction: advanced mode can stop sending the document schema three times.** Two of the three copies are the same JSON string — 2,600 of 6,680 schema tokens on the lending `Payslip` class. `extraction.agentic.restate_schema_in_system_prompt: false` drops the duplicate. ⚠️ **It defaults to on, and is an A/B knob rather than a recommendation:** restating a schema in prose often improves adherence, so measure completeness on your own documents first — a token saving that loses list rows is a loss. The real benefit is context-window headroom, not dollars, since all three copies sit inside the prompt cache. **Configuration → Prompt Preview**'s System Prompt tab shows the duplicated block and counts it, so the saving is visible when you switch it off — labelled as an approximation, because the browser renders it from your class schema while the backend generates it from a Pydantic model and the real block is larger ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).
...
- **The evaluation report view now shows everything the markdown report does.** It shipped covering the headline scores, the per-field tables and the methods list, and left the rest behind a **Markdown report** button — so in practice it was a summary with a click-through, which was not the intent. Added, in the markdown's own order: the document-split figures in the summary tiles and a **Section split analysis** table (which expected section matched which predicted one, page order, unmatched predictions, split errors, the graded packet score); **All metrics** with the rating the report applies to each; excluded sections listed with their reasons and pages, not just counted; a per-section score in each section header, a **Section metrics** table, and the not-evaluated state with the same how-to-fix steps the markdown gives per failure type; the **Confidence** and **Weight** columns; nested field comparisons as expandable rows under an aggregate attribute, so a Hungarian-matched list shows which item was paired with which; and **How scores are computed**, condensing the methods, array-matching, weighting and split-metric explanations. Nested values are no longer flattened to one truncated JSON string — a cell says "3 items" or "5 fields" and opens the structure on demand. A parity test reads the generator's section headings out of `models.py` and requires a counterpart in the component, so the two cannot drift apart again silently.
