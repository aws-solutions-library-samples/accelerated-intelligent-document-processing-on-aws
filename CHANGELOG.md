...
- **`metering` table partitioning: write time, not queue time.** `save_reporting_data.save_metering_data` now partitions on `datetime.now(UTC)` at write time — which for metering is document-completion time, since the writer runs at workflow end. Every metering row lands in the current partition, so the trailing re-materialization window that earlier drafts required is unnecessary. `initial_event_time` is preserved as a column for consumers who need queue-time semantics. **Semantic consequence for consumers of raw `metering`:** the predicate `WHERE date = '2026-08-18'` shifts meaning from "docs queued that day" to "docs completed that day" — both known consumers (Monitor dashboard, in-tree analytics agent) benefit from the completion-time interpretation, and consumers who need queue-time filter on the `initial_event_time` column directly. `metering` also gains an `hour` partition key so the current-hour tail query in the tier picker can partition-prune to ~40 MB instead of scanning the whole day. **Upgrade path (automatic):** a CFN custom resource (`MeteringHourMigrationCustomResource`) runs during `update-stack` and relocates historical `metering/date=X/*.parquet` files into `date=X/hour=HH/` subdirs before the Glue table's new projection takes effect — the metering table `DependsOn` the migration, so dashboards keep working against the old layout until the migration completes. Fresh installs no-op immediately (empty bucket). For very large historical volumes (>30K old-layout files), the custom resource fails fast with a clear error pointing at `scripts/migrate_metering_hour_partition.py` for manual execution before retrying `update-stack`.
