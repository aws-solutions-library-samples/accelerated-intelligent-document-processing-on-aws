...
- **The type gate now checks calls into the shared `idp_common` library, and the errors that were hidden behind that gap are fixed** ([#1109](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1109)). `basedpyright` honours `PYTHONPATH`, and neither `make typecheck` nor either CI set one; with `reportMissingImports` configured `"none"`, `idp_common` did not resolve, so the checker knew no signature in the shared library and could produce no diagnostic about any call into it — the boundary nearly every Lambda and script uses to reach this solution's core. The gate read every tracked `.py` file, reported a count matching `git ls-files` exactly, exited 0, and could not have caught a signature change at any call site. `pyrightconfig.json` now carries an `extraPaths` naming the five first-party package roots, so resolution is a property of the configuration rather than of how the gate was invoked; the entries are **relative**, which pyright resolves against the directory holding the config, so they cannot reach a different checkout — an absolute path can, and an environment-level fix that does so produces confident wrong answers rather than silent ones. Eleven errors were behind the gap and every one is fixed rather than baselined, so the gate is at zero with resolution live and no suppression list was added. Two were live faults: the configuration updater called `set_bda_project_arn` without the ARN it requires, so `bdaSyncStatus` was never set to `needs-sync` and the resulting `TypeError` surfaced only as a warning log — it now calls `set_bda_sync_status`, which writes just that attribute; and `scripts/discover_model_limits.py` constructed `BedrockClient(region_name=...)` against a parameter named `region`. That script carried a second fault the type gate still cannot see, found while confirming the first and fixed here too: it called a `call_model` method that has never existed on `BedrockClient`, so every run died and its handler reported the `AttributeError` to the operator as "Check AWS credentials and permissions". `reportAttributeAccessIssue` is `"none"` repo-wide, which is why resolution alone does not surface it. A third, the rule-validation orchestrator's unexecutable failure recorder, is the one the entry above fixed independently — it was in this set, which is the concrete case for why the gate needed to be able to see it. One was a declaration defect in the library: `ConfigurationReader.get_merged_configuration`'s overloads omitted the default on `as_model`, so both callers relying on the documented dict default matched no overload at all. The rest were a nullable operand and a nullable dict key at call sites in two test modules and a resolver. `scripts/tests/test_pyright_config.py` fails on the specific combination that made this invisible — `reportMissingImports: "none"` together with unresolvable first-party packages — and asserts resolution four ways, because "it resolves" is satisfied by a foreign tree: the search paths stay relative and inside the repository (and are rejected if a symlink leaves it), a live basedpyright run resolves all five packages through the values the config carries, basedpyright's **own reported search-path order** makes one of the configured `extraPaths` the first provider of each package in **every** reported execution environment, and no `executionEnvironments` entry may declare its own `extraPaths`. The last two are not redundant, and both were added because a weaker form was measured to pass while the gate was blind: a single `executionEnvironments` entry carrying a foreign `extraPaths` took a planted wrong call from 2 errors to 0 for a whole subtree with every other assertion green, because `--verbose` prints one search-path block per environment and only the first was being read — and that first block belongs to the vendored subtree, not to the environment that checks `src/` and `lib/`. Requiring the *configured* directory rather than merely one inside the repository also closes the variant with no `extraPaths` at all: an environment **root** is inserted ahead of `extraPaths`, so a root at a staged build copy of the library would win while still being in-repo. All four strip `PYTHONPATH` from the subprocess, since a probe that inherits one measures the caller's shell. `reportMissingImports` stays at `"none"`, measured rather than assumed: raised to `"warning"` it reports 44 findings over 25 modules and none is first-party.
- **`make typecheck-pr` no longer reports other people's merged work as your regression.** It asked for `origin/<branch>` first — and in this repository `origin` is the **GitLab mirror** while pull requests are opened against the `github` remote, so the first candidate succeeded and the answer was wrong. The stale ref was a *remote-tracking* one, and a remote-tracking ref only moves on `git fetch`: measured in one clone, `origin/develop` was 79 commits behind `github/develop` and **151** Python files were selected where 10 had changed. Every commit merged in between is attributed to the current branch, so their type errors are reported as the developer's own. The base ref is now ordered by which remote a pull request is actually diffed against, printed with its SHA and date on every run so a stale answer is never silent, and **warned about — not fatally — when the preferred ref is provably behind another candidate**, in which case the fresher one is used; a fork remote is never allowed to override a preferred one, since a fork's `develop` can be ahead purely by carrying unmerged work. Seven tests drive it against a throwaway repository built in the stale state, including one that reimplements the replaced resolution and measures that it *did* select a file only the base branch changed — without that back-reference the others would only prove the new function exists.
