...
- **Long-context Claude models (a `:1m` model ID) are no longer reported at up to double their real cost** ([#899](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/899)). `config_library/pricing.yaml` carried a separate entry per `:1m` model at Anthropic's long-context premium — 2× input, 1.5× output, in all three region blocks, for Sonnet 4.6 and 5 and Opus 4.6, 4.7, 4.8 and 5 — and applied it to every token metered under that key. But the premium applies only to a request whose input exceeds 200,000 tokens, and the `:1m` suffix never reaches Bedrock at all: the client strips it and sends the `context-1m-2025-08-07` beta header, so the suffix survived only in the metering key, where the pricing table matched it. A benchmark arm was reported at $125.42 against $68.82 for the same work, a 1.82× gap on token volumes that differed by 1.06×, with the largest single request in the whole study at 34,015 input tokens — 94% of the apparent gap was the rate card. Nothing was ever overspent; the defect was in reporting, but it made these models look unaffordable in every document cost table, Athena rollup, benchmark summary and evaluation report. Metering keys now name the model actually invoked, so they carry no `:1m` suffix (`idp_common.bedrock.model_utils.metering_model_id`, applied at both emission sites; a service-tier suffix like `:flex` is still kept, because it does change the price of every request), and the 18 `:1m` pricing entries are retained at their base model's rates so that metering written earlier still prices exactly. Threshold-aware pricing was considered and rejected as unimplementable: token counts are summed per step and model across every call on a document before any price is looked up, so a 200K threshold would charge the premium on ten 30K-token calls, and the agentic extraction path sees only Strands' usage already accumulated across the agent loop. **Upgrade-visible:** reported costs for `:1m` models drop, by up to 2× on input tokens, so cost comparisons across this upgrade are not like-for-like. Cost figures already written to the reporting tables are not rewritten — only new records, and recomputations of old metering, use the corrected rates; a deployment that saved a Custom pricing table keeps the premium values until it restores defaults. The remaining known gap: a request genuinely above 200,000 input tokens is now under-reported. See [cost considerations](docs/cost-calculator.md#what-the-estimate-cannot-express-prices-that-change-with-request-size).
