...
- **An Annotator got a free-text box instead of the class dropdown, and nothing said why.** `getConfigVersion` is `Admin, Author, Viewer` and excludes **Annotator** — the role the annotate screen exists for — so an annotator's config fetch is denied, the class list comes back empty, and the editor fell through to its free-text branch. The only visible difference was three words dropping out of the field description. Both views use the same component; what differs is the data the caller's role can fetch. This mattered more once the class became editable for annotators: a typed class that no config defines produces a section with no schema, which extracts nothing, and the resolver bounds the characters but deliberately not the membership. The editor now distinguishes "could not read the classes" from "there are none" and locks the field with the reason and who can do it instead, rather than offering an unconstrained box to the role least able to know the vocabulary. 3 tests. **Not fixed:** giving annotators the real list needs either `Annotator` on `getConfigVersion` (which exposes the whole merged config to the lowest-privilege role) or a config-table read grant for `test_set_resolver` so the class names can ride on `getAnnotationQueue`, which is already `Admin, Author, Annotator`. The second would also close the `documentClass` membership-validation gap. Left as a decision.
