...
- **A generated Z3 `rule_id` is now a deterministic function of the rule text, instead of being neither unique nor stable.** `RuleTranslator._generate_rule_id` hashed the rule text together with `int(time.time() * 1000)`, under a comment reading "Use hash of rule text + timestamp for uniqueness". Millisecond resolution is far coarser than the rate the method is called at, so identical rule text in a loop produced one id — 2,000 back-to-back calls gave 3 distinct ids — while the same text translated a moment later produced a different one. Nothing was broken by that: every consumer was traced, and the id reaches only log lines, `TranslationError` context and the returned `RuleJSON.rule_id`; the translation cache is keyed on the rule text, in memory and in S3. What the comment did was invite exactly the reliance that *would* have made it a defect — storing translated rules under their id would have let one rule's translation overwrite another's. The timestamp is gone, so the id is a digest of the rule text alone. Re-translating a rule now keeps its id, which also makes the id agree with the key its translation is already cached under, and removes a live oddity where a cache hit returned the original id while a cache miss on identical text minted a new one. **Action:** none. Clicking “Generate RuleJSON” on an existing rule produces a different `rule_id` than the one currently stored under `x-aws-idp-rule-json`, once, and the same one on every regeneration after that; nothing reads that field, so a stored rule keeps working either way. It remains distinct from the author-written `x-aws-idp-rule-id`, which does need to be unique ([Z3 rule validation](docs/rule-validation-z3.md), [#1118](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1118)).
- **The Z3 rule translator's Bedrock retry parameter now says what it does.** `RuleTranslator._invoke_bedrock` took `max_retries: int = 2` and looped `for attempt in range(max_retries)` — two attempts in total, one initial call and one retry — while its docstring documented "(default: 3)". Two things disagreed at once: the default by one, and what the parameter counted, since the loop and every log line and error message inside it treat the value as total attempts rather than as retries after the first. Nothing in the repository passes the argument, so no call behaved differently from the code; what was wrong was the only description a reader tuning throttling behaviour would consult, on the single Bedrock-facing method every rule translation and every value extraction goes through. The parameter is now `max_attempts`, documented as the total including the first, with the default stated as the 2 it has always been. The retry ladder is unchanged: a translation still gets one retry on a throttle, a service-unavailable, a too-many-requests or a request timeout before the `TranslationError` the engine converts into a second extraction attempt against the raw document text. The `TranslationError` context key stays spelled `max_retries`, because that field is observable and renaming it is a separate change. **Action:** none, unless you call `_invoke_bedrock` directly, in which case pass `max_attempts` ([#1122](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1122)).
