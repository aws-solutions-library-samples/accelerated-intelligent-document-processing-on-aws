...
- **One stalled Bedrock request no longer costs a whole document's extraction, and the shards that already succeeded are no longer thrown away with it** ([#1014](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1014)). The agentic extraction client allowed a single request to stall for **600 seconds** inside a function Lambda kills at 900 — its maximum, so the budget could not be widened — while the retry ladder above it was allowed 300 seconds of total backoff. Those two sum to exactly 900 and left nothing for the work itself: a shard that met a transient `Read timed out` ran into the wall clock instead of retrying, Step Functions read the resulting `Sandbox.Timedout` as **deterministic** (one attempt, deliberately, since [#917](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/917)), and so the one failure a retry would have cleared became the one failure not retried. `ExtractionShardMap` declares no failure tolerance, so Step Functions' default of zero then failed the whole Map and **discarded the output of the shards that had completed** — leaving the document with zero rows extracted, which is a worse outcome for that document than not sharding at all. Observed deterministically on a 400-row statement under advanced extraction with integrated confidence, where the same document under separate confidence completed in 123–137 s; integrated mode does extraction and confidence in one invocation, so its exposure to a stall is several times longer. The read timeout is now **180 s**, leaving room for two or three attempts plus the work, and the three numbers that have to add up live together in `idp_common.utils.bedrock_utils` with `tests/unit/extraction/test_shard_timeout_budget.py` asserting the inequality — the relationship was previously stated only in a comment, which is why each number could look individually reasonable. `ExtractionShardMap` also gains a single Map-level retrier, so the per-shard S3 persistence that exists precisely so "only incomplete shards re-infer" can finally be used; a failure tolerance above zero was deliberately **not** adopted, because completing a document with shards missing is silent data loss and worse than the failure it would replace. ⚠️ A legitimately slow single request is now cut off and retried rather than waited out; observed per-call latency is far below 180 s (a 3,200-row document completes in 408 s across many calls).
