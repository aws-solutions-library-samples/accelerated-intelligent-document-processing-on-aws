...
- **The offline test suites no longer depend on the machine they run on: `make test-packages-cicd` now takes the AWS region, credentials and profile away from every suite instead of handing three of them a region** ([#988](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/988)). Three suites under `src/lambda/` could not be collected or run without an ambient AWS region, so they passed on a developer machine — where botocore reads one from the shared AWS config file — and failed on a CI runner, which has neither that file nor any `AWS_*` variable. Measured from each suite's own directory with every `AWS_*` variable stripped and `HOME` pointed at an empty directory: `external_idp_group_mapping` gave 8 passed and 41 errors, `save_reporting_data` gave a collection error and ran nothing at all, and `test_file_copier` gave 7 failures, every one of them `botocore.exceptions.NoRegionError`. The same three now give 49, 8 and 7 passed in that environment. None of this affected production: the Lambda runtime always sets `AWS_REGION`, and a client built at module scope is the right shape for a handler because a warm invocation reuses it. The fix was pinning `AWS_DEFAULT_REGION` on the three recipe lines, which made CI green while leaving the suites dependent on their caller, so `pytest` run directly in those directories — or `make test` through `scripts/run_all_tests.py`, which sets nothing — still failed.
  The region now comes from each suite's own `conftest.py`, except for `save_reporting_data`, which needed none: its dependency came from the library rather than the handler. `idp_common.utils.settings_helper` built an SSM client while being imported, and because `idp_common.utils` is imported by `idp_common.s3` and thence by most of the Lambdas here, importing the library at all required a resolvable region. That client is now created on first use and memoized, matching the sibling `idp_common.monitoring.settings_cache`, which has always done it that way; the client is still built at most once per process, on the first `get_settings` call instead of at import. `external_idp_group_mapping`'s three `TestGroupMapping` tests keep a region inside their own environment dictionaries, because they reload the handler under `patch.dict(..., clear=True)`, which empties the environment a `conftest.py` populates — the one case a conftest cannot reach, now explained at both sites. A duplicated `boto3.resource("dynamodb")` in `test_file_copier/index.py`, which built the resource twice at import, is gone.
  With the suites self-contained, the recipe's pins are removed and replaced by the opposite arrangement: a `HERMETIC_AWS` wrapper that unsets every AWS variable and points `AWS_CONFIG_FILE` and `AWS_SHARED_CREDENTIALS_FILE` at an empty file, applied to all 34 pytest invocations in `test-packages-cicd`. The local run is then the CI run rather than a weaker version of it, so the next suite to inherit this trap fails for whoever writes it. Stripping the credentials as well as the region is deliberate — a suite in this target that reaches a real endpoint should fail loudly rather than transact against whichever account the developer is signed in to. All 34 invocations were measured under a fully empty environment: the three above were the only failures, and the other 31 were already clean. `scripts/tests/test_offline_suites_are_hermetic.py` keeps the arrangement honest, deriving everything from the Makefile rather than restating it — it parses the wrapper out of the Makefile and **measures** that a client built under it genuinely cannot resolve a region (a stripping wrapper that has quietly stopped stripping would turn the whole gate into a no-op), checks that every pytest invocation in the recipe still goes through it and that no line hands a suite an `AWS_*` value, and then collects every suite the recipe names under that stripped environment, which takes about 27 seconds. Collection catches a client built at import time, the shape all three instances had; one built inside a test body is caught by the recipe running the suite under the same wrapper, which costs nothing because the suite runs anyway. Both halves were verified by reintroducing the defect and watching the relevant half fail.
