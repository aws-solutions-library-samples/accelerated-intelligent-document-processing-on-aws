...
- **Designate mode is now editable in the Schema Builder.** `x-aws-idp-instance-array` shipped in 0.6.6 and was preserved by the schema editor's round-trip, but had no editor of its own — so the documented zero-cost answer to multi-record packets was only reachable by pasting JSON into a class or hand-editing YAML. There is now an **Instance Array** control beside the regex fields, a select over the class's own array-of-object properties rather than a text box, because configuration validation hard-rejects any other value and a typo would be accepted in the editor and then fail config load. A value set outside the UI that no longer names an array-of-object is flagged, never silently dropped.
