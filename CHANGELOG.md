...
- **An Admin could change a document's class in the document view but not in the annotate view, and was told it was a permission problem ([#674](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/674)).** It was never a permission check: `isReadOnly` was `!canAnnotate || !selected.reviewObjectKey`, and a document carrying authored ground truth has no review-queue record, so the editor went read-only whatever the role. The disabled dropdown then read *"You do not have permission to change this class"* two lines under an alert promising the reviewer could "correct the values". Editing is now gated on role alone, matching the document view. The review record still gates the review *workflow* — claim, release, mark reviewed — which is handled separately. The flag could not simply be flipped, because the annotate view's save calls `completeSectionReview`, which requires that record and throws without it: `onSave` is now supplied only when there is a review to complete, and otherwise the editor falls back to the direct-to-S3 write the document view already uses. That is the right path for an authored label anyway — there is no draft to confirm, nothing to tag `reviewed-human`, and no confidence-curve signal to record. 5 tests.
