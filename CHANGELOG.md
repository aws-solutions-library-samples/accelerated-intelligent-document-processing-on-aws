...
- **A class that disagrees with ground truth is now flagged where the class is displayed ([#627](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/627), [#630](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/630)).** Arriving at a low-scoring document from Test Studio's *Documents with Lowest Weighted Overall Scores*, there was no way to tell whether the score was low because a field was extracted wrongly or because the page was classified as the wrong document type in the first place — the Sections and Pages tables showed only what the run predicted, with nothing to compare against, and the only ground-truth view was the evaluation report, in a different place and a different shape. The per-page comparison was already being computed (`doc_split_metrics.page_details` in `evaluation/results.json` carries every page's expected and predicted class) but nothing surfaced it: the markdown report renders section-level tables only, so a page misclassification was visible as a number with no way to see which page went where. Three additions, all in place rather than in a new panel: **(1)** the **Document Sections** and **Document Pages** tables show a **Class mismatch** alert beside a Class/Type value that disagrees with ground truth, with a hover giving the expected class, the class this run assigned, and the pages involved — including in **Edit mode**, next to the class dropdown, where the class can actually be corrected; **(2)** the Visual Editor's **Show Evaluation** toggle now compares the section's `document_class` above the field list, showing both values whether or not they agree, reading the same `results.json` the field comparison already loads (no extra request) and falling back to the baseline file's own `document_class.type` when a document has no page-level comparison; **(3)** all three explain inline that a misclassified page was extracted against the **wrong schema**, so its extraction score is a symptom rather than the cause. A section's verdict is derived from **its pages** rather than by pairing section ids: when the split itself is wrong there is no single counterpart section, but every page still has a ground-truth class, so a section that merged two ground-truth documents reports *both* classes it spans instead of an unhelpful "no match" — and is flagged even when one of those classes equals its own, because there the boundary is what is wrong. Two deliberate silences: a row whose class matches gets no annotation, and a page ground truth says nothing about is left unannotated rather than flagged, since an incomplete baseline is not a misclassification — so a document with no evaluation renders **exactly** as it did before, with no added screen space and no request made. Page numbers are 1-based to match the page viewer (`page_indices` in `results.json` are 0-based, computed as `page_id - min(page_id)`); `results.json` is derived from the document's own evaluation report URI so a historical run snapshot resolves to that run's results; a fetch failure (results pruned by retention, an older document) leaves the tables unannotated rather than raising. 41 tests, including row-scoped wiring tests that fail on an off-by-one in the page join (counting indicators alone would not — one alert on the wrong row is still one alert). See [docs/evaluation.md](docs/evaluation.md#seeing-which-pages-were-misclassified-web-ui).
...
- **The evaluation report described every failed section as a missing configuration, whatever had actually gone wrong ([#628](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/628)).** The ⚠️ EVALUATION FAILED block asserted "no configuration was found for document class **X**" and printed the matching "Reasons for failure" / "How to fix" list unconditionally — so an unparseable extraction response, a baseline type mismatch, or an empty nested object in the schema all sent the reader off to add a config entry that already existed, while the real cause sat further down under "Detailed error". `evaluate_section` now tags each failure with a `failure_type` (`missing_schema_configuration`, `empty_nested_object`, `extraction_parsing_failed`, `baseline_data_validation_error`, `schema_configuration_error`, `unexpected_error`) and the report leads with the actual reason and shows remediation matching *that* cause. A results file written before the field existed shows the reason with **no** "How to fix" list, deliberately: guidance for the wrong cause is worse than none. The zeroed "Metrics (Failure State)" table now also says what the zeros are — *not scored*, not *scored zero* — since read as accuracy they look alarming beside a healthy document-level score, which is how the issue was first noticed. The no-op-class cause behind that original report was fixed separately (see the `evaluation_skipped` work above); this is the residual mis-description for causes that are real. See [docs/evaluation.md](docs/evaluation.md#when-a-section-fails-to-evaluate).

- **A discovered document class whose name contained a space broke BDA sync for every such class ([#624](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/624)).** Discovery can emit class ids like `Task cards` or `Blank page`, and the id was composed verbatim into a blueprint name — but `bedrock-data-automation:CreateBlueprint` requires `blueprintName` to match `[a-zA-Z0-9-_]+`, so every affected class failed with a raw `ValidationException` (`Succeeded: 0, Failed: 3`) and the config version was left holding a BDA project with **zero** blueprints. The project name was already sanitized; blueprint names were not. Fixed on both sides. **Discovery** now normalizes the id at its single write path (`Task cards` → `Task-cards`), keeping the readable original in `description` when the class has none, so the guarantee no longer depends on the LLM obeying the prompt's "no spaces" instruction — and the multi-section auto-detect prompt no longer *suggests* a label with a space (its own example was `"W2 Form"`, injected verbatim as the class name via `class_name_hint`, which overrode the schema prompt's rule; the hint is now sanitized before injection too). **BDA sync** sanitizes the class id at all four sites that compose or match a blueprint name — create, lookup, orphan cleanup, and the blueprint optimizer's own create path — because sanitizing at only some of them is worse than none: lookup would stop finding the blueprint it created (duplicate creates) and orphan cleanup would delete a live one. For the same reason the new `sanitize_class_name` deliberately **preserves underscores** rather than reusing `_sanitize_project_name` (which maps `_` → `-`): BDA permits `_` in a blueprint name, so any id that already worked renders byte-identically and no existing blueprint is orphaned by this change. A class id with nothing usable in it at all (`???`) now fails with an actionable message naming the character set instead of calling the API with a name that cannot be valid. **The upgrade path is handled too:** a version saved before this fix still holds the spaced spelling — that *is* the config that hit this bug — so re-discovering such a class replaces the stale entry instead of adding the normalized id beside it, which would have left two classes composing the same blueprint name prefix and fighting over one blueprint on every sync. Only an id that normalizes to the class being written is replaced, so two curated classes that happen to normalize alike are never silently collapsed. Multi-document discovery also now reports the id it actually saved rather than the raw model label, which no longer names anything in the config once the id is normalized. Relatedly, a failed class now carries **why** it failed through to the sync response, so the UI reports the actual cause rather than a bare failed count with the reason only in CloudWatch. The rule lives in one place (`idp_common/config/class_names.py`) shared by Discovery, BDA sync and the optimizer, and matches what the Web UI's Schema Builder already enforced for hand-authored classes (`[a-zA-Z0-9-_]+`) — UI-only enforcement was why this reached production. See [docs/discovery.md](docs/discovery.md#class-name-normalization). Reported with full root-cause analysis by [@saygenie](https://github.com/saygenie).

- **`config-validate` rejected every GovCloud config, and every config that names a model by ARN ([#648](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/648)).** The model-ID check compared the configured value to the `pricing.yaml` catalog by exact string match. GovCloud Bedrock has no bare cross-region model-ID path — a config there must name an account-scoped inference-profile ARN — so the two forms could never match and all four sections failed at once. Not gated by `--strict`: the check sets `valid = False`, so it also failed `config upload` (which validates by default; `--no-validate` was the only way past it). The same exact-match miss hit commercial `foundation-model/…` ARNs and provisioned-throughput ARNs. An ARN is now reduced to the model ID it names (`resolve_model_id_from_arn`, bounded split so the model ID's own trailing `:0` survives) before the catalog lookup. An ARN whose resource is opaque — `application-inference-profile/<uuid>`, whose underlying model needs a GetInferenceProfile call to determine — now yields a **warning** instead of an error: it is unverifiable, not wrong. A bare model ID that does not match is still an error, since that is a typo. Reported with a diagnosis and a proposed patch by [@karolytanczos-govcio](https://github.com/karolytanczos-govcio).
...
- **Test Studio: files dropped directly into an existing test-set folder in S3 no longer stayed invisible.** `getTestSets` short-circuited already-registered folders (`if prefix in existing_test_sets: continue`), so a manual `input/`/`baseline/` add updated neither `fileCount` nor the input↔baseline pairing check. Discovery now reconciles registered folders (allowlisted to `status in {COMPLETED, FAILED}` and `labelJobStatus != RUNNING`, capped at 25 probes / 5 s of budget per request mirroring `_reconcile_label_state`, with a 30 s warm-container TTL keyed on a per-prefix memo so the UI's 3 s fast poll doesn't repeat two paginated `list_objects_v2` calls per registered set on every tick — the memo resets on cold start). The Refresh button does NOT bypass the TTL; a direct-S3 add can be up to 30 s stale in the UI. Signature is `input_count:baseline_count:max_last_modified|src=<synthetic\|uploaded>` — the marker check is folded in so a late `.source` write from a synthetic generator still invalidates the fast path. Reconcile **never** hard-fails partial input↔baseline pairing on an already-registered row: that signal is "botched upload" and belongs to first-discovery only (`_create_test_set_tracking_entry`). Partial pairing at reconcile time is one of a set of legitimate states — the draft-labeling harvester writing baselines one document per poll, `clear_draft_labels` deliberately keeping human labels while dropping drafts, direct-S3 add of an input before its baseline, or a stack-managed dataset (HuggingFace deployers `fake-w2` / `ocr-benchmark` / `fcc` / `docsplit`, ConfBench) with per-doc processing failures — and reconcile refreshes `fileCount` + `contentSignature` while leaving `status` / `labelState` / `error` alone. The one remaining hard-fail is "No input files found" (every input deleted, a genuinely broken state). Keying the softening on the validator's error class rather than on `labelState` survives `_reconcile_label_state`'s in-place demotion, which runs earlier in the same `get_test_sets` call. Reconcile only rewrites `source` when the current value is marker-owned (null / uploaded / synthetic), so stack-managed provenance strings (`huggingface:*`, `confbench:*`) are preserved through every reconcile. Adds `updatedAt` (an ISO timestamp of the row's last reconcile — currently for API consumers, not yet surfaced in the UI) to the `TestSet` GraphQL type.
