...
- **`make check-branch-protection` measures whether the CI gates actually block a merge.** They do not: `develop` has no branch protection, so `lint-cicd`, `typecheck-pr`, `cfn-lint`, `dep-audit`, the SRT scan, `api-test-static` and the parity guard are all advisory — a pull request can be merged with every check red, and since the GitHub workflows are `pull_request`-only, a direct push to `develop` runs none of them. `CLAUDE.md` and `CI_TEST_COVERAGE.md` warned about this in prose ("being visible is not being blocking"), which is how it sat unnoticed; those warnings are now replaced by a command. `scripts/sdlc/check_branch_protection.py` derives the expected required-check list by **parsing** `.github/workflows/*.yml` for the job names GitHub turns into status contexts — a hardcoded inventory would drift on the next rename — then asserts via read-only GitHub API calls that protection is enabled, that every context a PR produces is required, that stale approvals are dismissed, that force-push and deletion are blocked, and that an approving review is required. It reports the three contexts that cover all eight shared gates (those gates are *steps* inside three jobs, and GitHub can only require job-level contexts) and the two that must **stay** advisory: `build-docs.yml` and `generate-dep-manifest.yml` are path-filtered, so a required check there would sit pending forever and block every merge. Opt-in and non-blocking by design — it needs network access and a token with `administration:read`, and reports "not protected" until [#933](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/933) is closed, which needs repository admin — so it is in neither `lint-cicd` nor `test_ci_gate_parity.py`'s `SHARED_GATES`, and exits 0 with an explanation when it cannot run (`--fail-on-skip` makes that an error, which is how to run it once #933 closes). Parsing and assertion logic is tested offline against recorded API payloads in `scripts/tests/test_check_branch_protection.py`. See [Testing](docs/testing.md#whether-any-of-this-actually-blocks-a-merge).
