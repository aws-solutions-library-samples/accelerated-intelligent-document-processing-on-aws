...
- **Evaluation: parent verdicts and section/document metrics now derive from Stickler's row-level `field_comparisons`, eliminating the parent-vs-drilldown contradiction on list-valued documents ([#625](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/625)).** The v0.6.3 stickler cleanup had switched IDP to consuming Stickler's confusion-matrix nodes for both the per-attribute verdict and section rollup, but Stickler exposes two rollup views and **neither captures every failure mode**: `cm.overall` collapses list fields to the item level (masks leaf failures inside Hungarian-paired items, so a list where every item was "paired" but 4 of 10 leaves inside were wrong reported the parent as ✓ with 100% precision — exactly the shape sirikaro reported), and `cm.aggregate` drops rejected items entirely (misses list-item false discoveries when whole items fall below `match_threshold`). Only Stickler's row-level `field_comparisons` — the same rows the UI drilldown displays — carries a threshold-gated, per-user-config `match` verdict for every leaf. This release moves all three rollups (per-attribute `matched`, section `_stickler_counts` and derived precision/recall/F1/accuracy/FAR/FDR, and the aggregation Lambda's run-level top-level metrics) to a uniform pass over those rows: parent ✓ iff every drilldown row under it is ✓; section/run counts are the 5-way classification of each row (tp/tn if `match=True`; fa/fn/fd for the three `match=False` shapes). Result: parent, section, document, and run-level views now agree with the drilldown by construction, and every failure mode surfaces at every level. `STICKLER_RESULT_VERSION` bumps `1.0`→`2.0` since `_stickler_counts` semantics change (contract stamp fires a version-drift warning on rolling deploy or when reading a v1.0 payload). Two new golden fixtures pin the shapes: `partial_list_failure` (4-of-5 items rejected, both leaves wrong per item) and `list_leaves_wrong_items_kept` (sirikaro's shape — items above threshold, leaves inside wrong). Nine new aggregation-Lambda unit tests and three per-doc regression tests. **User-visible caveat for historical data:** runs recorded on v0.6.3–v0.6.5 have inflated section metrics on list-heavy configs (the amount of inflation depends on how many leaves inside kept items were wrong); re-run those evaluations after upgrading for accurate comparison. Bob's earlier FAR/FDR alignment (`a77e14336`) is preserved — the row derivation makes it a natural consequence rather than a separate rule.
