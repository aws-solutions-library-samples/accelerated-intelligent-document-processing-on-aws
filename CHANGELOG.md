...
- **`idp-cli` output meant for a program is no longer rendered as terminal output: `--json` parses, and YAML printed to stdout is not folded at 80 columns** ([#905](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/905)). `config-revisions --json` emitted its payload with Rich's `print_json`, so wherever styling is on — a real terminal, a pty, or `FORCE_COLOR` in the environment — the JSON began with an ANSI escape and `idp-cli config-revisions --json | jq` failed at character 0. An audit of every machine-readable path found the same defect in eight more places, and two of the three ways Rich alters a payload need no colour at all: it hard-wraps at the console width, which is 80 columns when stdout is not a terminal, so `idp-cli config-download > config.yaml` broke every longer line *in the redirected file*; and it reads `[...]` inside a string value as markup, so a field description reading `value [in USD]` rendered as `value ` and a prompt containing `[/bold]` failed the command outright with `MarkupError`. All nine paths — `config-revisions --json`, `status --format json`, the YAML that `config-download` and `config-template` print when `--output` is omitted, and the JSON schemas from `discover`, `discover-multidoc` and `bootstrap` — now write through one `emit_json` / `emit_raw` helper that puts the payload on stdout unchanged. Human-facing tables, progress and status lines are still styled. A static test guard fails if a new command renders a payload through the console again. Separately, the `idp_cli` suite no longer depends on the environment's colour setting: an autouse fixture pins the CLI console to no styling at a fixed width, so it passes identically plain, under `FORCE_COLOR=1` and under `NO_COLOR=1 TERM=dumb` — previously two tests failed whenever colour was forced on, which is what blocked `make test-packages-cicd`. See [IDP CLI](docs/idp-cli.md#machine-readable-output).
