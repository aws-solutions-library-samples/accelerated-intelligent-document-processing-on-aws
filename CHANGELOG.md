...
- **Analytics chat agent knows about the reporting rollup tables and picks the cheapest tier by requested range.** The rollup tables shipped in the previous entry are only useful if consumers know they exist. The natural-language → SQL agent's schema-disclosure tool (`get_table_info`) previously returned "Unknown Table" for `metering_hourly` / `metering_daily` / `metering_docs_*` / `control_plane_hourly` / `data_plane_lambda_hourly`, so the agent read the overview blurb, tried an educated guess against the rollup, hit `COLUMN_NOT_FOUND` or a wrong table (the daily rollup uses `day` not `hour_ts`; `n_docs` lives on the docs table not the cost table), then fell back to raw `metering` — meaning wide-range questions still scanned GBs. `get_table_info` now returns a full column-per-column description of each rollup table, and the agent's system prompt carries a tier picker (`<2h` raw · `2–24h` hourly · `>24h` daily) with the cost-vs-docs split rule as canonical templates. Measured on `idp-dev-qs`: a "docs + pages last 30 days" question dropped from **2.28 MB** scanned (over 3 failed rollup attempts + fallback to raw) to **510 bytes** on the docs rollup, first try. Sample queries carry a doc-hours disclaimer — `metering_docs_daily.n_docs` is a per-doc-per-hour count (`SUM(hourly n_docs)`), not a cross-day unique-doc count, so the query renames the output column `doc_hours` accordingly and cross-day unique counts still route to raw `metering` with `COUNT(DISTINCT document_id)`. `AgentMessagesDisplay.tsx` gets the same guidance for users reading the in-app schema help.
