...
- **Product demo videos can be produced from a changelog section, a PR or a named feature: `.claude/skills/product-demo.md`.** The only way to get a demo of a new feature was to record one by hand, so [docs/demo-videos.md](docs/demo-videos.md) trailed the product by releases and most changes never got one. The new skill reads the material (`CHANGELOG.md` `[Unreleased]` or a release section, a PR/MR fetched the way `pr-review.md` does, or a feature's doc), checks the stack actually runs it (version and build date in the navigation; an unmerged change is *blocked — not deployed*, never improvised), proposes **three** different storyboards for the user to choose from, writes the chosen one in full and confirms it, prepares fixtures and rehearses every chapter unrecorded, then records it with the UX-review recorder and hands over `demo.mp4`, a ready-to-paste `docs/demo-videos.md` entry with the manual asset-upload step, and the storyboard saved in `scripts/demo_storyboards.yaml` so the same demo can be re-recorded on the next release. `scripts/ux_recorder.py start --kind demo --title … --subtitle …` is the recorder's new mode: no persona, a title card from the title and subtitle lines instead of the stack name, a **Key takeaways** end card read from `demo.md`, and `demo.mp4`/`demo.srt` outputs; reviews are unchanged. Narration and click handling are shared, with one rule reversed: a review keeps a mis-aimed click and reports it, a demo re-takes. Demos default to shipped samples and synthetic data; anything else in frame is named and confirmed once before recording, and nothing under `scratch/` is ever committed. See [scripts/README.md](scripts/README.md#ux-review-and-demo-recorder-ux_recorderpy).

- **Test Studio → Compare Test Runs shows a Comparator Changes panel.** Lists only the attributes whose applied comparator, threshold or provenance ("configured" vs "auto-inferred") differs between two runs — silent when nothing differs, so it doesn't noise up the common case. Backed by two new provenance fields on each `AttributeEvaluationResult` in `results.json` (`inference_source`, `inference_why`) that carry Stickler 1.0's per-field decision from `spec.explain()`. The trace is shown as a tooltip on auto-inferred rows so an operator can see WHY the picker chose (`type:str -> LevenshteinComparator@0.7`, `name-token:invoice_id -> ExactComparator@1.0`, …). Useful when an annotation is removed from a config (numbers stay the same, source flips from "configured" to "auto-inferred") or when the Stickler picker's rules shift under an upgrade.

- **A whole-repository quality review is now a repeatable procedure, not a one-off: `.claude/skills/repo-quality-review.md`.** The existing review skills are per-change (`code-review.md` for your own diff, `pr-review.md` for someone else's), so "how healthy is this codebase?" had no method and its findings could not be reproduced next quarter. The new skill fans one subagent out per dimension across ten of them (architecture, security, test strategy, observability, code quality, CI/CD, documentation, frontend, empirical rigor, engineering-practice signals) and then consolidates, which is where the cross-dimension patterns appear — the manual review it is drawn from had six of ten independent reviewers land on the same defect class without coordinating. It carries the offline commands that produce each baseline number (resources per CFN stack, the share of Python actually reached by ruff and basedpyright including a `pyrightconfig.json` `include` path that no longer exists and so silently excludes a 6,791-line file, registered vs orphaned test roots, which CloudWatch alarms reach a subscribed SNS topic, an IAM wildcard census, doc-to-template drift in both directions, and which `make` gates run on which CI and which of those are blocking rather than merely visible) — every command was run in the repo before being written down — plus the two recurring defect classes with the known instances as worked examples: a control that exists as an artifact but is never consulted at the decision point, and a fix applied to the instance while the class stayed open. Read-only by construction (no writes outside a scratch path, no AWS write APIs, no pushes), and it needs the Agent tool authorized explicitly. Its guard test closes the class it belongs to rather than only its own instance: both structural assertions enumerate from the directory, so **every** `.claude/skills/*.md` must carry a row in the `CLAUDE.md` skill table — which meant registering `sync-pii-anonymizer.md`, until now indexed nowhere — and every `.cline/skills` entry must be a symlink to its canonical counterpart. Review of the skill closed the third side of that triangle and three gaps found by measuring rather than reading: every `.claude` skill must now also *have* a `.cline` entry or a row saying why not, which surfaced six absences against 27 skills — five deliberately Claude-only live-stack tiers, each now carrying its reason in a `CLINE_EXEMPT` row, plus `sync-pii-anonymizer.md`, which that rationale did not cover because it is an offline vendored-code resync with no live-stack step, and which therefore got the symlink it was missing (27 skills, 22 symlinks, five explained absences); the guard's content assertions now police **structure** — every measurement label `A`…`I2`, the three mandatory sections, all ten dimension rows — after a mutation run showed the entire 353-line measurement section, the dimensions table and the output contract could each be deleted green, and that pinning bare phrases rather than heading forms let the withdrawal register be deleted green too; and the Class 2 search for hand-maintained inventories was re-keyed from `grep -i 'hard.coded'` — which returned only the one instance the skill already excuses, because it matched an author's confession in a comment — onto the *shape* of the list, an `ast` walk for module-level path-like collections that returns 15 inventories and flags the 8 that nothing in their file cross-checks against a tree walk, `test_ci_gate_parity.py`'s `SHARED_GATES` among them. The skill also now states what an offline review cannot establish, roughly what a run costs, and how to reach `ruff` from a git worktree, where there is no `.venv`. ([#939](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/939))

- **UX reviews can be recorded as a narrated, captioned video.** The browser UX review (`.claude/skills/ux-test.md`) produced only a text report, so the only way to show a finding to the team was to re-run the review live. `scripts/ux_recorder.py` is a sidecar to that skill: it attaches a second DevTools session to the tab the agent is driving, captures screencast frames (Chrome emits one only when the screen changes), and takes `mark --say "…"` / `note` / `pause` / `resume` / `stop` between the agent's steps. `render` then speaks the narration with Amazon Polly's generative engine (Ruth by default), paces the footage for a person rather than for the model that drove it — idle gaps clamped, paused stretches dropped, each mark's frame held until the narrator has started, speed-ups capped at 3×, a settle before the next chapter — adds title and end cards (the end card lists the report's Findings) and an English subtitle track toggled in the player, and logs every click with its coordinates and the element actually under the pointer, drawing each click as a ring on the last frame before it and holding that frame (a single-page app changes within tens of milliseconds, so a marker drawn live lands on the destination page; a marker away from the intended element is the automation missing, and the click table says what it hit), and writes `review.mp4`, `review.srt`, `segments.json` and a chapter table into `review.md`, all under gitignored `scratch/ux-recordings/`. Two things measured on the way: ffmpeg 8's concat demuxer misplaces still images given `duration` lines (a 4 s title card landed at 0.6 s), so the video is an explicit one-symlink-per-frame image sequence; and the MP4 muxer always enables the first subtitle track, so "captions off by default" is a player setting, not a file property. Amazon Nova Sonic was considered and not used: it is a conversational speech-to-speech model, not a verbatim text-to-speech engine. No new dependency — the WebSocket client is vendored; boto3 and Pillow are already pinned. `make ux-record-deps` checks the tools. See [scripts/README.md](scripts/README.md#ux-review-and-demo-recorder-ux_recorderpy).



- **Testing now has a published map: [docs/testing.md](docs/testing.md).** The repo's test methods were documented in four places and none listed all of them — `CLAUDE.md`'s skill table, one procedure per method under `.claude/skills/`, `scripts/sdlc/docs/CI_TEST_COVERAGE.md` (pipeline-internal, never published) and `docs/release-validation/README.md` (the live-stack tiers only) — so "what tests exist, and which of them run on my PR?" had no single answer, and the browser UX review appeared in no published doc at all. The new page covers all seven layers (offline suites, static gates, UI unit, SRT + dependency audit, the GitLab-only integration smoke suite, the manual live-stack tiers, benchmarks), each with what only that layer can prove, its `make` target, whether each CI runs it, and where its results are recorded. The UX review — the only tier that opens a browser — is now also a first-class release tier (`release-validation` tier G) and appears in the live-tier table and `CI_TEST_COVERAGE.md`, with how to record one. `scripts/tests/test_testing_doc.py` keeps the page from going stale the way the CI gate set used to: it fails if a `stacktest-*` or `transform-deploy-test-*` target exists that the page does not describe, if a cited `make` target no longer exists, if a linked skill or doc has moved, or if the page drops out of the sidebar.

- **`make check-branch-protection` measures whether the CI gates actually block a merge.** They do not: `develop` has no branch protection, so `lint-cicd`, `typecheck-pr`, `cfn-lint`, `dep-audit`, the SRT scan, `api-test-static` and the parity guard are all advisory — a pull request can be merged with every check red, and since the GitHub workflows are `pull_request`-only, a direct push to `develop` runs none of them. `CLAUDE.md` and `CI_TEST_COVERAGE.md` warned about this in prose ("being visible is not being blocking"), which is how it sat unnoticed; those warnings are now replaced by a command. `scripts/sdlc/check_branch_protection.py` derives the expected required-check list by **parsing** `.github/workflows/*.yml` for the job names GitHub turns into status contexts — a hardcoded inventory would drift on the next rename — then asserts via read-only GitHub API calls that protection is enabled, that every context a PR produces is required, that stale approvals are dismissed, that force-push and deletion are blocked, that an approving review is required, and that `enforce_admins` is on so an administrator cannot push straight past all of it. It reports the three contexts that cover every gate — all eight shared gates are *steps* inside a **single** job (`developer_tests`) and GitHub can only require job-level contexts, so those eight collapse to exactly **one** requireable context and therefore share one red mark, which is worth knowing before reading a failure — plus the three that must **stay** advisory: `build-docs.yml` and `generate-dep-manifest.yml` are path-filtered, and `Test Results` is a check run an action creates via `check_name:` behind an `if:`, so none of the three reports on every PR and a required one would sit pending forever and block every merge. (`Test Results` is not a job, so it is found by reading `check_name:` inputs rather than job names.) It reads **both** enforcement mechanisms — classic branch protection and rulesets — since a branch can be fully governed by a ruleset while the classic endpoint reports nothing, and it distinguishes "not protected" from "cannot see": the classic endpoint needs repository admin and answers 404 without it, so `GET .../branches/develop` (readable with plain `pull` access) is cross-checked and `--json` reports `protected: null`, not `false`, when even that fails. That summary response also nests a `protection.required_status_checks` object readable at `pull` scope, so once protection exists a non-admin run compares the required-check list rather than giving up, while reporting the five settings it cannot read as **unverified rather than satisfied**. Ruleset rules are asserted on their *parameters*, not merely their presence — a `pull_request` rule with `required_approving_review_count: 0` requires no review — and a context is treated as conditional, so advisory rather than requireable, when a `branches:`/`branches-ignore:` filter does not select the branch or the job sits behind a job-level `if:`, not only when the workflow is `paths:`-filtered. Measured 2026-09: `develop` reports `"protected": false` and no ruleset rule governs it — the five active rulesets are inherited from the enterprise, four `target=repository` and one `target=tag` — so this is a verified "not protected". Opt-in and non-blocking by design — it needs network access and a token, and reports "not protected" until [#933](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/933) is closed, which needs repository admin — so it is in neither `lint-cicd` nor `test_ci_gate_parity.py`'s `SHARED_GATES`, and exits 0 with an explanation when it cannot run (`--fail-on-skip` makes that an error, which is how to run it once #933 closes). Parsing and assertion logic is tested offline against recorded API payloads in `scripts/tests/test_check_branch_protection.py`. See [Testing](docs/testing.md#whether-any-of-this-actually-blocks-a-merge).
- **Every number on the Well-Architected page is now derived from the templates by a test, `scripts/tests/test_well_architected_doc.py`.** Nothing checked those claims before, and four were wrong at once when the page was reviewed: an SNS email subscription no template creates, all four dead-letter-queue figures attributed to the wrong queue, "authorization is enforced per operation on Cognito group membership" when 26 of the 118 operations need only a valid session, and a point-in-time-recovery count that contradicted the same page two sections later. The guard checks four things — that every `make` target the page cites resolves in the Makefile it is invoked from (`make test-integration` did not exist at the repo root); that every logical id, parameter, repository path and relative doc link exists, including a check that *every* `AWS::ApiGateway::Method` in `nested/api-resolvers/template.yaml` is named, which is what caught one route being described as the only one; that each stated count matches a fresh count of the eleven solution templates and of `scripts/api_rbac_expectations.yaml`, with no expected values hardcoded in the test, and with every occurrence of a count phrase required to agree so a page cannot say "22" in the summary and "twenty-three" in the pillar; and that each external link's final URL after redirects equals the requested URL, because `docs.aws.amazon.com` answers **200** for a retired page by redirecting to the guide index, so a status-code checker passes three links that no longer point at the content they claim. The link group is opt-in via `CHECK_DOC_LINKS=1` and skips if a control URL is unreachable, since `pytest scripts/tests` runs in both CIs with no guaranteed egress; it is not a no-op — with the variable set it fetches all ten links and fails on any redirect or non-2xx. Each of the twenty-two assertions was verified to fail by mutating the markdown or the underlying template and observing the failure.
- **The release process is documented: [docs/release-runbook.md](docs/release-runbook.md)** ([#931](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/931)). `scripts/aws-release.sh` publishes to the three public `aws-ml-blog-*` buckets and nothing explained what it requires, what order the steps go in, or how to recover — a bus factor of one on the project's most consequential process, for a solution two sibling repositories pin themselves to. The runbook covers preconditions (branch and CHANGELOG state, validation tiers, the toolchain the publisher checks and the three things it does not, the exact AWS API set), the ordered steps with a success check for each, the failure paths that actually exist (re-running a partial publish is safe because nothing is deleted and checksums are written only on success; the window where the floating `idp-main.yaml` names a version whose artifacts are not yet public; rolling the two mutable keys back by re-publishing the previous tag; the four recorded rollback wedges and their recovery commands), what is not automated (everything — no CI job in either pipeline performs a release), and a post-release verification checklist of unauthenticated checks. Every claim is marked as verified from code, verified from a committed record, or unverified, and the gaps are listed rather than guessed. `scripts/tests/test_release_runbook_doc.py` fails if the script's regions or flags drift from the page, if a cited `make` target disappears, if the count of files `make version` stamps stops matching the page, if a step is deleted, renumbered or reordered (a runbook that says to tag before publishing is worse than none), or if the page drops out of the sidebar. It does not check the prose inside a section, which is why the page marks every claim with how it was established.
...
- **The threat model is published, linked and kept current by a build gate: [docs/threat-model.md](docs/threat-model.md).** The STRIDE corpus under `security/threat-modeling/` was reachable only by browsing the repository tree — nothing in the docs site, the docs index or the top-level README pointed at it — and it had fallen six releases behind, so where it *was* read it still described the retired AppSync API layer rather than the API Gateway REST API and dispatcher that replaced it. The corpus is refreshed against the current templates and code (98 threats, up from 83 recorded): the single authorized `POST /op/{field}` route with an authenticate-only Cognito authorizer, the SigV4 Lambda Function URL used for streaming chat, the five-group Cognito model, the CMK and TLS-enforcing bucket/queue/table policies as actually deployed, the concurrency admission-control path, and the feature-platform extension and pipeline-hook points as an extension trust boundary you own. Mitigations that depend on a change which has not merged are recorded as *pending* with their issue number and do **not** upgrade a threat's status ([#919](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/919), [#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920), [#921](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/921), [#927](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/927), [#928](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/928)). The model's README now carries a machine-readable `Last reviewed against version` field, and `make check-threat-model-currency` — run from `make lint-cicd`, so both CI systems execute it — fails when that field falls more than one release behind `VERSION`, and also fails on drift in the generated Threat Composer export. One release is the threshold because zero would red-line `develop` the moment `VERSION` is bumped at the start of a cycle, while two or more is the interval this model already drifted through unnoticed. ([#932](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/932))
- **The three pii-anonymizer test suites now run on every pull request, and the documented standing-failure baseline is enforced rather than asserted in prose** ([#974](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/974)). `feature-platform/pii-anonymizer/{feature-api,hook,ui-deployer}` were registered test roots, so `make test` ran them locally, but `make test-packages-cicd` — the target both GitHub and GitLab run on a PR — never named them: a failing test in the feature API's RBAC suite was invisible to CI for as long as it existed. All three are offline (moto) and add about 2.5 seconds. Registering them found a second problem immediately: the hook suite needs no AWS at all — it fakes S3 at the boundary — but `hook/handler.py` constructs its S3, DynamoDB and Bedrock clients at module scope, so importing it resolves a region, which on a developer machine comes silently from `~/.aws/config` and in CI does not exist. All 17 tests died at import with `botocore.exceptions.NoRegionError` on the first CI run. A `conftest.py` now pins a region and static fake credentials for that suite, so it behaves the same wherever it runs; making those bindings lazy as well is left as a follow-up, because its tests patch methods on the client objects themselves. Separately, `scripts/tests/test_standing_failure_baseline.py` now ties the "no standing failure set" claim to something checkable: the accepted-failure table in `.claude/skills/full-test-battery.md` sits between markers, the count stated in its heading must equal the number of rows in it, [docs/testing.md](docs/testing.md) and `.claude/skills/release-validation.md` must state the same count, and any row that is present must name a test file that still exists and carry a date. Three documents had claimed zero failures for weeks while one reproduced daily, because nothing connected the claim to the list.
- **The project-governance files an outside contributor or a security reporter looks for now exist: [SECURITY.md](SECURITY.md), [GOVERNANCE.md](GOVERNANCE.md), [ROADMAP.md](ROADMAP.md) and [.github/CODEOWNERS](.github/CODEOWNERS).** The repository had none of them, so a reporter with a live vulnerability had no repo-local statement of where to send it, and review requests were not routed to the contributors who know a subsystem best. `SECURITY.md` states the channel unambiguously — exploitable findings privately to AWS Security via [HackerOne](https://hackerone.com/aws_vdp), hardening suggestions to GitHub issues — plus which versions are supported (the latest release only; no backports) and a pointer to `security/` for the threat model and curated per-release test results. `CODEOWNERS` routes review requests for the agent framework, business rule validation, the Python SDK and the workshop content to their reviewers, and everything else to the maintainer. `GOVERNANCE.md` describes how a change gets accepted and the `develop`/`main` branch and release model, deferring the mechanics to `CONTRIBUTING.md` rather than restating them. `ROADMAP.md` carries themes and explicit non-goals, no dates. A new offline gate, `scripts/tests/test_codeowners.py`, keeps the set from rotting: every CODEOWNERS pattern must still match a tracked path, every rule must name a routable owner, and every relative link and heading anchor must resolve — none of which the docs-site build covers, since it reads only `docs/`. ([#936](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/936))

### Changed

- **`CLAUDE.md`'s claim about [docs/testing.md](docs/testing.md) now says what its guard enforces, and the guard now checks the suite-level part of it** ([#986](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/986)). `CLAUDE.md` stated that every test *method* in the repo was mapped on the page and named `scripts/tests/test_testing_doc.py` as what kept that true. That guard's seven tests checked live-tier `make` targets, layer entry points, target existence, link resolution and page reachability, and walked no test suite at all — so the claim was unenforced, and a contributor could reasonably conclude that adding tests without touching the page would be caught. Measured while correcting it, the claim was off by three orders of magnitude: 527 Python test modules and 88 Vitest spec files, against 5 test modules and 2 individual test functions named anywhere on the page. (A function count is deliberately not quoted: four defensible definitions of "a test function" give four different numbers spanning about fifty, and `pytest` reports more again because parametrisation expands them, so the module count is the figure that survives restating.) The claim is now narrowed, in `CLAUDE.md` and in the page's own opening sentence which made the same one, to what the page is designed to be — a map of tiers and `make` entry points keyed by target, not an index of functions, so a method added inside a suite that already runs correctly needs no page edit. Narrowing alone would have left a weaker claim equally unenforced, so two derived checks now back it: every directory on disk holding a `test_*.py` must be registered in `scripts/run_all_tests.py`, and every suite that registry *excludes* from `make test` must be named on the page, asserted in both directions so a row that outlives the exclusion it describes fails too. The first is a re-derivation rather than a new rule — `run_all_tests.py` already refuses to run when it finds an unregistered test directory, but it backs `make test`, which runs in **neither** CI, while `pytest scripts/tests` runs in both, so until now tests in a brand-new location failed only on a developer's machine and only if that developer ran `make test`. The page gains the table that disclosure needs, naming each directory `make test` does not run and why, which was previously discoverable only by reading the script. Verified by mutation in three directions: a test module in an unregistered directory, an exclusion the page does not name, and a page row for a suite that is no longer excluded. Review of this change found three more defects and they are fixed here rather than deferred. An exclusion used to cover everything *nested under* it, so excluding `scripts` quietly accepted every future test directory beneath it — proved with a probe whose `assert False` left the suite green — and an exclusion now covers only the directory it names, with `nested/bedrockkb/src/s3_vectors_manager/tests` listed separately as a result. `samples/lambda-hook-inference/GENAIIDP-w2-copy-consistency` was excluded on the stated ground that it collects zero pytest tests; it collects six and all six pass, so it is now registered and gated, and the real obstruction — a module-scope import that collides with its sibling hook directories — is recorded where the decision is made. A count of test roots in `CONTRIBUTING.md` was also **deleted rather than kept in step**: registering one more suite failed a blocking gate purely because the prose quoted a number, and the sentence's useful content is that there are many roots each with their own `conftest.py`, so a bare `pytest` from the root collides — the count was scenery nobody acts on, and the guard that policed it is removed with the reasoning recorded at the site. Deriving a number at test time is worth it when the number *is* the claim a reader acts on, such as how many resource policies deny non-TLS requests; it is a tax when the number is structural trivia. And the new wording check crashed with a bare `FileNotFoundError` on a dangling Markdown symlink, which the docs-site build leaves behind whenever a source page is renamed; three exist in a working checkout today, and because CI never builds the site into that directory it failed only locally, which is the worst place for a diagnostic that offered no guidance.

- **The TLS-only transport gate now closes a class rather than two resource types, and the SNS decision is recorded rather than absent** ([#987](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/987)). `scripts/tests/test_secure_transport_deny.py` enumerated SQS queues and S3 buckets, which closed "every queue and bucket refuses non-TLS calls" completely and closed nothing else — a third policy-bearing resource type inherited no coverage and nothing said so. Measured on this tree that was not hypothetical: **nine** policy-capable resource types are present and two were covered. The gate now finds every policy document in every discovered template that carries a `Principal` — which is what makes a policy a *resource* policy rather than an identity policy, with trust policies excluded by property name — and requires the type it sits on to be either enforced or exempted with a written reason. A resource type nobody has considered fails on the day it lands. **No SNS topic policy was added.** Three topics declare none, and a topic with no policy permits only the owning account, so an explicit policy *replaces* that default and getting it wrong breaks publishing at runtime rather than at deploy time — that needs a live check against a deployed stack, so the deny is not added blind. What replaces it is enforceable: the exemption for a type justified by "declares no resource policy" fails the moment one appears, so whoever adds a `TopicPolicy` — the headless alerts work in #984 is the likely occasion — has to decide about the deny then. The types that *do* declare policies are exempt on the narrower, checked ground that every statement faces this account's own root or an AWS service principal: six KMS keys, one ECR repository, and one REST API whose policy exists only on its PRIVATE branch and grants a wildcard principal **bounded** by `aws:SourceVpce` to the supplied interface endpoint. The guard treats an *unbounded* wildcard as the finding rather than a wildcard as such, because refusing the bounded form would reject the documented private-API idiom. Four guards, each verified to fail against the defect it exists for by mutation: adding an SNS `TopicPolicy`, introducing an unclassified policy-bearing type, removing the private API's bounding `Condition`, and granting a foreign account on a KMS key.

- **`--headless` alert delivery is documented as a required post-deployment step, and the decision not to add a parameter is recorded** ([#984](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/984)). A standard deployment subscribes `AdminEmail` to `AlertsTopic`; `--headless` removes that parameter along with the Web UI and Cognito resources, so it creates no subscription and every alarm publishes into a topic with no recipient — successfully, which is why nothing distinguishes it from a healthy deployment. An optional `AlertsEmail` parameter was considered and deliberately rejected: headless operators are automating, and most attach a pager, a chat webhook or an existing operational topic through their own infrastructure-as-code, so it would be a parameter most of them never set on the variant whose design goal is fewer moving parts. That makes documentation the entire mitigation, so two pages that got it wrong are fixed and the mitigation is now a checked artifact. `docs/monitoring.md` claimed "the stack subscribes the `AdminEmail` address" with no mode carve-out, and now names both modes and carries a `Headless deployments: you set up alert delivery` section with the `aws sns subscribe` commands for email and `https`. `docs/govcloud-operations.md` still described the stack as creating "alarms for two step-function conditions" — it creates many more — and now points at the single documented list instead of keeping a second copy that goes stale, and states the per-variant subscription position. Three new guards in `scripts/tests/test_alerts_topic_subscription.py` pin this: the headless transform must still remove the subscription **and** still keep the topic and its ARN outputs (otherwise the documented remedy is impossible to follow), all three documents must still carry the obligation phrase rather than merely mentioning the topic, and the transform may not restate an alarm count in a comment — it said "all 12 alarms" while `template.yaml` declared fourteen, which is the same defect class the derived-at-test-time document guards exist for.

- **The living threat model is now inside the retired-service documentation gate, and a dead exclusion can no longer pass silently** ([#995](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/995)). `security/threat-modeling/**` was excluded from `make check-retired-services` because the corpus was a set of point-in-time review artifacts pinned to the release each assessed. Giving it a currency gate of its own (`make check-threat-model-currency`) made it a living description of the current architecture, and a living document has no business being exempt from the gate that stops documentation presenting a removed service as current. The exclusion is narrowed to the four genuinely dated artifacts — the curated per-version test-result snapshots, the version-range AppSec review reports, the Threat Designer session exports — and the gate now scans 245 documentation files rather than 221, with the threat-model files pinned into the scanned set by a test so the exclusion cannot come back as a quick fix. Bringing the corpus in flagged 13 correct lines, all of them because `historicalMarkers` covered `was removed` but not the present-tense forms this corpus uses. Ten are now covered by marker forms (`AppSync is fully removed`, `subscriptions are gone`, `are replaced by`, `former AppSync`, `AppSync-era`, and the two revision-history shapes) and three by allowlist entries, because their pastness is carried only by a verb tense that no phrase pattern can detect safely. **The markers are anchored to the service name, and the near-miss fixtures that forced that are the point of the change:** written as keyword searches they exempted `The ALB is gone, so the AppSync GraphQL API is now reached directly`, `The former ALB hostname is replaced by the AppSync GraphQL endpoint` and `Polling is replaced by AppSync subscriptions` — in the last two AppSync is the *replacement*, which asserts the opposite of absence. Separately, `excludedPaths` had no staleness check at all while allowlist entries have always had one, because an exclusion's effect is an absence of findings and so cannot fail by matching too little: this exclusion's justification ended "REVISIT ONCE PR #960 LANDS" for most of a release cycle, #960 landed, and nothing asked. A dead exclusion glob now fails the gate, with an explicit `mayBeAbsent` flag for the three that cover gitignored build output and a test pinning that set so the flag cannot become a blanket escape.
- **Four published granular-assessment cost figures no longer quote a magnitude.** The `−78% Bedrock cost` / `4–5× more` figures in the [retirement note](docs/migration-granular-retirement.md), the [Extraction & Confidence guide](docs/extraction-and-confidence.md) and this file were produced by the solution's own cost reporting while `_get_unit_cost` still resolved a pricing unit by substring, so `cacheReadInputTokens` bound to a row's `inputTokens` price and cache reads were billed at up to 10× their real rate. Granular was a cache-heavy arm (measured ~1:1 cacheWrite:cacheRead), which is exactly the shape that error acts on, and the A/B output was never committed, so it cannot be re-priced after the fact — only re-run. All four now state the effect as a direction, with the reason recorded on the page rather than only in an issue. The direction is not in doubt: fresh-input and output tokens were always priced correctly, so correcting the price shrinks the ratio by a bounded factor and cannot reverse it, and granular's 0% geometry result involves no pricing at all. Scope is bounded in the other direction too — the benchmark harness has its own pricer (`benchmarks/harness/lib.py::price_metering`) which has matched both the model key and the unit name exactly since the file was added, so nothing under `benchmarks/results/` or in `docs/benchmarking/` was ever affected. No pricing data changed: the remaining entries that carry no cache rate are already gated by `_NO_CACHE_UNITS_EXPECTED` in `tests/unit/reporting/test_pricing_lookup.py`, which asserts for each one that the model is absent from `CACHEPOINT_SUPPORTED_MODELS` and is not an implicit cacher — i.e. that the rate is unreachable rather than merely unpublished. `config_library/pricing.yaml` now points at that rule from the data itself, including the warning not to "fill a gap" on a bare `amazon.nova-*-v1:0` id by copying rates from its commercial `us.` profile: those are the GovCloud entries and the omission is the decision. ([#966](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/966))

- **`stickler-eval` upgraded 0.5.0 → 1.0.0, and the IDP-side type-only comparator fallback is deleted.** An auto-generated schema (no explicit config for a document class) used to be stamped with per-leaf `x-aws-idp-evaluation-method` defaults — every string got `FUZZY@0.85`, every number got `NUMERIC_EXACT@0.01`, every array-of-object got `HUNGARIAN`. That fallback ignored field NAMES: `invoice_id` and `notes` both became FUZZY. Stickler 1.0's [comparator inference](https://github.com/awslabs/stickler/releases/tag/v1.0.0) picks per-leaf by Python type AND field-name token (`*_id` → Exact, `amount` → Numeric, `notes` → Fuzzy, date-typed strings → Date), so the mapper now sets `x-aws-stickler-infer-unspecified: true` at the schema root and lets Stickler do the picking. Operator annotations still win: any leaf carrying `x-aws-idp-evaluation-method` is translated first and inference does not touch it. Also picks up the [breaking changes](https://github.com/awslabs/stickler/releases/tag/v1.0.0) v1.0 shipped: the abstract comparator hook renamed to `_compare` (our `LLMComparator` matches), `x-aws-stickler-match-threshold` on the array field itself is now rejected as an unread extension and moved onto the item object where the element-class builder reads it, and structured-list scoring shifts because paired-but-weak matches now count as false discoveries rather than false negatives. Golden fixtures rebaselined. `STICKLER_RESULT_VERSION` bumped `2.0` → `3.0` so the aggregation Lambda flags cross-version compares in the drift-warning gate. Numerical outputs drift on auto-generated / un-annotated schemas where inference now picks per-field-name (`invoice_id` → Exact rather than 0.5.0's fallback Fuzzy). Structured-list metrics as reported by IDP are unchanged — the Hungarian FD/FN swap in Stickler 1.0 (upstream #289) is scoped to Stickler's own per-class metrics; IDP's `_stickler_counts` is derived leaf-by-leaf from row-level `field_comparisons` and the section-goldens verify no shift. New transitive deps land in the evaluation Lambda: `phonenumberslite` (Stickler 1.0's `PhoneComparator` backing library) and `json-schema-to-pydantic` (Stickler's schema→Pydantic converter). `jinja2` moved from a Stickler transitive to a direct dep of `idp_common[all]` and `idp_common[multi_document_discovery]` because Stickler 1.0 dropped it; `psutil` was also dropped and OCR's optional memory-logging now no-ops when it's absent (see `ocr/service.py`). Also drops `scikit-learn`/`scipy` from Stickler's transitive graph — both are now direct deps of the extras that use them.

- **[Well-Architected](docs/well-architected.md) is now a review template you fill in, and its six-pillar assessment states what the solution actually implements.** The page contained no numbers and six untrackable "Consider implementing…" bullets, two of which were already done (an integration suite exists; X-Ray is instrumented on 19 Lambdas), and it claimed WAF protects an AppSync API when there are zero `AWS::AppSync` resources left anywhere — the WebACL is an optional REGIONAL IP allow-list on the API Gateway REST stage, created only when `WAFAllowedIPv4Ranges` leaves its `0.0.0.0/0` default. Each pillar now names the mechanism a customer can go and inspect (the fourteen alarms and their thresholds, the two dashboards, `CustomerManagedEncryptionKey` and the 32 TLS-only bucket and queue policies, `PermissionsBoundaryArn` threading, the 24 Step Functions retry blocks over the Bedrock client's 7, the DLQ `maxReceiveCount` values, the `ConcurrencyTable` conditional-update admission cap, the metering ledger), and the bullets are replaced by a per-pillar checklist with **empty** comment/owner/date columns — the answers belong to the customer's deployment, not to this project. A new section splits what ships active, what ships off by default (`CircuitBreakerEnabled`, `BedrockGuardrailId`, `DeployInVPC`), and what has no resource at all and so is entirely the customer's: confirming what is subscribed to `AlertsTopic`, an AWS Budget, MFA on the user pool, CloudTrail, WAF managed rules, and reviewing the optional deployment service role. Corrections of record: point-in-time recovery covers 10 of the 12 tables a default deployment creates, not all (`ConcurrencyTable` is a reconciled ephemeral counter and `ChatDocumentSessionsTable` holds TTL'd chat-ownership records), `LogLevel` already defaults to `WARN`, and Guardrails are bring-your-own rather than built in. The Security pillar now also states the API authorization posture as measured rather than as intended: of the 118 operations in `scripts/api_rbac_expectations.yaml`, 90 are group-restricted and 2 are IAM-only, but 26 are `groups: ANY` and 17 of those carry no ownership or configuration-version check, so any authenticated pool user can read document content — plus the `AllowedSignUpEmailDomain` parameter that decides who can get an account at all, and the 125 `Resource: "*"` IAM statements. ([#937](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/937))

- **A stale comment on `DocumentQueueDLQAlarm` in `template.yaml` quoted the wrong redrive numbers and was copied into the Well-Architected page before anyone checked it.** It read "maxReceiveCount 1000 against a 30s visibility timeout" for `DocumentQueue`; 1000 is `DiscoveryQueue`'s `maxReceiveCount` (against a 900s timeout, so ~250 hours of retrying, not 8), and 30s is `DocumentQueueDLQ`'s own visibility timeout rather than `DocumentQueue`'s. `DocumentQueue` is 500 against 60s, which is where the ~8-hour figure actually comes from. The comment now states the right pair and records what it used to say, and the page's DLQ section is a table of the four queues that really declare a `RedrivePolicy` — separating them from the seven `...DLQ` queues that are Lambda dead-letter or asynchronous-`OnFailure` targets and are not governed by `maxReceiveCount` at all.
- **`CONTRIBUTING.md` is rewritten against the repository as it is today.** It still described the deleted `patterns/pattern-1/`, `pattern-2/` and `pattern-3/` directories, put `typecheck-pr`'s default target branch at `main` (it is `develop`), pointed at a `src/ui/.eslintrc.json` that does not exist, listed Docker as a prerequisite the build does not use, and omitted `uv`, which the publish flow does require. It mentioned none of the gates a contributor is actually judged on — `make cfn-lint`, `make dep-audit`, `make srt-scan`, `make api-test-static`, `make typecheck-pr` — and neither CI system. The new version derives prerequisites and versions from `check_prerequisites()` and the `engines` fields rather than from prose, separates the per-commit gates from the pre-PR ones with measured timings, states that whole-repo `make typecheck` currently exits non-zero on a clean `develop` so an inherited failure is not mistaken for a new one, carries the dependency-confusion rule (never install a first-party package by bare name — see [dependency-confusion](docs/dependency-confusion.md)) as a first-class instruction, and points at [testing](docs/testing.md) and `.claude/skills/` instead of restating them. It also names the two CI asymmetries: GitHub's quality and security workflows are `pull_request`-only, so a direct push to `develop` runs none of them, and the AWS-dependent integration suite is GitLab-only. Security reports go to AWS/Amazon Security, never a public issue (also stated in `SECURITY.md`, added under [#936](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/936)). Four further points it now gets right: the setup sequence activates the virtual environment and installs `basedpyright` from npm, because neither `make setup` nor `make setup-venv` puts the tools the gate recipes call as bare commands on `PATH` — without activation `make ruff-lint` fails with `make: ruff: No such file or directory` and `Error 127`, and `basedpyright` is an npm devDependency that neither target installs at all; 88 columns is described as the formatter's wrapping preference rather than an enforced check, because `E501` is not among the selected lint rules and `ruff.toml`'s `extend-exclude` uses bare directory names that match at any depth, leaving roughly a third of the tracked `.py` files unlinted, among them every file under `src/lambda/` and every file under `scripts/` ([#975](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/975)); `patterns/unified/tests` is named among the test roots `make test` runs; and `make lint-cicd`'s row no longer claims to run the same set as `make lint`. `make setup-venv`'s completion message and `scripts/sdlc/typecheck_pr_changes.py` now say the same thing the document does — the latter prints the one-line npm remedy instead of a `FileNotFoundError` traceback when `basedpyright` is absent. `scripts/tests/test_contributing_doc.py` keeps the page from drifting the way it did before: every cited `make` target and repository path must exist, the numeric claims are re-derived at test time from the Makefile, `scripts/run_all_tests.py`, `publish.py`'s prerequisite floors, the `engines` fields and `ruff` itself rather than restated, and the setup block must name every tool the gate recipes invoke bare that the setup targets leave off `PATH`. ([#930](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/930))
...
- **The granular-assessment retirement plan no longer quotes a cost figure nobody can reproduce** ([#938](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/938)). `docs/planning/retire-granular-assessment-plan.md` cited a −74% cost saving whose only source was a gitignored `scratch/` path that no longer exists, so no reader could audit it; the effect is now stated as a direction with no percentage. The −78% figure in the shipped [retirement note](docs/migration-granular-retirement.md) is a different experiment whose data is also uncommitted, so it was not substituted for it. The `~$0.09/doc` target in PR-G1's `Validate:` bullet — the granular-OFF half of the same pair, and prescriptive rather than descriptive since a future implementer would treat it as an acceptance criterion — is replaced by a relative comparison measured in the same run. The plan's header now records that planning documents written before September 2026 predate the [reproducibility rules](docs/benchmarking/index.md#reproducibility--honesty-rules), and gives a second, independent reason to distrust cost figures of that vintage: `_get_unit_cost` in `idp_common/reporting/save_reporting_data.py` **used to** fall back to a substring match, and `"inputTokens" in "cacheReadInputTokens"` is true, so on a model whose pricing entry omitted `cacheReadInputTokens` cache reads were billed at the fresh-input rate — a bounded overstatement of `1/(0.1 + 1.15f)` for a cache-write share `f`, and exactly zero for the entries that price it explicitly. The fallback was removed under [#926](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/926), so it affects figures measured before that fix and no new ones.

### Fixed

- **No Lambda under `src/lambda/` or `nested/api-resolvers/src/lambda/` writes its unredacted invocation event to CloudWatch any more, and the guard that says so now covers both trees instead of one** ([#977](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/977), [#985](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/985)). The dataflow scan added for [#921](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/921) was rooted at the api-resolver tree only, so a green run said nothing about `src/lambda/`, where **40 log calls in 38 files** logged the whole event. The most consequential was `agent_chat_processor`, which wrote the user's chat prompt, the caller's Cognito `sub` and — after [#954](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/954) added it to that line — the caller's group list, once per conversation turn. This is a data-minimisation and log-hygiene defect rather than an access-control one: the log group is encrypted with the customer-managed key and readable only to principals holding CloudWatch Logs permissions on it. But that is a wider audience than the people entitled to read a given user's conversation, and reading processor logs to diagnose a failed turn is routine operational work. Operators of existing stacks should treat these log groups as containing user content, not telemetry, when setting retention. All 40 sites now pass the event through the canonical `sanitize_event_for_logging`, which preserves the event's shape — the keys and list lengths an operator actually needs — while replacing denylisted values with `***REDACTED***` and capping content-shaped strings at 500 characters. **Fifteen** handler directories carry an `IDPCommon*Layer` and import the redactor from the library; **21** carry no layer, cannot reach `idp_common` at runtime at all (SAM packages each function from its own `CodeUri`), and take the established byte-identical vendored copy, taking the number of committed copies from 9 to 30. Nothing was exempted, including the 13 CloudFormation custom-resource handlers among them, whose events are stack metadata today: an exemption granted on the strength of what an event currently happens to carry has to be re-audited whenever its producer changes, and nothing would prompt that.

  The guard is what stops this recurring, so its own reach is now measured rather than assumed. `scripts/tests/test_resolver_log_sanitizer.py` (64 tests, now 70) scans both trees and every Python file in each handler package rather than `index.py` alone — which is how the two copies of the chat processors under `src/lambda/chat_stream_processor/vendored/` were invisible to it. Templates are discovered by **content**, through the same `scripts/discover_templates.sh` that `make cfn-lint` and `make check-arn-partitions` use, so the layer scan reads all 30 templates in the repository instead of a hardcoded pair; the `src/lambda` functions alone are declared across four of them. `scripts/sync_resolver_log_sanitizer.sh` no longer carries a hand-written list of destination directories — at 30 entries that list was the arrangement's largest maintenance cost — and instead derives them from the handler sources, on the same rule the test uses: a directory is a destination because a file in it imports the sibling `log_sanitizer` module. And a new test asserts that **every** directory in the repository holding an `AWS::Serverless::Function` with Python sources is either scanned by the guard or named in an explicit out-of-scope list with a reason, so a new feature-platform package or nested stack arrives as a failure asking for a decision. Thirteen trees are recorded as out of scope for this change; five of them hold a further **29** raw-event log calls, 15 of those in the `patterns/unified/src` pipeline steps, which are left for a change of their own because those are the logs operators read most and what to keep in them deserves its own review.

  Each new or widened check was verified by reintroducing the defect it exists for and confirming the suite goes red — 13 such probes, all of which failed as required before being reverted: the raw `json.dumps(event)` back in `agent_chat_processor`; the same leak in a file that is not `index.py`; `src/lambda` dropped from the scanned roots (the state before this change); a Lambda tree neither scanned nor recorded; a stale out-of-scope entry; the sync script and the guard disagreeing about which trees to scan; a vendored copy drifting by one line, and the same drift seen through the loaded denylist; a vendored copy deleted while its import remains; a hand-rolled denylist reintroduced; a layer-free function importing the library (an ImportError at cold start); and the scan narrowed back to `index.py` only. The permanent self-test table that proves the scan is not a text match grew to 22 spellings, which is not hypothetical: of the 40 sites, three used `print` rather than a logger, six passed the event as a lazy `%s` argument, three interpolated the bare name into an f-string, and two sliced `json.dumps(event, default=str)` to bound its length — a cap on log volume that redacts nothing.
- **An invocation of the Web UI API can no longer supply the identity the group checks read** ([#978](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/978)). `idp_common.api_adapter.normalize_event` treated an event carrying its own top-level `arguments` and `identity` as a legacy resolver event and passed it through unchanged, so for any invocation of that shape the caller stated its own `cognito:groups` and the dispatcher's default-deny check — and every resolver check beneath it — was evaluated against the caller's own claim. AWS AppSync was removed in 0.6.0, so that shape no longer arrives from a transport that authenticated anybody: API Gateway builds the event itself and a request body cannot add top-level keys to it, which means presenting one required a principal already holding `lambda:InvokeFunction` on the function directly. It was therefore a privilege-escalation amplifier rather than an open door — it turned "can invoke this function" into "can invoke it as Admin" — and the `identity` shape that produced it is now refused. The adapter builds `identity` from the API Gateway authorizer's verified claims only, refuses an asserted identity that contradicts them (rather than silently preferring either), and refuses one presented with no verified claims at all, with the refusal rendered as `403 Unauthorized` rather than escaping as a 5xx. This is the same precedence rule the chat streaming endpoint already applied to a body-supplied caller identity. **The IAM-gated service-to-service paths are unaffected**: an explicitly null `identity` is this repository's marker for an invocation gated by IAM on the function ARN, asserts no groups, and still passes through. Two guards were added and both were verified to fail against the previous behaviour — one drives the dispatcher end to end with an operation and its required groups read from the RBAC manifest at test time, the other discovers every consumer of the adapter by parsing the tree so a new entry point is covered as soon as it exists.
- **The Web UI's "update available" indicator now works — it never has** ([#962](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/962)). The indicator reads one key, `artifacts/genai-idp/idp-main-latest.json`, with an anonymous (unsigned) `GetObject`. The publisher wrote that key with no ACL, and `set_public_acls` did not cover it either: it sits at `<prefix>/`, a *parent* of the `<prefix>/<VERSION>/` tree the ACL pass paginates, so it fell between the two mechanisms. Whether that mattered turned on an open question — whether the release buckets grant anonymous read by bucket policy rather than by object ACL — and the question is now answered by measurement. A credential-free `GetObject` against all three release regions on 2026-09-18: `idp-main.yaml` returned **200** and `idp-main-latest.json` returned **403**, in every region. Two keys in the same prefix answering differently is only possible if the ACL is what grants the read, so object ACLs are load-bearing and the pointer had never been publicly readable by any deployed stack. The resolver fails closed and logs at `INFO`, so every customer's Build Info panel reported "no update available" indistinguishably from being genuinely current. Fixed in three places: `_upload_version_pointer` applies `public-read` inline on a `--public` publish, so the pointer is readable from the moment it exists rather than depending on a later pass; `set_public_acls` also covers the key, which repairs the pointers written by every previous release once a new one is published; and a new `verify_public_readability` step re-checks every key a public publish advertises — both main templates and the pointer — with an unsigned client at the end of the run, **failing** the publish when one answers 403 rather than reporting success. That last part is the durable half: both this and the sibling issue [#963](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/963) were defects a publish reported as successful and a consumer discovered silently much later, and neither was visible in the publish output. A bucket with Object Ownership set to *bucket owner enforced* rejects ACLs outright; that case falls back to writing the pointer without one, since such a bucket grants public read by policy instead. [#963](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/963) is closed as not applicable — the `--headless` and `--govcloud` transforms are a deploy-path artifact and no transformed template is published to the release buckets. [docs/release-runbook.md](docs/release-runbook.md) now states the ACL question as settled, including in the rollback procedure, where a hand-written pointer needs `public-read` applied by hand or no customer can read it.

- **A section of more than 20 pages no longer fails on Bedrock's many-image pixel cap, and an image-shaped rejection is now explained and not retried** ([#994](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/994)). Amazon Bedrock caps each image at 8,000 px per side, but at **2,000 px** once a single request carries more than 20 image blocks — counting `document` blocks and images returned inside a `toolResult`. Because the limit binds on the request's image *count*, no per-image guard could see it: every page was individually well inside 8,000 px and the request failed as a whole with `image exceed max allowed size for many-image requests: 2000 pixels`. Measured on 29 stored pages against `us.anthropic.claude-sonnet-5` in us-west-2: 2,001 px rejected, 2,000 px accepted at 117,035 input tokens, and the same pages accepted at 2,150 px in a 5-image request. `BedrockClient.invoke_model` now sweeps every assembled request with `image.fit_images_in_request` immediately before `converse` — the only point that sees a whole request, so classification, assessment, summarization, evaluation and few-shot examples are covered as well as extraction — counting the blocks, downscaling anything over the applicable cap, and refreshing each block's declared `format` (a lossless image can come back as JPEG). It works on a **copy** of the request spine, because `invoke_model` is often handed the caller's own `content` list and fitting in place would permanently downscale a cached few-shot image or a page-image list a later pass reuses. Extraction also clamps at page-load time so the reduction lands in the section's `metadata.image_downscale` and the agentic (Strands) path, which builds its own requests and never reaches this client, is covered too. The sweep is best-effort — an image it cannot resize is sent unchanged with a warning rather than failing a request Bedrock might accept — and emits one aggregate warning per request instead of one per image. A clamp now lands exactly on the cap rather than 2.7% below it, because the fit loop's 5%-per-pass floor must not overshoot a limit that is exact.

  **The page count at which this bites depends on the configuration, because what counts is the images in one *request*.** Simple extraction clamps at 21+ pages. Advanced (agentic) extraction doubles the images one agent invocation will carry — it re-sends attached pages every turn and `view_image` can add a further copy — and that per-request figure is bounded by the worse of the shard planner's two regimes — `max_pages_per_shard` while the page ceiling holds, and the largest range `_rebalance_to_cap` returns once the repack fires. Nothing simpler works: neither `max_pages_per_shard` nor `ceil(pages / max_concurrent_batches)` bounds it, because once the page ceiling would need more shards than `max_concurrent_batches` allows the repack discards those ranges and repacks into exactly that many **token-balanced** groups, ignoring the cap, so one dense page can take a shard to itself. Nor does planning under a single large token budget, because the budget decides *whether* the repack fires at all: on the shipped defaults a 50-page section with one dense page plans as ten 5-page shards under an unbounded budget and as `[1, 41, 1, …]` at Sonnet 4.6's real 18,400-token budget. The repack regime is consulted only when some budget could trigger it, so a section with no OCR text at all — whose degenerate zero-token split reports a large tail shard the planner never produces — is not clamped for it. On the **shipped defaults** (`max_concurrent_batches: 10`, `max_pages_per_shard: 5`) that puts the clamp at roughly **101 pages**; at `max_concurrent_batches: 5` about 51, at 2 about 21, and with sharding off (`max_concurrent_batches: 1`) 11. Raising `max_concurrent_batches` therefore *raises* the threshold, since it caps the shard count as well as the parallelism. Holistic classification sends a whole packet in one request and so is clamped on the same >20 rule. Deliberately pessimistic: some lost resolution is cheaper than a rejected section. **The clamp is not free on every model.** Claude 4.7+, Opus 5 and Sonnet 5 tokenize images on a tier whose own target is a ~2,576 px long edge, so 2,000 px is roughly 20% below the resolution they would otherwise receive and costs ~15% of the image tokens; on Sonnet 4.6, Haiku 4.5 and the 3.x family the tier target is ~1,568 px and the clamp costs nothing. Extraction accuracy has **not** been measured with and without it. Set `image.target_width` / `target_height` to 2,000 or less, or keep requests at 20 images or fewer, if you would rather not have it applied.

  Diagnosis is fixed in three places. `bedrock_utils.is_image_request_rejection` matches image dimension/byte rejections, is judged by error code first, so a throttle that carries a code is never turned into a permanent failure however it is worded (a Strands-wrapped exception carries no code, so the marker list is also narrowed to phrases that name an image limit explicitly — a wording it misses is retried as before rather than merely losing its tailored message, which on the agentic path means up to 50 attempts inside a 300-second backoff budget), is consulted **before** the overflow matcher, and short-circuits the retry ladder — a Strands-wrapped `ValidationException` matched `DEFAULT_RETRYABLE_ERRORS` by substring and was retried up to 50 times behind exponential backoff before anything could classify it. Extraction raises a distinct `ExtractionImageRejected` naming the image count, the largest dimension and the one remedy that applies (`extraction.image.target_width` / `target_height`). And the token estimate that made the request look oversized is corrected: images are now priced with Claude's 28 px-patch math capped at the model's resolution tier (1,568 tokens before Claude 4.7, 4,784 after), where the old uncapped `(w*h)/750` figure over-stated a 2550x3301 page 2.4x — 11,223 against a measured 4,761. Discovery's hardcoded 2,048 px image cap, 48 px over the limit for the many-page sample documents it sends, is corrected to 2,000.

  ⚠️ **Two things this entry does not claim.** `is_input_token_overflow` was **not** classifying the pixel rejection as a context-window overflow: that matcher requires input/context/prompt vocabulary which `image exceed max allowed size for many-image requests` does not carry, so the message had no explanation rather than the wrong one. What #994 actually reported came from the other direction — Bedrock reports an oversized request **payload** as `Input is too long for requested model`, a genuine overflow match, and the uncapped token estimate then over-stated the page images 2.4x, so the message compared an inflated estimate against a window the request had not exceeded. The corrected estimate and a note naming the image cap on the overflow message are what address that; the new matcher is what gives a genuine image rejection its own class and remedy. And a section of 20 pages or fewer is **not** automatically unaffected: as described above, Advanced mode reaches the clamp at 11.

  Three residual gaps, all known: an agentic conversation can still cross 20 blocks through repeated `view_image` results that the single doubling does not cover; a **resumed** agentic run skips sharding altogether and sends the whole section to one agent, which the load-time estimate cannot see (`max_images_per_agent` still caps it at 20 attached); and nothing bounds a request's **total payload** — #994 measured 29 pages at 27.7 MB rejected on size alone, so on the order of 60 clamped pages in one request reaches that wall again and surfaces as `Input is too long for requested model`. Out of scope on purpose: `bedrock/sizing.py`'s `_TOKENS_PER_IMAGE`, which feeds agentic shard budgets, and `discovery/embedding_service.py`'s 2,048, which is a single-image embedding request and not a Converse many-image one — the issue's claim about it is incorrect. See [Configuration](docs/configuration.md#a-request-with-more-than-20-page-images-caps-every-image-at-2000-px) and [Extraction and Confidence](docs/extraction-and-confidence.md#oversize-page-images-metadataimage_downscale).

- **Every confidence-recovery model call now checks the Lambda's remaining time before it is made, closing the one path that did not** ([#958](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/958)). The self-healing confidence ladder can spend sequential model calls in three places: further bisection of a batch the model truncated, a same-model retry round over the rows left unscored, and an escalation round on a model with a bigger output cap. Bisection and escalation consulted the wall-clock deadline threaded down from `context.get_remaining_time_in_millis()`; the retry rung did not. It stopped after `max_retries` rounds or on a round that recovered nothing, so a sequence of rounds that each recovered *something* while never truncating was bounded only by the round budget, at a real model call per round, however little of the 900-second Assessment Lambda was left. The check now sits in the shared recovery helper and runs **per call** rather than per round, which matters for more than tidiness: a round-level check has to price a whole round before making any of its calls, and the only safe price is a worst case (chunks × a 60-second per-call assumption). That is not a bound on time but a cap on chunk count — with 800 seconds remaining it refuses any round past 11 chunks however fast the model is, which for a 200-row list at a token-aware batch size of 4 discarded recovery measured at under a second and turned a clean section into an error-severity `assessment_incomplete`. Pricing one call at a time bounds the wall clock exactly, keeps every row the budget did cover by stopping mid-pass, and treats the two rungs alike — priced per round, the cheap same-model rung could be refused while the slower, dearer escalation rung proceeded, because the two size their chunks differently. **What an operator sees when it fires:** the section reports `assessment_incomplete` (error) with the time budget named in the message and its real remedy — more Lambda time, or less work per section — rather than the generic "rows could not be scored", which implies the rows are unscorable when they were simply never attempted. It does *not* report `assessment_deadline_reached`: that rung sits below `assessment_incomplete` in the severity ladder and this guard only fires while rows are missing, so the warning is reachable only when a later rung scored every row anyway. Its message and the processing-report summary line were both corrected — each previously said escalation had been "cut short" in runs where escalation had in fact completed. This was the most concrete lead on the 900-second `Sandbox.Timedout` ×3 symptom reported on [#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894), but it is **not** confirmed as its cause — that failure has never been reproduced here, so #894 stays open for it and for the batch sizer. Behaviour is unchanged when no deadline is threaded in (local runs and non-Lambda callers) and unchanged whenever the remaining time is sufficient: a unit test pins that a 200-row list with ample budget recovers exactly as many rows, in exactly as many calls, as with no deadline at all. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).
- **A systemic confidence-assessment failure is visible again: degraded sections now publish a metric, and an alarm watches its volume** ([#996](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/996)). When the confidence pass fails deterministically, the Assessment Lambda keeps the extraction and degrades the section rather than failing the document — the right trade for one section, and the reason a *fleet-wide* confidence failure stopped being visible: such documents **complete**, so no failed execution, no DLQ message and no existing alarm moves, and `ProcessingIssueCount` is a DynamoDB attribute rather than a CloudWatch metric, so nothing aggregated it. A missing `bedrock:InvokeModel` grant, a confidence model whose input limit every section exceeds, or a model id not enabled in the region would have been visible only as an error-severity processing issue in the Sections panel, one document at a time. Each degrade now publishes `AssessmentConfidenceUnavailable` (value 1) into the stack's own metric namespace, and the new `AssessmentConfidenceUnavailableAlarm` fires at **ten or more within fifteen minutes**, publishing to `AlertsTopic` with the three most likely causes and their fixes in its description. The threshold watches volume rather than the first occurrence on purpose: unlike a stale-output purge failure, one degraded section is an expected, self-limiting outcome — an unusually large section against a small-context confidence model produces it with nothing misconfigured — while a systemic cause degrades every section of every document. The metric put is wrapped in its own `try`, because the path it sits on exists precisely to avoid failing a document whose extraction succeeded, so a lost telemetry point is the cheaper failure (a unit test pins that an unavailable CloudWatch does not fail the document). A **Confidence Assessment Degraded** widget is appended to the unified-pattern dashboard at a 15-minute period with the alarm threshold drawn as an annotation, so the trend and the trigger are read together; it is appended below the existing widgets, so no widget positions shift. No IAM or environment change was needed — `AssessmentFunction` already carried `METRIC_NAMESPACE` and a `cloudwatch:PutMetricData` statement scoped to that namespace. Stack resource count goes from 313 to 314 against CloudFormation's 500 limit. See [Monitoring](docs/monitoring.md#confidence-assessment-degraded).
- **The optional CloudFormation service role no longer amounts to account administrator, and it no longer claims to be least privilege** ([#927](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/927)). `iam-roles/cloudformation-management/IDP-Cloudformation-Service-Role.yaml` granted role and policy creation on `Resource: "*"` together with `iam:PassRole` on `"*"`, which is enough to create a role with any policy and pass it anywhere, so the role was transitively equivalent to administrator while its header comment described it as least privilege. Role creation and modification are now scoped to a `ManagedStackNamePrefix` name pattern and, for the actions that support the condition key, gated on `iam:PermissionsBoundary` matching a new **required** `CreatedRolePermissionsBoundaryArn` parameter — the same ARN must be passed to the IDP stack as its `PermissionsBoundaryArn`. That parameter is deliberately separate from the new optional `ServiceRolePermissionsBoundaryArn`, which caps the deployment role *itself*: one ceiling has to be tight and the other wide, so a single parameter could not serve both, and passing the tight ARN as the role's own boundary would leave it unable to deploy anything (the intersection contains neither `cloudformation:` nor `iam:`). `iam:PassRole` is scoped to those role paths with an `iam:PassedToService` condition, `iam:DeleteRolePermissionsBoundary` is gone, the trust policy is restricted to the CloudFormation service principal in the same account with a **single** statement using `StringEqualsIfExists` on `aws:SourceAccount` and `ArnLikeIfExists` on `aws:SourceArn` (an earlier two-statement shape paired plain operators with a `Null` both-absent guard, which matched no `Allow` at all — and so made the role unassumable — if CloudFormation populated exactly one of the two keys), and five explicit denies stop the boundary being stripped, either boundary policy being edited, the role editing itself, the role republishing its own `<StackName>-PassRolePolicy` to hand itself to any service, and IAM user or access-key creation. Three dead service wildcards were removed (`textract:*`, `sagemaker:*`, `application-autoscaling:*` — nothing declares a resource in any of them, and Textract and SageMaker are called at runtime by Lambda execution roles rather than by CloudFormation). `appsync:*` was removed in an earlier revision of this change on that same reasoning and has been **restored**, as its own `IDPLegacyAppSyncUpgradeCleanup` statement documented as vestigial and retained for upgrades only: a CloudFormation service role performs **deletions** as well as creations, so an in-place update of a stack created before 0.6.0 must delete the 151 `AWS::AppSync::*` resources that older template declared — under this role's permissions, not the caller's — and that in-place path is documented and supported with no minimum upgrade source. Deriving the required action set from the *current* templates alone, which is what the validator does, cannot see that obligation; applied consistently it would justify dropping `apigateway:*` the next time the UI transport changes. The statement can be deleted once no pre-0.6.0 stack remains to be upgraded. ⚠️ **Upgrade-visible, and it lands on stacks that already exist.** `RoleName` is unchanged (`<StackName>-CFServiceRole`), so updating the service-role stack replaces the policy on the *same* role ARN: there is no new role and no opt-in, and every IDP stack already pointed at that ARN starts deploying under the tightened policy on its next operation. Three configurations then wedge a stack in `UPDATE_ROLLBACK_FAILED`, because the missing action fails the rollback as well as the update — roles created before this change carry no permissions boundary, so `iam:UpdateRole` and friends are denied on them; a stack whose name does not match `ManagedStackNamePrefix` (matched case-sensitively) fails on role and policy writes; and setting the IDP stack's `PermissionsBoundaryArn` back to empty drops the property, which is a `iam:DeleteRolePermissionsBoundary` call the role no longer holds. All three are detectable before you update, and the README's new "Updating an Existing Deployment" section gives the detection command, the exact failure signature and the recovery for each, plus the `continue-update-rollback --resources-to-skip` path out. A new `RoleName` was considered as an opt-in and rejected: `RoleName` triggers replacement, so renaming inside the same stack would *delete* the role every existing IDP stack references and leave them unable to roll back at all. The real opt-in is deploying a second service-role stack under a new stack name and re-pointing each IDP stack at it with `update-stack --role-arn`, which the section documents. The remaining `service:*` grants stay broad **by design** — a service role must be able to create, update and roll back every resource type in every optional feature, and a permission missing from a rollback wedges the stack in `UPDATE_ROLLBACK_FAILED` (see [#632](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/632)) — and the template comments, `Description` and README now say so instead of claiming otherwise. `scripts/sdlc/validate_service_role_permissions.py`, the gate that should have caught this, now also fails on an IAM write action granted on `Resource: "*"` without a permissions-boundary condition and on a `PassRole` without `iam:PassedToService`; a mutation test pins that it fails on the pre-fix template. Those checks were themselves defeated by seven shapes that a hardening gate has to see, and each is now caught with a test that fails without the fix: a `!Sub`-built ARN covering every role in the account (the repository's *own* ARN convention, so the check was blind to the shape it was most likely to meet), a bare `Action: '*'`, a wildcarded action such as `iam:Pass*` or `iam:DeleteRolePermissionsB*`, `Effect: Allow` with `NotAction`, a boundary or `PassedToService` condition written with an `...IfExists` or `ForAllValues:` operator that is vacuous when the key is absent, a grant placed in an `AWS::IAM::Policy` or `AWS::IAM::RolePolicy` resource rather than inline on a role, and a lone `Null` guard accepted as if it were a `PassedToService` restriction. Parsing `!Sub` operand text needed a second YAML loader, `PatternCFNLoader`, which is covered by the same `test_cfn_loader_safety.py` tripwire as the existing one — it preserves more of the document, so every value it yields still has to be inert data. The gate deliberately reports only missing wildcards and IAM actions and does **not** report extra permissions. See [iam-roles/cloudformation-management/README.md](iam-roles/cloudformation-management/README.md) and [AWS services and IAM roles](docs/aws-services-and-roles.md#deployment-roles).
- **The Web UI API dispatcher now denies by default: an operation whose required groups were never declared is refused, not dispatched** ([#928](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/928)). Under AppSync, a field with no `cognito_groups` directive was denied at the API layer; after the move to API Gateway the dispatcher only authenticated, so group enforcement depended on every downstream resolver remembering to check and a newly added operation was open unless someone added one. The dispatcher now checks the caller's groups — taken from the verified JWT claim, never the request body — against a per-operation required-groups manifest before routing, and returns `403 Unauthorized` for a field with no manifest entry. The manifest is generated from `scripts/api_rbac_expectations.yaml`, the file the static scan and the live RBAC harness already assert against, so there is no second list to maintain; `make api-test-static` fails on drift and the dispatcher fails closed if the manifest is missing, unreadable, of an unsupported version, or **structurally invalid** — every policy entry is validated when the manifest is loaded, so a malformed one cannot reach the group comparison and be evaluated in a shape that admits the wrong caller. Because a deny-all state is per-request indistinguishable from a legitimate denial, the dispatcher also logs the fixed marker `API_RBAC_MANIFEST_UNAVAILABLE` at ERROR once per cold start when no usable manifest loaded; alarm on that string to tell a broken policy file apart from callers who lack the groups they need. The per-resolver checks are unchanged and still run: the dispatcher is the field-level floor, the resolver enforces per-object scope on top of it. ⚠️ **This is a floor for group-scoped operations only** — 26 of the 118 declared operations are declared `ANY`, where the dispatcher enforces authentication but not group membership, so a forgotten resolver check on one of those is still reachable by any authenticated caller ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)); the `idp_common.api_adapter` passthrough for events that carry their own `identity` sat outside it until the entry below closed that path ([#978](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/978)). ⚠️ **Adding an API operation now requires declaring its groups** in that file and regenerating the manifest (`python3 scripts/sdlc/generate_api_rbac_manifest.py`), or the new operation returns 403 for everyone including Admin. See [RBAC](docs/rbac.md) and [the REST migration notes](docs/migration-appsync-to-rest.md).
- **Agent Chat's group restriction is now also enforced inside the Lambda that runs the agents, and the streaming endpoint no longer lets the request body override a caller identity the transport proved** ([#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920)). Agent Chat is reachable two ways — `POST /op/sendAgentChatMessage`, whose resolver checks `cognito:groups`, and a Lambda Function URL that streams token deltas by driving `agent_chat_processor` directly — so the check in front of one path did not apply to the other. The processor now applies the same Admin/Author/Viewer check itself, before any agent call and outside the handler's convert-error-to-stream-frame path, using the group claim (and only that claim) that the resolver forwards; a unit test asserts the two group lists cannot drift apart, and the static scan now compares the processor's group tuple with the declared policy in both directions, so widening it fails too. **Be clear about what that gate does today: it is defence in depth, and on neither live path is it the check that denies a request.** On the dispatcher path the resolver already refuses an unauthorized caller before it invokes the processor, so the processor's copy never sees one; on the streaming path the Function URL forwards no Cognito group claim at all, so the gate has nothing to evaluate and stands down exactly as it does for backend IAM invocations (recorded as GAP-07 in `scripts/api_rbac_expectations.yaml` and AUTH.T14 in [the RBAC threat model](security/threat-modeling/feature-threats/rbac-authentication.md)). It is kept because it is the check that holds if the resolver's own gate is ever removed or bypassed, and because it becomes load-bearing the moment GAP-07 closes; the streaming route now applies it synchronously before the response is committed, since a denial raised after that point could only be rendered as an error frame on an HTTP 200. Both streaming routes resolve the caller through one helper that prefers the transport-verified principal and refuses a body-supplied identity that contradicts it with a 403, and both declare Pydantic request models so a malformed body is rejected with a 422 before any processing. On the commercial transport the browser actually uses, the SigV4 principal is a *pool-wide* Cognito session name rather than a per-user one, so attribution still comes from the body value — what changed is that the body can no longer displace a per-user identity when one is present, and cannot silently win a contradiction. `make api-test-static` gained Function-URL checks (S6–S9) that discover every `AWS::Lambda::Url` and its routes from the template and fail when a route is undeclared, prefers a client-supplied identity, omits the conflict refusal, or reaches a handler with no group check.
- **CloudWatch alarms now reach somebody: the `AdminEmail` address is subscribed to the alerts topic** ([#922](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/922)). Of the 12 CloudWatch alarms the stack creates, 11 publish directly to one topic, `AlertsTopic`, and the twelfth (`BedrockServiceOutageAlarm`) publishes to `CircuitBreakerTopic` and reaches `AlertsTopic` through the circuit-breaker manager Lambda — and **no `AWS::SNS::Subscription` to `AlertsTopic` existed**. The topic ARN was emitted as the `SNSAlertsTopicARN` output and an operator was tacitly expected to subscribe by hand, so a default deployment raised every alarm to an audience of nobody. This is the third instance of the same silence in this subsystem, after the non-existent metric name ([#746](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/746)) and the KMS grant that was gated on the circuit breaker. The new subscription follows the pattern `FailureNotificationSubscription` in `scripts/sdlc/cfn/codepipeline-s3.yml` already used: an email subscription behind a condition that is false when no address was supplied. `scripts/tests/test_alerts_topic_subscription.py` now pins that the topic has a subscriber and that, across **every** template in the repository, no alarm publishes to a topic whose subscriptions cannot exist on the deployments where the alarm does. ⚠️ **An SNS email subscription delivers nothing until the recipient clicks the confirmation link** — a second email, *"AWS Notification - Subscription Confirmation"*, arrives at `AdminEmail` and the subscription stays in `PendingConfirmation` until it is followed, without failing the deployment; the token is valid for about two days. Note also that every SNS email carries a one-click unsubscribe link, so a recipient can silently remove the subscription and nothing in the stack notices. ⚠️ **`--headless` deployments are not fixed by this change**: they strip the `AdminEmail` parameter, so they keep `AlertsTopic` and all 12 alarms and still get zero subscriptions. Subscribe your own recipients to the `SNSAlertsTopicARN` output — whether the headless variant should gain its own optional alerts-email parameter is still open. Not deploy-verified: the confirmation behaviour described here is AWS-documented SNS behaviour, not something observed on a live stack. See [Who receives the alerts](docs/monitoring.md#who-receives-the-alerts) for the status check and for subscribing a distribution list, AWS Chatbot or a pager instead of one mailbox.
- **A pipeline hook declaring `onError: fail` now actually aborts the document, at every hook point the active processing mode reaches** ([#919](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/919)). The policy is the documented way for a feature to *gate* processing, and PII redaction is the case that matters, but it worked at only one of the seven points. The dispatcher raised a bare `RuntimeError`, and the six post-step hook states caught `States.ALL` and routed **forward** on purpose (so a transient dispatcher fault cannot discard an otherwise-good document); `States.ALL` matches the fail policy too, and Step Functions takes the first matching catcher, so a redaction hook could fail and its document would be classified, extracted and delivered unredacted with nothing to signal it. The dispatcher now raises a distinct `HookFatalError` for that policy only, and each hook state catches that error name **before** its `States.ALL` entry, routing to a terminal `PreprocessingHookFailed` / `PostStepHookFailed` / `PostExtractionHookFailed` state; the forward-routing `States.ALL` behaviour for non-gating faults is unchanged. Catcher **order** is the whole fix, so `patterns/unified/tests/test_workflow_hook_fatal_catch.py` enumerates the hook states from the state-machine definition — a hook point added later is covered the day it appears — and that suite, along with the rest of `patterns/unified/tests`, now runs in both CIs (it was registered in `scripts/run_all_tests.py` but in neither). The GitLab integration suite's hook step additionally proves it live: a hook failing on purpose at `ocr.postHook` ends the execution FAILED with `HookFatalError` and never reaches classification. `onError` is also now a constrained enum (`Literal`) in the configuration models, not a bare string: a hand-edited config carrying `Fail` or `fail-fast` used to disable the gate silently, and is now rejected with the three accepted values named. ⚠️ **That validation runs on the stack-deployment path as well as on upload.** `update_configuration`'s custom resource builds `IDPConfig` for every configuration it saves and has no per-item `try`/`except`, so a `CustomConfigPath` YAML carrying an out-of-enum `onError` produces a **failed stack create/update** (`cfnresponse.FAILED` naming the offending field, e.g. `ocr -> postHook -> 0 -> onError: Input should be 'continue', 'skip-remaining' or 'fail' (type: literal_error)`), not a rejected upload. The failure is clean and rolls back — it is not the wedged-rollback class seen with empty `pricing.yaml` units — but fix the value in your custom config *before* upgrading rather than diagnosing it from a rollback. Records **already in the table** are not retro-validated: the dispatcher is packaged boto3-only and reads DynamoDB directly, so it still normalizes an unrecognised stored value to `continue` — now with a `logger.warning` naming the hook and the value, and an `onErrorInvalid` field on that hook's result so the disagreement appears in the Step Functions execution history instead of nowhere. Deliberately *not* promoted to `fail`: an unrecognised value has only ever been interpreted as `continue`, so no gate that ever worked can be rescued by tightening it, while every pipeline currently running on such a record would start failing documents on upgrade — and an older dispatcher reading a policy name added by a later release should degrade to "observe", not abort. ⚠️ **A `fail` policy at a hook point the active mode does not reach is silently inert.** In BDA mode (`use_bda: true`) the state machine has no `postOcr`, `postClassification` or `postExtraction` state, so the dispatcher is never invoked there and a gate registered at one of them never runs; the docs now carry a hook-point-by-mode table and this is tracked as [#982](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/982). When a gate *does* abort, the tracking row and UI show only `FAILED`, so both terminal `Fail` states now build their `Cause` with `CausePath`/`States.Format` from the dispatcher's own message — the `ExecutionFailed` event names the failing hook's `featureId` and hook point instead of only explaining the catcher-ordering invariant to a future developer. ⚠️ If you rely on `onError: fail` to gate, re-check documents processed before this release. See [Pipeline hooks](docs/feature-platform.md#pipeline-hooks) and [hook points by processing mode](docs/feature-platform.md#not-every-hook-point-exists-in-every-processing-mode).
- **A Lambda timeout is retried once in every workflow state, and a failed evaluation-failure recorder no longer takes the document with it** ([#917](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/917), [#918](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/918)). The single-attempt timeout retry policy had been applied to `EvaluationStep` only, so the other eleven Lambda task states — OCR, classification, extraction planning, shard extraction, shard merge, in-process extraction, assessment, results processing, policy classification, rule validation and summarization — still retried `Sandbox.Timedout` / `States.Timeout` / `Lambda.Unknown` on the transient ladder: eight attempts of a 900-second timeout at 2.5× backoff, about 5.2 hours holding a workflow-concurrency slot before failing the same way it failed the first time. Each of those states now carries its own timeout retrier with `MaxAttempts: 1`, and the throttling and Lambda-service ladders are untouched. Separately, `RecordEvaluationFailure`'s own `Catch` used `ResultPath: null`, which passed its input through unchanged — the bare document dict, with no `document` member — so the next state's `document.$: $.document` raised `States.Runtime`, which no `Catch` intercepts, failing the execution and discarding a fully processed document at the last step. That error is now captured under `$.RecordFailureError` and a `Pass` state rebuilds the `{ document: … }` envelope. `scripts/tests/test_state_machine_retry_policies.py` enumerates the Lambda task states out of `workflow.asl.json` (24 of them today), so a state added later is covered without editing the test. Because it enumerates rather than names states, it also makes one rule **repo-wide**: every Lambda task state must retry a throttling or service error at least three times, so a new state with no `Retry` block fails the gate. All 24 existing states already comply; the requirement is written up under [Adding a Lambda task state to the workflow](docs/configuration.md#adding-a-lambda-task-state-to-the-workflow). See [configuration](docs/configuration.md#step-functions-retry-configuration) and [evaluation](docs/evaluation.md).
- **The workflow-concurrency counter can no longer be driven below zero, and a counter that already went negative now repairs itself** ([#915](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/915), [#916](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/916)). Admission is gated on `active_count < MaxConcurrentWorkflows`, so a counter at -N ran N workflows above the configured ceiling indefinitely with nothing reporting it: documents processed, queues drained, every graph read healthy, and the stack simply spent more on Bedrock and Textract than it was told to. The workflow tracker's decrement was unconditional and per-invocation, so a redelivered terminal event (the EventBridge rule allows 3 retries) released a slot a second workflow still held. The decrement is now refused when the counter is already 0 — reported as the new `ConcurrencyCounterUnderflow` metric and `ConcurrencyCounterUnderflowAlarm`, and absorbed rather than raised, because the document is already terminal — and carries a `dec#<executionArn>` marker written in the same DynamoDB transaction, so each execution releases exactly one slot however many times its event is delivered. A counter that is already negative now repairs itself, on the **next admitted document**: the queue processor's increment already asked DynamoDB for the updated value, so a post-increment `active_count` of 0 or below means the counter was negative, and the processor raises it to the executions actually running plus the slot it just claimed (raising it only tightens admission, the conditional write guards on the exact value observed, and it can never write below zero). The negative value it held is published as `ConcurrencyCounterActive` *before* the repair, so `ConcurrencyCounterNegativeAlarm` still fires on a counter that healed itself. Detecting it there is what makes the repair reachable at all: the reconciler runs only when an increment is *refused* for being at the ceiling, which a negative counter never is. For calibration, a negative counter was never permanent even before this — each admission increments it and the now-floored decrements absorb the excess once it reaches the ceiling — so the harm it did was bounded to roughly one generation of over-admission rather than lasting forever. Also added: `ConcurrencyCounterNegativeAlarm`, TTL on `ConcurrencyTable` (`ExpiresAfter`) so the markers are reaped, and a belt-and-braces `try/except` around the tracker's `record_document_run` call. That last one changes no behaviour: `record_document_run` already wraps its whole body in `except Exception` and is documented as never raising, so a throttle or S3 error there was already swallowed and the decrement already ran. The outer guard is there so that contract cannot be broken silently by a later edit inside the function. See [Monitoring](docs/monitoring.md#workflow-concurrency-counter).
- **The documentation no longer says the Web UI talks to AWS AppSync, and a gate keeps it that way** ([#929](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/929)). AppSync was removed in 0.6.0 and replaced by an API Gateway REST API with a dispatcher Lambda, but the prose describing it was never swept: 172 lines across 47 documents still presented the GraphQL endpoint, its subscriptions and the renamed `AppSyncVisibility`/`UsePrivateAppSync` parameters as things a deployment has, and `CLAUDE.md` listed AWS AppSync under "Key AWS Services Used" a few hundred lines above its own correct explanation that AppSync had been removed. A reader following those documents looked for an endpoint no stack has had for nine releases. Every mention was triaged rather than replaced wholesale, because two kinds of mention are correct: prose that says what the architecture *used to* be, and identifiers deliberately keeping the old name (the retained `schema.graphql` typed contract, the `appsync:*` grant an in-place upgrade needs to delete a pre-0.6.0 stack's own API, the vestigial `APPSYNC_API_URL` env var). `docs/architecture.md` is rewritten rather than patched — the request path it described was the old one — and now also documents the three extension mechanisms the codebase actually has (pipeline hooks, the Feature Platform, `idp_common` extras), which an architecture document had no business omitting. `make check-retired-services` (`scripts/sdlc/check_retired_services.py`, run from `lint`, `fastlint` and `lint-cicd`, so both CIs) fails if a document presents a retired service as current, reading an explicit registry of retired services paired with a justified allowlist for the historical and vestigial mentions — the same triage pattern `scripts/security/dep_audit_allowlist.json` uses. The 172 figure is the gate's own count, reproducible with `mkdir -p /tmp/pre-929 && git archive c33e0725d^ | tar -x -C /tmp/pre-929 && python scripts/sdlc/check_retired_services.py --root /tmp/pre-929`. Because "AppSync" is not the misleading part of the worst claims — seven lines said status updates arrive "via GraphQL subscriptions", a mechanism that has not existed since 0.6.0, without naming the service at all — the registry also matches the mechanism-claim forms, while leaving the subscription *operations* alone: `schema.graphql` still declares them and the UI still generates types from them. A marker that says a service is gone exempts only the sentence it appears in, reconstructed across a Markdown block so an 80-column line wrap does not hide it; the earlier ±1-line window let every historical sentence in the corpus silently exempt its two neighbours, which is exactly how these documents are written. The reintroduction half of the check matches a `Type:` declaration rather than any occurrence of the resource type, so naming `AWS::AppSync::GraphQLApi` in a comment explaining why the legacy grant is retained no longer reports the service as back, and it no longer returns before the prose scan runs. An allowlist entry for a claim that another change owns carries `expiresAfterVersion`, which the registry loader compares against the `VERSION` file — offline, with no API call — so the exemption fails the build rather than outliving its reason. The gate also ignores a local build's own output: `.aws-sam/` was skipped only at the repository root, and the copy of `lib/idp_common_pkg` that the `feature-platform/` builds vendor into their Lambda source directories was not skipped at all, so anyone who had built locally saw 174 findings in gitignored files that CI, building from a fresh clone, never sees — a gate that is red for a local-only reason teaches people to ignore it.

- **A model that cannot emit a valid tool-use sequence fails in seconds with the model named, instead of being retried for 45 minutes** ([#895](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/895)). Advanced (agentic) extraction on Amazon Nova Lite fails mid-stream with Bedrock's `modelStreamErrorException: Model produced invalid sequence as part of ToolUse` — the model emits a `toolUse` block the protocol rejects, and it emits the same block on every attempt. Because `modelStreamErrorException` is otherwise a genuinely transient code (`ConverseStream` really does break mid-stream for transport reasons), the classifier surfaced every one as `TransientError` and the state machine retried it up to eight times per shard task: one three-hour benchmark refresh logged 247 of them across 40 shard-runtime log streams, with documents sitting in `ExtractionShardMap` for 45+ minutes and the Nova Lite grid taking 3.5 hours against ~2.5 for every other model. The code stays in the transient vocabulary — real mid-stream breaks are still retried — but `transient_errors` now checks a narrow `DETERMINISTIC_MESSAGE_MARKERS` list **first**, ahead of the error-code, exception-type and class-name lookups, so this one outcome overrides the code-level verdict. The extraction path translates it into a named `ModelInvalidToolUseSequence` whose message states that the failure reproduces on retry with the same request and so is treated as deterministic rather than transient, names the model id that produced it, and lists three remedies: reduce `extraction.agentic.shard_token_budget` / `max_pages_per_shard` so each call emits less (AWS's [Nova tool-use troubleshooting guide](https://docs.aws.amazon.com/nova/latest/userguide/tools-troubleshooting.html) attributes this error largely to inference parameters and output budget), switch to a model measured on this path (Claude Sonnet 5 / Sonnet 4.6 / Opus 5, OpenAI GPT-6 Astra, xAI Grok 4.6), or use `extraction.mode: simple`, which needs no tool use in its default configuration. Upgrade-visible: a configuration that previously ground through 45 minutes of retries before failing now fails within seconds with a message naming the cause — better, but a behaviour change, and it applies to any model that produces this outcome, not only Nova Lite. Nova Lite is unaffected everywhere else: it remains fully supported, and the default, for the confidence-assessment pass and works for Simple extraction. See [Extraction and Confidence](docs/extraction-and-confidence.md#supported-models-for-agentic-extraction).

- **`idp-cli` output meant for a program is no longer rendered as terminal output: `--json` parses, and YAML printed to stdout is not folded to the console width** ([#905](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/905)). `config-revisions --json` emitted its payload with Rich's `print_json`, so wherever styling is on — a real terminal, a pty, or `FORCE_COLOR` in the environment — the JSON began with an ANSI escape and `idp-cli config-revisions --json | jq` failed at character 0. An audit of every machine-readable path found the same defect in eight more places, and two of the three ways Rich alters a payload need no colour at all: it hard-wraps at the console width — taken from whichever standard stream is a terminal, falling back to 80 columns when none is — so `idp-cli config-download > config.yaml` broke every longer line *in the redirected file*; and it reads `[...]` inside a string value as markup, so a field description reading `value [in USD]` rendered as `value ` and a prompt containing `[/bold]` failed the command outright with `MarkupError`. All nine paths — `config-revisions --json`, `status --format json`, the YAML that `config-download` and `config-create` print when `--output` is omitted, and the JSON schemas from `discover`, `discover-multidoc` and `bootstrap` — now write through one `emit_json` / `emit_raw` helper that puts the payload on stdout unchanged. Human-facing tables, progress and status lines are still styled. A static test guard fails if a new command renders a payload through the console again. Separately, the `idp_cli` suite no longer depends on the environment's colour setting: an autouse fixture pins the CLI console to no styling at a fixed width, so it passes identically plain, under `FORCE_COLOR=1` and under `NO_COLOR=1 TERM=dumb`. The plain, CI-equivalent run was green before this change; it was forced-colour environments — a pty runner, or `FORCE_COLOR` exported in a shell — where two tests failed, so the suite was passing without verifying colour-independent behaviour. See [IDP CLI](docs/idp-cli.md#machine-readable-output).

- **`idp-cli` progress and status lines move to stderr on the commands whose stdout carries a payload, so redirecting that stdout yields the payload alone** ([#905](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/905)). Writing the payload unrendered was necessary but not sufficient: human text still shared stdout with it. `idp-cli status --batch-id b --format json | jq` printed four lines of search progress first and failed at character 0 — the same symptom #905 reported — and `idp-cli config-download > config.yaml` put `Downloading config from stack: my-stack` on the file's first line, which `yaml.safe_load` then accepted *silently* as an extra top-level key, so the config was wrong with nothing to say so. `status`, `config-download` and `bootstrap` in local mode now send their progress, headers and error lines to stderr; interactive users still see every line, and `2>/dev/null` discards them without touching the payload. Rendered tables stay on stdout, where they are the output. One behaviour change: `status --show-details --format json` no longer renders the matching-documents table, because it would land in front of the JSON that already lists those documents. `discover` and `discover-multidoc` still interleave a `Discovered schemas:` heading with their payload on stdout — use `-o` / `--output` with those two, as [IDP CLI](docs/idp-cli.md#machine-readable-output) now says.

- **Confidence scoring stops spending model calls on a batch that can never fit, and says why** ([#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894)). The self-healing ladder halves any confidence batch whose response the model truncated, which converges only while a smaller batch can fit; with a class marked `x-aws-idp-multi-instance: true` one row of the outer list is a whole document instance carrying its own long inner list, so a single-row call still truncated and halving could not help. The ladder had no terminal condition for that case: it kept re-running the impossible call through the retry rounds and the bisection tree, then reported the generic `assessment_incomplete`, whose remedy — shrink `extraction.confidence.list_batch_size` — is the one thing that provably cannot work when the batch is already one row. It now stops as soon as a one-row call truncates: the same-model retry rung is skipped, at most one escalation round runs (a larger output cap is the only remedy that can succeed), and the section reports an error-severity `assessment_row_too_large` naming the confidence model, its output-token cap, the list field and class, the offending row's approximate serialized size, and that lowering `list_batch_size` cannot help. On a shape that batches (8 rows at batch 4) this halves the primary calls, 28 → 14; on the single-outer-row shape #894 reports, the call count is unchanged and the gain is the diagnosis. ⚠️ Two things stay open under #894. The batch sizer still measures only the outer row's column count (it logs `cols=2 per_row~80` for a row holding 100 transactions), so **a multi-instance class with a long inner list still cannot be fully assessed**. And the reported symptom — an Assessment Lambda hitting its 900-second limit three times (`Sandbox.Timedout` ×3) after a 45-second extraction, leaving the document in `ASSESSING` — is **not explained** by this loop and so is not fixed by this change: the retry rung already stopped on no progress before it, and the wall-clock deadline guard was already present in the release where those timeouts were observed. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).

- **A failed confidence pass no longer throws away the extraction it was scoring** ([#901](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/901)). Confidence scoring runs after extraction has written its results and been paid for, but a deterministic `ValidationException: Input is too long for requested model.` from the confidence model failed the whole document — two observed runs discarded $17.34 and $7.05 of extraction that had returned 1,200 of 1,200 rows at 1.000 cell accuracy, in order to report the loss of the advisory part. A deterministic (non-retryable) failure in the Assessment step now degrades instead: the extracted data is returned, the document succeeds, and the missing confidence is recorded as an error-severity `assessment_failed_confidence_unavailable` processing issue on the section, shown in the **Status** column of the document's Sections panel (its popover lists the issue). Note that this issue is written to the section record only, not into the section's `result.json`, so it does not appear in the Visual Editor's Processing Report tab, which reads that file. Transient failures — throttling, read timeouts, 5xx — are untouched and still raise so Step Functions retries the section. Upgrade-visible: a document that previously failed outright now completes with an error-severity processing issue and no confidence values on the affected section, so HITL confidence routing and the UI threshold signals do not apply to it. Sections whose scored rows fall materially short of the extracted rows also now report `assessment_coverage_incomplete` (warning past 5% of rows unscored, error at 25% or more **and** at least 10 unscored rows, with per-field counts) — a shortfall the completeness gate computed but every caller discarded — suppressed when the self-healing ladder already reported an error for the same section, which describes the same rows with a cause attached. Automatically re-batching an oversized confidence input so the pass succeeds rather than degrades is still open as part of #901. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).

- **Long-context Claude models (a `:1m` model ID) are no longer reported at up to double their real cost** ([#899](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/899)). `config_library/pricing.yaml` carried a separate entry per `:1m` model at a 2× input / 1.5× output premium, in all three region blocks, for Sonnet 4.6 and 5 and Opus 4.6, 4.7, 4.8 and 5, and applied it to every token metered under that key. No such premium exists for those models. Bedrock's published price list gives each of the six exactly one input-token rate per routing mode and service tier, with no token-volume band on any model — verified against the AWS Price List offer file `AmazonBedrockFoundationModels`, version 20260911124410, retrieved 2026-09-17, where a long-context band would appear as a distinct `-long-ctx` usage type as it does for two OpenAI models — and Anthropic states the same directly, pricing the full 1M-token context window at standard per-token rates on Claude 4.6 and later, so a 900K-token request costs the same per token as a 9K-token one. The 2×/1.5× structure was Anthropic's pricing for the earlier Sonnet 4 / Sonnet 4.5 1M-context beta. The suffix also never reaches Bedrock — the client strips it and sends the `context-1m-2025-08-07` beta header — so it survived only in the metering key, where the pricing table matched it. A benchmark arm was reported at $125.42 against $68.82 for the same work, a 1.82× gap on token volumes that differed by 1.06×; 94% of the apparent gap was the rate card. Nothing was ever overspent; the defect was in reporting, but it made these models look unaffordable in every document cost table, Athena rollup, benchmark summary and evaluation report. Metering keys now name the model actually invoked, so they carry no `:1m` suffix (`idp_common.bedrock.model_utils.metering_model_id`, applied at both emission sites; a service-tier suffix like `:flex` is still kept, because it does re-price every request made in it), and the 18 `:1m` pricing entries are retained at their base model's rates — the file doubles as the model-ID list configuration validation accepts, and `:1m` keys still arrive from older metering and from the Athena rollups. **Upgrade-visible:** reported costs for `:1m` models drop, by up to 2× on input tokens, so cost comparisons across this upgrade are not like-for-like. Cost figures already written to the reporting tables are not rewritten; only new records, and recomputations of old metering, use the corrected rates. A deployment holding a saved Custom pricing table is corrected too for new records, since its stale `:1m` rows are no longer looked up, but its premium values still apply when an old `:1m`-keyed record is recomputed and on the `data_mart_rollup` agent-cost path, which builds its key from the raw configured model ID — restore pricing defaults to clear them. See [cost considerations](docs/cost-calculator.md#what-the-estimate-cannot-express-prices-that-change-with-request-size).

- **A document uploaded once is processed once, however many times SQS redelivers its message** ([#904](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/904)). Under a saturated queue the `QueueProcessor` Lambda ran at the 128 MB default with 50-message batches and a 30 s timeout, so two thirds of its invocations under load timed out mid-batch. Lambda then reported no batch outcome, SQS redelivered every message in the batch including those whose workflow had already started, and each redelivery started another execution because executions had no name: one upload ran six times and a 4,384-document batch produced 14,727 executions, all billed, with every alarm reading OK. Executions are now named after the SQS message (`<basename>-<message-id>`), so a redelivered message is refused by Step Functions and acked as a no-op with its concurrency slot handed back, and each message is deleted the moment its execution exists instead of at the end of the batch. The processor runs at 1024 MB with 10-message batches and a 60 s timeout, `DocumentQueue` keeps its ~8 hour retry window (500 receives × 60 s), and a new `QueueProcessorErrorsAlarm` reports the processor timeouts that no failure-keyed alarm could see. Upgrade-visible: execution names change from random UUIDs to `<basename>-<message-id>`, and the new alarm fires on the first timeout. See [Monitoring](docs/monitoring.md#queueprocessorerrorsalarm--a-processor-that-cannot-finish-its-batches) and [Troubleshooting](docs/troubleshooting.md#documents-processed-more-than-once).

- **Test Studio: running a test set against a large configuration profile no longer fails with "Item size has exceeded the maximum allowed size".** The configuration table gzip-compresses profile bodies, so a profile can be several megabytes raw, but the test runner copied the decompressed body inline onto the run's DynamoDB metadata record. Any profile past roughly 390KB of JSON therefore saved fine on the Configuration page and then failed every Run Test at submit with DynamoDB's raw error. The run now records its captured configuration as a gzip Binary attribute in the same shape the configuration table uses, so a profile that fits there fits on every run that captures it; results, comparison and MLflow export read the compressed form and still read runs created before this change. A body that does not fit even compressed is refused at submit with a message that names the raw and compressed sizes. See [Troubleshooting](docs/troubleshooting.md).

- **The Schema Builder's Evaluation Method dropdown now offers methods for a field that references a shared class, instead of rendering empty** ([#906](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/906)). A property written as a bare `{"$ref": "#/$defs/Address"}` carries no `type` of its own — the type lives on the referenced definition — and the attribute inspector filtered the eight evaluation methods by the property's own `type`, so every option was filtered out: the field could not be given an evaluation method from the UI at all, with nothing on screen saying why. That shape is not exotic, and the two routes to it disagreed, which is what made the bug look intermittent: adding an attribute that references an existing class writes `$ref` with no `type`, while the inspector's "Reference Existing Class" picker keeps `type: "object"` beside it. The inspector now follows the `#/$defs/<name>` pointer into the designer's class list and filters on the referenced definition's declared type, so an object reference offers Semantic and LLM exactly as an inline `"type": "object"` field always did, and a reference to a scalar definition offers that scalar's methods. If the type still cannot be determined — a dangling or remote reference — every method except Hungarian is offered rather than none, because an empty dropdown removes the only way to configure the field while an over-broad one still lets the user choose; Hungarian is withheld there because it needs a list of objects, which a field reaching that fallback never is. Three other places that read a property's `type` without resolving `$ref` are corrected with the same helper: the preview's Statistics tab counted a bare-`$ref` property into no type bucket at all, and a JSON default value or const typed into such a field was stored as a raw string instead of being parsed. This is not a regression: the type filter has behaved this way since it was introduced in February 2026 and reproduces on 0.6.7.dev1. The backend was never affected — Stickler's JSON Schema field converter resolves `$ref` natively — so a method set by hand-editing the schema JSON was always honored at evaluation time.
...
- **Test Studio → Executions list no longer times out at 20 s on stacks that have accumulated a few hundred test runs, and the sample-doc probe used by the Comparator Changes panel no longer scans the whole tracking table.** Two co-located regressions with the same root cause — an unbounded DynamoDB read on the API-Gateway request path. `getTestRuns` was hydrating every testrun in the selected date range via a serial 100-key `BatchGetItem` loop that also ignored `UnprocessedKeys`; on a stack with ~500 runs it hit the dispatcher's 20 s read timeout, and any items DDB deferred were silently dropped so the list appeared shorter than it was. The resolver now caps the GSI query at `_GET_TEST_RUNS_ABSOLUTE_MAX = 100` (single-batch, guaranteed sub-second) and takes a new optional `maxItems: Int` GraphQL argument so an operator can under-fetch (or match the historical shape) from a real Cloudscape `CollectionPreferences` modal in place of the previous page-size cycler. `UnprocessedKeys` are re-issued with capped exponential backoff (six attempts, 50 ms → 500 ms) rather than dropped. Separately, the sample-doc probe behind the Comparator Changes panel (`_iter_completed_doc_keys`) was running a paginated full-table Scan with a client-side `begins_with(PK, "doc#{run_id}")` filter — on a tracking table with tens of thousands of doc rows the Scan alone exceeded API Gateway's 29 s ceiling and returned 504 for the whole panel; it now reads the testrun's `Files` list from its `metadata` row and `BatchGetItem`s the specific `doc#{run_id}/{file}` keys, terminating in one round trip. The UI surfaces a "Showing the N most recent test runs" info banner when the response filled the requested cap, and the settings persist to `localStorage`. See [test_results_resolver/index.py](nested/api-resolvers/src/lambda/test_results_resolver/index.py) and [TestResultsList.tsx](src/ui/src/components/test-studio/TestResultsList.tsx).

- **Cost reporting no longer misprices cache tokens: a model's price is resolved by exact id, and an unpriced service is reported as unpriced instead of $0.00** ([#926](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/926)). The pricing lookup behind the `metering` table fell back to a case-insensitive substring match in both directions when it found no exact entry, so a model id with no pricing row of its own bound to a *related* row and, within it, `cacheReadInputTokens` bound to that row's `inputTokens` price — the first unit every Bedrock row lists. The measured case is `eu.amazon.nova-2-lite-v1:0:flex` (selectable, cache-capable, and with no pricing row of its own): a cache read resolved to `3.9E-7`, the *base*-tier fresh-input rate, against its real `4.88E-8` — 7.99x. Both directions of error were possible: cache reads cost a tenth of fresh input on Claude and a quarter on Nova, so charging them at the input rate overstates, but a cache **write** costs 1.25x fresh input on Claude, so charging it at the input rate *understates* it by 20%. Resolution is now by exact key with progressively shorter `/`-delimited suffixes, longest match winning — the same rule the benchmark harness already used — with no substring step. A unit an existing pricing entry deliberately omits still costs $0.00, but a service with **no** entry now writes SQL `NULL` to `unit_cost` and `estimated_cost` with a WARNING naming it, so `WHERE unit_cost IS NULL` finds the gap instead of it reading as free usage; `SUM()` totals are unchanged either way. The same substring rule existed a second time, independently, in the web UI's Estimated Cost table (`DocumentPanel.tsx`), where it produced the same 7.99x figure; that lookup is now the identical exact-key walk, extracted into its own module with unit tests. ⚠️ A model with a complete pricing entry was always priced correctly, so this affected only deployments that used a model id with no entry of its own or an entry missing `cacheReadInputTokens` — among shipped selectable models, the EU Nova 2 Lite service tiers. Cost figures for a cache-heavy workload on one of those, reported by an earlier release, are wrong and should be re-run. Cost comparisons already published in this CHANGELOG and in the docs predate this fix and are being re-derived and restated separately under [#966](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/966), which is why this entry restates no published figure of its own.

  Removing the fallback exposed what it had been papering over, all of it fixed here. Both shipped LambdaHook pricing rows (`GENAIIDP-mistral-ocr-hook` at $0.004/page, `GENAIIDP-cohere-parse-hook` at $0.0015/page) were the only two keys in the table with no `/`, and the metering key embedded the hook's full ARN — so they had *never* matched exactly and the substring fallback was the only reason they priced at all. The metering key now carries the bare function name (`{context}/lambda_hook/{function-name}`) instead of the ARN, because an ARN embeds an account id and a region and so can never be a shippable pricing key, and its `:` delimiter is not something a `/`-suffix walk can split; both rows are re-keyed to `lambda_hook/<function-name>` to match, and a bare-`GENAIIDP-<name>` key in an existing user config still resolves through the suffix walk. `utils.lambda_metering` emitted the unit `invocations` while `pricing.yaml` prices `requests` — the key matched so nothing warned, but every Lambda invocation costed $0.00; the emitter now says `requests`. The BDA process-results function's deliberate `bda/documents-skip` counter gains an explicit `$0.0` row, so a skipped document no longer trips the incomplete-spend warning. Missing pricing rows were added, including `us.anthropic.claude-3-5-sonnet-20240620-v1:0`, which shipped presets pin, and `global.amazon.nova-pro-v1:0`, which two configuration dropdowns offer and which had no entry at all. The last of those was found by fixing the coverage test, which enumerated only CloudFormation `AllowedValues:` lists and therefore read **zero** models from `patterns/unified/template.yaml`, whose dropdowns are JSON-Schema `enum:` blocks: it now covers 87 selectable model ids rather than 34. See [Reporting Database](docs/reporting-database.md#cost-calculation-process) and [Lambda hook inference](docs/lambda-hook-inference.md#metering-and-cost-tracking).

  ⚠️ **Two things this entry does not claim.** Cache reads were **not** mispriced for every cache-heavy model: the pinned `bedrock/us.anthropic.claude-sonnet-4-5-20250929-v1:0` entry already carried `cacheReadInputTokens: 3.3E-7` before this change, so the substring fallback never ran for that model and its cache reads were always priced correctly. And reported cost was not uniformly *too high*: as described above, `cacheWriteInputTokens` was charged at the fresh-input rate, which is 80% of its real Claude rate, so the error ran in both directions.
- **Two controls that existed but were never consulted — the CLI is type-checked, and X-Ray traces can be filtered by document id — plus an explicit package marker for `idp_common.agents.utils`.** Both controls failed silently, in the direction that produces no output. (1) `pyrightconfig.json` listed `idp_cli/idp_cli` in `include`, a path that has not existed since the CLI moved to `lib/idp_cli_pkg/idp_cli`; basedpyright contributes nothing for a missing include entry, so `cli.py` (6,791 lines) and the rest of the package were checked by nothing while `make typecheck` looked clean over the 329 files that remained. The corrected path adds 6 files and exposed 4 warnings, all pandas-stub artifacts where manifest values flow into `os.path` calls, now fixed rather than baselined. The repair also covers the three first-party distributions that had never been in `include` at all — `lib/idp_sdk/idp_sdk`, `lib/idp_feature_sdk/idp_feature_sdk` and `lib/idp_mcp_connector_pkg/idp_mcp_connector` — which measured 64 further files and ~29,000 further lines for **zero** new errors, so there was no reason to leave them out. `make typecheck` also goes from red to green, as two pre-existing `reportOperatorIssue` errors in `src/lambda/calculate_capacity/index.py` are fixed by making a page count explicitly `float` — but only **in a checkout where basedpyright can resolve a virtual environment** (measured: `develop` exits 3 with 2 errors over 329 files; this branch exits 0 with 0 errors over 399). `pyrightconfig.json` sets `venvPath: "."` and `venv: ".venv"`, so where no `.venv` exists at the repository root — a bare `git worktree`, for instance — basedpyright exits 3 and `make` exits 2 no matter what this change does. That caveat costs nothing here because **neither CI runs `make typecheck`**: it is in neither `lint` nor `lint-cicd` nor either CI config. What CI runs is `make typecheck-pr`, and `scripts/sdlc/typecheck_pr_changes.py` overwrites `include` with just the changed files and returns `returncode if returncode in [0, 1] else 0`, which swallows exit 3 outright. Tracked as [#975](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/975), which measures the wider version of the same gap: both Python gates skip 225 tracked files between them, including every file under `scripts/tests/` — this PR's own new guards among them. (2) Four pipeline Lambdas (OCR, classification, extraction, assessment) passed `{document.id}` — a one-element set — to `xray_recorder.put_annotation`, which **drops** it: `aws_xray_sdk`'s `Entity.put_annotation` validates the value against `(int, float, bool, str)` and on a mismatch logs `ignoring unsupported annotation value type` and returns without storing anything. It never stringifies. So trace filtering by document id did not work on exactly the four functions that do the expensive work; the two rule-validation functions already had it right. All 15 `put_annotation` calls in the tree are now correct (the repository has no `put_metadata` calls). A related template-side asymmetry is **not** fixed here and is tracked as [#983](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/983): `EnableXRayTracing` controls only the two state machines, 19 Lambdas hardcode `Tracing: Active` so the parameter cannot turn Lambda tracing off, and the two rule-validation functions — the two whose `put_annotation` calls were already correct — declare no `Tracing` at all and so run in `PassThrough`, where a segment exists to hold the annotation only if something upstream already sampled. (3) `idp_common/agents/utils/` had no `__init__.py` and now has one, which makes the package boundary explicit. ⚠️ **Correction to the original issue ([#924](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/924)): the user-facing failure it described never happened, and no release was affected.** The issue held that setuptools `packages.find` skips a directory with no `__init__.py`, so the subpackage was absent from the built wheel and `agent_factory.create_conversational_agent()`'s unguarded `..utils.conversation_manager` / `..utils.memory_provider` imports would raise `ModuleNotFoundError` on a non-editable install. Measured, they would not: for a `pyproject.toml` `[tool.setuptools.packages.find]` table `namespaces` defaults to **true**, so setuptools dispatches to `PEP420PackageFinder` and discovers an unmarked directory anyway. `read_configuration(expand=True)` on the pre-fix tree returned 82 packages including `idp_common.agents.utils`; a wheel built from pre-fix source contained both modules and imported cleanly once installed `--no-deps` into a clean virtual environment. The `__init__.py` is kept as hygiene — depending on the `namespaces` default is fragile, because the same tree behaves differently under an explicit `packages` list, under `setup.cfg`'s strict `find`, and under any tool that walks the directories itself. ⚠️ Nothing could have caught any of the three, so each ships with the assertion that closes its class: `scripts/tests/test_pyright_config.py` fails if any `include` path is missing or holds no Python, **derives the expected inventory from `lib/*/pyproject.toml`** rather than from a list in the test body (a hardcoded list is the same silent-hole shape as the bug: a new distribution absent from `include` would not have been noticed), and fails if an `exclude` entry shadows an `include` entry, which basedpyright resolves silently in `exclude`'s favour; `scripts/tests/test_package_discovery.py` enforces the explicit-marker **policy** across all five first-party distributions (documented as a policy gate, deliberately stricter than setuptools, not a model of it) and, separately, **builds each wheel offline and requires every source package to be in it** — the policy test honours the pyproject `exclude` array by design, which left `exclude` as an unguarded route to a module that exists in the tree and not in the wheel; and `scripts/tests/test_xray_annotations.py` parses the AST and fails on a collection literal in an annotation value **passed positionally or by keyword**, on an annotation key outside `aws_xray_sdk`'s `_valid_annotation_key_characters` (`[A-Za-z0-9_]`, likewise dropped silently), and on a `*`/`**` splat that would hide either, then backs all three up by driving a real `aws_xray_sdk` `Segment` offline and asserting which keys and values the SDK actually keeps. Ruff's `B018` is **not** the answer for (2) and was not enabled: measured, it flags a useless expression *statement* and not a set literal in argument position, and `patterns/` is excluded from ruff wholesale in any case. ([#923](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/923), [#924](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/924), [#925](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/925))

- **Advanced extraction's shard agents are now told that their tables have already been parsed** ([#900](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/900)). When the pre-flight table parse succeeds, the agent's instructions gain a `PRE-PARSED TABLE DATA AVAILABLE` block: the table count, the total row count, the column list, what the `--- PAGE N ---` markers mean, and the `parse_table` → `map_table_to_schema` → `finalize_table_extraction` sequence, ending with the point that `finalize` reads the mapped rows from the agent's own state so it never has to write the rows out itself. That block was appended on the single-pass path only, so the sharded path — the default for any multi-page table document, and the path the page-range wording was written for — sent none of it, even after its shards started running the same pre-flight parse. It now comes from one shared helper called by both paths, and each shard agent additionally receives its own concrete page range plus a scope note: its text already holds only its assigned pages (so all of it, including the column-header block, goes to `parse_table`), and the block's table and row totals are for the whole section, so parsing fewer rows is correct for a shard rather than incomplete. The single-pass instruction is byte-identical to before, which a unit test pins. ⚠️ **The saving is unmeasured.** Confirming it needs a live before-and-after benchmark arm; the expected direction (fewer extraction output tokens with completeness unchanged) is inferred from the analogous single-pass fix in the [advanced extraction study](docs/benchmarking/studies/advanced-extraction-refinements.md), which cut extraction output tokens 55% over an identical 12-run arm. See [extraction and confidence](docs/extraction-and-confidence.md#deterministic-table-parsing).

- **The Web UI API dispatcher's log group is encrypted with the customer-managed key, and takes its retention from `LogRetentionDays`** ([#972](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/972)). `HttpApiDispatcherLogGroup` in `nested/api-resolvers/template.yaml` set no `KmsKeyId`, so the highest-traffic log group in the deployment — the dispatcher fronting every routable operation of the Web UI's REST API — was encrypted with the AWS-owned CloudWatch Logs key while 111 of the 114 log groups in the four templates that receive the customer-managed key already used it — `template.yaml` (56 groups), `nested/api-resolvers/template.yaml` (33), `patterns/unified/template.yaml` (20) and `nested/multi-doc-discovery/template.yaml` (5). The remedy was one property: the key's `Allow CloudWatch Logs to use the key` statement is unconditional, and `CustomerManagedEncryptionKeyArn` was already declared in that template and already passed by the parent, so no key-policy, parameter or wiring change was needed. The same resource hardcoded `RetentionInDays: 30` — the only one of the 114 that did not use the parameter — which made `LogRetentionDays` a partial control; it now uses `!Ref LogRetentionDays`. That parameter defaults to `30`, so a default deployment sees no change in effective retention and only a stack that has overridden it is affected. The two remaining unencrypted groups (`StacknameCheckFunctionLogGroup`, `ReadPreviousIDPPatternFunctionLogGroup`) are unchanged: both belong to custom-resource-only Lambdas that run solely during a stack operation and both already carry the matching cfn_nag `W84` suppression and checkov `CKV_AWS_158` skip.

  Encryption here had been a convention that nothing checked, so the fix comes with a gate. `scripts/tests/test_log_group_encryption.py` enumerates `AWS::Logs::LogGroup` resources by parsing the source templates and asserts that each one sets `KmsKeyId` — resolving `Fn::If` branches and treating `AWS::NoValue` and the empty string as absent, in both short and long intrinsic form — and that each takes `RetentionInDays` from `Ref` of `LogRetentionDays` specifically, so that wrapping a literal in an intrinsic or pointing at a different parameter does not satisfy it. That runs over the four templates that own the key or take its ARN as a mandatory parameter, covering 114 of the repository's 158 log groups. The encryption rule's only exemptions are the two custom-resource groups above; each is verified rather than trusted, by requiring the `W84` suppression to still be present with a written reason and by re-deriving the custom-resource-only property from the template through the sibling gate in `test_lambda_log_groups.py`. The retention rule has no exemptions.

  Counting the wider class honestly: 35 log groups across 14 further templates do not set `KmsKeyId` unconditionally. Of those, 24 across 8 templates omit the property outright — separate feature, sample-hook and verification-fixture stacks that cannot reach the main stack's key without new wiring — and are recorded as a pinned known set the gate fails on if it grows *or* shrinks. The other 11 across 6 templates set it conditionally on a `CustomerManagedEncryptionKeyArn` parameter that defaults to empty, which is the correct form for a template whose key really is optional; six of them are in `feature-platform/main-stack-extensions/template.yaml`, whose only deployment is as a nested stack that the parent unconditionally passes the key to, and the gate now asserts that unconditional wiring rather than assuming it. None of the 11 is unencrypted in a real deployment. Membership in all three categories is derived from a structural property of each template rather than a hand-kept list, and a new meta-test discovers every log-group-declaring template in the repository by content and fails if any is covered by no rule — the check whose absence let the first draft of this gate ship with those 11 groups in no category at all. It runs under `make test-packages-cicd`, which both CIs already execute.
- **Every UI API resolver Lambda that logs its request now redacts it with the canonical denylist, and none writes a whole chat prompt or a raw Cognito identity to CloudWatch** ([#921](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/921)). Twelve resolvers under `nested/api-resolvers/src/lambda/` are changed, in two groups. **Ten** carried their own shortened hand-copy of the key list in `idp_common.utils.log_sanitizer`; every copy was eight key names short of it (credential- and key-style names), and for **six** of those eight — `access_key`, `accesskey`, `passwd`, `private_key`, `privatekey`, `x-api-key` — a matching request field was logged rather than replaced with `***REDACTED***`. (The other two, `secret_key` and `secretkey`, were already redacted: the copies matched by substring and listed `secret`. `x-api-key` was a real gap because the copies had `apikey` and `api_key`, and neither is a substring of the hyphenated form.) Nothing compared the copies, so nothing reported the gap. **Two more** — `finetuning_jobs_resolver` and `list_documents_range_resolver` — did not redact at all: each dumped its entire invocation event, `identity.claims` (Cognito `sub`, `email`, group membership) included, on every authenticated call. All twelve now call `sanitize_event_for_logging`: the three that carry an `idp-common` layer import it from the library, and the nine that carry no layer — SAM packages each function from its own `CodeUri`, so they cannot reach the library at runtime — ship a byte-identical committed copy of the stdlib-only module, synced by `scripts/sync_resolver_log_sanitizer.sh` (the same guarded-vendoring pattern as the document-list resolvers' vendored `config_scope.py`, and as `src/lambda/chat_stream_processor/vendored/`). The canonical redactor also **truncates** long string values, which none of the hand-copies did: a string held directly under a content-shaped key (`prompt`, `text`, `content`, `body`, `answer`, `response`, `markdown`, `extracted_text` and similar) is capped at 500 characters with a `...[TRUNCATED N chars for log hygiene]` marker. Concretely, `agent_chat_resolver` logs its whole `arguments` object and accepts a prompt of up to 100,000 characters, so a user's full chat message used to land in CloudWatch Logs verbatim; it is now capped at 500 characters. `scripts/tests/test_resolver_log_sanitizer.py` now fails if **any** resolver logs its invocation event without sanitizing it — checked on the dataflow, so `f"{event}"`, `logger.info("%s", event)` and `print(event)` are caught as readily as `json.dumps(event)`, while `event.get("fieldName")` is not — as well as if a vendored copy drifts by one byte, if the effective denylist stops matching the canonical one, or if any file under that tree defines a local key list again, under any variable name and in any expression position including a function's default argument.
- **Every queue and every bucket now carries a resource policy that refuses non-TLS calls, and a gate enumerates the class instead of trusting the convention** ([#964](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/964)). Nine of the ten `AWS::SQS::QueuePolicy` resources in `template.yaml` — ten of the eleven across the whole repository, the eleventh being in `patterns/unified/template.yaml` — carried an explicit `Deny` conditioned on `aws:SecureTransport: false`; the data-mart rollup dead-letter queue's policy did not, so of the queues that had a policy at all it was the one where a caller pointed at an HTTP endpoint would have been served rather than refused. SQS endpoints are HTTPS and every AWS SDK uses TLS unless deliberately reconfigured, so this is defence in depth rather than an exposed path, but the deny is what makes the guarantee explicit and auditable, and the queue's messages can carry document identifiers and processing metadata. The omission survived because nothing enumerated the class: `lib/idp_sdk/idp_sdk/_core/s3_security.py` covers buckets the CLIs create through the API, but no gate walked the templates, so the control was applied nine times by hand and missed once with nothing able to tell the difference. Writing that gate exposed the same omission one step earlier — eight `AWS::SQS::Queue` resources had no `AWS::SQS::QueuePolicy` at all (`DiscoveryQueue`, `DiscoveryDLQ`, `JobTrackerDLQ`, `TestResultCacheUpdateQueue`, `TestFileCopierFunctionDLQ` and `TestSetFileCopierFunctionDLQ` in `template.yaml`, plus `BootstrapQueue` and `BootstrapDLQ` in `feature-platform/idp-data-generator/template.yaml`), and since ten of the eleven pre-existing queue policies grant nothing and exist only to carry this deny, there was no principled line between those queues and these eight. All eight policies are added here rather than registered as exemptions, so coverage is now 19 queue policies for 19 queues and 15 bucket policies for 15 buckets, and the gate asserts that pairing instead of checking only the policies that happen to exist; a queue with no resource policy cannot be reached cross-account at all, which bounds what the eight were exposed to, and each new policy contains only the deny, so it grants nothing and cannot change existing access. `DiscoveryQueuePolicy` and `DiscoveryDLQPolicy` join `DiscoveryBucketPolicy` in the `--headless` transform's strip set, since that variant removes the multi-document discovery queues along with the rest of the UI surface and a policy left pointing at a removed queue would be rejected at deploy time. `scripts/tests/test_secure_transport_deny.py` discovers every queue and bucket policy by parsing the templates `scripts/discover_templates.sh cfn` finds by content — 30 templates today, and any added later, in these templates or a new one — and asserts the deny's semantics rather than its wording: `Effect: Deny`, a principal covering everyone (`"*"` or `{AWS: "*"}`, but not `{Service: "*"}`, which reaches AWS service principals only), an action covering the service namespace, a `Condition` whose *only* test is `Bool`/`BoolIfExists` on `aws:SecureTransport` against false (a second key would AND with it and narrow the deny to that key's scope), and a `Resource` that resolves through `Ref`, `Fn::GetAtt` or `Fn::Sub` to the logical id of the policy's own target — so a deny scoped to a prefix-nested sibling, `DocumentQueueDLQ` on a policy targeting `DocumentQueue`, is a finding rather than a pass. Where an `Fn::If` guards the statement the deny must hold on every branch, because carrying it on one leg only enforces nothing in the other configuration, and where an intrinsic sits somewhere the gate cannot evaluate it fails rather than guessing. A further rule requires each policy's resource-level `Condition` to match its target's, so a policy cannot outlive the queue it points at — cfn-lint does not fail on that mismatch, which would otherwise surface at deploy time. No behaviour change for a deployment whose clients already use TLS, which is every default configuration.
- **The PII Anonymization feature API no longer builds a DynamoDB client while its module is being imported, which had made its test suite depend on test order and on the developer's AWS profile** ([#974](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/974)). `feature-api/handler.py` bound `boto3.resource("dynamodb")` at module scope, and pytest sets fixtures up before it enters a `@mock_aws`-decorated test, so the fixture that imports the handler always created that resource outside the mock. With an assume-role profile in the environment, botocore deferred the `sts:AssumeRole` to the first signed request — which happened inside the mock — so moto served it, registered the minted key in its IAM backend, and thereafter resolved the caller's account from that key rather than its own default; the query was routed to a lazily created, empty backend while the tables lived under moto's default account, and the first DynamoDB test in the file failed with `ResourceNotFoundException` surfacing as a 500. No request ever reached real AWS. The resource is now created on first use and memoized, so importing the module resolves no credentials and builds no client; Lambda behaviour is unchanged, since the container is reused and the resource is still built at most once per container, on the first invocation instead of at init. The suite starts moto in the fixture that performs the import and replaces the ambient credentials with static fakes, so it cannot resolve a real provider chain whatever the shell holds, and all fourteen tests now pass in isolation as well as together. The same module-scope pattern remains in `pii-anonymizer/hook/handler.py` and `ui-deployer/handler.py`, where it is latent because those suites do not use moto and their tests patch methods on the client objects themselves.

- **The test-diagnosis guidance no longer assumes a broken `idp_common` install announces itself as an `ImportError`** ([#974](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/974)). Two tests in `feature-platform/main-stack-extensions/tests/test_apply_feature_config_preset.py` have been reported as repo defects more than once when they are an environment artifact: the Lambda catches a missing `idp_common` on purpose and degrades — it logs at ERROR and applies the preset without recording a revision — so the tests that exercise the non-degraded path fail on a bare assertion that reads like a regression in the uninstall path. The file gives `2 failed, 18 passed` whenever `idp_common` is unimportable and `20 passed` whenever it is, on the same commit and the same interpreter, and identically under Python 3.12 and 3.13 — so it is not a version incompatibility either, despite two interpreters on one machine appearing to disagree when their editable installs point at different places. [docs/testing.md](docs/testing.md) and `.claude/skills/full-test-battery.md` now say this, name the two tests and their two figures, tell you to check `idp_common.__file__` resolves inside your own checkout before reading anything else, and record that both CIs run `python:3.13-bookworm` so a local 3.12 run is not what CI measures.
