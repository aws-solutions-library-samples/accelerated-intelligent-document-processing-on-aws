...
- **The project-governance files an outside contributor or a security reporter looks for now exist: [SECURITY.md](SECURITY.md), [GOVERNANCE.md](GOVERNANCE.md), [MAINTAINERS.md](MAINTAINERS.md), [ROADMAP.md](ROADMAP.md) and [.github/CODEOWNERS](.github/CODEOWNERS).** The repository had none of them, so a reporter with a live vulnerability had no repo-local statement of where to send it, and review requests were not routed to the contributors who know a subsystem best. `SECURITY.md` states the channel unambiguously — exploitable findings privately to AWS Security via [HackerOne](https://hackerone.com/aws_vdp), hardening suggestions to GitHub issues — plus which versions are supported (the latest release only; no backports) and a pointer to `security/` for the threat model and curated per-release test results. `CODEOWNERS` routes review requests for the agent framework, business rule validation, the Python SDK and the workshop content to their reviewers, and everything else to the maintainer. `GOVERNANCE.md` describes how a change gets accepted, the `develop`/`main` branch and release model, and the pre-release validation battery. `ROADMAP.md` carries themes and explicit non-goals, no dates. A new offline gate, `scripts/tests/test_codeowners.py`, keeps the set from rotting: every CODEOWNERS pattern must still match a tracked path, the handles in CODEOWNERS and `MAINTAINERS.md` must agree, and every relative link and heading anchor must resolve — none of which the docs-site build covers, since it reads only `docs/`. ([#936](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/936))
