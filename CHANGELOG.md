...
- **The Web UI API dispatcher's log group is encrypted with the customer-managed key, and takes its retention from `LogRetentionDays`** ([#972](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/972)). `HttpApiDispatcherLogGroup` in `nested/api-resolvers/template.yaml` set no `KmsKeyId`, so the highest-traffic log group in the deployment — the dispatcher fronting every routable operation of the Web UI's REST API — was encrypted with the AWS-owned CloudWatch Logs key while 106 of the 109 log groups across the three main-deployment templates used `CustomerManagedEncryptionKey`. The remedy was one property: the key's `Allow CloudWatch Logs to use the key` statement is unconditional, and `CustomerManagedEncryptionKeyArn` was already declared in that template and already passed by the parent, so no key-policy, parameter or wiring change was needed. The same resource hardcoded `RetentionInDays: 30` — the only one of the 109 that did not use the parameter — which made `LogRetentionDays` a partial control; it now uses `!Ref LogRetentionDays`. That parameter defaults to `30`, so a default deployment sees no change in effective retention and only a stack that has overridden it is affected. The two remaining unencrypted groups (`StacknameCheckFunctionLogGroup`, `ReadPreviousIDPPatternFunctionLogGroup`) are unchanged: both belong to custom-resource-only Lambdas that run solely during a stack operation and both already carry the matching cfn_nag `W84` suppression and checkov `CKV_AWS_158` skip.
