...
- **`TestRun` now reports `avgWeightedOverallScore`, the run-level "Avg Weighted Score" figure.** Callers that only wanted the headline number previously had to fetch the whole per-document `weightedOverallScores` map and average it themselves, which meant every consumer owned a copy of the definition (the Test Studio UI has seven). The field is an *unweighted* mean **across** documents — each document's own score already honours the config's `x-aws-idp-evaluation-weight` field weights, so every document counts equally regardless of how many fields it has. That is deliberate and is what the UI has always displayed, but it is a choice: it is not a field-count-weighted roll-up, and it legitimately differs from `overallAccuracy`, which is an unweighted micro-average over every field comparison. Documents with no score are skipped rather than counted as zero (one unscored document would otherwise drag the run-level number down), as are non-finite values, so the backend figure equals the one the UI computes from the same map. Purely additive — no migration: runs aggregated before this release return a value too, because the resolver recomputes it from the cached per-document map when the field is absent.
