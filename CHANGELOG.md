...
- **Seven Lambdas had no log group of their own, so their logs were kept forever.** A function with no `LoggingConfig` logs to the group Lambda auto-creates, and Lambda's auto-create sets **no retention policy** — so nothing ever expired and the storage was billed indefinitely. Affected the six UI-facing feature-platform resolvers (`listInstalledFeatures`, `listCatalogFeatures`, `getFeatureLaunchUrl`, `checkFeatureEntitlement`, `subscribeFeature`, `unsubscribeFeature`), which are hit on every page load, and `BatchPreProcessorFunction`, a data-plane Lambda whose volume scales with documents through the Jobs API batch path. Each now has a log group at `/${AWS::StackName}/lambda/<FunctionLogicalId>` honouring `LogRetentionDays` (default 30) and the stack's customer-managed KMS key; `LogRetentionDays` is now forwarded to the feature-platform nested stack, which never received it. **Lambdas that run only during a CloudFormation stack operation are deliberately left as they are** — custom-resource handlers and the feature-platform install hooks (`RegisterFeature`, `RegisterFeatureHooks`, `ApplyFeatureConfigPreset`) are very low volume and log nothing but stack operations, so indefinite retention on an auto-created group is an accepted cost. ⚠️ **The old auto-created `/aws/lambda/<fn>` groups for the seven are not deleted by this change** and remain with no expiry until removed by hand. A new gate, `scripts/tests/test_lambda_log_groups.py`, holds all 13 templates to three rules — every Lambda has a `LoggingConfig` unless exempt, every log group sets `RetentionInDays`, and no `LogGroupName` references a function resource (the [#818](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/818) defect) — and *verifies* each exemption is genuinely custom-resource-only rather than trusting the list, so a data-plane Lambda cannot be added to it to silence the gate. ([#826](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/826))
