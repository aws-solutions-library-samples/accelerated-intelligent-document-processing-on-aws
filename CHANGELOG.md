...
- **Agent Chat's group restriction is now enforced in the Lambda that runs the agents, and the streaming endpoint takes the caller's identity from the transport rather than the request body** ([#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920)). Agent Chat is reachable two ways — `POST /op/sendAgentChatMessage`, whose resolver checks `cognito:groups`, and a Lambda Function URL that streams token deltas by driving `agent_chat_processor` directly — so the check in front of one path did not apply to the other. The processor now applies the same Admin/Author/Viewer check itself, before any agent call and outside the handler's convert-error-to-stream-frame path, using the group claim (and only that claim) that the resolver forwards; a unit test asserts the two group lists cannot drift apart. Both streaming routes now resolve the caller through one helper that prefers the transport-verified principal and refuses a body-supplied identity that contradicts it with a 403, and both declare Pydantic request models so a malformed body is rejected with a 422 before any processing. `make api-test-static` gained Function-URL checks (S6–S9) that discover every `AWS::Lambda::Url` and its routes from the template and fail when a route is undeclared, prefers a client-supplied identity, omits the conflict refusal, or reaches a handler with no group check. A Function URL forwards no Cognito group claim, so on that transport the processor's gate stands down exactly as it does for backend IAM invocations; that residual difference is recorded as GAP-07 in `scripts/api_rbac_expectations.yaml` and AUTH.T14 in [the RBAC threat model](security/threat-modeling/feature-threats/rbac-authentication.md).
