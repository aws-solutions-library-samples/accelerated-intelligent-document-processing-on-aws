...
- **The `--headless` and `--govcloud` template transforms can now be deploy-tested for real, with a document put through them.** ([#527](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/527)) Neither transform has ever been deployed by an automated test. Every deployment-variant probe deploys the **standard** template with different parameters — the `jobsapi` probe was once *named* "headless" but exercises the additive `EnableJobsApi` parameter, not the transform, which is exactly the confusion that let the gap persist. So the transforms were covered only by offline unit tests plus a `ValidateTemplate` call, and **none of those tiers can prove a transformed template deploys.** Three defects landed in precisely that gap: [#676](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/676), [#677](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/677), and the `SuppressAdminInvite` dangling parameter — templates valid, transforms successful, failure only once CloudFormation began creating resources (in one case after ~15 resources).

  `make transform-deploy-test-headless` / `-govcloud` / `-all` (`scripts/sdlc/transform_deploy_test.py`) run the **documented user path** — `idp-cli deploy --headless|--govcloud --from-code . --wait` — so publish → transform → deploy is all under test, and the CLI handles the per-variant parameter differences itself (notably that the headless template has no `AdminEmail` parameter, and passing one is a CloudFormation `ValidationError`). Each variant then asserts its transform's structural promises — `--headless`: UI/Cognito/CloudFront/API-resolver stack **absent**, no `ApplicationWebURL` export, processing core intact; `--govcloud`: no `AWS::CloudFront::*` or `AWS::Lambda::Url`, the LWA chat-stream family gone (#677), UI **retained** on API Gateway — and processes `lending_package.pdf` end to end with the same OCR/extraction/classification assertions the primary suite makes, because a stack that deploys but cannot process a document is still broken. Reuses the CI harness's own deploy/IAM/cleanup/inference helpers so the manual runner cannot drift from CI, and always tears down (`KEEP=1` to inspect).

  ⚠️ **A green `govcloud` run in a commercial account does not mean GovCloud works.** It proves the CloudFront-free template deploys and processes documents; partition-correct ARNs, GovCloud model availability and the BDA project rejection are invisible outside `us-gov-*` — **two of the three defects above would not have been caught by a commercial run.** `REGION=us-gov-west-1` against a GovCloud account exercises those, and the runner is built to run there unchanged. The commercial path also overrides `ConfigurationPreset` back to `lending-package-sample`, since the GovCloud preset pins model IDs that don't exist in commercial and would fail the document test for the wrong reason.

  **Deliberately not wired into CI** (~1h+ per variant, and the transforms rarely change) — but built so it can be: the validators and result dicts already match the probe framework's shape. 22 unit tests with no AWS or subprocess pin the parts that decide whether a green run means anything: each structural assertion fails when the thing the transform should remove survives, teardown happens on success/validation-failure/exception alike, `--headless` is deployed without `--admin-email`, and a rolled-back stack is reported as a **deploy** failure — that last one caught a real bug in this runner, since `"COMPLETE" not in "ROLLBACK_COMPLETE"` is False and the stack was being sent on to validation. See [.claude/skills/transform-deploy-test.md](.claude/skills/transform-deploy-test.md).

- **Bulk document download from the document list — the per-document ZIP export, applied to a selection.** Getting the extractions for a set of documents meant opening each one and downloading it individually ([#679](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/679)). The list's **Download** menu now offers the same three scopes (**All data** / **Predictions** / **Baselines**) for every ticked row, producing one archive with a folder per document plus a root `manifest.json` indexing them. The exporter is the one the Document Details page already used, so layout, presigned-URL fetching, cancellation and per-file error recording are unchanged. Notable details: the list's existing Excel export and the new ZIP exports differ in *scope* — Excel covers every filtered row, the ZIPs cover only the selection — so they are grouped under labeled headings in one menu rather than sitting side by side as two identical-looking download buttons. Document keys that collide once sanitized for the filesystem (`a/b.pdf` and `a_b.pdf` both → `a_b.pdf`) get a numeric suffix instead of silently overwriting each other in the shared archive. Fetches now run 5-at-a-time rather than strictly serially, which is what makes a multi-document export finish in a reasonable time. Because the archive is built in browser memory, selections above 25 documents raise a size/time warning in the confirmation dialog, which for bulk exports also states the document count and defaults page images off; the progress modal reports documents as well as files, and errors are attributed to the document they came from. Selection is hydrated through `getDocument` before export because list rows carry no `Sections`/`Pages`; that fan-out is chunked 5-at-a-time rather than fired all at once, since `getDocumentDetailsFromIds` drops rejected fetches silently and 100 simultaneous calls would make throttling invisible. A document whose detail fetch fails still appears in the archive with its attributes rather than vanishing, but the failure is now recorded as an error against that document in both manifests and in the completion dialog — an archive missing a document's predictions must not read as a clean export. Cancel is honoured between hydration rounds (not just between file fetches) and relabels to "Cancelling…", and the Excel export stays available while a ZIP export runs — only the ZIP scopes are held. 27 new tests, including the presign routing, the key-collision case, and the preflight-error plumbing. [docs/web-ui.md](docs/web-ui.md#bulk-download-from-the-document-list)

- 🔒 **CI now fails on a known-vulnerable dependency — it previously could not.** The security gate on this repo is `srt_security_review`, which runs checkov, semgrep, bandit and **syft**. Syft builds an SBOM: an *inventory*, with no vulnerability matching. So a HIGH CVE in a pinned package could merge with every security job green, and did — the gap was found by comparing our SRT output against an independent ASH scan, whose grype stage flagged dependency CVEs SRT structurally cannot see. New `dep_audit` job (`make dep-audit`, `scripts/security/dep_audit.py`) closes it. It reuses the manifests `scripts/generate-dep-manifest.sh` already produces — every Python and Node dependency across `uv.lock`, the `package-lock.json` files and every `requirements.txt`, 1,835 pinned packages — and matches them against [OSV](https://osv.dev), the advisory source behind grype and pip-audit. Fails on HIGH/CRITICAL unless triaged in `scripts/security/dep_audit_allowlist.json` **with a justification**, mirroring how `scripts/srt/issues.json` triages SRT. Two deliberate properties: a network failure exits 2 rather than 0, because an audit that could not run must not read as an audit that passed; and the report surfaces GitHub's `last_known_affected_version_range` for advisories that carry no `fixed` event, which is the difference between a real finding and an artifact of an out-of-band fix (see the `xlsx` allowlist entries — SheetJS patches ship from `cdn.sheetjs.com`, which OSV cannot express, so every version matches an open-ended range). Runs on every push and MR in parallel with the other fast checks, needs no AWS, and adds no wall-clock to the gate.
...
- 🔒 **Scanner suppressions are now expressed as metadata in the code, so a re-scan starts where the last triage finished.** Roughly 118 of the 306 HIGH findings in the out-of-band ASH scan were false positives that had to be re-triaged by hand each run, because our justifications lived only in `scripts/srt/issues.json` — a file ASH never reads. ASH runs the *real* scanner binaries, so it inherits any pragma the scanner honours natively; each mechanism below was verified locally against the actual binary rather than assumed. **bandit:** every MEDIUM finding in the tracked tree (30, the class ASH escalates to HIGH) now carries `# nosec` with a per-site justification, taking bandit to **zero MEDIUM/HIGH repo-wide**. This also fixed a genuine bug: two analytics prompts had `# nosec B608` on the *opening* line of a triple-quoted f-string, which made it part of the string rather than a comment — so it suppressed nothing *and* shipped the literal text `# nosec B608 - AI agent prompt template...` to the model on every call. The pragma has to sit on a line inside the string node; the closing `"""` works. **detect-secrets:** `# pragma: allowlist secret` added to the dummy-credential and IAM-action-name lines it misreads, verified to coexist with `# nosec` on the same line. Note it dedupes by secret *hash*, so suppressing one occurrence surfaces the next — all 9 occurrences of the two template patterns are annotated and the templates now scan clean. **checkov / cfn-nag / semgrep** already had 521 / 157 / 43 inline suppressions in use and needed no change. New `.ash/.ash.yaml` (validated against ASH v3.5.9's published `AshConfig.json`) covers only what cannot be inline: generated data files with no comment syntax (`images/*.drawio`, benchmark result JSON, notebook output cells) and dependency advisories, which have no inline mechanism at all. Deliberately **not** suppressed: the ~149 checkov Lambda-hardening findings ASH rates HIGH and SRT rates LOW — those are consciously accepted gaps, not false positives, and hiding them would erase a backlog we may choose to work off. A guard test rejects the dangerous shape this file invites — a `path` entry with no `rule_id` on a source file or template, which would suppress *every* ASH finding on that path.

- 🔒 **Pillow 12.1.1/12.2.0 → 12.3.0, closing 18 advisories (11 HIGH) on the untrusted-document path.** This is a document processor: attacker-supplied images and PDFs reach Pillow directly through `ocr/service.py`, `ocr/document_converter.py`, `image/` and `discovery/`, so heap out-of-bounds writes in `Image.paste`/`crop`, `ImageCmsTransform.apply` and `ImageFilter.RankFilter`, plus decompression-bomb DoS in the PDF/FITS/font parsers, are reachable rather than theoretical. Two things had hidden this. **(1) The scanner was reading a dead file.** `lib/idp_common_pkg/setup.py` declared `Pillow==12.1.1`, but `pyproject.toml` has a PEP 621 `[project]` table, so setuptools takes all metadata from there and never reads `setup.py`'s `extras_require` — the built `requires.txt` resolved 12.2.0. That file had drifted badly (missing 4 extras, a stale `analytics` extra, `boto3==1.42.0` vs `>=1.42.86`, `pytest>=7.4.0` vs `>=9.0.3`) while remaining what static SCA tools parse, so it is **deleted** rather than re-synced; `make version` and a stale docstring reference are updated. **(2) An arm64 build constraint capped the PII Anonymizer hook below the fix.** SAM's `PythonPipBuilder` asks pip for exactly one platform tag, `manylinux2014_aarch64`, and Pillow publishes none for 12.3.0 — hence the previous `pillow>=12.1.1,<12.3` pin, which left the 9 HIGH 12.3.0-only advisories unfixed in the one component that renders untrusted customer PDFs. The hook now builds via `BuildMethod: makefile` (`hook/Makefile`), passing **both** `--platform manylinux2014_aarch64` **and** `--platform manylinux_2_28_aarch64`: the dependency set straddles the manylinux generations (Pillow 12.3.0 is 2.27/2.28-only, `pypdfium2` is still 2014-only), so either tag alone fails on one of them, and pip takes a wheel matching any listed tag. Same pattern as `feature-platform/confbench-testset/shared/Makefile`. Verified by a real cross-arch `sam build`: Pillow 12.3.0, aarch64 native `.so`s, all source and deps present.

- 🔒 **Supply-chain hardening: `src/ui/.npmrc` sets `min-release-age=7`.** A newly published — possibly compromised-maintainer — package version could previously resolve into a build the moment it appeared. The single finding both SRT and ASH flagged at HIGH. Honoured by npm >= 11.10 and ignored with a warning by older clients, so it is safe while `engines.npm` still allows >= 10; `npm ci` installs from the committed lockfile and is unaffected either way, which is correct — this constrains the `npm install` that *adds or upgrades* a dependency, exactly where the risk is.

- 🔒 **Least-privilege `GITHUB_TOKEN` default on the two workflows that lacked one** (`developer-tests.yml`, `generate-dep-manifest.yml`; checkov `CKV2_GHA_1`). Both jobs already declared scoped job-level permissions, but with no top-level `permissions:` block any *future* job added to either file would silently inherit the repository default, which can be write-all. Now `contents: read` is the floor.

- 🔒 **`VpcEndpointSecurityGroup` declares explicit egress** (`scripts/vpc-endpoints.yaml`, cfn-nag `F1000`). Omitting `SecurityGroupEgress` makes CloudFormation add allow-all-outbound. Interface-endpoint ENIs only answer inbound 443 and security groups are stateful, so replies need no egress rule of their own — outbound is now pinned to HTTPS inside the VPC. (The pre-existing `SecurityGroupEgress` resource nearby is on the *Lambda* SG, not this one, which is why the finding was accurate.)
...
- **An agent that declined the table tool could throw away the whole table, and the completeness report called it clean.** Observed live at Sonnet 5: the OCR pre-flight found 3 table regions with ~103 rows and rated the deterministic parser `STRONGLY_RECOMMENDED`; the agent declined it — stated reason *"the Amount column was OCR-corrupted with jumbled text instead of numbers, so parse_table/map_table_to_schema couldn't cleanly map usable numeric values"* — and then returned `Transactions: null`. All 100 rows gone, section `COMPLETED`, scalar accuracy **1.000**. It treated *"I cannot map this cleanly"* as *"therefore no rows"*, and nothing disagreed: the retry loop that exists for exactly this never fired, because an empty list breaks no schema constraint unless the config sets `minItems` — and this schema did not. Three fixes. **(1) The prompt states the rule outright.** Declining the tool obliges the agent to extract the table *directly*; a list whose rows are visible must never come back `null` or `[]`; and if one column is unreadable, every row is still emitted with that single **cell** null — not the row dropped, not the list dropped. In both the base `SYSTEM_PROMPT` (so it applies with table parsing off) and the table addendum's fallback section (which listed *when* to fall back but never that returning nothing was not an option). **(2) The in-loop validator now uses the evidence it already had.** The OCR pre-flight counts pipe-table rows before extraction, so when it finds a substantial table (its own existing `>30`-row threshold, the same signal that drove the tool guidance) and **every** declared list comes back with no rows, the result is rejected and the agent gets a correction round naming the field and the row count. Deliberately narrow: all-empty only (a populated sibling means the tables plausibly belong to *it*), single-agent only (per-shard the inference is unsound — a shard legitimately holds none of the rows), and it does **not** enter the validation report, so escalation cost is unchanged. It is, however, deliberately **not** gated on `extraction.agentic.validation.enabled` — that flag defaults to false and the config that produced this bug has it false, so gating the check on it left the safety net dead on exactly the configurations that needed it. (Caught by live verification after the first attempt did gate it.) A guard against silent data loss cannot itself be off by default; full JSON-Schema validation stays opt-in as before. **(3) The Processing Report stops contradicting itself.** It printed `✓ Completeness Validation: All schema constraints satisfied` immediately above the warning that the list was empty — technically true, and read as a clean bill of health. It now shows `⚠` and names the list, the OCR row count, whether the table tool ran, and that `minItems` would make it a hard constraint; `metadata.completeness_check` gains `unexplained_empty_lists` and `complete` while `schema_constraints_met` keeps its exact meaning. 26 tests, including the live case, the ungating, and the five conditions under which the check must stay quiet.
...
- **Two ways to lose a workflow-concurrency slot, and no history to attribute the next one to.** Following up the leaked counter above: the investigation *exonerated* the decrement path — 91 decrements for 91 terminal executions across the incident window, 177 for 177 across a wider one, and **zero** decrement failures in the entire retained log history — so rather than manufacture a root cause, these are the two paths that provably could lose a slot, both now closed. **(1) The queue processor decremented a slot the running execution still held.** `document_service.update_document` sat inside the same `try` as `StartExecution`, so a failure there — a DynamoDB throttle is enough — did two wrong things at once: it released a slot belonging to a workflow that was *already running* (leaving the counter one **below** the true in-flight count, which over-admits work) and returned failure, so SQS redelivered the message and a **second** workflow started for the same document. `StartExecution` is now an explicit transfer of ownership: past it, this function neither decrements nor reports failure. **(2) The workflow tracker swallowed a failed decrement and returned HTTP 200.** `decrement_counter` caught every exception, logged it, and returned `None`; the handler put that `None` in the response body and reported success — so one throttled write cost a slot **forever**, with no retry, no alarm, and nothing in the response saying so. It now retries with backoff (`DECREMENT_MAX_ATTEMPTS`, default 4) and **raises** when it still cannot apply the write, so the event is redelivered instead of the slot being lost quietly; telemetry is emitted strictly after the write, where it cannot re-enter the retry loop and double-subtract. It also publishes `ConcurrencyCounterActive` on every completion, because the reason the original leak could not be pinned to a moment in time is that **nothing ever recorded the counter** — only its end state was visible; a new dashboard widget plots it against `ConcurrencyCounterDrift` with the `MaxConcurrentWorkflows` ceiling annotated. And because the tracker can now fail an event rather than swallow it, a new **`WorkflowTrackerDLQAlarm`** watches the dead-letter queue those events land in — the existing drift alarm and the reconciler are both *downstream* of the cause, so this one fires on the first undelivered completion instead of waiting for drift to accumulate. Ten tests across both functions, including the exact post-start-failure case and the "raises rather than losing the slot" case. [docs/monitoring.md](docs/monitoring.md) gains a "Workflow Concurrency Counter" section on why this failure is quiet — every other signal looks *idle* rather than broken — and how to read the widget.
...
- **Hardcoded `arn:aws:` in runtime Python broke Bedrock Data Automation and Knowledge Base queries in GovCloud.** ([#527](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/527)) A GovCloud user reported every BDA invoke failing with `The provided ARN is invalid`: `BdaService` built its data-automation profile ARN as `f"arn:aws:bedrock:{region}:..."`. AWS account IDs do not exist across partitions, so that ARN is meaningless in `aws-us-gov`. It was one instance of a class, because the gate that exists to prevent exactly this — `make check-arn-partitions` — only ever scanned CloudFormation templates and Step Functions ASL, **never Python**.

  | Site | Consequence outside the commercial partition |
  |---|---|
  | `bda_service.py` | every BDA invoke fails — the reported defect |
  | `blueprint_optimizer.py` | same, on the blueprint-optimization path |
  | `query_knowledgebase_resolver` | every Knowledge Base query fails on an invalid model ARN |
  | `stack.py` teardown | permissions boundary silently not deleted → leaked IAM policy |
  | `create_finetuning_job.py`, `pack.py` | IAM / bucket policies match nothing while *appearing* to apply |
  | `finetuning_job_creator` | latent: a guard treated only `arn:aws:bedrock:` as an ARN (measured to produce the same output either way, so no observable defect — but the guard now says what it means) |

  All fixed by **deriving** the partition (`sts:GetCallerIdentity()["Arn"].split(":")[1]`) or, for the Lambda, injecting it from the template as `AWS_PARTITION: !Ref AWS::Partition`. Two of these call sites were re-implementing an ARN the repo already had a correct helper for — `bda_ocr.build_profile_arn` — which also fixes a second bug in the same lines: the naive `region.split("-")[0]` produced the geo prefix `ap` where Asia Pacific needs `apac`.

  New gate `scripts/check_python_arn_partitions.py`, wired into `make check-arn-partitions` (493 files, 0 findings). Documentation is exempted structurally (docstrings only) rather than suppressed, and a legitimate match takes an inline `# arn-partition-ok: <reason>` honoured at or after the ARN's line within the same statement — directional because `ruff format` reflows long expressions and moves a trailing comment to the closing line, which silently un-suppressed 7 sites during development. ⚠️ The `bda` OCR backend and BDA mode remain unavailable in GovCloud for the separate reason in [#676](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/676); this fixes the ARN, not that.

- **The document download's Baselines scope produced nothing but 403s, and "All data" silently dropped its baseline half.** The exporter signed every asset client-side with the browser's Cognito credentials, but the identity pool's authenticated role grants `s3:GetObject` on the Output, Input and Configuration buckets only — the **EvaluationBaselineBucket** is deliberately absent. So a baseline URI got a validly-signed URL that S3 rejected at fetch time: "Download Baselines (ZIP)" yielded an archive of `document-attributes.json` + `manifest.json` with every baseline recorded as `HTTP 403 Forbidden` in `errors[]`, and the `all` scope quietly lost its `baseline/` tree the same way. Verified both directions against the live `us-west-2` stack: `simulate-principal-policy` returns `implicitDeny` for the authenticated role on the baseline bucket (and `allowed` on the output bucket), while the backend `getFileContents` resolver — which holds `S3ReadPolicy` on the baseline bucket behind its own bucket allow-list — returns a presigned URL that fetches `200`. Presigning is now bucket-aware: Output/Input stay client-side (no extra round trip), anything else routes through the `getFilePresignedUrl` resolver, which is the route the Sections panel's per-section **Download Baseline** button already used. Chosen over adding the baseline bucket to the Cognito role, which would hand every authenticated user blanket read access to ground-truth data and would only take effect after a stack update; this fix works on already-deployed stacks. Affects the per-document download on the Document Details page as shipped in 0.6.5, not just the new bulk export.
- **Two more consumers still read `type` straight off a `$ref` property, so a group authored in the UI schema editor read as untyped ([#678](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/678)).** [#638](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/638) fixed the confidence prompt's description formatter; these are the two places that same blind spot survived, found while assessing a follow-up report ([#664](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/664)). **(1)** Classification's `{CLASS_AND_ATTRIBUTE_NAMES_AND_DESCRIPTIONS}` placeholder silently dropped every child of a `$ref` group: with no `type` of its own the group fell to the scalar branch and was emitted as one bare leaf, so `Signatures` appeared where `Signatures.Signature-of-taxpayer1` and its sibling should have — while a byte-equivalent **inline** group walked correctly, making the listing depend on how the class happened to be authored rather than on what it contains. **(2) An array declared as a bare `$ref` into `$defs` was never row-scored at all, and the ladder burned a full escalation round rediscovering that.** The confidence enhancer read `type` off the raw property, so `{"$ref": "#/$defs/TxnList"}` defaulted to `TYPE_STRING` → the attribute was treated as a **scalar**, the model's per-row confidence list was collapsed to a single default `{"confidence": 0.5}` leaf, and reconciliation padded it to N null placeholders that no model could ever fill. The escalation ladder's schema-mismatch guard caught the dead-end and skipped — correctly, given the enhancer — but reported the property as `declared as 'scalar'`, which is not a type the schema contains anywhere, so the operator was sent hunting for a `type: string` that does not exist. Both halves are fixed together: the enhancer now dereferences before reading `type` (so the rows are genuinely scored per-row, and per-sub-field thresholds resolve from the referenced item definition instead of silently falling back to the uniform container threshold), and only *because* of that is it correct for the guard to stop skipping. Fixing the guard alone would have removed the brake while leaving the dead-end in place — trading a clear root-cause flag for two wasted model rounds. The threshold read on the property itself is deliberately left un-dereferenced, since honoring a threshold declared on the definition rather than the property is a change to threshold *inheritance* semantics. The dereferencer #638 introduced moves to `config/schema_utils.py` (import-light, so classification can use it without pulling in the assessment service's Bedrock/S3 clients) and is now the single implementation behind all four consumers. **(3) The integrated-confidence and BDA enrichment path had the same blind spot, so the same schema produced HITL alerts in one confidence mode and none in the other.** `enrich_assessment_with_thresholds` also read `type`/`items` off the raw property, so a `$ref`-wrapped array resolved zero per-sub-field thresholds and fell back to the uniform container threshold: a sub-field with `x-aws-idp-confidence-threshold: 0.99` breached at 0.95 raised **2 alerts** in `confidence: separate` and **0** in `integrated`. All three shapes — `$ref` container, `$ref` items, fully inline — now agree.

**(4) The Web UI prompt preview no longer disagrees with the prompt that actually runs.** `PromptPreview.tsx`'s `getAttributeNamesForClass` is a documented port of the Python walk, and fixing only the backend would have left the preview showing a bare `Signatures` for 35 shipped classes while the model received `Signatures.Signature-of-taxpayer1`. The port now dereferences too, with 8 TS tests mirroring the Python expectations case for case.

Dereferencing also makes recursive `$defs` reachable for the first time, so the classification walk guards on the `$ref` targets already entered on the current branch and emits a re-entered property as a leaf rather than recursing forever. It additionally needed a **hard ceiling on the walk**: because that guard is per-*branch*, a NON-cyclic `$defs` DAG is legitimately re-entered on every sibling branch, and a 2 KB schema with a 3-way fan-out 12 levels deep expanded to ~265K names (16 deep: ~14M, 34 s in a Lambda) — `MAX_ATTRIBUTES_PER_CLASS` could not help, since it truncates the *result* after the walk has built the whole list. Deliberately **not** consolidated: `assessment/threshold_resolver.py`'s own `_deref`, whose dangling-ref and definition-wins-over-sibling semantics are load-bearing for threshold inheritance — switching it would change which threshold wins for a `$ref` property carrying a local `x-aws-idp-confidence-threshold`. Also here: `ruff.toml` excluded all of `idp_common/config` as a directory, so the new shared helper silently landed with no lint, isort or format enforcement; that entry is now file-by-file, and anything new added to the package is linted by default. 22 Python tests + 8 UI tests, including the recursive, DAG-expansion, dangling, remote, non-dict and tuple-form-`items` cases.

- **Every GovCloud deployment rolled back on a BDA project that region cannot accept — even Textract-only stacks.** ([#676](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/676)) `patterns/unified/template.yaml` created the stack-scoped BDA-as-OCR project unconditionally, with no `Condition` and no parameter to disable it, so `PATTERNSTACK` failed with `ValidationException: Sync project does not support video/audio/document modality in Standard Output Configuration` and the root stack rolled back with ~30 resources cancelled — regardless of deploy mode (`--govcloud`, `--headless`) and regardless of the configured `ocr.backend`. Availability was never the issue: BDA *is* offered in `us-gov-west-1`; what GovCloud rejects is this one project shape, a **SYNC** project carrying a `document` block in its `standardOutputConfiguration`. Graceful degradation for BDA-less regions was already the documented intent (the env var's own comment says the ARN is empty there and the `bda` backend then errors clearly) but the unconditional resource made that path unreachable. Two fixes. **(1)** The resource, its function and its log group are now gated on `ShouldCreateBDAOCRProject` (`AWS::Partition == aws`), and `BDA_OCR_PROJECT_ARN` falls back to `""` behind the same condition — so no GovCloud or China deploy can reach the failing API call at all. **(2)** The custom-resource handler treats a capability-gap `ValidationException` exactly like "BDA unavailable here" — SUCCESS with an empty ARN — which also covers a commercial region that refuses the project shape. Deliberately narrow: the message must contain both a "not supported" phrase and a marker naming the unsupported capability, so an ordinary validation error (a malformed project name) still fails the stack loudly. ⚠️ The `bda` OCR backend is therefore **not available in GovCloud**; use `ocr.backend: textract`, which is the built-in default (the GovCloud preset sets no `ocr:` key at all, so the default applies). Also new: a cfn-lint `--region us-gov-west-1` gate on the **nested pattern template**, which no GovCloud lint has ever covered — the existing gate lints only the transformed main template. To be clear about what that does and does not buy: it would **not** have caught this bug, because `E3006` flags AWS resource *types* absent from a region and this was a runtime API rejection inside a `Custom::` resource. It closes the adjacent gap — a future GovCloud-illegal resource type added to the pattern stack, which until now nothing checked before deploy.

- **`--govcloud` removed the chat streaming Function URL but left its handler, which cannot exist in GovCloud.** ([#677](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/677)) `ChatStreamProcessorFunction` layers in the AWS Lambda Web Adapter, published only by commercial-partition account `753240598075`. AWS account IDs do not exist across partitions, so `arn:${AWS::Partition}:` substitution cannot make that ARN resolve — the deploy died with a 403 on `lambda:GetLayerVersion` that reads like a missing *resource-based policy* rather than a partition mismatch, which no identity policy on the deploy role can fix. The documented `LambdaWebAdapterLayerArn` override had no target either: there is no GovCloud LWA publication, and the parameter's comment wrongly implied one exists. The transform now removes any function layering in LWA, together with its log group, its Lambda permission, the IAM statements granting invoke on it, and the now-dead override parameter and condition. An inline policy emptied by that pruning is **dropped entirely** rather than left with `Statement: []`, which IAM rejects with `Syntax errors in policy` (400) — a rollback in a different, more confusing place, and the trap the reporter hit. This does **not** disable chat, contrary to the reporter's stated concern: the polling transport the UI falls back to is served by the retained `AgentChatProcessorFunction` / `ChatWithDocumentProcessorFunction` behind the UI REST API, and the streaming function was never in that path — so `docs/govcloud-architecture.md`'s "the UI polls for chat answers instead" still holds. GovCloud validation now also fails the publish on any surviving reference to a resource the transform deleted, instead of deferring it to a deploy-time rollback.

- **Every `--headless` deployment had been failing at template validation since 2026-07-16 — before creating a single resource.** `b301a1fa7` added a `SuppressAdminInvite` condition (it tests `AdminEmail` against a CI sentinel to suppress the Cognito invite email) but did not add it to the headless transform's removal list. Headless removes the `AdminEmail` **parameter**, and a Condition left referencing a deleted parameter is not dead weight that CloudFormation ignores — it rejects the entire template up front:
  ...
  Confirmed against the real CloudFormation `ValidateTemplate` API, and it matches the "fails earlier on a parameter mismatch" aside in [#676](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/676) — which is how it was found; nothing in CI was watching. The condition is now stripped for headless. It was already inert (its only consumer, `AdminUser`'s `MessageAction`, is removed too, and **no** resource is gated on it), so this is a pure unblock with no behavioural change.

  **New CI gate.** `validate_headless_template` in the integration tier now transforms the **packaged** `.aws-sam/idp-main.yaml` and validates it through real CloudFormation `ValidateTemplate` — closing the gap the offline tests structurally cannot, since they transform the *source* `template.yaml`, which differs from the published artifact (SAM expansion, nested-stack URLs). It reuses the template the publish step already built, so it costs one S3 put and one API call rather than a second ~1h SAM build, and a failure is folded into the run verdict instead of aborting before the primary suite reports. This is the check `publish --headless` already performs and no CI job ever invoked. It deliberately does **not** deploy a headless stack: it proves the template is well-formed, not that a headless stack stands up — that remains a tracked follow-up in [CI_TEST_COVERAGE.md](scripts/sdlc/docs/CI_TEST_COVERAGE.md), where this item was already an open TODO.

  **Why CI missed it for six weeks.** The headless template *is* validated during `publish --headless`, but only via a live CloudFormation call that no CI job makes; and the one offline gate that lints it deliberately discards every finding except `E3006` ("resource type absent from region") as out-of-scope, so an `E1020` for a removed-parameter reference was filtered out by design. Two offline, credential-free gates now close that hole: a **structural** check that no surviving `Ref`/`Fn::Sub` anywhere — Conditions, Rules and Outputs included, not just Resources — points at a parameter the transform removed; and a cfn-lint gate asserting **zero Error-level findings** on the transformed real template (warnings stay ungated, since headless legitimately leaves unused conditions and unreachable `Fn::If` branches behind). Both fail against the pre-fix transformer.

- 🔒 **The multi-document discovery pipeline no longer hands a pickle between steps.** The clustering step wrote `cluster_data.pkl` and the analyze step called `pickle.loads()` on it (bandit `B301`, suppressed only with `# noqa: S301`, which bandit does not honour) — an arbitrary-code-execution sink on load. The payload was plain numbers throughout, so pickle bought nothing. Serialization moves into `ClusterResult.to_artifact()` / `from_artifact()` next to the existing `to_serializable()`, so one format is shared instead of duplicated across two Lambdas, and the artifact is now `cluster_data.json`. The round trip has to restore what JSON cannot represent — numpy arrays and integer dict keys (`get_cluster_ids()` compares keys with `cid >= 0`, which raises on a string) — so four tests pin it, including that sampling off a restored object matches sampling off the original. These handlers previously had no test coverage at all. ⚠️ A discovery job already in flight across the upgrade fails and must be re-run, since the two steps disagree on the artifact format.

- 🔒 **`nanoid` → 3.3.18 and `js-yaml` → 4.3.1 in the UI and docs-site build trees** (GHSA-2v37-7h3g-55p8, GHSA-5p4m-2wfm-xmqj, both HIGH). Pinned via `overrides` within the existing major line: an unconstrained `>=` floor resolved nanoid to 6.0.1 and js-yaml to 5.4.0, either of which would likely have broken postcss/astro. Both projects now report zero HIGH npm-audit findings.

- **14 bandit suppressions that bandit ignored now actually apply.** Lines carrying only `# noqa: S310`/`S102` were suppressed for ruff but invisible to bandit, so they resurfaced as HIGH in any bandit-based scan. Each now also carries `# nosec` with a *specific* justification rather than a blanket one, verified per call site: eight are the CloudFormation-supplied custom-resource `ResponseURL` (not a user-controlled URL, and not the "fixed HTTPS endpoint" a generic justification would have claimed), three are deployment-set env-var endpoints, and two are `exec()` of this repo's own shipped source under test.

- **The release benchmark gate reported single-sample noise as a regression, twice, and both times it was wrong.** `aggregate.py --compare` paired rows on `cell|doc|repeat` and judged each pair alone: one failed run was a `NEW FAILURE`, one ≥0.02 mean shift was an accuracy regression. Pairing on the repeat *index* is meaningless to begin with — repeat 2 of the new run is not "the same run" as repeat 2 of the baseline, they are independent samples of the same (cell, doc) — so it discarded exactly what repeats are for. During the v0.6.5 fix verification this produced a reported new FAILURE (`tt-simple-int` on the 800-row doc) **and** a −0.143 accuracy drop, and **neither reproduced**; both cost real investigation time before being traced to non-determinism. Comparison now aggregates over repeats per (cell, doc): **failures are compared as rates** (`1/3 → 1/3` is the same behaviour; `0/3 → 3/3` is a regression; a partial increase is reported but tagged *confirm before believing*), **quality and cost are mean-vs-mean and must clear the run-to-run spread the data itself shows** on either side, anything clearing the threshold but not the spread prints as `INCONCLUSIVE, add repeats` and is excluded from both verdict lists, and failed runs no longer pollute quality means so a failure cannot also manufacture an accuracy regression. With `repeats: 1` the behaviour is identical to before (mean = the single value, spread = 0). And the release-gate grid `corefast` now runs **`repeats: 3`** — accuracy really is stable at 1 sample, which is why the broad grids stay there, but the gate answers a different question ("did the release change anything") for which one draw is not evidence. Bounded cost: `corefast` is deliberately the ≤100-row subset, 10 cells × 3 docs × 3 = 90 runs per side. 17 tests.

- **The benchmark harness could silently run a suite on the wrong configuration, and did.** Config files were named `<cell>__<class>.yaml` with no suite in the name, but suites share cell names (`core_cells` backs `corefast`, `core` and `coresynth`). So building one suite **overwrote another's config files while leaving that suite's index untouched** — the index advertised `extraction_model: sonnet5` while the file it pointed at was pinned to sonnet-4-6. The run executed on sonnet-4-6, recorded "sonnet5" in its metadata, and the resulting before/after comparison silently spanned **two different models**: every number attributed to the wrong configuration, with nothing in the output looking unusual. It was caught only by reading `metadata.extraction_model` out of a run's S3 output and finding it disagreed with the summary. Two fixes: config filenames are now namespaced per suite so two suites cannot collide, and `run_matrix.py` **refuses to launch** unless every config file on disk matches the axes its index claims — comparing against what `make_configs.set_path` would actually write, so a reshaped knob like `ocr.features` cannot false-positive. Six tests, including the exact model-mismatch case.
...
- 🔒 **The UX-test session no longer widens a live stack's Cognito app client.** Setup enabled `ALLOW_ADMIN_USER_PASSWORD_AUTH` on the UI client for the duration of the session on the belief that setting a known password non-interactively required it. It does not: `admin-set-user-password --permanent` is an admin API on the user pool and ignores the client's `ExplicitAuthFlows`, and the browser then signs in over SRP, which the UI client already allows. That flow is needed only by `admin-initiate-auth`, which the script never calls. It mattered more here than in `test_api_rbac.py`, which reverts in a `finally` even under `--no-teardown`: this is two invocations with an open-ended session between them, guarded only by a printed reminder.
