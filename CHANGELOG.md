...
- **Extraction: a section holding several documents of the same class can now extract all of them.** Set `x-aws-idp-multi-instance: true` on a class and its effective schema becomes a list of that class, so `inference_result` holds `{"instances": [ … ]}` — one entry per document found — while you keep authoring the class as one record. A section can also now report how many documents it found, and `extraction.multi_instance_detection.enabled` asks the model that question directly so the loss is visible even when the flag is off. Detection covers **Simple extraction only**; the wrapper itself works in both Simple and Advanced (agentic) mode, so a flagged class extracts every record either way ([#772](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/772)). In the Web UI both modes are a single **Documents per section** control under **Configuration → Document Schema**, including a picker for a class that already lists its records (`x-aws-idp-instance-array`). See [extraction and confidence](docs/extraction-and-confidence.md) ([#715](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/715), [#753](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/753), [#565](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/565)).

- **Extraction: simple mode now coerces and validates its output.** It did a raw `json.loads` and passed whatever came back downstream, so a wrong type or a non-ISO date reached storage unchallenged; Advanced extraction has had this for some time and this brings the same guarantee to the path most deployments run. Deterministic repair runs first without a model call (currency and thousands separators in both conventions, named-month and unambiguous numeric dates), then full JSON-Schema validation with optional escalation. ⚠️ **Coercion is on by default and changes stored values** — `"$1,234.00"` is now stored as `1234.0`, so a re-run on this release can differ from the same document's earlier output. Both halves are individually switchable under **Configuration → Extraction**.

- **Extraction: experimental — send the document schema as a required tool instead of describing it.** `extraction.forced_tool.enabled: true` declares your schema as a Bedrock tool the model must call, so a malformed-JSON reply becomes structurally impossible for the fields the schema declares. ⚠️ **Off by default and unproven** — it constrains the shape of the reply, not the accuracy of the values in it. Routes that cannot carry a tool schema fall back to the prompt automatically. **Configuration → Prompt Preview** gains a **Tool Schema** tab showing the exact tool that is sent, with its tokens included in the previewed total, so the cost of turning this on is visible before you do. See [extraction and confidence](docs/extraction-and-confidence.md).

- **Extraction: advanced mode can stop sending the document schema three times, and both duplicates are now removable.** ~5,700 schema tokens per request on the lending `Payslip` class where ~1,600 would do. `extraction.agentic.restate_schema_in_system_prompt: false` drops the system-prompt copy, which is byte-identical to the tool schema. `extraction.agentic.prose_schema` shrinks the copy in the task prompt — and has **three** settings rather than two, because that one is not pure duplication: the tool schema is built from a Pydantic model generated from your schema, and that conversion drops any description belonging to an *object*, so your class description and any group description reach the model **only** through the prompt. `minimal` keeps exactly those plus the field names (372 tokens instead of 1,485 on `Payslip`); `names` keeps the field names alone (176). Prefer `minimal` unless your classes carry no class or group descriptions. In **Simple** extraction the same duplication exists only with forced tool use on, where the tool schema loses nothing — there `extraction.forced_tool.drop_prose_schema: true` is a clean deletion. ⚠️ **All of it defaults to today's behaviour and these are A/B knobs rather than recommendations:** restating a schema in prose often improves adherence, so measure **completeness** first — a token saving that loses list rows is a loss. ⚠️ **And measured on the benchmark suite, neither knob buys anything either** — no completeness or accuracy cost, and no benefit: every copy sits inside the prompt cache, so the dollar saving is roughly a tenth of the token count, and reclaiming the tokens does **not** make long documents split into fewer parts, because the schema text is not counted when the pipeline decides how to split a document ([#775](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/775)). They ship as instruments for measuring the question on your own documents, not as tuning recommendations. `drop_prose_schema` applies only to requests that really do carry the tool schema; on a route that cannot (a custom Lambda hook, GPT-5.x) the prompt copy is kept rather than leaving the model with no schema at all, and the section records why. **Configuration → Prompt Preview** shows and counts both blocks, so the saving is visible before you switch anything off — labelled as an approximation, because the browser renders it from your class schema while the backend generates it from a Pydantic model and the real block is larger. Measured results in [benchmarking guidance](docs/benchmarking/config-guidance.md) ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).

- **Classification: the model now reports how sure it is of the class, and “not scored” is distinguishable from “certain”.** Every page and section previously carried a hardcoded `confidence: 1.0`, so a clear W-2 and an ambiguous form that could have been three things looked identical. The default `classification.confidence.mode: topk` asks for ranked candidate classes with probabilities ("80 % W-2, 15 % 1099") rather than one self-rated number, which models answer as ~0.95 for everything; the score has its own sortable **Class conf.** column with the model's reasoning and alternatives behind it. ⚠️ **A fabricated `1.0` is no longer written anywhere** — an unscored page or section is `null`, and `1.0` is reserved for paths that genuinely assert a class. How useful the number is depends strongly on the classifier: Nova 2 Lite answered exactly 0.95 on 90 % of pages, while Haiku 4.5 produced a graded distribution. See [classification](docs/classification.md) ([#673](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/673)).
...
- **Models: xAI Grok 4.6 is selectable, and is the first non-Claude/non-Nova model that works for agentic extraction.** Available as `us.xai.grok-4.6` and `global.xai.grok-4.6` (the cheaper of the two). Unlike the GPT-5.x family it is served on the standard Converse API, so it flows through the normal client path. ⚠️ **Three things the AWS model card advertises do not work, verified live:** Flex and Priority service tiers are rejected, so no tier-suffixed IDs are offered. Request access in Amazon Bedrock before selecting it.
...
- **Models: auto-sizing no longer lets a model's output cap starve its own shard budget.** Reserving room for the response by subtracting the output window from the input window went negative on a model whose output cap is large relative to its context (Grok 4.6), collapsing the shard budget to the 2,000-token floor — so the model with the largest context behaved like the smallest.
...
- **Test Studio: a draft-labeling run no longer reports accuracy it does not have.** Such a run has no baseline by construction, so it showed `Page Level Accuracy 0.000` and empty score tables under an alert explaining there were no metrics, warned twice about the same thing, and could sit badged `EVALUATING` indefinitely while re-aggregating on every view. Clearing a set's draft labels left it permanently claiming to have them with an estimated accuracy for labels that no longer existed; a set that arrived with ground truth was labelled `Unlabeled` in the editor while two other statements on the same screen said otherwise; and the labeling dialog described a 100-document set as 50, because it counted the page it had loaded, then labeled all 100.
