...
- **A Simple-mode extraction that returned a fraction of a long list no longer reports success silently, and the too-large-section failure now says what to change.** With over-splitting fixed (#726), a Simple-mode section is one request: an 800-row / 17-page statement returned 43 rows with `COMPLETED` and no processing issue (the population check passes because the list is non-empty), and 25+ pages failed with Bedrock's bare *Input is too long for requested model*. Two warnings close the gap without changing what is extracted: `extraction_rows_below_ocr_estimate` compares the rows extracted with the table rows the section's OCR text contains (fewer than half, with at least 30 OCR rows), counted at any depth so a multi-instance wrapper is not mis-counted; `extraction_section_exceeds_model_input` is raised before the call when the single request is estimated above the model's input window. The failure's error entry now explains that Simple mode sends the whole section as one request and recommends Advanced (agentic) extraction or splitting the document. Simple mode deliberately does not shard — that is Advanced mode's capability.
