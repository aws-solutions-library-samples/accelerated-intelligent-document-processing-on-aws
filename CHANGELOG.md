...
- **The benchmark harness silently dropped the two reference corpora, so a `core_docs` suite measured 7 of its 9 documents and reported like a clean sweep ([#766](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/766)).** `run_matrix.py` launches one local PDF per run; the reference corpora (`realkie`, `ocr_bench`) are test *sets* on the stack with no PDF under `benchmarks/corpus/docs`, and the launch loop `continue`d past them under a comment claiming they were "handled separately" — there is no such path. The two skipped documents were the only ones with real documents and human-verified labels, i.e. the only guard against a change that looks good on generated documents and regresses on real ones. The launcher now names them, excludes them from the plan count instead of inflating it, records `docs_named` / `docs_run` / `docs_unlaunchable` in `runmap.json`, and exits rather than running nothing when a suite names only reference corpora; the class-mismatch note no longer advises a `--class` that cannot work for them. Reporting only — launching reference corpora through the harness (via the Test Studio TestRunner path) remains open on [#766](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/766). Harness only; no product code.
