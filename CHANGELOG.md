...
- **IDP Monitor data mart (Phase 1) — hourly + daily rollup tables on top of `metering`.** The dashboard's "additive-over-wide-range" queries (KPI cards, cost-by-stage, cost trends, volume time series, distributions) previously scanned raw `metering` on every page load — GBs of Parquet for 7d/30d ranges, 15–90s query time, frequent `503`s at the 29-second API Gateway ceiling. Three new Glue tables (`metering_hourly`, `metering_daily`, `control_plane_hourly`) plus a single scheduled `DataMartRollupFunction` (hourly + daily EventBridge crons) fix this at the pipeline layer. Marketplace-repo consumers (idp-monitor) read from the rollup tables via a tier picker (`<2h → raw`, `2-24h → hourly`, `>24h → daily`) and pay a KB scan instead of a GB scan on wide ranges. Rollups are pure append-only — `metering` is now partitioned by write time (= doc completion time) instead of `initial_event_time` (queue time), so metering rows never land in past partitions. Late-arrival / trailing-window / Iceberg complexity that earlier drafts introduced is gone. See [docs/planning/monitor-data-mart.md](docs/planning/monitor-data-mart.md).

- **Control-plane cost tracking, first-class in the dashboard.** New `control_plane_hourly` table + KPI split lets operators see "what did document processing cost me" (data plane) separately from "what is the IDP infrastructure itself costing me while idle" (control plane — dashboard resolvers, AI-summary agents, natural-language agents, test-set polling, test-run aggregation, config resolvers, capacity planners, discovery, fine-tuning, agent chat, the rollup Lambda itself). Populated by the same rollup Lambda, which queries CloudWatch for `AWS/Lambda/Duration` + `Invocations` plus custom `IDPControlPlane/*` metrics (`AthenaBytesScanned`, `BedrockInputTokens`, `BedrockOutputTokens`) emitted by control-plane Lambdas via a new shared helper `idp_common.metrics.emit_control_plane_cost_metric`. Discovery is subtractive using the whitelist model — only the ~7 data-plane Lambdas (OCR / Classification / Extraction / Assessment / Summarization / Evaluation / WorkflowTracker) carry `idp:plane=data`; everything else is implicitly control plane. Stack membership is scoped via the CloudFormation-native `aws:cloudformation:stack-name` tag, so no custom `idp:stack` tag is defined or maintained. New features (autotune, hooks, agents, future work) require zero tagging work — they show up under Control Plane automatically. See §10 in the design doc for the widget-to-category routing and the illustrative drill-down.
