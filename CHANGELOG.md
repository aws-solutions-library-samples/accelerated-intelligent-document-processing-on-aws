...
- **Prompt caching: a per-class warning when a cache point can never engage, and an `extraction.prompt_cache: off` knob.** A `<<CACHEPOINT>>` only creates a cache entry if the prefix before it clears the model's minimum cacheable prefix — 512 tokens on Claude Opus 5 / Fable 5, 1,024 on Sonnet 5 / Sonnet 4.6 / Opus 4.8, 2,048 on Opus 4.7 and **4,096 on Opus 4.6 / 4.5 / Haiku 4.5**; below it Bedrock reports zero cache reads and writes, raises nothing, and bills the prefix at full price on every request. Measured across the shipped presets, 25% of classes never cache at the 1,024 tier and none do on Haiku 4.5, with nothing in the product saying so. `idp-cli config validate` (and the SDK validate operation) now warns per class, naming the class's estimated prefix and the model's minimum (estimate within ~10% of Bedrock's own count on the surveyed classes, and agreeing with the survey on every class that falls under the Sonnet minimum). A cache write also costs 1.25× and pays back only on a second same-prefix request inside the 5-minute TTL, so a low-volume deployment paid about +25% on the prefix for nothing with no way to decline; `extraction.prompt_cache: off` now sends no cache points on either path. Docs carry the per-model table with the non-monotonicity called out. Not in this change: a per-class cache-efficiency view in the Processing Report ([#780](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/780) stays open for it).
