...
- **Sustained Bedrock throttling no longer makes an Advanced-mode retry ladder hammer the service it is backing off from.** Once the ladder had spent its backoff allowance it clamped every remaining sleep to zero and kept going, so on the shipped numbers 43-45 of 50 requests went out back to back at a service that was refusing them — roughly eight times the requests needed, multiplied by every document in flight, and buying almost nothing because a refused call returns in well under a second. The ladder now stops when its allowance is spent (5-7 attempts on those numbers, typically 6) and lets the state machine retry the step, which has minutes to hours of backoff where one invocation has 90 seconds, releasing the Lambda concurrency slot instead of holding it to spin. **Action:** none. A tight *deadline* still only shortens sleeps and keeps trying, because there the attempts themselves are what consume the clock and may still succeed.

- ⚠️ **A retried Advanced-mode extraction sent the prompt again as a *second copy*, and two transient Bedrock errors in one section then failed it outright** ([#1296](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1296)). The agent's retry ladder handed the prompt back to a conversation that already held it, so a throttle or a read timeout was not a delay but a step towards a hard failure: the prompt ends in a cache point, and on a caching-capable Claude model — where the request also carries a system and a toolConfig cache point — the third copy reached five `cache_control` blocks against Bedrock's limit of four and the section was rejected with `ValidationException`, keeping no rows and writing no result. One copy short of that nothing was raised at all and the document text and every attached page image were re-sent and charged for again. A retry now **resumes** the conversation the failed attempt started, which both keeps the request within the limit and carries the completed tool rounds forward instead of repeating them. ⚠️ **Action, for 0.6.10 only, and only in Advanced mode with `extraction.agentic.table_parsing.enabled: true`: re-check the row counts of any large table extracted by a document that logged a transient extraction error.** The second copy of the prompt restated "extract every row" on top of tool results the agent had already produced, and `map_table_to_schema` accumulates rows across calls, so an agent that acted on the restatement could have emitted the same rows twice — and nothing detects that, because the completeness check only fires on a shortfall. We have not observed it, and cannot rule it out; a duplicated row set is the reason to look rather than a reason to expect. ⚠️ The duplication predates 0.6.10 but only became reachable in it, when the shard time budget stopped letting botocore retry a stalled request one layer lower, where nothing was re-appended ([#1014](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1014)).

## [0.6.10]

### Added

- **Claude Opus 5.5 is selectable for every model-taking service.** `us.`, `eu.` and `global.anthropic.claude-opus-5-5` (and the `:1m` extended-context variants) are in every configuration picklist, priced, and covered by the auto-sizing limits — 1M context, 128K max output, prompt caching, whole-PDF `document` blocks and the reasoning-effort control. It costs less than Opus 5 in every category ($4/$20 per MTok against $5/$25) and reads cache at 0.05x input where every other model here charges 0.1x. Two behaviours differ and both are handled in code: thinking cannot be disabled at any effort level, so `output_config.effort` is the only way to reduce thinking spend and defaults to `medium`; and a forced `toolChoice` is rejected, so selecting Opus 5.5 with opt-in forced-tool extraction falls back to the prose schema and records why in the section's audit metadata. ⚠️ As with every other model in the picklists, a GovCloud deployment cannot reach it through the `us.`/`eu.`/`global.` prefixes and needs the bare model ID or an account-scoped inference-profile ARN. It is in the benchmark matrix but nothing has been run yet, so the [Configuration Guidance](docs/benchmarking/config-guidance.md) tables are unchanged.

- **Processed documents can be added to a test set, including documents that have no ground truth yet** ([#812](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/812)). A test set's **Add documents** menu has a new **From processed documents** source: pick completed Production documents and they are copied in with any ground truth already saved for them. A document with none is added unlabeled, the dialog names those documents before you confirm and warns when that would make a labeled set read as unlabeled, and a document already in the set is skipped so reviewed labels are never overwritten. Open to Authors as well as Admins, because every key is checked server-side against the configuration profiles the caller may access. See [Adding Documents to Existing Test Sets](docs/test-studio.md#adding-documents-to-existing-test-sets).

- **The orphaned-blueprint cleanup is reachable from the CLI and the SDK** ([#1207](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1207)). A replace-mode BDA sync that could not delete a blueprint it had already removed from the project now prints those ARNs and names the cleanup as the remedy — and the cleanup, which previously existed only as an unreachable branch of a resolver, is now `idp-cli config-sync-bda --direction cleanup-orphaned` and `config.sync_bda(direction="cleanup_orphaned")`. ⚠️ The operation is account-wide and destructive, and the profile you name decides which blueprints survive, so the CLI confirms before proceeding (`--force` for scripts) and the cleanup refuses to run unless that profile exists — a typo would otherwise delete every prefixed blueprint in the account while reporting success. There is no Web UI control yet.

- **`document_class` is written on every metering row, and the four per-document rollup tables carry it as a grain dimension.** One class is derived per document from its sections — no distinct class → `unknown`, one → that class, more than one → `mixed` — with excluded sections (instruction and legal boilerplate) filtered out, so a W2 with an instructions page reads as `w2` rather than `mixed`. `metering_hourly`, `metering_daily`, `metering_docs_hourly` and `metering_docs_daily` widen their grouping key to match, and historical rows that pre-date the column bucket correctly. Consumers that `SUM` across the rollup grain are unaffected; a naive `COUNT(*)` on rollup rows would now over-count. See [reporting-sql-layer.md](docs/reporting-sql-layer.md#track-a--new-tables-data-phase-1-had-nowhere-to-put).

- **Operational hardening for the data-mart rollups: a backfill mode, an hourly reconciler, an absence alarm, and an orchestrated schema migration.** `mode: "backfill"` re-runs the four per-document rollups over a caller-supplied hour range under the existing idempotency guard; a new `:35` reconciler fills gaps left by missed schedules; and `DataMartRollupAbsenceAlarm` pages when the rollup Lambda records zero invocations for four consecutive hours, which is the failure the DLQ alarm cannot see. The one-shot migration that repopulates the two per-document hourly rollups at the widened `document_class` grain runs as a Step Functions state machine — bounded, resumable, and held under Athena's DML ceiling. See the [migration runbook](docs/data-mart-migration-runbook.md).

- **Confidence calibration is measured against exact per-cell ground truth, and the shipped default grader is well calibrated but cannot rank errors.** The benchmark scorer now joins each confidence score to the cell it describes and reports ECE, Brier and AUROC through the same engine the product acts on, so a calibration error that grows or ranking power that falls gates a configuration change. Measured over 466,322 cells: every arm passes calibration, but `separate` mode on the default Nova Lite grader places all 70,320 of its cells in a single confidence bin at AUROC 0.417 — so a worst-first review queue ordered by those scores is ordered arbitrarily, where the same extraction graded by Claude Sonnet 5 reaches 0.859. **No default changed:** Nova Lite is a ~130× cost decision, and this supplies the half of that trade-off that had been assumed. See [confidence calibration](docs/benchmarking/studies/confidence-calibration.md) ([#935](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/935)).

- **The `Annotator` role can now be assigned from an external IdP group claim, via the new `ExternalIdPAnnotatorGroupName` parameter** ([#968](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/968)). The deployment creates five Cognito groups but the group mapping understood only four, so a federated user could not hold `Annotator` and arrived with no group at all. The parameter is optional and defaults to empty, so an existing stack needs no action. ⚠️ If you set it, the IdP becomes authoritative for `Annotator` in both directions, so do not also assign the role by hand; and the `allowedTestSets` scope an annotator needs is still assigned only in User Management, so a claim-driven annotator is denied everything until an Admin assigns them a test set. See [External Identity Provider](docs/external-idp.md#annotators-and-the-allowedtestsets-scope).

- **A clear message for an account with no role.** A user whose account has not been placed in a Cognito group now sees "Your account has not been granted access yet", with the roles an administrator can grant, instead of a navigation whose every page fails; a session that cannot be read is reported separately as the transient condition it usually is ([RBAC](docs/rbac.md), [#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)).

- **Deep Analysis's August 2026 vendor assessment of the accelerator is published, and `docs/references.md` gains an Analyst Reports section for it.** A nine-page independent review, following the firm's November 2025 one, which singles out the empirical cost-and-accuracy work — Test Studio, confidence calibration and the Auto Optimizer — as ahead of the commercial field. The PDF is the publisher's copyrighted work rather than repository content and is **not** covered by this repository's MIT-0 license; `docs/analyst-reports/PROVENANCE.md` records the rights grant per report. The documentation site serves it directly. See [Analyst Reports](docs/references.md#analyst-reports).

### Changed

- ⚠️ **Six end-of-life Bedrock models are no longer offered anywhere a model is chosen.** `us.amazon.nova-premier-v1:0` and five Claude versions (`claude-3-5-sonnet-20240620-v1:0`, `claude-3-5-sonnet-20241022-v2:0`, `claude-3-7-sonnet-20250219-v1:0`, `claude-3-haiku-20240307-v1:0`, `claude-opus-4-20250514-v1:0`) now answer `ResourceNotFoundException: This model version has reached the end of its life`, established against the live Bedrock API rather than read off a model card. ⚠️ Four of them were **code defaults**, so summarization, both rule-validation fields and LLM-based evaluation now default to a current model (Nova Pro, Claude Sonnet 4.5 and Claude Haiku 4.5) — a deployment that left any of those at its default gets a working model, and a different one, where it previously got an error. Three were EU mapping targets and now name live Claude 4.5 models. `idp-cli config-validate` and `config-upload` reject a configuration that pins one; loading a stored configuration that names one is unaffected, so nothing breaks further on upgrade. ⚠️ **Upgrade-visible:** `KnowledgeBaseModelId` is a CloudFormation parameter, so a stack whose current value is one of the removed models has its next update **rejected** — pass a current model for it in the same update. See [Retired and legacy Bedrock models](docs/configuration.md#retired-and-legacy-bedrock-models).

- ⚠️ **`idp-cli` now exits non-zero in seven places where it reported failure and exited 0** ([#1230](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1230)). **This changes a public interface.** A pipeline that reads these exit codes and proceeds on 0 will start stopping, which is correct but is not something anybody opted into, so read the list before upgrading a script. There is no opt-out flag.
  - **`status --wait`** exits 1 for a batch that finished with failures, as the polled form always did, and 2 when the watch ended without a verdict. `idp-cli status --wait && deploy` previously proceeded after a batch in which every document failed. `process --monitor` and `rerun --monitor` still exit 0 whatever the batch did, so downstream collection of the documents that succeeded is unaffected.
  - **`test-result`** exits 1 when the run's status is `FAILED` or when any file failed, so `test-result ... && deploy` no longer proceeds on a failed evaluation. The accuracy figures are still printed first.
  - **`delete-documents`** exits 1 when *every* deletion failed; a partial failure still exits 0 and reports the per-document list.
  - **`delete --force-delete-all`** exits 1 when the CloudFormation deletion failed, after the cleanup phase has run.
  - **`config-download`** with a profile that does not exist raises instead of emitting a YAML null document — `config-download --config-profile lendnig > config.yaml` used to exit 0 and leave a file every downstream step reads as an empty configuration. Fixed in the SDK, so both surfaces and the default-profile resolution are covered.
  - **`config-upload --config-profile ""`** is refused before anything is written; the configuration previously landed on a bare `Config` key, invisible to `config-list` and read by nothing, and was announced as active.
  - **A single-document JSON status result that found no document** reports `{"document_id": null, "status": "UNKNOWN", "exit_code": 2}` instead of a batch summary, aligning the JSON and table formats.

- ⚠️ **Eighteen API operations now require an assigned Cognito group** ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979), [#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033)). Eleven document reads and mutations (`getDocument`, `listDocuments`, `listDocumentsByDateRange`, `getDocumentVersion`, `compareDocumentVersions`, `getFileContents`, `getFilePresignedUrl`, `queryKnowledgeBase`, `deleteAgentJob`, `deleteChatSession`, `sendChatDocumentMessage`) and seven reads that lead to document content (`listDocumentsDateShard`, `listDocumentsDateHour`, `listDocumentVersions`, `getDocumentCount`, `getStepFunctionExecution`, `getChatMessages`, `getCircuitBreakerStatus`) were reachable by any authenticated user, including one in no group at all — which self-service sign-up produces when `AllowedSignUpEmailDomain` is set, and on a deployed stack that chain composed end to end from no knowledge of the deployment. They now require membership of at least one of Admin, Author, Reviewer, Annotator or Viewer. ⚠️ **Before upgrading, place every user who needs document access in a group** (User Management, or `idp-cli`): an existing user in no group loses the document reads, the timeline, version history, the workflow view, chat transcripts and the processing-paused badge. ⚠️ **This gates the API, not the document buckets** — the Cognito Identity Pool's authenticated role grants every signed-in user direct `s3:GetObject`/`ListBucket` on the Input and Output buckets regardless of group, and it does not apply to the chat streaming Function URL, which carries no group claim ([RBAC](docs/rbac.md), [Identity Pool group scoping](docs/planning/identity-pool-group-scoping-plan.md)).

- ⚠️ **Both per-user access scopes are now enforced on the S3 object key, not just on the API record.** `allowedConfigVersions` (which confines a non-admin to named [Configuration Profiles](docs/configuration-profiles.md)) and `allowedTestSets` (which confines an Annotator to named test sets) were enforced by the operations that serve content by name and not by `getFileContents`/`getFilePresignedUrl`, which validated only the bucket — so a scoped user could fetch an excluded profile's configuration, or an excluded test set's documents and ground truth, as files. Both now match the caller's scope against the key before any S3 call, and fail closed: an absent claim or a failed record lookup denies. ⚠️ **A scoped user loses reads they could previously perform**, which is the intent; widen their scope in User Management if they need them. ⚠️ **The Configuration bucket is also removed from the Cognito Identity Pool's authenticated role**, which granted every signed-in user direct browser-side read and list on it regardless of group. The web UI never used it, but **custom browser-side code or an integration that reads the Configuration bucket with Identity Pool credentials will now get AccessDenied** — route it through `getFileContents` / `getSampleDocumentUrl`. Per-document read scoping remains open. See [RBAC](docs/rbac.md#configuration-profile-scoping-use-case-isolation) ([#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033)).

- ⚠️ **Ten caller-visible API status changes, four of them turning a previously-successful 200 into a 403** ([#1041](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1041)). The three upload paths (`uploadDocument`, `uploadSampleDocument`, the discovery upload) took an unbounded destination bucket and key prefix from the request and are now confined to the buckets the deployment wires, refusing a target outside them with 403 where a presigned POST was previously minted. On the read side `getFileContents`/`getFilePresignedUrl` answer 403, 400 and 400 for a disallowed bucket, a malformed `s3Uri` and an absent object where all three were 500, and an unconfigured allow-list now refuses every request instead of failing open. `getChatMessages` for another user's session answers 403 rather than 500; `listChatSessions` clamps an over-large or non-positive `limit` instead of erroring; and `listDocumentsByDateRange` refuses a span over 365 days with a 400 naming the maximum instead of running past the dispatcher's budget. ⚠️ **Alerting and client code keyed on the old statuses needs updating** — the reason the refusals moved is that a monitored 5xx rate was counting deliberate policy denials. See [the status a refusal arrives with](docs/rbac.md#the-status-a-refusal-arrives-with-and-why-it-matters-operationally) and [the 365-day cap](docs/migration-appsync-to-rest.md#listdocumentsbydaterange-is-capped-at-365-days).

- ⚠️ **`EvaluationMetrics`' four `avg_*` scores are now `Optional[float]` and can be `None` where they were `0.0`**, so `f"{metrics.avg_accuracy:.1%}"` raises and client code that formats or compares them needs a guard. They are `None` when nothing in scope reported the metric, and **also whenever `document_class` is passed** — those figures come from whole-document metrics, which previously answered a class-scoped question with a whole-document number. The class-scoped answer moved to `by_document_class[<class>]`, which now carries all four scores and whose `avg_accuracy` **changes value**, because sections the pipeline excluded from evaluation are no longer counted in the denominator. `SearchResult.confidence` is `None` rather than `0.0` when nothing matched, `DocumentListResult.count` and `EvaluationBaselineListResult.count` are documented as page sizes, and `DocumentInfo` gains `batch_id` and drops the always-`None` `num_pages`/`document_class`. See [the SDK reference](docs/idp-sdk.md).

- ⚠️ **`rule_validation.semaphore` now bounds the rule-validation orchestrator's Bedrock calls, which it previously did not, so consolidation gets slower and quieter on rule-heavy documents** ([#1053](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1053)). The orchestrator handed out a fresh semaphore per task, so the configured value had no effect on that step at all and what bounded it was the event loop's default thread pool. A 14-rule document made 14 concurrent calls with `semaphore: 1` configured; it now makes the configured number. ⚠️ **This is an intended throughput reduction:** at 4,096 MB a 14-rule document goes from about two waves to three. Raise `semaphore` if you would rather have the speed and your Bedrock quota can carry it, remembering the account-level rate is this value times the documents in flight — at the default `MaxConcurrentWorkflows` of 100, a `semaphore` of 5 permits up to 500 concurrent calls from this step alone. No shipped preset overrides the key. See [rule validation configuration](docs/rule-validation.md#configuration-parameters).

- ⚠️ **`minItems` on a list field is a hard floor in Advanced extraction and advisory in Simple, and the guidance now says which** ([#1048](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1048), [#1047](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1047)). In **Advanced** mode the constraint reaches the agent's tool boundary, so a list under the floor is rejected where it is produced and an extraction that cannot reach it fails keeping no rows and no diagnosis — a floor a document cannot legitimately reach is a decision to fail those documents. ⚠️ And once a section **shards**, which the shipped `max_concurrent_batches: 10` means any Advanced-mode section over five pages does, the floor is enforced **per shard**, so a section-sized floor is unsatisfiable: `minItems: 100` over a 17-page section fails a document that genuinely contains 800 rows. Use `extraction.row_shortfall_action` instead on a sharding configuration. In **Simple** mode the same shortfall is a warning, and `extraction.validation.fail_action: reject` records `parsing_succeeded: false` without failing the section or routing it to human review. Two agentic-extraction notebooks are corrected to match ([#1063](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1063)). See [`minItems` in Simple vs Advanced mode](docs/extraction-and-confidence.md#minitems-on-a-list-field--a-warning-in-simple-mode-a-hard-floor-in-advanced).

- ⚠️ **Three fine-tuned benchmark configurations named a Bedrock model that only existed in one account, and now ship the base model instead** ([#1093](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1093)). `config_library/unified/docsplit/docsplit_finedtuned_config.yaml` and the two `ocr-benchmark` fine-tuned files pinned a `custom-model-deployment` ARN, which is account-scoped, so from any other account Bedrock answers `AccessDeniedException` and the stage fails outright. All five values are now `us.amazon.nova-2-lite-v1:0`, the base model the fine-tune was derived from. **Action:** nothing, unless you installed one of these three files explicitly — they are not selectable presets. ⚠️ If you did and you own those deployments, the stage was working and will now run on the **base model**, so your classification and extraction quality changes silently on upgrade; re-point `model` at your own deployment ARN, which each file now shows the shape for. A gate in `config_library/test_config_library.py` fails any account-scoped Bedrock ARN in the shipped library from now on.

- ⚠️ **The standalone seller entitlement service logs at `WARN` by default, and the feature-platform control-plane Lambdas redact the event they log** ([#1040](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1040)). That service declared `LogLevel: INFO` and is deployed by a path that never passes the parameter, so `INFO` was what a seller got; it now matches the root template. ⚠️ A seller who wants the louder level must pass `LogLevel=INFO` explicitly. The nine control-plane handlers behind the Extensions catalog now redact through the same denylist the other handlers use.

- **Publishing a test set version happens on the test set's own page, in a dialog** ([#903](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/903)). It moves from the Test Sets table's **Actions** menu to the top of the individual set's page, beside the draft-label and annotation controls whose pass it completes. The dialog names the version number and document count, collects the **Label** and **Notes** the API has always accepted (nothing previously sent them), lets you publish without moving the set's active reference point, and says why it is unavailable when it is. ⚠️ The action is gone from the Test Sets table, which keeps its **Version** column; the mutation takes one set at a time, so no bulk publishing is lost. It is offered only to Admins and Authors, the groups it has always been restricted to — other roles previously saw it enabled and got a 403. See [Publishing a version](docs/test-studio.md#publishing-a-version).

- **The Nova fine-tuning CLIs and the live agent test scripts moved out of the `idp_common` package.** The four fine-tuning scripts are now at `scripts/model_finetuning/` and the DynamoDB service example at `scripts/examples/`, so every command on the [Nova fine-tuning page](docs/nova-finetuning.md) runs from the repository root rather than from inside the installed library. The seven operator-run agent scripts moved to `lib/idp_common_pkg/manual_tests/agents/`, so `idp_common.agents.testing` is no longer importable and no longer ships in the distribution. Nothing in the library imported any of them.

- ⚠️ **The UI lint is strict, and that reaches the release build.** `src/ui`'s `build` script is `lint && typecheck && build:only`, and the lint now runs with `--max-warnings 0` and fixes nothing, so `make ui-build` and the UI build inside `python3 publish.py` fail on a single `@typescript-eslint/no-explicit-any` where they previously succeeded. The tree is clean today; if a publish stops on an ESLint warning, `make ui-lint-fix` plus a real fix for the non-auto-fixable findings is the remedy. Note also that `make lint` is a mixture — its Python half auto-fixes while its UI half only reports.

- **Advanced extraction: the measured effect of greedy decoding on Bedrock's `Model produced invalid sequence as part of ToolUse`.** `topK: 1`, and AWS's full `temperature 0` + `topP 1` + `topK 1` triple, leave Nova Lite's failure rate statistically unchanged, so the inference-parameter explanation is closed by measurement. The trigger is the request's schema **shape** rather than its size. **No configuration option was added** — a user-facing knob whose measured effect is null would be worse than none. See [greedy decoding and tool use](docs/benchmarking/studies/greedy-decoding-tool-use.md) ([#956](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/956)).

- **`extraction.agentic.restate_schema_in_system_prompt` guidance corrected: turn it off for shard headroom, not for dollars.** Dropping the system-prompt copy of the schema costs no quality and saves 5–7% of input-side extraction tokens, which is real headroom against `shard_token_budget`. It is **not** a cost saving: those tokens are cache reads at about a tenth of input price, while extraction output tokens rise ~17% on a list-heavy class, so extraction cost's central estimate rises rather than falls. See [schema restatement and tokens](docs/benchmarking/studies/schema-restatement-tokens.md) ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).

- **Several authorization statements in the RBAC reference corrected.** `listUsers` is Admin-only rather than filtered per caller; five operations (`getAgentChatMessages`, `updateChatSessionTitle`, `updateAgentChatMessage`, `validateTestSetForFinetuning`, `listAvailableModels`) are declared in the schema but routed from nowhere, so the dispatcher's default-deny refuses them to every caller including Admin, where the page had described them as open; and the role table is introduced as the five roles it lists. The same `listAvailableModels` claim is corrected on the fine-tuning guide ([RBAC](docs/rbac.md), [Custom model fine-tuning](docs/custom-model-finetuning.md)).

### Fixed

- 🔒 **Some documented `pip install` commands could install a third party's package instead of this repository's.** The first-party packages require each other by bare name, and those names on public PyPI belong to unrelated parties, so a path install that omits a sibling — `pip install -e lib/idp_sdk`, or the `cd lib/idp_cli_pkg && pip install -e .` the Quick Start and the deployment guide gave for the CLI — let pip satisfy the missing name from the index. Every documented invocation now installs the siblings it needs on one command line, or points at `make setup`. **Action:** if you followed one of those instructions, run `python scripts/check_first_party_deps.py`; if it fails, follow the remediation in [Installing First-Party Packages Safely](docs/dependency-confusion.md) — and because the package ran in an environment holding AWS deployment credentials, inspect the artifact you received without installing it and rotate those credentials if you cannot rule out that it executed.

- ⚠️ **Two document-deletion selectors matched more broadly than documented, and both fed a delete** ([#1133](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1133)). A batch id was an unanchored substring, so deleting `batch-1` also selected `batch-10/…` and `archive/batch-1x/…`; it is now matched as a leading path segment, which also makes `batch_id=""` — a substring of every key, and so a selector for everything — select nothing. Separately a document's output purge enumerated siblings sharing its key as a byte prefix (`invoice.pdf.bak/…`), and that path deletes **all object versions and delete markers**, so a collision destroyed a sibling's entire output history. ⚠️ **If a collision already occurred the loss is not recoverable from versioning** — deletion is by version id, and the output bucket carries no Object Lock, replication or backup. A sibling's **current** outputs can be rebuilt by reprocessing; its stored run history cannot. **Action: if you relied on a batch id selecting by substring, pass `pattern` instead** (`--pattern "*batch-1*"`), where the breadth is visible in what you wrote.

- ⚠️ **A document-deletion selector that names nothing, or a scan that fails, is now refused instead of reported as a completed delete** ([#1187](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1187)). `get_documents_by_batch` and `get_documents_by_pattern` returned a list of keys and wrapped their whole scan in one broad handler, so a throttle, a missing table or a server error came back as `[]` — indistinguishable from "that batch holds no documents" — and the deleter reported `success=True, deleted_count=0` while the CLI printed "No documents found" and exited 0. A partial failure part way through pagination or filtering was worse still, deleting a subset and reporting it as complete. Faults now surface as `IDPProcessingError` and exit 1, and a non-string or empty selector is refused before any scan. **Action: a direct caller of either library function now sees an exception where it previously saw an empty list;** `idp-cli delete-documents` and `client.batch.delete_documents` already refused a falsy selector in their own layer.

- ⚠️ **Uninstalling a feature destroyed the active configuration profile's revision history, and a stack that has had one uninstalled has already lost it** ([#1111](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1111)). The hook registration Lambda wrote the profile head row with `put_item`, which replaces the whole item, so a single successful call deleted `LatestRevision`, `PublishedRevision`, the three fields linking the profile to its BDA project and the `_feature_id` stamp, and reported success. The next save was then numbered revision 1 again and overwrote the stored body of the original revision 1, whose S3 key is write-once. The symptoms are a duplicated revision number with no "current" badge, new documents falling back to the mutable profile head, the live revision becoming deletable, and a profile silently unlinked from its BDA project. The write is now a targeted conditional `update_item`. **Action:** if a feature has ever been uninstalled from, or rolled back on, a stack, whichever profile was active at that moment lost its counters and any revision cut afterwards overwrote an earlier body; this change does not reconstruct them.

- ⚠️ **A Z3 rule with an `Int` threshold could report Pass for a document that fails it** ([#1057](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1057)). A reading for an `Int` parameter was truncated toward zero before the solver saw it, so `days_late <= 30` against an extracted 30.9 compared 30 against 30 and returned a confident Pass. Truncation is unsound in both directions and no rounding rule fixes it, so a reading that cannot be held by its declared type is now **refused**, matching what path-based extraction already did; a `Real` parameter holds a fractional reading exactly rather than rounding it to a double. **Action:** declare a genuinely fractional parameter `Real` — `Int` means the quantity is whole, and the constraint language compares the two, so a `Real` parameter works with a whole-number threshold at no cost. Documents whose rules were passing on a truncated reading will now report **Fail** or **Information Not Found** with the reason, and both are the corrected verdict: **re-run any compliance report you relied on that has an `Int` threshold.** See [Z3 rule validation](docs/rule-validation-z3.md).

- ⚠️ **A Z3 rule could be decided using a different document's readings** ([#1115](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1115)). The path-based data extractor memoized each reading under the *address* of the document it read, and CPython reuses addresses, so once a document was freed the memo could answer for the next one with the previous one's values — producing a compliance verdict computed against another document's data, at normal confidence, with nothing marking it. Reading fifty successive documents through one extractor, 48 of the 50 readings were another document's. **There is no way to identify affected verdicts retrospectively.** The reachable surface is the library's path-mapping extraction route (`DataExtractor`, `ValidationSystem`, `Z3EngineAdapter`, as used by the dual-engine notebook and by code embedding them) and **not** the deployed pipeline's Z3 route, which does not go through this memo. The memo is now scoped to a single `extract_values` call.

- ⚠️ **A throttled or timed-out Bedrock call during rule validation is now retried instead of ending the document — and it can no longer be answered with a compliance verdict** ([#1101](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1101)). Nine `except` blocks absorbed transient faults instead of raising, so the retry ladders the task states carry were unreachable and the failure surfaced as one of three wrong outcomes: a fault in one rule's model call returned `Information Not Found`, which is a **verdict** counted in the document's summary; a fault elsewhere in the per-section step lost the classification and left the document failed and un-retryable; and a fault during consolidation returned an empty result normally, so the document completed with no verdicts and nothing recorded. Transient causes now re-raise under the one name the task states list. ⚠️ Two residuals: a ladder that exhausts every attempt leaves the sections unflagged, and a **sustained** regional throttle can now hold a workflow-concurrency slot for hours before stopping at `WorkflowExecutionTimeoutSeconds` as `TIMED_OUT`, which records no per-section diagnosis. See [Rule Validation](docs/rule-validation.md#a-transient-fault-is-never-answered-with-a-verdict).

- ⚠️ **A document that fails rule validation or the collate step now records *why* on the sections it affects, and one failure recorder that had never run now runs** ([#1064](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1064)). Three handlers held an explanation of the failure and raised without writing it, and none of their task states has a `Catch`, so the explanation existed only in the Step Functions cause and the log — the rule-validation orchestrator's recorder could not even execute, naming a `Status` member that does not exist and a keyword the document service does not accept, both swallowed as "Failed to update document status". All three now persist an error-severity processing issue before re-raising the original exception unchanged. ⚠️ Look at the Sections panel rather than the document list: the list's **Processing Issues** badge is only refreshed by the two steps that write the whole document, so a per-section rule-validation failure can leave it showing its previous value. A transient failure records nothing, because the state machine is about to retry it. See [Where a failed rule validation shows up](docs/rule-validation.md#where-a-failed-rule-validation-shows-up).

- ⚠️ **Rule validation's character chunker had a non-terminating path, and `overlap_percentage: 0` asked for no overlap and got the maximum** ([#1090](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1090), [#1091](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1091)). The character fallback re-appended the same tail on every pass until the function exhausted its memory and Lambda killed it — fast and loud rather than a silent hang, and reachable on a section of whitespace-only OCR text, which a failed scan or a batch of separator sheets can produce. Separately the page chunker's single-page overlap branch used `page_content[-overlap_size:]`, and `x[-0:]` is the whole string, so `overlap_percentage: 0` repeated the entire previous page: on a document whose pages are large relative to `max_chunk_size` that roughly doubled the prompt tokens of every fact-extraction call, with the only symptom being the bill moving opposite to the knob. **Action:** if you have set `overlap_percentage` above 50, the character fallback now repeats 50% rather than the figure you set, and logs a warning naming both numbers. No action is needed at the shipped default of 10.

- ⚠️ **Agent chat transcripts were losing about half their messages, and existing transcripts are incomplete** ([#1098](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1098)). Every message was appended by reading the whole transcript, growing it by one and writing it back with nothing to detect an intervening write, and a single agent turn adds several messages within microseconds — four messages submitted persisted two, with every individual write succeeding. Appends are now conditional on a version attribute and retried with jitter, which lost no message at any fan-out measured; a message that still cannot be stored is dropped rather than forced through, and counted on a new `AgentTranscriptMessageDropped` metric and alarm. The same correction is applied to the multi-turn conversation memory behind agent chat. **Action:** transcripts already stored are not repaired and remain missing whatever was overwritten; no migration is needed. ⚠️ During the Lambda version rollover a writer still on pre-upgrade code can overwrite a new one's append; that window closes without intervention.

- **A queued agent-transcript write is now waited for, and a drain that could not finish is reported separately from a dropped message** ([#1110](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1110)). Transcript messages are written on a thread pool and the drain path existed, but nothing in production reached it, so the last messages of an analytics agent's turn — the part a user is most likely to be looking at — could be abandoned when the execution environment froze. The pool is now drained at `IDPAgent.__exit__`, bounded at two seconds because for a sub-agent that happens mid-turn with the user waiting, and what the bound leaves behind is counted on `AgentTranscriptDrainIncomplete` with its own alarm rather than on the dropped-message counter — an abandoned write frequently commits later, where a dropped message is gone. Only the analytics agent records a transcript. A message's `sequence_number` is also now unique within a job, computed from the stored transcript's length inside the same conditional write ([#1106](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1106)). See [Agent Transcript Write Failures](docs/monitoring.md#agent-transcript-write-failures).

- **Five read-modify-write paths no longer discard a concurrent writer's data** ([#1111](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1111)). HITL review history is now appended by DynamoDB itself, so several annotators reviewing different sections of one document — the designed workflow — no longer overwrite each other's entries. The fine-tuning deployment's pricing entry, the Test Studio draft-label harvest and opening an annotation draft each write under a condition and recover by merging or by reporting the winner; a lost harvest failure entry was what left a draft-label job running forever, and two annotators pressing **Start annotating** on one set each opened a transition of which only one survived. Opening an annotation draft can now report that it could not start, in the one case where a set's first publish commits in the same window. The configuration writer and the shared whole-document write are catalogued in that issue and left open.

- ⚠️ **A Z3 rule whose constraint misspells a parameter is rejected when the rule is generated, instead of failing every document that rule is evaluated against** ([#1058](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1058)). The validation method was documented as raising for an undeclared parameter and its loop body was `continue` and two comments. The cost was not the missing check — the solver does reject the unknown atom — but that a translated rule is cached under a key derived from the rule description, so one bad translation was re-read and re-failed as *Information Not Found* for every later document carrying that description. It now raises, so the Config Editor's **Generate RuleJSON** reports which token could not be resolved and the retry costs one translation. See [Z3 rule validation](docs/rule-validation-z3.md).

- **Rule validation consolidates the verdicts this run produced, instead of everything sitting under the document's rule-validation prefix** ([#1143](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1143)). Consolidation globbed the prefix that the previous run's cleanup deletes into, so a surviving object was merged as though this run had written it — and because the verdicts are for the *same* document, a rule whose verdict changed from `Fail` to `Pass` between runs could be consolidated at the stale value and the compliance decision followed it. A deterministic cleanup failure (`AccessDenied` from a missing `s3:ListBucket` or `s3:DeleteObject`) was logged at WARNING and the document completed on mixed verdicts. Consolidation now reads exactly the per-section URIs this run's Map produced, which also removes a stale object's ability to push the section count past one and route a single-section document through LLM summarization. **Action:** none; no reprocessing or migration is needed.

- **A rule-validation compliance report keeps its statistics when a page reference is unusual, and no longer comes back empty when the model numbers its references** ([#1114](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1114), [#1052](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1052)). A model answering `"supporting_pages": [1, 2]` rather than `["1", "2"]` raised inside the sort key, and the catch-all around the whole method turned that into a report with every statistic discarded and no processing issue recorded — a document that looked like one where nothing had been evaluated. References are now canonicalised where they are collected, so the published `supporting_pages` is a deduplicated, totally ordered list of strings; page 1 is no longer listed twice on a document that routes some rules to the solver and some to the model, and a value that cannot be a page is dropped from the aggregate and named in a warning. A consolidation that does fail now keeps the policy types, rules, counts and pages gathered before the failure and says above the statistics that it did not complete. Contributed in part by @mrhard9090. See [Page Number Handling](lib/idp_common_pkg/idp_common/rule_validation/README.md#page-number-handling).

- **One stalled Bedrock request no longer costs a whole document's extraction, and the shards that already succeeded are no longer thrown away with it** ([#1014](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1014)). A shard invocation is capped at Lambda's 900 seconds, and four separate timeout and retry numbers spent that budget without anything checking they added up — so a shard meeting a transient `Read timed out` ran into the wall clock instead of retrying, Step Functions read the resulting `Sandbox.Timedout` as deterministic, and because `ExtractionShardMap` tolerates no shard failures it then **discarded the output of the shards that had completed**, leaving the document with zero rows. The terms now live together in `idp_common.timeout_budget` with the whole inequality asserted, and the Map gains a retrier so the per-shard persistence can finally be used, plus a `Catch` naming the section. ⚠️ An execution that fails this way now reports `ExtractionShardMapFailed` rather than `States.ExceedToleratedFailureThreshold`, so alerting keyed on the old name needs updating.

- ⚠️ **A section whose extraction failed now says so in the Sections panel, instead of showing nothing** ([#1049](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1049)). Both extraction Lambdas wrote the section to the tracking table only after the call returned, so a failure left the section holding whatever classification had put there: no status icon, no issue, and nothing identifying which section failed. Each entry point now persists the section before re-raising, as an error-severity `extraction_failed` issue carrying the exception's own explanation and remedy, and the Status column distinguishes **Failed** from **Incomplete**. A transient failure is deliberately not marked. Also fixed: the document list's **Processing Issues** badge was stamped back to 0 for any document whose execution failed, and the cleanup that releases a section's per-shard results had never deleted anything. ⚠️ The constant bounding a processing issue's free-size fields is now `ProcessingIssue.MAX_ROOT_CAUSE_BYTES`, replacing `MAX_ROOT_CAUSE_CHARS`, and the unit changed with the name. See [where a failed section shows up](docs/extraction-and-confidence.md#where-a-failed-section-shows-up).

- **A section left with no confidence scores, or with fabricated ones, is now reported** ([#1006](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1006), [#1036](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1036)). Several early returns in Assessment left a section unscored and completely silent — they wrote to `document.errors`, which is read only on a failed document — so no processing issue reached the section, no `AssessmentConfidenceUnavailable` metric was published, and the document completed looking fully scored. Worst of the group: a section whose pages were all absent had its confidence assessed against empty text and came back **with** a score for every field, which is indistinguishable in the UI, in HITL routing and in the reporting lake from one the model read off the page. That pass no longer runs; the section is reported as unscored. A section whose class the configuration no longer contains, and pages classification could not place, now surface too (`extraction_class_not_configured`, `classification_failed`, `classification_page_no_content`, `classification_invalid_class_fallback`). ⚠️ **Upgrade-visible:** a document containing pages the classifier cannot place will now show the **Processing Issues** badge; defining a catch-all class the model can legitimately choose removes the condition. ⚠️ **Library callers:** `AssessmentService.process_document_section` now raises `ValueError` for a falsy document, a document with no sections, or an unknown `section_id`. See [Monitoring](docs/monitoring.md#confidence-assessment-degraded) and [Classification](docs/classification.md#pages-classification-could-not-classify).

- **A declared `maxItems` now bounds the row-shortfall check's OCR evidence, so a list extracted to its schema's own ceiling is no longer called truncated** ([#1046](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1046)). `maxItems: 15` beside a 40-row table, correctly capped at 15, scored 15/41 and was reported as a probable truncation — under `extraction.row_shortfall_action: fail` that is a lost document over a complete extraction. ⚠️ Because the check only applies from 30 rows up, a ceiling below that takes the field out of the check entirely, including a shortfall against the ceiling itself; that is deliberate, and it is what makes `maxItems` a usable per-field opt-out. An under-declared ceiling weakens the check in proportion at any size, so declare one your longest expected document can reach. **No shipped preset declares `maxItems` on any array field**, so nothing changes for a deployment that has not set one. See the row-shortfall section of [extraction and confidence](docs/extraction-and-confidence.md).

- **Advanced extraction: a required list or nested object no longer costs a correction round per shard** ([#1078](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1078)). A shard is told to leave a field null when its own pages do not contain it, but the shard's tool kept `required` non-nullable for a container — so for a required list or group the instruction named the one answer the tool rejected, and a shard holding none of a required table spent a round discovering it had to emit `[]`, with the feedback validator reporting that same payload as valid. A required array or nested object now accepts `null` in the shard model, and presence is judged once on the merged section against the real class schema. The structural half does not relax and row-count bounds are untouched. **No shipped configuration paid this cost.** See [`minItems` in Simple and Advanced mode](docs/extraction-and-confidence.md).

- **Test Studio's confidence figures and the stored confidence curves now key a list index the same way** ([#1066](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1066)). The test-set resolver keyed a list index off the list's length rather than its depth, so a one-row table keyed `Transactions.Date` where the evaluation package — which is what a stored confidence curve is keyed by — keyed `Transactions[0].Date`. The rule now has one implementation. ⚠️ One user-visible change: a list-level confidence measured against a single null row is no longer excluded from `minConfidence`, `confidenceThreshold` and `alertCount`, which is the answer a multi-row list already gave; an *empty* list is still treated as absent.

- **Five OCR defects where the page text stayed correct and the page image or the discarded data did not** ([#1158](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1158)). An image embedded in a `.docx` contributed the literal `[Image - OCR failed]` under any backend other than `textract`, so a scanned page or screenshot pasted into a Word document contributed nothing and the document completed with that content missing. A CSV row with more fields than its header lost every field past the header's width — the path reached *because* the file was ragged was the one discarding the ragged data. The hardcoded 1x1 JPEG written when image creation fails declared one component in its header and three in its body, so it passed a header check and raised `broken data stream` in whatever first read it. And a continuation page's repeated table header pushed that page's last rows off the image while leaving them in the page text. ⚠️ **An unreadable image dimension in configuration is now rejected instead of ignored:** a malformed `ocr.image.target_width` or `target_height` fails configuration validation with the offending value named, where it previously deployed and silently applied the default ceiling. An empty value still means "not configured".

- ⚠️ **Word documents now paginate at the DPI they are rendered at, which changes how many pages a `.docx` becomes** ([#1156](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1156)). The layout budget deciding where a Word page breaks was computed at a hardcoded 150 DPI while the canvas is sized from the configured DPI, which the OCR service builds at 300 by default — so pages broke early and each extra page was separately uploaded, OCR'd, classified and billed, while below 150 DPI the overflow was drawn off the bottom edge and a vision-capable call saw a page image with content missing. Page text was correct in both directions, which is why neither surfaced. **Action: expect a different page count, and not only for Word.** An 80-paragraph document becomes 2 pages at 300 DPI where it was 3; CSV, Excel and markdown-rendered content shift too, because continuation pages now reserve room for the repeated table header. Anything that recorded a page count before this — a stored total, a per-page cost comparison, an evaluation baseline keyed by page index — will not line up with a fresh run, so re-run rather than compare across the change.

- **The header row of a Word table is emphasised in the rendered page image** ([#1157](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1157)). Header detection compared `table.rows[0] == row`, and `python-docx` builds a fresh row object on every access with no `__eq__`, so **no row of any table in any `.docx` was ever marked as a header** — each lost its bold and grey background in the page image, which is what a vision-capable classification or extraction call sees. The header is now taken from Word's own repeat-as-header flag where the author set it (reading its value, so an explicit "not a header" is honoured), with the first row as the fallback. **Action:** none; re-run OCR on a Word document to pick up the corrected page image, or leave it, since the extracted text is unchanged.

- **A metering unit no pricing entry lists is no longer assumed to be free** ([#1212](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1212)). Both cost implementations treated a unit absent from a matched `pricing.yaml` entry as `$0.00`, and every numeric member of a Bedrock `usage` block is forwarded into metering by design — so a field AWS adds to that block reaches the cost path without anyone adding it and is billed at exactly zero, in the cheap direction, with no NULL to query for. The non-chargeable units are now declared in one place and anything else an entry omits is reported as unpriced: `unit_cost` and `estimated_cost` reach the reporting table as SQL `NULL`. **No reported cost changes** — the declared set covers exactly what was already treated as free. ⚠️ It is an accepted assumption rather than an established fact: the set is populated from AWS's published price list, and nothing in the code can tell "not chargeable" apart from "not listed", so a unit AWS charges for but publishes no dimension for would still be billed at $0.00. See [reporting database](docs/reporting-database.md#cost-calculation-and-pricing).

- **A configuration key the solution does not recognise is now reported at every depth, with the path it belongs at** ([#1134](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1134)). The unknown-key warning fired at the top level only — the level where a typo is least likely — so below it a misspelled or mis-nested key was dropped in silence and the guard or cost lever the operator believed they set stayed at its shipped default, which is indistinguishable from a working configuration. The report now names each key by dotted path, and **a key at the wrong depth gets the path it belongs at** where that is unambiguous (`ocr.dpi` → `ocr.image.dpi`), which is the more damaging half: `ocr.dpi: "abc"` was accepted in silence while `ocr.image.dpi: "abc"` raises, so the value was routed around the validator. It **reports rather than rejects**, so a stored configuration that loads today still loads; `config-validate --strict` keeps its top-level-only contract. Dead keys found this way in six shipped presets and five other shipped documents are removed.

- **A mistyped setting inside a pricing row or a model-limit row is now named instead of dropped in silence** ([#1211](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1211)). The Pricing and Model Limits records rejected an unrecognised key at the top of the record but took the permissive default for the rows inside them, so `max_input_tokenz` left the shipped context window in force while the UI reported the save as successful — and `max_input_tokens` is the window the Bedrock client resolves a model against. The equivalent in a pricing row is a `price` written beside `name` and `units` rather than inside a unit, which leaves the shipped rate in force and the cost figures looking plausible. Each record now reports every key it will drop at any depth, with the row index in the path. **It reports and does not reject**, so after editing either panel, confirm the value you set is the value in effect. See [Pricing and Model Limits are separate records](docs/configuration.md#pricing-and-model-limits-are-separate-records-and-they-report-the-same-way).

- ⚠️ **The capacity planner costed every plan against the account's most generously provisioned Bedrock model, and judged the SLA on the median document instead of the tail** ([#1217](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1217)). Per-model TPM limits were collapsed by taking their **maximum**, so a plan whose steps call a narrowly provisioned model was costed as though it had the headroom of whichever model the account is most entitled to. The limits are now reduced over the models the plan's steps actually call, taking the binding minimum and naming the model that holds it in the log — the actionable half, since a quota increase request has to name one model. ⚠️ Only the token-limited half is narrowed; the request-per-minute half is still the smallest RPM quota across every mapped model, so an unused model with a low RPM limit can still cap the figure. Separately, `exceedsLimit` and the SLA warning are now judged on the **P99** rather than the median, which is the reading the UI already presented. ⚠️ **A deployment that reads as within its SLA today may read as exceeding it after this change**, with no change to the documents or the configuration. See [capacity-planning.md](docs/capacity-planning.md#4-capacity-calculation-and-results).

- **The capacity planner's Bedrock requests-per-minute requirement is now derived from a peak hour and counts every Bedrock call a step made** ([#1216](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1216)). Two defects pushed the same figure in opposite directions, so each partly masked the other: the request half multiplied average requests per document by a whole day's document count before dividing by sixty, asking for many times the rate the account will ever see; and the metering scan stopped at the first key matching a step, so a step invoking Bedrock under several keys — which a per-class model override or an escalation produces — was counted once. Undercounting is the dangerous direction, because it reports a quota as sufficient when it is not, and fixing the window alone moves the figure to that side, which is why both are fixed together. On a representative plan the required RPM was 17.6 with neither fixed and is 3.3 with both.

- **Five smaller capacity-planner defects** ([#1219](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1219)). A UI-selected latency time range was written into the process environment, so one user's six-hour view became the default for every later request that warm container served; the window is now a parameter. A step running below one Bedrock request a minute produced **no row at all**, and in a report whose purpose is to list the quotas an operator must check, a missing row reads as a step that is switched off. Turning granular assessment off made the whole report unavailable on a stack whose assessment history sits under granular metering keys; that step's request row is now dropped instead. A report is no longer refused when the only timing a document carries is its workflow timestamps, which is the ordinary shape on a BDA-mode stack. And a maximum-latency target of zero is refused as out of range instead of reported as a field the operator left empty. Also corrected: the headline recommendation described every plan it can actually produce as "(based on unknown source)", including one built entirely from measured timestamps. See [capacity-planning.md](docs/capacity-planning.md#5-aws-service-quota-management).

- **The capacity planner's "actual pages per document" diagnostic was an average that weighted the last document scanned above all the rest, and it was read from an attribute the tracking table does not carry** ([#1218](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1218)). The accumulation halved the weight of everything already seen at every step, so page counts of 1, 10 and 100 reported 52.8 against a true mean of 37 — and the figure was read from `number_of_pages`, a column of the Athena reporting table and never an attribute of the table this function scans, so it was never available on any stack and its line was never printed. The report logged "Using configured page values (no metering data)" every time, which is also what it says when there genuinely is no metering. It now reads `PageCount`, averages once, and names how many records it averaged. No planned quota figure changes.

- **A BDA sync that cannot read the project now fails, instead of reading an unreadable project as an empty one** ([#1162](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1162)). `_retrieve_all_blueprints` answered `[]` for any `ClientError`, which is also how a caller learns a project associates no blueprints — so a throttle or a missing permission was indistinguishable from "BDA is empty", and in replace mode `bda-to-idp` acted on it by **removing every document class from the profile** while `idp-to-bda` created a **second blueprint for every class**. Seven further defects in the same file go with it, including a throttle while verifying a version's recorded project **creating a second project** and orphaning the first one's blueprints, a nested object or array silently dropped from the extraction contract so a class reported success with a whole line-items section missing, and a blueprint created but never associated with the project being reported as success.

- **Four more BDA sync defects that reported success while losing information.** A sync's `processed_classes` — the only part of the result that says *which* classes reached BDA — read two keys the sync does not emit, so `idp-cli config-sync-bda` printed a list of `unknown` under "Classes synced" while the counts beside it stayed correct ([#1208](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1208)). A schema property the transform drops was filed against whichever class a shared attribute happened to name, and because the collection then filtered on that label the warning arrived nowhere: the class came back `success` with no warnings and a section missing ([#1193](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1193)). A blueprint the sync could not delete in replace mode was left out of the project, invisible, still counted against the account's blueprint limit, and reported only to a log line — it now reaches `ConfigSyncBdaResult.orphaned_blueprint_arns` and the CLI, on the failure paths as well as the successful ones ([#1194](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1194)). And a request for a project's `DEVELOPMENT` blueprint list answered from `LIVE`, since the argument was accepted and discarded ([#1126](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1126)).

- **Resolving the BDA pattern's system defaults no longer fails outright** ([#1203](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1203)). `load_system_defaults("pattern-1")` raised `FileNotFoundError` naming a defaults module deleted when v0.6 split assessment into confidence and geometry modules. The SDK's `config download --format minimal` and `validate_config` propagated that, but the worse symptom was quieter: both upload paths catch it and fall back to storing the configuration **unmerged**, with a warning in the log and a successful result to the caller, so a BDA-flavoured configuration was stored without its defaults and nothing said so where anyone would see it. The BDA pattern now resolves to the same confidence, geometry and human-review defaults as the pipeline pattern. `docs/configuration.md` also now states what actually selects the pattern files.

- **A BDA project ARN can no longer reach the Bedrock Data Automation APIs as `None`, and `bdaSyncStatus` is set again** ([#1085](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1085), [#1130](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1130), [#1109](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1109)). An optional ARN flowed into nine parameters declared as strings; every read now goes through one accessor that raises naming the class and the missing ARN, and the optimizer reports that as a failed optimization rather than surfacing `TypeError: 'NoneType' object is not iterable` two frames later. Separately the configuration updater called `set_bda_project_arn` without the ARN it requires, so `bdaSyncStatus` was **never** set to `needs-sync` and the resulting `TypeError` surfaced only as a warning log; it now calls `set_bda_sync_status`. `scripts/discover_model_limits.py` also called a `call_model` method that has never existed on `BedrockClient`, so every run died and reported the `AttributeError` to the operator as "Check AWS credentials and permissions".

- **A document delete that could not clear its tracking-list row no longer reports success** ([#1238](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1238)). A faulted shard query became an empty result and then "nothing was deleted", which is recorded without an error — so a throttled query was reported to the web UI, the SDK and the CLI as a completed delete while the row stayed in the document list. Faults in list-entry cleanup and run-record deletion now reach the caller as `success=False` with the error code, and the UI's multi-select no longer reads a partial failure as a complete delete. Three changes keep the stricter reporting usable: every DynamoDB and S3 call on the delete path retries a transient fault under a five-second budget, a fault on the exact-key delete no longer skips the two fallback strategies, and the tracking record is **kept** when list-entry cleanup fails because its `QueuedTime` is the only route to the orphaned row. ⚠️ That last one means a document whose delete failed stays in the list while its input file and outputs are already gone; re-running the delete is what clears it.

- **The monitoring timeline and the web UI's execution viewer now name the failure that ended a workflow, not the first one it survived** ([#1185](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1185)). The pipeline retries throttles and timeouts in many places, so a failed execution routinely carries an earlier failure it recovered from, and both readers picked the earliest — naming a state that succeeded and an error nobody needs to act on. They now take the state from the last task-level failure and the error text from the terminal event, with the failure vocabulary derived from the Step Functions service model (sixteen event types where the widest private copy knew six) rather than hand-listed. ⚠️ **Two widenings are visible on executions that succeeded.** A step whose task failed and then succeeded on retry previously read as failed — a red step with its error panel open on a document that processed cleanly — and now clears on the state's own later success event. And a failure routed through a `Catch` onto a recovering path now reads **failed** for ten event types that previously read as still in progress, or, where the state also exited, as succeeded. See [Naming the state that failed](lib/idp_common_pkg/idp_common/README.md#-naming-the-state-that-failed).

- **The error analyzer can now answer which state and which Lambda function failed, where it previously could answer neither** ([#1061](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1061), [#1081](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1081), [#1171](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1171), [#1139](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1139), [#1127](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1127)). Asked which state a document's workflow failed in, it reported `at state 'None'` and then recommended checking the state it had just failed to identify — the history was walked in the wrong direction, matched against an event type the API never emits, and reported the *first* failure rather than the terminal one. The function name was worse: it was read from two fields that none of the five relevant event types carries, so `failed_functions` was **always empty in production** while that value is what selects the CloudWatch log group the agent goes on to search. The name now comes from the invoked function's scheduling event, reached by following `previousEventId`, covering both Lambda integration styles and three invoke-time failure types that were never read — including the one a throttle at invoke time produces. The walk also pages backwards far enough to reach the failing state's entry on a long execution, where a single 100-event page routinely did not. ⚠️ Attribution still infers causality from adjacency inside a concurrent `Map`, so the state named can belong to a sibling iteration. ⚠️ And a `lambda:InvokeFunction` denial names no function even now, because the scheduling event is never emitted; it is reported under the enclosing **state's** name in new `failed_states` and `primary_failed_state` fields ([#1183](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1183)). A layer ARN, an event-source-mapping id or a code-signing-config id is never reported as the failing function ([#1128](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1128)), and a malformed event field no longer discards an entire document investigation as "document not found".

- **A throttle spelled `Throttling` is now recognised as transient** ([#1132](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1132)). `is_transient_error` matched throttling by the single name `ThrottlingException`, and several AWS services answer with the legacy spelling instead — measured as 345 of 345 throttles under concurrent CloudFormation reads — so a genuine throttle was classified **permanent** in both directions at once: it recorded a permanent per-section diagnosis and lost its retry across all eight retrying task states. The vocabulary is now read from botocore's own standard retry mode at import time, which brought in eight missing spellings. `LimitExceededException` is deliberately excluded, since it is a rate limit on some services and a full quota on others. Nothing is removed and no ladder changes, so the effect is strictly that failures which previously ended a document on their first attempt are now retried.

- **A document whose last section has been reviewed no longer falls back to *In Progress*, and no longer keeps listing sections to review** ([#1214](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1214)). The shared document writer emitted `HITLSectionsPending` only when the list was non-empty, so the one transition that matters — the last section being reviewed — wrote nothing and left the previous value in place. The document was reported complete while still carrying the sections just reviewed, and re-confirming one of them wrote *In Progress* over *Completed*, leaving a row that said the review was in progress, that it was complete, and that the document was out of the queue. The **Process Changes** path had the same defect. One side effect worth knowing: reprocessing a document that is mid-review now also resets its completed-sections record.

- **The web UI's agent chat no longer streams the model's reasoning to the browser, or streams the answer from the middle, when a `<thinking>` block spans two chunks** ([#1269](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1269)). Displayable text was decided by re-running a regex over the whole accumulated buffer, so with no closing tag yet the block matched nothing and the reasoning was sent as a delta the UI appends and cannot take back; and because the "already sent" marker was a character count into that buffer, the arrival of the closing tag made the cleaned text shorter and suppressed the answer that followed. Measured over all 631 one-, two- and three-chunk splits of one block-bearing response, 564 leaked reasoning and 609 streamed something other than the answer; 631 of 631 are now correct. The **durable** surface was affected too: a stream ending without a `result` event persisted the leaked text to the chat history table. Text is now released only once it is known to sit outside a block. The same defect in `idp-cli chat` is fixed under [#1234](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1234).

- ⚠️ **Six `idp-cli` options were accepted, validated and then never read, so the command did something other than what was asked for** ([#1233](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1233)). One is now honoured and five are refused, each decided on its own. **`--config-revision` is honoured on the `--test-set` path of `process`/`run-inference`**: a run was previously recorded, and later compared, as if a revision were pinned when it had used whatever the profile then held, which let a comparison attribute a difference to a configuration change that never applied. **`--config` on `process`/`run-inference` is refused**, because honouring it is not available — the refusal names the two-step form that works (`config-upload` the file as a profile, then `--config-profile`). **Four `discover` combinations are refused** before any Bedrock call: `--auto-detect` with `-g`/`--class-hint` or with `--page-range`, `--detect-only` without `--auto-detect`, and more `--page-label` than `--page-range`. ⚠️ **Five of the six now exit non-zero where they previously exited 0**; each refusal prints the corrected invocation. Nothing in this repository passed any of them. See [the CLI reference](docs/idp-cli.md#discover).

- ⚠️ **Seven `idp-cli` comma-separated options read a blank segment as a real value** ([#1235](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1235)). `"".split(",")` is `[""]`, so an `if not values` guard after a bare split is unreachable and a trailing comma counts as another value. `--document-ids ","` announced "Selected 2 document(s) for deletion", attempted a `DeleteObject` with a zero-length key twice, printed "Deleted 0/2" and **exited 0**, which an automated cleanup step cannot tell from a clean deletion; `--file-types ","` produced an empty output directory reported as a successful download; `--test-run-ids ""` asked the service to abort a run whose id is the empty string, and a trailing comma made `test-compare` render a blank column of `N/A` as a successful comparison. All seven now share one parser that drops blanks, so a stray comma is harmless and a value with no non-blank segment is **refused with exit 1** naming the option. ⚠️ That is a new non-zero exit for a value that previously ran and did the wrong thing quietly; nothing in this repository passes one. A `delete-documents` run in which every deletion failed still prints "Deleted 0/2" and exits 0, which is left as its own change.

- **`idp-cli status` reported an aborted batch as ALL COMPLETED and exited 0, and showed every document under "Queued" on a stack with a preprocessing hook** ([#1235](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1235)). The progress display bucketed documents with an `if/elif` chain naming eleven `DocumentState` members and sent the other twelve to `queued` by falling off the end. `ABORTED` and `REDACTED_SUPERSEDED` are terminal, so a finished batch containing one counted zero failures and a single aborted document reported "IN PROGRESS" forever, so `status --wait` never terminated. And `PREPROCESSING` is set for every document whenever a hook is registered, so on a stack with PII anonymization the running count read 0 for the whole run. The four buckets are now one declared partition of `DocumentState` that the SDK's monitor and the CLI both read, with no fallback branch in either.

- **Six more `idp-cli` reporting defects** ([#1235](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1235)). `delete` without `--wait` said "✓ Stack deleted successfully!" while CloudFormation was still deleting, and the branch naming the console path and the `--force --wait` command to follow was unreachable. `status` crashed with `'<' not supported between instances of 'datetime.datetime' and 'str'` for a batch where one completed document has an end time and another does not, taking `--monitor` down with it. `search-tracking-table`'s "Documents with metering" over-reported and was order-dependent, and a metering entry stored as a bare number silently dropped every later stage for that document. `test-compare` printed "No configuration differences to display" for every comparison, including two runs on different models, because the config list was a literal `[]` with a `TODO`. And four messages were losing the bracketed text that was the point of them, because Rich reads `[...]` as a style tag and drops what it cannot resolve — `chat` told the user to run an install command that fixes nothing, omitting the `[agents]` extra that was the missing piece, and the test-set overwrite prompt lost its `[y/N]`.

- **The `generate-manifest` test-set path no longer produces a test set that scores nothing while reporting success** ([#1231](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1231)). Six defects, all of which exited 0. A failed upload left behind the `.uploading` marker whose *removal* is what lets the backend see a finished test set, making a complete set permanently invisible with no way to recover but deleting the S3 object by hand; the marker's lifetime is now a context manager. `--file-pattern` and `--baseline-dir` matched case-sensitively, so a corpus of `.PDF` files produced an empty manifest, and an uppercase corpus that *was* selected matched no baselines and produced a test set with a complete `input/`, unreferenced baseline objects and `Matched 0/2` as the only sign — both now fold case, with a new `--case-sensitive` flag for a pattern meant literally. A matched baseline directory reported `Uploaded baseline` whether or not it uploaded anything. A test set over 1000 documents was read and cleared 1000 keys at a time, so **a run over a larger set was reported complete while documents were still processing** and an overwrite left the remainder orphaned under a prefix reported as emptied, mixing one test set's inputs into another's — re-check any result you took from a test set of over 1000 documents. And the overwrite confirmation was inside a `try/except Exception`, so a non-interactive stdin raised `EOFError`, was swallowed, and the overwrite proceeded: **if you refreshed a test set that way, its baselines were deleted and are not recoverable from the CLI.** `--force` (`-y`) is the non-interactive route. See [generate-manifest](docs/idp-cli.md#generate-manifest).

- **`--parameters` no longer discards what you typed without saying so, in either CLI** ([#1220](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1220)). `idp-cli deploy` and `idp-feature-cli deploy-pack` each carried the same inline parsing, and three shapes were mis-read by both with no error — so the deploy succeeded, every affected parameter kept its publish-time default, and CloudFormation cannot afterwards distinguish a parameter never submitted from one submitted at its default. A space before the `=` made the entire flag ignored; an underscore in the key truncated it, so `Log_Level=DEBUG` was submitted as `Level=DEBUG`; and an `=` inside a value split the pair, which reached the documented federation options — an identity provider serving metadata from a query string lost everything after the `?` and had two parameters invented from it. All three now parse as written, whitespace around the `=` is tolerated, and a value that looks like it swallowed a pair is named.

- **`idp-cli reprocess` now runs** ([#1229](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1229)). The command forwarded its arguments to a name the `@cli.command` decorator binds to a `click.Command` object rather than to the function beneath it, so every invocation raised `TypeError` before any of its own code ran — including its refusal of an invocation naming neither `--document-ids` nor `--batch-id`. Nothing was reprocessed and the operator got a traceback. `reprocess` is the spelling `--help` and [the CLI reference](docs/idp-cli.md#reprocess--rerun-inference) point at; the working spelling until now was the deprecated alias `rerun-inference`.

- ⚠️ **`idp-cli deploy --govcloud` refuses a build that produced no GovCloud template, instead of deploying the commercial one** ([#1233](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1233)). With `--from-code` the GovCloud variant was returned only if the build had emitted it, and there was no other branch — so a build reporting success without writing it handed back the commercial template, printed the ordinary "Build complete" line and deployed it, and the first sign of the ignored flag would be a stack creation failing minutes later on a CloudFront resource. This is a fail-safe rather than the repair of an observed failure: this repository's publish path fails a `--govcloud` build outright, so no wrong-partition deployment can have come from here. ⚠️ **It changes what a deploy does** on that one input: exit 1 and nothing created, where a commercial-partition stack was previously created. `--headless` keeps its existing fallback to the full template. See [the `--govcloud` deploy option](docs/idp-cli.md#deploy).

- **`idp-cli discover` derives the filename it writes each schema under, and keeps the write inside `-o`** ([#1232](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1232)). In directory-output mode the filename was the schema's `$id` interpolated whole — a label generated by the model from the document's content, not a path, and not one the operator authored. It is now reduced through this repository's canonical class-id rule and the resolved path checked to be a direct child of the directory `-o` named. The visible change is that a class id outside that character set is written under a rewritten name (`Bank Statement` becomes `Bank-Statement.json`), with a line naming the substitution, matching the name the same class id already gets when discovery saves it to a configuration profile. See [the `discover` command reference](docs/idp-cli.md#discover).

- **`idp-cli chat` no longer shows an agent's internal reasoning in place of its answer when a reasoning block is split across two streamed chunks** ([#1234](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1234)). A chunk ending inside a `<thinking>` block carries no closing tag, so the regex matched nothing and the block was printed — the one category of text the command exists to withhold — and because the cursor was a character count into the same buffer, the answer that followed was then either suppressed or emitted from its middle. Which of the two a turn produced depended on where the service put the chunk boundary, and the quieter one is the more dangerous: an answer printed from its middle still looks like an answer. Text is now released only once it is known to sit outside a block, so the visible output only ever grows.

- **`idp-cli`'s configuration commands honour `--region`** ([#1039](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1039)). Ten of them — `config-upload`, `config-download`, `config-activate`, `config-list`, `config-revisions`, `config-delete`, `config-sync-bda` and `bootstrap` among them — resolved the configuration table's name in the region asked for and then read or wrote it in whatever region the ambient credentials resolved to. Against a nonexistent table that is a `ResourceNotFoundException`; on an account holding same-named tables in more than one region it was a successful write to **another stack's** configuration, reported as success. `--region` now takes precedence, and a command with no region resolvable anywhere fails saying so. See [Region and its precedence](docs/idp-cli.md#region-and-its-precedence).

- **`idp-cli stop-workflows` no longer crashes when a Step Functions execution cannot be stopped.** The handler named an exception that is not in the service model (`ExecutionNotFound`; the real one is `ExecutionDoesNotExist`), and an `except` clause evaluates its expression at raise time, so any `stop_execution` failure raised `AttributeError` from inside the handler and propagated past the `except Exception` meant to count it. The failure is now counted and reported, and an execution that is already gone counts as stopped.

- **`client.search.query()`, `client.evaluation.get_report()` and `client.evaluation.get_metrics()` return a result instead of raising, and `client.document.list()` reports a real count.** Five `idp_sdk` operations built their result objects with keyword arguments the models did not declare, so each raised `TypeError` on every input — reported to the caller as a plausible "Failed to …" processing error, including the `client.search.query(...)` example in the client's own docstring. `document.list()` was the silent case, returning `count` as `None` and losing each document's batch id. The evaluation report and metrics also read an S3 key the pipeline does not write and now read the artifact it does, so `get_report()` raises `IDPResourceNotFoundError` for a document that has not been evaluated against a baseline. See [the SDK reference](docs/idp-sdk.md).

- **Every public `idp_sdk` result model can now be imported from `idp_sdk`.** Eighteen of them — including `BaselineInfo` and `FieldComparison`, which `evaluation.list_baselines()` and `evaluation.get_report()` now return — were public in `idp_sdk.models` but never re-exported, so the import block the [SDK reference](docs/idp-sdk.md) tells you to copy raised `ImportError` on 13 of its names.

- **`client.config.validate()` answers in an installation that cannot run the checks, instead of raising out of an import** ([#1252](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1252)). The method's whole contract is to return a `ConfigValidationResult`, the checking is done by `idp_common`, and an environment assembled by other means than a resolved install can be missing part of it — a Lambda package here is a handler tree plus a copy of the library pruned to fit the deployment limit. The guard covered the file load alone, so an `ImportError` from the validation stack escaped the method. Both frames are now guarded, `validate_config` no longer describes an `ImportError` as a finding about the document, and the result carries a new `validation_available` field so a caller can tell a configuration that was checked and found wrong from an installation that cannot check one. `valid` stays False, so anything gating on it keeps refusing.

- ⚠️ **Chat-with-Document config-version scope: enforced on the REST path, and now honestly reported as unenforced on the streaming one** ([#970](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/970), [#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920)). The lookup queried a UsersTable index no template declares, so every query failed and the result was read as "unrestricted" — `allowedConfigVersions` had never constrained a chat turn. It now resolves the caller through the immutable Cognito `sub`, falling back to the email join, and any failure to evaluate the scope denies the turn. No table change is required. **On commercial deployments this does not yet restrict chat**, because the browser streams from a Lambda Function URL that forwards no verified per-user caller; the processor now logs once per turn that it did not enforce the scope rather than appearing to have checked. Enforcement applies to turns reaching the backend through the REST API, which is the path GovCloud deployments take. See [RBAC](docs/rbac.md#known-limitations).

- ⚠️ **A scoped user whose email address changes keeps their configuration-profile restriction** ([#1038](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1038)). `allowedConfigVersions` lives on a user record that could previously only be found by email, and a user with no record is unrestricted by design — so an address that stopped matching its record silently lifted that user's restriction, which an external IdP re-applying its attribute mapping, a self-service address change, or a difference in letter case could each cause. The record is now reached first through a `SUB#<sub>` pointer keyed on the immutable Cognito `sub`, and a caller presenting neither identifier is denied. **No table change and no migration is required.** After upgrading a deployment that uses an external IdP, open **User Management** once as an Admin to back-fill the `sub`; a record whose address had already changed beyond letter case is not reachable that way and keeps the email-only join. See [Resolving whose scope](docs/rbac.md#resolving-whose-scope--two-keys-and-the-lookup-fails-closed).

- **The Redaction Report no longer reveals which documents have a redaction record to a caller who may not read them, and a user scoped to a pattern can use the web UI again** ([#1038](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1038)). The report's two single-document routes resolved the record before the caller's scope, so an absent document answered "not found" and a present one "access denied" — one bit of the audit table's contents per request, to a caller entitled to neither. Separately, scope entries may be patterns such as `tenant-a_*` and the backend has always honoured them, but the UI filtered profiles by exact name, so a pattern-scoped user saw an empty Configuration Profile list and a permanently disabled Reprocess button. ⚠️ Patterns still have to be **set** through the API or the CLI — the Admin scope picker offers only existing profile names.

- **A circuit breaker that cannot read its own state no longer switches itself off** (one item of [#934](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/934)). The Queue Processor treated any DynamoDB error on the state read as "allow", so a ConcurrencyTable fault disabled the breaker rather than the traffic. A failed read is now judged by whether a retry can fix it: a transient fault refuses admission, since the message returns to `DocumentQueue` and its redrive budget absorbs about eight hours of retries; a terminal fault (access denied, table missing) still admits work, because it cannot be retried away, and is reported through a new `CircuitBreakerCheckFailed` metric whose `Classification` dimension distinguishes the two. Reachable only with `CircuitBreakerEnabled=true`. See [Circuit breaker](docs/circuit-breaker.md#when-the-state-cannot-be-read).

- **A pipeline hook registered where the active processing mode cannot reach it is no longer accepted in silence** ([#982](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/982)). `postOcr`, `postClassification` and `postExtraction` are states on the Pipeline branch only, so under `use_bda: true` a hook registered at one of them never ran and its `onError: fail` policy never gated — with the config record, the UI and an empty execution history all consistent with a working hook. A PII-redaction gate registered that way let the document complete un-redacted with no signal. Both install paths and the configuration write paths now **refuse** an `onError: fail` hook at an unreachable point and accept an advisory one with a warning, and the `preprocessing` dispatch audits the whole hook set per document, recording what the chosen branch will skip. ⚠️ **A feature stack that registers such a hook now fails to install while `use_bda: true`** — the hook was never running, so nothing that worked stops working, but disable it, move it to `preprocessing`, downgrade its `onError` or run the pipeline mode before re-installing. See [Pipeline hooks](docs/feature-platform.md#not-every-hook-point-exists-in-every-processing-mode).

- **The feature platform can now be switched on for a stack that was created without it** ([#845](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/845)). Setting `EnableFeaturePlatform` from `false` to `true` failed every time with `Export with name <StackName>-TrackingTableName is already exported`, because the main template and the nested platform stack each declared that export under opposite conditions and one update cannot hand an export name between stacks. The nested stack is now its only producer and the main template keeps a plain output. Stacks that already have the platform on upgrade with no extra step. ⚠️ Adopting the platform on a stack older than this release takes **two** updates: upgrade first with the parameter `false`, then set it to `true`. ⚠️ On a **platform-off** stack the upgrade withdraws the main stack's `<StackName>-TrackingTableName` export, so check `aws cloudformation list-imports --export-name <StackName>-TrackingTableName` first and read the stack's `TrackingTableName` output instead.

- ⚠️ **`EnableXRayTracing` now controls X-Ray tracing on the Lambda functions, not only on the two state machines** ([#983](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/983), [#1037](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1037)). Nineteen functions hardcoded `Tracing: Active`, so setting the parameter to `false` left the X-Ray bill unchanged; and the nested feature-platform stack's nine Lambdas both ignored the parameter and shared an explicit role with no `xray:PutTraceSegments` grant, so tracing was declared and **no segment was ever written**. Every traced function now resolves its mode through the parameter, and four functions that annotated X-Ray segments without declaring a mode now declare one. ⚠️ **Upgrade-visible.** The default is unchanged at `true`, so a default deployment traces what it traced before **plus** those functions — expect a small increase. `false` now reduces X-Ray cost across the main, pattern and feature-platform stacks, **but not to zero if you have an extension installed from the Extensions catalog**: those stacks are deployed independently and set `Tracing: Active` in their own `Globals`. See [Monitoring](docs/monitoring.md#x-ray-tracing).

- ⚠️ **A published test set version holds the labels it was published with, so what a version refers to can no longer change underneath a run scored against it** ([#1035](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1035)). Publishing wrote a version row and copied nothing: the labels a version stood for were copied later, whenever annotation next opened a draft. Publishing v1, regenerating draft labels and then annotating therefore left v1 standing for labels that had never been v1's, and a run pinned to v1 scored against them with nothing warning. ⚠️ **Versions published before 0.6.10 may already hold the wrong labels and cannot be repaired** — the run form marks a version with no stored labels, and a run pinned to one scores the set's current labels. ⚠️ Publishing a large set now takes a moment, and a set with more than 3000 baseline objects is **refused** rather than recorded as a version no stored bytes can back; retrying a publish that reported an error is safe. Separately the Schema Builder keeps a `$defs` definition and its references intact across a save, where a non-object definition (`{"type": "string", "enum": [...]}`) was previously written back as an empty object the first time someone opened that class and saved. See [Publishing a version](docs/test-studio.md#publishing-a-version).

- **The Schema Builder writes one shape for an attribute that references a shared class** ([#957](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/957)). The Add Attribute dialog produced a bare `{"$ref": ...}` while the inspector's picker and the importer kept a sibling `type: "object"` beside it, so the same user-visible action left the designer holding two different nodes — which is why the empty Evaluation Method dropdown of [#906](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/906) looked intermittent. All three routes now write the bare form every shipped configuration already used, so saved schemas are unchanged, and the picker no longer leaves an attribute's inline `properties` and `required` underneath the new reference.

- **A failed document read no longer renders a blank page, and permission errors no longer tell you to try again** ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)). Opening a document the API refused left the detail view empty with nothing logged, and a knowledge-base query that failed left its answer showing `...` indefinitely; both now report what happened. The document list showed the server's raw message and the file viewers advised retrying, which can never resolve an authorization failure, and a server-side S3 or KMS denial is now reported as a server error rather than as a missing role.

- **Agent chat: best-effort memory and monitoring can no longer abort a chat turn** ([#1099](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1099)). Three hook callbacks did risky work outside the `try` meant to contain them, so a failure in something whose whole purpose is to be discardable — a turn of history, a throttling record, a debug log line — ended the user's turn. None has a demonstrated trigger in the shipped system, so this is hardening of a contract the rest of the module keeps.

- **An agent conversation-memory limit of zero loaded every stored turn instead of none.** `DynamoDBMemoryHookProvider` truncated history with `turns[-max_history_turns:]` behind a `len(turns) > max_history_turns` test, which at zero is true for any non-empty conversation and admits the one value the slice cannot express, so `MAX_CONVERSATION_TURNS=0` — the obvious way to turn conversation memory off — loaded the whole session into the agent's context. The shipped templates set 20, so this needed reconfiguration to reach.

- **Headless and GovCloud template transforms now report the IAM policy statements they delete** ([#981](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/981)). Both transforms drop IAM and S3 bucket policy statements as well as whole resources, and did so with no statement-level record — and because a policy with fewer statements is still a valid policy, CloudFormation deploys the result happily and an over-deletion only surfaces later as a runtime access-denied in the partition the template was deployed to. Each removal is now logged naming the resource, the count and what matched, with an explicit zero when nothing was dropped, and SDK callers can read the same records from `policy_statement_removals`. See [GovCloud architecture](docs/govcloud-architecture.md#auditing-the-iam-statements-a-transform-removed).

- **Importing `idp_common` no longer requires an ambient AWS region** ([#988](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/988)). `idp_common.utils.settings_helper` built an SSM client while being imported, and `idp_common.utils` is imported by `idp_common.s3` and thence by most of this repository's Lambdas, so importing the shared library at all needed a resolvable region and any consumer without one failed before running a line of its own code. The client is now created on first use and memoized. Nothing in production was affected, since the Lambda runtime always sets `AWS_REGION`.

- **Documentation:** the threat model records AUTH.T07 (config-version scope bypass) as *Partially Mitigated* rather than *Mitigated*, with the four residuals that keep it there named on the entry, and PII.T04's record-existence residual closed. Threat counts are reconciled across the corpus against the generated export.

- **Five shipped `ocr-benchmark` presets no longer carry a `criteria_validation:` block nothing reads** ([#1145](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1145)). Thirteen keys — a model, a semaphore, two prompts, chunking sizes and inference parameters — for a service whose module no longer exists, discarded on load: a reader adjusting `semaphore: 3` there to bound rule-validation concurrency was editing a value with no effect, and the prompt text was a second copy free to drift from the one the pipeline uses. The block is deleted rather than renamed, because several of its keys live on nested objects under `rule_validation` where a bare rename would move them from ignored-with-a-warning to ignored in silence.

## Templates
   - us-west-2: `https://s3.us-west-2.amazonaws.com/aws-ml-blog-us-west-2/artifacts/genai-idp/idp-main_0.6.10.yaml`
   - us-east-1: `https://s3.us-east-1.amazonaws.com/aws-ml-blog-us-east-1/artifacts/genai-idp/idp-main_0.6.10.yaml`
   - eu-central-1: `https://s3.eu-central-1.amazonaws.com/aws-ml-blog-eu-central-1/artifacts/genai-idp/idp-main_0.6.10.yaml`
