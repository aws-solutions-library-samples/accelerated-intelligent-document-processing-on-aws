...
- **A section of more than 20 pages no longer fails on Bedrock's many-image pixel cap, and when an image-shaped rejection does happen it is no longer reported as a context-window overflow** ([#994](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/994)). Amazon Bedrock caps each image at 8,000 px per side, but at **2,000 px** once a single request carries more than 20 image blocks — counting `document` blocks and images returned inside a `toolResult`. Because the limit binds on the request's image *count*, no per-image guard could see it: every page was individually well inside 8,000 px and the request failed as a whole with `image exceed max allowed size for many-image requests: 2000 pixels`. Measured on 29 stored pages against `us.anthropic.claude-sonnet-5` in us-west-2: 2,001 px rejected, 2,000 px accepted at 117,035 input tokens, and the same pages accepted at 2,150 px in a 5-image request. `BedrockClient.invoke_model` now sweeps every assembled request with `image.fit_images_in_request` immediately before `converse` — the only point that sees a whole request, so classification, assessment, summarization, evaluation and few-shot examples are covered as well as extraction — counting the blocks, downscaling anything over the applicable cap, and refreshing each block's declared `format` (a lossless image can come back as JPEG). Extraction also clamps at page-load time so the reduction lands in the section's `metadata.image_downscale` and the agentic (Strands) path, which builds its own requests, is covered too; that path counts each page twice when deciding whether 20 will be crossed, because the agent re-sends attached pages every turn and `view_image` can add a second copy of a page. The sweep is best-effort — an image it cannot resize is sent unchanged with a warning rather than failing a request Bedrock might accept — and emits one aggregate warning per request instead of one per image. Separately, the misdiagnosis is fixed at the source: `bedrock_utils.is_image_request_rejection` now matches image dimension/byte rejections and is checked **before** the overflow matcher, extraction raises a distinct `ExtractionImageRejected` naming the image count, the largest dimension and the one remedy that applies (`extraction.image.target_width` / `target_height`), and the token estimate that made the request look oversized is corrected: images are now priced with Claude's 28 px-patch math capped at the model's resolution tier (1,568 tokens before Claude 4.7, 4,784 after), where the old uncapped `(w*h)/750` figure over-stated a 2550×3301 page 2.4× — 11,223 against a measured 4,761. Discovery's hardcoded 2,048 px image cap, 48 px over the limit for the many-page sample documents it sends, is corrected to 2,000. The clamp costs roughly 15% of the image tokens, because Converse downscales to about this size before tokenizing in any case; to avoid the re-encode, set `image.target_width` / `target_height` to 2,000 or less. See [Configuration](docs/configuration.md#a-request-with-more-than-20-page-images-caps-every-image-at-2000-px) and [Extraction and Confidence](docs/extraction-and-confidence.md#oversize-page-images-metadataimage_downscale).
