...
- **Annotating a test set now creates a version of it, explicitly, and preserves the labels it moves away from.** Starting annotation on a set, even one that already has ground truth, commits to a new version of it, and the queue link should identify that transition. Framed as a link problem; the defect underneath was larger. `publish_test_set_version` wrote a **DynamoDB row and copied no S3 objects**, while annotation wrote straight to `{id}/baseline/` — so a version number was immutable and its *content* was not, and a run stamped `TestSetVersion = 3` could not be reproduced against the labels it actually scored.

  **Start annotating** opens a version transition, shown as `v1 → v2`, and the editor stays read-only until it is open. That ordering is the point: editing first and versioning after would record what the labels became rather than what they were. Agreeing snapshots the current baselines to `{id}/versions/{n}/baseline/`, so the number refers to bytes; a set that arrived with ground truth and was never published has that state captured first as **As uploaded**, since otherwise the labels a set was uploaded with are exactly the ones overwritten with no record. A run scores the set's **current labels** by default, including annotation in progress — the correct-then-run loop the review-effort panel invites — and records the open draft it scored under. Pinning a published version is explicit, via a **Test set version** picker on the run form beside the configuration-revision one; a pinned run stages *that version's* snapshot, and every historical run and set without one keeps reading the live baselines. **Copy queue link** carries `?v=`, and a link from a transition that has since been published says so rather than silently enrolling you in a different one.

  Two constraints shaped it. The editor saves through a **presigned POST straight from the browser**, so no Lambda observes an annotation write and copy-on-write has nothing to hang on — hence an explicit action, which is what was being asked for. And the transition is *read* from `getAnnotationQueue` rather than probed with the mutation: probing would open a transition merely by visiting the page, the silent commitment the whole change removes. The snapshot paginates, because `list_objects_v2` caps at 1000 and a 2000-document set exists.

- **A page can be opened at full size from the page-regrouping board.** "The thumbnail isn't quite large enough to see the detail you might need to see" — and deciding which section a page belongs to means reading it. Free: pages already render at 1200px wide and the card showed them at 112px, so the preview displays detail already in memory rather than fetching anything. A menu item rather than a click on the card, so it neither competes with the drag gesture nor becomes mouse-only.

- **A section that holds several documents of the same class can now extract all of them — `x-aws-idp-multi-instance` ([#715](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/715)).** Set the flag on a class and its *effective* schema becomes a list of that class: `inference_result` holds `{"instances": [ … ]}`, one entry per document found in the section, while you keep authoring the class as **one record**. It is a schema *transform*, not an output envelope — the wrapper is declared in the class schema, so `inference_result` stays a valid instance of its own declared schema and the prompt, the generated Pydantic model, the JSON-Schema validator, the off-schema filter, assessment's list branch and Stickler's Hungarian row matching all work unchanged. Opt-in per class and never auto-detected: auto-detection would make every single-document section pay for an extra nesting level. Reachable from **Configuration → Document Schema** as a single **Documents per section** control — one question with three answers (*one document* / *several, this class already lists them* / *several, wrap my single-record class*) rather than two separate near-synonymous toggles that each disabled the other. The record-array picker appears only in the middle branch; the shape preview (`instances[ ] → <Class> → { your fields }`) and the baseline-migration warning only in the last. The preview is the real defence against setting this on a class that is already a list of records — a validation message fires after you have saved. Invalid combinations now also **block the save** rather than being flagged and then accepted: the schema editor gained a client-side preflight mirroring the backend's class-level rules, because Ajv only checks standard JSON-Schema keywords and knew nothing about `x-aws-idp-*`. Configuration validation rejects the flag alongside `x-aws-idp-instance-array` (they answer opposite questions) and rejects it on a class that already declares a top-level `instances` property; it only **warns** when the class's top level is nothing but one record array, because an internal array is not evidence of anything — an invoice with `line_items[]` is a single-instance document and the flag correctly gives `instances[i].line_items[j]`. Downstream: confidence keys become `instances[i].Field`, reporting writes **one Athena row per document** with a new `record_index` column (existing column names unchanged, so existing queries keep working, but `(document_id, section_id)` is no longer unique), Z3 rules address a record explicitly (`…inference_result.instances[0].NetPay`), and the public SDK gains an additive `instances` list. ⚠️ The SDK's `confidence` map is **not** purely additive: it now walks groups and lists, so it gains dotted/indexed keys (`Group.Sub`, `List[0].Sub`) for **every** class — list attributes previously contributed nothing at all — and `get_results` emits `None` leaves where it previously emitted nothing. ⚠️ **Evaluation baselines must be migrated** — see Changed. ⚠️ Three things also DEGRADE for a flagged class, all documented: BLANK-vs-MISSING field handling and the empty-declared-list warning walk only a class's top level and so stop applying to the record's own fields; the confidence prompt loses sub-field descriptions for a group or list nested inside a record; and the evaluation report's per-attribute breakdown becomes one attribute (`instances`) carrying every field's rows rather than one per field (the rows are all still there and still drillable). Few-shot example prompts are also not rewritten — a flat example now contradicts the requested shape, and configuration validation warns about it. Not applicable in BDA mode. See [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md).

- **Designate mode is now editable in the Schema Builder.** `x-aws-idp-instance-array` shipped in 0.6.6 and was preserved by the schema editor's round-trip, but had no editor of its own — so the documented zero-cost answer to multi-record packets was only reachable by pasting JSON into a class or hand-editing YAML. It is now the middle branch of the **Documents per section** control above, offering a **select over the class's own array-of-object properties** rather than a text box, because configuration validation hard-rejects any other value and a typo would be accepted in the editor and then fail config load. A value set outside the UI that no longer names an array-of-object is flagged, never silently dropped. The picker's validity check is the same predicate that blocks the save, so it cannot report a designation as invalid that the pipeline would in fact accept — including the `{"$ref": "#/$defs/…"}` form the editor itself emits, which is resolved rather than read as untyped.

- **A wrong packet split can now be corrected by dragging pages between sections, without losing the annotations already on the document.** The case that motivated it: a test set contained a document whose split had grouped pages into the wrong sections, carrying annotations that could not be lost, and the editor showed the grouping read-only (`Page indices … (read-only)`). The grouping *is* ground truth — `split_document.page_indices` is what the doc-split metrics score classification against — so a wrong split makes the classification ground truth wrong, and the only available repair, re-running extraction, would have destroyed the field values being protected.

  **Saving a new grouping writes the grouping and the class and nothing else.** Extracted values, their `reviewed-human` provenance and the edit history all survive. They may no longer match their pages, so the warning afterwards *names* the sections that moved and re-extraction is offered per section — never performed, because that is precisely the loss this exists to prevent.

  The board puts sections side by side rather than stacked: the hard part of a large packet is not scroll length but needing the page you are moving and its destination on screen at once, otherwise the drag is held through a scroll. Multi-select with shift-click extends over *document* order, because a bad split normally misplaces a contiguous run — one mistake should take one action, not five. Every page also carries a **Move to** menu: a drag-only interface is unusable by keyboard or screen reader, and this is not a screen an annotator can route around. An **Expand** button gives the columns roughly twice the height in a modal, rendering the same body so the two views cannot drift.

  A packet split is a **partition**, so the rules are enforced rather than advisory: every page in exactly one section, no empty section (deleting one discards its field values, so it stays deliberate — drag the pages out first), and a page the split had *dropped* may be added, since that is the defect a reviewer is there to fix. Non-contiguous sections are supported because the pipeline can produce them. Sections are renumbered so their ids follow page order: several consumers take a group index from a section's position in a list (`compute_graded_packet_metrics` enumerates them) and nothing otherwise guarantees that list is in page order, so making ids agree with it removes the ambiguity for every consumer at once.

  **A section's pages are ordered, and the order is editable too.** It is ground truth in its own right rather than a display detail: `split_accuracy_with_order` compares the two `page_indices` lists with `==`, and the graded packet score is half a Kendall's Tau over each page's position within its section (`calculate_final_score` weights ordering at 0.5). A packet assembled out of reading order therefore needs that recorded, or the pipeline is measured against the wrong answer. Dropping a page **onto another page** places it immediately before that one, within a section or arriving from another; dropping onto the column's empty space places it where document order says it belongs, so correcting a split does not invent a custom order as a side effect. **Order within this section → Move earlier / Move later** is the route without a pointer. A reordered section is marked **Custom page order** and offers a one-click return to document order, because an out-of-order column otherwise reads as a rendering bug. Implemented by making each page card a drop target as well as a draggable, which yields an insert position within and across columns without `@dnd-kit/sortable` and its container-transfer edge cases.

  **Available on both surfaces, and honest about the one difference.** In Test Studio it writes the baseline via `updateTestSetDocumentSections` (Admin, Author, Annotator; scope-checked per test set). On a processed document it writes the document record via `updateDocumentSections` (Admin, Reviewer, Annotator — matching `completeSectionReview`, the sibling that edits section content). `processChanges` was *not* reused: it clears each changed section's extraction data and requeues the document because it delegates persistence to the pipeline — it never writes the document back — so reprocessing is its persistence mechanism rather than a requirement of re-grouping. A grouping-only edit persists itself, as `complete_section_review` already does for field edits. That introduces one hazard, so the save is refused while the document is `QUEUED` or `RUNNING`, with the arrangement left on screen rather than discarded. `Edit Mode → Process Changes` remains the opt-in reprocess, and its label already said so.

  The **page-numbering conversion is the only arithmetic in the feature** and lives in two tested helpers, because baselines store 0-based `page_indices` while page ids are 1-based — except BDA / Pattern-1, which is 0-based. The shared validation is therefore deliberately base-agnostic: baking a base into shared code would have put an off-by-one where it reached both surfaces at once, and an off-by-one here would shift every section by a page while still writing valid JSON. `usePageThumbnails` was extracted along the way (`ImageUri` is an `s3://` URI needing signing, and this is the second surface needing it), keeping the historical-version pinning that is easy to lose by reimplementing. Both new operations route via `FIELD_ALIASES`, so the `HttpApiFieldFunctionMap` SSM parameter — at its 8 KB ceiling — is unchanged.

  83 tests across the rules (19), the board (22), the test-set mutation and payload (16), the document mutation (13), and the two wirings (7 and 6) — each behaviour revert-checked against a deliberately broken version.

- **The annotation queue now shows what each document was classified as.** The remaining half of the question *"when we get classification wrong, how does that surface"* — the run-level panel above answers it for a scored run, but the queue is where the review work happens and it exposed the class nowhere at all. It is the only column that can reveal a misclassification: a document extracted against the wrong schema is often *confidently* wrong, so its confidence and alert count look entirely normal, and worst-first ordering puts it **last**. Costs nothing — the queue already reads each section's baseline for label state, so the class comes off a file it was opening anyway. A packet reports each distinct class once, in order, rather than one badge per section. **Shown, not scored, and that is a deliberate limit rather than an unfinished feature.** Making a class *mismatch* raise an alert was considered and is not buildable as such: the draft under review is itself the candidate ground truth, so there is nothing to compare against; classification carries no real confidence (every `DocumentClassification` in the pipeline is constructed with a hardcoded `1.0`, `0.0` only on error); and a test set declares no expected class — `documentClassType` is a *kind* enum (`SINGLE_CLASS` / `MULTI_CLASS` / `PACKET_SPLITTING`), not a class name. A signal would have to be created first. So the class is put in front of a human rather than turned into a number, and is kept out of the queue ordering **and** out of the review-effort estimator — whose alert counts and confidence→accuracy curve are both defined in terms of field confidence, so folding a different kind of signal into them would double-count and corrupt the calibration the estimate's honesty rests on. 4 resolver tests, including that the class survives the hop into the queue resolver (a different resolver from the documents page) and that a baseline with no class reports an empty list rather than a blank badge.

- **The evaluation report is a React component instead of a rendered markdown file, leading with the accuracy figures.** It was built as an add-on and still looked like one: a `.md` artifact displayed in a panel with a little embedded HTML for expanding nested fields, so the headline number — how accurate was this document? — had to be read out of prose, and nothing could be sorted, filtered or linked. **No pipeline change and no new field:** the same data has always been written as JSON (`DocumentEvaluationResult.to_dict`), and `results.json` is a sibling of `report.md` under `{input_key}/evaluation/`, so the component derives its own URI from the report URI the document record already carries. It leads with **extraction accuracy** and **classification accuracy as separate figures**, because they fail independently — a document can be classified perfectly and extracted badly, or the reverse, and one number cannot stand for both — plus F1 with precision and recall beneath it. The extraction tile states whether it is the weighted score or the plain match rate, since those are different numbers that were previously both just "accuracy". A figure with no value is **omitted rather than rendered as 0%**: a zero is indistinguishable from "scored nothing correctly", which is the one actively wrong reading available, and a document that could not be scored at all (no section had an extractable schema) says so instead of showing zeros. Below that, one expandable section per document section with the per-field table — expected, extracted, score, and the comparator's own *reason* in a tooltip, which is the most useful thing on the row when a score is surprising and was previously buried in a markdown cell. Sections containing mismatches expand by default; **Mismatches only** hides everything that matched. A wrong class is called out at the top of its section because it makes the field table below it meaningless rather than merely wrong (shared with the classification work above). **Download and print are not lost:** the markdown artifact is still generated and is one click away behind **Markdown report**, which is where those two actions already lived — and a document evaluated by an older build whose JSON cannot be read falls back to the markdown automatically rather than showing an error. The URI derivation is pinned to the full `evaluation/report.md` tail rather than any `.md`, so it cannot silently derive a results path that does not exist. The document summary was left alone deliberately — it was raised as a candidate for the same treatment and explicitly deferred. 16 new tests over the summary model, including that a malformed payload never throws and that an attribute with no `matched` flag counts as a mismatch rather than a pass (treating unknown as passing would overstate accuracy).

- **A browser UX-test skill (`make ux-test`) — the one test layer in this repo that opens a browser.** There is a lot of testing here (`make test`, `api-test`, `stacktest-*`, SRT, ZAP, the benchmark suite) and none of it drives the UI, so everything below the UI can be green while a button does nothing or a critical flow is broken. That is not hypothetical: correcting a document's classification in a test set shipped broken for several versions and was found in the field rather than by our own testing. Each run produces **two** things, deliberately different in kind: functional pass/fail per flow, and a **usability critique** — the half no deterministic test can produce, and the reason a flow that technically works but confuses everyone who tries it now gets reported. Scope is intentionally narrow for the first pass: a **local browser against an already-deployed stack**, invoked by a developer. Explicitly *not* CI integration, remote/headless browser infrastructure, or a new extension — all three were considered and deferred, on the grounds that two or three flows working beats twenty half-specified. `scripts/ux_test_session.py` does setup and teardown *only*, creating a throwaway Cognito user so a UX run never uses a real operator's account, and reusing `rbac_common.py` rather than growing a second implementation of "make a user in this pool"; it deliberately holds no opinions, because a script pretending to assess usability is the thing this layer exists to avoid. `scripts/ux_flows.yaml` carries six starter flows whose ids **match the ground-truth correction QA plan**, so a finding is referenced the same way in both and stories gathered in review extend the file rather than being reported separately in different words; six known-missing flows are listed rather than silently absent. Safety properties are tested since the script touches Cognito: the throwaway address is `@example.invalid` (RFC 2606, so a stray invite cannot be delivered), passwords satisfy the pool policy and never repeat, and the stack's Cognito **app client is deliberately left alone**: setting a known password non-interactively needs only `admin-set-user-password --permanent`, an admin API on the user pool that ignores the client's `ExplicitAuthFlows`, and the browser then signs in over SRP, which the UI client already allows. Widening the client would have bought nothing and left a live stack accepting password-based admin auth for as long as the session ran. Setup prints its own teardown command, because an unrun teardown is the failure mode that leaves a known password behind. Three tests guard the *skill* rather than the code: that it still says to tear down and never to report a flow as passed without exercising it, that it is registered in `CLAUDE.md`, and that `.cline/skills/ux-test.md` is a symlink rather than a copy that would silently diverge. 15 new tests.

- **Classification errors are now visible at the *run* level, and any field is linkable.** The question this answers is *"when we get classification wrong, how does that surface?"* A test run surfaced classification as two aggregate percentages and nothing else, which understates it badly: extraction runs against the schema of whatever class a document was assigned, so a wrong class makes every field for that document unreliable — and it can be *confidently* wrong, meaning the document raises few confidence alerts and therefore sorts **low priority** in the annotation queue's worst-first order. The failure most likely to matter was the one least likely to be seen. **No new computation:** `DocSplitMetrics.section_details_with_order` was already computed for every evaluated document and serialized into its `results.json`, rendered only into the markdown report and then discarded — `test_execution_aggregation_function` loaded each document's `doc_split_metrics` and forwarded only the five numeric graded-packet fields. Now a **Classification errors** panel on the test run, above the breakdown tables because a wrong class invalidates the field numbers below it, listing each affected document with expected vs predicted class and pages, each row deep-linking into the annotation queue where the class is corrected. The three kinds are kept **apart** rather than lumped as "errors" — `class` (wrong schema ran), `unmatched` (a splitting difference, not a labelling one) and `order` (extraction unaffected; it is what split-accuracy-*with-order* penalises) — because they imply different fixes. The list is **capped at 200** (the whole run result is one DynamoDB attribute, and a 500-document run with a wrong class list produces thousands) with wrong-class errors sorted first so page-order noise cannot crowd them out, and the true total reported when it truncates — "Showing the first 200 of 340", never a silent cut. Runs aggregated before this re-aggregate automatically on first view; the Athena fallback path has the percentages but not the per-section detail, and renders the panel empty rather than broken. **The per-document surface is [#652](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/652)'s, not this one's** — that shipped a page-derived comparison across four surfaces while this branch was open, and it is better than what was here: deriving from `page_details` sidesteps section-id pairing entirely, so a document whose *split* is wrong still has a ground-truth class for every page. This branch's own version was deleted in favour of it, and the evaluation report now reports misclassified pages through the same shared index. **Field-level deep links:** clicking a field offers `Copy link to field`, producing a URL that reopens the document with that field selected and scrolled to, for asking a colleague about one value. Navigation only — the recipient's access is still checked on arrival, so sharing one outside a test set's scope grants nothing. Chosen over routing questions to a designated subject-matter expert's own queue, which assumes an organisation structured that way and adds a workflow to maintain.

- **Advanced extraction can stop sending the document schema three times.** It transmits the class schema **three times per request** — as prose in the task prompt, appended to the system prompt, and as the extraction tool's input schema — and the last two are the *same JSON string*, measured at 2,600 of 6,680 schema tokens on the lending `Payslip` class (**38%**). `extraction.agentic.restate_schema_in_system_prompt: false` drops the duplicate; the agent can still fetch the schema on demand. ⚠️ **It defaults to on, and this is an A/B knob rather than a recommendation:** restating a schema in prose often improves how closely a model follows it, so measure completeness on your own documents before turning it off — a token saving that loses list rows is a loss. Note also that all three copies sit inside the prompt cache, so the dollar saving is roughly a tenth of the token count; the real benefit is context-window headroom, which is what pushes a long document into extra shards. Editable in the Web UI under **Configuration → Extraction → Advanced extraction settings** ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710), [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md)).

- **Experimental: send the document schema as a required tool instead of describing it.** Simple extraction asks for JSON in the prompt and parses the reply; `extraction.forced_tool.enabled: true` instead declares your schema as a Bedrock tool the model must call, so a malformed-JSON reply becomes structurally impossible for the fields the schema declares. ⚠️ **Off by default and genuinely unproven** — it constrains the *shape* of the reply, not the accuracy of the values in it, so a model that would have returned a stray key may instead return a worse value that fits. Whether that is a net win is being measured; if it is not, it stays off. Routes that cannot carry a tool schema (a custom Lambda hook, the GPT-5.x path) fall back to the prompt automatically, and a model that accepts the tool but replies in prose anyway — which happens, and is not an error — also falls back; both are recorded under `metadata.forced_tool` so the two cases are distinguishable from "it had no effect". Property names Bedrock rejects are handled transparently, so no extracted field is renamed. Editable in the Web UI under **Configuration → Extraction → Schema Enforcement (experimental)**. See [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md).

- **Configuration Profiles now keep a revision history, so saving a configuration no longer destroys the one it replaced.** Editing a configuration was a one-way overwrite: the only way to keep the previous state was to create a *new* named version (`usecaseA_v1`, `usecaseA_v2`, …) — and `Save as Version` is Admin-only, so a scoped Author could not iterate at all without either an admin or losing their previous work. Every save now records an immutable **revision**, listed per profile in a new **Revision history** panel where you can compare any two revisions with the existing field-level diff, restore an earlier one, label one to keep it, or (as an admin) delete one. Restoring is forward-only — the restored configuration is saved as a *new* revision, so nothing is ever rewritten. A stack upgrade also cuts a revision of `default` instead of overwriting it silently, which makes an upgrade's configuration changes diffable and rollback a single restore. The last 20 revisions per profile are retained, plus anything labeled, pinned by a test run, or currently in use; a save that changes nothing records nothing, so repeated deployments cannot push your real history out of that window. See [docs/configuration-profiles.md](docs/configuration-profiles.md).

- **Pin a Configuration Profile revision for processing, testing, and labeling.** Revisions were recordable but not selectable: everything ran under whatever the profile held at the time, so you could not run one test against `lending` r5 and another against r7 — the one comparison revisions exist to make possible. A **Configuration revision** picker now sits beside the profile picker in Test Executions, Upload Documents, Reprocess, and Generate draft labels (and `--config-revision` on the CLI), defaulting to the profile's current revision and appearing only when the profile actually has history. Test runs record the pinned revision alongside the test-set version they scored against, so a metric delta is attributable to the *configuration* or the *ground truth* rather than ambiguous, and a pinned revision is exempt from retention pruning so the comparison stays readable. Every document is pinned at queue time whether or not you chose a revision, so a save made mid-flight can no longer change the configuration under an in-flight document. See [docs/configuration-profiles.md](docs/configuration-profiles.md#selecting-a-revision-for-processing-or-testing).

- **Multi-document sections are now visible instead of silent.** When one section turns out to contain several separate documents of the same class — because classification found no type change to split on ([#565](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/565)) — the section now reports an **instance count**, surfaced in the UI, plus an `extraction_multi_instance_detected` warning and `MultiInstanceSections` / `MultiInstanceRecordsRecovered` CloudWatch metrics. A class whose schema is already modelled as a packet of records can name its own instance axis with `x-aws-idp-instance-array: <propertyName>` to get the count with no schema change and no effect on extraction output.

- **The per-page `document_boundary` signal is persisted.** The classifier's `start`/`continue` decision — what `sectionSplitting: llm_determined` splits on — is now stored on the page record, in run snapshots, and exposed on the API, so an unexpected section merge can be inspected after the fact rather than re-derived from Lambda logs.

- **Classification confidence is real, and "not scored" is now distinguishable from "certain" ([#673](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/673)).** Every page and section carried a hardcoded `confidence: 1.0`, so there was no way to tell a clear W-2 from an ambiguous form that could have been three things — while `docs/classification.md` documented a `confidence` key and showed it in example output, and the *default* prompt asked the model for a `classification_reason`. Both were parsed by nothing: those output tokens were bought on every page of every document and dropped on the floor. Both are now read, and reach the page record, the run snapshot, the API (`Page.ClassConfidence` / `Page.ClassReason` / `Section.Confidence`), and the Web UI, where the Pages and Sections tables show the score beside the class and a page's cell is hoverable for the model's own "Why this class?". Section confidence is the **minimum** across the section's pages — a mean hides the one page the classifier was unsure about, which is the page a reviewer needs. The parse is tolerant of what models actually emit (`0.95`, `"0.95"`, `95`, `"95%"`) and ignores anything unparseable rather than failing a classification that otherwise worked. ⚠️ **A fabricated `1.0` is no longer written anywhere**: an unscored page or section is now **null**, and `1.0` is reserved for the paths that genuinely assert a class (a document-name regex match, a single-class configuration, a page-content regex match, a human correction in the UI). This changes the long-standing `section_confidence` column in the reporting lake from a constant 1.0 to a real value or null, so a query that filtered or averaged on it will see different results — the old numbers were not measurements. **BDA mode is scored immediately and for free**: BDA's matched-blueprint confidence *is* the classification confidence there (the blueprint decides the class), it was already being read for the HITL decision, and it is now stored instead of discarded. In **pipeline mode the default now asks for ranked candidates** (`classification.confidence.mode: topk`, below), so out of the box every page carries a real score and its runner-up classes. See [docs/classification.md](docs/classification.md#classification-confidence).

- **Classification now reports how sure it is of the class, by default: `classification.confidence.mode: topk`.** Asking a model for a single "how sure are you?" number gets ~0.95 on everything, which is worse than no score because it invites automated escalation on noise — so the default mode asks for **ranked candidate classes with probabilities** instead ("80 % W-2, 15 % 1099"). Forcing the model to enumerate and rank alternatives makes it distribute probability mass rather than default to near-certainty (Tian et al., *Just Ask for Calibration*, EMNLP 2023 — the same reasoning behind extraction's `G1`/`P1` path), and the runner-ups are the direct answer to the question a suspicious classification actually raises: what else could this have been? They are stored per page and listed in the UI's "Why this class?" popover. **It is on by default because the cost was measured first, not assumed:** over 298 pages of a 13-class corpus, `topk` added +17 % to the classification step — which is only ~3 % of total document cost on the default classifier, so **~0.5 % of the bill**, inside run-to-run variance — and changed classification accuracy by nothing consistent (+0.013 on Nova 2 Lite, −0.007 on Claude Haiku 4.5, opposite signs). A signal that catches 43 % of the default model's own misclassifications from 8 % of its pages is worth half a percent. `verbalized` (one number) and `off` (spend nothing) are the alternatives, and all of it is editable in the Web UI under **Configuration → Classification → Class confidence**. ⚠️ **How *useful* the number is depends strongly on the classifier**, and that difference is large: Nova 2 Lite answered exactly 0.95 for 90 % of pages (a coarse two-level flag), while Haiku 4.5 produced a graded distribution supporting real review routing — 73 % of errors caught from 11 % of pages at 94 % precision, at 6.4× the classification cost. Measure yours before routing work on the score. Implementation notes: the instruction is spliced in *before* the document content so it stays inside the prompt-cache prefix; both blocks are editable prompts; a page is scored on the probability of the class **actually stored** rather than the top of the list (they differ when the model contradicts its own ranking); candidates outside the configured vocabulary are dropped; and probabilities are rescaled only when they sum to more than 1.0 — mass left below 1.0 legitimately means "possibly some other class". See [docs/benchmarking/classification-confidence.md](docs/benchmarking/classification-confidence.md).

- **Class confidence reads as a measurement, not as more text after the label.** The score arrived appended to the class name in the Pages table — two unrelated values in one cell, with nothing to say the percentage was clickable. It now has its own sortable **Class conf.** column there: click a percentage for the model's reasoning and its ranked alternatives, plain text when it scored a page but explained nothing, `—` when it did not score it at all (never `0%`). Sorting is the functional half — least-confident-first is how a reviewer finds the pages worth a second look — and it needed enabling, since the table was `sortingDisabled` and so every column that already declared a sort field never offered one; they all work now, via the design system's own collection hook. The **Sections** table deliberately does NOT get the column: a section score is an aggregate of numbers already shown per page one table below, and the extra column squeezed that table until its own headers wrapped and Actions fell off the panel. `Section.Confidence` remains on the API and in the reporting lake. For the same width reason, the **Instances** column becomes a badge inside `Class/Type`, shown only when a section really does hold more than one document — a column that printed `1` on every row of a normal document was spending width to say nothing. The number is styled identically at every value: no classification threshold is configured, so a traffic light would assert a verdict the system has not defined, and on a classifier that answers ~0.95 for most pages it would paint a coarse flag as calibration.

- **Classification confidence is now measurable, not just reportable.** A confidence nobody checks is worse than none, so the evaluation report's per-page classification rows now carry the classifier's own `predicted_confidence` next to the `correct` flag that was already there, and the benchmark harness turns the pair into `class_calibration_separation` — mean confidence on correctly classified pages minus mean on incorrectly classified ones — alongside `class_accuracy` and `n_class_scored_pages`. Near-zero separation means the model is equally confident when it is right and when it is wrong, and the score must not route work; `aggregate.py --compare` flags a 0.03 drop as a regression on the same footing as the existing field-level separation. This is deliberately the gate on turning confidence-driven escalation on, rather than shipping it on the assumption that the numbers are useful.

- **Schema-enforced model output, plumbing only (not yet enabled).** `BedrockClient.invoke_model` accepts a Converse `toolConfig`/`toolChoice` so a schema can be put on the wire rather than described in prose, and the GPT-5.x Responses path accepts an `output_schema` for native `json_schema` strict mode. Both are opt-in and no caller uses them yet. Note that Anthropic `strict` tool use and structured outputs are rejected by Bedrock on Converse, InvokeModel **and** Mantle Messages alike, so on Claude and Nova a forced `toolChoice` is the strongest enforcement available. Property names are sanitized before they go on the wire ([#709](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/709)) and mapped back on the response, so a class authored with spaces in its field names — 4 of the 152 shipped preset classes — can be sent as a tool schema without any field being renamed downstream. `invoke_model` raises rather than sanitizing silently, because renaming without returning the reverse map would rename every extracted field with no way back.
- **Both new behaviours are individually switchable.** `extraction.coercion.enabled` and `extraction.validation.enabled` (plus `validation.fail_action` and `coercion.date_order`) are editable in the Web UI under **Configuration → Extraction**, so if either causes trouble it can be turned off without a redeploy. The #686 classification cost increase is likewise switched off with `classification.sectionSplitting: disabled`.
- **Simple extraction now coerces and validates its output.** Previously it did a raw `json.loads` and passed whatever came back downstream, so a wrong type or a non-ISO date reached storage unchallenged — Advanced extraction has had full-schema validation for some time, and this brings the same guarantee to the path most deployments run. First, deterministic repair without a model call (currency and thousands separators in both conventions, named-month and unambiguous numeric dates, boolean-ish strings), recording every change under `metadata.coercion` and **refusing** anything genuinely ambiguous (`01/02/2024`, 2-digit years, fractional-to-integer) rather than guessing. Then full JSON-Schema validation, with `extraction.validation.fail_action` deciding what happens: `warn` (default) and `reject` are free, while `escalate` re-extracts only the failing fields with a stronger model. Only the failing fields are merged back, so an over-eager escalation cannot overwrite fields that already validated, and a failed escalation returns the original extraction unchanged. A remaining violation now raises an `extraction_validation_failed` processing issue on the section (an **error** under `reject`, a **warning** otherwise) naming the failing fields and the first few concrete violations — so the free default actually makes the violation visible in the document list's **Processing Issues** column and the **Processing Report**, rather than leaving it in the section result JSON. ⚠️ **Coercion is on by default and changes stored values**, so an extraction re-run on this release can differ from the same document's earlier output (`"$1,234.00"` is now stored as `1234.0`). Measured A/Bs showed no accuracy change — the models already return correct types for scalar fields, and the evaluation comparators are format-tolerant — but a comparison against historical results using an **exact** comparator, or any downstream consumer that reads the raw string, will see the difference. Set `extraction.coercion.enabled: false` to keep the model's output verbatim; validation still reports the mismatches. See [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md).

- **A revision can now say what changed.** `handle_update_custom_configuration` — the path the Web UI, the CLI and the SDK all take for an ordinary edit — accepted no notes, so every such revision recorded an author and a timestamp but nothing about the intent; only the special operations ("Profile created", "Reset to default", "Restored from r3") carried any. A profile's history was therefore a column of anonymous timestamps, which is unusable for an automated loop that cuts one revision per attempt. `idp-cli config-upload --revision-notes "raised topK to 20"` (and `upload(revision_notes=...)`) now records the note on the revision, shown as **Notes** in the Revision history panel. Deliberately distinct from `--version-description`, which sets the *profile's* description and is overwritten by every save; a note belongs to one revision and is immutable. Creating a profile with notes uses them in place of the generic "Profile created", while the operation-specific notes are left alone since those describe what the operation *was*.

- **Revisions are now readable from the CLI and SDK, so an automated loop can iterate inside one Configuration Profile instead of minting a profile per attempt.** Revisions were recordable and pinnable but invisible to a programmatic caller: `config-upload` never said which revision it had just created, so a script had no number to pin; `config-list` withheld the revision counters the underlying query already returned; and there was no way to enumerate or fetch a revision at all. A tuning loop therefore had to encode each attempt in a *profile name* — and every one of those profiles then shows up in the profile pickers and `allowedConfigVersions` scope lists of the whole deployment. `config-upload` now prints `Revision: r7` and the flags to process under it (`upload()` returns `revision`), `config-list` gained a `Rev` column (`published_revision` / `latest_revision` on the SDK), a new `config-revisions --config-profile <name>` lists a profile's history with `--json` for scripting (`config.revisions()`), and `config-download --config-revision 7` fetches an exact revision (`download(config_revision=...)`). A pruned revision **fails** rather than quietly returning the profile's current configuration — substituting a different configuration under the name you asked for would look like a success, and its numbers would go into a comparison. See [docs/idp-cli.md](docs/idp-cli.md#config-revisions) and [docs/configuration-profiles.md](docs/configuration-profiles.md#revisions-from-the-cli-and-sdk).

- **Reporting SQL layer — hourly + daily rollup tables on top of `metering`.** A SQL layer on top of the raw `metering` Parquet lake that lets consumers (idp-monitor, in-tree analytics agents, and any future dashboard/report) answer "additive-over-wide-range" cost/volume questions in KB-scale scans instead of GB-scale. Previously every 7d/30d dashboard load ran a live Athena aggregation over raw `metering` — GBs of Parquet, 15–90s query time, frequent `503`s at the 29s API Gateway ceiling. Six new Glue tables (`metering_hourly`, `metering_daily`, `metering_docs_hourly`, `metering_docs_daily`, `control_plane_hourly`, `data_plane_lambda_hourly`) plus a single scheduled `DataMartRollupFunction` (hourly at :05 UTC, daily at 00:15 UTC via EventBridge crons) precompute the aggregations. Consumers pick a tier by range (`<2h → raw`, `2-24h → hourly`, `>24h → daily`) and by grain (unit/cost → `metering_*`; volume/pages → `metering_docs_*`) and pay a partition-pruned rollup scan on wide ranges. Rollups are pure append-only — `metering` now partitions by write time (= doc completion time) instead of `initial_event_time` (queue time), so metering rows never land in past partitions, and no trailing-window / Iceberg complexity is needed. See [docs/reporting-sql-layer.md](docs/reporting-sql-layer.md).

- **Analytics chat agent can now actually query the reporting rollup tables it was told about.** Phase 1 shipped the tables *and* the first-step overview the agent reads — `get_database_overview()` on `develop` describes all six rollups. But the second-step drill-down tool `get_table_info(...)` — which the agent calls to get columns and sample queries before writing SQL — routed only `metering`, `document_evaluations`, `rule_validation_*`, and `document_sections_*`; asking for any rollup name fell through to `## Unknown Table`. The agent therefore had to guess SQL from the one-paragraph overview blurb, which doesn't say `metering_daily` uses `day` (not `hour_ts`) or that `n_docs` lives on `metering_docs_*` (not the cost tables) — so the query returned `COLUMN_NOT_FOUND`, the agent retried a few times, then fell back to raw `metering` for a full-day scan. `get_table_info` now returns a full column-per-column description of each rollup table, and the agent's system prompt carries an explicit tier picker (`<2h` raw · `2–24h` hourly · `>24h` daily) with the cost-vs-docs split rule as canonical templates. Measured on `idp-dev-qs`: a "docs + pages last 30 days" question dropped from **2.28 MB** scanned on raw metering (after 3 failed rollup attempts) to **~510 bytes** on the docs rollup, first try. (The observed run hit `metering_docs_hourly` — the tier picker's `>24h → daily` guidance would route this to `metering_docs_daily` at ~30 rows scanned, an even smaller read.) Sample queries carry a doc-hours disclaimer — `metering_docs_daily.n_docs` is a per-doc-per-hour count (`SUM(hourly n_docs)`), not a cross-day unique-doc count, so the query renames the output column `doc_hours` accordingly and cross-day unique counts still route to raw `metering` with `COUNT(DISTINCT document_id)`. `AgentMessagesDisplay.tsx` gets the same guidance for users reading the in-app schema help.

- **Bedrock token cost from control-plane agents is now measured, not always zero.** `control_plane_hourly` has always had `bedrock_tokens_in / bedrock_tokens_out / est_bedrock_cost` columns, but no in-repo Lambda ever populated them because nothing called `emit_control_plane_cost_metric` with Bedrock args — Track C in `docs/reporting-sql-layer.md` §Phase-2. A new reusable Strands hook, `ControlPlaneCostHook`, registers on `AfterInvocationEvent`, reads the agent's cumulative `event_loop_metrics.accumulated_usage`, and emits the per-invocation delta (not the cumulative total) with the concrete `bedrock_model` used at inference time. It is wired on all six in-tree Strands agents via a `with_cost_hook(hooks, component, model_id)` helper (kept in one place instead of duplicated per agent): the chat orchestrator (`chat-orchestrator`), the analytics agent (`analytics-agent`), the error analyzer (`error-analyzer`), the code-intelligence agent (`code-intelligence`), the quick-start agent (`quick-start`), and the external MCP agent (`external-mcp`); the marketplace idp-monitor's `monitor-agent` picks up the same hook via a companion PR. New CloudWatch metrics `IDPControlPlane/BedrockInputTokens` and `IDPControlPlane/BedrockOutputTokens` (dimensioned by `Component`, `FunctionName`, `Model`) start emitting the moment the hook is deployed; the rollup Lambda picks them up on its next hourly tick and sums across `Component` streams per `FunctionName`, so a Lambda hosting multiple agents (e.g. `ChatStreamProcessorFunction` running the orchestrator + specialised agents) still lands one row per `(function, model)`. The hook itself only emits the counter that actually grew (zero-side counters are skipped) and asymmetric-regression handling means one counter dropping doesn't over-emit the other. Verified live on `idp-dev-fix`: a 4-message chat session emitted 203 K input / 5 K output tokens with `est_bedrock_cost = $0.25` in the next `:05` rollup — first time the column is non-zero on that stack. Latent gap: the hook doesn't yet read `cacheReadInputTokens` / `cacheWriteInputTokens` — no in-repo agent has prompt caching enabled today, so the counters stay accurate until the first one does; wiring cache tokens is tracked as a follow-up.

- **Control-plane cost tracking, first-class in `control_plane_hourly`.** The fifth rollup table breaks out "what is the IDP infrastructure itself costing me while idle" (dashboard resolvers, AI-summary agents, natural-language agents, test-set polling, test-run aggregation, config resolvers, capacity planners, discovery, fine-tuning, agent chat, the rollup Lambda itself) separately from data-plane doc processing. Populated by the same rollup Lambda, which queries CloudWatch for `AWS/Lambda/Duration` + `Invocations` plus custom `IDPControlPlane/*` metrics (`AthenaBytesScanned`, `BedrockInputTokens`, `BedrockOutputTokens`) emitted by control-plane Lambdas via a new shared helper `idp_common.metrics.emit_control_plane_cost_metric`. Discovery is subtractive using the allowlist model — only the 23 data-plane Lambdas (per-doc processors: OCR / Classification / Extraction / Assessment / Summarization / Evaluation / BDA path / Rule Validation / ingest + tracking + pipeline hooks + HITL section-review callback) carry `idp:plane=data`; everything else is implicitly control plane. Stack membership is scoped by walking the CloudFormation stack tree (root + nested) via `cloudformation:ListStackResources` and filtering on the CFN-native `aws:cloudformation:stack-name` tag — no custom `idp:stack` tag is defined or maintained. A new lint check (`scripts/check_data_plane_tags.py`, wired into `make lint` / `fastlint` / `lint-cicd`) enforces the tag on every allowlisted Lambda — a rename or missing tag fails CI, so silent misattribution to `other-control` is turned into a loud build failure. New features shipped INSIDE the accelerator's root stack tree (autotune, hooks, in-stack agents) require zero tagging work — they show up under Control Plane automatically. **Caveat:** Feature Platform extensions deployed as separate top-level stacks (independent `cfn.create_stack` calls) carry their OWN `aws:cloudformation:stack-name` tag and are NOT walked by the rollup Lambda's stack-tree discovery — an extension's Lambdas are invisible to `control_plane_hourly` unless the extension explicitly registers with the reporting pipeline. This is intentional (extensions are owned by their own accounts/authors and shouldn't be silently billed against the host's cost KPI), but worth calling out for anyone building an extension who wants cost visibility.

- **A configuration knob can no longer ship invisible to the Web UI ([#707](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/707)).** The configuration editor renders entirely from a schema hardcoded inline in CloudFormation, kept by hand in step with the Pydantic models in `idp_common/config/models.py` — so a new setting could be fully functional via YAML/CLI, pass every test, and have no control in the UI. That happened to `extraction.coercion`, caught only by someone manually asking where it appeared. A new gate (`scripts/sdlc/tests/test_config_ui_schema_parity.py`, run by `make test`) walks every field reachable from `IDPConfig` and requires each one to either have a control in the embedded schema or appear in an explicit exceptions list with a written reason — separated into "should never be settable" (derived, machine-written, metadata, notebook-only) and "acknowledged UI gap", so the second list is visible and shrinkable. It checks the reverse direction too, since `IDPConfig` is `extra="ignore"` and an unbacked control silently discards whatever the user types; that check immediately found one — `discovery.output_format.sample_json` is rendered by the UI and present in the system defaults but read by nothing, and its default is stale. The exception lists are self-cleaning: an entry that gains a control, or that stops naming a real field, fails the build.

- **A "Create profile" button, so making a Configuration Profile no longer means knowing that "Save as" means "copy".** The Configuration Profiles table — the surface for opening, activating, comparing and deleting profiles — could **import** a profile from a file or the config library but could not create one from an existing profile, which is how most profiles actually get made. That action lived one level down, in the editor's **Actions** menu as "Save as Profile…", so the flow was: find the source profile, open it, wait for it to load, then find a menu item whose name does not say "copy". **Create profile** now sits in the table itself: pick the source (prefilled with the checked row, else the profile currently open), name the copy, and it is created and opened in the editor with the source untouched. The new profile inherits the source's full configuration (its `Default` merged with its `Custom` layer), so it is a true clone rather than an empty shell. This is now the documented way to get an editable copy of the stack-managed `default`. The editor's action is **kept and renamed to "Save current edits as new profile…"**, because it is not the same operation — it snapshots what is in the editor right now, *including unsaved edits*, where the table's button copies the last saved state; the modal now says which one you are getting. One shared modal (`CreateConfigProfileModal`) backs this, the Discovery panels' existing "create a profile" button, and nothing else, so the validation rules are the same everywhere. Also demotes the table's **Delete Selected** from the primary button — a destructive action should not be the visual default on a table whose main job is creating and opening profiles. See [docs/configuration-profiles.md](docs/configuration-profiles.md#available-actions).

- **xAI Grok 4.6 is now selectable, and it is the first non-Claude/non-Nova model that works for agentic extraction.** Available as `us.xai.grok-4.6` (US regions) and `global.xai.grok-4.6` (EU, APAC, and most other commercial regions — also the cheaper of the two at $2.00/$6.00 per 1M vs $2.20/$6.60). Unlike the OpenAI GPT-5.x family, which needs the `bedrock-mantle` Responses API and is consequently locked out of agentic extraction and Discovery, Grok is served on the standard **Converse** API — so it flows through `BedrockClient.invoke_model` and the Strands `BedrockModel` unchanged, and `toolConfig` works with all three `toolChoice` modes. It is offered for OCR, classification, extraction (simple **and** agentic), assessment/confidence, summarization, evaluation, chat, multi-document cluster analysis, and the rule-validation and agent paths — 14 of the 18 model picklists. Its 500K context window also yields a ~90K-token shard budget, against ~18K for a 200K-context Claude model. Tune it with `reasoning_effort` (`none`/`low`/`medium`/`high`/`xhigh`); reasoning is always on and defaults to `low`. ⚠️ **Three things the AWS model card advertises do not actually work, verified live on 2026-09-02:** Flex and Priority service tiers are rejected (*"The provided service tier is not supported for this model"*), so no tier-suffixed Grok IDs are offered; prompt caching never engaged (four back-to-back identical 20,033-token prompts each reported `cacheReadInputTokens = 0`), so do not budget for a caching discount; and GovCloud is left unsupported because the card's geo-CRIS claim contradicts a standing live finding that `us.`/`global.` prefixes are rejected in GovCloud regions. Grok is **not** offered for **Discovery** or **Policy Discovery** — it rejects whole-PDF `document` blocks outright (*"This model doesn't support documents"*; text and image input only), so those four picklists omit it and config validation rejects the combination at save time rather than failing a document two stages in. `temperature` / `top_p` are hard-rejected by the model and silently omitted from requests. See [docs/grok-models.md](docs/grok-models.md).

- **The Document List can reach Test Studio's documents again — a Production / Test Studio switch beside the search box.** 0.6.5 moved test-run submissions onto their own `TypeDateIndex` partition so they stopped burying real uploads, and the `listDocuments` API has taken a `view: PRODUCTION | TEST` argument ever since — but no UI passed it, so the `TEST` partition was unreachable from the console and its documents looked deleted. A **SegmentedControl** now sits to the left of **Find documents** (source first, then search within it) and selects the partition. Deliberately a two-state switch rather than a "show test documents too" toggle: the API has no combined view, and merging them client-side would undo the separation — on this stack the default 30-day window holds 148 production documents against 3,447 test ones, so the uploads would be buried again. The **Find documents** field is also widened, since it now shares its row with the switch and had collapsed to about half its former width — cramped for what it filters, which are S3 object keys. In the Test Studio view the table gains a **Test Run** column (linked to that run's results in Test Studio) immediately after Document ID, the header reads **Test Studio Documents**, the empty state points at Test Executions rather than stopping at "no documents", and **Abort / Reprocess / Delete** are disabled with a tooltip explaining that a run owns its documents' lifecycle — reprocessing or deleting one would silently invalidate the results scored against it and the confidence calibration derived from them. The switch is **not persisted**, unlike the time-period and custom-range preferences: a sticky view would mean returning days later, seeing none of your uploads, and concluding documents were lost. Side effect worth knowing: documents that `sendTestRunToReview` flips into the HITL hopper were unreachable from the Document List entirely, and now are not.

- **A `Latest` option in the Document List's Load menu — now the default — for "just show me the most recent documents" regardless of age.** The menu only offered time windows topping out at 30 days plus a custom range, and defaulted to **2 hours**, so a stack quiet for longer than that opened on an empty table that read as a broken deployment; the fix was to guess a range. `Latest` is empty only when there are genuinely no documents. It matters most for the **Reviewer** role, whose queue is age-agnostic: a document pending review for three days was invisible under the 2-hour default. ⚠️ The default only applies where no preference is stored (fresh deployment, new browser, cleared site data) — anyone with a saved Load choice keeps it. This also unifies two defaults that disagreed: the layout resolved an empty preference to 2 hours while `useGraphQlApi`'s own parameter fallback was 2 days. `Latest` drops the date bounds entirely: the `TypeDateIndex` GSI is keyed `(ItemType, InitialEventTime)` and queried newest-first, so an unbounded query returns the newest documents whatever their age, and the resolver already handled absent start/end times. It also loads far less: a windowed period drains **every** page in the range, which on the new Test Studio view means ~18 sequential requests for this stack's 3,447 test documents in 30 days, where `Latest` stops after one. ⚠️ **It is capped, and says so.** It stops at 200 delivered rows, and the header counter reads `(N+)` rather than `(N)` whenever documents remain beyond what was loaded, so a capped list never reads as complete. The cap counts *delivered* rows, not requests, because the reviewer RBAC filter and the config-version scope filter are applied **after** DynamoDB's `Limit` — a requested page of 200 can arrive with 3 rows, so stopping after a fixed number of pages would hand a scoped user a near-empty list and label it "the latest 200". A 10-request ceiling bounds the pathological case (a scope that matches almost nothing), logs a warning naming the cause, and shows the `+`.

- **Three alarms and two dashboard widgets for the SQS layer, which had none — a document could dead-letter or the queue could stop draining with every existing alarm reading `OK` ([#761](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/761)).** `WorkflowErrorsAlarm` and `SlowExecutionsAlarm` both read the state machine, so neither can see a document that never got an execution: a dead-lettered document emits no `ExecutionsFailed`, and a document still waiting in the queue emits no `ExecutionTime`. **`DocumentQueueDLQAlarm`** and **`QueueSenderDLQAlarm`** fire on the first message with no threshold to tune — the first means a document exhausted roughly 8 hours of retries (`maxReceiveCount` 1000 against a 30 s visibility timeout), the second is worse, an upload that never reached the queue and therefore has no tracking record and is absent from the Web UI rather than shown as failed. Both also notify on recovery (`OKActions`), because DLQ depth does not decay on its own, so the `OK` is how you know someone drained it rather than muted it. **`DocumentQueueStalledAlarm`** deliberately does **not** alarm on queue depth or on message age: the queue processor refuses a message by letting its visibility timeout lapse while workflow concurrency is saturated, so messages wait by design, and because a refused message is never deleted and re-sent, `ApproximateAgeOfOldestMessage` measures time since the *original* send and climbs monotonically for as long as saturation lasts — a healthy stack draining a bulk upload would trip any fixed age threshold, and a Bedrock outage would trip it too (the circuit breaker pushes visibility out to `CircuitBreakerRecoveryTimeoutSeconds`) while `BedrockServiceOutageAlarm` was already reporting the real cause. It instead requires age above the new **`QueueStalledAgeThresholdSeconds`** parameter (default 1800) **and** zero `NumberOfMessagesDeleted`, sustained across six 5-minute periods, so deep-but-draining stays `OK` at any depth. A circuit-breaker pause trips it too and that is deliberate — the paused processor extends visibility without deleting, so both conditions hold, and suppressing it would mute the queue signal exactly when documents pile up; correlate with `BedrockServiceOutageAlarm` on the same topic. The 30-minute window is deliberately longer than every other alarm here: a message leaves the queue only when a workflow slot frees up, so a healthy stack saturated by long-running documents can legitimately dequeue nothing for as long as its slowest document takes, and a shorter window would page on ordinary large-packet processing. Thirty minutes of zero dequeues is where "slow" and "stuck" stop being distinguishable from outside and deserves attention either way — it is either a wedge or a capacity shortfall. The `FILL(m2, 0)` in that expression is load-bearing and pinned by a test: SQS publishes no deletion datapoint at all when nothing is draining, so without it the expression would go missing during the exact incident and `TreatMissingData: notBreaching` would read the silence as healthy — [#746](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/746)'s failure mode by another route. Two widgets land on the main dashboard: **Document Queue Backlog** (depth, in-flight, and age-of-oldest on a right axis annotated at the threshold) and **Dead-Letter Queue Depth** (document, queue-sender and workflow-tracker DLQs on one graph). The other seven DLQs in the template remain unalarmed; they are feature-scoped rather than on the ingest path. See [docs/monitoring.md](docs/monitoring.md#the-queue-alarms--the-gap-both-of-the-above-leave).

### Changed

- **⚠️ Turning on `x-aws-idp-multi-instance` requires migrating that class's evaluation baselines.** Evaluation compares a prediction against a stored baseline **of the same shape**, so a wrapped prediction against a flat baseline scores every field as missing-on-one-side and the class's accuracy collapses to ~0 **with no error anywhere**. This is the one part of the feature that can break a working deployment, so migrate in the same change as the flag: `python3 scripts/migrate_multi_instance_baselines.py --stack-name <stack>` (dry run by default; `--apply` to write, `--backup-suffix` to keep originals, `--direction unwrap` to roll back). It is idempotent, touches only classes that set the flag, and on rollback refuses to flatten a baseline holding more than one record because that would discard ground truth you authored. **It migrates shape, not content:** wrapping a one-record baseline gives `instances` of length 1, and the extra records for documents that really contain several were never in the baseline — the old pipeline could not extract them — so you have to add them or evaluation will score the newly-found records as false positives. The script lists every document it touched.

- **Confidence-curve keys no longer depend on how many rows a list happened to have.** `_flatten_confidences` keyed the index off list *length* rather than depth (`prefix if len(node) == 1 else prefix[i]`), so any single-element list lost its index: a one-row table keyed `Transactions.date` while a two-row table keyed `Transactions[0].date`, and confidence-curve points therefore did not join across documents. Multi-instance sections made it acute (`instances.Field` vs `instances[0].Field`). ⚠️ Keys for single-element lists change shape (`F.x` → `F[0].x`); points already stored under the old key will not join with new ones, but a list whose length *varied* never joined and now does, so this replaces two inconsistent shapes with one — but a list that was **always** single-element joined fine before, and its stored history is orphaned with no migration and no read-side fallback.

- **Auto-sizing no longer lets a model's output cap starve its own shard budget.** `bedrock/sizing.py` reserves room for the model's response by subtracting the usable output window from the usable input window — sound while every family's output cap was small relative to its context, and wrong the moment one was not. Grok 4.6 (500K context, 524,288 output cap) made the subtraction negative, collapsing the shard budget to the 2,000-token floor and making the model with the largest context window shard the most finely of any. The output reserve is now capped at a fraction of the usable input window. The fraction (0.65) was chosen as the largest value that leaves the derived shard budget of **every** pre-existing model row bit-identical — the binding case is the 200K/128K Claude families, whose usable output is 65.5% of their usable input — and a parametrized regression test pins those numbers so a future change to the cap cannot silently re-shard existing models. Alongside it, `maxTokens` now defaults to the Converse-standard `inferenceConfig.maxTokens` carrier for every family instead of only Nova, with Claude keeping its verified `additionalModelRequestFields.max_tokens`. Previously a model in neither family got no cap at all, so Bedrock's small truncating default applied and an explicitly configured `max_tokens` was silently discarded — that affected the Qwen, Llama, Gemma and Nemotron IDs already in the picklists, and any `application-inference-profile` ARN, not just Grok.

- **A feature/extension install no longer mints a Configuration Profile per release — it cuts a revision of one profile ([#697](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/697)).** `applyFeatureConfigPreset` wrote `Config#<featureId>-v<version>`, so twelve releases of one extension left twelve profiles behind — and a profile is not a version: it is an access-control object an admin must add to every scoped user's `allowedConfigVersions`, a document-visibility partition, a confidence-curve bucket in Test Studio, and a permanent row in the Configuration Profiles table. Presets now write `Config#<featureId>` and record each release as a **revision** of it, so an upgrade is a diff an admin can read and an upgrade that changes nothing records nothing. The preset is merged over the host's `Config#default` at install time and stored as a full configuration, so the recorded revision is the same shape a later admin edit produces. Uninstall now also deletes the profile — including legacy `<featureId>-v*` profiles from before this change, and their revision history — activating `default` first if the feature's profile is the active one, because a feature's configuration carries that feature's pipeline hooks inline and leaving it active after uninstall points every subsequent document at deleted Lambdas. ⚠️ **Existing `<featureId>-v<version>` profiles are left alone** by an upgrade; they are swept the next time that feature is uninstalled, or deletable with `idp-cli config-delete`. See [docs/feature-platform.md](docs/feature-platform.md).

- **"Configuration version" is now "Configuration Profile", and "version" means a snapshot of one.** One term was doing two jobs — the named configuration you select and control access to, and (once history existed) a point in its history — which produced the unusable phrase "versions of a config version". A **profile** is the named entity (`default`, `Production`, `lending`): the access-control unit, the document partition, and what you activate. A **revision** (`r7`) is an immutable snapshot of one profile's configuration. This matches how *version* already worked for [document versions](docs/document-versions.md) and test set versions. **Nothing you have scripted changes.** No API field or stored attribute was renamed — `getConfigVersions`, `versionName`, `ConfigVersion`, and `allowedConfigVersions` all still refer to profiles. The CLI and SDK additionally *accept* the new name: `--config-profile` / `config_profile=` alongside `--config-version` / `config_version=`, both setting the same value, with the older spelling kept for backward compatibility and no plan to remove it (the SDK raises `ValueError` rather than guessing if you pass both with different values). Docs and `--help` now show the new spelling. `docs/configuration-versions.md` remains as a pointer to the new page.
...
- **The extraction-value editors' "Revision History" tab is now "Edit history".** Three different things kept a history and two of them used the word *revision*: an immutable numbered snapshot of a configuration, and a log of field-level edits to an extracted value or label. The edit log is now named for what it is — matching its own underlying `_editHistory` data and the `EditHistoryTab` component that renders it — so **revision** means exactly one thing in the product. `docs/configuration-profiles.md` now carries the whole vocabulary in one table (profile / configuration / revision / version / edit history), linked from [document-versions.md](docs/document-versions.md) and [test-studio.md](docs/test-studio.md). Documents and test sets keep their **versions**: those names are in the public API (`listDocumentVersions`, `publishTestSetVersion`) and in stored DynamoDB keys, so renaming them for symmetry would mean a data migration and a breaking change for no user benefit.

- **The Configuration Profiles table no longer wraps its rows.** Its column widths asked for 90% in percentages plus a further 300px in fixed columns, so every column was squeezed below its content on any viewport and the text wrapped — eating vertical space above the configuration editor. Fixed widths are now used only where content size is predictable, with the two free-text columns flexing into what remains; **Created** is hidden by default (still in the preferences gear) since **Updated** answers the same question; and rows render at compact density. The table is also now covered by tests, including an assertion that the width budget cannot over-commit again — the original bug passed lint, type checking, the full UI suite and a live deploy test.
...
- **`extraction.agentic.validation` moved to `extraction.validation`, and validation is now ON by default.** Simple extraction runs the same validate-and-retry path, so the setting was no longer agentic-only and the nesting was misleading. Stored configurations are migrated automatically on read — **no action is required** — and `config_format_version` advances to `0.7`. The default `fail_action` also changes from `escalate` to **`warn`**: `warn` is free (no extra inference, just a visible `ProcessingIssue` instead of a silent schema violation), whereas `escalate` re-extracts failing fields with a stronger model and costs real money, so it must now be opted into deliberately. Previously validation defaulted to *off*, which meant the guard against silent schema violations was disabled on exactly the configurations that needed it.
  - **Four shipped presets asked for `MM/DD/YYYY` on a field declared `format: date`.** `bank-statement-sample`, `rule-validation`, `ds11-passport-application` and `lending-package-sample-govcloud` each had a date field whose description mandated US ordering while its schema declared ISO-8601. That contradiction was harmless while nothing enforced the schema; with coercion on it is not — unambiguous rows would be normalized to ISO and rows whose day is ≤ 12 left alone, so one transaction list would hold both conventions. Those descriptions now ask for ISO-8601, matching the schema, so the column is consistent. Evaluation is unaffected (those fields use the format-tolerant `DATE` comparator). If you want US ordering, drop `format: date` from the field rather than leaving the two in conflict.
  - ⚠️ **A profile that turned validation on but never named a `fail_action` stops escalating on upgrade.** It inherited the old `escalate` default (as does a profile whose stored `fail_action` is null or blank, which the config editor can produce), and now inherits `warn` — so schema failures become a visible `ProcessingIssue` instead of a paid re-extraction. Set `extraction.validation.fail_action: escalate` explicitly on those profiles to keep the previous behaviour.
  - The configuration editor shows the relocated setting correctly for pre-existing custom profiles: the raw sparse delta it reads is relocated too, so the editor and the pipeline never disagree about a setting.
  - **On rollback to an earlier release**, the older code detects the newer stored format and allows the rollback to complete rather than blocking it. It ignores the unknown `extraction.validation` key, so validation reverts to its old default (off); if that older code then re-saves the configuration, custom validation settings are dropped and must be re-entered after rolling forward.

- **Configuration validation is stricter on Bedrock ARNs that "look like" a model ID.** A configuration whose Bedrock ARN resolves to a resource that has model-id shape (contains a `.`, isn't a UUID) but doesn't match the accelerator's `valid_models` list now **rejects the upload** (`valid: false`) instead of producing a warning. This turns the "typo in the ARN's resource-id, partition, or account" class of bug into a loud CI/upload failure rather than a silent misconfiguration that fires at first Bedrock call. ⚠️ **Upgrade path for legitimate ARNs newer than the accelerator's `valid_models`**: an ARN for a cross-region inference profile or a model that ships after the accelerator's version was cut will resolve to a real-looking model ID (e.g. `us.anthropic.claude-4-5-sonnet-…`), so the validator will refuse it. Fix: add the model to `config_library/pricing.yaml` — the file `valid_models` is read from — or pin the accelerator to a release that already lists it. (An entry there is required for that model's cost reporting regardless, per the "adding a Bedrock model" checklist, which is what makes rejecting an unlisted model defensible rather than merely strict.) ARNs whose resource is a plain UUID (`application-inference-profile/<uuid>`) or lacks model-id shape are unaffected and still produce a warning rather than a reject. Note the check is a no-op when `config_library/pricing.yaml` isn't on disk, so it only fires for the CLI/SDK run from a repository checkout and in CI — never from a pip-installed `idp_common` inside a Lambda.

- **`metering` table partitioning: write time, not queue time. Semantic change to the existing `timestamp` and `date` columns.** `save_reporting_data.save_metering_data` now partitions on `datetime.now(UTC)` at write time — which for metering is document-completion time, since the writer runs at workflow end. Every metering row lands in the current partition, so the trailing re-materialization window that earlier drafts required is unnecessary. **This changes the semantics of an existing data-lake column in place**: the `timestamp` column and the `date` partition now mean "completed at" where they previously meant "queued at". A consumer that filters `WHERE date = '2026-08-18'` gets "docs completed that day" instead of "docs queued that day"; `initial_event_time` is preserved as a column for consumers who still need queue-time. Both known consumers (Monitor dashboard, in-tree analytics agent) benefit from the completion-time interpretation. `metering` also gains an `hour` partition key so the current-hour tail query in the tier picker can partition-prune to ~40 MB instead of scanning the whole day. **Upgrade path (automatic):** a CFN custom resource (`MeteringHourMigrationCustomResource`) runs during `update-stack` and relocates historical `metering/date=X/*.parquet` files into `date=X/hour=HH/` subdirs before the Glue table's new projection takes effect — the metering table `DependsOn` the migration, so dashboards keep working against the old layout until the migration completes. The migration in turn `DependsOn` `SaveReportingDataFunctionV2`, the sole writer of `metering/` parquet, so the new write layout is live *before* the migration snapshots the old-layout key list; without that ordering, every document completing during the update would have written an old-layout file after the snapshot and become permanently invisible to the new projection. A residual window remains for invocations already in flight on the previous code version (roughly one invocation duration) — a deployment that ingests continuously should either quiesce uploads for the update or re-run the idempotent `scripts/migrate_metering_hour_partition.py` afterwards. Fresh installs no-op immediately (empty bucket). For very large historical volumes (>30K old-layout files), the custom resource fails fast with a clear error pointing at `scripts/migrate_metering_hour_partition.py` for manual execution before retrying `update-stack`. **Cutover boundary caveat:** the migration infers each historical file's hour from its own `timestamp` column, which was queue-time before Phase 1. New writes are completion-time. So `date`/`hour` mean queue time before the cutover and completion time after, with no explicit boundary marker — any query that reads both sides of the cutover mixes both semantics silently, and at hour grain any document whose processing spanned an hour boundary (not just midnight) can appear in a different partition than a query would expect. See [docs/reporting-sql-layer.md](docs/reporting-sql-layer.md) §2.3.

### Fixed

- **Two boolean settings whose names end in "prompt" were rendered as free-text boxes in the Configuration form.** **Fallback To Prompt** (under *Schema Enforcement (experimental)*) and **Restate Schema In System Prompt** (under *Agentic Extraction*) are declared `type: boolean` in the pattern's config schema, like every other true/false setting — but `ConfigBuilder` chose the widget by testing the field's **path** for `prompt` / `description` *before* honouring the declared type, and it made that choice for the whole subtree, so a boolean whose key merely ends in `_prompt` got the roomy textarea meant for prompt templates. Not just cosmetic: the field then stored a **string**, and the backend config model accepts only the spellings pydantic reads as a bool — `off` happened to work, while `disabled`, `yes ` with a trailing space, or simply clearing the box is a config `ValidationError` rather than a setting. Fixed at the cause rather than per field: a declared `boolean` / `number` / `integer` now wins over the name heuristic, which stays in place for strings (it is why no prompt in the schema has to declare `format: textarea`). Those two are the only fields in the shipped schema the heuristic caught, verified by walking every node of the unified pattern's schema; a future one is covered by default. 5 tests, each revert-checked.

- **The Prompt Preview was blind to everything schema enforcement puts on the wire, and its token total said so by not moving.** It rendered `extraction.system_prompt` / `task_prompt` with placeholders filled and nothing else, so with **Schema Enforcement** on (Simple + `extraction.forced_tool.enabled`) the forced `toolSpec` — tool name, tool description, input schema, forced `toolChoice` — appeared nowhere, and turning the setting on changed the previewed total by **exactly 0**. That is the one number people open this page for: on the shipped lending `Payslip` class the toolSpec is ~1,250 estimated tokens against ~1,485 for the prose schema, so ~45% of the schema budget was invisible. In **Advanced** mode the same hole existed on the system prompt, where `agentic.restate_schema_in_system_prompt` — which **defaults on**, and whose entire selling point is token count — appends an `Expected Schema:` block the preview never showed. Now: a **Tool Schema** tab (Simple + forcing only) shows the `toolSpec` with its tokens in the label and in the total, counted from the **compact** form actually serialized onto the request rather than from the display indentation; the **System Prompt** tab (Advanced + restatement only) ends with the config-derived `Expected Schema:` block, marked as such and absent from the Raw System Template tab, which stays the user's own text. Both are gated on `extraction.mode`, which is authoritative over `agentic.enabled` server-side (`reconcile_mode_and_agentic`) — so Advanced never shows a forced-tool tab and Simple never shows a restatement — and neither reaches the Confidence Assessment step, which sends no forced tool. **Two honest gaps are stated in the UI rather than papered over.** The property-name sanitizer is *not* ported to TypeScript: names outside `^[a-zA-Z0-9_.-]{1,64}$` are rewritten for the wire and restored in the stored result, so the tab shows the names you authored, says renaming happens, and counts how many on this class. And the restated block is an **approximation** — the backend restates `model_json_schema()` of a Pydantic model generated from the class, which adds a `title` per field and an `anyOf`/`null` union per optional field while dropping object-level descriptions, making the real block **roughly 1.7–2.9x larger** than the browser can render (measured across the four lending preset classes). The previewed saving from turning the restatement off is therefore a floor. Also corrected while here: the existing multi-document-probe warning claimed the probe in **Advanced** mode, where `_build_wire_schema` returns the schema untouched — a warning about something never sent. Preview-only; no runtime behavior changed. See [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md) and [docs/web-ui.md](docs/web-ui.md#prompt-preview).

- **A section that lost records to a same-class merge is no longer silent ([#753](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/753), closes [#565](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/565)).** When one section spans several consecutive records of the same class, classification has no type change to split on, the class schema describes one document, and the model overwhelmingly answers with one object — so records 2..N were simply absent from the response and **nothing anywhere reported it**: section `SUCCESS`, document `COMPLETED`, `ProcessingIssueCount: 0`, instance count `1`. Reproduced live on a 4-page, 3-statement synthetic pay statement: 1 of 3 records, zero issues. The array-recovery path added in 0.6.6 cannot catch this — it fires only when the model returned a list, which is the branch where nothing had been lost yet. Extraction now asks the model, **in the same inference**, how many separate documents of the class the pages contain; when that exceeds the number of records in the result the section raises `extraction_multi_instance_suspected` (**warning**) naming both numbers, reports the model's count as the section's instance count with `metadata.instance_extracted_count` beside it, and emits a `MultiInstanceSectionsSuspected` metric. The Sections-panel badge says the records are *missing* rather than claiming they were extracted. Detection only — it never changes the extracted data, never fails a section, and never turns on a schema flag for you. `extraction.multi_instance_detection.enabled` defaults to **false**, and the reason is a measurement rather than caution. Measured on two real labeled corpora via Test Studio (80 paired runs, identical documents per pair, only the toggle differing): **it is very good at the job** — on 40 bank-check images scored against their committed baselines it found all **18** multi-check images, raised **0** false alarms on the 22 single-check images, and reported the count **exactly** right on all 18 (2 to 8 checks), for input tokens +1.8% and output −0.5%. **What those 18 flags mean is worth stating precisely, because an earlier draft overstated it:** the extracted rows were counted afterwards and **no checks were lost in either arm** — `BANK_CHECK`'s schema is a single `checks` array, so that class already models several checks per sheet. Detection was reporting a *configuration* finding (the class never declares its instance axis, so the section reports 1 instance for 6 documents), which is real and worth acting on — set `x-aws-idp-instance-array: checks` — but is not averted data loss. Turn detection on where a section can hold several documents **and the class schema describes only one**; that is the combination that loses records. But on a corpus with no multi-record documents to find it is pure cost: RealKIE-FCC-Verified lost ~1.3 accuracy points (0.7678 → 0.7552; worse on 14 of 40 documents, better on 1, sign test **p = 0.001**), diffusely across four attributes. A default has to be safe for the deployment that gets no benefit — so it ships off, with a strong recommendation to **turn it on per configuration profile** wherever one section can hold several documents of the same class. With it off the extraction prompt is byte-identical to earlier releases. The question put to the model is editable via `extraction.multi_instance_detection.question` (supports `{DOCUMENT_CLASS}`), like every other prompt; two clauses in the shipped wording are load-bearing and pinned by tests. The **Prompt Preview** warns when either server-side schema transform — the wrapper or the detection probe — means the rendered prompt is not the one sent. One extra integer of output, no second call; the field is stripped before anything downstream sees the result. Covers Simple extraction on both the prompt and forced-tool paths — **Advanced (agentic) extraction is not covered yet**.

- **A list row containing a nested group or an inner list was reported as unscored however well the model had scored it.** `_row_confidence_missing` looked exactly one level down, so a nested group inside a list row was mistaken for a confidence leaf — a group has no `confidence` key, so the lookup returned None — and an inner list was skipped by the dict filter entirely. Measured live on a 3-record pay statement whose rows carry an `Employee` group and an `Earnings` list: every leaf came back at 0.99–1.0 with OCR geometry and `truncated_calls: 0`, and the section still reported `assessment_incomplete` (**error**, which renders it Incomplete in the UI) for all 3 rows after burning a `claude-sonnet-5:1m` escalation call that recovered 0 — a stronger model reproduces the identical shape, so the self-healing ladder had no way out. The predicate now recurses. Pre-existing for **any** list-of-object attribute whose rows contain a group or an inner list; multi-instance sections make every record a row, so it became universal there.

- **The Z3 rule engine could not address a list element, and a path that had become wrong went quiet instead of failing.** `_extract_path` was dot-notation only, so `instances[0].NetPay` (or any `LineItems[2].amount`) was unreachable; and because a miss returns `None` and `None` means "optional parameter absent", a rule whose path no longer resolved simply stopped firing with no error. Subscripts are now supported (including negative indices and chains like `a[0][1]`; out-of-range is a miss, a malformed subscript now raises rather than resolving to `None`), and a miss that lands on a multi-instance wrapper logs the path to write instead.

- **Re-running Discovery on a class that already existed erased every class-level setting an author had put on it ([#764](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/764)).** The write path replaced the class dict wholesale, and the discovery LLM emits only `$id`, `x-aws-idp-document-type` and `properties` — so `x-aws-idp-extraction-model`, `-extraction-escalation-model`, the per-class prompts, `-confidence-threshold`, `-confidence-escalation-model`, `-document-name-regex`, `-document-page-content-regex`, `-page-types`, `-exclude-from-processing`, `-examples`, `-multi-instance` and `-instance-array` all vanished. Nothing warned: Discovery reported success and the class looked right (better, even, with fresh properties), and the regression surfaced only in the **next** document processed — as a different model, an escalation that stopped happening, an excluded class that started being processed, or records 2..N silently dropped again. Discovery now owns what it produces and carries everything else forward, by the rule "preserve anything discovery did not emit" rather than a list of keys to keep, so an extension key added later is covered by default; a setting discovery *does* replace is logged at write time. The same wholesale overwrite existed on the **BDA blueprint-optimization** path and in **schema bootstrap**, and both are fixed with the shared rule. (BDA→IDP *sync* with `sync_mode: replace` still replaces classes by design — there the BDA project is the declared source of truth.) Renaming a class id carries its settings across the rename too — normalization used to double as a silent reset. **Two things are deliberately not carried**, because they describe the property map that discovery just replaced rather than the class: `required` / `$defs` (a stale `required` is validated against every extracted object, so it would report a missing property on every document forever), and `x-aws-idp-instance-array` when the array property it names is no longer in the schema — keeping that one fails configuration validation outright, which would abort the entire save rather than lose one setting. Both drops are logged; re-declare the instance array on the new schema if the class still holds several records per section. Per-**property** keys (`x-aws-idp-evaluation-method` / `-evaluation-threshold`, `-source-page-types`) are likewise replaced along with their property, since a regenerated attribute can legitimately change type. `description` is replaced too — discovery is asked for one — and that replacement is now warned about, because the classification prompt's class table is built from it.

- **No CloudWatch alarm notification was ever delivered on a default deployment — every alarm in the stack was console-only.** Found while adding the queue alarms above, and it is the reason that work would otherwise have shipped inert. `AlertsTopic` is encrypted with the stack's customer-managed key on **every** deployment, but the key-policy statement granting CloudWatch permission to use that key was wrapped in `!If [CircuitBreakerEnabledCondition, ...]` — and `CircuitBreakerEnabled` defaults to `"false"`. So on a default stack every alarm action failed with *"CloudWatch Alarms does not have authorization to access the SNS topic encryption key"*, affecting `WorkflowErrorsAlarm`, `SlowExecutionsAlarm`, `ConcurrencyCounterDriftAlarm`, `WorkflowTrackerDLQAlarm`, `StaleOutputPurgeFailedAlarm` and `DataMartRollupDLQAlarm` alike. The failure was invisible in every place an operator looks: the alarms transitioned to `ALARM` normally in the console and on the dashboard, the SNS subscription showed as confirmed, and the only evidence was each alarm's **Actions** history — verified on a live default stack, which had been silently failing `SlowExecutionsAlarm` deliveries for weeks. This is [#746](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/746)'s report-OK-by-silence shape one layer further out: the alarm was right, the notification path was not, and nothing in the alarm resource itself was wrong. The grant is now unconditional and a unit test rejects any condition on it. ⚠️ **If you upgrade a stack that has been alarming silently, expect notifications to start arriving immediately** — that is the fix working. `describe-alarm-history --history-item-type Action` shows the before/after.

- **`WorkflowTrackerDLQAlarm` now notifies on recovery as well**, matching the other DLQ alarms: DLQ depth does not decay on its own, so the `OK` is the only signal that separates a drained queue from a muted alarm.

- **The benchmark harness could not run the two reference corpora, and said so in a way that sent you nowhere ([#766](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/766)).** `run_matrix.py` launches one local PDF per run; the reference corpora (`realkie`, `ocr_bench`) are test *sets* on the stack with no PDF under `benchmarks/corpus/docs`, so the launch loop `continue`d past them under a comment claiming they were "handled separately" — there is no such path. A suite naming `core_docs` therefore measures **7 of its 9 documents**, and the two it drops are the only ones with real pages and human-verified labels — the only guard against a change that looks good on the harness's own generator and regresses on real documents. The document-class filter did print a note, but it filed them as belonging to another class and advised `--class <their class>`, which **cannot work**: there is no config for a reference corpus, so that run exits on a missing index. And nothing was recorded anywhere, so a scored 7-of-9 grid was indistinguishable from a complete one after the fact. The launcher now reports them by their real cause, keeps the class-mismatch note for documents another `--class` genuinely can run, and records `docs_named` / `docs_run` / `docs_unlaunchable` / `docs_other_class` in `runmap.json` — which `aggregate.py` now copies into the committed `meta.json`, since the runmap itself is gitignored. A missing synthetic PDF is now a single up-front error naming `gen_corpus.py` instead of a per-run silent skip (`--estimate` still answers before the corpus exists). Reporting only — launching reference corpora through the harness remains open on [#766](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/766); run them through Test Studio meanwhile. Harness only; no product code.

- **Analytics agent system prompt's classification-count template rendered `FROM document_sections_<class 'type'>`.** The template read `FROM document_sections_{type}` inside an f-string, so the `{type}` placeholder — meant to be a literal for the LLM to substitute per document class — instead interpolated Python's built-in `type` object. LLMs mostly compensated for the nonsense, but any strict SQL emission would have hit a table-not-found. The placeholder is now escaped as `{{type}}`, which renders to the intended literal `{type}` in the outgoing system prompt.

- **⚠ Chat-companion sub-agents now persist to `ID_HELPER_CHAT_MEMORY_TABLE` — behaviour change on deploy.** `AgentFactory.create_agent` has always constructed a `DynamoDBMemoryHookProvider` per session and passed it in via `hooks=`, but four of the six in-tree agent creators (`analytics`, `error_analyzer`, `code_intelligence`, `external_mcp`) previously called `strands.Agent(...)` without forwarding that kwarg, so the memory hook was silently dropped on the floor. Those four agents' conversations were memoryless: each new turn from the same user session started fresh, no history reload. This MR replaces the ad-hoc bare-arg pattern with `hooks=with_cost_hook(kwargs.get("hooks"), component, model_id)`, which preserves whatever the factory passed in. **Net effect:** when a user opens a directly-selected chat with any of those four agents, their prior turns in the same `session_id` are now reloaded from DDB and future turns are written back (subject to the existing `max_message_size_kb=8.5` truncation). The orchestrator-mediated subagent path is unaffected — the orchestrator continues to strip `hooks` from `sub_kwargs` so sub-agent tool chatter doesn't contaminate the top-level session row (see `orchestrator/agent.py:67`). If a deployment prefers the previous no-memory behaviour for a specific agent, unset `ID_HELPER_CHAT_MEMORY_TABLE` — that env var already gates the hook creation at the factory.

- **Confirming labels bypassed the version transition that editing them respects.** Found by working the annotate screen as an Annotator on a live stack. "Labels are correct — mark reviewed" was enabled before any transition was open, though it calls `completeSectionReview` for every section and tags each one `reviewed-human` — the `draft-machine → reviewed-human` change a version exists to bracket. Gating only the editor therefore left a second, ungated route to the same commitment: an entire set could be confirmed, and its provenance rewritten, with no version recording what the labels had been. It now requires an open transition, and **Start annotating** is the only primary action until there is one — Claim was unconditionally primary, so two solid-blue buttons competed while the actual prerequisite sat lower down inside an alert.

- **A reviewed document could not be reached again until every other one was done.** Reviewing a document drops it out of the queue, and the only route back was the action on the "Queue complete" alert — which by definition appears once all the remaining work is finished. Searching the document by name until then returned "0 matches" above "No documents to review", with nothing indicating it was being filtered out; that is exactly the state an annotator who has just mis-saved is in, and `getAnnotationQueue` had supported `includeCompleted` all along. The reveal now sits in the queue rail, and the empty state distinguishes "no match for your filter" from "nothing to review".

- **The label-accuracy estimate was printed to a tenth of a percent it had not earned.** One review of a twelve-document set moved a displayed 85.1% to 92.2% — a seven-point swing under a digit claiming tenths. `EstimateConfidence` exists, per its own docstring, "so a cold-start estimate is never rendered with the same authority as a measured one", and the headline number was the one place ignoring it: the qualifier line said "estimate still based on other sets" while the figure above it read like a measurement. The decimal is now spent only on a curve actually measured on this set, and the quality tier beside it agrees rather than showing "92%" and "92.2% est." in the same row.

- **The per-page menu on the page-regrouping board was clipped by the column it sits in.** On the annotate surface the section column is a 320px `overflow: auto` box; the menu ran 32px past it, cutting off its last item ("Move later") entirely, and inherited the column's 118px width, wrapping all three labels onto two lines each. So the keyboard route the menu exists to provide was barely readable and partly unreachable. Same failure as the drag preview before it moved to a body-level overlay — a positioned element cannot escape a scrolling ancestor by z-index — and fixed the same way, by portaling the menu out of it.

- **Two dead ends on an annotator account.** A set badged "Needs labeling first" sat above a link reading "Open queue — most confidence alerts first", but generating those labels is `generateDraftLabels`, which is Admin/Author only: the blocker was stated to the one role that cannot clear it, with nobody named to take it to and a link promising an ordered queue that opens empty. The card now names who can — or tells an Admin/Author it is theirs to do — and the link reads "Open set" when there is no queue to order. Separately, the Extensions section badged uninstalled extensions **Install** for every role, though `subscribeFeature` is Admin-only and the feature page already routes everyone else to an "awaiting admin install" screen; for a non-admin the badge now reads the state ("Not installed", "Update available") rather than naming an action, which is all it claimed to do anyway.

- **Six identical `getMyProfile` calls per page load.** `useUserRole` is consumed by the navigation, the annotation landing page, the annotation workspace and each feature page — all mounted together, each running its own effect for an answer that is per-user and fixed for the session. Now fetched once and shared, keyed by the token's subject rather than a bare "already fetched" flag: signing out and back in as a different user is routine, and a key-less cache would hand the new session the previous user's scope, failing **open** for the least-privileged role — the one `allowedTestSets` exists to constrain. A rejection is not cached either, so one transient error cannot pin an account to the unrestricted default.

- **Uploading a test set from a zip produced 0 documents, in the layout the wizard itself documents.** The extractor partitioned entries with `'/input/' in file_path` — a substring test with a leading slash — so a zip built exactly as the **Required zip structure** diagram shows, with `input/` at the root, matched nothing and every file was skipped as "not in input/ or baseline/ folder". Only a zip with a wrapping folder passed, which is why the four pre-deployed HuggingFace sets worked and a hand-made one never did. The same leading slash appeared again in both extraction loops behind `if len(parts) == 2`, so fixing the partition alone would have logged "Validation passed: N input documents match N baseline documents" and then written nothing. Both now match a whole path segment. macOS archive noise (`__MACOSX/`, AppleDouble `._*`) is excluded too, since `__MACOSX/input/._doc.pdf` did contain `/input/` and was taken for a document. Every zip fixture in the test file was wrapped, and its helper reimplemented the partition inline rather than calling the extractor, so the bug was invisible to that file by construction.

- **A test set uploaded from a zip took the archive's name instead of the one typed into the form.** `TestSetUploadInput` had no `name` field at all, so the wizard collected and validated a name it had nowhere to send and the resolver derived one from the filename — "my-test-set" plus `Archive.zip` produced a set called Archive. The wizard's success toast then interpolated the form value it had never sent, confirming a name the server did not receive; it now reports the id the server returned. `name` is optional rather than required because this mutation has shipped since 0.6.1, and the fallback is logged.

- **Correcting a document in a set that arrived with its own ground truth never counted as reviewed.** The queue read "0 of 73 documents reviewed" however many were fixed, above a toast saying the document had been marked reviewed. `reviewed` is derived from `labelSource == 'reviewed-human'`, and the direct-to-S3 save wrote an edit-history entry but never the label source. That path is not an edge case: `completeSectionReview` requires a review record that only draft labeling creates, so an authored-ground-truth document *must* save directly, and it was the only path available to the set in question. Two comments asserting the direct path had "nothing to tag reviewed-human" were the reasoning behind it — who authored a label and whether this reviewer has now checked it are different facts.

- **Every save confirmation named `result.json`.** It took the last segment of the baseline key, which is that filename for every document in every set.

- **Editing a field kept the model's confidence attached to your typing, and losing the edit took one click.** Both from the second browser UX review of the annotation editor. A field a human had overwritten still displayed `Confidence: 100.0%` and was still labelled `Predicted:` — the model never produced that text, so neither claim was about the value on screen; hand-typed ground truth was being decorated with a score that belonged to the value it replaced. The confidence line is now suppressed on an edited field and replaced with *"Edited — the model's confidence no longer applies"*, and the provenance label reads `Your value:`. Separately, `beforeunload` was the only guard on unsaved edits, and an in-app route change is not a page unload: correcting ten fields and then clicking a nav link discarded all ten silently, with no prompt and no toast. `useBlocker` is the right tool and is unavailable — it needs a data router, and the app mounts a plain `<HashRouter>` — so `useUnsavedChangesGuard` intercepts in-app hash links in the capture phase instead, leaving modifier-clicks, external links and same-location links alone. 12 UI tests; the guard tests were checked against the old behaviour and fail there.

- **Every field value was a text input nested inside an element claiming `role="button"`, and the click-to-locate capability was mouse-only with no visible affordance.** ARIA forbids interactive descendants of a `button`, and screen readers computed the wrapper's name from its own subtree — so each field announced twice, once as a button named after its label and once as the textbox with the same label. The wrapper is no longer a control; a real `Show <field> on the page` button carries the action, works in read-only mode (where there is no input to focus), and is named honestly as `Select <field>` when the field has no bounding box to show. Tabbing into a value now also selects the field, which the wrapper's tab stop used to do. 5 tests, all of which fail against the old tree.

- **The run cost table printed `$2.39999999999999996` and `$2.4e-7`.** It interpolated raw numbers, so a price of exactly $2.40 arrived with its binary floating-point tail and anything below 1e-6 arrived in scientific notation. `toFixed(4)` is not the fix on its own — applied to a per-unit price of 2.4e-7 it renders `$0.0000`, which reads as free — so the money column keeps fixed places and extends only when a value would otherwise round away, while the unit-cost column, which spans eight orders of magnitude in one table, uses adaptive precision. The estimated-cost cell was also re-formatting an already-formatted string through `toFixed(4)`, undoing the precision. Long model IDs and subtotal labels no longer truncate to `bedrock/us.amaz…` and `Assessment Sub…`. 11 tests.

- **An annotator's class correction ran to completion while the UI reported that it had failed.** Observed on a dev stack as *"Could not re-extract this document: undefined"*. `reextractTestSetDocument` is Annotator-reachable and succeeded; the editor then polls `getDraftLabelJob` for the outcome, and that was absent from the resolver's `ANNOTATOR_ALLOWED_FIELDS`, so the poll 403'd. Starting a job you cannot observe is not a capability — worse than a plain refusal, because the work happened and the UI said it had not. `getDraftLabelJob` is now Annotator-reachable, with the same per-set `allowedTestSets` assertion its siblings carry, so group membership alone still does not expose another team's jobs.

  The `undefined` was a second bug, and the one that made this un-diagnosable from the screen: the editor's three catch blocks read `(err as Error).message`, but a GraphQL rejection is `{errors: [{message}]}`, so the real cause — *"Unauthorized: 'getDraftLabelJob' requires Admin or Author group"* — was dropped. All three now use the existing `getErrorMessage` helper, which has handled both shapes since before this file was written. 2 tests, one of which asserts that **every** Annotator-reachable field asserts per-set scope somewhere on its path, so the next addition to that allowlist cannot skip it.

- 🔒 **An Annotator can now see the class list, and receives only the class list.** `getConfigVersion` was `Admin, Author, Viewer` and excluded **Annotator** — the role the annotate screen exists for — so their config fetch was denied, the class dropdown had nothing to offer, and the editor fell back to a free-text box: the role least able to know the vocabulary got an unconstrained field, and a class no config defines yields a section with no schema and so no extracted fields. Meanwhile the *write* side of a class correction, `reextractTestSetDocument`, has always accepted Annotator — the missing permission was a config **read**, which nobody intended to withhold.

  `Annotator` is now on the operation, and the resolver reduces the response for any caller with no group in `{Admin, Author, Viewer}` to the class names and descriptions alone — the only fields the dropdown reads. Prompts, model ids, pricing and every other setting stay out of reach of the lowest-privilege role. The reduction is an **allow-list**, so a config key added later cannot leak by default, and a user in both `Annotator` and `Author` keeps the full payload because it is keyed on the absence of an entitled group rather than the presence of Annotator. Chosen over the two alternatives — granting the whole config, or moving the class list onto `getAnnotationQueue`, which would have needed a config-table grant for a Lambda that has none. Recorded in `scripts/api_rbac_expectations.yaml`, which is what makes the change reviewable; 9 tests, and the editor still locks the field with a reason if the list genuinely cannot be read.

- **An Admin could change a document's class in the document view but not in the annotate view, and was told it was a permission problem ([#674](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/674)).** It was never a permission check: `isReadOnly` was `!canAnnotate || !selected.reviewObjectKey`, and a document carrying authored ground truth has no review-queue record, so the editor went read-only whatever the role. The disabled dropdown then read *"You do not have permission to change this class"* two lines under an alert promising the reviewer could "correct the values".

  Fixing it properly meant separating **three** server capabilities the UI had collapsed into one flag, no two of which accept the same groups: changing the class goes through `reextractTestSetDocument` (**Admin, Author, Annotator**), saving fields on a draft goes through `completeSectionReview` (**Admin, Reviewer, Annotator** — not Author), and saving fields on an authored label goes through `uploadDocument` (**Admin, Author** — not Annotator). So the class is now gated on its own capability and offered to all three roles, while read-only is chosen per save path. `onSave` is supplied only when there is a review to complete, otherwise the editor falls back to the direct write the document view already uses. Two latent bugs fell out of the same analysis: an Author editing a draft-labelled document would have been refused at save, and `updateDocumentClass` guarded on the field flag, so an enabled dropdown would have accepted a class change and silently discarded it. 11 tests, four of which read the groups out of `schema.graphql` so the client's copy of the rules cannot drift.

- **The run-level classification-errors panel's deep links did not navigate.** `testSetAnnotateHref` already supplies both the leading `#` and the `?doc=` parameter, so adding either produced `##/test-studio/...`: everything after the first `#` is the fragment, HashRouter matched nothing, the query was discarded, and every row landed on the app root — disabling the one route into the panel's purpose. The test asserted `stringContaining('doc=...')`, which a double-hash href satisfies; it now asserts the link's shape.

- **One edited table row claimed authorship of its whole column.** The edited-path key dropped array indices, so correcting `LineItems[3].Rate` set the key `LineItems.Rate` — which every row's Rate cell also computes. All of them then rendered `Your value:` with the model's confidence suppressed, a false provenance claim on values the reviewer never touched *and* the loss of real confidence data on them. Indices are now preserved in a dedicated `editedFieldPaths` set; `predictionChanges` and `baselineChanges` keep their collapsed key, so the visual editor modal's own tracking is untouched.

- 🔒 **A caller-supplied `documentClass` reached S3 object metadata unvalidated.** It flows reviewer → `reextractTestSetDocument` → `generate_draft_labels` → SQS → S3 user metadata → `Document.from_s3_event` → a forced class and section, and `reextractTestSetDocument` is reachable by **Annotator**, the lowest-privilege group; the dispatcher deliberately does not deep-validate nested input fields. Now shape-validated at both call sites against the character set all 120 shipped class names use, with `fullmatch` rather than `^...$` (which matches before a trailing newline). Also declared on `GenerateDraftLabelsInput`, where it was honoured but undocumented. Membership against the deployment's configured classes is **not** checked — this Lambda has no `CONFIGURATION_TABLE` grant — so a well-formed but unconfigured class still yields a schema-less section and no fields.

- **Starting any test run failed outright: the metadata writer read a variable out of its caller's scope.** `_store_test_run_metadata` set `item["Purpose"] = purpose`, but `purpose` is a local of the *handler*, not a parameter — a `NameError` on every call, and since the function's `except` re-raises, every `startTestRun` failed. Introduced by the draft-labeling work in this same cycle and caught by `basedpyright` in CI (`reportUndefinedVariable`), not by the test suite: `Purpose` had tests, but only for its **readers**. A reader test passes happily against a value nothing ever writes. `purpose` is now a parameter defaulting to `"scoring"`, and `_store_test_run_metadata` has 5 tests of its own — all of which fail against the broken version.

- **A draft-labeling run still printed `Classification: Page Level Accuracy 0.000` under the alert saying it had no accuracy metrics.** The backend emits structural zeros for a run that scored nothing, and the run-level view rendered them, alongside a "Documents with Lowest Weighted Overall Scores" table showing its headers over an empty body. A hard zero is a stronger and more wrong claim than N/A — it asserts total failure where the honest answer is "not applicable". Both are now suppressed for such a run. The **cost** breakdown is deliberately kept: a labeling run over 100 documents spends real money and that is the one number it genuinely has. 4 tests.

- **A draft-labeling run warned that its accuracy metrics were "not available", on a screen that already said why.** Such a run has no baseline by construction — producing the labels is the point — so evaluation never runs and there are never accuracy metrics. The run-level view showed a yellow *"No Accuracy Data — Test run completed but accuracy metrics are not available"* two lines under *"Context: Draft labeling run"*, describing the expected outcome as a fault. The server already owned the rule (`_is_draft_labeling_run`, which prefers the persisted `Purpose` and falls back to an exact `Context` match) but only used it internally, so the UI had nothing to go on but the free-text context the fallback exists to distrust. Now exposed as `isDraftLabeling` on `TestRun` — a new field on an existing type, so no new resolver and no growth in the `HttpApiFieldFunctionMap` SSM parameter (verified: the template diff is empty, and entries there are one per resolver function, not per field). The draft case gets an info alert saying what to do next; the genuine case keeps its warning and now names the likeliest cause. 1 resolver test.

- **Clearing a test set's draft labels left it permanently claiming to have them, with an estimated accuracy for labels that no longer existed.** `clearDraftLabels` deleted the baseline objects and dropped the label-job pointers but never moved `labelState`, so a set cleared for a retest went on reporting **Draft (machine)** and a 97.6% estimated accuracy while all 100 of its documents read `Unlabeled`. `_reconcile_label_state` could not repair it: that probe is keyed on `fileCount`, and clearing drafts removes baselines without changing membership, so `labelProbedFileCount` still matched and the set was skipped on every subsequent list — the wrong state was permanent, not transient. With no labels left the state is now set to `unlabeled` directly; where non-draft ground truth survives, `kept` counts label objects rather than documents and cannot answer the coverage question, so the probe marker is dropped and `_validate_test_set_files` re-derives it — the same helper registration uses, so the two cannot disagree. 3 resolver tests, all of which fail against the old code.

- **The ground-truth editor called uploaded ground truth `Unlabeled`, contradicting two other statements on the same screen.** The queue card read `Uploaded` and the alert above read *"Already ground truth"*, because the server treats a baseline with no `labelSource` as authored ground truth (`_attach_label_metadata` falls back to `uploaded`). The editor read the same absence off the parsed JSON and rendered "no labels" — but it only ever has parsed JSON when a baseline exists, so absence there means the pipeline did not write the file, i.e. a human did. 4 tests.

- **Five things the live UX-validation run caught, four of them in the fixes above.** Verified on a dev stack after deploying the batch. The `Est. label accuracy` column asserted a verdict while its estimates were still in flight — one request per set, fired after the table renders — so a labeled row read `Ground truth` for a second and then flipped to `76.1% est. Bronze`; a pending request is not a verdict, and it now reads `Estimating`. The legend claimed authored ground truth "carries no estimate", which the row beside it disproved: `quality_tier` returns at least Bronze even at `prior` estimate confidence, where no observation from the set contributed, so uploaded ground truth does get a prior-derived figure — the legend now says what a low figure there actually means. The new locate button rendered on every field even on a baseline whose banner read *"No field geometry available — bounding-box highlighting is disabled"*, so it is now offered only where there is a box to show (focus still selects the field, which needs no geometry). The disabled class dropdown's reason was a `placeholder`, which only renders when nothing is selected — i.e. never, on a document that has a class — and has moved to `constraintText`. And the cost table read `$0` unit cost beside `N/A` estimated cost on count rows like `totalTokens`, saying both "free" and "not priced" at once; those now read `—` in both columns. 3 tests.

- **A save confirmation was a raw S3 key, and nine smaller things the UX review flagged.** The success toast printed `Ground truth saved to opus-conf-bank-statement/baseline/152aa83d_doc_0001.pdf/sections/1/result.json` — where the bytes went, not what the reviewer had done; it now states the consequence. Also: the `Est. label accuracy` column gained a legend covering all four tiers, since a `Bronze` badge told you Bronze was bad without saying what better looked like; a set of hand-authored ground truth showed a bare `-` in that column while the machine drafts measured *against* it showed a percentage, inverting the trust signal, and now reads `Ground truth` with the reason, distinct from a draft's `Not assessed yet`; `Documents (0)` while the first page loads no longer asserts an empty set; `None of 278 fields` names its subject; the destructive `Clear draft labels` no longer looks identical to `Refresh` next to it; the disabled class dropdown says whether it is locked by a running re-extract or by permissions; `max: 0` before a test set is chosen no longer reads as a hard limit of zero; the re-extract alert says what it does and roughly how long it takes; and the annotation queue shows one primary button at a time — whichever action comes next — instead of two solid-blue primaries side by side.

- **A fully ground-truthed test set contradicted itself on one screen, and reviewing a document appeared to make its quality worse.** Both from the UX review, and both were presentation rather than logic. **(1)** A missing review key has two *opposite* causes — the set has no labeling run, or the document already carries ground truth and draft labeling deliberately skipped it — and the annotate pane assumed the first, telling a reviewer whose every document was already ground truth to *"Generate draft labels for the set first"* while the queue beside it said *"Ground truth — nothing to review"*. The queue rail already drew the distinction, with a comment naming the trap; the pane now draws the same one and says the document is already ground truth, nothing will overwrite it, and it can still be opened to inspect. **(2)** The **Set up team annotation** dialog priced three review strategies over a set with nothing to review — "about 0 of 0 documents (0–0)", "Est. effort —", "Audit sample 0" — with **Continue still enabled**. It now says every document already has ground truth, suggests running a test instead, and disables Continue. **(3)** Reviewing one document moved a set from "91.7% est. Bronze" to a **red** "Not rated / Unrated". The verdict was correct — with enough evidence to test whether confidence *ranks* correctness, the estimator found it does not (the AUROC-below-chance case) and withdrew a figure it had been inferring from a cross-set prior — but red reads as a fault the reviewer caused, when the preceding Bronze number was the less honest of the two states. "Unrated" is now coloured as an absence of a claim rather than an error, and states its reason **inline** instead of on hover, since that reason is the entire content of the verdict and the one thing a user cannot infer. Documented in [test-studio.md](docs/test-studio.md) so the transition reads as the estimator becoming honest rather than the labels getting worse. 6 tests, two of which fail against the previous presentation.

- **A draft-labeling run showed `EVALUATING` indefinitely, and every view of it triggered a pointless full re-aggregation.** Observed on a dev stack: a run `COMPLETE` with 100/100 documents processed and `CompletedAt` set, still badged `EVALUATING` three days later. A draft-labeling run **creates** the baseline — that is its purpose — so the copier skips baseline staging and evaluation never runs, and it has no aggregate metrics to wait for, ever. But `_awaiting_metrics` only asked "terminal, and no cached metrics?", which is permanently true for such a run. Two costs, from one cause: the badge said the run was still evaluating when nothing was, and because the cache-update enqueue is gated on the same predicate, **each view queued an aggregation that re-reads every document's `results.json` from S3** to produce extraction metrics that are structurally empty. The run's purpose is now **persisted** on its record (`Purpose`) and excluded from the predicate. Previously the only marker was the free-text `Context` string `"Draft labeling run"` — which a user can type themselves — so that remains as a fallback for records created before `Purpose` was written, matched **exactly** rather than as a substring so a context that merely mentions labeling cannot suppress a real scored run's badge. A persisted `Purpose` wins over `Context`. 5 tests, three of which pin the behaviour that must not regress: a scoring run with no metrics still awaits them.

- **The labeling dialog described a 100-document set as 50, then labeled all 100.** `getTestSetDocuments` is paginated at 50, and the UI counted the page it had loaded: the Documents header read `(50)`, the dialog said `50 document(s)`, and the button offered to `Label 50 document(s)` — while select-all sends no object keys, so the server walked the whole set and labeled every unlabeled document in it. Nothing was mislabeled and nothing was missed; the number shown was simply wrong, in the one direction that erodes trust in the rest of the numbers. **The paginated response now reports the set's size** (`totalCount`, read from the stored `fileCount` the resolver already fetches — O(1), and it stays O(1) as sets grow rather than requiring a walk per dialog open). The header reads `(50 of 100)`, and the dialog stops claiming a page-derived figure for a whole-set operation: select-all now says it covers every document in the set that needs labels and defers the exact count to the job, which already reports `total` and `skippedAlreadyLabeled` on its progress banner. **A real functional limit came out of the same place and is now visible:** choosing *specific* documents can only offer the loaded page, so document 75 of 100 was unreachable without paging to it — the dialog now says so instead of presenting a partial list as the whole set. 2 resolver tests, both failing against the previous response shape. **And the dialog now pages**, so picking specific documents is no longer limited to whatever the list behind it happened to be showing: it fetches its own pages (always starting at page 1, since what you can select must not depend on where that list is scrolled to), and **a selection survives turning the page** — held by object key outside the current page, because a Cloudscape table only knows the items it is given, so a selection stored in the page would appear to clear itself the moment the user paged. Paging offers one page ahead rather than arbitrary jumps, because S3 continuation tokens are sequential: `ceil(total / pageSize)` page buttons would let someone click page 5 and be shown page 1's documents labelled as page 5. 4 dialog tests, including that select-all still sends **no** object keys (enumerating the page would silently halve the job).

- **The class dropdown in the test-set editor offered the wrong classes — the built-in lending-package preset rather than the deployment's own ([#662](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/662)).** The editor picks the class list from the config version stamped on the baseline, which is right when a pipeline run produced those labels. **Hand-uploaded and synthetic ground truth carries no such stamp** — nothing produced it through the pipeline — and the fallback was the literal string `'default'`, which resolves to the *built-in preset*. So a set of uploaded labels was offered `Payslip` / `US-drivers-licenses` / `Bank-checks` / `Bank-Statement` / `W2` / `Homeowners-Insurance-Application` on a deployment running none of them, and the document's real class (`Invoice`) rendered as "Not defined in this config version". Nothing failed and nothing logged. The fallback is now: the baseline's stamp → the test set's declared version → the deployment's **active** config → the built-in default only when there is no active version at all. **The silence was half the defect:** the config badge was suppressed precisely when the version was `'default'`, so the one signal that would have exposed this was hidden in exactly the failing case. It is now always shown and names the fallback when it is one ("… (active config)", "built-in default"). Re-extraction runs under the same config whose classes were offered, so the dropdown and the re-run cannot disagree. The chain moved out of the component into a tested function — an unexamined fallback is what this was — with 6 cases including that a blank stamp counts as absent rather than as a config named "".

- **Correcting a misclassified document's class now actually changes it. It previously reported success while silently discarding the correction — and destroyed the document's reviewed status on the way.** Found by the first browser UX review of this UI, on a flow that had already been flagged as *"shipped broken for several versions and found in the field"*. Observed: change a class `Bank-Statement` → `W2`, accept the warning, wait 34s, get "Re-extracted as W2" — and the document is still `Bank-Statement`, still holding Bank-Statement fields, with `labelSource` demoted `reviewed-human` → `draft-machine`. An impossible-looking combination that is the signature of the bug: the destructive half of the operation persisted and the corrective half was overwritten. **Root cause was a false premise stated in the code's own docstring** — *"the class is pinned by writing it to the baseline first, since classification is skipped for pages that already carry one"*. That is not true on this path. `reextract_test_set_document` wrote the corrected class into the test set's **baseline**, but the re-run classifies from the **input document** and never reads that baseline; the pipeline re-derived `Bank-Statement`, and the harvest then wrote its own result back over the pin. So the pin was overwritten while the demotion (written in the same step, and never revisited) stuck. Fixed by sending the class **into the run**, along the path `configVersion` already travels: resolver → test runner → SQS → `test_file_copier`, which stamps it as `document-class` S3 object metadata → `Document.from_s3_event` reads it into a new `forced_document_class` field → the classification step applies it to every page, which routes into that step's **pre-existing** already-classified skip rather than adding a second skip path to keep in step. The baseline write stays, for two reasons that are now documented as load-bearing rather than assumed: a run that never completes still records the correction, and a `reviewed-human` label must be demoted because the harvest refuses to overwrite one. The forced class deliberately **overrides** an existing classification rather than seeding it — the request means "the class you chose is wrong", so re-classifying would reproduce the error. Extracted as `idp_common.classification.apply_forced_document_class` so the rule is testable without standing up the handler's document service, metering and X-Ray. **The test that let this ship asserted only the baseline write**, and its docstring repeated the false premise; it now also asserts the class reaches the run, and fails against the old code. Applying the class also **builds the section** covering the document's pages, which stamping pages alone does not: the already-classified skip returns the document untouched, so a document fresh from OCR reached extraction with `sections == []`, extracted nothing, and the run "succeeded" having produced a summary and no fields. Caught by running the flow on a live stack after the first version passed every unit test — the tests asserted the pages got the class, not that anything downstream could act on it. One section for the whole document is the right reading of "this document is a W2" (the reviewer corrected a *document's* class, not one span of pages), and mirrors what the classification service already does when section splitting is disabled. 15 new tests across the four hops, including that an older payload with no such field is a no-op, that an ordinary run stamps no class metadata and gets no section invented for it, and that sections from the previous class are replaced rather than left behind.
- **A long class name in the document details tables no longer overlaps the column next to it.** Class names are conventionally CamelCase with no spaces (`Estimated2024AnnualTaxableDistributions`), which gives the browser no break opportunity, so the `Class/Type` cell in **Document Sections** and **Document Pages** rendered the name as one unbreakable line straight across the **Page IDs** / **Class conf.** column — both tables already set `wrapLines`, which cannot help without a break point. The class name now breaks mid-word (`overflow-wrap: anywhere`, which also lowers the element's min-content width so the surrounding flex row will narrow it) and stays inside its own column in both view and edit mode.

- **Updating an already-installed ConfBench Test Set extension no longer fails on Lambda's package-size limit.** A stack installed before 2026-08-13 could not be updated to any later version: all four `ingest/` functions failed with `Function code combined with layers exceeds the maximum allowed size of 262144000 bytes. The actual size is 280347130 bytes`, and the extension rolled back and stayed on its old version. The move of `pyarrow` out of each function's `requirements.txt` and into `SharedLayer` was itself correct — the new code artifact is 8.5 MB and the steady state is ~109 MB, less than half the cap. What failed was the **transition**: CloudFormation attaches a new layer with `UpdateFunctionConfiguration` *before* it calls `UpdateFunctionCode`, so for one moment the new 135.8 MB layer was combined with the OLD 144.6 MB code that still bundled `pyarrow` — two copies at once, summing to exactly the reported 280,347,130 bytes. A fresh install was never affected, which is why this surfaced only on upgrade. The layer build now strips the ~35 MB of `pyarrow` that `pyarrow.parquet` cannot reach at runtime (`include/`, `src/`, `tests/`, Cython sources, and the 24 MB Arrow Flight RPC stack), bringing it to 100.2 MB and the transition to 244.8 MB — 17.3 MB inside the limit. Because pruning by filename is silently fragile across `pyarrow` releases, `shared/prune_layer.py` refuses to build blind: every prune rule must match at least one file (a layout change that pruned nothing would otherwise let the layer quietly regrow), and no surviving ELF object may carry a `DT_NEEDED` on a library being removed (a future `pyarrow` that links Flight from the parquet path now fails the build instead of the Lambda cold start). Both guards are pinned by `tests/test_prune_layer.py`. The `262,144,000 - 144,555,306` ceiling is derived in code from the legacy artifact's measured size rather than hardcoded, and can be deleted once no stack remains on a pre-2026-08-13 version. See [feature-platform/confbench-testset/shared/README.md](feature-platform/confbench-testset/shared/README.md).

- **Documented: the #653 boundary rules mis-split a long table whose pages reprint a running header, and how to fix it per class.** A fund/distribution schedule or brokerage statement typically reprints its title, logo and column headers on **every** page and paginates with a **bare** page number (`7`), not `Page 7 of 17`. That defeats the rules' priority order: their pagination test does not recognise a lone number, and their opening-header-block test — which is evaluated *before* the continuation test — is satisfied by the running header, so a continuation page is read as the start of a new document. The classifier says so in its own reasoning: *"The page number '15' … indicates this is part of a multi-page document, **but the presence of the full title** … confirms this is the start."* It is **silent**: every fragment stays 100% complete and the document still reaches `COMPLETED`, so only the section count changes — and a consumer reading one section sees a truncated list and blames extraction, which is exactly what happened to this repo's own CI (a 16-page, 532-row table reported as "expected 532 fund items, got 193"; the 193 were pages 1–6). **The fix is the rules' own `PRECEDENCE` clause: state the boundary rule in the class description** (`BOUNDARY: … only the page numbered 1 is "start"; every other page is "continue"`), which measured 10/10 correct against 0/10 without it. **Deliberately not fixed in the shared prompt**, because every generic lever was measured and each one buys this case at the cost of a case #653 already balances — prompt wording took the no-pagination over-split fixture from 7/10 to 4/10, 2/10 or 0/10 depending on the phrasing, and `contextPagesCount: 1` fixes both over-split directions but takes the back-to-back-duplicates fixture from 10/10 to 5/10. The full measured table, and which lever suits which corpus, are in [docs/classification.md](docs/classification.md#known-failure-mode-repeating-running-headers) ([#750](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/750)).

- **The alarm for "workflow executions are failing" has never once fired, and now does.** `WorkflowErrorsAlarm` — the solution's primary failure alert, wired to the **Workflow Alerts** SNS topic — was defined against the `AWS/States` metric `ExecutionsFailedCount`, which Step Functions does not publish (the metric is `ExecutionsFailed`). CloudWatch creates an alarm on a never-published metric without complaint, so it was provisioned successfully and then sat in `INSUFFICIENT_DATA` permanently, notifying nobody. Confirmed on a live stack: **141 failed executions over 14 days and zero alarm state transitions in its entire history.** Nothing could have caught this — `cfn-lint` checks that an alarm is well-formed, not that its metric exists — and the dashboard widget beside it used the correct metric name the whole time, so the two disagreed about the same signal with no visible symptom. Both `WorkflowErrorsAlarm` and `SlowExecutionsAlarm` also now set `TreatMissingData: notBreaching`, matching the stack's five other alarms: without it they idled in `INSUFFICIENT_DATA` whenever there was no traffic, which is exactly what a wrong metric name looks like and is how this stayed hidden. A static test now pins both properties for every alarm in every template, and forbids the bogus metric name as raw text so a CloudWatch **dashboard** cannot regress the same way. ⚠️ **On upgrade, a stack that has been failing silently will start sending notifications immediately** — with `ErrorThreshold` defaulting to `1`, that is one alert per 5-minute window containing a failed execution. That is the fix working; raise `ErrorThreshold` if you routinely submit documents you expect to fail. Note the alarm counts *failed executions*, so a failure the state machine catches and handles still finishes as a successful execution and does not appear here ([#746](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/746), [docs/monitoring.md](docs/monitoring.md#alarms-the-stack-creates)).

- **Saving a new Configuration Profile onto a name that already exists no longer silently overwrites that profile.** The backend's `saveAsVersion` path writes straight to the named profile, and the Web UI only checked that the name was non-empty and had legal characters — so "Save as Profile" with a name already in use replaced that profile's entire configuration with the one being saved, with no warning and no confirmation. The prefilled name made it reachable by accident: it defaults to `<profile>-copy`, which already exists the second time you copy the same profile. `saveAsNewVersion` now refuses a name that is taken (it already refused `default` and the active profile), and both create-profile modals show the collision inline and disable their submit button before you click it. The previous state was recoverable from revision history — an overwrite does cut a revision — but nothing told the user an overwrite had happened. **Note this is the client-side and hook-level guard only; the `updateConfiguration` API still honors `saveAsVersion` against an existing profile**, which is what the CLI/SDK rely on to detect "exists → plain update", so a direct API caller can still replace a profile deliberately.

- **A model named by an inference-profile ARN is now recognized by the sampling-parameter capability gate.** `is_claude_4_7_model()` matched on the bare model ID only, so a config naming `arn:...:inference-profile/us.anthropic.claude-sonnet-5` — the form [docs/configuration.md](docs/configuration.md) recommends for cost-allocation tagging, and the only form available in GovCloud — was not recognized as a model that rejects `temperature`. Bedrock returned a 400 for every such request. Both this and the new xAI Grok gate now resolve the ARN first. Opaque `application-inference-profile/<uuid>` ARNs still cannot be resolved without a `GetInferenceProfile` call and remain a documented limitation.

- **`parse_model_id()` could not read a service-tier suffix off a model ID with no version segment.** It assumed a `:flex`/`:priority` suffix meant a third colon-delimited part, which held while every ID ended in `:0` or `:1m`. A newer-style ID such as `us.xai.grok-4.6:flex` split into exactly two parts, so the tier was never extracted and the whole string was sent to Bedrock as the `modelId` — an invalid identifier. No shipped model currently needs this, so it is a latent fix rather than a user-visible one.

- **Chat-with-Document sent `temperature` to models that reject it.** The chat Lambda builds its own `converse_stream` request rather than going through `BedrockClient`, and gated `temperature` on a Claude-4.7-specific check. `chat.temperature` always resolves to a float, so any model outside that check received it — which for xAI Grok is a hard 400 on every turn. The gate now uses the shared `strips_sampling_params()` predicate. Also fixed on the same path: `chat.reasoning_effort` was resolved from config and then discarded for all Converse models (it only ever reached the OpenAI Responses branch), so the documented knob did nothing for Claude; it is now routed to each family's carrier.

- **Re-uploading a document with the same filename no longer shows the previous document's extraction ([#719](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/719)).** OCR has a retry-safe recovery path (`discover_existing_ocr_pages`) that scans `<key>/pages/` and reuses complete page results across Step Functions retries. On a re-upload — replacing a W2 with a W3 under `test.pdf`, say — the previous document's pages were still present, so OCR skipped, and classification/extraction consumed text from the OLD document while the UI showed stale results with no error. On every S3 upload event the queue sender now purges `<key>/pages/` in the output bucket before enqueuing (nothing to purge on a first upload). The scope is deliberately just `pages/` — that is all OCR's recovery reads — so an upload to `foo` can never destroy a nested document at `foo/bar.pdf/*`, and the queue sender does not need broad delete rights over the output bucket. The reprocess resolver's broad `<key>/*` purge is unchanged (its "start over" intent is deliberate). `<key>/runs/` (per-run version-history manifests) is preserved in both paths. **Behavior change:** a re-upload of the identical file no longer reuses the prior OCR cache — every re-upload re-OCRs from scratch, matching the "I want fresh results" intent that re-uploading a filename implies. A partial purge cannot fail silently: it raises, is logged at ERROR, and publishes `StaleOutputPurgeFailed` in the stack's own metric namespace from both the ingest and the reprocess path. A single new `StaleOutputPurgeFailedAlarm` (any occurrence in 5 minutes → `AlertsTopic`) covers both, and the main dashboard gains a **Stale Output Purge Failures** count widget paired with a Logs Insights table of the affected keys — the same alarm-plus-widget treatment `ConcurrencyCounterDrift` gets, because a surviving `pages/` artefact silently re-opens this bug on the next upload. See [docs/monitoring.md](docs/monitoring.md#stale-output-purge-on-re-upload).

- **OCR silently dropped small, faint and skewed characters, because the default page-image resolution was too low for Textract to see them ([#729](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/729)).** When `ocr.image` was left unset — the default for every shipped preset — pages were rendered at 150 DPI and capped to 951×1268, about 110 DPI for A4. Textract does not report low confidence at that resolution; it omits the characters from its response **entirely**, so a page number, box number or hand-filled value simply never reaches the prompt, and the model correctly returns `null` for a field that is plainly legible in the source. In the reported case a `Page 2` indicator on a photographed IRS form came back `null` while the explainability rationale — accurately describing the *word* "Page" at 98% OCR confidence, with no page number in the OCR output to describe — read as though the value had been available. The defaults are now **300 DPI** with the ceiling raised to **2600×3600**, which does not bind for A4 or US Letter, so those render at full 300 DPI (2482×3510 and 2550×3300). The reported field now extracts as `"Page 2"` at 0.95 confidence, and Textract's per-page word confidence improved on every page of that document (98.36→98.92%, 93.46→95.11%, 95.86→96.83%, with words below 90% falling 11→5, 41→39 and 39→31). **This costs essentially nothing.** The old ceiling was justified as a token-consumption control, but Bedrock downscales images to its own long-edge ceiling before tokenizing, so LLM spend saturates — measured end to end across classification, extraction and assessment, going from 897×1269 to 2000×2829 moved total input tokens by 1.4% (22,598 → 22,914). What it does cost is memory: an A4 page is ~26 MB while in flight, so ~520 MB concurrent at the default `ocr.max_workers: 20` against the OCR function's 4096 MB. If you raise `max_workers` substantially, watch that function's memory metric — and prefer lowering `max_workers` over lowering `dpi`. ⚠️ **Re-running OCR on an existing document can now yield more text than before**, so extracted values for affected fields may change from `null` to a real value; stored page images are also larger, which increases S3 usage for the working bucket. To restore the previous behavior set `ocr.image.dpi: 150` with `target_width: 951` / `target_height: 1268`. Note that these two knobs interact and the ordering trap is easy to hit: `target_width`/`target_height` **cannot raise** resolution, because the page is rendered at `dpi` first and images are never upscaled — raising the ceiling without also raising `dpi` is a no-op. To cut LLM cost, downscale for the prompt via `classification.image` / `extraction.image` / `extraction.confidence.image` and leave OCR at full fidelity. See [docs/ocr-image-sizing-guide.md](docs/ocr-image-sizing-guide.md).

- **The dependency-vulnerability gate is green again.** `make dep-audit` — and with it every CI pipeline, on `develop` as well as on every branch — was failing on five HIGH advisories in two transitive packages nobody had changed: `mistune` (`GHSA-6m44-fpc8-c3rq`, RecursionError DoS via excessive emphasis markers, reached through `nbconvert` ← `jupyter`) and `fast-uri` (four host-confusion / SSRF advisories, reached through `ajv` in the web UI). `mistune` is bumped 3.3.0 → 3.3.4 in `lib/idp_common_pkg/uv.lock`; `fast-uri` is pinned forward via the existing `overrides` block in `src/ui/package.json`, constrained to `^3.1.6` so it stays on the 3.x line rather than taking the 4.x major that a bare `>=` would have pulled under `ajv`.

- **Multi-page documents are no longer split into one section per page ([#653](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/653)).** With `sectionSplitting: llm_determined` (the default), an **unpaginated** multi-page form or statement was fragmented into a separate section per page. Measured on **DocSplit-Poly-Seq** (500 packets, 7,330 pages, 2,027 sections, 5,000 packet-runs), split accuracy on multi-section packets improves on four of five models and regresses on none: Qwen3-VL +0.117, Claude Opus 5 +0.040, Nova 2 Lite +0.030, Claude Sonnet 5 +0.013 (all p<0.05, paired bootstrap + Wilcoxon), gpt-5.6-sol +0.004 (ns). Under-split rate is 0.000 in all ten cells, so the over-merge guard holds, and page-level *class* accuracy moves at most 0.015 — the change touches boundaries only. On #653's reported two-page form, Sonnet 5 goes from 6/24 to 10/10; on a 4-page packet holding two copies of one form, 1/10 to 5/5. Unpaginated documents are the worst case: a 3-page statement was kept whole in **0 of 5** runs before the fix and **3 of 5** after. It was silent: every row still came back, so completeness read 100% and the status read COMPLETED; what changed was the output shape (a result file, evaluation section, reporting row and UI section per page, with null scalars on the continuations). The cause was the prompt, not the model: each page is classified in an **independent** call that never sees the previous page, yet the prompt asked whether this page "continues the previous document" — a question the input cannot answer, and failing runs said so in their own reasoning. `classification.task_prompt` now asks a question one page *can* answer — is this a **first** page? — from pagination, then an opening identity block, then continuation evidence. Two clauses are a matched pair: one stops over-splitting, the other stops back-to-back copies of one form collapsing into a single section. ⚠️ **Still incomplete.** An unpaginated multi-page document is split roughly 40% of the time even with the fix — the rules lean on pagination markers, so corpora whose scans lack them (RVL-CDIP, for example) benefit least. `classification.contextPagesCount: 1` is not the answer: on the 4-page two-copies packet it scores **0/5** on its own by merging all four pages. Separately and more significantly, `llm_determined` over-splits **1.5×–2.3×** on real-world packets *regardless of prompt* — a larger, separate problem that these rules narrow but do not close. The rules sit inside the prompt-cache prefix, so they are not re-billed per page. ⚠️ **If you have customized `classification.task_prompt`, your stored prompt wins and you will not get this fix** — re-apply it, or reset the prompt to the default. The presets that pin their own prompt (`config_library/*/ocr-benchmark`, `scripts/sdlc/config/nuveen.yaml`, the notebook example configs) were on the wrong side of exactly that rule and are now synced, with a repo-wide guard (`scripts/tests/test_classification_prompt_copies_in_sync.py`) to keep them there. Reported by @sindhu-achuthan; root cause, fix and benchmark by @mmiakashs.

- **A class with a single top-level group scored with no confidence at all, and the report showed that as "nothing to measure" rather than as a loss ([#713](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/713)).** Evaluation strips a synthetic wrapper key that assessment sometimes puts around an object's confidence map (`{"Item #6": {…}}` against an extracted `{"LineItemRate": …}`) — without that, every confidence lookup misses. But it decided what was a wrapper from **key count alone**: one non-metadata key holding two or more confidence children. A schema whose top level declares one object property (a single `$ref` group, common in form-style classes) is indistinguishable from a wrapper by that test, so it was unwrapped too and **every** confidence score beneath it was dropped — `prediction_confidences` came back empty and AUROC, ECE and Brier were all `null` at coverage 0.0. The unwrap is now gated on evidence from the data rather than shape: the key must name no field that exists in the extracted value (a real field appears on both sides; a synthetic envelope only on the confidence side) while its contents *do* name fields that exist. Accuracy scores are unchanged — values always passed through intact, only the confidence did not. ⚠️ Affected classes reported **no** confidence calibration before this fix and report real numbers now, so **calibration metrics on runs recorded earlier are not comparable with current ones** for those classes; accuracy and field scores remain comparable, and the results blob shape is unchanged (`stickler_result_version` stays `1.0`).

- **A retired Bedrock model is no longer offered in the model picklists ([#708](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/708)).** `us.anthropic.claude-3-5-haiku-20241022-v1:0` reached end of life — `Converse` returns `ResourceNotFoundException: This model version has reached the end of its life` (verified live in us-west-2) — but it was still listed in the configuration editor's model enums and in `KnowledgeBaseModelId`, so selecting it produced a runtime failure several stages into a document rather than a validation error at save time. It is removed from both template enums, from `config_library/pricing.yaml`, from the `<<CACHEPOINT>>` support list, from the deploy-time US→EU model swap table, and from the docs. `config_library/model_config_limits.yaml` is unchanged: the retired ID had no entry of its own, it matched the shared `claude-3` pattern that the still-offered Claude 3.x IDs also use. A new build gate (`scripts/sdlc/tests/test_retired_models_not_offered.py`) fails if a retired ID reappears on any of those surfaces. ⚠️ **A stored configuration that still pins this model keeps loading, and `idp-cli config-validate` now rejects it.** Model IDs are plain strings, not a closed enum, so nothing revalidates a stored configuration against the picklist — removal hides the trap without breaking a config that already fell into it, and neither a stack update nor a configuration save is affected (the same contract as the earlier Sonnet `:1m` and Claude 3.x/4.0/4.1 picklist retirements). What does change is `config-validate` / `client.config.validate()`, which checks model IDs against `config_library/pricing.yaml`: pinning a retired model is now reported as `valid: false` — "invalid model ID" — which is the point, since the alternative was discovering it mid-document. Repoint the affected stage at a current model; `us.anthropic.claude-haiku-4-5-20251001-v1:0` is the direct replacement. The related `us.amazon.nova-premier-v1:0` "marked by provider as Legacy" error is **account-scoped**, not an end of life, so that model stays selectable; both failure shapes are now described in [docs/configuration.md](docs/configuration.md#retired-and-legacy-bedrock-models).

- **Test Studio's "Estimated cost" no longer under-reports a run whose documents finish more than a day after it started.** The cost query partition-pruned `metering` to a fixed two days — the date embedded in the test-run ID plus one. That was correct while `metering.date` meant *queue* time, but it now means **completion** time, so a run held open by human review, throttling, or sheer size had its later completions fall outside the window and silently vanish from the total: no error, just a number that was too low. The window is now derived from the run's own `CompletedAt` (already recorded on the run) and spans its start date through the day its last document completed, plus one for a metering row written just after a near-midnight completion. A run that finishes the same day — nearly all of them — prunes to exactly the same two partitions as before, so nothing gets slower. The span is capped (7 days; `COST_QUERY_MAX_PARTITION_DAYS` on the resolver raises it) so an abandoned run can't scan the whole lake and re-break the section by timing the query out, and a capped window logs a warning naming the cost as an under-estimate rather than reporting it as fact.

- **A concurrency-drift misconfiguration can no longer wedge document intake permanently.** Correcting a leaked workflow-concurrency counter requires a drift sample to survive from `RECONCILE_GRACE_SECONDS` up to `RECONCILE_SAMPLE_MAX_AGE_SECONDS`. Those are independent stack settings, and with the max-age below the grace period that window is empty: every sample is discarded as stale before it can mature, so the counter is never reconciled and — once it reaches `MAX_CONCURRENT` — the stack stops admitting documents with no self-healing path, which is the exact state reconciliation exists to escape. The max age is now clamped to at least twice the grace period at start-up, with a warning naming both values. Defaults were already safe (4x); this only affects a deployment that set both by hand.

- **A custom domain now serves the Web UI's data operations, not just its pages.** With `CustomDomainUrl` set under `WebUIHosting=APIGateway`, the bundle still called the raw `execute-api` hostname, which a browser reaching the app through the vanity domain often cannot resolve at all in `ApiGatewayVisibility=PRIVATE` — the app shell loaded and every data call failed. The API base is now the custom domain. **Action on upgrade:** the domain needs both base-path mappings to the `api` stage (empty *and* `api`); see [Custom domain](docs/deployment-private-network.md). Note that once `CustomDomainUrl` is set the raw `execute-api` URL still serves the app shell, but that shell's data calls now target the vanity host — so **access the app via the vanity domain only**; the execute-api URL will load and then fail every operation. CloudFront-hosted and non-custom-domain deployments are unchanged.
...
- **Test Studio: files dropped directly into an existing test-set folder in S3 no longer stayed invisible.** `getTestSets` short-circuited already-registered folders (`if prefix in existing_test_sets: continue`), so a manual `input/` / `baseline/` add updated neither `fileCount` nor the input↔baseline pairing check. Discovery now reconciles registered folders (skiplisted rather than allowlisted, so a legacy row without a `status` attribute is reconcilable rather than permanently invisible; capped at 25 probes / 5 s of budget per request mirroring `_reconcile_label_state`, with a 30 s warm-container TTL keyed on a per-prefix memo so the UI's 3 s fast poll doesn't repeat two paginated `list_objects_v2` calls per registered set on every tick — the memo resets on cold start). The Refresh button does NOT bypass the TTL; a direct-S3 add can be up to 30 s stale in the UI. Signature is `input_count:baseline_count:max_last_modified|src=<synthetic\|uploaded>` — the marker check is folded in so a late `.source` write from a synthetic generator still invalidates the fast path. Reconcile **never** hard-fails partial input↔baseline pairing on an already-registered row — that signal is "botched upload" and belongs to first-discovery only — and honors the validator's transient-error sentinel at every call site (first-discovery, `_reconcile_label_state`, and `remove_documents_from_test_set`), so an S3 throttle can no longer write a poisoned FAILED row, demote labeled → unlabeled, or overwrite `fileCount` with 0. An `AccessDenied` on the `.source` marker is treated as a persistent IAM misconfiguration and logged at ERROR naming the missing `s3:GetObject`, distinct from throttling. The extractor Lambda binds `test_set_id = None` per record and guards its FAILED status write, so a malformed S3 event no longer NameErrors and a bad record following a healthy one no longer flips the previous set to FAILED. Reconcile only rewrites `source` when the current value is marker-owned (null / `uploaded` / `synthetic`), so stack-managed provenance strings (`huggingface:*`, `confbench:*`) are preserved. Adds `updatedAt` (an ISO timestamp of the row's last reconcile — currently for API consumers, not yet surfaced in the UI) to the `TestSet` GraphQL type.

- **Saving a configuration through the CLI or SDK recorded no revision at all, and deleting a profile that way orphaned its history in S3.** Revision history needs two things: the counters and index in DynamoDB, and the recorded configurations in S3 under the Configuration bucket. The SDK told `idp_common` about the table but never about the bucket, and a missing bucket is how the revision store is *switched off* — so `idp-cli config-upload` silently cut nothing (a profile maintained by CLI had no history and none of the protection revisions exist to give), `config-revisions` reported "no revisions" for profiles that had several, and `config-delete` left every revision body behind, paying storage for a profile nothing could reach and letting a later profile of the same name appear to inherit the deleted one's history. The Lambdas always had both variables set, which is why this only surfaced when the CLI was pointed at a live stack. Requesting history from a stack that genuinely has no Configuration bucket now says so rather than returning an empty list.

- **`idp-cli config-list` failed outright if any one profile predated the `IsActive` attribute.** DynamoDB omits an attribute that was never written, so the key came back present with value `None`, the intended default never applied, and validation rejected `None` for a boolean — taking down the whole listing rather than the single row. Also found by running the CLI against a live stack.
...
- **A section holding several documents no longer discards all but the first** ([#687](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/687)). When the model returned a JSON array for a single-object schema, the previous code stored it under a `raw_array` key that nothing ever read, marked the section FAILED, and reported a misleading "0/N schema fields populated". Every record had been extracted correctly and all of them were thrown away. Now every instance is preserved, the first populates `inference_result` so no downstream consumer changes, and the section is flagged for review.

- **`sectionSplitting` is no longer ignored for single-class configurations** ([#686](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/686)). A config defining exactly one document class short-circuited classification and produced one section spanning every page, whatever `sectionSplitting` said — so `page` did nothing and multi-record packets collapsed by construction. All three strategies now behave as documented. **Cost note:** `llm_determined` is the default and now performs real boundary detection, so a **multi-page** single-class deployment that never set `sectionSplitting` will begin making classification inference calls where it previously made none. Single-page documents are unaffected (there is nothing to split, so the call is skipped). Set `classification.sectionSplitting: disabled` to keep the zero-inference path when one input file is always exactly one document. Measured classification cost is ~$0.0008/page — +1.5% of a list-heavy document, but the dominant cost on a page-heavy, field-light one.

- **`sectionSplitting` is no longer ignored when a document's *name* matches a class regex** ([#705](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/705)). A document whose ID matched a class's `x-aws-idp-document-name-regex` took a second short-circuit with the identical defect to [#686](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/686): one section spanning every page, whatever `sectionSplitting` said, so `page` did nothing and a packet of several same-class records collapsed into one section with extraction returning only the first. The filename says *what* the packet holds, never *where* each record starts, so it can substitute for the class decision but not for boundary detection. All three strategies now behave as documented, and the regex match stays authoritative over the class — the model's own class output is discarded, only its per-page boundary signal (or, under holistic classification, its segment ranges) is used. **Cost note:** `llm_determined` is the default, so a name-matched **multi-page** document will begin making classification inference calls where it previously made none — for those documents the regex is no longer a "skip the LLM" shortcut. Single-page matches are unaffected and keep the zero-inference path. Set `classification.sectionSplitting: disabled` (or `page`) to keep it inference-free when one input file is always exactly one document.

- **`update_document_section` no longer erases processing issues.** That write replaces the whole section record, so omitting `ProcessingIssues` was deleting issues an earlier stage had written — extraction persists through this path, so section status icons stayed blank until the collate step rewrote the document.

- **A section skipped because its class is excluded from processing now shows the "Skipped" badge that explains why its panels are empty** ([#704](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/704)). The flags existed end to end except for the write: no DynamoDB writer put `Excluded` / `ExclusionReason` into the item, so the badge could never render and an excluded section was indistinguishable from one that had simply produced nothing. All three section writers now persist them — the full document item, the atomic single-section update (a whole-record replace, which was additionally *erasing* the flag when extraction wrote the section it had short-circuited), and the run/version snapshot — and the version resolver's allow-list passes them through so historical runs keep the badge too. Sections that are not excluded are unchanged.

- **A classification failure's per-page detail now reaches the stage that reports it** ([#706](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/706)). `Document.metadata` was a declared field missing from both `to_dict` and `from_dict`, so `metadata["failed_page_exceptions"]` — written expecting a later reader — existed only for the lifetime of one Lambda invocation and never crossed a Step Functions hop. Same class of defect as the per-page `document_boundary` signal. The key is omitted when empty, so payloads for documents that set no metadata are unchanged.

- **Evaluation reports no longer contradict themselves on list-valued documents ([#625](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/625)).** Parent list-field verdicts used to read ✓ while the expanded drilldown showed red child rows, and section metrics could report 100% precision on documents with most of a list wrong. Parent, section, and run-level views now derive from the drilldown itself, so they cannot disagree; historical runs recorded on v0.6.3–v0.6.6 should be re-run for accurate comparison. Per-doc `results.json` now carries a `stickler_result_version` stamp so downstream tooling that reads the blob directly can distinguish old and new numbers on the same document. See [docs/evaluation.md](docs/evaluation.md).

  **Known limitations** on list-valued extractions that remain — both blocked on upstream Stickler and documented in [awslabs/stickler#275](https://github.com/awslabs/stickler/issues/275):

  - A list nested inside another list's items (e.g. `Invoices[i].LineItems[j]`) has failures at the inner level invisible to every Stickler rollup surface, so section metrics on that shape still misreport. `overall_score` on the document reflects the truth.
  - Only when Stickler pairs every predicted item to a ground-truth item and *every* pair scores 0 (right count of items but every value entirely wrong) do section counts drop the rejected leaves — precision reads 100% on that specific pathology. The document's `overall_score` and per-item drilldown reflect the truth.

- **A field added to the API's `Section` type was invisible to the Web UI, and nothing failed to say so ([#711](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/711)).** The UI held two hand-written Section shapes — one in `types/documents.ts`, one local to the sections table — and neither derived from the generated GraphQL types, so regenerating after a schema change did not make the new field readable and no compile error or test pointed at the gap. `Excluded` / `ExclusionReason` had already gone missing from one of them, `InstanceCount` needed hand-wiring into both, and the same class of miss previously blanked confidence alerts in the historical-version view. Both shapes now derive from the generated `Section`, so a new field arrives with `npm run codegen`; a new drift test additionally fails when any `Sections { … }` selection set falls behind the schema. That test found three selection sets already behind: the historical-version query was not requesting `Excluded`/`ExclusionReason`, and two others were not requesting `ProcessingIssues` — so their generated result types omitted the section **Skipped** badge and quality-status data.

- **A BDA job whose completion callback never arrives no longer hangs the workflow forever ([#755](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/755)).** In BDA mode (`use_bda: true`) the `BDA_InvokeDataAutomation` step uses `.waitForTaskToken`, so it blocks until an external `SendTaskSuccess`/`SendTaskFailure` returns the token — and it had **no `TimeoutSeconds`**. Any break in that path (a stuck BDA job, a lost task token, a dropped EventBridge delivery, the completion handler erroring before it responds) left the execution waiting indefinitely, in the worst possible shape for monitoring: it never fails so emits no `ExecutionsFailed` (**`WorkflowErrorsAlarm` blind**), never completes so emits no `ExecutionTime` (**`SlowExecutionsAlarm` blind**), and holds a workflow-concurrency slot throughout — so a stack could lose capacity with every alarm reading `OK`. The step is now bounded by a new **`BDACallbackTimeoutSeconds`** parameter (default **7200**, i.e. 2 hours; range 60–86400), and because it catches `States.ALL` and routes to the fail state, tripping the bound produces a genuinely FAILED execution that fires `WorkflowErrorsAlarm` and lets the workflow tracker release the slot and mark the document `FAILED`. The timeout is deliberately **not** retried — re-invoking would start a second BDA job, paying twice and risking double-processing. Raise the parameter if very large packets hit `States.Timeout` on documents that were progressing normally; the asymmetry favours a generous value, since too high only delays detection while too low fails healthy work. **Pipeline mode is unaffected** (direct Lambda invokes with their own timeouts cannot hang). See [docs/monitoring.md](docs/monitoring.md).

- **The docs no longer overstate how far the ambiguous-date protection reaches ([#717](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/717)).** `extraction.coercion` refuses to convert an all-numeric ambiguous date (`03/04/1985`) and records an `ambiguous_date` refusal — but that only engages when the model returns the **raw string**. Measured on a live stack, both Claude Sonnet 4.6 and Nova Lite resolved the ambiguity to M/D/Y themselves and emitted ISO directly, so coercion never saw a string, and there was **no refusal, no ProcessingIssue and no `metadata.coercion` entry** — the guess was indistinguishable from a correct reading. `date_order` does not help either, since it also only applies to values coercion processes. The property that holds is "*coercion* never guesses a day/month order", not "the pipeline never does". **Behaviour is unchanged** — this is a documentation fix. [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md) now states the limitation with the measurement and two free mitigations: instruct the model to return dates verbatim (which puts the value back on coercion's path and makes the refusal authoritative), and/or set `date_order` to your corpus convention — in that order, since `date_order` alone does nothing for a value that never reaches coercion. Detecting it automatically was prototyped and **rejected as not planned**: the check can only fire on evidence that the *document* is ambiguous, which on a normal US or EU corpus is a large fraction of all dates, and it cannot say which reading is correct.

- **A revision's confidence fingerprint changed depending on how the configuration had been read, and the Test Studio docs described a workaround nobody could perform ([#698](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/698)).** Every revision records a `confidenceFingerprint` — a hash of the configuration that determines what a confidence number *means* (extraction model and sampling parameters, assessment settings) — so a future release can key confidence→accuracy curves per configuration instead of per profile. It was not a stable hash. A configuration reaches `confidence_fingerprint()` either straight from a save (JSON, so `float`) or read back from DynamoDB, whose only numeric type is `Decimal`, and `json.dumps` cannot serialize `Decimal` and falls back to `default=str` — so `temperature: 0.0` hashed **three** different ways: as the number, as `"0.0"`, and as `"0"` when DynamoDB returned an unscaled zero. Since `temperature` and `top_p` are exactly the values in the hashed subset, one configuration had several fingerprints — the property a fingerprint exists to rule out. **Nothing misbehaved, because nothing keys off it yet**; this is fixed now rather than later so it cannot become a mystery for whoever implements the per-revision keying. Both fingerprints (`class_fingerprint` too, where it is likewise latent — the shipped sample configs carry no numerics under `classes` — but which is the BDA resync signal) now normalize numerics before hashing, with `bool` special-cased so `enabled: true` cannot collapse into `enabled: 1`. Fingerprints recorded before this differ from what the same configuration hashes to now, which is harmless while nothing compares them; a future consumer must read a mismatch on a pre-normalization revision as *unknown*, not *changed*.

  Separately, both [docs/test-studio.md](docs/test-studio.md) and [docs/configuration-profiles.md](docs/configuration-profiles.md) told operators to "reset the affected curve" after a model or assessment change. **There is no way to do that** — `CurveStore.reset` has no caller anywhere outside a unit test, and no GraphQL operation, CLI command or UI control reaches it. Both notes now state the limitation plainly (after changing a profile's extraction model or assessment configuration, treat that set's review-effort estimate as unreliable until fresh observations accumulate) instead of prescribing an action that does not exist, and the Test Studio note additionally records a **second, pre-existing** limitation found while investigating: the estimate reads the test set's combined curve rather than a per-configuration one, so it blends observations across every configuration the set has been labeled or scored under. That is tracked separately — it predates Configuration Profile revisions by a month and is not caused by them.
