...
- **Bedrock token cost from control-plane agents is now measured, not always zero.** `control_plane_hourly` has always had `bedrock_tokens_in / bedrock_tokens_out / est_bedrock_cost` columns, but no in-repo Lambda ever populated them because nothing called `emit_control_plane_cost_metric` with Bedrock args — Track C in `docs/reporting-sql-layer.md` §Phase-2. A new reusable Strands hook, `ControlPlaneCostHook`, registers on `AfterInvocationEvent`, reads the agent's cumulative `event_loop_metrics.accumulated_usage`, emits the per-invocation delta (not the cumulative total) with the concrete `bedrock_model` used at inference time, and is registered on the analytics chat agent (component `analytics-agent`) and the marketplace idp-monitor's `monitor-agent`. New CloudWatch metrics `IDPControlPlane/BedrockInputTokens` and `IDPControlPlane/BedrockOutputTokens` (dimensioned by `Component`, `FunctionName`, `Model`) start emitting the moment the hook is deployed; the rollup Lambda picks them up on its next hourly tick. Latent gap: the hook doesn't yet read `cacheReadInputTokens` / `cacheWriteInputTokens` — no in-repo agent has prompt caching enabled today, so the counters stay accurate until the first one does; wiring cache tokens is tracked as a follow-up.

- **`test_results_resolver` opts into Athena result reuse on the async cache-update path.** The Athena queries in this resolver (`_get_evaluation_metrics_from_athena`, `_get_cost_data_from_athena`) run exclusively from the SQS-driven `_aggregate_test_run_metrics` worker that populates the DynamoDB metadata cache (`testRunResult`); the `getTestRun` read path returns from that DDB cache and never touches Athena, so this change does not affect Test Studio page-load latency (the DDB cache already serves it in sub-second). What the change does cover is the narrow case where the same run is re-aggregated by the SQS worker within 60 minutes — for example, a page load detects the cached metrics are missing a key that a later release added and enqueues a re-aggregation, and the 5-minute worker-side throttle then admits a second one before the shape stabilises. On that second run, the workgroup's off-by-default `ResultReuseConfiguration.ResultReuseByAgeConfiguration(Enabled=True, MaxAgeInMinutes=60)` now applies. Every query in the resolver is scoped by `document_id LIKE '<test_run_id>/%'` and reaches Athena only after status is `COMPLETE` / `PARTIAL_COMPLETE`, so the reused result set is over sealed data. The 60-minute window is set on `_execute_athena_query`'s default parameter — a future caller that wants a shorter TTL or reuse off can pass `reuse_max_age_minutes` explicitly; no in-tree caller does yet.
