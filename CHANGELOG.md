...
- ⚠️ **The same group requirement now covers the document reads that lead to content, not only the ones that return it.** `listDocumentsDateShard`, `listDocumentsDateHour`, `listDocumentVersions`, `getDocumentCount`, `getStepFunctionExecution` and `getChatMessages` require membership of at least one of Admin, Author, Reviewer, Annotator or Viewer. They return no extracted value themselves, which is why they were left open — but an object key, a run's section and page URIs, the list of extracted attribute names, a model-written description of what a page contains, and the workflow execution input are steps of one chain rather than separate disclosures, and on a deployed stack that chain composed end to end for a self-registered account in no group, starting from no knowledge of the deployment at all. **Before upgrading, place every user who needs document access in a group** (User Management, or `idp-cli`): a user in no group loses the document timeline, the version history panel and the workflow view along with the document reads. Platform, profile and feature-catalog reads stay open to any authenticated user, and each now records why that is the right answer for the individual operation rather than for the group it sits in. ⚠️ **This still gates the API, not the document buckets** — the Identity Pool's authenticated role grants every signed-in user direct S3 access to them regardless of group, so the object bytes remain reachable without any API call; the two candidate fixes and their obstacles are set out in [Identity Pool group scoping](docs/planning/identity-pool-group-scoping-plan.md) ([RBAC](docs/rbac.md), [#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033)).
