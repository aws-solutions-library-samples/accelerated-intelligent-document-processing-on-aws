...
- **Confidence scoring stops retrying a batch that can never fit, instead of running the Assessment Lambda into its 900-second limit three times** ([#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894)). The self-healing ladder halves any confidence batch whose response the model truncated, which converges only while a smaller batch can fit; with a class marked `x-aws-idp-multi-instance: true` one row of the outer list is a whole document instance carrying its own long inner list, so a single-row call still truncated and halving could not help. The ladder re-ran that impossible call — a ~60 s model call per attempt, times the retry rounds and the bisection tree — until the Lambda timed out, after which Step Functions retried the section twice more (`Sandbox.Timedout` ×3) and the document stuck in `ASSESSING` after a 45-second extraction. It now stops as soon as a one-row call truncates: the same-model retry rung is skipped, at most one escalation round runs (a larger output cap is the only remedy that can succeed), and the section reports an error-severity `assessment_row_too_large` naming the confidence model, its output-token cap, the list field and class, the offending row's approximate serialized size, and that lowering `extraction.confidence.list_batch_size` cannot help. ⚠️ The batch sizer still measures only the outer row's column count (it logs `cols=2 per_row~80` for a row holding 100 transactions), so **a multi-instance class with a long inner list still cannot be fully assessed** — #894 stays open for the sizer; what changes is that the run fails fast with an actionable message rather than burning ~45 minutes and stalling the document. Sections whose scored rows fall materially short of the extracted rows also now report `assessment_coverage_incomplete` (warning past 5% of rows unscored, error at 25% or more, with per-field counts), a shortfall the completeness gate computed but every caller discarded. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).

- **A failed confidence pass no longer throws away the extraction it was scoring** ([#901](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/901)). Confidence scoring runs after extraction has written its results and been paid for, but a deterministic `ValidationException: Input is too long for requested model.` from the confidence model failed the whole document — two observed runs discarded $17.34 and $7.05 of extraction that had returned 1,200 of 1,200 rows at 1.000 cell accuracy, in order to report the loss of the advisory part. A deterministic (non-retryable) failure in the Assessment step now degrades instead: the extracted data is returned, the document succeeds, and the missing confidence is recorded as an error-severity `assessment_failed_confidence_unavailable` processing issue on the section, visible in the Sections panel and the Processing Report. Transient failures — throttling, read timeouts, 5xx — are untouched and still raise so Step Functions retries the section. Upgrade-visible: a document that previously failed outright now completes with an error-severity processing issue and no confidence values on the affected section, so HITL confidence routing and the UI threshold signals do not apply to it. Automatically re-batching an oversized confidence input so the pass succeeds rather than degrades is still open as part of #901. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).
