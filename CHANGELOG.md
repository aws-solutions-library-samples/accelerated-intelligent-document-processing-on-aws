...
- **A section whose extraction failed now says so in the Sections panel, instead of showing nothing** ([#1049](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1049)). Every extraction failure raises, and both extraction Lambdas wrote the section to the tracking table only *after* the call returned — so on a failure the write never happened and the section's record still held whatever classification left there. Opening the document showed no status icon, no issue and nothing identifying which section had failed; the explanation existed only in the Step Functions cause and the logs. Each entry point now persists the section before re-raising, so the failure is recorded on it as an error-severity `extraction_failed` issue whose details carry the exception's own explanation and remedy. This covers the whole family — an input too large for the model, an image Bedrock rejected, an invalid tool-use sequence, and a materially incomplete list under `extraction.row_shortfall_action: fail`. The last of those is what made it worth fixing: it deliberately writes the partial rows and an `extraction_rows_below_ocr_estimate` diagnosis before failing, and that diagnosis now reaches the record the UI reads as well as the section's `result.json`. The exception itself is unchanged — the Step Functions cause still names the rows extracted, the OCR row estimate and the remedy — and a tracking-table write that fails cannot replace it. A failed section's per-shard results are deliberately kept rather than released, since they are what lets a retry re-infer only the shards that did not finish. See [where a failed section shows up](docs/extraction-and-confidence.md#where-a-failed-section-shows-up).
