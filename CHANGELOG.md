...
- **Annotating a test set now creates a version of it, explicitly, and preserves the labels it moves away from.** The requirement, as it was put: "if we start annotations on any dataset, even if we still have ground truth, we're effectively committing that we'll be creating a new version of the dataset. So I think we need to hash the queue link to a version that is a transition from existing state to next state." Framed as a link problem; the defect underneath was larger. `publish_test_set_version` wrote a **DynamoDB row and copied no S3 objects**, while annotation wrote straight to `{id}/baseline/` — so a version number was immutable and its *content* was not, and a run stamped `TestSetVersion = 3` could not be reproduced against the labels it actually scored.

  **Start annotating** opens a version transition, shown as `v1 → v2`, and the editor stays read-only until it is open. That ordering is the point: editing first and versioning after would record what the labels became rather than what they were. Agreeing snapshots the current baselines to `{id}/versions/{n}/baseline/`, so the number refers to bytes; a set that arrived with ground truth and was never published has that state captured first as **As uploaded**, since otherwise the labels a set was uploaded with are exactly the ones overwritten with no record. A run pinned to a version now stages *that version's* snapshot, falling back to the live baselines for every set and historical run that has none. **Copy queue link** carries `?v=`, and a link from a transition that has since been published says so rather than silently enrolling you in a different one.
