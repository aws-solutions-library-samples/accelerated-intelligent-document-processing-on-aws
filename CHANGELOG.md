...
- **Test Studio: files dropped directly into an existing test-set folder in S3 no longer stayed invisible.** `getTestSets` short-circuited already-registered folders (`if prefix in existing_test_sets: continue`), so a manual `input/`/`baseline/` add updated neither `fileCount` nor the input↔baseline pairing check. Discovery now reconciles registered folders (allowlisted to `status in {COMPLETED, FAILED}` and `labelJobStatus != RUNNING`, with a 30 s warm-container TTL keyed on a per-prefix memo so the UI's 3 s fast poll doesn't repeat two paginated `list_objects_v2` calls per registered set on every tick — the memo resets on cold start). The Refresh button does NOT bypass the TTL; a direct-S3 add can be up to 30 s stale in the UI. Signature is `input_count:baseline_count:max_last_modified|src=<synthetic\|uploaded>` — the marker check is folded in so a late `.source` write from a synthetic generator still invalidates the fast path. Genuinely-broken pairing on a new folder still flips to `FAILED`; three categories of already-registered row are protected from that hard fail — rows inside the labeling lifecycle (`labelState in {draft, labeled}`, so the draft-labeling harvester's per-poll partial baselines and `clear_draft_labels`'s intentional partial pairing are not flagged), and rows carrying an unmanaged provenance value (`source not in {null, uploaded, synthetic}` — HuggingFace-backed benchmark deployers such as `fake-w2`, `ocr-benchmark`, `fcc`, `docsplit`, and the ConfBench extension can leave partial coverage on completion and don't participate in the UI's `labelState` flow). Reconcile only rewrites `source` when the current value is marker-owned (null / uploaded / synthetic), so those stack-managed values are preserved through every reconcile. Adds `updatedAt` (an ISO timestamp of the row's last reconcile — currently for API consumers, not yet surfaced in the UI) to the `TestSet` GraphQL type.
