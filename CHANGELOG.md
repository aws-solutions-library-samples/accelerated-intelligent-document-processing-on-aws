...
- **Four authorization statements in the RBAC reference corrected.** `listUsers` is Admin-only rather than filtered per caller; `getAgentChatMessages`, `updateChatSessionTitle` and `updateAgentChatMessage` are declared in the GraphQL schema but routed from nowhere, so the dispatcher's default-deny refuses them to every caller including Admin; and the role table is introduced as five roles, which is how many it lists ([RBAC](docs/rbac.md)).
