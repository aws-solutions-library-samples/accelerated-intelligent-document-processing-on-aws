...
- **Rule validation's character chunker had a non-terminating path, and `overlap_percentage: 0` asked for no overlap and got the maximum.** Two defects in the same pair of functions. `_chunk_text_with_overlap` computed each chunk's start from an `end` index it had already clamped to the length of the text, so once a chunk reached the end the start stopped advancing: the same tail slice was appended on every pass, and the loop terminated only when the requested overlap rounded down to zero characters. A document that reached it would hold the rule-validation Lambda allocating until the runtime killed it at the function timeout — no error, no partial result, and the whole invocation billed. It is a latent trap rather than something a realistic document hits: the character chunker is only the fallback for text in which the page parser finds no pages at all, which takes `<page-number>` markers with no content between any of them plus a total length over the chunk budget, and `_chunk_pages_with_overlap` — the function every document does go through — always terminated. The loop now decides a chunk is the last one before moving the start, and the overlap is bounded below the chunk size so that `overlap_percentage: 100`, a legal setting, still leaves a chunk's worth of forward progress and logs that it was reduced. Separately, the page chunker's single-page overlap branch took the tail of the previous page with `page_content[-overlap_size:]`, and `x[-0:]` is `x[0:]` — the whole string — so **`overlap_percentage: 0` repeated the entire previous page rather than none of it.** On a document whose pages are large relative to `max_chunk_size` that roughly doubled the prompt tokens of every fact-extraction call, and the extra content was a verbatim duplicate of what the model had already been shown. Nothing failed and no verdict looked wrong; the only symptom was the bill, moving in the opposite direction to the knob that was turned. That branch now repeats nothing when nothing was asked for ([#1090](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1090), [#1091](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1091)).
