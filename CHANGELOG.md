...
- **Advanced extraction can stop sending the document schema three times ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).** It transmits the class schema **three times per request** — as text in the task prompt, restated in the system prompt, and as the extraction tool's input schema, ~5,700 tokens on the lending `Payslip` class where ~1,600 would do. Two settings remove the two duplicates: `extraction.agentic.restate_schema_in_system_prompt: false` drops the system-prompt copy (byte-identical to the tool schema, so it can cost nothing — and measured as costing nothing), and `extraction.agentic.prose_schema` shrinks the prompt copy. On Simple extraction the same duplication exists only with forced tool use on, where `extraction.forced_tool.drop_prose_schema: true` removes it. Turning them off leaves the schema on the wire exactly **once**.

  `prose_schema` has **three** settings rather than two, because the prompt copy is not pure duplication on the Advanced path: the tool schema there is built from a Pydantic model generated from your schema, and that conversion keeps each *field's* description but drops any description belonging to an *object* — so your class description and any group description reach the model **only** through the prompt (verified on the lending preset: `Payslip` loses 3 of 37, and four of the preset's six classes declare only a class description and lose all of it). `minimal` keeps exactly those plus the field names (~372 tokens instead of ~1,485); `names` keeps the field names alone (~176). Prefer `minimal` unless your classes carry no class or group descriptions. On Simple extraction the tool schema is built from your schema directly and loses nothing, so dropping the prompt copy there is a clean deletion.

  ⚠️ **All of it defaults to today's behaviour, and these are A/B knobs rather than recommendations:** restating a schema in prose often improves how closely a model follows it, so measure **completeness** on your own documents before turning anything off — a token saving that loses list rows is a loss. `drop_prose_schema` applies only to requests that really do carry the tool schema; on a route that cannot (a custom Lambda hook, GPT-5.x) the prompt copy is kept rather than leaving the model with no schema at all, and the section records that it was asked for and why it was not applied. Note also that all copies sit inside the prompt cache, so the dollar saving is roughly a tenth of the token count and the first request per class after a change is a cache miss; the real benefit is context-window headroom, which is what pushes a long document into extra shards. Editable in the Web UI under **Configuration → Extraction → Advanced extraction settings** and **→ Schema Enforcement (experimental)**. Measured results: [docs/benchmarking/config-guidance.md](docs/benchmarking/config-guidance.md); reference: [docs/extraction-and-confidence.md](docs/extraction-and-confidence.md).
