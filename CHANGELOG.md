...
- **Cost reporting no longer overcharges cache reads: a model's price is resolved by exact id, and an unpriced service is reported as unpriced instead of $0.00** ([#926](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/926)). The pricing lookup behind the `metering` table fell back to a case-insensitive substring match in both directions when it found no exact entry, so a model id with no pricing row of its own bound to a *related* row and, within it, `cacheReadInputTokens` bound to that row's `inputTokens` price — the first unit every Bedrock row lists. Cache reads cost a tenth of fresh input on Claude and a quarter on Nova, so reported cost was too high, never too low: `eu.amazon.nova-2-lite-v1:0:flex` and `:priority` (both selectable, both cache-capable, neither priced) billed cache reads at the base-tier fresh-input rate, 8x their real price, and a cache-heavy Sonnet 4.5 document measured 0.09735 correct against 0.39435 reported, a 305% overstatement. Resolution is now by exact key with progressively shorter `/`-delimited suffixes, longest match winning — the same rule the benchmark harness already used — with no substring step. A unit an existing pricing entry deliberately omits still costs $0.00, but a service with **no** entry now writes SQL `NULL` to `unit_cost` and `estimated_cost` with a WARNING naming it, so `WHERE unit_cost IS NULL` finds the gap instead of it reading as free usage; `SUM()` totals are unchanged either way. Three missing pricing rows were added, including `us.anthropic.claude-3-5-sonnet-20240620-v1:0`, which shipped presets pin and which had been costing $0.00 outright. ⚠️ A model with a complete pricing entry was always priced correctly, so this affected only deployments that used a model id with no entry of its own or an entry missing `cacheReadInputTokens` — among shipped selectable models, the EU Nova 2 Lite service tiers. Cost figures for a cache-heavy workload on one of those, reported by an earlier release, are high and should be re-run. See [Reporting Database](docs/reporting-database.md#cost-calculation-process).
