...
- **Every queue and bucket resource policy now refuses non-TLS calls, and a gate enumerates the class instead of trusting the convention** ([#964](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/964)). Nine of the ten `AWS::SQS::QueuePolicy` resources in `template.yaml` carried an explicit `Deny` conditioned on `aws:SecureTransport: false`; the data-mart rollup dead-letter queue's policy did not, so it was the one queue in the stack where a caller pointed at an HTTP endpoint would have been served rather than refused. SQS endpoints are HTTPS and every AWS SDK uses TLS unless deliberately reconfigured, so this is defence in depth rather than an exposed path, but the deny is what makes the guarantee explicit and auditable, and the queue's messages can carry document identifiers and processing metadata. The omission survived because nothing enumerated the class: `lib/idp_sdk/idp_sdk/_core/s3_security.py` covers buckets the CLIs create through the API, but no gate walked the templates, so the control was applied nine times by hand and missed once with nothing able to tell the difference. `scripts/tests/test_secure_transport_deny.py` now discovers every queue and bucket policy by parsing the templates `scripts/discover_templates.sh cfn` finds by content — 11 queue policies and 15 bucket policies across 30 templates today, and any added later, in this template or a new one — and asserts the deny's semantics rather than its wording: `Effect: Deny`, a principal covering everyone, an action covering the service namespace, a `Bool` test of `aws:SecureTransport` against false, and a `Resource` that reaches the policy's own target. No behaviour change for a deployment whose clients already use TLS, which is every default configuration.
