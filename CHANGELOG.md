...
- **UI acceptance testing (`make uat-testing`): the first test tier that renders a page.** Every existing tier is headless by construction — `make ui-test` is Vitest in jsdom against a mocked API, `make api-test` drives the REST dispatcher directly, the primary suite drives `idp-cli`, and the `stacktest-*` probes do a single HTTP GET on `ApplicationWebURL` — so nothing could observe whether a **task is completable in the browser**. That is the class of defect which deploys clean, passes every unit test, returns 200 and answers RBAC correctly: a document wedged in `EVALUATING`, a HITL gate silently blocking an unattended run, a nav entry leading nowhere, a `Viewer` offered a Delete button the API will refuse. Closes the open "Browser/e2e UI test (Playwright)" item in `scripts/sdlc/docs/CI_TEST_COVERAGE.md`. **One command owns the lifecycle** — deploy a throwaway stack → provision one Cognito user per role → Playwright against the deployed UI → emit reports → **tear everything down**, with teardown in a `finally` so a crash or Ctrl-C still cleans up, and the stack name recorded *before* the deploy begins so a half-failed deploy is destroyed rather than orphaned; a stack passed as `STACK_NAME` is never deleted and `KEEP=1` preserves everything. **Assertions are task completion, not appearance**: `waitForDocumentTerminal()` polls the app's own status text against the real status vocabulary and treats "never reached a terminal state" as the failure, which keeps the tier insensitive to the Cloudscape churn that makes UI suites rot. **No AWS credentials needed** — form login via Playwright `storageState`, with permanent passwords set through the same `scripts/rbac_common.py` helpers `make api-test` uses, so it runs from a stock CI runner with no IAM role and no OIDC provider. Reports add **interaction cost** (clicks, field edits, navigations and the route path per scenario), counted by an in-browser capture-phase listener so it sees the extra clicks a dropdown or modal forces; reported, never asserted, as a usability signal ordinary tests have no opinion about. The harness is a **separate npm package** under `src/ui/e2e/` so Playwright and its browser download stay out of the three CI paths that `npm ci` in `src/ui`, and it adds **no `data-testid` attributes** — selectors are accessible roles and the nav's own link text. Validated against a live stack: 10/12 scenarios pass in 119s, including the APIGateway hosting variant (`WebUIHosting=APIGateway`), whose `/api/` base path the routes handle explicitly. Nightly CI in `.github/workflows/ui-uat.yml` against a persistent stack, with no `pull_request` trigger because this repo is public and a fork PR must never reach the credentials. See [docs/testing-ui-acceptance.md](docs/testing-ui-acceptance.md).
