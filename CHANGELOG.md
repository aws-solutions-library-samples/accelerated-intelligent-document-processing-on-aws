...
- ⚠️ **Nothing waited for a queued agent-transcript write, so the last messages of a turn could be abandoned when the Lambda execution environment froze** ([#1110](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1110)). Transcript messages are written on a thread pool so a DynamoDB round trip stays off the agent's critical path, and the drain path existed — `DynamoDBMessageLogger.shutdown` waits for the pool, and `DynamoDBMessageTracker.shutdown` calls it — but **nothing in production reached it**: the tracker was constructed and registered as a Strands hook, a hook registry has no teardown event, and `IDPAgent.__exit__` closed only the MCP client, so the only object with a route to the pool was unreachable from the moment it was created. This is a second loss path on the attribute the conditional-write fix above guards: that one stops writers overwriting each other, and does nothing for a write that never ran. An `atexit` handler is not an alternative and the reason is worth stating, because it is the fix most likely to be reached for: a Lambda invocation ends with the execution environment being **frozen**, not shut down — the process is not signalled and does not exit, so nothing triggers interpreter shutdown — and when the environment is eventually reclaimed the runtime is sent `SIGTERM` only for functions with a registered **external extension**. Neither agent function has one, so the shutdown phase has a 0 ms budget and the process ends on `SIGKILL`, which runs no Python. The pool is now drained at `IDPAgent.__exit__`, which every logger built in production passes through — the `with agent:` in the analytics processor, and a sub-agent's `with specialized_agent:` in the orchestrator, which is where the chat path's loggers all live. The drain is **bounded** at two seconds rather than unconditional, because for a sub-agent that exit happens mid-turn with the user waiting on the orchestrator's answer, and anything still outstanding when the bound expires is counted on the existing `AgentTranscriptMessageDropped` metric instead of being waited on indefinitely — a bounded drain that reports what it could not finish. **How much was at risk:** narrowing the pool to one worker bounds how many writes run at once, not how many are waiting, since the pool's queue is unbounded — 50 submitted at width one leaves 49 queued. What bounds it in practice is the producer: a message reaches the logger once per model or tool round trip while a write costs two DynamoDB round trips, so the steady-state queue is empty and the exposure is the write in flight plus whatever arrived during it — in other words the tail of the turn, which is the part a user is most likely to be looking at. **What a data point on the metric now means:** "the transcript may have a gap" rather than "a message was destroyed". For the two pre-existing causes the message is gone; for a drain timeout the write can still commit if that environment is thawed for another invocation, though not in time for the session reading the transcript and not at all if it is reclaimed instead. Closing the pool also releases its worker thread, which is not a daemon thread and otherwise idled for the remaining life of a warm execution environment — one per sub-agent, per invocation. See [Agent Transcript Messages Dropped](docs/monitoring.md#agent-transcript-messages-dropped).
- **A message's `sequence_number` is now unique within a job.** Every sub-agent in a turn builds its own logger on the same job record, and each counted its own ordinal from zero, so the same value named several different messages in one transcript ([#1106](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1106)). The ordinal is now the message's 1-based position in the stored transcript, computed as `len(stored) + 1` from the array the write already had to read — inside the existing conditional write, so it costs **no extra round trip and adds no second source of truth**, and the condition is what makes it exact: a write commits only if nothing appended in between, so no two stored messages can have computed the same length. A rejected append recomputes it on the fresh read and a dropped message consumes no ordinal, so the stored series is 1..N with no duplicates and no gaps. **What the collision did and did not affect:** nothing sorts on this field. The writer appends, so the stored array is already in commit order, and the UI renders that order as it is; the value reached the screen only as one component of a React list key that also contains the array index, so a repeat produced no duplicate-key warning and no visible mis-ordering. Where it was actually read was the drop-path log line the monitoring guide tells an operator to search for, and there a repeat made the identifier ambiguous — four loggers each reporting "sequence 3". Those log lines now name the message's role and timestamp instead, because a message that was never stored has no position in the transcript to name. Transcripts already stored keep the ordinals they were written with and need no migration; appends to them continue from the stored length. The conversation-memory provider behind agent chat derives its own ordinal from a microsecond timestamp rather than an instance counter, so it never had this shape and is unchanged.
