...
- **Evaluation: parent verdicts and section/document metrics now derive from Stickler's row-level `field_comparisons`, with leaf-normalized weighting ([#625](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/625)).** The v0.6.3 stickler cleanup consumed Stickler's confusion-matrix nodes for verdict + rollup, but Stickler exposes two rollup views and **neither captures every failure mode**: `cm.overall` collapses list fields to the item level (masks leaf failures inside Hungarian-paired items — sirikaro's original ✓-with-red-children case), while `cm.aggregate` drops rejected items entirely (misses list-item false discoveries). Only Stickler's row-level `field_comparisons` — the same rows the UI drilldown displays — carries a threshold-gated, per-user-config `match` verdict for every leaf. All three rollups (per-attribute `matched`, section `_stickler_counts` + derived precision/recall/F1/accuracy/FAR/FDR, and the aggregation Lambda's run-level top-level *and* per-field metrics) now come from a uniform pass over those rows, so parent, section, document, and run-level views agree with the drilldown by construction — and top-level agrees with the per-field breakdown within the run summary. Rows are weighted by leaf count when the row's value is structured (Stickler emits one row per leaf for Hungarian-paired items but one row per item for rejected/missing/extra items — without weighting a truncated 5-item / 2-leaf list would report recall 0.571 instead of the leaf-normalized 0.400, and truncation is the flagship failure mode for bank-statement / agentic-table configs). Anonymous-root rows (paths beginning with `[` or `.`) are filtered by a shared helper on both per-doc and run-level paths so a stray row can't move section metrics without also moving parent verdict. Empty-container values (`None`, `""`, `[]`, `{}`) are treated as absent uniformly. `STICKLER_RESULT_VERSION` bumps `1.0`→`2.0` since `_stickler_counts` semantics change (contract stamp fires a version-drift warning on rolling deploy or when reading a v1.0 payload). Three new golden fixtures pin the shapes: `partial_list_failure` (4-of-5 items rejected), `list_leaves_wrong_items_kept` (sirikaro's shape — items paired, leaves inside wrong), `truncated_and_hallucinated_lists` (dropped/hallucinated items — leaf-normalized recall/precision). Twelve new aggregation-Lambda unit tests and four per-doc regression tests. **User-visible caveat for historical data:** runs recorded on v0.6.3–v0.6.5 have inflated *and* deflated section metrics on list-heavy configs depending on whether the failure was leaves-inside-kept-items (inflated 1.0) or dropped/hallucinated items (deflated to 0.4); re-run those evaluations after upgrading for accurate comparison. Bob's earlier FAR/FDR alignment (`a77e14336`) is preserved. The tri-state "partial-match" UI verdict from sirikaro's original suggestion is a separate follow-up (`FormFieldRenderer.tsx` consumes `attr.matched` as boolean; a real "partial" state needs a cross-Lambda contract change + UI). Deep-nested list-of-lists (a list nested inside another list's items) is a known limitation blocked on the upstream Stickler fix at [awslabs/stickler#275](https://github.com/awslabs/stickler/issues/275) — Stickler's own `field_comparisons` doesn't descend past the outer list boundary on that shape.
