...
- **A valid sign-in could land on a permanently blank page, with no error and no way out but a reload nobody suggested.** Found by the first browser UX review, and hit repeatedly while verifying other work — it also strands background jobs, because the page that dies is the one polling them. Four defects composed into a dead end, and any one of them broken alone would have been survivable. (1) **Two byte-identical `GetCredentialsForIdentity` calls raced** — same `IdentityId`, same ID token, one returning 200 with credentials and its twin `400 NotAuthorizedException: Invalid login token. Couldn't verify signed token.` The token was fine; that is simply Cognito's answer to the duplicate, and the sibling request proved it by succeeding. The duplicates came from `useCurrentSessionCreds` being mounted in three places (`App`, `ChatPanel`, `use-agent-chat`), each fetching unconditionally on mount. (2) **The failure was swallowed** — logged behind a `// XXX surface credential refresh error` TODO — leaving `currentCredentials` undefined with no retry until the next **15-minute** interval tick. (3) **The entire authenticated app was gated on those credentials** (`Routes.tsx`), so their absence fell through to the unauthenticated tree. (4) **That tree renders nothing when you are in fact authenticated**: `/login` goes to Amplify's `<Authenticator>`, which renders its children once `authStatus === 'authenticated'`, and it is given none. Net state: authenticated so no sign-in form, no credentials so no app, an empty `<div class="App">`. Fixed at all four points. Every caller in the app now shares **one in-flight promise** (`api/auth-session.ts`), so a cold start issues a single call. Scoping that to one hook would only have reduced the race: seven modules call `fetchAuthSession` directly, `useUserRole` is mounted from ~15 components, and React StrictMode doubles every on-mount effect in development. A forced refresh keeps a separate slot, since `useUserRole` forces one precisely because it believes the cached session is stale — serving it a pending unforced result would hand back the staleness it is trying to escape. A failed fetch **retries** on a 400ms/1.2s/3s backoff, which makes the race invisible. The hook exposes a `credentialsStatus` so callers can tell "authenticated, credentials arriving" from "not signed in" — conflating those is what turned a transient failure into a dead end — and `Routes.tsx` now renders a spinner while pending and, on failure, an **actionable error with Retry and Reload** instead of nothing. A session that returns no credentials counts as an error rather than "ready", so the app never mounts against undefined credentials and fails somewhere less explicable. **Also fixes a timer leak in the same file:** the interval handle lived in a plain local, re-initialised to `null` on every render, so `if (!interval)` was always true, the `else` branch was dead, and `clearInterval` never had a handle to clear — every `authStatus` transition leaked another 15-minute timer, each calling `fetchAuthSession()` forever. Now a `useRef`. 8 new tests; the de-duplication test fails against the old code.

- **Seven of the CSP `connect-src` sources were invalid and silently discarded by Chrome, so the policy was not doing what it was written to do.** `cognito-idp.*`, `cognito-identity.*`, `*.appsync-api.*`, `*.execute-api.*`, `*.s3.*`, `wss://*.appsync-realtime-api.*` and `wss://*.execute-api.*` all place a wildcard in the middle of a host, which CSP does not permit — only the leftmost label may be `*`. Each logged *"contains an invalid source … It will be ignored"*, seven console errors on every page load. The rule was **already documented in a comment directly above the policy**, added for `*.on.aws`, while seven sources in the same string violated it. They were also redundant: `https://*.amazonaws.com` and `wss://*.amazonaws.com` already match every one of those hosts at any subdomain depth — which is why the app worked throughout, and is the empirical proof that removing them changes no behaviour. Removed rather than corrected, so the policy states what it actually enforces. **Separately, this exposed a dead exclusion in `make check-arn-partitions`:** its service-principal check tried to skip comment lines with `^[[:space:]]*#`, but that pattern runs *after* `grep -n` has prefixed each line with its number, so it never matched and comments were never excluded. The console-domain check three lines below already had it right (`^[0-9]*:[[:space:]]*#`); the two now agree.
