...
- **A variable *named* like a credential no longer red-lines the security gate, and the scope decision is made once instead of per line** ([#1086](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1086)). Bandit's `B105`/`B106` match an identifier's **name** against a password wordlist — `pass_count`, `next_token` and a constant whose name ends `_TOKEN` all match — and rate the finding LOW. The scanner this repository runs promotes both to HIGH unconditionally, before its own severity mapping, so a dict key in a test fixture arrived at the same priority as a real secret; three times in one day that took the blocking gate red on `develop`, and each response added another per-line `# nosec`, which leaves the fourth occurrence to be rediscovered by whoever it blocks next. The decision now lives in the gate layer this repository owns: in **test-only** files the two checks are printed under their own non-blocking heading, and everywhere else they gate exactly as before. It has to live there because it is expressible nowhere else — the promotion is compiled into the scanner binary with no configuration surface, and Bandit cannot scope a check to a subset of paths: the only config file the invocation auto-discovers applies its `skips` repo-wide, a per-path section in it is silently ignored, and neither check takes plugin configuration, so the wordlist cannot be narrowed either. The remaining alternative was to stop reporting the checks everywhere, which would hide a genuine hardcoded secret anywhere in the tree. Verified by scanning rather than by reading: two identical credential-shaped assignments, one appended to a test module and one to a shipped Lambda handler, produce one blocking finding and one non-blocking report. **What this gives up:** a real credential pasted into a test fixture is reported at Bandit's own LOW severity and does not block the build. Which files count as test code is computed per path and closed over every tracked Python file, so the three shipped Lambda directories whose names begin with `test` keep gating, and the exemption is registered with that reasoning and a universe-closure ratchet. Also corrected: a comment that merely quoted the `# nosec` pragma while arguing against using one was itself read by Bandit as a live blanket suppression for its line.
- **`make test` now decides whether the documented "no standing failures" baseline is true, rather than the documentation checking itself** ([#1095](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1095)). The accepted-failure table in the full-test-battery procedure states a count, and the only check on it was that the count equalled the number of rows — so the documents could assert a clean baseline indefinitely while the tree was red and their own gate would agree. Nothing read a test result: the battery produced none, and the one `test-reports/` directory in the tree is written by a different target for a single package and read by nothing at all. The runner now writes each root's results as JUnit XML under `test-reports/`, with a `run_all_tests.json` summary naming the observed, declared, unexpected and resolved sets, and compares the failures it observed against the declared table in **both** directions: a failure the table does not declare is red as before, and a declared row whose test passed is red too, so a row cannot outlive its cause and become a waiver over a passing test. The comparison happens in the process that produced the results, so nothing sits between the measurement and the verdict and there is no staleness rule to get wrong — those files are outputs, and nothing reads them back to decide anything. A row must name a pytest node id, because that is the form a run reports; a row written as prose fails the gate instead of being silently skipped by the comparison. A root that dies before collecting anything names no node id, so no row can cover it and it is always red. While the table is empty, which it is, the verdict is what it always was.
- **A dead exemption fragment no longer hides behind a live sibling** ([#1097](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1097)). The retired-service documentation gate excuses individual *lines* by pinning a regular expression to the line's text, and staleness was checked per entry: an entry counted as live as soon as any one of its alternatives matched. So when a single alternative stopped matching — because the line it named was reworded, or its emphasis markers moved in a reflow — nothing reported it. The entry stayed green on the strength of its siblings, the line it was written for quietly stopped being excused, and the failure surfaced somewhere else entirely as unexcused findings in a document nobody had edited deliberately. Staleness is now evaluated **per alternative** across both of that gate's exemption surfaces, and the gate names the individual fragment that shielded nothing together with the remedy: re-pin it to the wording it was written for, or delete it — never edit the prose to suit the pattern. Applying it turned up nine dead fragments, every one of them in the historical-marker patterns, which had no staleness check at any granularity: three were pinned to sentences that no longer exist anywhere in the tree, two duplicated an allowlist entry already covering the same line, and four were unanchored phrasings that shield nothing. All nine are deleted rather than grandfathered and no tolerance was introduced. The allowlist's own 26 alternatives are all live, including the eight pinned fragments in the entry whose justification had warned about precisely this. The service detector is deliberately outside the check, and the reason is recorded where the next reader will look: a detector that finds nothing is a clean tree, whereas an exemption that shields nothing is a standing licence over whatever next occupies the line it named.
