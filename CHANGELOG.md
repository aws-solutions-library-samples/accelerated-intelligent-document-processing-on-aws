...
- **Sustained Bedrock throttling no longer makes an Advanced-mode retry ladder hammer the service it is backing off from.** Once the ladder had spent its backoff allowance it clamped every remaining sleep to zero and kept going, so on the shipped numbers 42 of 50 requests went out back to back at a service that was refusing them — the opposite of what backoff is for, multiplied by the documents in flight. It bought almost nothing in exchange, because a refused call returns in well under a second. The ladder now stops when its allowance is spent (6 attempts on those numbers instead of 50) and lets the state machine retry the step, which has minutes to hours of backoff available where one invocation has 90 seconds, and which releases the Lambda concurrency slot immediately instead of holding it to spin. A tight *deadline* still only shortens sleeps and keeps trying: there is no burst to suppress that close to the wall and the attempts may still succeed.
