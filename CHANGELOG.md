...
- **Test Studio: files dropped directly into an existing test-set folder in S3 no longer stayed invisible.** `getTestSets` short-circuited already-registered folders (`if prefix in existing_test_sets: continue`), so a manual `input/`/`baseline/` add updated neither `fileCount` nor the input↔baseline pairing check. Discovery now reconciles every folder on every call: gated by a `contentSignature` (`input_count:baseline_count:max_last_modified`) so unchanged folders take a no-DDB-write fast path and the UI's 3 s poll doesn't burn writes, skipped for rows in `UPDATING`/`PROCESSING` so it never races the copier/extractor. A newly-added input without its baseline flips the row to `FAILED` with the same error string the new-folder path produces; adding the baseline back clears the row. Live-validated on `idp-dev-qs` — the first reconcile self-corrected two undocumented drifts (`fake-w2` 2000→2005, `realkie-fcc-verified` 75→85). Adds `updatedAt` to the `TestSet` GraphQL type.
