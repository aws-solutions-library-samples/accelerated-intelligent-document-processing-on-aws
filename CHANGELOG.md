...
- **Evaluation reports no longer contradict themselves on list-valued documents ([#625](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/625)).** Parent list-field verdicts used to read ✓ while the expanded drilldown showed red child rows, and section metrics could report 100% precision on documents with most of a list wrong. Parent, section, and run-level views now derive from the drilldown itself, so they cannot disagree; historical runs recorded on v0.6.3–v0.6.5 should be re-run for accurate comparison. See [docs/evaluation.md](docs/evaluation.md).
