...
- **The confidence ladder's same-model retry rung now checks the Lambda's remaining time before each round, like the other two rungs already did** ([#958](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/958)). The self-healing ladder has three places it can spend sequential model calls — further bisection of a truncated batch, a same-model retry round over the rows the model left unscored, and an escalation round on a bigger-output model — and two of them consulted the wall-clock deadline threaded down from `context.get_remaining_time_in_millis()`. The retry rung did not: it stopped after `max_retries` rounds or on a round that recovered nothing, so a sequence of rounds that each recovered *something* while never truncating was bounded only by the round budget, at a real model call per round, however little of the 900-second Assessment Lambda was left. All three rungs now estimate the round's cost (chunks × a 60-second per-call assumption, the floor every guard in the module already used) and stop before starting one that will not fit in the remaining time minus the 90-second safety reserve, flagging `deadline_reached` so the section reports the existing `assessment_deadline_reached` warning instead of the document dying on a hard timeout. This was the most concrete lead on the 900-second `Sandbox.Timedout` ×3 symptom reported on [#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894), but it is **not** confirmed as its cause — that failure has never been reproduced here, so #894 stays open for it and for the batch sizer. Behaviour is unchanged when no deadline is threaded in (local runs and non-Lambda callers) and unchanged whenever the remaining time is sufficient; a unit test pins that the rung still runs its rounds under a generous deadline, so the guard bounds the rung rather than disabling it. See [extraction and confidence](docs/extraction-and-confidence.md#large-list-batching-list_batch_size) and [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).
