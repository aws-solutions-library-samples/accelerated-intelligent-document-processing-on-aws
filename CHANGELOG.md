...
- **`document_class` is written on every metering row, and the four per-document rollup tables carry it as a grain dimension.** `save_reporting_data.save_metering_data` derives one class per document from `document.sections` using the 0/1/N rule — zero distinct section classes → `'unknown'`, one → that class, more than one → `'mixed'`. `Section.excluded` sections (instruction/legal boilerplate) are filtered from the derivation, so a W2 with an instructions page reads as `'w2'` instead of `'mixed'` — a correctness improvement over the read-side query the IDP Monitor widget used at query time. The four per-document rollup Glue tables (`metering_hourly`, `metering_daily`, `metering_docs_hourly`, `metering_docs_daily`) widen their grouping key to include `document_class`. The rollup Lambda's `INSERT` queries derive the value via `COALESCE(m.document_class, dc.document_class, 'unknown')` with a CTE joining `document_sections_*`, so historical rows that pre-date the column bucket correctly. Consumers that `SUM` across the rollup grain are unaffected by the widened dimension; a naive `COUNT(*)` on rollup rows would now over-count. See [reporting-sql-layer.md §Track A](docs/reporting-sql-layer.md#track-a--new-tables-data-phase-1-had-nowhere-to-put).

- **Rollup backfill mode, hourly reconciler, and absence alarm — ops hardening for the data-mart rollup Lambda.** `mode: "backfill"` iterates a caller-supplied hour range and re-runs the four per-document rollups per hour with the existing idempotency guard (already-written partitions are no-ops). A new EventBridge rule `DataMartRollupReconcileSchedule` fires `mode: "reconcile"` at `:35` each hour — 30 min after the `:05` hourly — to fill in gaps left by missed schedules or exhausted async-retry budgets; cheap when nothing is missing, self-healing when something is. A new CloudWatch alarm `DataMartRollupAbsenceAlarm` pages the operator when the rollup Lambda records zero invocations for two consecutive 1-hour windows — catches the class of failure where a redeploy dropped the EventBridge schedule from the stack (which the DLQ alarm cannot see, because that fires on failures, not absences). See [reporting-sql-layer.md §Track D](docs/reporting-sql-layer.md#track-d--operational-hardening).

- **Rollup schema migration orchestrated as a Step Functions state machine.** `DataMartMigrationStateMachine` (`AWS::Serverless::StateMachine`, STANDARD type) drives the one-shot migration that repopulates the two per-document hourly rollup tables (and their daily rollups) at the widened `document_class` grain. Flow: `CheckMarker` → `RouteOnMarkerState` (short-circuit / skip-purge / full-flow) → `InitialPurge` (Lambda, `mode: purge_rollup_prefixes` — wipes S3 for all four rollup prefixes so pre-widening parquet doesn't collide) → `WriteInProgressMarker` (Lambda, `mode: write_marker`) → `PlanChunks` (Lambda, `mode: plan_migration_chunks` returns `days ÷ chunk_hours` `(start, end)` pairs) → `MigrateChunks` (Map, `MaxConcurrency: 8` × `ReservedConcurrentExecutions: 12`, each iteration calls Lambda `mode: backfill --start=… --end=… --arms=[metering_hourly, metering_docs_hourly]` with SFN-native `Retry` on `Lambda.ServiceException` / `States.Timeout`) → `BackfillDailyRange` (Lambda, `mode: backfill_daily_range` — aggregates the now-populated hourly rollups into `metering_daily` + `metering_docs_daily`, 30 days sequentially) → `CheckMigrationSuccess` (Lambda, `mode: check_hours_failed` — treats `hours_partial>0` as unclean, not just `hours_failed>0`, so partial-arm failures block completion rather than silently marking the migration done) → `WriteCompletedMarker` or `MigrationHadFailures` (Fail). Only the two hourly metering arms are re-run (`metering_hourly`, `metering_docs_hourly`) — the other two arms `control_plane_hourly` and `data_plane_lambda_hourly` don't have `document_class`, are never purged, and are populated by the routine `:05` hourly cron. SFN was chosen after live-fire discovery on `idp-dev-qs1` (2026-09-22) that a Lambda-only design couldn't fit real-volume windows in Lambda's 45 min async-retry budget: hundreds of hours × Athena work on a busy stack exceeds a single 900 s invocation, and per-chunk retries need their own budget rather than sharing one. At `MaxConcurrency: 8`, a full 720-hour (30-day) migration completes in ~30 min on idle load; the ceiling is deliberately kept under Athena's default 20-DML-per-workgroup limit (8 chunks × 2 arms = 16 concurrent queries, 4 slots left for scheduled hourly / daily / reconciler crons that share the same Lambda). Two-phase SSM marker at `/idp/<stack>/data-mart-rollup/migration-complete` (`state=in_progress` written after purge, before backfill; `state=completed` only after `hours_failed=0` **and** `hours_partial=0`) means restarts resume rather than restart — HeadObject-skip idempotency on partitions the prior execution already wrote makes resumption a fast no-op on completed ranges. The `doc_class` CTE that joins raw `metering.document_id` against every discovered `document_sections_*` table is date-partition-pruned to `WHERE date BETWEEN <target-1d> AND <target+1d>` per rollup query, so a migration doesn't fan out `20 tables × 30 partitions` concurrent S3 GETs and trip `HIVE_S3_THROTTLING` on the reporting bucket. `mode: backfill_migrate` from the pre-SFN design is retained as a deprecated Lambda no-op so any queued async retries from the transitional design succeed cleanly rather than DLQ-firing. New alarm `<stack>-data-mart-migration-failure` fires on any FAILED/TIMED_OUT/ABORTED SFN execution and points at the new operations runbook. CustomResource properties: `MigrationVersion: "v1"` (bump to re-fire), `Days: 30` (retention window; `[1, 90]`), `ChunkHours: 1` (per-chunk time slice — 1 h is the safest default because worst-case per-chunk work is ~10-30 s of Athena, comfortably under Lambda's 900 s cap with ~30× margin), `ForceFresh: "false"` (customer-safe default — never destructively re-migrates on a routine version bump; set to `"true"` explicitly when you deliberately want to wipe and restart, then flip back). Raw `metering` is NOT rewritten: its Glue schema gains `document_class` as a nullable column and Parquet schema evolution returns `NULL` for pre-widening rows, which the rollup Lambda's `COALESCE`-with-JOIN CTE handles transparently. See [docs/data-mart-migration-runbook.md](docs/data-mart-migration-runbook.md) for operations.
