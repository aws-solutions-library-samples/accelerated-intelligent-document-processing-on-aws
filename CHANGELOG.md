...
- **Advanced extraction can now say "I could not read this" instead of inventing a value.** A `required` attribute was rendered as a non-nullable field in the model handed to the extraction agent, so an unreadable value had no representable answer — of `null`, an empty string, omitting the field and `0.0`, only **`0.0`** was accepted — and the retry loop then told the agent its answer had failed validation and asked it to fix it. For a numeric attribute that produced a **fabricated zero that is schema-valid, raises no error, and is indistinguishable from a real zero**; on one unreadable column it happened for all 100 rows while the section reported as valid. Simple mode was never affected, because it emits plain JSON and a `null` survives to be reported. Required-ness is now relaxed only for the model the agent fills in, and is still enforced where it can be reported rather than fabricated: `extraction.validation` checks the result against the real schema, treats a null as absent, and reports `'X' is a required property` exactly as simple mode does — feeding the agent one self-correction round and, with `fail_action: escalate`, a re-extraction. The retry feedback now also tells the agent to supply a value only if it can actually read it, and to leave it null otherwise rather than substitute a placeholder ([#782](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/782)).
...
- **Extraction: advanced mode can stop sending the document schema three times.** Two of the three are the same JSON object — 2,595 of 5,692 schema tokens (46%) on the lending `Payslip` class — and `extraction.agentic.restate_schema_in_system_prompt: false` drops the duplicate. Every copy sits inside the prompt cache, so the dollar saving is roughly a tenth of the token count. It stays **on by default** anyway: restating a schema in prose often improves adherence — so if you turn it off, judge it on **completeness**, because a token saving that loses list rows is a loss. **Configuration → Prompt Preview** counts the duplicated block — as a floor, since the real block the backend generates is larger — so the saving is visible before you switch it off ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).
