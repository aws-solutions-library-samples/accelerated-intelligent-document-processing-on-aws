...
- **`test_results_resolver` opts into Athena result reuse for completed test runs.** Every query in the resolver is scoped by `document_id LIKE '<test_run_id>/%'`, and a test run's metering rows are written before the run completes and never mutated after — so the same "cost breakdown by run" query is served from Athena's result cache on every refresh of the Test Studio results page. The workgroup default is off, so the resolver now passes `ResultReuseConfiguration.ResultReuseByAgeConfiguration(Enabled=True, MaxAgeInMinutes=60)` on every `StartQueryExecution`. Repeat views of a completed run's page now respond in one round-trip instead of a full metering scan. The 60-minute window is a caller parameter (`reuse_max_age_minutes`) so a specific call site can lower it or turn reuse off — in-progress runs are gated at the resolver layer (only reached after `COMPLETE` / `PARTIAL_COMPLETE`), so cache staleness on an unfinished run isn't reachable.
