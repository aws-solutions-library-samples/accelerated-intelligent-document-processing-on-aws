...
- **Class confidence reads as a measurement, not as more text after the label.** The score arrived appended to the class name in the Sections and Pages tables — two unrelated values in one cell, with nothing to say the percentage was clickable. It now has its own sortable **Class conf.** column in both tables: a plain badge in **Document Sections** (a section score is an aggregate with no reasoning of its own), and a link in **Document Pages** where clicking it opens the model's reasoning and its ranked alternatives. Sorting is the functional half — least-confident-first is how a reviewer finds the pages worth a second look, and both tables were previously `sortingDisabled`, so every column that already declared a sort field never offered one; they all work now, via the design system's own collection hook. An unscored row shows `—`, never `0%`. Two deliberate restraints: the number is styled identically at every value (no configured classification threshold exists, so a traffic light would assert a verdict the system has not defined — and on a classifier that answers ~0.95 for most pages it would paint a coarse flag as calibration), and the adjacent per-field column is reworded to **Low-conf. fields** so two neighbouring columns do not both read "confidence" for different measurements.
