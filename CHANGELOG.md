...
- **A document's ground-truth vs predicted classification is now visible in the UI, per page and per section ([#627](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/627), [#630](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/630)).** Arriving at a low-scoring document from Test Studio's *Documents with Lowest Weighted Overall Scores*, there was no way to tell whether the score was low because a field was extracted wrongly or because the page was classified as the wrong document type in the first place — and the Sections table showed only what the run predicted, with nothing to compare it against. The per-page comparison was already being computed (`doc_split_metrics.page_details` carries every page's expected and predicted class) but nothing rendered it: the markdown report shows section-level tables only, so a page misclassification was visible as a number with no way to see which page went where. A new **Classification vs Ground Truth** panel on the document detail page — above the Sections table, since a class mismatch explains the extraction scores below it — shows the three split accuracies with the page counts behind them, a **Pages** table (ground-truth class, the class this run assigned, match/mismatch) and a **Sections** table pairing each ground-truth section with the predicted section that matched it, page ranges included. Two things it deliberately does not hide: **extra predicted sections that matched no ground-truth section** are listed as their own rows, so an over-split shows both halves of the error rather than only the missing ground-truth section; and a section matched on pages and class but not on **page order** is flagged distinctly rather than folded into either verdict. A **Mismatches only** toggle defaults on when there are mismatches and off when the run was clean, so the table never reads as "no data" when it means "nothing wrong". The panel renders **nothing at all** unless the document was evaluated against ground truth carrying section boundaries — an empty panel would assert that the comparison exists and is perfect, a different claim. It also states inline that a misclassified page was extracted against the wrong schema, so its low extraction score is a symptom rather than the cause: fix the classification first. Page numbers are converted to 1-based to match the page viewer (`page_indices` in `results.json` are 0-based, computed as `page_id - min(page_id)`), `results.json` is derived from the document's own evaluation report URI so a historical run snapshot resolves to that run's results, and a fetch failure (results pruned by retention, or an older document) degrades to "not available" rather than an error banner. 23 tests. See [docs/evaluation.md](docs/evaluation.md#seeing-which-pages-were-misclassified-web-ui).
