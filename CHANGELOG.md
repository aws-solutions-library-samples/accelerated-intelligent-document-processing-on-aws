...
- **The UI API resolver Lambdas now redact their request logs with the canonical denylist instead of a hand-copied one, and no longer write a whole chat prompt to CloudWatch** ([#921](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/921)). Ten resolvers under `nested/api-resolvers/src/lambda/` each carried their own shortened copy of the key list in `idp_common.utils.log_sanitizer`, and every copy had fallen eight key names behind it (credential- and key-style names), so a request field matching only a newer name was logged rather than replaced with `***REDACTED***`. Nothing compared the copies, so nothing reported the gap. All ten now call the canonical `sanitize_event_for_logging`: the two that carry an `idp-common` layer import it from the library, and the eight that carry no layer — SAM packages each function from its own `CodeUri`, so they cannot reach the library at runtime — ship a byte-identical committed copy of the stdlib-only module, synced by `scripts/sync_resolver_log_sanitizer.sh` (the same guarded-vendoring pattern as the two document-list resolvers' vendored `config_scope.py`, and as `src/lambda/chat_stream_processor/vendored/`). The canonical redactor also **truncates** long string values, which none of the hand-copies did: values under a content-shaped key (`prompt`, `text`, `content`, `body`, `answer`, `response`, `markdown`, `extracted_text` and similar) are capped at 500 characters with a `...[TRUNCATED N chars for log hygiene]` marker. Concretely, `agent_chat_resolver` logs its whole `arguments` object and accepts a prompt of up to 100,000 characters, so a user's full chat message used to land in CloudWatch Logs verbatim; it is now capped at 500 characters. `scripts/tests/test_resolver_log_sanitizer.py` fails if a copy drifts by one byte, if the effective denylist stops matching the canonical one, or if any file under that tree defines a local key list again under any variable name — in a bare tuple/list/set literal, a dict literal, or wrapped in `frozenset(...)`/`set(...)`.
