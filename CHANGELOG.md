...
- **506 tests in 22 registered suites now run on every pull request, and the check that was supposed to notice they did not has been widened to the whole tree** ([#980](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/980)). `make test` discovers every test directory in the repository, but it runs in neither CI; what a pull request runs is `make test-cicd -C lib/idp_common_pkg` plus a hand-enumerated `make test-packages-cicd`, and 22 directories were registered in the first and named in neither. Among them: the eight API-resolver suites, including the 25 tests covering the download allow-list, script-type download forcing and fail-closed refusals, and the 16 covering the discovery upload target — added by a fix whose own description said those tests were what would have caught the defect — plus the benchmark harness that decides what a release report claims, the eight feature-platform extension suites, the Lambda hook-inference samples, and two custom-resource suites. The gate that detects this existed and was scoped to one subtree, `src/lambda/`, with a note saying the argument applied more widely; it now derives its universe from every directory holding a tracked `test_*.py` and its covered set by parsing both CI-invoked recipes, so a suite reachable from neither is a test failure rather than something found by hand months later. Four of the 22 passed locally and failed under the wrapper that strips the AWS environment, each because something on its import path builds a boto3 client at module scope: three needed a region and one needed credentials to sign a presigned POST, and each now supplies its own from its own `conftest.py` rather than having one pinned for it by the caller, which is what makes running it through the wrapper mean anything ([#988](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/988)). The reasons in the excluded registry were re-measured too: the one claiming a Lambda-runtime-only dependency had outlived its own remedy, and the reason that replaces it names the single stale assertion actually holding that root back and is computed in both directions, so fixing the test invalidates the entry instead of leaving it standing. [Testing](docs/testing.md) records which targets a pull request runs, which it does not, and why the two are not the same set.
