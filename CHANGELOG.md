...
- **Three controls that existed but were never consulted: the CLI is type-checked, `idp_common.agents.utils` is packaged, and X-Ray traces can be filtered by document id.** Each failed silently, in the direction that produces no output. (1) `pyrightconfig.json` listed `idp_cli/idp_cli` in `include`, a path that has not existed since the CLI moved to `lib/idp_cli_pkg/idp_cli`; basedpyright contributes nothing for a missing include entry, so `cli.py` (6,791 lines) and the rest of the package were checked by nothing while `make typecheck` looked clean over the 329 files that remained. The corrected path adds 5 files and exposed 4 warnings, all pandas-stub artifacts where manifest values flow into `os.path` calls, now fixed rather than baselined; `make typecheck` also goes from red to green, as two pre-existing `reportOperatorIssue` errors in `src/lambda/calculate_capacity/index.py` are fixed by making a page count explicitly `float`. (2) `idp_common/agents/utils/` had no `__init__.py`, and setuptools `packages.find` skips such directories, so the subpackage was absent from the built wheel while `agent_factory.create_conversational_agent()` imported `..utils.conversation_manager` and `..utils.memory_provider` unguarded — an editable install resolves those from the source tree, so it could only fail on a wheel-based deployment, on the agent chat path, in front of a user. It was the only such directory. (3) Four pipeline Lambdas (OCR, classification, extraction, assessment) passed `{document.id}` — a one-element set — to `xray_recorder.put_annotation`, which X-Ray drops or stringifies, so trace filtering by document id did not work on exactly the four functions that do the expensive work; the two rule-validation functions already had it right. All 15 `put_annotation` calls in the tree are now correct. ⚠️ Nothing could have caught any of the three, so each fix ships with the assertion that closes its class: `scripts/tests/test_pyright_config.py` fails if any `include` path is missing or holds no Python, `scripts/tests/test_package_discovery.py` fails if any directory holding `.py` files in any of the five first-party distributions would not be packaged, and `scripts/tests/test_xray_annotations.py` parses the AST and fails on a collection literal in an annotation value. Ruff's `B018` is **not** the answer for (3) and was not enabled: measured, it flags a useless expression *statement* and not a set literal in argument position, and `patterns/` is excluded from ruff wholesale in any case. ([#923](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/923), [#924](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/924), [#925](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/925))
