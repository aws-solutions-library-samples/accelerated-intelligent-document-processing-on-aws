...
- **Every public `idp_sdk` result model can now be imported from `idp_sdk`.** Eighteen of them — including `BaselineInfo` and `FieldComparison`, which `evaluation.list_baselines()` and `evaluation.get_report()` now return — were public in `idp_sdk.models` but never re-exported from the package, so the import block the [SDK reference](docs/idp-sdk.md) tells you to copy raised `ImportError` on 13 of its names. Two tests derive the closure from `idp_sdk.models.__all__` and from the documented block itself, rather than listing names, so the next model added cannot go missing the same way.

- **Confidence calibration is measured against exact per-cell ground truth, and the shipped default grader is well calibrated but cannot rank errors.** The benchmark harness could not join a confidence score to the cell it described — the traversal collected the bare scalar and discarded the field path — so the only confidence statistics available were distributional, and calibration went unmeasured on the one corpus with exact per-cell truth. The scorer now walks `explainability_info` with the same field-path rule the product keys its stored confidence curves by, joins each path to the synthetic corpus's truth through its unique row tags, and reports ECE, Brier and AUROC per document through the shipped `ConfidenceCurve` engine, so the figures are about the reliability bars the product itself acts on rather than a lookalike. `aggregate.py --calibration` pools them per configuration arm across a completed grid, which costs S3 reads and no inference; `cell_stats` pools them per cell; and `compare_cells` gates on the pooled figures, flagging a calibration error that grows, ranking power that falls, and — whatever the size of the step — a cell that crosses the unreliable-ECE or chance-AUROC bar, past which the review-effort estimator stops recommending a review subset at all. Measured over 466,322 cells of the v0.6.8 grid with a v0.6.9 replication: every arm passes calibration, and `separate` mode on the default Nova Lite grader places every one of 70,320 cells in a single confidence bin with AUROC 0.417, so a worst-first review queue ordered by those scores is ordered arbitrarily; the same extraction graded by Claude Sonnet 5 spreads over six bins and reaches AUROC 0.859. The worst arm has 1,413 wrong cells, every one of its 54,484 cells in the top confidence bin at a mean confidence of 0.995, and a calibration error of 0.021 — comfortably passing. **No default changed:** Nova Lite is a ~130× cost decision, and this supplies the half of that trade-off which had been assumed rather than measured. See [confidence calibration](docs/benchmarking/studies/confidence-calibration.md) ([#935](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/935)).

- **A clear message for an account with no role.** A user whose account has not been placed in a Cognito group now sees "Your account has not been granted access yet", with the roles an administrator can grant, instead of a navigation whose every page fails. A session that cannot be read is reported separately as the transient condition it usually is, rather than as a missing role ([RBAC](docs/rbac.md), [#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)).

- **A contributor-side guard against writing straight to `develop` or `main`.** Branch protection is a repository setting and enabling it needs admin that nobody working in this tree has, so every CI gate is advisory and nothing stopped a change reaching a shared branch without a pull request. Two client-side halves now make that a deliberate act: a `PreToolUse` hook on `Bash` (`scripts/hooks/check_shared_branch.py`) that refuses a `git commit` landing on a shared branch, a `git push` whose destination resolves to one, and a `gh pr merge` with a concluded failing check; and a real git `pre-push` hook installed once per clone with **`make install-git-hooks`**. Overrides are per decision and go in front of the command — `ALLOW_SHARED_BRANCH=1 git push …`, `ALLOW_RED_MERGE=1 gh pr merge …` — because a variable exported in one shell does not survive to the next. ⚠️ **On a machine that sets `core.hooksPath` system-wide** (some managed developer machines point it at a directory of hook runners) git runs those hooks, and this one is reached only because they chain to it; they forward its arguments but not its stdin, so it gets no ref list and falls back to judging by `HEAD`. Exiting quietly there is how a hook can be installed, reported successful and refuse nothing, so it refuses instead and names the basis it used. That substitution is wrong in both directions, and the two directions arise on different machines: a push whose destination *is* a shared branch while `HEAD` is not on one is allowed on a redirected machine (the `PreToolUse` half still refuses it from the command line, so the residue is a push typed into a plain shell), while the refusal of a push made *from* a shared branch reaches any machine, because git also supplies no ref list for a push that has nothing to send. Every refusal therefore names the basis it used and both of its possible causes. `make install-git-hooks` says all of this when it detects the redirect, never overwrites an existing `pre-push.bak`, and warns that it **replaces** rather than chains to a hook already there. Neither half is a substitute for required status checks: a merge through GitHub's web interface runs no code on a contributor's machine, so **a pull request can still be merged with all checks red**. What else is not covered — `--no-verify`, anything that reaches `git` other than as the first word of a command, and history written onto a shared branch by `merge`, `cherry-pick`, `revert`, `rebase` or `am` — is listed in `CLAUDE.md` rather than implied to be handled. See [CONTRIBUTING.md](CONTRIBUTING.md#branching) ([#933](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/933), [#1055](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1055)).

- **`make lint` now fails on a private AWS account id committed in a tracked file's contents (`make check-account-ids`).** The `PreToolUse` commit-text hook inspects the *command* text of a commit or a pull-request creation, so an id inside a file was structurally outside its reach: a file's contents never appear in the invocation that commits it, and no pattern in that hook could have caught one. The new gate reads every tracked text file instead. A bare 12-digit match is far too weak to block a build on — this tree holds upwards of a thousand such runs and all but a handful are benign — so each is resolved in three stages and only what survives all three fails: shape rules for runs that are not standalone numbers (the fractional part of a decimal literal, a UUID's final group, a slice of a hexadecimal digest — a single MD5-named corpus document accounted for 125 candidates on its own), an allowlist keyed by value for ids that are public or synthetic, and per-line `<path>:<line-pattern>` entries for the residual where an id is consumed as configuration at run time rather than being prose. `--summary` prints the current split rather than any figure being written into prose that a later merge would falsify. All four carve-outs — those three plus the set of file extensions it does not read at all — are registered in `scripts/tests/gate_exemptions.json` with a per-member reason and a ratchet, and the residual entries name no id, so neither the gate nor the registry becomes a further occurrence. ⚠️ **It is a detector for ids committed by accident, not a control against deliberate evasion**, and its docstring names the blind spots rather than leaving a green run to imply there are none: an id split across a string concatenation, an id inside a binary file, an id inside a long hexadecimal token, and swapping an already-exempt line's id for a different one — which count pinning cannot catch, because an entry that deliberately names no id has no value to compare against. The occurrences that prompted it — 175 across 17 files — were redacted in the same change, by the kind of text they sat in: published prose lost the account clause, since a stack name and region already identify a run; assistant instructions take a named placeholder plus the command that resolves it at run time, which makes those procedures account-agnostic; and values that have to stay syntactically valid, such as saved notebook output and a fake caller identity in a test, take a documentation placeholder. **This does not remove anything from git history and does not try to** — the repository is public, so those commits are already cloned, and an account id is not a credential. Six lines keep a real id as a recorded residual, because on each the id is read as a value at run time so removing it is a behaviour change needing its owner: a CI variable's fallback and a bucket name derived from it, the fake caller identity in two of their tests, and two IAM trust statements where the id *is* the trust relationship. An entry that shields nothing **fails**, with no per-entry tolerance, so a residual cannot outlive the occurrences it covers. Wired into `make lint`, `make fastlint` and `make lint-cicd`, so both CIs run it ([#1067](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1067)).

- **Markdown links are a gate.** `make check-markdown-links` reads every tracked `.md` file — derived from `git ls-files`, so a new page or a `README.md` in a tree nobody thought of is covered — and resolves each relative link, each `#anchor` against the headings the target actually has, and, for a page the docs site publishes, whether a relative link into `docs/` points at a page `docs-site/setup.sh` symlinks. That last one is the failure review cannot see: the link works in the repository and 404s on the published site, and the remedy at the call site is the GitHub blob URL. It also reports a code fence that swallows content, because content inside one is content no check can read. **53 findings across 31 files are fixed with it: 45 broken links across 28 files, and 8 code fences across 5 files** — four of those links needed no edit of their own, because repairing the fence above them brought the headings they pointed at back into the document. Anchors are slugified by the same rule `github-slugger` uses, including that it keeps the variation selector in `⚠️`, which is what makes a link to an emoji heading resolve. External `http(s)` URLs are deliberately not fetched: a blocking gate that needs egress fails on somebody else's outage. Runs in `lint`, `fastlint` and `lint-cicd`, so both CIs ([testing guide](docs/testing.md#2-static-gates-lint-types-and-hand-written-scanners), [#1068](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1068)).

### Changed

- **The Nova fine-tuning CLIs and the live agent test scripts moved out of the `idp_common` package.** The four fine-tuning scripts are now at `scripts/model_finetuning/` and the DynamoDB service example at `scripts/examples/`, so every command on the [Nova fine-tuning page](docs/nova-finetuning.md) runs from the repository root rather than from inside the installed library. The seven operator-run agent scripts moved to `lib/idp_common_pkg/manual_tests/agents/`; they drive real Bedrock and Athena against a deployed stack, and `idp_common.agents.testing` is consequently no longer importable and no longer ships in the distribution. Nothing in the library imported any of them. They were counted as library source by `--cov=idp_common`, which understated `idp_common`'s line coverage by 3.35 points.
- **Advanced extraction: the measured effect of greedy decoding on Bedrock's `Model produced invalid sequence as part of ToolUse`.** `topK: 1`, and AWS's full `temperature 0` + `topP 1` + `topK 1` triple, leave Nova Lite's failure rate statistically unchanged (0.787 → 0.812 → 0.838 at n=160 per arm, none significant) — so the inference-parameter explanation that [#895](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/895) left open is closed by measurement rather than still pending. The trigger is the request's schema **shape** rather than its size: a 1,696-character schema passes where a 401-character one fails, and an array of objects with sibling scalar properties is demonstrably sufficient to provoke it (necessity is left open, and the study says why). The per-model `topK` carrier routing table is documented, because it is what makes `topK` not simply a setting you can turn on. **No configuration option was added** — shipping a user-facing knob whose measured effect is null would be worse than not having one. The null is quoted with its power floor throughout, so it is not read as proof of identity. See [greedy decoding and tool use](docs/benchmarking/studies/greedy-decoding-tool-use.md) ([#956](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/956)).
- **`extraction.agentic.restate_schema_in_system_prompt` guidance corrected: turn it off for shard headroom, not for dollars.** Advanced extraction does transmit the class schema three times per request, and dropping the system-prompt copy costs **no** quality — completeness, cell and scalar accuracy were exactly 1.000 across all 50 runs of both arms. It saves 5–7% of input-side extraction tokens on documents whose agent loop is stable, which is real headroom against `shard_token_budget`. It is **not** a cost saving: those tokens are cache reads at about a tenth of input price, while extraction **output** tokens rise ~17% on a list-heavy class — bimodally, with runs entering a mode the control arm never reached — so extraction cost's central estimate rises rather than falls. See [schema restatement and tokens](docs/benchmarking/studies/schema-restatement-tokens.md) ([#710](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/710)).
- ⚠️ **Six end-of-life Bedrock models are no longer offered anywhere a model is chosen.** `us.amazon.nova-premier-v1:0` and five Claude versions (`claude-3-5-sonnet-20240620-v1:0`, `claude-3-5-sonnet-20241022-v2:0`, `claude-3-7-sonnet-20250219-v1:0`, `claude-3-haiku-20240307-v1:0`, `claude-opus-4-20250514-v1:0`) are gone: an invocation returns `ResourceNotFoundException: This model version has reached the end of its life`. That was established against the live Bedrock API — `get-foundation-model` answers with the end-of-life message in `us-east-1` and `us-west-2`, and none of the six appears in `list-foundation-models` — rather than read off a model card, because an inference profile keeps reporting `ACTIVE` after the foundation model behind it is withdrawn, which is what made a picklist built from profile names look healthy. Four of the six were also **code defaults**, so a stage nobody had configured ran a dead model: summarization, both rule-validation fields, and LLM-based evaluation. ⚠️ Those four stages now default to a current model (Nova Pro for summarization, Claude Sonnet 4.5 and Claude Haiku 4.5 for the others), so a deployment that left any of them at its default gets a working model — and a different one — where it previously got an error. Three were EU `MODEL_MAPPINGS` targets, so an EU deployment was being rewritten from a dead US model onto a dead EU one; those now name live Claude 4.5 models. All six are gone from the template enums, the UI's per-class override dropdown, the quota-code and prompt-caching tables and every code default — including the fallback each of seven agent modules used when its configured model could not be read, which is a default in every sense that matters at runtime and was the last place a dead model survived: an agent that could not reach its configuration reported `ResourceNotFoundException` for a retired model instead of the configuration error that actually stopped it. Those fallbacks now resolve to one shared constant taken from the declared default, so there is no second copy of a model id to retire separately. A model id written as a literal anywhere in production Python is now checked against the retirement registry, which is the check that found them; the surfaces already covered read Pydantic defaults and template enums, and a string in an `except` branch is neither. Their `config_library/pricing.yaml` rows and `model_config_limits.yaml` patterns are **deliberately kept** — a cost report covering documents processed while a model was selectable, and a limits lookup for a stack whose stored configuration still names one, both have to keep resolving. `idp-cli config-validate` and `config-upload` now reject a configuration that pins one, naming the end-of-life date, rather than letting the failure appear part-way through a document; loading a stored configuration that names one is unaffected, so an existing deployment does not break on upgrade — it keeps failing at inference exactly as it already was, until the stage is repointed. ⚠️ **Upgrade-visible:** `KnowledgeBaseModelId` is a CloudFormation parameter, so a stack whose current value is one of the removed models has its next update **rejected** with `Parameter value ... does not match AllowedValues`; pass a current model for it in the same update. See [Retired and legacy Bedrock models](docs/configuration.md#retired-and-legacy-bedrock-models) ([#1039](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1039)).
- ⚠️ **Document-content API operations now require an assigned Cognito group.** Eight document reads (`getDocument`, `listDocuments`, `listDocumentsByDateRange`, `getDocumentVersion`, `compareDocumentVersions`, `getFileContents`, `getFilePresignedUrl`, `queryKnowledgeBase`) and three mutations (`deleteAgentJob`, `deleteChatSession`, `sendChatDocumentMessage`) were reachable by any authenticated user, including one in no group at all — which self-service sign-up produces when `AllowedSignUpEmailDomain` is set. They now require membership of at least one of Admin, Author, Reviewer, Annotator or Viewer. **Before upgrading, place every user who needs document access in a group** (User Management, or `idp-cli`): an existing user in no group will lose access they have today. Platform, profile and feature-catalog reads are unchanged. ⚠️ **This gates the API, not the document buckets.** The Cognito Identity Pool's authenticated role grants every signed-in user direct `s3:GetObject`/`ListBucket` on them regardless of group, and the web UI reads them that way, so this is **not** a control on the document bytes. It also does not apply to the chat streaming Function URL, which carries no group claim ([RBAC](docs/rbac.md), [#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)).
- ⚠️ **Seven more API operations now require an assigned Cognito group: the reads that lead to document content, not only the ones that return it.** `listDocumentsDateShard`, `listDocumentsDateHour`, `listDocumentVersions`, `getDocumentCount`, `getStepFunctionExecution`, `getChatMessages` and `getCircuitBreakerStatus` require membership of at least one of Admin, Author, Reviewer, Annotator or Viewer. Five of the seven return no extracted value themselves, which is why they were left open — but an object key, a run's section and page URIs, the list of extracted attribute names, a model-written description of what a page contains, and the workflow execution input are steps of one chain rather than separate disclosures, and on a deployed stack that chain composed end to end for a self-registered account in no group, starting from no knowledge of the deployment at all. The other two are narrower: `getChatMessages` is bounded by a session-ownership check that returns *allowed* when its table is unset, and `getCircuitBreakerStatus` returns a `lastError` that reads `Manual pause by <administrator email>: <reason>` after a manual pause. **Before upgrading, place every user who needs document access in a group** (User Management, or `idp-cli`): a user in no group loses the document timeline, the version history panel, the workflow view, chat transcripts and the processing-paused badge along with the document reads. The caller's own profile and chat session list, the published release number, the two fine-tuning job reads and the three feature-platform reads stay open to any authenticated user, and each now records why that is the right answer for the individual operation rather than for the group it sits in. ⚠️ **This still gates the API, not the document buckets** — the Identity Pool's authenticated role grants every signed-in user direct S3 access to them regardless of group, so the object bytes remain reachable without any API call; the two candidate fixes and their obstacles are set out in [Identity Pool group scoping](docs/planning/identity-pool-group-scoping-plan.md) ([RBAC](docs/rbac.md), [#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033)).
- **Several authorization statements in the RBAC reference corrected.** `listUsers` is Admin-only rather than filtered per caller; five operations (`getAgentChatMessages`, `updateChatSessionTitle`, `updateAgentChatMessage`, `validateTestSetForFinetuning`, `listAvailableModels`) are declared in the GraphQL schema but routed from nowhere, so the dispatcher's default-deny refuses them to every caller including Admin, where the page had described them as open to any authenticated user; and the role table is introduced as five roles, which is how many it lists. The same claim about `listAvailableModels` sat on the fine-tuning guide, so both pages are corrected rather than only the reference ([RBAC](docs/rbac.md), [Custom model fine-tuning](docs/custom-model-finetuning.md)).

- ⚠️ **Both per-user access scopes are now enforced on the S3 object key, not just on the API record.** The product restricts a user two ways that this release makes hold for the **bytes** as well: `allowedConfigVersions`, which confines a non-admin to named [Configuration Profiles](docs/configuration-profiles.md), and `allowedTestSets`, which confines an Annotator to named test sets. `getFileContents` and `getFilePresignedUrl` take an `s3Uri` from the caller and validated only the **bucket**, so a profile's stored configuration and a test set's documents and ground truth could be fetched as files by a user the same scope excluded from the profile or the set — the operations that serve them by name enforced the scope, and the operation that serves them by key did not. Both operations now resolve the caller's scope and match it against the key: the profile in a `config_revisions/<profile>/` key, the test set in a `<test_set_id>/` key. It is applied in the one function both fields share, before any S3 call, so the presigning field — which hands out a URL that carries its own authorization until it expires — cannot be served without it. The check fails **closed**: an absent claim, a caller no user record matches by either key, or a failed record lookup all deny, because "cannot evaluate the scope" must not resolve to "unrestricted". A refusal is a 403 naming neither the object nor the profile or test set it belongs to, and no S3 call is made, so it does not report whether the object exists. Keys no scope partitions are unaffected — the bundled config library, the sample documents, and every document object — so no existing viewer, export or annotation flow changes for an unscoped user. ⚠️ **A scoped user loses reads they could previously perform**, which is the intent; if a user needs a profile's or test set's files, widen their scope in User Management. ⚠️ **The Configuration bucket is also removed from the Cognito Identity Pool's authenticated role**, which granted every signed-in user direct browser-side `s3:GetObject` *and* `s3:ListBucket` on it regardless of group — an API check on that content is worth nothing while the content is readable without the API. The web UI never used it: the bucket name is referenced nowhere in the UI source, and the Configuration-bucket content the UI shows (bundled samples, the config library, profile bodies) comes from resolvers that apply the scope. **If you have custom browser-side code or an integration that reads the Configuration bucket with Identity Pool credentials, it will now get AccessDenied** — route it through `getFileContents` / `getSampleDocumentUrl` instead. The Test Set bucket was never on that role. The Input and Output buckets stay, because the UI genuinely signs those GETs in the browser; per-**document** read scoping remains open and needs a document ownership model rather than a key rule ([#1033](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1033)). The resolver gains read-only access to the user table for the lookup. Threat model: UI.T06 is re-scoped and retitled to what is actually left, AUTH.T03's first residual is corrected — "any document that group can see" was true of an unscoped caller and false of a scoped one — and the configuration-revision store is added to the asset table it was missing from. See [RBAC](docs/rbac.md#configuration-profile-scoping-use-case-isolation).

- ⚠️ **Ten caller-visible API status changes, four of them turning a previously-successful 200 into a 403** ([#1041](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1041)). The three upload paths — `uploadDocument`, `uploadSampleDocument` and the discovery upload — took a destination bucket and key prefix from the request and bounded neither; they are now confined to the buckets the deployment wires, with two write-once locations (the configuration revision store, and a run's version manifest) excluded, and a target outside either is refused **403** where a presigned POST was previously minted. On the read side `getFileContents`/`getFilePresignedUrl` answered 500 `InternalError` for a bucket outside the allow-list, a malformed `s3Uri` and an absent object, which are now 403, 400 and 400; and an allow-list that is **unconfigured** used to fail *open* to any bucket the function's role could reach, where a mis-built stack now refuses every request. `getChatMessages` for a session owned by another user answers 403 rather than 500. `listChatSessions` clamps a `limit` above 200 instead of honouring it, and `limit: 0` or `-1` on it or on `listAgentJobs` returns a clamped page instead of a DynamoDB-validation 500. `listDocumentsByDateRange` refuses a span over 365 days with a 400 naming the maximum, instead of accepting it and running past the 20-second budget the dispatcher allows a resolver; inside the cap it may now return a page plus a `nextToken` where it previously ran to a timeout, as may `listFinetuningJobs`, which no longer scans the whole table. ⚠️ **Alerting and client code keyed on the old statuses needs updating.** The reason the refusals moved is that a monitored 5xx rate was counting deliberate policy denials, so a real rise in server faults was masked by them. The authorization harness that exists to catch this class was scoring an inconclusive result as a pass — `make api-test`'s published 0.6.9 report carried 43 cells reading `500 ✅` beneath an overall PASS — so a check now records one of pass, fail, error, skip or warn, and only the first of those counts as a pass. See [the status a refusal arrives with](docs/rbac.md#the-status-a-refusal-arrives-with-and-why-it-matters-operationally) and [the 365-day cap](docs/migration-appsync-to-rest.md#listdocumentsbydaterange-is-capped-at-365-days).

- ⚠️ **The standalone seller entitlement service logs at `WARN` by default, and the feature-platform control-plane Lambdas redact the event they log** ([#1040](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1040)). That service declared `LogLevel: INFO`, is deployed by a path that never passes the parameter, and has no installer manifest to supply one, so `INFO` was literally what a seller got; it now defaults to `WARN`, matching the root template. ⚠️ A seller who wants the louder level must now pass `LogLevel=INFO` explicitly. The nine control-plane handlers behind the Extensions catalog were outside the log-redaction guard's scope on the stated ground that they are built and versioned separately from the main stack — which is not true of them, since `template.yaml` deploys them as a nested stack and the publisher builds them on every publish — and they now redact through the same canonical denylist the other handlers use. Two further trees named under that same premise, the unified pattern's handlers and the Bedrock Knowledge Base custom resources, are recorded with their site counts pinned rather than fixed. Both defects were the same shape: an exemption justifying a *set* with a reason that is a property of individual *members* and false for some of them. Every exemption in the repository is now registered and, where a predicate exists for it, its premise is computed against the tree instead of asserted in a comment.

- **Publishing a test set version happens on the test set's own page, in a dialog** ([#903](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/903)). It was an item in the **Actions** menu on the Test Sets table, which is several clicks from the set it published and a table that otherwise acts on rows; it now sits at the top of the individual test set page, beside the draft-label and annotation controls whose pass it completes. The dialog names the version number it will create and how many documents it covers, and collects the optional **Label** and **Notes** the API has always accepted — previously nothing sent them, so every version was named after its number with empty notes. Clearing **Make this the active reference** publishes without moving the set's reference point, which the one-click action always moved. The control now says why it is unavailable — the page still loading, a draft-labelling run still writing labels, a set still being written to, or no documents — and the set's own page can see the third of those because `getTestSetDocuments` now returns the set's status. ⚠️ The action is gone from the Test Sets table, which keeps its **Version** column; the mutation takes one test set at a time, so no bulk publishing is lost. It is also offered only to Admins and Authors, the groups `publishTestSetVersion` has always been restricted to — other roles previously saw it enabled and got a 403. See [Publishing a version](docs/test-studio.md#publishing-a-version).

### Fixed

- **The error analyzer no longer reports a Lambda layer name or an event-source-mapping id as the function that failed.** The Lambda tool's event-history parser carried a fallback that read a function name out of the seventh colon-separated segment of a `resource` ARN whenever no `:function:` marker was found. Every Lambda *function* ARN contains that marker, so the fallback could not fire for anything a Step Functions history actually carries — and the ARNs that *could* reach it are Lambda ARNs that are not function ARNs, for which segment seven holds a layer name, an event-source-mapping uuid or a code-signing-config id. Measured directly: a layer ARN yields the layer's name, not its version. Those values were appended verbatim to `failed_functions` and `primary_failed_function`, which is what the agent names to an operator as the failing function and what selects the log group it goes on to search — so a plausible-looking wrong name is worse there than no name. The branch is removed: an event whose `resource` is a Lambda ARN with no function name now contributes nothing, which is already what an unreadably short ARN did. No input the pipeline produces behaves differently, so there is nothing to do; this completes the correction begun in [#1061](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1061), which fixed the length guard that made the same branch raise `IndexError` ([#1128](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1128)).
- **Advanced extraction: a required list or nested object no longer costs a correction round per shard** ([#1078](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1078)). Two boundaries apply to one shard of a sharded section, and only one of them was shard-scoped. The in-loop *feedback* validator drops `required`, deliberately and correctly — a shard is given only its own pages and is told "if a field does not appear in your pages, leave it null — another shard will provide it", so a document-level required field is legitimately absent from most shards. The shard's `extraction_tool`, however, was built from a model that kept `required` non-nullable for a **container**: the transport transform widens scalar *leaves* so an agent can abstain on a cell it cannot read, and it adds no null branch to an array or to a nested object. So for a required list or group the shard's instruction named the one answer its own tool rejected — measured as `list_type` and `model_type` against the model directly — and a shard covering pages that genuinely contain none of a required table spent a round discovering it had to emit `[]` rather than `null`, with the feedback validator reporting that same payload as satisfying every constraint, so the feedback did not point at the cause. The fix scopes the tool boundary per shard rather than propagating `required` into the feedback validator: a required array or nested object now accepts `null` in the shard's model, and presence is judged once on the **merged** section against the real class schema, where a null property reads as absent and is reported by `extraction.validation` like any other required-property violation. The structural half deliberately does not relax — `required` is still present, so an omitted key, an empty tool call and a misspelled key set still fail per shard, which is the guard that stops a shard answering nothing and merging as a success — and **row-count bounds are untouched**, so the documented per-shard `minItems` floor behaves exactly as before. That separation is the reason the two constraints get different guidance: a shard holding none of the rows *can* satisfy `required`, and can satisfy no section-sized `minItems` at all. **No shipped configuration paid this cost:** three preset files declare `required` on a list (`LineItems`, in the `realkie-fcc-verified` family), none declares it on a nested object, and none of the three enables Advanced mode — so the round fell on a configuration that turns Advanced mode on with a required list, which is the documented Advanced use case. Both fan-out routes take the shard model, because a fix applied to the in-process runtime and not the Step Functions one would be half a fix. Two residuals are recorded rather than left to be discovered. A required property that is a **bare `$ref`** is not widened, since there is no keyword to widen without resolving the reference and the `anyOf` alternative renders an array field as a wrapper model, which breaks the shard merge's list detection — such a property still costs one round per shard. And during a Lambda version rollover a shard running new code can persist a `null` that a merge step still running old code refuses; that raises on the merge step, which Step Functions retries, so the window is one retry and closes without intervention. No migration is needed and the persisted shard format is unchanged. See [`minItems` in Simple and Advanced mode](docs/extraction-and-confidence.md).
- **Test Studio's confidence figures and the stored confidence curves now key a list index the same way** ([#1066](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1066)). The test-set resolver carried its own copy of the field-path rule, keying a list index off the list's **length** rather than its depth, so a one-row table keyed `Transactions.Date` there while the evaluation package — which is what a stored confidence curve is keyed by — keyed `Transactions[0].Date`. Nothing crossed the two, so no number served was wrong; what made it worth fixing is that the resolver's own two walkers are compared against each other by path-set intersection, and a length-keyed rule makes that comparison depend on the data. Two consequences were measured, both of which the depth-keyed rule removes: with a **one-element** list of null rows the list-level confidence was excluded from the headline score while the same payload with two null rows excluded nothing, and should `explainability_info` ever carry more than one wrapper element every confidence path gains a `[0]`/`[1]` prefix that the extraction-result walk cannot produce — which switches absent-field exclusion off for the whole document rather than failing. The rule now has one implementation, exported from `idp_common.evaluation` and imported by the resolver. The two traversals stay where they are and that is deliberate rather than a partial job: one needs the `confidence_threshold` beside each score, which the shared flattener skips, and the other needs an **empty list** to count as an absent field, which the shared flattener cannot report because an empty list has no scalar leaf — so they take the rule and not the walk. ⚠️ **One user-visible change:** a list-level confidence measured against a single null row is no longer excluded from `minConfidence`, `confidenceThreshold` and `alertCount`, which is the answer a multi-row list already gave. An *empty* list is still treated as absent, unchanged. The resolver runs on the base Lambda layer and the import adds nothing to it: that layer is `core + docs_service + image`, the evaluation package's eagerly-imported modules are standard-library-only, and the resolver already imports from it.
- **Two agentic-extraction notebooks state the schema-validation configuration that ships** ([#1063](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1063)). `notebooks/examples/step3_extraction_using_agent_idp.ipynb` and `notebooks/misc/agentic-extraction-validation-and-escalation.ipynb` each gave four facts that no longer hold: the block's path, `enabled`'s default, `fail_action`'s default, and what `reject` does. The block is `extraction.validation` — a sibling of `agentic` under `extraction`, because Simple extraction runs the same validate-and-retry path — `enabled` defaults to `true`, `fail_action` defaults to `warn` because `warn` is free and an on-by-default guard should not spend money on every failure, and `reject` records `parsing_succeeded: false`, which the section's Processing Report and the UI's Processing Report tab read and nothing in the status path does: the values are still written, the section and the document still complete, and **no review task is created**. Human review is routed by the top-level `hitl` block against per-field confidence. Both notebooks also now carry the `minItems` guidance for Advanced mode — a hard floor at the agent's tool boundary, enforced per shard — and a note that a misspelled or mis-nested key under `extraction` is accepted in silence, which is the reason a wrong path in a walkthrough is not a harmless typo; that mechanism is [#1134](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1134). Nothing about the notebooks' behaviour changed: a stored configuration on the pre-v0.7 path is relocated on read, which is why every captured cell output still reproduces byte for byte. Notebook prose is read by no lint or doc gate, so both files were checked by hand and their structure — cell count, cell ids, execution counts and captured outputs — verified unchanged.
- **The benchmark harness no longer records a measurement it could not take as a measurement of zero** ([#1079](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1079)). Six readers returned the same value for "there was nothing there" and for "I could not read it": an S3 object that was missing and one that was undecryptable both answered `None`, and a tracking row that held no metering and a DynamoDB failure both answered an empty map — which prices to `$0.00`. That matters more than an ordinary silent failure because these artifacts are published and an unread figure recorded as zero drags an average toward zero, making a configuration look cheaper, faster or less accurate than it is in a document a reader takes at face value. The shape that made the class visible was a release stack whose KMS key entered pending deletion, leaving every object in its output bucket present, listable and undecryptable, so the grid read as one that had recorded nothing. That is the normal end state of a torn-down benchmark stack rather than an exotic one: ten of the twelve stacks the committed artifacts name no longer exist. Every reader now returns a three-state result — present, absent, or failed with the reason — through one shared type rather than six copies of a convention, and the type is built so the distinction cannot be dropped by accident: truth-testing one raises, the value is unavailable unless the read happened, and substituting a default is allowed for an absence and refused for a failure. The callers act on it: the cost path leaves `cost` null and names the state in `cost_unread` rather than pricing an unread metering row, while a genuinely unmetered run still prices to `$0.00`; a document whose section objects did not all read contributes no metric at all rather than an accuracy over whichever subset decrypted; per-cell roll-ups count the excluded runs, so a mean over a thinned sample says it was thinned; the pooled calibration report gains an `unreadable` bucket so an undecryptable grid is no longer counted as a run that emitted no confidence; and the calibration backfill asks every row instead of probing the first empty prefix, which is the case the previous probe reported as clean. The test bootstrap for `benchmarks/tests` had the same defect in the layer meant to detect the others — a working-directory-relative path behind an `importorskip`, so running a suite from the wrong directory reported a green exit having run nothing — and now resolves the harness from its own location and errors rather than skipping, with one test invoked from a foreign working directory to prove it. No committed benchmark artifact was affected: across all 7,124 rows in the 105 committed benchmark result files that carry rows, of which 6,931 are successful runs, none carries a zero or null cost, an empty token map, a zero-section score, or a calibration figure derived from an unreadable read. See [Read failures do not silently become measurements either](benchmarks/matrices/METHODOLOGY.md#read-failures-do-not-silently-become-measurements-either).
- ⚠️ **`rule_validation.semaphore` now bounds the rule-validation orchestrator's Bedrock calls, which it previously did not, so consolidation gets slower and quieter on rule-heavy documents** ([#1053](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1053)). The orchestrator's semaphore was behind a property that discarded its cached value on **every** access: the guard meant to drop a semaphore left over from a previous event loop tested `semaphore._loop != loop`, and `asyncio.Semaphore` inherits `_loop` as a class attribute initialised to `None` and assigns it only when an acquire actually has to wait — so on a fresh semaphore the guard was always true. Both call sites spell it `async with self.semaphore:`, which re-evaluates the property once per task, so every task acquired its own semaphore and none of them ever waited. **The configured value had no effect on that step at all.** What bounded it instead was the event loop's default thread pool, because the Bedrock client call is blocking and goes through `loop.run_in_executor(None, ...)`: `min(32, os.cpu_count() + 4)` threads, which is larger than the default of 5 on any container with more than one CPU. Measured with a real thread pool, a 14-rule document made 14 concurrent calls with `semaphore: 1` configured; it now makes the configured number. ⚠️ **This is a throughput reduction and it is intended.** How large it is depends on that pool width rather than on the setting, so the figure worth acting on is the deployed one: at 4,096 MB the orchestration function's pool is roughly 6 to 7 threads, so a 14-rule document went from about two waves to three. Measured end to end against Claude Sonnet 4.5 on a 16-CPU machine — a 20-thread pool, so all 14 rules ran at once — consolidation took 16.9 s at the default of 5 against 5.7 s unbounded, which is an upper bound on the change rather than the deployed one. Raise `semaphore` if you would rather have the speed and your account's Bedrock quota can carry it, remembering that the account-level rate is this value times the number of documents in flight: at the default `MaxConcurrentWorkflows` of 100 a `semaphore` of 5 already permits up to 500 concurrent calls from this step alone. No shipped configuration preset overrides the key, so every deployment has been running the fact-extraction step at 5. The fact-extraction step in `RuleValidationService` was already bounded correctly, so the two steps disagreed about what one config key meant; both now read one shared implementation, which tracks the loop it handed a semaphore out on rather than reading a private asyncio attribute. The test that covers it counts **peak** concurrent model calls through a real `ThreadPoolExecutor` whose width is deliberately set above the limit, and measures in the same run that the pool on its own admits more — a test that replaces the executor with an inline stand-in serialises the calls and passes against the defect. See [rule validation configuration](docs/rule-validation.md#configuration-parameters).
- **A Z3 rule whose constraint misspells a parameter is rejected when the rule is generated, instead of failing every document that rule is evaluated against** ([#1058](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1058)). `RuleJSON._validate_constraint_parameters` was documented as raising `ValueError` for a constraint referencing an undeclared parameter, and its `Raises:` clause plus its place in the constructor's validation list read as though a rule were checked; the loop that was supposed to do the checking had a body consisting of `continue` and two comments, so the whole method could be replaced with `return` without any test noticing. The cost was not a missing check — the solver does reject the unknown atom — but where it lands: a translated rule is persisted under a key derived from the rule **description**, so one bad translation was re-read from the cache and re-failed for every later document whose rule carried that description, each reporting *Information Not Found*, until the description changed. It now raises, which `RuleTranslator` surfaces as a `TranslationError`, so the Config Editor's "Generate RuleJSON" button reports which token could not be resolved and the retry costs one translation. The check and the solver read **one** vocabulary (`z3/smt_grammar.py`): the same tokeniser, so a word inside a string literal such as `(= property_type "condo")` is not read as a parameter — which a regular expression over the raw text does, and the translator's own few-shot examples teach the model to emit that form — and the same operator set, which `_apply_smt_operator` now gates on rather than mirrors, so `mod` and `distinct` are accepted where a hand-written keyword list omitted them. The check is bounded to **names**, deliberately: a token that is neither a name nor a numeral, parenthesis balance and operator arity stay with the solver, which keeps the construction-time check from holding a second definition of what a numeral is. It also imports no solver, because `RuleJSON` is built on the configuration-resolver Lambda, whose layer ships `idp_common` without `z3-solver` — a test constructs a rule with `z3` unimportable to hold that property. See [Z3 rule validation](docs/rule-validation-z3.md) and `lib/idp_common_pkg/idp_common/rule_validation/README.md`.
- **The S3 Vectors custom resource's handler tests run in CI, and every one of them can now fail.** `nested/bedrockkb/src/s3_vectors_manager` was held out of `make test` by one assertion in `test_handler.py`, so its five tests reached neither CI while the IAM-scope suite beside them did; the root is now registered to run and the `make test-packages-cicd` recipe names the directory rather than its `tests` subdirectory, taking that package from 19 gated tests to 24. The assertion that held it back drove `get_s3_vector_info` by stubbing `get_index`, which the handler does not call — it always attempts `create_index` and distinguishes an index it created from one that already existed by whether that raises `ConflictException` — so both arms of the status it reports are now exercised through `create_index`, and the index ARN handed to the Bedrock Knowledge Base is compared in full rather than for presence. Two further tests in the file asserted nothing at all: one compared no bucket name against its table of expected values, and one checked `hasattr` against a `Mock`, which is true for every name. The first now asserts each sanitization rule and that the result is a name `create_s3_vector_resources` will accept; the second scans the handler for the `s3vectors` operations it actually calls and checks each against botocore's service model, which is the one claim in the file a mock cannot establish. Each test was confirmed to fail against a deliberately broken handler ([#1083](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1083)).

- **Counts stated in prose about CI gates and about the tree are now derived or removed, and a gate holds them there.** Documentation across `CLAUDE.md`, `docs/testing.md`, `CONTRIBUTING.md`, the `Makefile`, `scripts/README.md`, the branch-protection checker and the CI-coverage page described the shared CI gate set and the type gate's file coverage with figures that nothing computed; the only assertion on the gate list's size was a floor, which admits any number above it. `scripts/tests/test_documented_counts.py` now derives the shared-gate total from `test_ci_gate_parity.py`'s `SHARED_GATES`, derives how many of them are steps in GitHub's single `developer_tests` job by reading that workflow, derives how many status-check contexts hold one, and fails if any document states a different number — in both directions, so adding a gate without updating the prose fails too. The topology statement is the part worth keeping and is now correct: eight of the ten shared gates share one requireable context, and the SRT scan and the dependency audit are jobs of their own, so the ten produce three contexts. Where a figure was incidental — how many files `basedpyright` reads — the literal is gone in favour of the command that produces it, since that number changes with every commit that adds a file and pinning it would fail routine changes; the patterns for it remain only to stop a figure returning, and are tested against the dated `#975` statements they must not disturb. `make typecheck-pr` is also no longer described as something CI runs; the type gate is the whole-tree `make typecheck` ([#1084](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1084)).

- **A BDA project ARN can no longer reach the Bedrock Data Automation APIs as `None`.** `BdaBlueprintService.get_or_create_project_for_version` was typed as returning an optional string, on the strength of one branch that returned the constructor's ARN when `CONFIGURATION_TABLE_NAME` was unset — a branch no constructed instance can reach, because the constructor builds a `ConfigurationManager`, which refuses to exist without that same variable. It now returns a string and raises when the table is absent, which is what every other failure in the method already did, and `blueprint_optimizer.py` passes the result to two parameters declared as strings with no guard needed. The optimizer reports the raise as a failed optimization through the handler it already has; previously a `None` would have surfaced two frames later as `TypeError: 'NoneType' object is not iterable`, because the blueprint lookup answers `None` for an absent ARN. ⚠️ This class of defect is invisible to the type gate: `pyrightconfig.json` sets `reportArgumentType` to `none`, and enabling it repo-wide is a four-figure cleanup rather than part of this change — with the rule on, the attribute itself still flows into string parameters at ten further sites in the same service ([#1085](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1085)).

- ⚠️ **A Z3 rule could be decided using a different document's readings.** The path-based data extractor memoized each reading under the *address* of the document it read (`id(data)`), and CPython reuses addresses: once a document was freed, the next allocation could land on the same address, and the memo then answered for that new document with the previous one's values. Those readings are the values bound into the solver, so the outcome is not an error or a missing field — it is a compliance verdict computed against another document's data, reported with normal confidence and with nothing in the output marking it. On the affected route this was the normal case rather than an edge one: reading fifty successive documents through one extractor, **48 of the 50 readings were another document's**, and the in-place-modification form below failed every time it was run. What is non-deterministic is only *which* documents collide, since that depends on what else the program allocated in between — a pair of documents that collided in one reproduction did not in a more minimal one. **There is no way to identify affected documents retrospectively**: nothing was recorded that would tell such a verdict apart from a correct one. It cannot be ruled out that a verdict was decided this way; equally, there is no evidence that one was. The reachable surface is the library's path-mapping extraction route — `DataExtractor`, `ValidationSystem` and `Z3EngineAdapter`, as used by the dual-engine notebook and by any code embedding them — and not the deployed pipeline's Z3 route, which extracts values in its own LLM call and hands them straight to `Z3Validator` without going through this memo. The memo is now scoped to a single `extract_values` call, so the extractor keeps no document state at all between calls and one instance is safe to reuse; `clear_cache()` is retained for callers and does nothing. The same change fixes a second reading of the same cause: a document modified in place between two calls was served its pre-modification values, because the address had not changed ([#1115](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1115)).
- **A rule-validation report keeps its statistics when a page reference is unusual, and the document's page list no longer counts one page twice.** Page references are now canonicalised where they are collected rather than repaired in the sort: each is coerced to `str` with its ordering value parsed alongside, so the published `supporting_pages` in `consolidated_summary.json` is a list of strings, deduplicated and totally ordered — numeric references by value, everything else by its own text. Four consequences. A document that routes some rules to the solver and some to the model mixes `str` and `int` by construction, and page 1 was listed twice, once in each shape. Non-numeric references all shared one sort key, so their order came from set iteration and differed between interpreters; the document-level list is now reproducible, while each rule's own list in the same file is ordered by the engine that produced it and a solver rule's non-numeric citations still carry that shared key. A reference that `str.isdigit()` accepts but `int()` refuses — `'²'`, `'₂'`, `'②'`, or a decimal string past the 4300-digit conversion limit — raised in the sort key, and a value that cannot go in a set at all (a `list`, a `dict`) raised even earlier; both now stay out of the way of the report, with the non-page values dropped from the aggregate and named in a warning. The **per-rule** lists are untouched and still hold exactly what each response returned, so nothing is lost. The one consumer of the document-level list, the sample health-insurance review extension's claim-detail API, already declared it `string[]` on the UI side, so this brings the value it serves into agreement with its own contract. Separately, the broad `except` around the consolidation no longer discards the report: it keeps the policy types, rules, counts and pages gathered before the failure, records the reason in `error`, and the Markdown report states above the statistics that consolidation did not complete, so partial figures cannot be read as complete ones. Those partial figures are now also internally consistent — a rule is counted once it has been read rather than before, so the report no longer claims more rules than it details nor divides by the larger number, and a policy type interrupted part way through keeps the rules it did read instead of vanishing from the report. That path still records no processing issue — `rule_validation_not_consolidated` is attached by the orchestration Lambda when its handler raises, and this method deliberately does not raise — which is why the surviving statistics, the banner and a logged traceback are what make an instance of it visible. See [Page Number Handling](lib/idp_common_pkg/idp_common/rule_validation/README.md#page-number-handling) ([#1114](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1114)).
- ⚠️ **The error analyzer named no failing workflow state at all, and one malformed event field discarded an entire document investigation.** Asked which state a document's workflow failed in — the question the agent's Step Functions tool exists to answer — it reported `Step Function execution FAILED at state 'None'`, then recommended checking the state it had just failed to identify. Three independent defects stood between the tool and the answer, and any one of them alone produced that output. The event history was fetched newest-first and walked as though it were oldest-first, so the failure was seen before any state transition. The walk matched state transitions against a `type` of `StateEntered`, which the history API never emits — every transition is prefixed with the state's own type, `TaskStateEntered` and eight more, and the unprefixed spelling belongs to the *detail* field — so no transition was ever read on any real execution regardless of order. And the failure selected was the **first** in the history rather than the terminal one, which on this workflow is frequently a throttle that was retried and recovered: naming it points at a state that went on to succeed, with an error nobody needs to act on. Events are now ordered by the history event `id`, which the API documents as a required sequential integer and which — unlike a timestamp, whose millisecond resolution ties routinely between adjacent events — gives a total order with no direction to infer and no tie to resolve. Transitions are matched on the type *suffix*, so all nine forms are read and any the service adds are too. The terminal failure is the one reported, which also prefers the `ExecutionFailed` event naturally, since a failed execution ends on it. ⚠️ **The history is still read as a single un-paginated page of the newest 100 events**, which is enough to reach the failure but not always the failing state's entry; when the page is incomplete the analysis now says the state could not be identified and why, rather than reporting a bare `None` that reads as a finding — full pagination is tracked in [#1127](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1127). Separately, the same agent's Lambda tool raised `IndexError` on a Lambda ARN too short for the segment its length guard admitted; because the call sits inside the document-context lookup's exception handler, the result came back as *document not found* with a message about list indices, throwing away every request id and failed-function name already gathered — and that flag is what decides whether the agent investigates at all. A malformed resource now contributes nothing instead. The tests could not have caught the type-name defect: every fixture in the module was built with the spelling the service never sends, which is a suite validating against an input that cannot occur, so the fixtures were re-pointed at real event types and the retry shape that distinguishes first-failure from last-failure was added ([#1061](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1061), [#1081](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1081)).
- ⚠️ **Agent chat transcripts were losing about half their messages, and existing transcripts are incomplete** ([#1098](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1098)). Every message an agent added was appended by reading the whole stored transcript, growing it by one and writing it back, with nothing to detect that another write had happened in between — so concurrent appends overwrote each other and the last writer won. Agent monitoring is on by default and a single agent turn adds several messages within microseconds, so the overlap was the normal case rather than an edge case: four messages submitted persisted two. Nothing reported it, because every individual write succeeded. The append is now conditional on a version attribute read alongside the transcript, so an overlapping write is rejected by DynamoDB and rebuilt on a fresh read instead of replacing the other message, and a rejected append backs off for a jittered interval before retrying. The backoff is not a refinement: rejection alone stops the silent overwrite but leaves the losers re-entering the same race in phase, and measured across writer counts that plateaued around a quarter of the messages still lost. With jitter, no message was lost at any width measured, up to and past the widest fan-out the system can actually produce. A message that still cannot be stored after the retry budget is dropped rather than forcing it through by overwriting the transcript — which would trade one lost message for all of them — and the drop is now **counted on a new `AgentTranscriptMessageDropped` CloudWatch metric with an alarm** (`AgentTranscriptMessageDroppedAlarm`, ten drops in 15 minutes), not only written to a log, because a log line cannot be alarmed on and invisibility is what let this defect run. The same correction is applied to the multi-turn conversation memory behind agent chat, where appending rewrote the newest stored item and two invocations serving one session could each drop the other's message; there a *failed* read also no longer stands in for a session with no history, which had forked a conversation into a second item. **Action:** transcripts already stored are not repaired by this change and remain missing whatever was overwritten; agent runs from here on are complete. Records written before the upgrade are appended to normally — the guard is added by the first new message — so no migration is needed and the stored format is unchanged. ⚠️ The guard binds writers that *have* it: during the Lambda version rollover, a writer still running pre-upgrade code updates the transcript without touching the version attribute, so a new writer's condition still holds and can overwrite it. That window is the rollover itself and closes without intervention; eliminating it would need the backfill migration this change deliberately does not do. A second way to lose a transcript entry — a queued write abandoned when the execution environment freezes — is the next entry below. Nine more read-modify-write sites of this shape remain across the codebase and are **not** fixed here ([#1111](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1111)).
- ⚠️ **Rule validation's character chunker had a non-terminating path, and `overlap_percentage: 0` asked for no overlap and got the maximum.** **Action:** if you have set `overlap_percentage` above 50, the character-based fallback now repeats 50% rather than the figure you set, which changes the prompt content of the affected calls; it logs a warning naming both numbers when it does. No action is needed at the shipped default of 10. Two defects in the same pair of functions. `_chunk_text_with_overlap` computed each chunk's start from an `end` index it had already clamped to the length of the text, so once a chunk reached the end the start stopped advancing: the same tail slice was appended on every pass, and the loop terminated only when the requested overlap rounded down to zero characters. The failure is **fast and loud rather than a silent hang** — each pass appends exactly `overlap_chars`, 3,200 bytes at the shipped 10% of a 32,000-character chunk, so a 4 GB function is exhausted within seconds, and Lambda enforces memory as a cgroup limit, so the process is killed rather than raising. The state machine retries it and records the failure; what is lost is the invocation, not the diagnosis. **Reaching it takes an unusual document but not an implausible one.** The character chunker is only the fallback for text in which the page parser finds *no* pages, and text carrying no markers at all does not qualify — that is parsed as a single page. The way in is markers with no content between any of them, which a section of whitespace-only OCR text produces: the split pattern's trailing `\s*` consumes the whitespace so every page strips to empty and is dropped, while those characters still count toward the length that decides whether chunking happens. A failed scan, an image-only PDF or a batch of separator sheets can do that. `_chunk_pages_with_overlap`, the function every document goes through, always terminated. The loop now decides a chunk is the last one before moving the start, and the overlap is bounded at **half a chunk**: `overlap_percentage: 100` is a legal setting, and merely making it finite is not enough, because a one-character stride emits one chunk — one model call — per character. A request above 50 is reduced and logged. Separately, the page chunker's single-page overlap branch took the tail of the previous page with `page_content[-overlap_size:]`, and `x[-0:]` is `x[0:]` — the whole string — so **`overlap_percentage: 0` repeated the entire previous page rather than none of it.** On a document whose pages are large relative to `max_chunk_size` that roughly doubled the prompt tokens of every fact-extraction call, and the extra content was a verbatim duplicate of what the model had already been shown. Nothing failed and no verdict looked wrong; the only symptom was the bill, moving in the opposite direction to the knob that was turned. That branch now repeats nothing when nothing was asked for. Note the percentage governs the single-page branch and the character fallback only: where the previous chunk held several complete pages, the whole last page is repeated regardless of it ([#1090](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1090), [#1091](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1091)).
- **An agent conversation-memory limit of zero loaded every stored turn instead of none.** The same zero-length-slice shape as above, found by auditing the tree for it rather than by a report: `DynamoDBMemoryHookProvider` truncated history with `turns[-self.max_history_turns:]` behind a `len(turns) > self.max_history_turns` test, which at zero is true for any non-empty conversation and admits the one value the slice cannot express. `max_history_turns` comes from the `MAX_CONVERSATION_TURNS` environment variable with no validation, so setting it to 0 — the obvious way to turn conversation memory off — loaded the whole session into the agent's context. The shipped templates set 20, so this needed reconfiguration to reach. Zero is now handled before the slice.
- ⚠️ **Nothing waited for a queued agent-transcript write, so the last messages of an analytics agent's turn could be abandoned when the Lambda execution environment froze** ([#1110](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1110)). Transcript messages are written on a thread pool so a DynamoDB round trip stays off the agent's critical path, and the drain path existed — `DynamoDBMessageLogger.shutdown` waits for the pool, and `DynamoDBMessageTracker.shutdown` calls it — but **nothing in production reached it**: the tracker was constructed and registered as a Strands hook, and `IDPAgent.__exit__` closed only the MCP client, so the only route to the pool ran through a private callback table. This is a second loss path on the attribute the conditional-write fix above guards: that one stops writers overwriting each other, and does nothing for a write that never ran. **Scope:** only the analytics agent records a transcript. A logger needs `job_id`, `user_id` and `AGENT_TABLE`, and agent chat passes no identifiers while `AGENT_TABLE` is set on just two functions — the analytics processor and an MCP handler that builds no agent — so agent chat stores no transcript to lose and its multi-turn history is a different attribute with its own guard. **Why an interpreter-exit hook is not the alternative**, since that is the fix most likely to be reached for: the mechanism that would flush a pool is not `atexit.register` but `threading._register_atexit(_python_exit)` in `concurrent/futures/thread.py`, whose comment says as much, and it joins every worker with **no timeout** — an unbounded join is exactly what must not happen at an agent boundary. Reaching it needs an orderly interpreter shutdown, which neither end of a Lambda environment's life provides: an invocation ends with the environment **frozen** rather than exited, and Python's default `SIGTERM` disposition terminates without running exit hooks of any kind, so even a function whose extension earns it a `SIGTERM` would flush nothing. The pool is now drained at `IDPAgent.__exit__`, which both places an agent is entered on that path reach — the analytics processor's `with agent:` and a sub-agent's `with specialized_agent:` in the orchestrator, the latter now entered on the same statement that creates it so a raise in between cannot leak an undrained pool. The drain is **bounded** at two seconds rather than unconditional, because for a sub-agent that exit happens mid-turn with the user waiting; two seconds covers a full contended ladder for the write in flight (fifteen attempts, thirty round trips, at most 0.59 s of jittered backoff) with room for a queue behind it. It bounds the *wait*: reporting what the wait left behind adds one synchronous CloudWatch call, which is why the report is a single datum carrying the count rather than one call per message. **How much was at risk:** narrowing the pool to one worker bounds how many writes run at once, not how many are waiting, since the pool's queue is unbounded — 50 submitted at width one leaves 49 queued. What bounds it in practice is the producer: a message reaches the logger once per model or tool round trip while a write costs two DynamoDB round trips, so the steady-state queue is empty and the exposure is the write in flight plus whatever arrived during it — the tail of the turn, which is the part a user is most likely to be looking at. Closing the pool also releases its worker thread, which is not a daemon thread and otherwise idled for the remaining life of a warm execution environment, one per sub-agent per invocation. See [Agent Transcript Write Failures](docs/monitoring.md#agent-transcript-write-failures).
- **A drain that could not finish is now reported on its own metric and alarm, `AgentTranscriptDrainIncomplete` / `AgentTranscriptDrainIncompleteAlarm`,** rather than on the dropped-message counter beside it. The two do not license the same conclusion. A dropped message is gone: the append exhausted its retries or its reads kept failing. A write the bounded drain gave up on was abandoned, not cancelled — Lambda resumes unfinished background work if that execution environment is thawed for another invocation — so it frequently commits, just after the session reading the transcript has moved on, and is lost only if the environment is reclaimed instead of reused. Counting it as a dropped message would page whoever alarmed on a destroyed transcript every time an environment froze and thawed. The new alarm mirrors its sibling's shape (ten or more messages in fifteen minutes, `AlertsTopic`, `notBreaching` on no data) and its datum carries the number of messages left behind, so `Sum` counts messages rather than drains. Both alarm descriptions now say which of the two conditions they are about and point at the other.
- **A message's `sequence_number` is now unique within a job.** Every sub-agent in an analytics agent's turn builds its own logger on the same job record, and each counted its own ordinal from zero, so the same value named several different messages in one transcript ([#1106](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1106)). The ordinal is now the message's 1-based position in the stored transcript, computed as `len(stored) + 1` from the array the write already had to read — inside the existing conditional write, so it costs **no extra round trip and adds no second source of truth**, and the condition is what makes it exact: a write commits only if nothing appended in between, so no two stored messages can have computed the same length. A rejected append recomputes it on the fresh read and a dropped message consumes no ordinal, so among messages written by this code the series is 1..N with no duplicates and no gaps. Two limits on that: a transcript written before this change keeps whatever its per-logger counters produced, since appends continue from its length rather than repairing it; and during the Lambda version rollover a writer still on pre-upgrade code appends without touching the version attribute, so it and a new writer can collide or skip — the same window the version guard itself cannot close, and it needs no intervention. **What the collision did and did not affect:** nothing sorts on this field. The writer appends, so the stored array is already in commit order, and the UI renders that order as it is; the value reached the screen only as one component of a React list key that also contains the array index, so a repeat produced no duplicate-key warning and no visible mis-ordering. Where it was actually read was the drop-path log line the monitoring guide tells an operator to search for, and there a repeat made the identifier ambiguous — several loggers each reporting "sequence 3". Those log lines now name the message's role and timestamp instead, because a message that was never stored has no position in the transcript to name. Note the ordinal is a position in the stored array and not a row number on screen: the UI drops empty-content messages and splits a mixed text-plus-tool-use assistant message into several rows. One related change to a public accessor: because the write now works on its own copy of the message, the dicts `DynamoDBMessageTracker.get_messages()` returns no longer acquire a `sequence_number` — the ordinal belongs to the stored transcript, and assigning it on the shared object would mutate it from a worker thread and more than once if the append were rebuilt. The conversation-memory provider behind agent chat derives its own ordinal from a microsecond timestamp rather than an instance counter, so it never had this shape and is unchanged.
- **A rule-validation compliance report no longer comes back empty when the model numbers its page references.** The consolidated summary sorted its collected page references with a key that called `.isdigit()` on each value, which exists on `str` and not on `int` — so a model answering `"supporting_pages": [1, 2]` rather than `["1", "2"]` raised inside the sort key, and the catch-all around the whole method turned that into a report with every statistic discarded. The report looked like a document where nothing had been evaluated, and because the catch-all returns normally rather than raising, no processing issue was recorded either. The key now coerces before testing, so both shapes sort. Contributed by @mrhard9090 ([#1052](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1052)).
- ⚠️ **A document that fails rule validation or the collate step now records *why* on the sections it affects, and one failure recorder that had never run now runs.** Three handlers on the unified pattern held an explanation of the failure and raised without writing it, and none of their task states has a `Catch`, so the document's own record carried no account of what went wrong — the Sections panel showed nothing, and the explanation existed only in the Step Functions cause and the CloudWatch log. The per-section rule-validation Lambda owned no section write at all; the collate step raised after its last write, discarding both the per-section attribution it had just computed and the FAILED status it had just assigned. The rule-validation orchestrator was the one that tried, and **it could not execute**: it named a `Status` member that does not exist and passed a keyword the document service does not accept, both of which landed in a surrounding `except` that logged "Failed to update document status" and swallowed them — a line that reads like a transient DynamoDB problem. It had never recorded anything, and nothing in the tree ran it. All three now persist an error-severity processing issue before re-raising the original exception **unchanged**, so the document's Sections panel shows which sections failed and why. ⚠️ Look there rather than at the document list: the list's **Processing Issues** badge is only refreshed by the two steps that write the whole document, so a per-section rule-validation failure can leave it showing its previous value — the per-section write updates one section atomically and cannot know the document-wide total, since it does not read the stored item and ten sections are written concurrently. A transient failure records nothing, because the state machine is about to retry it and a marked section would show red through an eight-attempt ladder and then clear; the retry that justifies that suppression is now guaranteed for every failure the library calls transient, which is the entry below. `document.errors` is deliberately **not** given a home: it is the free-text signal `ProcessingIssue` replaced, it has never been persisted, and a document-level issue would raise the list badge with no text behind it — where a diagnosis is genuinely document-scope and no section can be named, it stays in the exception rather than being attributed to a section by guess. The shared body lives in one library module, so the extraction path added in the previous release and these three cannot drift apart on the rules that matter — preserve other stages' issues, never mask the original exception, skip transient failures, bound the free-size text against DynamoDB's item ceiling. A new cross-language gate fails when the backend's set of "the stage raised" codes and the UI's disagree, because that set is what makes a section read **Failed** rather than the milder **Incomplete** and it is a hand-written literal on the UI side. See [Where a failed rule validation shows up](docs/rule-validation.md#where-a-failed-rule-validation-shows-up) ([#1064](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1064)).
- ⚠️ **A throttled or timed-out Bedrock call during rule validation is now retried instead of ending the document — and it can no longer be answered with a compliance verdict.** Rule validation absorbed transient faults in nine `except` blocks rather than raising, so the retry ladder its task states already carry — eight attempts at 2.5× backoff from ten seconds, about 2.8 hours, provisioned for exactly this — was unreachable, and the failure surfaced instead as one of three wrong outcomes. A fault in a single rule's model call returned `Information Not Found`, which is not an error channel but a **verdict**: one of the configured `recommendation_options`, counted in the document's summary, and read by the sample health-insurance feature as *insufficient documentation*. A fault anywhere else in the per-section step set `Status.FAILED` and returned, after which the Lambda raised a plain `Exception` built from that status — so by the time Step Functions saw the failure the original exception, the only thing carrying the classification, no longer existed, and `Exception` is in no retry list: the document finished failed and had to be re-uploaded. A fault during consolidation was the quietest, because that step returned an **empty** result normally, so the document completed with no verdicts, no failed status and nothing recorded anywhere. Each of those blocks now asks `idp_common.utils.transient_errors` first and re-raises a transient cause under `TransientError`, the one name the three rule-validation task states list; a deterministic failure keeps its existing behaviour exactly, because eight more attempts cannot validate a document that fails the same way every time. Two adjacent faults that silently changed a *result* rather than reporting a failure are covered the same way: a page whose text could not be read is a page whose policy-matching regexes never ran, so a policy type evidenced only there went unmatched and none of its rules were validated; and the cleanup that clears the previous run's per-section results is not best-effort, since the orchestrator reads exactly the prefix it clears, so a cleanup that silently did not happen meant the last run's verdicts were consolidated as this run's. **The two classifications are now one.** Step Functions can only match an exception's class name, while transience is a property of the error code and the cause chain — a question no retry list can ask — so the library answers it once and puts the answer on the wire under a single name, rather than each state carrying a copy of the vocabulary that drifts. Three shapes were **observed** against live `bedrock-runtime` — a read timeout, a connect timeout and an unreachable endpoint — each transient to the library and matching none of the names those states carried before. Four more are **derived from botocore's bundled service model** rather than from an induced failure: `ModelTimeoutException`, `ModelNotReadyException`, `InternalServerException`, and a throttling code arriving under a class botocore did not name after it, which is what botocore produces when the code is not in the service's error map. ⚠️ Two residuals. A ladder that exhausts every attempt leaves the sections unflagged, because a Lambda cannot tell which attempt it is on; the execution still fails and the failure alarms and dead-letter queue still count it, and extraction and assessment share this. And making these two long ladders reachable changes how a **sustained** regional throttle ends: instead of failing within seconds carrying a recorded diagnosis, a document can now hold a workflow-concurrency slot for hours — each long ladder spends up to 2.82 hours of backoff, with no `MaxDelaySeconds`, so the last wait alone is 1.70 hours — and then stop at the state machine's own `WorkflowExecutionTimeoutSeconds` (21,600 s by default) as `TIMED_OUT`, which records no per-section diagnosis at all. The ladders themselves are unchanged; what is new is that rule validation can now reach them. See [Rule Validation](docs/rule-validation.md#a-transient-fault-is-never-answered-with-a-verdict) ([#1101](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1101)).
- **`make typecheck` read build output that is not part of this repository, and failed on it.** `basedpyright` discovers files by walking the filesystem and has no ignore-file support at all — no such key in `pyrightconfig.json`, no such command-line option, and the string occurs nowhere in the package it ships — so `include` naming `feature-platform` reached the two copies of `lib/idp_common_pkg` that the idp-data-generator feature's packaging step stages beside its build contexts. Those copies are one revision behind, so anyone who had packaged that feature locally got 20 `reportReturnType` and `reportCallIssue` errors in files that cannot be fixed by editing a tracked file, while a clean checkout and both CIs stayed green: the gate's answer depended on what you had built. Both paths are now in `exclude`, one entry each, each with its own reason and a premise the tree computes — an ignore rule covers the path, so a file under it cannot become tracked without that rule being edited, and git tracks nothing under it today. Coverage is unchanged and now exactly closed: `basedpyright` analyses every `.py` file git tracks, and the gate asserts that identity directly rather than against a recorded count. The class-level half is what stops the next staged copy doing this again — `scripts/tests/test_pyright_config.py` fails if *any* ignored `.py` file is inside the walk, naming the directory, which is a local test failure rather than 20 errors on everybody's machine. `ruff` needed no equivalent because it honours the ignore file natively, and its gate already asserted the same property; the type gate was the half that had neither. Also fixed while registering the new carve-out: the exemption-discovery scan matched the name fragment `EXCLUD`, which does not match `EXCLUSION`, so `TYPECHECK_SCOPE_EXCLUSIONS` — the live carve-out taking notebooks out of the typecheck coverage closure — had never been discovered or registered while every meta-test passed.
- 🔒 **Some documented `pip install` commands could install a third party's package instead of this repository's.** The first-party packages require each other by bare name, and those names on public PyPI belong to unrelated parties, so a path install that omits a sibling — `pip install -e lib/idp_sdk`, or the `cd lib/idp_cli_pkg && pip install -e .` that the Quick Start and the deployment guide gave for the CLI — let pip satisfy the missing name from the index. Every documented invocation now installs the siblings it needs on one command line, or points at `make setup`. **Action:** if you followed one of those instructions, run `python scripts/check_first_party_deps.py`; if it fails, follow the remediation in [Installing First-Party Packages Safely](docs/dependency-confusion.md) — reinstall in one pass, and because the package ran in an environment holding AWS deployment credentials, inspect the artifact you actually received without installing it and rotate those credentials if you cannot rule out that it executed. A new gate, `scripts/tests/test_doc_install_commands.py`, now reads every `pip install` in the tree's Markdown and notebooks and fails on any that could resolve a first-party name from an index; the existing environment check could only ever confirm an install that had already happened.
- ⚠️ **A Z3 rule with an `Int` threshold could report Pass for a document that fails it.** A reading for a parameter declared `Int` was truncated toward zero before the solver saw it, so a rule reading `days_late <= 30` against an extracted value of 30.9 compared 30 against 30 and returned a confident Pass — no error, no warning, and a result whose recorded value looked self-consistent. Truncation is unsound in both directions and no rounding rule fixes it, because the correct verdict depends on the comparator: rounding the same reading up to 31 would report Pass for `days_late >= 31`. A reading that cannot be held by its declared type without losing information is now **refused**, which is what path-based extraction already did with the same input — the two extraction routes had disagreed, and the route that truncated is both the default for a rule with no path mappings and the fallback when path extraction fails. The check is shared by all three routes into the solver rather than written per route, and it holds for any spelling of a fractional reading, not just a Python `float`. A `Real` parameter still accepts a fractional reading, and now holds it **exactly** rather than rounding it to a double, so an equality rule decides on the reading instead of on its first 17 significant digits — the constraint's own decimal literals are parsed the same way, since making one side of a comparison exact and leaving the other rounded is wrong in the other direction. **Action:** a rule whose reading is genuinely fractional should declare the parameter `Real` rather than `Int`. `Int` means the quantity is whole, and the constraint language compares `Int` and `Real` against each other, so a `Real` parameter works with a whole-number threshold and costs nothing. This is a correctness choice rather than type hygiene, because the declared type also shapes what the extraction model answers: asked for an `Int` from a document whose underlying fact was 30.9, a model answered `31`, and no check downstream can see that the fact was fractional. Documents whose rules were passing on a truncated reading will now report **Fail** or **Information Not Found** with the reason, and both are the corrected verdict: re-run any compliance report you relied on that has an `Int` threshold. A decimal that happens to be whole, such as `30.0`, still binds as `30` ([Z3 rule validation](docs/rule-validation-z3.md), [#1057](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1057)).
- **506 tests in 22 registered suites now run on every pull request, and the check that was supposed to notice they did not has been widened to the whole tree** ([#980](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/980)). `make test` discovers every test directory in the repository, but it runs in neither CI; what a pull request runs is `make test-cicd -C lib/idp_common_pkg` plus a hand-enumerated `make test-packages-cicd`, and 22 directories were registered in the first and named in neither. Among them: the eight API-resolver suites, including the 25 tests covering the download allow-list, script-type download forcing and fail-closed refusals, and the 16 covering the upload resolver — added by a fix whose own description said those tests were what would have caught the defect — plus the benchmark harness that decides what a release report claims, the eight feature-platform extension suites, the Lambda hook-inference samples, and two custom-resource suites. The gate that detects this existed and was scoped to one subtree, `src/lambda/`, with a note saying the argument applied more widely; it now derives its universe from every directory holding a tracked `test_*.py` and its covered set by parsing both CI-invoked recipes, so a suite reachable from neither is a test failure rather than something found by hand months later. Four of the 22 passed locally and failed under the wrapper that strips the AWS environment, each because something on its import path builds a boto3 client at module scope: three needed a region and one needed credentials to sign a presigned POST, and each now supplies its own from its own `conftest.py` rather than having one pinned for it by the caller, which is what makes running it through the wrapper mean anything ([#988](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/988)). The reasons in the excluded registry were re-measured too: the one claiming a Lambda-runtime-only dependency had outlived its own remedy, and the reason that replaces it names the single stale assertion actually holding that root back and is computed three ways — the named test must still resolve to exactly one collected test, must still fail, and its file must still pass without it — so fixing, renaming or deleting it all invalidate the entry instead of leaving it standing. The excluded set is matched by exact directory, not by prefix: the entry excluding the repository's own `scripts/` directory is there for one mis-collected file, and as a prefix that one sentence would have exempted the whole gate layer beneath it. [Testing](docs/testing.md) records which targets a pull request runs, which it does not, and why the two are not the same set.
- **`make check-branch-protection` no longer treats a closed issue as the trigger to become a blocking gate.** Branch protection is off on **both** long-lived branches — `develop`, which pull requests target, and `main`, which is the default branch and the one releases are cut from — and enabling it needs repository admin that no contributor and no CI token has, so the decision to stop pursuing it from inside the repository is recorded rather than tracked as open work. Seventeen places across the checker, the `Makefile`, `CLAUDE.md` and the repo-quality-review skill described that issue as pending and named its *closure* as the moment to wire the check into `lint-cicd` with `--fail-on-skip` — which, followed literally, would red-line every branch forever for a condition nobody in the tree can fix. Each now states the real condition: a repository **setting** changing, by one of two routes that are not the same permission — repository admin enabling classic protection, or an organization or enterprise owner publishing a branch ruleset, which needs no repository admin and is a path the "needs admin" reasoning did not cover. The check stays opt-in, read-only and network-optional, exits 0 with an explanation when it has no token, and reports `exit 1` with one `not_protected` finding as its documented steady state; its `--json` output replaces a bare issue number with a decision record naming the state and the trigger, and names the branches a run did **not** read, because one invocation answers for one branch and that is why `main` went unexamined. Why the gap cannot be closed from here is now stated as the structural fact it is: enforcement is server-side, so a merge taken through GitHub's own Merge button runs no code from this tree and nothing in the repository can turn a red check into a refused merge. New guards fail if any lint target *reaches* the check — computed from `make -n`, so a prerequisite added to a target that `lint-cicd` runs indirectly fails too, which a text read of the three lint recipes could not see — if it is added to the CI-parity `SHARED_GATES` or named in either CI configuration, if the branch-summary read that tells "not protected" apart from "cannot see" is dropped, or if the wording goes back to describing the issue as pending. Also corrected: the finding no longer claims a direct push runs *no* GitHub workflow, since two path-filtered publisher workflows do trigger on push — no *gate* workflow does.
- **Two gates that ran on every pull request and could not fail now can.** The UI lint gate auto-fixed the tree it was judging: `make ui-lint`, which both `make lint` and `make lint-cicd` reach, ran `eslint --fix`, so in CI it repaired the ephemeral checkout and then reported it clean — and because `prettier/prettier` is configured as an error and is entirely auto-fixable, the UI formatting gate had no failure mode at all. It now runs `npm run lint` with `--max-warnings 0` and fixes nothing, so the five `warn`-level rules that were advisory forever (`no-unused-vars`, `no-explicit-any`, `no-shadow`, `react/no-array-index-key`, `react/jsx-filename-extension`) fail it too; `make ui-lint-fix` is the new auto-fixing entry point, and it re-runs the strict check afterwards so its exit status reports what is left rather than what it repaired. Separately, the type gate in both CIs was `make typecheck-pr`, which narrows `basedpyright` to the files a pull request changed and therefore cannot see a break the change caused in a file the diff did not touch — the ordinary shape of a type error, since a changed signature fails at its callers. Both CIs now run **`make typecheck`** over the whole tree (~47 s, 0 errors today), which is also the check that `pyrightconfig.json`'s proven all-tracked-files `include` closure is a closure *of*. `make typecheck-pr` remains as a fast local convenience and is documented as not a gate. Its three fail-open paths are closed: an exit code meaning "the checker did not run" is a failure rather than a pass, an unparseable report is a failure, and the number of files `basedpyright` reports analysing is reconciled against the number selected — a run that resolved to nothing printed `0 errors` indistinguishably from a clean run. It also no longer writes a temporary config into the repository root. It had no tests; it now has 24.

  ⚠️ **`--max-warnings 0` reaches the release build, not just the gate.** `src/ui`'s
  `build` script is `lint && typecheck && build:only`, so `make ui-build` and the UI
  build inside `python3 publish.py` are now strict too: a single
  `@typescript-eslint/no-explicit-any` in `src/ui/src` will fail a publish that
  previously succeeded. The tree is clean today, so nothing fails now — but if a
  publish stops on an ESLint warning, that is this change and `make ui-lint-fix`
  plus a real fix for the non-auto-fixable findings is the remedy.

  ⚠️ **`make lint` is now a mixture of fixing and checking.** Its Python half still
  auto-fixes (`ruff-lint`, `format`) while its UI half only reports. A clean
  `make lint` may have edited your Python and will not have edited your TypeScript.
  `make lint-cicd` is also not check-only and never was — it reaches `codegen-check`,
  which regenerates `src/ui/src/graphql/generated/` and, outside CI, leaves it
  regenerated; `CONTRIBUTING.md` now says so where it previously claimed otherwise.

- **The CI parity test can now see the two gates its own rationale was written about, and a gate that is in neither CI.** `scripts/tests/test_ci_gate_parity.py` exists because the SRT security scan and the dependency audit were GitLab-only, and neither appeared in the list of gates it compares — deleting GitHub's SRT step left all 20 of its tests passing. Both are now asserted, along with the whole-tree type check. The deeper gap was structural: a gate present in the `Makefile` and absent from **both** CIs has no foothold in either config, so comparing the configs finds nothing to report. The check-shaped `Makefile` targets are now derived from the file, and each must be either reached by both CIs or registered with a reason for that one target — so a new check cannot be added and left out of CI silently. The list of `make lint` prerequisites that `lint-cicd` must cover is now read from `lint`'s own rule line instead of being a hardcoded subset of 8 of its 12, and comments and `echo` lines are stripped before searching, because the recipe's own error messages mention gate names it does not invoke.

- **Coverage is now stated to be advisory, which is what it has always been.** GitHub's report declared `thresholds: "60 80"` beside `fail_below_min: false` and GitLab published a Cobertura report with no minimum, so the pair read as a 60% gate that did not exist. No threshold has been invented: coverage is measured for `lib/idp_common_pkg` alone, one of the ~64 test roots, so a repository-wide minimum computed from it would be a number about one package presented as a number about the repository. Both CI configs and `CONTRIBUTING.md` now say so, and widening the measurement is recorded as the prerequisite for enforcing it.
- **A section whose extraction failed now says so in the Sections panel, instead of showing nothing** ([#1049](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1049)). Every extraction failure raises, and both extraction Lambdas wrote the section to the tracking table only *after* the call returned — so on a failure the write never happened and the section's record still held whatever classification left there. Opening the document showed no status icon, no issue and nothing identifying which section had failed; the explanation existed only in the Step Functions cause and the logs. Each entry point now persists the section before re-raising, so the failure is recorded on it as an error-severity `extraction_failed` issue whose `root_cause` carries the exception's own explanation and remedy, and the Status column distinguishes a section that **Failed** from one merely flagged as **Incomplete**. This covers the whole family — an input too large for the model, an image Bedrock rejected, an invalid tool-use sequence, and a materially incomplete list under `extraction.row_shortfall_action: fail`. The last of those is what made it worth fixing: it deliberately writes the partial rows and an `extraction_rows_below_ocr_estimate` diagnosis before failing, and that diagnosis now reaches the record the UI reads as well as the section's `result.json`. A transient failure is deliberately **not** marked, since the state machine retries it and a marked section would clear itself hours later. The exception itself is unchanged — the Step Functions cause still names the rows extracted, the OCR row estimate and the remedy — and a tracking-table write that fails cannot replace it. A processing issue's free-size fields are now bounded (4 KB for the technical cause, 1 KB per value in its structured payload, both measured in bytes), removing the **middle** so the failure at the start and the remedy at the end both survive; a schema-validation failure on a long list could otherwise render the extracted rows into the record and push it past DynamoDB's item limit, at which point nothing was recorded at all — five such messages in one field are 70% of the item ceiling on their own. ⚠️ The constant naming that bound is `ProcessingIssue.MAX_ROOT_CAUSE_BYTES`, replacing `MAX_ROOT_CAUSE_CHARS`; code outside this repository that read the old name needs updating, and the unit changed with the name. Three adjacent defects are fixed alongside: the document list's **Processing Issues** badge was stamped back to 0 for any document whose execution failed, because the end-of-execution status write re-derived the count from a document object carrying no sections; the cleanup that releases a section's per-shard results from the working bucket addressed a prefix built from the section's ordinal id while the shards are keyed by class and page range, so it had never deleted anything; and with that corrected, the shard-merge step no longer releases them at all, because the merge requires every shard present and a transient fault while writing its response would otherwise leave a retry with nothing to merge, while the in-process step releases its checkpoint and shards only after its response has been built, so the same fault costs one section's re-inference rather than the section itself. See [where a failed section shows up](docs/extraction-and-confidence.md#where-a-failed-section-shows-up).

- ⚠️ **`client.search.query()`, `client.evaluation.get_report()` and `client.evaluation.get_metrics()` return a result instead of raising, and `client.document.list()` reports a real count.** Five `idp_sdk` operations built their result objects with keyword arguments the result models did not declare, so each raised `TypeError` on every input — reported to the caller as a plausible "Failed to …" processing error by the broad `except Exception` around it, including the `client.search.query(...)` example in the client's own docstring. `document.list()` was the silent case: the unknown keyword was dropped rather than rejected, so it returned `count` as `None` and lost each document's batch id on every call. The evaluation report and metrics also read an S3 key the pipeline does not write and now read `<document key>/evaluation/results.json`, the artifact it does — so `get_report()` needs the document to have been evaluated against a baseline, and raises `IDPResourceNotFoundError` if it has not. `DocumentListResult.count` and `EvaluationBaselineListResult.count` are page sizes, named as such, because neither a DynamoDB scan nor an S3 prefix listing reports a total; `DocumentInfo` carries `batch_id` and no longer carries the `num_pages`/`document_class` that were always `None`.

  ⚠️ **`EvaluationMetrics`' four `avg_*` scores are now `Optional[float]` and can be `None` where they were `0.0`** — so `f"{metrics.avg_accuracy:.1%}"` raises `TypeError` rather than printing `0.0%`, and client code that formats or compares them needs a guard. They are `None` when nothing in scope reported the metric, which includes a stack whose evaluations have not run yet, and **also whenever `document_class` is passed**: those figures come from each document's whole-document metrics, which cannot answer a question about one class of section, and previously returned a whole-document number that read as a class-scoped one. The class-scoped answer moved to `by_document_class[<class>]`, which now carries all four scores per class rather than `avg_accuracy` alone — and its `avg_accuracy` **changes value**, because sections the pipeline excluded from evaluation carry no scores and are no longer counted in the denominator. `SearchResult.confidence` is `None` rather than `0.0` when nothing matched, for the same reason: a zero reads as a measurement.

  `reportCallIssue` and `reportReturnType` are now **errors** in `pyrightconfig.json` rather than warnings, which is what makes this class fail `make typecheck` instead of exiting 0 over it, and `scripts/tests/test_pyright_config.py` pins those severities so they cannot be downgraded silently. See [the SDK reference](docs/idp-sdk.md).

- **`idp-cli stop-workflows` no longer crashes when a Step Functions execution cannot be stopped.** The handler named an exception that is not in the Step Functions service model (`ExecutionNotFound`; the real one is `ExecutionDoesNotExist`), and an `except` clause evaluates its expression at raise time — so any `stop_execution` failure raised `AttributeError` from inside the handler and propagated past the `except Exception` meant to count it. The failure is now counted and reported, and an execution that is already gone counts as stopped.

- **One stalled Bedrock request no longer costs a whole document's extraction, and the shards that already succeeded are no longer thrown away with it** ([#1014](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1014)). A shard invocation is capped at 900 seconds — Lambda's maximum, so the budget cannot be widened — and four separate numbers spent it without anything checking that they added up. The agentic client allowed a single request to stall for **600 seconds**; botocore multiplied that by its own attempt count, which was **eight**, inside one call where no deadline check could see it; a second, larger read timeout for the non-streamed confidence call ran in the same invocation whenever confidence is in `separate` mode, multiplied by five for the same reason; and the retry ladder above them was allowed 300 seconds of backoff. A shard that met a transient `Read timed out` therefore ran into the wall clock instead of retrying, Step Functions read the resulting `Sandbox.Timedout` as **deterministic** (one attempt, deliberately, since [#917](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/917)), and so the one failure a retry would have cleared became the one failure not retried. `ExtractionShardMap` tolerates no shard failures, so Step Functions then failed the whole Map and **discarded the output of the shards that had completed** — leaving the document with zero rows extracted, a worse outcome for that document than not sharding at all. Observed deterministically on a 400-row statement under advanced extraction with integrated confidence, where the same document under separate confidence completed in 123–137 s. Every Bedrock client now makes exactly **one** attempt per call, the agentic read timeout is **180 s**, the backoff allowance is **90 s**, and the terms live together in `idp_common.timeout_budget` with `tests/unit/extraction/test_shard_timeout_budget.py` asserting the whole inequality against the clients' resolved configuration — the relationship was previously stated only in a comment, which is why each number could look individually reasonable. `ExtractionShardMap` also gains a single Map-level retrier, so the per-shard S3 persistence that exists precisely so "only incomplete shards re-infer" can finally be used, and a `Catch` so a failed Map reports `ExtractionShardMapFailed` naming the section and carrying the Map's error output instead of a bare `States.ExceedToleratedFailureThreshold`. A failure tolerance above zero was deliberately **not** adopted, because completing a document with shards missing is silent data loss and worse than the failure it would replace. ⚠️ An execution that fails this way now reports `ExtractionShardMapFailed` rather than `States.ExceedToleratedFailureThreshold`, so any alerting keyed on the old name needs updating. ⚠️ The agentic path streams, so the 180-second read timeout bounds a socket going **silent** between response events, not how long a generation may take — a healthy long generation never approaches it — but a Bedrock client that now retries a transient fault once instead of five times relies on the application-level ladder above it, which is bounded by the invocation deadline rather than by an attempt count.

- **A pipeline hook registered where the active processing mode cannot reach it is no longer accepted in silence** ([#982](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/982)). `postOcr`, `postClassification` and `postExtraction` are states on the Pipeline branch only — BDA does those three steps inside one Bedrock Data Automation invocation — so under `use_bda: true` a hook registered at one of them never ran, its `onError: fail` policy never gated, and nothing anywhere said so: the config record showed the hook, the UI showed the hook, and the execution history was empty because the dispatcher was never invoked there. A PII-redaction gate registered that way let the document complete un-redacted with no signal. Both feature-install paths (`registerFeatureHooks` and a hook shipped inside a config preset) and the configuration write paths now **refuse** an `onError: fail` hook at a point the target configuration's mode cannot reach, and accept an advisory or disabled one with a warning; because `use_bda` can change after a hook is registered, the `preprocessing` dispatch — the one invocation ahead of the routing decision — also audits the whole hook set on every document and records what the chosen branch will skip under `unreachableHooks` in the Step Functions execution history. ⚠️ **A feature stack that registers an `onError: fail` hook at one of those three points now fails to install while `use_bda: true`, and a configuration save that writes one is rejected** — the hook was never running, so nothing that worked stops working, but the remedy has to be applied before re-installing: disable the hook, move it to `preprocessing` (the one point both modes reach), downgrade its `onError`, or run the pipeline mode. Saves that do not touch the hook or the mode, resetting a profile to default, restoring a profile revision, and reading an already-stored configuration are all unaffected. Mapping the three points onto BDA's own output boundaries remains open. See [Pipeline hooks](docs/feature-platform.md#not-every-hook-point-exists-in-every-processing-mode).

- **The feature platform can now be switched on for a stack that was created without it.** Setting `EnableFeaturePlatform` from `false` to `true` used to fail every time with `Export with name <StackName>-TrackingTableName is already exported by stack <StackName>`, because the main template and the nested platform stack each declared that export name under opposite conditions and a single CloudFormation update cannot hand an export name from one stack to the other. The nested platform stack is now its only producer; the main template keeps a plain stack output, which also makes the tracking table name readable from `describe-stacks` whether or not the platform is enabled. A new structural gate fails if any export name ever has two producing stacks again, or if any host export changes producing stack. Stacks that already have the platform on, with or without extensions installed, upgrade with no extra step. ⚠️ Adopting the platform on a stack older than this release takes **two** updates: upgrade first, leaving the parameter `false`, then set it to `true` — combining them still collides on the older template's export. ⚠️ On a **platform-off** stack the upgrade withdraws the main stack's `<StackName>-TrackingTableName` export; if anything in the account imports it — most plausibly a hand-wired reference in a `--headless` deployment — the upgrade rolls back, so check `aws cloudformation list-imports --export-name <StackName>-TrackingTableName` first and read the stack's `TrackingTableName` output instead. ([#845](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/845))

- ⚠️ **Chat-with-Document config-version scope: enforced on the REST path, and now honestly reported as unenforced on the streaming one.** The lookup queried a UsersTable index named `SubIndex` that no template declares, so every query failed and the result was read as "unrestricted" — `allowedConfigVersions` had never constrained a chat turn. It now resolves the caller's row through the immutable Cognito `sub`, via a `SUB#<sub>` pointer item on the existing UsersTable, and falls back to the `EmailIndex` join on the verified `email` claim; any failure to evaluate the scope denies the turn instead of allowing it. No table change is required. **On commercial deployments this does not yet restrict chat**, because the browser streams chat from a Lambda Function URL and that transport forwards no verified per-user caller; the processor logs, once per turn, that it did not enforce the scope, rather than appearing to have checked. Enforcement applies to turns reaching the backend through the REST API, which is the path GovCloud deployments take — a user restricted to specific configuration versions is denied documents outside them there, including documents carrying no version stamp. A chat turn attempted *during* a stack update may be refused with a "could not verify your access" message while the two functions update; retry once the update completes. Closing the streaming half needs the endpoint to verify a Cognito ID token ([docs/rbac.md](docs/rbac.md#known-limitations), [#970](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/970), [#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920)).

- ⚠️ **A scoped user whose email address changes keeps their configuration-profile restriction.** `allowedConfigVersions` lives on a user record, a user with no record is unrestricted by design (scoping is opt-in), and the record could previously only be found by email — so an address that stopped matching its record silently lifted that user's restriction, which an external identity provider re-applying its attribute mapping, a self-service address change, or a difference in letter case could each cause. The record is now reached first through a `SUB#<sub>` pointer item keyed on the immutable Cognito `sub`, and only then by the email join, and a caller presenting neither identifier is denied rather than resolved to an identifier matching no record. **No table change and no migration is required**, and nothing is affected while a deployment upgrades: the pointer reuses the table's existing key schema, and a record that does not yet carry a `sub` resolves by email exactly as before. A pointer exists only for a record that carries a scope, so resolving through it can only ever *tighten* what a caller sees. The `sub` is recorded when a user is created and back-filled by the Cognito sync that runs whenever an Admin opens **User Management**, so after upgrading a deployment that uses an external identity provider, open that page once as an Admin; a record whose address had already changed beyond letter case is not reachable by that sync and keeps the email-only join. See [Resolving whose scope](docs/rbac.md#resolving-whose-scope--two-keys-and-the-lookup-fails-closed) and [External Identity Provider](docs/external-idp.md) ([#1038](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1038)).

- **The Redaction Report no longer reveals which documents have a redaction record to a caller who may not read them, and a user scoped to a pattern can use the web UI again** ([#1038](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1038)). The report's two single-document routes resolved the record before the caller's access scope, so an absent document answered "not found" and a present one "access denied" — a difference worth one bit of the audit table's contents per request, to a caller entitled to neither. Access is resolved first now, and a document outside the caller's scope answers identically to one that does not exist. Separately, scope entries may be patterns such as `tenant-a_*` and the backend has always honoured them, but the UI filtered profiles by exact name, so a pattern-scoped user saw an empty Configuration Profile list and a permanently disabled Reprocess button; the client now matches the way the server does, and a malformed pattern matches nothing rather than breaking the page. ⚠️ Patterns still have to be **set** through the API or the CLI — the Admin scope picker offers only the profile names that already exist.

- **Threat model:** AUTH.T07 (config-version scope bypass) is recorded as *Partially Mitigated* rather than *Mitigated*, and the four residuals that keep it there are named on the entry: the `sub` join closes the email-divergence route only for a user record that records a `sub`, and the back-fill cannot reach one whose address had already changed beyond letter case; which *kind* of Cognito token can reach these operations is a property of the API Gateway authorizer rather than of this code (measured on a live deployment, an access token is refused 401 at the gateway before any Lambda runs); the class gate that holds the lookup together covers key provenance and failure handling only, with no rule about the matcher; and `idp_common/testset_scope.py`, which resolves the independent `allowedTestSets` axis, is discovered by that gate and carried as a suppression rather than fixed. PII.T04's record-existence residual is closed. Threat counts reconciled across the corpus against the generated export, and `make check-threat-model-currency` now fails when a document's stated count disagrees with it.

- **A circuit breaker that cannot read its own state no longer switches itself off** (one item of [#934](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/934)). The Queue Processor treated any DynamoDB error on the state read as "allow", so a ConcurrencyTable fault disabled the breaker rather than the traffic — the one failure direction a breaker must not have, and the opposite of the fail-closed rule applied elsewhere in the codebase. A failed read is now judged by whether a retry can fix it: a transient fault (throttling, read timeout, dropped connection, 5xx) **refuses admission**, since the message returns to `DocumentQueue` and its 500-delivery redrive budget absorbs about eight hours of retries before anything reaches the dead-letter queue. A terminal fault (access denied, table missing) still admits work, because it cannot be retried away and blocking every message on every poll is the wrong trade for a control that bounds spend rather than correctness; it is reported instead through a new `CircuitBreakerCheckFailed` metric whose `Classification` dimension distinguishes the two, so the state in which the breaker is protecting nothing can be alarmed on. "Admits work" is narrower than it sounds: for a table-wide fault the concurrency-counter write a few lines later fails for the same reason, so the document does not get through either way — what the terminal branch protects is the case where only the breaker record is unreadable and the pipeline is healthy. `DocumentQueueStalledAlarm`'s description gains the one cause its runbook could not distinguish: a sustained state-read failure holds messages without deleting them and emits no Bedrock error metrics, so a clear `BedrockServiceOutageAlarm` no longer implies a wedge. Reachable only with `CircuitBreakerEnabled=true` — on the default the state is never read. See [Circuit breaker](docs/circuit-breaker.md#when-the-state-cannot-be-read).

- **A section left with no confidence scores is now reported, however it got there** ([#1006](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1006)). Several early returns in the Assessment step left a section unscored and completely silent: they appended to `document.errors`, which is read only on a failed document, and did not raise, so no processing issue reached the section, no `AssessmentConfidenceUnavailable` metric was published and the alarm added in [#996](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/996) could not see them — the document completed looking fully scored. A section with nothing to assess (no extraction result, no pages, or an empty `inference_result` from a class that *does* have a schema) now records an error-severity `assessment_skipped_confidence_unavailable` issue and publishes the same metric, since the alarm's question is whether sections are coming back without confidence; the issue code says which cause. Sections no extraction was attempted on still report nothing — an excluded class, a class with no attributes to extract (which is what a page classified `unclassified` gets), or confidence assessment switched off — so the alarm is not tripped by ordinary throughput. **Library callers:** `AssessmentService.process_document_section` now raises `ValueError` for a falsy document, a document with no sections, or an unknown `section_id`, where it previously returned the document unchanged; no deployed path reaches those, but code calling `idp_common` directly (including the assessment notebooks) will see the error instead of a silently unassessed document. See [Monitoring](docs/monitoring.md#confidence-assessment-degraded).

- **Three more ways a section completed with no extracted data or no confidence scores and said nothing, one of them with the scores fabricated** (found reviewing [#1006](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1006)). All three wrote their only record to `document.errors`, which is read solely on a *failed* document, so the document completed looking healthy. **(1)** A section whose pages were all absent from the document had its confidence assessed against empty text and no images, and came back **with** a score for every field — worse than no confidence, because a fabricated number is indistinguishable in the UI, in HITL routing and in the reporting lake from one the model read off the page, and it suppresses the unscored-field signal HITL would otherwise raise. The confidence pass no longer runs for such a section; it is reported as unscored instead, and one missing only *some* of its pages records a warning-severity `assessment_pages_missing`. **(2)** A section whose named class the configuration no longer contains — renamed or deleted while documents were in flight, or an old document reprocessed under a newer configuration — completed with no fields and no report, because that case shared a code path with two situations that are not faults. It is now reported as `extraction_class_not_configured`, with a remedy naming the configuration rather than the confidence model; a class deliberately given no attributes, and a section classification could not classify, stay silent, so the alarm is still not tripped by blank pages. **(3)** Classification recorded no processing issue of any kind: a page whose classification failed, whose OCR artifacts were unusable, or whose class was coerced to `invalidClassFallback` wrote its reason into page metadata that never survived serialization, and the section holding it completed green with no fields. Those now surface on the section as `classification_failed`, `classification_page_no_content` and `classification_invalid_class_fallback`, on the `multimodalPageLevelClassification` path; `textbasedHolisticClassification` decides per segment and records none of them. ⚠️ **Upgrade-visible:** a document containing pages the classifier cannot place will now show the document list's **Processing Issues** badge, which is severity-blind. Those pages produced no extracted data, so the badge is accurate; defining a catch-all class the model can legitimately choose removes the condition. ⚠️ The out-of-vocabulary report's volume is bounded by configuration edits only on the default `multimodalPageLevelClassification` with `enforceValidClasses` on — the holistic method has no vocabulary-enforcement loop and `enforceValidClasses: false` disables it, so on those two an out-of-vocabulary prediction is stored verbatim and the rate follows model output; tune `ConfidenceUnavailableThreshold`, or define the classes the model keeps choosing. No new metric or alarm, and no template change. See [Monitoring](docs/monitoring.md#confidence-assessment-degraded) and [Classification](docs/classification.md#pages-classification-could-not-classify) ([#1036](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1036)).

- **`EnableXRayTracing` now controls X-Ray tracing on the Lambda functions, not only on the two state machines** ([#983](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/983)). Nineteen functions across the main and unified-pattern templates hardcoded `Tracing: Active`, so a parameter whose description reads "Enable X-Ray tracing" could not turn Lambda tracing off and setting it to `false` left the X-Ray bill unchanged — invisible until someone tried. Every traced function now resolves its mode through the parameter. Three pipeline functions that annotated X-Ray segments while declaring no mode at all — the two rule-validation functions and the rule-validation orchestrator — now declare one, so their `document_id` and `processing_stage` annotations land in a segment that exists rather than only when an upstream caller happened to sample; and the pipeline-hooks dispatcher, which declared `Tracing: Active` while its role held no `xray:PutTraceSegments` grant, can now write the segments it was already trying to write. ⚠️ **Upgrade-visible.** The default is unchanged at `true`, so a default deployment traces what it traced before, **plus** those four functions: X-Ray charges per trace recorded and per trace retrieved, so expect a small increase. `EnableXRayTracing=false` now reduces X-Ray cost across the main and pattern stacks where before it reduced almost nothing — but **not to zero if you have an extension installed from the Extensions catalog**, because those six stacks are deployed independently and set `Tracing: Active` in their own `Globals` section, which no parameter of the main stack reaches. The nested feature-platform stack is a different case and does now honour the parameter — see the entry below. Setting it to `false` also does not remove the functions' X-Ray write permission, because SAM attaches that whenever a tracing mode is declared. The four unified functions that carry no X-Ray instrumentation are deliberately left untraced. `scripts/tests/test_xray_tracing.py` fails any function that hardcodes a mode, inverts the condition, instruments X-Ray without declaring one, or declares one with a role that cannot write a segment. See [Configuration](docs/configuration.md#integration-and-tracing-parameters).

- **`EnableXRayTracing` also reaches the nested feature-platform stack** ([#1037](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1037)). That stack hardcoded `Tracing: Active` in its `Globals` block and never received the parameter, so its nine Lambdas — which are part of the main deployment whenever `EnableFeaturePlatform` is on, its default — kept tracing with the parameter set to `false`; measured on a live stack, all nine reported `Active` with a `LastModified` predating the update that turned tracing off. Those nine also share one explicit IAM role that SAM attaches its X-Ray policy only to a role it generates, so tracing was declared and **no segment was ever written**; the role now carries the grant. ⚠️ **Upgrade-visible.** On the default `EnableXRayTracing=true` those nine functions go from "Active but unable to write" to emitting segments, so an existing deployment gains a small X-Ray charge it did not have before; `false` now turns them off, which it did not. Extensions installed from the Extensions catalog are separate stacks an operator launches and still trace unconditionally — [Monitoring](docs/monitoring.md#x-ray-tracing) says which of their functions that actually costs money for, and what the options are. Two unused module-scope AWS clients are also gone from the BDA results handler, and the pipeline-hooks dispatcher and the test-execution aggregator build theirs on first use, so importing any of the three no longer needs an ambient region — which the suites loading them were masking.

- **The `Annotator` role can now be assigned from an external IdP group claim, via the new `ExternalIdPAnnotatorGroupName` parameter** ([#968](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/968)). The deployment creates five Cognito groups but the external-IdP group mapping understood only four, so `Annotator` was the one role a federated user could not hold — a locally created user could be placed in it, a federated user could not. Group-membership authorization is fail-closed, so the effect was a functionality gap rather than an access-control weakness: the user arrived with no group at all, was denied by the API, and the UI fell through to viewer-shaped navigation instead of their annotation queue. The parameter is optional and defaults to empty, so **an existing stack needs no action** and manually assigned annotators keep the role. ⚠️ If you do set it the IdP becomes authoritative for `Annotator` in both directions, like the other four roles — a federated user whose claim maps to another role but omits the annotator group is removed from it at their next fresh sign-in, so do not set it and also assign the role by hand. The `allowedTestSets` scope an annotator needs is still assigned only in User Management, so a claim-driven annotator holds the role with an empty scope — denied everything — until an Admin assigns them a test set. Unmapped claim values are also now named individually in the trigger's log, which previously went unreported whenever another group in the same claim mapped successfully. See [External Identity Provider](docs/external-idp.md#annotators-and-the-allowedtestsets-scope), which also documents two behaviours that apply to all five roles: removal only happens on a sign-in that still maps to at least one group, so clearing a user's IdP groups does **not** revoke their access; and a change in the IdP takes effect at the next fresh sign-in, never at a token refresh.

- **A failed document read no longer renders a blank page.** Opening a document the API refused, or that failed for any other reason, left the detail view empty with nothing logged; a knowledge-base query that failed left its answer showing `...` indefinitely. Both now report what happened ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)).

- **Permission errors no longer tell you to try again.** The document list showed the server's raw message and the file viewers advised retrying, which can never resolve an authorization failure. Both now say what is wrong and what to do, and a server-side S3 or KMS denial is reported as a server error rather than as a missing role ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)).

- **The Schema Builder writes one shape for an attribute that references a shared class** ([#957](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/957)). The Add Attribute dialog produced a bare `{"$ref": "#/$defs/Address"}` while the inspector's "Reference Existing Class" picker and the importer's inline-object extraction kept a sibling `type: "object"` beside it, so the same user-visible action left the designer holding two different nodes — which is why the empty Evaluation Method dropdown of [#906](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/906) looked intermittent, since the inspector reads a sibling `type` in preference to following the reference. All three routes now write the bare form that the exporter, the standard-class catalog and every shipped configuration already used, so saved schemas are unchanged. The picker also leaves less behind: it cleared an attribute's inline `properties` and `required` from a copy rather than from the attribute, so both stayed underneath the new reference as orphan keys.

- ⚠️ **A published test set version holds the labels it was published with, so what a version refers to can no longer change underneath a run scored against it** ([#1035](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1035)). Publishing wrote a version row and copied nothing: the labels a version stood for were copied later, whenever annotation next opened a draft, from whatever the set held at that moment. Publishing v1, regenerating draft labels and then annotating therefore left v1 standing for labels that had never been v1's, and a run pinned to v1 scored against them with nothing warning and nothing in the version row changing. Publishing now copies the set's baselines as part of recording the version. ⚠️ **Versions published before 0.6.10 may already hold the wrong labels and cannot be repaired**, because the state they were published with was never copied anywhere; the run form marks a version with no stored labels, and a run pinned to one scores the set's current labels instead. ⚠️ Publishing a large set now takes a moment, and a set with more than 3000 baseline objects is **refused**, with that as the stated reason, rather than recorded as a version no stored bytes can back — 3000 is what fits the 20-second budget the API's request dispatcher allows one call. Retrying a publish that reported an error is safe: the copy can outlast that budget, so an error does not prove nothing happened, and a retry either returns the version the first attempt created or reports that it is still running. Separately, the Schema Builder keeps a `$defs` definition and its references intact across a save. A definition authored by hand or by CLI as something other than an object — `{"type": "string", "enum": [...]}`, which the backend consumes correctly — was imported as an object and written back as an empty one, losing its type, enum and constraints the first time someone opened that class and saved; and a reference could be published without its target, leaving a pointer that resolved to nothing whenever the shared class was reached only from a definition body or from an array's `contains` constraint. No shipped configuration changes shape. See [Publishing a version](docs/test-studio.md#publishing-a-version).

- **Headless / GovCloud template transforms now report the IAM policy statements they delete** — both transforms drop IAM and S3 bucket policy statements as well as whole resources, and previously did so with no statement-level record of what was removed. Because a policy with fewer statements is still a valid policy, CloudFormation deploys the result happily and an over-deletion only surfaces later as a runtime access-denied, in the partition the template was deployed to. Each removal is now logged at `INFO` naming the resource (`<RoleLogicalId>.<PolicyName>` for a role's inline policy), the count and what matched, with a one-line summary per transform and an explicit zero when nothing was dropped; SDK callers can read the same records from the transformer's `policy_statement_removals`, which is the reliable channel since `GovCloudTemplateTransformer` configures no logging of its own. The three removals against the shipped template are now enumerated, along with the `CognitoAuthorizedRole` S3 policy the transforms leave untouched. See [GovCloud architecture](docs/govcloud-architecture.md#auditing-the-iam-statements-a-transform-removed). ([#981](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/981))

- **Importing `idp_common` no longer requires an ambient AWS region** ([#988](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/988)). `idp_common.utils.settings_helper` built an SSM client while being imported, and because `idp_common.utils` is imported by `idp_common.s3` and thence by most of this repository's Lambdas, importing the shared library at all needed a resolvable region — so any consumer in an environment without one failed at import, before running a line of its own code. The client is now created on first use, behind a lock, and memoized for the life of the process, matching the sibling `idp_common.monitoring.settings_cache`. Nothing in production was affected: the Lambda runtime always sets `AWS_REGION`. The offline test suites no longer depend on the machine they run on either — they previously passed on a developer machine, which reads a region and credentials from the shared AWS config file, and failed on a CI runner, which has neither.

- **`idp-cli`'s configuration commands honour `--region`** ([#1039](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1039)). Ten of them — `config-upload`, `config-download`, `config-activate`, `config-list`, `config-revisions`, `config-delete`, `config-sync-bda` and `bootstrap` among them — resolved the configuration table's name in the region asked for and then read or wrote it in whatever region the ambient credentials resolved to, because the client underneath was built with no region and had no parameter to accept one. Against a nonexistent table that is a `ResourceNotFoundException`; on an account holding same-named tables in more than one region it was a successful write to **another stack's** configuration, reported to the operator as success. `--region` now takes precedence, and when it is absent boto3's own chain resolves normally rather than a region being guessed, so a command with no region resolvable anywhere fails saying so. Behaviour inside Lambda is unchanged, where the runtime always supplies one. Separately, the benchmark matrix no longer launches runs after a configuration upload has failed: the upload's result was assigned and never read, and because that harness reuses deterministic configuration-version names, a failed upload usually left a previous grid's configuration in place — so the cells ran, succeeded, and produced a complete set of plausible numbers attributed to a configuration that never landed. Affected cells are skipped, the failure is recorded in the run artifacts and carried into the committed `summary.json`, and the run exits non-zero once the arms that did land have drained. See [Region and its precedence](docs/idp-cli.md#region-and-its-precedence).

- **The Python lint and type-check gates read the whole tracked tree.** `ruff.toml` excluded five bare directory names, which ruff matches at *any* path depth, so `ruff check` never opened 442 of the repository's 1230 tracked Python files — including 67 API-resolver Lambdas and 9 Bedrock Knowledge Base handlers under paths that name neither them nor their parent — and `basedpyright` examined 432, skipping 798. A clean gate therefore said nothing about most of the code that ships. Type checking now covers all 1230 files, which surfaced three genuine errors, one of them a defensive branch in the BDA results handler that could only ever have raised `TypeError` had anything reached it. Exclusions are now per file and generated, split so a file's formatting debt no longer costs its linting too, and ratcheted in both directions; and `make typecheck` no longer exits non-zero purely because a checkout has no `.venv`, which it did whether or not the tree had a single type error. 196 findings across 85 files, and a 186-file formatting sweep, are recorded as debt rather than fixed here ([#975](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/975), [#1042](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1042)).

- ⚠️ **`minItems` on a list field is a hard floor in Advanced extraction and advisory in Simple, and the guidance now says which** ([#1048](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1048), [#1047](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1047)). Two extraction messages and both documentation tiers recommended `minItems` as the way to make a list shortfall "a hard constraint" without saying what it costs, which is not the same in the two modes. In **Advanced** mode the constraint reaches the agent's tool boundary, so a list under the floor is rejected where it is produced: the agent spends its correction rounds on whole-section retries and, if it still cannot reach the floor, the extraction fails with *Failed to generate valid structured output* — **keeping no rows and no diagnosis of what was short**. So a floor that a document cannot legitimately reach (`minItems: 1` on a list that is sometimes genuinely empty) now reads as what it is: a decision to fail those documents. ⚠️ And once the section **shards** — which the shipped `extraction.agentic.max_concurrent_batches: 10` means any Advanced-mode section over five pages does, so this is the default case — the floor is enforced **per shard** (each shard agent gets the whole section's model as its tool while seeing only its page range), so a section-sized floor is unsatisfiable by any shard: `minItems: 100` over a 17-page section rejects every shard holding fewer than 100 rows, and a cover-page shard holds none, failing a document that genuinely contains 800 rows. The only floor every shard can satisfy is none, so on a sharding configuration use `row_shortfall_action` instead. In **Simple** mode the same shortfall is advisory — `extraction_list_truncated` is a warning, and the strongest thing `extraction.validation.fail_action: reject` does is record `parsing_succeeded: false`, which the Processing Report and the UI's report tab read and the status path does not; `reject` does not fail a section and does not route it to human review. `extraction.row_shortfall_action: fail` remains the setting to prefer when what you want is an enforced outcome, because it persists the partial rows, the error-severity issue and the Processing Report *before* failing the section. Also documented next to that setting: under `fail`, a list returning a few percent of its rows fails while one returning **none** of them completes, because an all-empty list is reported by `extraction_incomplete` instead — deliberate, since a genuinely empty list is common and indistinguishable from total loss, and closing it needs its own measurement, tracked in [#1046](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1046) with the OCR-evidence narrowing that keeps `fail` opt-in. See [`minItems` in Simple vs Advanced mode](docs/extraction-and-confidence.md#minitems-on-a-list-field--a-warning-in-simple-mode-a-hard-floor-in-advanced).

- ⚠️ **Three fine-tuned benchmark configurations named a Bedrock model that only existed in one account, and now ship the base model instead** ([#1093](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1093)). `config_library/unified/docsplit/docsplit_finedtuned_config.yaml` and the two `ocr-benchmark` fine-tuned files pinned `classification.model` and, in the ocr-benchmark pair, `extraction.model` to a `custom-model-deployment` ARN. That kind of ARN is account-scoped, so from any other account Bedrock answers `AccessDeniedException` — *the provided resource ARN is from a different account* — and the stage fails outright rather than degrading. All five values are now `us.amazon.nova-2-lite-v1:0`, the base model the fine-tune was derived from: the docsplit file's base-model sibling differed from it in exactly that one line, so the substitution is the value the repository already recorded. **Action:** nothing, unless you installed one of these three files. They are **not** selectable presets — the `docsplit` and `ocr-benchmark` values of **Configuration Preset** resolve to each directory's `config.yaml`, and the web UI's *Import from library* serves only `config.yaml`, `config.json` and `README.md` — so the only way to be running one is to have named it explicitly via `idp-cli config-upload -f`, `--custom-config`, or a **Custom Configuration Path** URI. ⚠️ If you did, and you own those deployments, the stage was working and will now run on the **base model** — so **your classification and extraction quality changes, silently, on upgrade**, because the fine-tune was the whole point of the file. Nothing errors and nothing in the UI flags it; the numbers simply move. Re-point `model` at your own deployment ARN before the next run, which each file now marks with a comment showing the shape. Note also that with the base model in place both arms of each A/B benchmark are the same configuration, so running the pair unmodified measures a delta of zero — that is the missing deployment, not a finding about fine-tuning. If you did not own the deployments, the stage was failing outright and will now succeed. Two quieter consequences are resolved by the same change, because the ARNs were in neither registry: a document processed under one of these files was priced as an unknown model (a NULL cost, reported as incomplete spend rather than a wrong figure) and limit-checked as one, which left `maxTokens` unset so Bedrock's small truncating default applied. The base model is present in `pricing.yaml` and matched by `model_config_limits.yaml`.

  Redacting the account id was considered and rejected: the ARN still would not resolve and the error would no longer say whose account it named. A load-time refusal was considered too — `idp-cli config-validate` does refuse an end-of-life model id — but that machinery is opt-in (no CI job, no `make` target, no custom resource invokes it), is deliberately kept out of the configuration-load path so a configuration that validated yesterday still loads after an upgrade, and already saw these ARNs and downgraded them to a warning as *unverifiable, not wrong*. All three files validated clean. What replaces it is an actual gate: `config_library/test_config_library.py` now fails on any `model` or `model_id` value in the shipped configuration library that names an account-scoped Bedrock ARN, over **every** YAML file rather than only the `config.yaml` the structural checks reach. It keys on the ARN's account segment being populated, so a foundation-model ARN — whose account segment is empty — still passes. It runs in `make test-config-library` and `make test-packages-cicd`, so both CIs. The three files are also documented for the first time in their directories' READMEs, including how to point one at your own fine-tuned deployment.
- **Agent chat: best-effort memory and monitoring can no longer abort a chat turn** ([#1099](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/1099)). Three hook callbacks on the agent chat path did risky work outside the `try` that was meant to contain them, so a failure in something whose whole purpose is to be discardable — a turn of conversation history, a throttling record, a debug log line — propagated into the agent loop and ended the user's turn. In `DynamoDBMemoryHookProvider.on_message_added` the message was read two lines above the guard, by subscript for the role and via `json.dumps` for the size check, so a message with no `role` raised `KeyError`, an empty message list `IndexError`, and content json could not encode `TypeError`; the whole body is now inside the guard, and a message too malformed to store is logged and dropped rather than stored under a substituted role, which would spend a write on a record that turn grouping discards when the history is read back. `DynamoDBMessageTracker._handle_throttling_with_agent_message` is installed over `ThrottlingMonitor._handle_model_exception`, which guards its own DynamoDB write, and did not guard its own — so attaching the DynamoDB logger removed a guard rather than adding a feature. What that exposed is failure to *submit* the write rather than the write itself, which is already absorbed on the worker thread. And `AgentMonitor.on_before_tool_invocation` serialised arbitrary tool input with `json.dumps` in a callback with no guard at all; because the call sits in an f-string argument it ran at **any** log level, not only when the debug line it was built for would be emitted, which made it the widest-exposure of the three. None of the three has a demonstrated trigger in the shipped system — Strands supplies `role` on every message it constructs, and tool input arrives already decoded from json — so this is hardening of a contract the rest of the module keeps rather than a fix for a failure in the field.

## [0.6.9]

### Added

- **IDP Monitor (Beta) is available from the Extensions catalog.** A real-time operations dashboard for document volume, latency, cost breakdown and trends, failures, throttles and AI insights, free of charge during its beta and published for **us-east-1**, **us-west-2** and **eu-central-1**. Subscribing accepts the beta licence terms shown on the [AWS Marketplace listing](https://aws.amazon.com/marketplace/pp/prodview-guhlipxo6hpl2). See [IDP Monitor](docs/extensions/idp-monitor.md).

- **Test Studio → Compare Test Runs shows a Comparator Changes panel.** It lists only the attributes whose applied comparator, threshold or provenance ("configured" vs "auto-inferred") differs between two runs, and is silent when nothing differs. Auto-inferred rows carry a tooltip explaining why that comparator was picked, which is what you need when an annotation is removed from a config and the numbers stay the same. See [Comparator inference rules](docs/evaluation.md#comparator-inference-rules).

- **The threat model is published and linked: [docs/threat-model.md](docs/threat-model.md).** The STRIDE corpus under `security/threat-modeling/` was reachable only by browsing the repository tree and had fallen six releases behind, so it still described the retired AppSync API layer. It is refreshed against the current architecture (98 threats) — the REST API and its dispatcher, the SigV4 streaming chat endpoint, the five-group Cognito model, the CMK and TLS-enforcing resource policies, and the feature-platform extension points as a trust boundary you own — and a build gate fails when it falls more than one release behind. ([#932](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/932))

- **The benchmarking guidance is refreshed and gains a study on advanced extraction.** [Configuration guidance](docs/benchmarking/config-guidance.md) is re-measured over the full configuration grid against real corpora and now carries a model axis, so it answers which model as well as which settings. The one-off topic studies are collected under [studies](docs/benchmarking/studies/README.md) with an index, among them the new [advanced extraction refinements](docs/benchmarking/studies/advanced-extraction-refinements.md) study, which measured a 55% cut in extraction output tokens with completeness unchanged over an identical 12-run arm.

- **Project governance and security-reporting files now exist: [SECURITY.md](SECURITY.md), [GOVERNANCE.md](GOVERNANCE.md) and [ROADMAP.md](ROADMAP.md).** `SECURITY.md` states where to send an exploitable finding — privately to AWS Security via [HackerOne](https://hackerone.com/aws_vdp), never a public issue — which versions are supported (the latest release only; no backports), and where the threat model and the curated per-release security test results live. ([#936](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/936))

### Changed

- **`stickler-eval` upgraded 0.5.0 → 1.0.0: evaluation comparators are now inferred per field, by name as well as type.** An auto-generated schema (a document class with no explicit evaluation config) used to have every string scored `FUZZY@0.85` and every number `NUMERIC_EXACT@0.01`, so `invoice_id` and `notes` were treated alike; Stickler 1.0's [comparator inference](https://github.com/awslabs/stickler/releases/tag/v1.0.0) picks per leaf (`*_id` → Exact, `amount` → Numeric, date-typed strings → Date). Operator annotations still win — any field carrying `x-aws-idp-evaluation-method` is honoured untouched. ⚠️ **Accuracy numbers will drift on un-annotated schemas**, so comparisons across this upgrade are not like-for-like; `STICKLER_RESULT_VERSION` moves `2.0` → `3.0` so cross-version comparisons are flagged in the UI. See [Comparator inference rules](docs/evaluation.md#comparator-inference-rules).

- **`--headless` deployments must set up their own alert delivery, and the docs now say so.** A standard deployment subscribes `AdminEmail` to `AlertsTopic`; `--headless` removes that parameter, so it creates no subscription and every alarm publishes to a topic with no recipient — successfully, which is why nothing distinguishes it from a healthy deployment. ⚠️ If you deploy headless, subscribe your own recipient to the `SNSAlertsTopicARN` output; [Monitoring](docs/monitoring.md#headless-deployments-you-set-up-alert-delivery) carries the `aws sns subscribe` commands for email and `https`. ([#984](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/984))

- **[Well-Architected](docs/well-architected.md) is now a review template you fill in, and its assessment states what the solution actually implements.** Each pillar names the mechanism you can go and inspect — the alarms and their thresholds, the two dashboards, the customer-managed key and the TLS-only bucket and queue policies, the permissions-boundary threading, the Step Functions retry blocks, the DLQ redrive values, the concurrency admission cap — and a new section separates what ships active, what ships off by default (`CircuitBreakerEnabled`, `BedrockGuardrailId`, `DeployInVPC`) and what is entirely yours (alerts subscribers, a budget, MFA, CloudTrail, WAF managed rules). The Security pillar states the API authorization posture as measured: of the 118 operations, 90 are group-restricted, 2 are IAM-only, and 26 are open to any authenticated pool user, 17 of those with no ownership or configuration-version check. ([#937](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/937))

- **The published granular-assessment cost comparisons now state a direction rather than a percentage.** The `−78%` and `−74%` Bedrock-cost figures in the [retirement note](docs/migration-granular-retirement.md), the [Extraction & Confidence guide](docs/extraction-and-confidence.md) and the retirement plan were produced while cost reporting mispriced cache reads (see the pricing fix below), and granular was a cache-heavy arm, so they cannot be re-priced after the fact — only re-run. The direction is not in doubt: correcting the price shrinks the ratio by a bounded factor and cannot reverse it. Nothing under `benchmarks/` was affected — the benchmark harness has always matched pricing keys exactly. ([#966](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/966), [#938](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/938))

### Fixed

- **The optional CloudFormation service role no longer amounts to account administrator** ([#927](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/927)). `iam-roles/cloudformation-management/IDP-Cloudformation-Service-Role.yaml` granted role and policy creation on `Resource: "*"` together with `iam:PassRole` on `"*"`, which is enough to create a role with any policy and pass it anywhere, so the role was transitively equivalent to administrator while describing itself as least privilege. Role creation is now scoped to a `ManagedStackNamePrefix` name pattern and gated on a new **required** `CreatedRolePermissionsBoundaryArn`, `PassRole` is scoped with `iam:PassedToService`, and five explicit denies stop the boundary being stripped or the role editing itself. ⚠️ **Upgrade-visible, and it lands on stacks that already exist**: `RoleName` is unchanged, so updating the service-role stack replaces the policy on the same role ARN and every IDP stack pointing at it deploys under the tightened policy on its next operation. Three configurations then wedge a stack in `UPDATE_ROLLBACK_FAILED` — a role created before this change (no permissions boundary), a stack name that does not match `ManagedStackNamePrefix`, and setting `PermissionsBoundaryArn` back to empty. All three are detectable before you update; the README's new "Updating an Existing Deployment" section gives the detection command, the failure signature and the recovery for each. Two consequences apply to **new** deployments through this role as well: `PermissionsBoundaryArn` on the IDP stack is no longer optional and must be the same ARN as `CreatedRolePermissionsBoundaryArn`, because `iam:CreateRole` is conditioned on it — an empty value fails every role and rolls the stack back; and `iam:PassedToService` includes `cloudformation` itself, which a nested-stack solution needs, since a nested stack inherits its parent's service role and CloudFormation passes the role to itself to do it. See [iam-roles/cloudformation-management/README.md](iam-roles/cloudformation-management/README.md).

- **The Web UI API dispatcher now denies by default: an operation whose required groups were never declared is refused, not dispatched** ([#928](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/928)). After the move from AppSync to API Gateway the dispatcher only authenticated, so group enforcement depended on every downstream resolver remembering to check and a newly added operation was open unless someone added one. The dispatcher now checks the caller's groups — from the verified JWT claim, never the request body — against a per-operation manifest generated from `scripts/api_rbac_expectations.yaml`, and returns `403` for an operation with no entry. ⚠️ **Adding an API operation now requires declaring its groups** in that file and regenerating the manifest, or it returns 403 for everyone including Admin. ⚠️ This is a floor for group-scoped operations only: 26 of the 118 operations are declared `ANY`, where authentication is enforced but group membership is not ([#979](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/979)). See [RBAC](docs/rbac.md).

- **An invocation of the Web UI API can no longer supply the identity the group checks read** ([#978](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/978)). `idp_common.api_adapter.normalize_event` treated an event carrying its own top-level `identity` as a legacy resolver event and passed it through, so a caller of that shape stated its own `cognito:groups` and every group check beneath was evaluated against that claim. It was a privilege-escalation amplifier rather than an open door — presenting such an event required a principal already holding `lambda:InvokeFunction` on the function — and it is now refused: identity is built from the API Gateway authorizer's verified claims only, and an asserted identity that contradicts them, or arrives with no verified claims, gets a `403`. IAM-gated service-to-service invocations are unaffected.

- **Agent Chat's group restriction is enforced inside the Lambda that runs the agents, and the streaming endpoint no longer lets the request body override a caller identity the transport proved** ([#920](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/920)). Agent Chat is reachable through the REST dispatcher and through a Lambda Function URL that streams token deltas, so the check in front of one path did not apply to the other; the processor now applies the same Admin/Author/Viewer check itself, before any agent call. Both streaming routes resolve the caller through one helper that prefers the transport-verified principal and refuses a contradicting body-supplied identity with a `403`, and both reject a malformed body with a `422`. On neither live path is the new gate the check that denies a request today — it is defence in depth that becomes load-bearing if the resolver gate is ever bypassed.

- **No Lambda writes its unredacted invocation event to CloudWatch any more** ([#921](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/921), [#977](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/977), [#985](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/985)). **40 log calls across 38 files** under `src/lambda/`, plus twelve API-resolver Lambdas, wrote whole invocation events — Cognito `sub`, email and group membership on every authenticated call, and in `agent_chat_processor` the user's full chat prompt once per conversation turn. This is a data-minimisation and log-hygiene defect rather than an access-control one: the log groups are encrypted with the customer-managed key and readable only to principals holding CloudWatch Logs permissions on them, but that is a wider audience than the people entitled to read a given user's conversation. Every site now passes the event through the canonical redactor, which preserves the event's shape while replacing denylisted values and capping content-shaped strings at 500 characters. Operators of existing stacks should treat these log groups as having contained user content when setting retention. 29 raw-event log calls remain in the `patterns/unified/src` pipeline steps and other out-of-scope trees, left for a change of their own.

- **CloudWatch alarms now reach somebody: the `AdminEmail` address is subscribed to the alerts topic** ([#922](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/922)). Every alarm the stack creates publishes to `AlertsTopic`, directly or through the circuit-breaker manager, and **no subscription to that topic existed** — the ARN was emitted as an output and an operator was tacitly expected to subscribe by hand, so a default deployment raised every alarm to an audience of nobody. ⚠️ **An SNS email subscription delivers nothing until the recipient clicks the confirmation link** in the *"AWS Notification - Subscription Confirmation"* email, which is valid for about two days; until then the subscription sits in `PendingConfirmation` without failing the deployment. ⚠️ `--headless` deployments are not fixed by this change — see the headless alert-delivery note above. See [Who receives the alerts](docs/monitoring.md#who-receives-the-alerts).

- **A systemic confidence-assessment failure is visible again: degraded sections publish a metric, and an alarm watches its volume** ([#996](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/996)). When the confidence pass fails deterministically the Assessment Lambda keeps the extraction and degrades the section, so such documents **complete** — no failed execution, no DLQ message, no alarm — and a missing `bedrock:InvokeModel` grant or a model id not enabled in the region was visible only as a processing issue in the Sections panel, one document at a time. Each degrade now publishes `AssessmentConfidenceUnavailable`, and `AssessmentConfidenceUnavailableAlarm` fires at ten or more within fifteen minutes with the three likely causes in its description; a **Confidence Assessment Degraded** dashboard widget shows the trend against that threshold. See [Monitoring](docs/monitoring.md#confidence-assessment-degraded).

- **A pipeline hook declaring `onError: fail` now actually aborts the document, at every hook point the active processing mode reaches** ([#919](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/919)). The policy is the documented way for a feature to gate processing — PII redaction being the case that matters — and it worked at only one of the seven points: the six post-step hook states caught `States.ALL` and routed **forward** on purpose, so a redaction hook could fail and its document would still be classified, extracted and delivered unredacted with nothing to signal it. The dispatcher now raises a distinct fatal error for that policy and each hook state catches it before its `States.ALL` entry, routing to a terminal failure state whose cause names the failing feature and hook point. ⚠️ **If you rely on `onError: fail` to gate, re-check documents processed before this release.** ⚠️ `onError` is now a constrained enum, and that validation also runs on the stack-deployment path — a `CustomConfigPath` YAML carrying `Fail` or `fail-fast` now produces a failed (cleanly rolling back) stack create/update rather than a rejected upload, so fix the value before upgrading. ⚠️ A `fail` policy at a hook point the active mode does not reach is silently inert: BDA mode has no `postOcr`, `postClassification` or `postExtraction` state ([#982](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/982)). See [Pipeline hooks](docs/feature-platform.md#pipeline-hooks).

- **A document uploaded once is processed once, however many times SQS redelivers its message** ([#904](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/904)). Under a saturated queue the `QueueProcessor` Lambda ran at 128 MB with 50-message batches and a 30 s timeout, so two thirds of its invocations under load timed out mid-batch; SQS then redelivered the whole batch and each redelivery started another unnamed execution — one upload ran six times, and a 4,384-document batch produced 14,727 billed executions with every alarm reading OK. Executions are now named after the SQS message, so a redelivery is refused and acked as a no-op with its concurrency slot handed back, the processor runs at 1024 MB with 10-message batches and a 60 s timeout, and a new `QueueProcessorErrorsAlarm` reports the timeouts no failure-keyed alarm could see. Upgrade-visible: execution names change from UUIDs to `<basename>-<message-id>`. See [Troubleshooting](docs/troubleshooting.md#documents-processed-more-than-once).

- **The workflow-concurrency counter can no longer be driven below zero, and one that already went negative repairs itself** ([#915](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/915), [#916](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/916)). Admission is gated on `active_count < MaxConcurrentWorkflows`, so a counter at -N ran N workflows above the configured ceiling with nothing reporting it: documents processed, queues drained, and the stack simply spent more on Bedrock and Textract than it was told to. The decrement was unconditional and per-invocation, so a redelivered terminal event released a slot a second workflow still held; it is now refused at 0 and carries a per-execution marker so each execution releases exactly one slot, and the next admitted document raises a negative counter back to the executions actually running. New `ConcurrencyCounterUnderflow` and `ConcurrencyCounterActive` metrics with alarms make both conditions visible. See [Monitoring](docs/monitoring.md#workflow-concurrency-counter).

- **A Lambda timeout is retried once in every workflow state, and a failed evaluation-failure recorder no longer takes the document with it** ([#917](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/917), [#918](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/918)). The single-attempt timeout policy had been applied to the evaluation step only, so the other eleven Lambda task states retried a 900-second timeout on the transient ladder — eight attempts at 2.5× backoff, about 5.2 hours holding a workflow-concurrency slot before failing the same way it failed the first time. Each state now carries its own timeout retrier at one attempt, with the throttling and Lambda-service ladders untouched. Separately, an error inside `RecordEvaluationFailure`'s own catcher raised `States.Runtime`, which no `Catch` intercepts, discarding a fully processed document at the last step; the envelope is now rebuilt. See [configuration](docs/configuration.md#step-functions-retry-configuration).

- **A section of more than 20 pages no longer fails on Bedrock's many-image pixel cap, and an image-shaped rejection is explained rather than retried** ([#994](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/994)). Bedrock caps each image at 8,000 px per side but at **2,000 px** once a request carries more than 20 image blocks, so no per-image guard could see it and the request failed as a whole with `image exceed max allowed size for many-image requests`. Every assembled request is now swept immediately before `converse` — so classification, assessment, summarization, evaluation and few-shot examples are covered as well as extraction — downscaling over-cap images on a copy of the request. A rejection that does slip through is now classified as deterministic and fails in seconds with the image count, the largest dimension and the one remedy that applies, instead of being retried up to 50 times behind exponential backoff. ⚠️ **The clamp costs image resolution on newer models**: Claude 4.7+, Opus 5 and Sonnet 5 tokenize images toward a ~2,576 px long edge, so 2,000 px is about 20% below what they would otherwise receive; extraction accuracy has **not** been measured with and without it. The page count at which it bites depends on configuration — roughly 21 pages for Simple extraction, and about 101 on Advanced's shipped defaults (`max_concurrent_batches: 10`), falling to 11 with sharding off. Set `image.target_width`/`target_height` to 2,000 or less, or keep requests at 20 images or fewer, if you would rather not have it applied. See [Configuration](docs/configuration.md#a-request-with-more-than-20-page-images-caps-every-image-at-2000-px).

- **A failed confidence pass no longer throws away the extraction it was scoring** ([#901](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/901)). Confidence scoring runs after extraction has been written and paid for, but a deterministic `Input is too long for requested model` from the confidence model failed the whole document — two observed runs discarded $17.34 and $7.05 of extraction that had returned 1,200 of 1,200 rows at 1.000 cell accuracy, to report the loss of the advisory part. A deterministic failure now degrades instead: the data is returned, the document succeeds, and the missing confidence is recorded as an error-severity processing issue on the section. Transient failures still raise so Step Functions retries. ⚠️ Upgrade-visible: a document that previously failed outright now completes with no confidence values on the affected section, so HITL confidence routing and UI threshold signals do not apply to it. See [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).

- **Confidence scoring stops spending model calls on a batch that can never fit, and says why** ([#894](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/894)). The self-healing ladder halves any truncated confidence batch, which converges only while a smaller batch can fit; with a multi-instance class one row of the outer list is a whole document instance carrying its own long inner list, so a single-row call still truncated and the ladder kept re-running the impossible call before reporting a generic error whose remedy — lower `list_batch_size` — provably cannot work. It now stops as soon as a one-row call truncates and reports `assessment_row_too_large`, naming the model, its output cap, the field and class, and that lowering the batch size cannot help. ⚠️ Two things stay open under #894: the batch sizer still measures only the outer row's column count, so a multi-instance class with a long inner list still cannot be fully assessed, and the reported 900-second `Sandbox.Timedout` symptom is not explained by this loop.

- **Every confidence-recovery model call now checks the Lambda's remaining time before it is made** ([#958](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/958)). The recovery ladder's bisection and escalation rungs consulted the wall-clock deadline; the same-model retry rung did not, so a sequence of rounds that each recovered something was bounded only by the round budget — a real model call per round, however little of the 900-second Assessment Lambda was left. The check now runs per call rather than per round, which bounds the wall clock exactly and keeps every row the budget did cover by stopping mid-pass. When it fires, the section reports `assessment_incomplete` with the time budget named and its real remedy — more Lambda time, or less work per section. See [Troubleshooting](docs/troubleshooting.md#confidence-assessment-failures).

- **A model that cannot emit a valid tool-use sequence fails in seconds with the model named, instead of being retried for 45 minutes** ([#895](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/895)). Advanced (agentic) extraction on Amazon Nova Lite fails with `Model produced invalid sequence as part of ToolUse` and emits the same block on every attempt, but because that error code is otherwise genuinely transient the state machine retried it up to eight times per shard — one benchmark refresh logged 247 of them, with documents sitting in extraction for 45+ minutes. That one outcome now overrides the code-level verdict and surfaces as a named error listing three remedies: reduce `extraction.agentic.shard_token_budget` / `max_pages_per_shard`, switch to a model measured on this path, or use `extraction.mode: simple`. Nova Lite is unaffected elsewhere — it remains supported, and the default, for confidence assessment and works for Simple extraction. See [Extraction and Confidence](docs/extraction-and-confidence.md#supported-models-for-agentic-extraction).

- **Long-context Claude models (a `:1m` model ID) are no longer reported at up to double their real cost** ([#899](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/899)). `config_library/pricing.yaml` carried a separate entry per `:1m` model at a 2× input / 1.5× output premium for Sonnet 4.6 and 5 and Opus 4.6 through 5. No such premium exists for those models — verified against the AWS Price List offer file and Anthropic's own statement that the full 1M-token window is priced at standard per-token rates from Claude 4.6 on; the structure was the earlier Sonnet 4/4.5 beta's pricing. Nothing was ever overspent, but these models looked unaffordable in every document cost table, Athena rollup and benchmark summary: one arm was reported at $125.42 against $68.82 for the same work. ⚠️ **Upgrade-visible**: reported costs for `:1m` models drop, so cost comparisons across this upgrade are not like-for-like; figures already written to the reporting tables are not rewritten. A deployment holding a saved Custom pricing table should restore pricing defaults to clear its stale rows. See [cost considerations](docs/cost-calculator.md#what-the-estimate-cannot-express-prices-that-change-with-request-size).

- **Cost reporting no longer misprices cache tokens: a model's price is resolved by exact id, and an unpriced service is reported as unpriced instead of $0.00** ([#926](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/926)). The pricing lookup fell back to a case-insensitive substring match, so a model id with no pricing row bound to a related row and, within it, `cacheReadInputTokens` bound to that row's `inputTokens` price — measured at 7.99× the real rate for `eu.amazon.nova-2-lite-v1:0:flex`. The error ran both ways: cache reads cost a fraction of fresh input, but a cache **write** costs 1.25× it, so charging writes at the input rate understated them by 20%. Resolution is now by exact key with progressively shorter `/`-delimited suffixes, in both the reporting pipeline and the Web UI's Estimated Cost table, and a service with no entry at all now writes `NULL` with a warning so `WHERE unit_cost IS NULL` finds the gap. Removing the fallback also fixed what it had been hiding: both shipped LambdaHook rows had never matched exactly (the metering key now carries the bare function name rather than an ARN), Lambda invocations were metered under a unit the table does not price and so costed $0.00, and missing rows were added — including one model the shipped presets pin and one that two configuration dropdowns offer with no pricing entry at all. ⚠️ A model with a complete pricing entry was always priced correctly, so this affected only ids with no entry or an entry missing `cacheReadInputTokens` — among shipped selectable models, the EU Nova 2 Lite service tiers. Cost figures for a cache-heavy workload on one of those should be re-run. See [Reporting Database](docs/reporting-database.md#cost-calculation-process).

- **The Web UI's "update available" indicator now works — it never has** ([#962](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/962)). The indicator reads one key, `artifacts/genai-idp/idp-main-latest.json`, with an anonymous `GetObject`, and neither the publisher nor its public-ACL pass covered that key — it sits one level above the tree the pass walks. Measured credential-free against all three release regions on 2026-09-18, `idp-main.yaml` returned 200 and the pointer returned 403 in every region, so object ACLs are load-bearing and the pointer had never been publicly readable by any deployed stack; the resolver fails closed, so every customer's Build Info panel reported "no update available" indistinguishably from being current. Fixed by applying the ACL when the pointer is written, covering it in the ACL pass so previous releases' pointers are repaired on the next publish, and adding a step that re-checks every key a public publish advertises with an unsigned client and **fails** the publish on a 403.

- **The Web UI API dispatcher's log group is encrypted with the customer-managed key and takes its retention from `LogRetentionDays`** ([#972](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/972)). The highest-traffic log group in the deployment — the dispatcher fronting every routable Web UI operation — set no `KmsKeyId`, so it used the AWS-owned CloudWatch Logs key while 111 of the other 113 groups in the four templates that receive the customer-managed key already used it. It was also the only one of the 114 that hardcoded `RetentionInDays: 30`, which made `LogRetentionDays` a partial control; that parameter defaults to 30, so only a deployment that overrode it sees a change in effective retention. The two remaining unencrypted groups belong to custom-resource-only Lambdas that run solely during a stack operation.

- **Every queue and every bucket now carries a resource policy that refuses non-TLS calls** ([#964](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/964)). Ten of the eleven `AWS::SQS::QueuePolicy` resources in the repository carried an explicit `Deny` conditioned on `aws:SecureTransport: false` and the data-mart rollup dead-letter queue's did not, and eight further queues had no resource policy at all. SQS endpoints are HTTPS and every AWS SDK uses TLS unless deliberately reconfigured, so this is defence in depth rather than an exposed path, but the deny is what makes the guarantee explicit and auditable. Coverage is now 19 queue policies for 19 queues and 15 bucket policies for 15 buckets; each added policy contains only the deny, so it grants nothing and cannot change existing access, and there is no behaviour change for a deployment whose clients already use TLS.

- **`anyio` is floored at 4.14.2 in the Lambda runtime dependencies** ([#1008](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/1008)). `anyio` reaches the agents, multi-document-discovery, code-intel and `all` extras transitively through `strands-agents`, and versions at or below 4.14.1 encode host names with IDNA 2003 in `TLSStream`, so a look-alike name can be accepted against a certificate issued for a different host (GHSA-82r6-8w77-94w6). The floor is declared once in the core dependency list rather than per extra, so no resolution path can miss it.

- **X-Ray traces can be filtered by document id on the four pipeline Lambdas that do the expensive work** ([#925](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/925)). OCR, classification, extraction and assessment passed `{document.id}` — a one-element set — to `put_annotation`, which validates the value type and **drops** it — logging that it is ignoring an unsupported annotation value rather than stringifying it — so the annotation was never stored and filtering by document id returned nothing on exactly those four functions. All 15 annotation calls in the tree are now correct. A related gap is not fixed here and is tracked as [#983](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/983): `EnableXRayTracing` controls only the two state machines, 19 Lambdas hardcode `Tracing: Active`, and the two rule-validation functions declare none and so run in `PassThrough`.

- **Advanced extraction's shard agents are now told that their tables have already been parsed** ([#900](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/900)). When the pre-flight table parse succeeds the agent's instructions gain a block naming the table and row counts, the columns, and the `parse_table` → `map_table_to_schema` → `finalize_table_extraction` sequence — but it was appended on the single-pass path only, so the sharded path, which is the default for any multi-page table document, sent none of it. Both paths now build it from one helper, and each shard also receives its own page range and a note that parsing fewer rows than the section total is correct for a shard. ⚠️ The saving is unmeasured: the expected direction (fewer extraction output tokens, completeness unchanged) is inferred from the analogous single-pass fix in the [advanced extraction study](docs/benchmarking/studies/advanced-extraction-refinements.md). See [extraction and confidence](docs/extraction-and-confidence.md#deterministic-table-parsing).

- **Test Studio: running a test set against a large configuration profile no longer fails with "Item size has exceeded the maximum allowed size".** The configuration table gzip-compresses profile bodies, so a profile can be several megabytes raw, but the test runner copied the decompressed body inline onto the run's DynamoDB metadata record — any profile past roughly 390KB of JSON saved fine on the Configuration page and then failed every Run Test at submit with DynamoDB's raw error. The run now stores its captured configuration compressed in the same shape the configuration table uses, still reading runs created before this change, and a body that does not fit even compressed is refused at submit with both sizes named.

- **Test Studio → Executions no longer times out on stacks with a few hundred test runs, and the Comparator Changes panel no longer scans the whole tracking table.** Both were unbounded DynamoDB reads on the request path: the executions list hydrated every run in the date range through a serial `BatchGetItem` loop that also dropped `UnprocessedKeys`, hitting the dispatcher's 20 s timeout at ~500 runs and silently returning a short list, while the panel's sample-document probe ran a filtered full-table `Scan` that exceeded API Gateway's 29 s ceiling and returned 504. The list now caps at 100 runs per query with a page-size preference in a `CollectionPreferences` modal, re-issues deferred keys with backoff, and shows a "Showing the N most recent test runs" banner when the cap was filled; the probe reads the run's own file list and fetches those keys in one round trip.

- **The Schema Builder's Evaluation Method dropdown now offers methods for a field that references a shared class, instead of rendering empty** ([#906](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/906)). A property written as a bare `{"$ref": "#/$defs/Address"}` carries no `type` of its own, and the inspector filtered the eight evaluation methods by the property's own type, so every option was filtered out and the field could not be given a method from the UI at all. The inspector now follows the reference and filters on the referenced definition's type; where the type genuinely cannot be resolved it offers every method except Hungarian rather than none. Three other places that read a property's type without resolving `$ref` are corrected with the same helper. The backend was never affected, so a method set by hand-editing the schema JSON was always honoured.

- **A test-set zip with only an `input/` folder is accepted again as an unlabeled set** ([#897](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/897)). The wizard's "Upload documents only" source, the docs and the resolver's validator all treated a missing `baseline/` folder as valid but unlabeled, while the zip extractor kept an unconditional check and failed every such upload with zero documents registered. Baselines that are present but do not line up with the inputs still fail. The Test Sets table also shows a failed set's full error in a keyboard-reachable popover instead of its first 15 characters, and a refused upload stays FAILED with that reason rather than being rewritten to a COMPLETED empty set on the next refresh.
...
- **Cohere Parse LambdaHook: with table and figure geometry but no confidence scores.** The new sample hook in `samples/lambda-hook-inference/GENAIIDP-cohere-parse-hook/` runs the hosted Cohere Parse API as a fully serverless OCR backend. Tables and figures highlight in the Web UI Visual Editor, and Parse's HTML tables are converted to Markdown so agentic extraction's deterministic table parser can read them. ⚠️ **Cohere Parse returns no confidence scores**, so `{OCR_TEXT_CONFIDENCE}` reads `N/A` and extraction confidence comes from the assessment model alone; do not tune HITL thresholds against OCR confidence on it. Images only, nine stable languages, 500 requests/minute. See [LambdaHook OCR](docs/lambda-hook-inference.md#cohere-parse-integration). (#848)

- **The processing workflow has an execution-level timeout, `WorkflowExecutionTimeoutSeconds` (default 6 hours), and a `WorkflowTimeoutsAlarm`.** An execution that stopped making progress could previously run for up to a year while holding a workflow-concurrency slot with every alarm reading OK. A tripped bound ends the execution `TIMED_OUT`, releases the slot, records `WorkflowStatus` `TIMED_OUT` on the document, and raises the new alarm; the dashboard plots timed-out runs. The default is about ten times the longest successful run measured on a production stack, but a timed-out execution discards completed work, so raise it for very large packets whose successful runs approach it. See [Monitoring](docs/monitoring.md). ([#757](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/757))

- **Prompt caching is now observable and switchable: the product reports whether each cache point did anything, warns when one can never engage, and `extraction.prompt_cache: off` sends none.** Each section's `result.json` carries `metadata.prompt_cache` (read / written / uncached input tokens and one of six states such as `caching`, `write-only` or `never-cached`), shown in the Processing Report, on each phase's row of the document cost table, and as `metadata.prompt_cache.*` columns in the `document_sections_<class>` Athena tables for per-class queries. `idp-cli config validate` warns per class when the prefix before a `<<CACHEPOINT>>` is below the model's minimum cacheable prefix (512 tokens on Claude Opus 5 / Fable 5, 1,024 on Sonnet 5, 4,096 on Opus 4.6 / 4.5 / Haiku 4.5), where Bedrock bills the prefix at full price and reports nothing; a quarter of the shipped classes never cached at the 1,024 tier. A cache write costs 1.25× and pays back only on a repeat within 5 minutes, so a low-volume deployment can now turn caching off (also in the Web UI's Extraction settings). Extraction only. See [configuration](docs/configuration.md#reading-cache-efficiency-back-per-phase-and-per-class). ([#780](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/780))

- **Multi-instance detection now also runs on Advanced (agentic) extraction, for sections short enough not to be sharded.** `extraction.multi_instance_detection` was skipped whenever agentic extraction was on, so a section that silently dropped records of one class raised no warning in exactly the mode recommended for large tables. Sharded sections are still not probed. Measured cost is about one accuracy point and up to 25% more extraction input tokens, so as in Simple mode treat detection as a diagnostic to run once rather than leave on. See [extraction and confidence](docs/extraction-and-confidence.md). ([#772](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/772))

- **Discovery suggests, and never silently sets, several documents per section when a sample holds more than one record of the discovered class.** The job details page shows *"This sample appears to contain N 'X' records"* with a one-click **Enable several documents per section** action that sets `x-aws-idp-multi-instance` on the class, and the jobs table shows an *"N records in sample"* badge. Discovery itself never changes the setting, because one sample cannot tell a multi-record class from a packet that should have been split. See [Discovery](docs/discovery.md#samples-that-hold-several-records-of-one-class). ([#765](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/765))

- **Test Studio's confidence→accuracy curves are keyed by the configuration that produced them, and the review-effort estimate says which curve it used.** The estimate used to read a set-wide blend of every configuration the set had ever been labeled or scored under, and a profile's revisions shared one curve even across a model swap. Curves are now keyed by the profile's **confidence fingerprint** (a hash of the confidence-relevant settings), and the estimate falls back revision → pooled profile → set aggregate → prior, reporting `curveSource` and `confidenceFingerprintSource` on the API and in the modal. Upgrade-visible: a set whose estimate used to blend configurations now reads a thinner, honest curve and can drop out of the gold tier. See [Test Studio](docs/test-studio.md#the-curve-is-measured-and-it-improves). ([#698](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/698), [#759](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/759))

- **Test sets can be edited in place: remove documents, add from the set's own page, or start a set empty.** A set's page gains row selection with a **Remove** action and an **Add documents** menu (bucket pattern, zip, or synthetic generation pre-targeted at the set); the create wizard gains **Start empty**. An empty set is a healthy state: it stays COMPLETED at zero documents, and running, publishing or labeling it is refused with a reason. See [Test Studio](docs/test-studio.md#adding-documents-to-existing-test-sets). ([#812](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/812))

- **The evaluation report view in the Web UI now shows everything the markdown report does.** It had shipped as a summary with a click-through to the markdown for the detail. Added: document-split figures and section split analysis, every metric with its rating, excluded sections with reasons, per-section scores, the not-evaluated state with how-to-fix steps, Confidence and Weight columns, expandable nested field comparisons, and a scoring explainer. See [Web UI](docs/web-ui.md#evaluation-report).

- **`TestRun` reports `avgWeightedOverallScore`, the run-level "Avg Weighted Score" figure the UI displays.** It is the unweighted mean across documents of each document's weighted score, skipping unscored documents, so callers no longer average the per-document map themselves. Additive: runs aggregated before this release return a value too.

### Changed

- **`LogLevel` defaults to `WARN` instead of `INFO`.** At `INFO` the accelerator can write presigned URLs, document contents and PII into CloudWatch Logs, contrary to the production guidance in [well-architected](docs/well-architected.md). Existing stacks keep their deployed value; the six shipped installable features still launch at the `INFO` pinned in their `feature.yaml`. `WARN` turns off the per-stage progress lines, the Web UI REST API access log and the dashboard's "executions over latency threshold" widget (see [Monitoring](docs/monitoring.md#loglevel--what-warn-turns-off)). Setting `WARN` on a stack with the Feature Platform enabled used to fail nested-stack parameter validation, because those templates accepted only `WARNING`; they now accept both. `idp-cli deploy --log-level` no longer defaults to `INFO`: omit it to take the template default or preserve an existing stack's value. See [configuration](docs/configuration.md#general-parameters). (#873)

- **Advanced extraction's shard budget subtracts the measured prompt overhead, so `context_buffer` is a true safety margin.** Shard planning budgeted against OCR page text only; the system prompt, the task prompt with the rendered schema, few-shot text and the tool schema were absorbed by the blanket 30% buffer, so a prompt-heavy configuration could still hit `Input too long`, and turning the schema restatement off never changed a shard count. The overhead is now estimated from what will actually be sent and reported in the sizing plan and the Processing Report. Typical documents keep their shard count; only pages denser than about 2K tokens each could split a page-bound shard. ([#775](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/775))

- **Content-Security-Policy hardening: `script-src` no longer trusts every HTTPS origin, and API Gateway hosting now sends a CSP at all.** The single surviving cross-origin script source is `https://cdn.jsdelivr.net/npm/`, which the Monaco editor loads from at runtime; dropping it would blank every editor in the UI. GovCloud and private-network deployments, which must use `WebUIHosting=APIGateway`, previously ran with no CSP and now send the same policy (with `frame-ancestors 'none'`). ⚠️ **Private-network deployments** must allow browser egress to jsDelivr or the Configuration, Pricing and schema editors render blank; see [private-network deployment](docs/deployment-private-network.md) and [API Gateway hosting](docs/apigateway-hosting.md). (#882)

- **Two IAM action wildcards are gone: the AgentCore runtime manager and the S3 Vectors manager grant only the APIs their handlers call.** The data generator's role held `bedrock-agentcore:*` (255 actions) on `Resource: "*"` where its handler uses five, and the Bedrock Knowledge Base's S3 Vectors role granted the four data-plane vector actions it never calls. Both are now scoped to the deploying account and Region, with no behaviour change for existing stacks. (#876)

- **Importing test-set documents by file pattern is Admin-only.** The bucket-pattern operations search a whole bucket, so an Author probing patterns could learn which documents exist and copy them, baselines included, into a set. Authors still create sets from a zip, by generation or empty, and still add documents by zip. See [RBAC](docs/rbac.md).

### Fixed

- **The Code Intelligence Agent was enabled by default, so a fresh deployment sent chat queries to a third-party service before anyone asked it to.** The docs said it was off, but the chat checkbox rendered pre-checked and every backend path treated an omitted flag as `true`, so code-related questions were routed to the hardcoded, unauthenticated DeepWiki MCP endpoint on stacks nobody had configured for it. All defaults are now `false` and the backend requires a literal boolean `true`; a deployment that wants the agent keeps it by checking the box. When enabled, the endpoint is still hardcoded and unauthenticated, so the sensitive-information warning still applies. See [Code Intelligence](docs/code-intelligence.md). (#883)

- **Chat-with-Document's config-version scope check no longer treats "cannot evaluate the scope" as "unrestricted".** A missing `USERS_TABLE_NAME` or an event with no caller identity silently disabled `allowedConfigVersions` RBAC for every caller; those cases now deny (`scope_unavailable`) and never reach Bedrock. Scoping remains opt-in, so a user with no scope row is still unrestricted. ⚠️ **Not fixed here:** the DynamoDB-failure path still fails open, because the lookup names a `SubIndex` that does not exist and the streaming transport carries no resolvable caller identity, so this check has never actually enforced anything for chat; that failure is now logged at ERROR rather than WARNING and is tracked with AUTH.T07. (#874)

- **The AgentCore Gateway manager Lambda can no longer grant itself IAM permissions.** Its execution role held `iam:CreateRole`, `iam:AttachRolePolicy` and `iam:PassRole` on `Resource: "*"`, a privilege-escalation path flagged High by the Security Invariant scan and confirmed in AppSec review, though none of it was reachable in practice. It now holds `iam:GetRole` and `iam:PassRole` on the single gateway execution role, and existing gateways keep working. (#875)

- **External IdP group mapping requires the group claim to have come from the configured provider, and the first federated sign-in now carries its role group.** The trigger assigned Cognito groups from `custom:idp_groups` without checking who set it; it now honours the attribute only for a user linked to the provider named by `ExternalIdPName`, and only on a fresh sign-in, leaving membership unchanged when it cannot confirm. Separately, the token override it returned used the response key for V2 triggers while the pool registers a V1 trigger, so the first token after a federated sign-in carried no role group; both keys are now emitted. ⚠️ **Applies only to deployments with an external IdP and `ExternalIdPGroupAttributeName` configured. Configure your IdP to send the group attribute for every federated user**, including users in no group, so each sign-in carries an authoritative value. See [external IdP](docs/external-idp.md).

- **`getStepFunctionExecution` checks the execution ARN it is given.** It passed the caller-supplied ARN straight to Step Functions, so it could return the input, output and history of any execution its role could reach, including a sibling deployment whose stack name extends this one's. It now requires the ARN to name this deployment's state machine and applies the caller's `allowedConfigVersions` to the execution's configuration version.

- **Federated users can sign in more than once on new deployments: `ExternalIdPEmailMutable` parameter.** The pool's `email` attribute was immutable and Cognito re-applies the IdP attribute mapping on every federated sign-in, so a SAML/OIDC user could sign in exactly once. The flag is fixed at pool creation, so it is gated on a new parameter defaulting to the historical `false`. ⚠️ **Set it to `true` when creating any stack that uses an external IdP** (`idp-cli deploy` does so automatically when `ExternalIdPType` is set) and **never change it on an existing stack**, which wedges the update; the CLI and SDK refuse such an update. Existing federated deployments need a new stack to get the fix. See [external IdP](docs/external-idp.md#federated-email-attribute-mutability). ([#835](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/835))

- **Simple extraction with `integrated` confidence no longer loses list rows: list-bearing classes are scored in a separate pass automatically.** On a class with a list field the single 1S-TopK response stopped emitting rows rather than erroring: 1–10 of 100 rows, and an 800-row list absent entirely, every one reporting `COMPLETED`. Such sections now run the plain extraction prompt and let the Assessment step score them, recorded as `metadata.confidence_mode_effective: separate`; `idp-cli config validate` warns and names the affected classes. ⚠️ **This costs more than the single inference configured:** about 2.5× the cost and 7.7× the wall-clock per 100-row document at Claude Sonnet 5 (model-dependent; at Sonnet 4.6 the separate pass was cheaper). A class whose lists you have verified come back complete can opt back in with `x-aws-idp-allow-integrated-lists: true`. Scalar-only classes and Advanced extraction are unchanged. (#795)

- **An escalation re-extraction that returned *less* data could be kept as an improvement.** With `extraction.validation.fail_action: escalate`, a 100-row list with one bad cell per row (100 errors) lost to the same list returned as `null` (1 error), and a valid 50-row result replaced a 100-row original. The gate now refuses any result that loses populated data, compares errors per field, and accepts each escalated field on its own merits; the decision and its reason appear in the Processing Report. ⚠️ **Stricter than before:** a valid re-extraction carrying less data is kept only when the original value was itself invalid. ([#791](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/791))

- **Advanced extraction can say "I could not read this cell" instead of inventing a value.** A `required` attribute was non-nullable in the model the agent fills in, so for an unreadable numeric cell the only accepted answer was a fabricated `0.0`, schema-valid and indistinguishable from a real zero; on one unreadable column that happened for all 100 rows while the section reported valid. Required scalars are now nullable, required-ness is enforced by `extraction.validation` and reported as `'X' is a required property`, and abstained cells are counted in `metadata.abstained_fields` and shown in the Processing Report. Two consequences: with `fail_action: escalate` an honest abstention now triggers a paid re-extraction, and the larger restated schema invalidates existing prompt caches once on deploy. ([#782](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/782))

- **Forced tool use: a group attribute whose name contains a space came back from Claude Sonnet 5 as a JSON string, failing every section.** `$defs` names were never made wire-safe, so a group called `Account Holder Address` produced a `$ref` with spaces that Sonnet 5 does not resolve (Sonnet 4.6 did); on the shipped `bank-statement-sample` with forcing on, 0 of 19 sections validated. Definition names and `$ref`s are now rewritten, and a group still returned as a string is parsed with its authored field names restored. ([#783](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/783))

- **Classification over-split a single multi-page document because a table's reprinted column headings looked like a new heading.** With `sectionSplitting: llm_determined`, the second or third page of a three-page bank statement was called `start` in about half of runs, silently splitting one statement into two or three sections. The default boundary rules gain a table-continuation rule, measured 8/15 → 15/15 correct on the benchmark statement (Nova 2 Lite) with the other fixtures unchanged. ⚠️ A configuration profile saved on a running stack keeps its stored classification prompt: reset it to the default or re-apply the `<boundary-detection-rules>` block to pick the rule up. A reprinted title and account header on every page is still not handled; use the per-class `BOUNDARY:` sentence for it. ([#726](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/726))

- **A Simple-mode extraction that returned a fraction of a long list no longer reports success silently, and the too-large-section failure says what to change.** An 800-row / 17-page statement returned 43 rows with `COMPLETED` and no processing issue, and 25+ pages failed with Bedrock's bare *Input is too long*. A new `extraction_rows_below_ocr_estimate` warning compares the rows extracted with the section's OCR tables of the same shape (needs Textract `TABLES` or BDA), and the failure is raised as `ExtractionInputTooLarge` with the request size, the model's window and the remedy: Advanced extraction, which shards, or splitting the document. (#843)

- **Advanced extraction: the class's root description and nested group descriptions now reach the tool schema the agent sees.** Tool-schema generation dropped every object-level `description`, so four of the six lending classes, whose only guidance is a root description, sent none; the prose copy of the schema in the task prompt masked it. ([#836](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/836))

- **A page image over 3.75 MiB no longer fails the whole document.** Bedrock's 5 MiB per-image limit applies to the base64 payload, so the effective budget is 3.75 MiB, and with the default (no resize) image settings any larger page failed extraction with `image exceeds 5 MB maximum`; 3.8% of a 293-document corpus failed this way. Oversize images are now downscaled proportionally to fit (and to the 8,000 px per-side cap), logged, and recorded per page in `metadata.image_downscale`. See [configuration](docs/configuration.md#effective-per-image-budget-375-mib-enforced-post-base64). ([#778](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/778))

- **Confidence scoring of long lists no longer over-sizes its batches, and Nova Lite no longer burns a 60-second, 10,000-token loop per document.** The rows-per-inference derivation measured column count from the first row only, trusted the configured ceiling when the model's output cap was unknown, and displayed a size derived from the extraction model rather than the confidence model; an over-sized batch truncates, and the halving-retry ladder ran one 800-row statement into five 900-second Lambda timeouts before `ABORTED`. Separately, Amazon Nova Lite at temperature 0 repeated the same row object until the cap on 25-row batches, so Nova Lite/Micro now score at most 12 rows per call and request a sized output budget. `list_batch_size` keeps its meaning as a ceiling; no configuration change. ([#784](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/784), #861)

- **Documents with many low-confidence fields no longer lose their status-tracking record.** Confidence alerts are stored inline in the tracking item; each assessment batch re-reported every alert, and a wide low-confidence table can legitimately produce thousands, so the item exceeded DynamoDB's 400 KB limit, the write failed, and the run showed no result. One alert per finding is kept and the stored list is capped per section and per document, keeping the lowest-scoring alerts and recording the true count; every field's confidence remains in `explainability_info`. (#814, #821)

- **A confidence alert on a batched list row names the row's position in the document, not inside the model call that scored it.** Alerts said `transactions[6]` for row 16 of the merged list, colliding with the genuine row 6; the alert surface is now regenerated from the merged assessment so every index is global and per-cell (`transactions[16].date`). ([#813](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/813))

- **Transient failures inside the extraction and assessment Lambdas are retried, and documents no longer fail on a Lambda still provisioning its code.** A botocore read timeout or dropped connection failed the document outright because the tasks retried only Bedrock's modeled error codes; the handlers now re-raise transient causes under one stable `TransientError` name that the extraction and assessment states retry, while deterministic failures are still not retried. `Lambda.CodeArtifactUserPendingException`, seen when a cold stack's first documents arrive, is now retried by every Lambda task. ([#787](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/787), #801)

- **A transient Bedrock timeout could consume a whole Lambda invocation asleep.** Agentic extraction's retry backoff doubled up to 1,800 seconds, twice the 900-second limit of the function it runs in, which is why one cost run showed 2 of 5 documents aborting in advanced integrated mode. Any one backoff is now capped at 60 seconds and 300 in total, and a sleep is shortened to fit the time left in the invocation. ([#786](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/pull/786))

- **Metering no longer breaks on Bedrock's new structured `usage` members.** Bedrock's Converse `usage` block gained `cacheDetails`, a list, which was summed into metering as if numeric: every document logged `unsupported operand type(s)` and the Web UI showed a spurious `cacheDetails` cost row. Only numeric usage members enter metering now; the cache read/write counts pricing uses are unaffected. (#852)

- **LambdaHook OCR backends: a hook slower than 60 seconds is no longer abandoned and re-invoked, and one returning no confidence scores no longer reports every line at 0.0.** The Lambda client used for hooks had boto3's default 60-second read timeout and its own retries, so a slow third-party OCR call was re-invoked while still running, re-billing the same work: one 5-page document produced 44 invocations and zero completed pages. The client now waits 840 seconds and leaves retrying to the hook wrapper. A line with geometry but no `Confidence` is rendered `N/A` instead of telling the assessment model the whole page was unreliable. (#848)

- **Two state machines carried hardcoded `arn:aws:states:::` service integrations that Step Functions rejects in GovCloud.** The Multi-Document Discovery and Fine-Tuning state machines failed there with "resource belongs to a different partition"; they now use the deployment's partition, and the Multi-Document Discovery CodeBuild's ECR login host uses `${AWS::URLSuffix}`. The partition check now discovers templates by content, so a new template anywhere in the repo is covered. (#877)

- **Lending sample: the Payslip's `EmployeeNumber` and `PayrollNumber` are extracted reliably.** The sample payslip is an ADP statement whose identifiers sit under a `CO. FILE DEPT. CLOCK NUMBER` header with no matching labels, so `PayrollNumber` was usually null and `EmployeeNumber` wandered between columns. The `lending-package-sample` presets now name the ADP columns (FILE and NUMBER). Config-only: a stack on the `default` profile receives it on upgrade; a stack on another profile re-applies the preset or edits the two descriptions. ([#839](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/839))

- **Every request-serving Lambda now has a log group with the stack's retention and a name that survives function replacement.** Thirteen functions (the UI-facing feature-platform resolvers, the batch pre-processor and the five sample Lambda hooks) had no log group of their own, so Lambda auto-created one with no expiry and their logs were billed forever; 21 others named their group after the function, which inverted the create order (`ResourceAlreadyExists` on custom-resource Lambdas) and left a never-expiring orphan on every replacement, 79 of them on one long-lived stack. All now use `/${AWS::StackName}/lambda/<FunctionLogicalId>` honouring `LogRetentionDays`, which is now also forwarded to the sample hook stacks and the feature-platform nested stack. ⚠️ **On an already-deployed stack the 21 renamed groups are recreated and their history (typically under 2 KB) discarded; repoint any saved CloudWatch query, alarm or bookmark pinned to an old `/aws/lambda/…` name. Pre-existing orphans and the old auto-created groups are not deleted by the update**; they are the groups under `/aws/lambda/<stack>-*` with `retentionInDays: null`, removable by hand or by an `idp_sdk` teardown. Custom-resource-only Lambdas deliberately keep their auto-created group. ([#818](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/818), [#826](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/826))

- **Two test runs of the same test set started within one second no longer collapse into one.** A run id is `<set>-<timestamp to the second>` and the metadata write was unconditional, so two arms of an A/B launched back to back, or two users clicking Run together, shared an id and every document attached to one run. The write is now conditional and a colliding id moves to the next second. ([#879](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/879))

- **A test run pinned to a profile or revision that does not exist is refused at submit, and a missing revision body is a clear error rather than an IAM `AccessDenied`.** Such a run was previously stored with an empty configuration and every document failed minutes later in OCR with `not authorized to perform: s3:ListBucket`, because S3 reports a missing key as 403 when the reader lacks list permission. The runner now rejects the run before anything is queued, and every revision-body reader has the scoped `s3:ListBucket`, so a missing body is a 404. See [Configuration Profiles](docs/configuration-profiles.md#selecting-a-revision-for-processing-or-testing). ([#878](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/878))

- **Test Studio file patterns match nested folders and treat every non-wildcard character literally.** `folder/**/*.pdf` matched nothing nested, and a `.`, `(`, `+` or `[` in a name mis-matched or errored, including in rule validation's section-file lookup. Patterns now follow ordinary glob rules (`**` any depth, `*` one level, `?` one character), case-insensitively; the folder path itself must be typed in its exact case. ([#808](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/808))

- **Legacy `.doc` files fail with an actionable message instead of processing as a blank page, and are no longer offered for upload.** The Word converter reads only the modern `.docx` container, so a `.doc` converted to one empty page and completed extraction with no content. It is now detected by its bytes (a `.doc` renamed `.docx` too) and the document fails naming the remedy: re-save as `.docx`. (#829)

- **Legacy `.xls` workbooks produce readable pages instead of one blank "Error reading Excel file" page, and the PII Anonymization hook redacts them instead of failing the run.** The `.xls` reader engine was never declared as a dependency, and the PII hook handed `.xls` to a reader that cannot open the legacy format; the workbook is now rewritten as `.xlsx` before redaction. (#799, #824)
...
- **Guidance: simple mode is measured complete to ~400 rows / ~10 pages per document, not ~1,600.** The earlier figure relied on the #726 over-splitting defect, which cut large statements into 6–18 sections and so bounded each extraction call; with the defect fixed, a single 800-row / 17-page statement in simple mode returned 43 rows while reporting `COMPLETED` with no processing issue, and 25+ pages fail with *Input is too long for requested model*. Advanced mode (which shards) is the recommendation beyond that size, and `minItems` is the one signal that turns the simple-mode truncation into a warning. Also measured: the shipped confidence batch sizing pays for one truncated Nova Lite call per 100-row section — a batch of 8 removed every truncation at −29% assessment cost and −38 s per document with identical results (`docs/benchmarking/config-guidance.md` §3 and §7; scored data under `benchmarks/results/v0.6.8/`).
...
- **Auto Optimizer documentation: architecture diagram, demo videos, and the EMNLP paper.** The README now carries the IDP Auto Optimizer diagram and a link to the published paper, and the [auto-optimizer guide](docs/extensions/auto-optimizer.md) has demo video walkthroughs.
...
- **Granular assessment is retired and its service deleted.** The `GranularAssessmentService` and the `extraction.confidence.granular` config field no longer exist. Large-list assessment is now handled by standalone batching (`extraction.confidence.list_batch_size`, default 25) with a bounded missing-row retry to reach 100% per-cell coverage — substantially cheaper than granular on a 120-row bank statement with equal accuracy. (No percentage is quoted deliberately; see [#966](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/966) and the reason recorded in the retirement note.) **Migration is a no-op:** leftover `granular.*` keys still validate and are ignored. See [Granular Assessment Retirement](docs/migration-granular-retirement.md).
...
- **Custom domain support for ALB-hosted private deployments** — new optional `CustomDomainUrl` stack parameter (under the *ALB Hosting* parameter group) lets the Web UI be reached through a customer-owned DNS alias, an Okta/SAML/OIDC SSO domain, or any other origin in front of the ALB **without losing access via the original ALB URL**. When set:
  ...
  Validated by `AllowedPattern` at template-parse time (lowercase-host `https://` URL, no path, no trailing slash). Default `""` resolves every conditional to `AWS::NoValue` so the rendered template is byte-identical to before for existing deployments — fully backward compatible. **Customer-side requirements** (outside the template): point the custom-domain DNS at the internal ALB, attach an ACM cert that covers the custom domain to the ALB listener (or use SNI), and — for federated IdPs — confirm Cognito's `oauth2/idpresponse` URL is registered as a redirect URI in the IdP. No Okta-side changes are needed for the custom domain itself; Okta only ever sees the Cognito `idpresponse` URL in this flow. See [the ALB hosting guide as it shipped in 0.5.15](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/blob/v0.5.15/docs/alb-hosting.md#custom-domain-in-front-of-alb).
