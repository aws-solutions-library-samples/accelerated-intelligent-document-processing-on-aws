...
- **Extensions UI: the subscription banner is one line, names AWS Marketplace, and links to where a subscription is actually managed.** "Subscription active" + a second row reading `Source: marketplace-live` becomes **Subscription active (AWS Marketplace)**, with a **Manage subscription** button pointing at the buyer's [AWS Marketplace subscriptions console](https://console.aws.amazon.com/marketplace/home#/subscriptions) — the only place a real subscription can be viewed, changed or cancelled, since the in-UI Cancel button drives the simulator's admin API and is offered for simulated grants only. The friendly label is applied to the *verified* banner only: the unverified banner still names the raw source (`simulated`, `advisory`, `auto`) verbatim, because there the exact mode is the whole point, and an unrecognised source still passes through raw rather than being given an invented name. The **Extensions** info panel drops its "Preview / no Marketplace extensions are available yet" paragraph and the three `(future)` qualifiers — Marketplace extensions ship now — and its lifecycle list is reordered Subscribe → Install → Ready → Update to match what an admin actually does, with a note that Marketplace extensions are published per Region.
