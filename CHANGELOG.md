...
- **Processed documents can be added to a test set, including documents that have no ground truth yet** ([#812](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/812)). A test set's **Add documents** menu, on the Test Sets table and on the set's own page, has a new **From processed documents** source: pick completed Production documents from a list and they are copied into the set with any ground truth already saved for them. A document with none is added **unlabeled**, the dialog names those documents before you confirm, and it warns when that would make a labeled set read as unlabeled, so a good example can join a set as soon as it finishes processing and be draft-labeled afterwards. This was the last open item of #812: the bucket-pattern import still excludes documents without a baseline and stays Admin-only, because a pattern match is a search over the whole bucket, while a pick from the caller's own document list is not, so the new operation (`addDocumentsToTestSetByKey`) is open to Authors as well. See [Adding Documents to Existing Test Sets](docs/test-studio.md#adding-documents-to-existing-test-sets).
