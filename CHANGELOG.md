...
- **A model that caches implicitly is no longer told it cannot cache.** The prompt-cache report's `no-cache-point` state said *"the prompt has no `<<CACHEPOINT>>` marker or the model does not support prompt caching"* — in the section report, the Web UI, and the docs. The second clause is false for six selectable models: OpenAI GPT-6 Astra caches **implicitly** on Converse (measured: a repeated 2,707-token prefix billed `inputTokens=2` / `cacheReadInputTokens=2707`) and in fact *rejects* an explicit cache point, while GPT-5.4/5.5 cache automatically and GPT-5.6 Sol/Terra/Luna get a translated Responses-API breakpoint. A user selecting any of them saw, on a cold first document, a message implying caching was unavailable — and the obvious reaction to that reading (add a `<<CACHEPOINT>>` marker) is exactly wrong for Astra. The six states are unchanged, so nothing in Athena or the cost table moves; only the wording, which now distinguishes "we sent no cachePoint" from "this model cannot cache" and names implicit caching where it applies. Warm runs were already correct: measured reads win, so the state becomes `caching` once a prefix repeats. See [docs/configuration.md](docs/configuration.md#reading-cache-efficiency-back-per-phase-and-per-class).
