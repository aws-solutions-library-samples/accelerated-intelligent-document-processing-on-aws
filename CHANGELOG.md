...
- **The five files an outside contributor or a security reporter looks for now exist: [SECURITY.md](SECURITY.md), [GOVERNANCE.md](GOVERNANCE.md), [MAINTAINERS.md](MAINTAINERS.md), [ROADMAP.md](ROADMAP.md) and [.github/CODEOWNERS](.github/CODEOWNERS).** The repository had none of them, so a reporter with a live vulnerability had no repo-local statement of where to send it and defaulted to a public issue, and review requests routed to the primary maintainer even in the three subsystems majority-written by someone else (the agent framework, business rule validation and the workshop content — the Python SDK is co-owned but still majority-written by the maintainer). `SECURITY.md` states the channel unambiguously — exploitable findings to AWS Security via [HackerOne](https://hackerone.com/aws_vdp) or aws-security@amazon.com, hardening suggestions to GitHub issues — plus which versions are supported (the latest release only; no backports) and a pointer to `security/` for the threat model and curated test results; it complements rather than replaces the organization-wide policy. `CODEOWNERS` ownership was measured from per-author file revisions — with the exact reproduction command recorded in both files, since a published number nobody can re-derive is not evidence — and every handle resolved through the GitHub API, so five paths (two subsystems plus `publish.py`, across two engineers) whose principal author's commits are attached to no GitHub account are deliberately left with the default owner and listed as needing the correct usernames rather than guessed at. ⚠️ **CODEOWNERS routing only reaches a user who holds write access to the repository**, and GitHub ignores an entry for anyone with read access silently; `MAINTAINERS.md` records that dependency with a dated permission table and both check commands — per-handle `collaborators/{handle}/permission`, and GitHub's own authoritative `codeowners/errors`, which reports 8 findings today — because three of the five named owners currently hold read access only, which leaves six of the nine specific rules routing to the default owner alone. `GOVERNANCE.md` describes the process actually followed (single maintainer with subsystem owners, `develop` as the integration branch, release-when-ready every one to two weeks, the pre-release live-stack battery) and is explicit about the questions that have no answer today — including that `develop` carries no branch protection and no branch-scoped ruleset, so neither a required status check nor a review approval is enforced and nothing blocks a merge ([#933](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/933)), and that the strings to enter as required status checks are the three **job** display names rather than the workflow names. `ROADMAP.md` carries themes and non-goals, no dates. A new offline gate, `scripts/tests/test_codeowners.py`, keeps all five files from rotting: every CODEOWNERS pattern must still match a tracked path (this repository has already deleted `patterns/pattern-1/` through `pattern-3/`), the handle sets in CODEOWNERS and `MAINTAINERS.md` must agree, and every relative link and heading anchor in the five files must resolve — none of which the docs-site build can cover, since it reads only `docs/`. ([#936](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/936))

### Changed

- **[Well-Architected](docs/well-architected.md) is now a review template you fill in, and its six-pillar assessment states what the solution actually implements.** The page contained no numbers and six untrackable "Consider implementing…" bullets, two of which were already done (an integration suite exists; X-Ray is instrumented on 19 Lambdas), and it claimed WAF protects an AppSync API when there are zero `AWS::AppSync` resources left anywhere — the WebACL is an optional REGIONAL IP allow-list on the API Gateway REST stage, created only when `WAFAllowedIPv4Ranges` leaves its `0.0.0.0/0` default. Each pillar now names the mechanism a customer can go and inspect (the twelve alarms and their thresholds, the two dashboards, `CustomerManagedEncryptionKey` and the 23 TLS-only bucket and queue policies, `PermissionsBoundaryArn` threading, the 24 Step Functions retry blocks over the Bedrock client's 7, the DLQ `maxReceiveCount` values, the `ConcurrencyTable` conditional-update admission cap, the metering ledger), and the bullets are replaced by a per-pillar checklist with **empty** comment/owner/date columns — the answers belong to the customer's deployment, not to this project. A new section splits what ships active, what ships off by default (`CircuitBreakerEnabled`, `BedrockGuardrailId`, `DeployInVPC`), and what has no resource at all and so is entirely the customer's: confirming what is subscribed to `AlertsTopic`, an AWS Budget, MFA on the user pool, CloudTrail, WAF managed rules, and reviewing the optional deployment service role. Corrections of record: point-in-time recovery covers 10 of the 12 tables a default deployment creates, not all (`ConcurrencyTable` is a reconciled ephemeral counter and `ChatDocumentSessionsTable` holds TTL'd chat-ownership records), `LogLevel` already defaults to `WARN`, and Guardrails are bring-your-own rather than built in. The Security pillar now also states the API authorization posture as measured rather than as intended: of the 118 operations in `scripts/api_rbac_expectations.yaml`, 90 are group-restricted and 2 are IAM-only, but 26 are `groups: ANY` and 17 of those carry no ownership or configuration-version check, so any authenticated pool user can read document content — plus the `AllowedSignUpEmailDomain` parameter that decides who can get an account at all, and the 123 `Resource: "*"` IAM statements. ([#937](https://github.com/aws-solutions-library-samples/accelerated-intelligent-document-processing-on-aws/issues/937))
