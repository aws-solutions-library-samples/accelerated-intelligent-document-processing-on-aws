...
- **A ready-made Seller Entitlement Service for paid extensions** (`feature-platform/seller-entitlement-service/`). A Feature Platform extension deploys into the **buyer's** account and the buyer owns its Lambda, environment and code — so software running there cannot enforce its own licence, and the host's entitlement check is advisory by construction. Enforcement requires the *seller* to hold something the buyer needs at runtime. This is a self-contained SAM stack a seller deploys once in their own account: an extension POSTs `{"productId": "prod-…"}` and receives a short-lived, account-bound token signed by a KMS **asymmetric** key (so an extension can verify with the public key without being able to mint), carrying a `kid` so the key can be rotated. Authentication is API Gateway **`AWS_IAM`** with a resource policy admitting any AWS principal, and the buyer account is read **only** from `requestContext.identity`, never the request body — so the seller needs no advance knowledge of buyers and buyers need no seller-issued credentials. Deliberate asymmetry with the host: this service **fails closed**, while the extension is required to hold a grace period on the last-known-good token so a seller-side outage never locks out a paying customer. Entitlement is checked with `SearchAgreements` `PartyType=Proposer`, **verified against a live listing across every caller/subscription combination** (an unsubscribed buyer returns an empty list rather than an error; `GetEntitlements` returns an empty list even from the seller account, confirming a usage-based SaaS listing has no entitlement records anywhere). A DynamoDB **activation roster** (`Retain` + PITR, `ProductIndex` GSI, fail-open writes) plus an `ActivationAttempt` EMF metric make "who has activated what?" answerable, read by `idp-feature-cli seller-service activations` with filters; `idp-feature-cli seller-service preflight|deploy` deploys it and refuses unless the credentials genuinely own the product (`marketplace-catalog:ListEntities`), and the template carries a `ServiceVersion` output stamped by `make version`. Ships with a threat model (`SELL.T01–T10`, the first component in this repo that runs in the *seller's* account), 16 mutation-tested static template assertions, a dynamic test against the deployed stage, and a ~60-payload hostile-input corpus asserting the handler never 5xxs and never reflects attacker input. The README's threat model does not overclaim: it stops parameter-flipping, simulator-pointing and direct-template deploys, not a customer who patches the extension — so it is deterrence plus a reconciliation trail. `tests/teardown_test_stack.sh` makes a test deployment disposable by cleaning up the two `Retain`ed resources.
